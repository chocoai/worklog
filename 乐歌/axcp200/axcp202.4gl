...
#該程式非freestyle程式!
{<section id="axcp202.description" type="s" >}
#應用 a00 樣板自動產生(Version:3)
#+ Standard Version.....: SD版次:0028(2017-04-17 14:50:21), PR版次:0028(2018-01-16 14:43:11)
#+ Customerized Version.: SD版次:0000(1900-01-01 00:00:00), PR版次:0000(1900-01-01 00:00:00)
#+ Build......: 000182
#+ Filename...: axcp202
#+ Description: 期末投入工時統計和分攤作業
#+ Creator....: 02114(2014-05-20 09:41:52)
#+ Modifier...: 02294 -SD/PR- 07423
 
{</section>}
 
{<section id="axcp202.global" type="s" >}
...
#170223-00004#3   2017/04/13 By lixiang  當 閒置費用金額大於0時，產生一筆工單單號為 'OH+SUB'的資料，記錄閒置費用
#170508-00029#1   2017/05/09 By 02111     QBE 的 CONSTRUCT 變數 lc_param.wc 改為 g_master.wc，後續 DELETE 語法條件才會正確。
#170509-00035#1   2017/05/17 By 09042     axc-00304将年度期别2014.10完整报出来
#170519-00062#1   2017/05/23 By zhaoqya   当axct200未取到成本中心时，xcbi001和xcbj004值给空格
#170617-00503#1   2017/06/18 By 02111     調整以下分攤公式執行的邏輯
#                                         6:標準工時*分攤權數 > 2:標準工時
#                                         7:實際工時*分攤權數 > 1:實際工時
#                                         8:標準機時*分攤權數 > 3:標準機時
#                                         9:實際機時*分攤權數 > 5:實際機時
#170618-00382#1   2017/06/28 By lixiang  計算前先檢查axct200中有沒有已經報廢的資料，有請出現提示訊息，跟哪些工單有問題(xcbi002 = sfaadocno and (sfaastus <> ‘X’ OR sfaastus <> ‘N’ ))，要把單子修正後才能計算
#170725-00076#4   2017/08/16 By 10046    "工單分攤數"取消小數點設置；xcbk202分攤金額(本位幣一)，最大值抓取改為絕對值
#170808-00032#1   2017/08/18 By 02295    尾差处理时,将差异放入金额最大的一笔中,在抓取最大一笔时,要用ABS()来判断是否是最大
#170818-00021#1   2017/08/18 By 02295    启用成本次要素时,抓取xcbj_t的sql中含有公共字段,但是xcdr中没有,这样导致在将xcbj替换成xcdr候的sql报错
#170725-00076#7   2017/08/31 By 10046    修改呼叫取位FUNCTION:s_curr_round，傳入的最後一個參數錯誤
#170915-00057#2   2017/09/15 BY 00566    原先的識別字OH+SUB 改為 OH-FIXED
#170828-00014#1   2017/09/21 By 05423    执行前判断日期是否小于等于成本关账日期，若是，则不可执行。
#180105-00029#1   2017/01/15 By 07423    效能調整,將金額取位s_curr_round改用s_num_round
#end add-point
...
{<section id="axcp202.main" type="s" >}
MAIN
...
{<section id="axcp202.init" type="s" >}
#+ 初始化作業
...
{<section id="axcp202.ui_dialog" type="s" >}
#+ 選單功能實際執行處
PRIVATE FUNCTION axcp202_ui_dialog()
   ...
   WHILE TRUE
      ...
      DIALOG ATTRIBUTES(UNBUFFERED,FIELD ORDER FORM)
         ...
         #CONSTRUCT BY NAME lc_param.wc ON xcbkcomp,xcbkld #170508-00029#1 mark
         CONSTRUCT BY NAME g_master.wc ON xcbkcomp,xcbkld  #170508-00029#1 add
            BEFORE CONSTRUCT    
...
{<section id="axcp202.transfer_argv" type="s" >}
...
{<section id="axcp202.process" type="s" >}
...
{<section id="axcp202.get_buffer" type="s" >}
...
{<section id="axcp202.msgcentre_notify" type="s" >}
...
{<section id="axcp202.other_function" readonly="Y" type="s" >}
...
END RECORD
   ...
   DEFINE l_xcbi001      LIKE xcbi_t.xcbi001    #170519-00062#1 add    
   DEFINE l_xcbjcomp     LIKE xcbj_t.xcbjcomp   #170618-00382#1 add
   DEFINE l_flag1        LIKE type_t.chr1       #170618-00382#1 add  
   #180105-00029#1 add --(S)--
   DEFINE l_ooaj006      LIKE ooaj_t.ooaj006    #成本單價小數位數   
   DEFINE l_ooaj007      LIKE ooaj_t.ooaj007    #成本金額小數位數   
   DEFINE l_roundtype    LIKE ooaa_t.ooaa002    #進位方式
   DEFINE l_str          STRING
   #180105-00029#1 add --(E)--
   ...
   #180105-00029#1 add --(S)--
   #取企业参数设置-金额舍入类型
   LET l_roundtype = cl_get_para(g_enterprise,'','E-COM-0006')
   LET l_str = l_roundtype
   LET l_roundtype = l_str.trim()
   IF l_roundtype NOT MATCHES '[1234]' OR cl_null(l_roundtype) THEN
      #进位方式不正确
      INITIALIZE g_errparam TO NULL
      LET g_errparam.code = 'sub-00293'
      LET g_errparam.extend = l_roundtype
      LET g_errparam.popup = TRUE
      CALL cl_err()
      LET g_success = 'N'
      RETURN
   ...
   #170618-00382#1--add---s
   LET g_sql = "SELECT DISTINCT xcbjcomp FROM xcbj_t ", 
               ...
               "   AND ",g_wc_xcbkld,
               "   AND ",g_wc_xcbkcomp
   PREPARE axcp202_xcbj_pre5 FROM g_sql
   DECLARE axcp202_xcbj_cur5 CURSOR FOR axcp202_xcbj_pre5  
   
   #axcp202: 計算前先檢查axct200中有沒有已經報廢的資料，有請出現提示訊息，跟哪些工單有問題(xcbi002 = sfaadocno and (sfaastus <> ‘X’ OR sfaastus <> ‘N’ ))，要把單子修正後才能計算
   LET g_sql = " SELECT DISTINCT xcbi002 ", 
               "   FROM xcbh_t,xcbi_t ,sfaa_t ",
               "  WHERE xcbhent   = xcbient AND sfaaent = xcbient AND xcbi002 = sfaadocno ",
               ...
               "    AND (sfaastus = 'X' OR sfaastus = 'N')  "             
               
   PREPARE axcp202_chk_xcbi_pre FROM g_sql
   DECLARE axcp202_chk_xcbi_cur CURSOR FOR axcp202_chk_xcbi_pre 
   
   LET l_flag1 = 'N'
   
   CALL cl_err_collect_init()
   FOREACH axcp202_xcbj_cur5 INTO l_xcbjcomp
      FOREACH axcp202_chk_xcbi_cur USING l_xcbjcomp INTO l_xcbi002 
         INITIALIZE g_errparam TO NULL
         LET g_errparam.extend = l_xcbi002
         LET g_errparam.code   = 'axc-00846'
         LET g_errparam.popup  = TRUE 
         CALL cl_err()
         LET l_flag1 = 'Y'
         CONTINUE FOREACH             
...
#   LET g_sql = "SELECT * FROM xcbj_t ",  #161124-00048#21 mark
   ...
   IF cl_get_para(g_enterprise,g_site,'S-FIN-6015') = 'Y' THEN   #启用成本次要素
      #170818-00021#1---add---s
      LET g_sql = "SELECT xcbjent,xcbjcomp,xcbjld,xcbj001,xcbj002,xcbj003,xcbj004,xcbj005,", 
                  "       xcbj006,xcbj010,xcbj011,xcbj020,xcbj021,xcbj101,xcbj102,xcbj103,",
                  "       xcbj104,xcbj105,xcbj111,xcbj112,xcbj113,xcbj114,xcbj115,xcbj121,",
                  "       xcbj122,xcbj123,xcbj124,xcbj125 FROM xcbj_t ", 
                  " WHERE xcbjent = '",g_enterprise,"'",
                  "   AND xcbj001 = '",lc_param.xcbk001,"'",
                  "   AND xcbj002 = '",lc_param.xcbk002,"'",
                  "   AND xcbj003 = '",lc_param.xcbk003,"'", 
   ...
   FOREACH axcp202_xcbj_cur INTO l_xcbj.*
      ...
      IF NOT cl_null(g_errno) THEN
         #ET g_errparam.extend = lc_param.xcbk002    #170509-00035#1 mark
         LET g_errparam.extend = lc_param.xcbk002||'.'||lc_param.xcbk003   #170509-00035#1 add
      ...
      #180105-00029#1 add --(S)--
      #取得成本單價與金額小數位數
      CALL axcp202_get_ooaj(l_xcbj.xcbjcomp,l_xcbj.xcbjld) RETURNING l_success,l_ooaj006,l_ooaj007
      ...
      #170519-00062#1 add -s
      SELECT xcbi001 INTO l_xcbi001 
        FROM xcbh_t,xcbi_t
      WHERE xcbient = g_enterprise
        AND xcbient = xcbhent 
        AND xcbidocno = xcbhdocno 
        ...
        AND TO_CHAR(xcbh001,'MM') = lc_param.xcbk003
      ORDER BY xcbi002 
      #如果xcbi001成本中心为空 给xcbj004空格
      IF cl_null(l_xcbi001) OR l_xcbi001 = ' ' THEN
         LET l_xcbj.xcbj004 = ' '
         LET l_xcbi001 = ' '
      END IF 
      #170519-00062#1 add -e
      
      #實際工時
      #IF l_xcbj.xcbj006 = '1' THEN #170617-00503#1 mark
      IF l_xcbj.xcbj006 = '1' OR l_xcbj.xcbj006 = '7' THEN #170617-00503#1 add
         ...
         FOREACH xcbi_cur_1 INTO l_xcbi002,l_sum_xcbi201
            ...
            #fengmy150731-----end           
           #CALL s_ld_sel_glaa(l_xcbk.xcbkld,'glaa001') RETURNING l_success,l_curr   #180105-00029#1 mark
           #IF l_success THEN                                                        #180105-00029#1 mark
              #LET l_xcbk.xcbk100 = s_curr_round(l_xcbk.xcbkcomp,l_curr,l_xcbk.xcbk100,'1')    
              #170725-00076#7  mark --s
              ...
              #170725-00076#7  mark --e
           #180105-00029#1 mark --(S)-- 
           #  #170725-00076#7  add  --s
           #   LET l_xcbk.xcbk101 = s_curr_round(l_xcbk.xcbkcomp,l_curr,l_xcbk.xcbk101,'4')     
           #   LET l_xcbk.xcbk111 = s_curr_round(l_xcbk.xcbkcomp,l_curr,l_xcbk.xcbk111,'4')     
           #   LET l_xcbk.xcbk121 = s_curr_round(l_xcbk.xcbkcomp,l_curr,l_xcbk.xcbk121,'4')     
           #   LET l_xcbk.xcbk202 = s_curr_round(l_xcbk.xcbkcomp,l_curr,l_xcbk.xcbk202,'5') 
           #   LET l_xcbk.xcbk212 = s_curr_round(l_xcbk.xcbkcomp,l_curr,l_xcbk.xcbk212,'5') 
           #   LET l_xcbk.xcbk222 = s_curr_round(l_xcbk.xcbkcomp,l_curr,l_xcbk.xcbk222,'5')
           #  #170725-00076#7   add --e
           #ELSE
           #   CONTINUE FOREACH
           #END IF
           #180105-00029#1 mark --(E)--
            #180105-00029#1 add --(S)--
            LET l_xcbk.xcbk101 = s_num_round(l_roundtype,l_xcbk.xcbk101,l_ooaj006)     
            LET l_xcbk.xcbk111 = s_num_round(l_roundtype,l_xcbk.xcbk111,l_ooaj006)     
            LET l_xcbk.xcbk121 = s_num_round(l_roundtype,l_xcbk.xcbk121,l_ooaj006)     
            LET l_xcbk.xcbk202 = s_num_round(l_roundtype,l_xcbk.xcbk202,l_ooaj007) 
            LET l_xcbk.xcbk212 = s_num_round(l_roundtype,l_xcbk.xcbk212,l_ooaj007) 
            LET l_xcbk.xcbk222 = s_num_round(l_roundtype,l_xcbk.xcbk222,l_ooaj007)
            #180105-00029#1 add --(E)--            
#            INSERT INTO xcbk_t VALUES(l_xcbk.*)  #161124-00048#21 mark
            ...
            #IF l_xcbk.xcbk202 > l_max_amt THEN   #170808-00032#1 mark
            IF s_math_abs(l_xcbk.xcbk202) > s_math_abs(l_max_amt) THEN #170808-00032#1 add
      ...
      #標準工時
      #IF l_xcbj.xcbj006 = '2' THEN #170617-00503#1 mark
      IF l_xcbj.xcbj006 = '2' OR l_xcbj.xcbj006 = '6' THEN #170617-00503#1 add      
         ...
         FOREACH xcbi_cur_2 INTO l_xcbi002,l_sum_xcbi203
           ...
           #CALL s_ld_sel_glaa(l_xcbk.xcbkld,'glaa001') RETURNING l_success,l_curr              #180105-00029#1 mark 
           #IF l_success THEN                                                                   #180105-00029#1 mark 
              #LET l_xcbk.xcbk100 = s_curr_round(l_xcbk.xcbkcomp,l_curr,l_xcbk.xcbk100,'1')     #170725-00076#4  mark 
              #170725-00076#7  mark --s
              #LET l_xcbk.xcbk101 = s_curr_round(l_xcbk.xcbkcomp,l_curr,l_xcbk.xcbk101,'1')     #liuym 20150811
              #LET l_xcbk.xcbk111 = s_curr_round(l_xcbk.xcbkcomp,l_curr,l_xcbk.xcbk111,'1')     #liuym 20150811
              #LET l_xcbk.xcbk121 = s_curr_round(l_xcbk.xcbkcomp,l_curr,l_xcbk.xcbk121,'1')     #liuym 20150811               
...
#            INSERT INTO xcbk_t VALUES(l_xcbk.*)  #161124-00048#21 mark
      ...
      #標準機時
      #IF l_xcbj.xcbj006 = '3' THEN #170617-00503#1 mark
      IF l_xcbj.xcbj006 = '3' OR l_xcbj.xcbj006 = '8' THEN #170617-00503#1 mark      
...
#            INSERT INTO xcbk_t VALUES(l_xcbk.*)  #161124-00048#21 mark
      ...
      #實際機時
      #IF l_xcbj.xcbj006 = '5' THEN #170617-00503#1 mark
      IF l_xcbj.xcbj006 = '5' OR l_xcbj.xcbj006 = '9' THEN #170617-00503#1 add
...
#尾差处理的范围是同一分摊方式不同工单
      IF l_xcbk.xcbk006 IS NOT NULL THEN   #没有工单，说明前面没有抓到xcbi资料，不该有尾差调整
         ...
         LET l_sum_xcbk212 = 0
         LET l_sum_xcbk222 = 0
         #170725-00076#4  add  --s
         SELECT MAX(ABS(xcbk202))INTO l_max_amt
           FROM xcbk_t
          WHERE xcbkent = g_enterprise
        ...
        #180105-00029#1 mark --(S)--   
        #  #170725-00076#7   add --s
        #   LET l_xcbk.xcbk101 = s_curr_round(l_xcbk.xcbkcomp,l_curr,l_xcbk.xcbk101,'4')     #liuym 20150811
        #   LET l_xcbk.xcbk111 = s_curr_round(l_xcbk.xcbkcomp,l_curr,l_xcbk.xcbk111,'4')     #liuym 20150811
        #   LET l_xcbk.xcbk121 = s_curr_round(l_xcbk.xcbkcomp,l_curr,l_xcbk.xcbk121,'4')     #liuym 20150811            
        #   LET l_xcbk.xcbk202 = s_curr_round(l_xcbk.xcbkcomp,l_curr,l_sum_xcbk202,'5') 
        #   LET l_xcbk.xcbk212 = s_curr_round(l_xcbk.xcbkcomp,l_curr,l_sum_xcbk212,'5') 
        #   LET l_xcbk.xcbk222 = s_curr_round(l_xcbk.xcbkcomp,l_curr,l_sum_xcbk222,'5')
        #   LET l_xcbj.xcbj103 = s_curr_round(l_xcbk.xcbkcomp,l_curr,l_xcbj.xcbj103,'5') 
        #   LET l_xcbj.xcbj113 = s_curr_round(l_xcbk.xcbkcomp,l_curr,l_xcbj.xcbj113,'5') 
        #   LET l_xcbj.xcbj123 = s_curr_round(l_xcbk.xcbkcomp,l_curr,l_xcbj.xcbj123,'5')
        #  #170725-00076#7   add --e
        #
        #ELSE
        #   CONTINUE FOREACH
        ...
        #180105-00029#1 mark --(E)-- 
         ...
         LET l_xcbk.xcbk111 = s_num_round(l_roundtype,l_xcbk.xcbk111,l_ooaj006)     
         LET l_xcbk.xcbk121 = s_num_round(l_roundtype,l_xcbk.xcbk121,l_ooaj006)                 
         LET l_xcbk.xcbk202 = s_num_round(l_roundtype,l_sum_xcbk202,l_ooaj007) 
         LET l_xcbk.xcbk212 = s_num_round(l_roundtype,l_sum_xcbk212,l_ooaj007) 
         LET l_xcbk.xcbk222 = s_num_round(l_roundtype,l_sum_xcbk222,l_ooaj007)
         LET l_xcbj.xcbj103 = s_num_round(l_roundtype,l_xcbj.xcbj103,l_ooaj007) 
         LET l_xcbj.xcbj113 = s_num_round(l_roundtype,l_xcbj.xcbj113,l_ooaj007) 
         LET l_xcbj.xcbj123 = s_num_round(l_roundtype,l_xcbj.xcbj123,l_ooaj007)
         #180105-00029#1 add --(E)--         
         ...
         UPDATE xcbk_t SET xcbk202 = xcbk202 + l_xcbj.xcbj103 - l_sum_xcbk202,
          ...
          WHERE xcbkent = g_enterprise
            ...
            AND xcbk005 = l_xcbj.xcbj005
            AND xcbk007 = l_xcbj.xcbj006
            #AND xcbk202 = l_max_amt  #170725-00076#4  mark
            AND ABS(xcbk202) = l_max_amt  #170725-00076#4  add
            AND rownum  = 1
      END IF
#处理尾差结束
   END FOREACH 
   ...
   #170223-00004#3--s
   #當 閒置費用金額大於0時，產生一筆工單單號為 'OH-FIXED'的資料，記錄閒置費用 (依年度、期別、成本中心、費用分類、分攤方式)
   LET g_sql = "SELECT xcbjent,xcbjcomp,xcbjld,xcbj001,xcbj002,xcbj003,xcbj004,xcbj005,", 
               "       xcbj006,SUM(xcbj104),SUM(xcbj114),SUM(xcbj124)",
               "     FROM xcbj_t ", 
               " WHERE xcbjent = '",g_enterprise,"'",
               "   AND xcbj001 = '",lc_param.xcbk001,"'",
               "   AND xcbj002 = '",lc_param.xcbk002,"'",
               ...
               "   AND ",g_wc_xcbkld,
               "   AND ",g_wc_xcbkcomp,
               "   AND (xcbj104 <> 0 OR xcbj114 <> 0 OR xcbj124 <> 0 ) ",
               "  GROUP BY xcbjent,xcbjcomp,xcbjld,xcbj001,xcbj002,xcbj003,xcbj004,xcbj005,xcbj006 "
   PREPARE axcp202_xcbj_pre3 FROM g_sql
   DECLARE axcp202_xcbj_cur3 CURSOR FOR axcp202_xcbj_pre3

   IF cl_get_para(g_enterprise,g_site,'S-FIN-6015') = 'Y' THEN   #启用成本次要素
      CALL axcp202_replace_sql()
      PREPARE axcp202_xcdr_pre3 FROM g_sql
      DECLARE axcp202_xcdr_cur3 CURSOR FOR axcp202_xcdr_pre3
   END IF
   
   FOREACH axcp202_xcbj_cur3 INTO l_xcbj.xcbjent,l_xcbj.xcbjcomp,l_xcbj.xcbjld,l_xcbj.xcbj001,l_xcbj.xcbj002,l_xcbj.xcbj003,
                                  l_xcbj.xcbj004,l_xcbj.xcbj005,l_xcbj.xcbj006,l_xcbj.xcbj104,l_xcbj.xcbj114,l_xcbj.xcbj124
      IF SQLCA.sqlcode THEN
         INITIALIZE g_errparam TO NULL
         LET g_errparam.code = SQLCA.sqlcode
         LET g_errparam.extend = "FOREACH:"
      ...
      CALL axcp202_date_chk(l_xcbj.xcbjcomp,lc_param.xcbk002,lc_param.xcbk003)
      IF NOT cl_null(g_errno) THEN
         #LET g_errparam.extend = lc_param.xcbk002  #170509-00035#1 mark
         LET g_errparam.extend = lc_param.xcbk002||'.'||lc_param.xcbk003   #170509-00035#1 add
         LET g_errparam.code   = g_errno
         LET g_errparam.popup  = TRUE 
         CALL cl_err()
      ...
      LET l_xcbk.xcbk004 = l_xcbj.xcbj004
      LET l_xcbk.xcbk005 = l_xcbj.xcbj005
      LET l_xcbk.xcbk006 = 'OH-FIXED'  #'OH+SUB'         #工單單號  #170915-00057 MODI  
      LET l_xcbk.xcbk007 = l_xcbj.xcbj006
      LET l_xcbk.xcbk101 = l_xcbj.xcbj104   #單位成本
      LET l_xcbk.xcbk111 = l_xcbj.xcbj114   #單位成本
      LET l_xcbk.xcbk121 = l_xcbj.xcbj124   #單位成本
      
      LET l_xcbk.xcbk202 = l_xcbj.xcbj104
      LET l_xcbk.xcbk212 = l_xcbj.xcbj114
      LET l_xcbk.xcbk222 = l_xcbj.xcbj124
      
      LET l_xcbk.xcbk100 = 0   #分攤數
     ...
     #IF l_success THEN                                                               #180105-00029#1 mark
        #LET l_xcbk.xcbk100 = s_curr_round(l_xcbk.xcbkcomp,l_curr,l_xcbk.xcbk100,'1') #170725-00076#4  mark
        #170725-00076#7  mark --s
        #LET l_xcbk.xcbk101 = s_curr_round(l_xcbk.xcbkcomp,l_curr,l_xcbk.xcbk101,'1')       
        #LET l_xcbk.xcbk111 = s_curr_round(l_xcbk.xcbkcomp,l_curr,l_xcbk.xcbk111,'1')     
        #LET l_xcbk.xcbk121 = s_curr_round(l_xcbk.xcbkcomp,l_curr,l_xcbk.xcbk121,'1')                 
        #LET l_xcbk.xcbk202 = s_curr_round(l_xcbk.xcbkcomp,l_curr,l_xcbk.xcbk202,'2') 
        #LET l_xcbk.xcbk212 = s_curr_round(l_xcbk.xcbkcomp,l_curr,l_xcbk.xcbk212,'2') 
        #LET l_xcbk.xcbk222 = s_curr_round(l_xcbk.xcbkcomp,l_curr,l_xcbk.xcbk222,'2')
        #170725-00076#7  mark --e
  ...
  #IF l_flag = 'N' OR l_xcbh_cnt = 0 THEN  #160913-00039#1 add
   ...
   IF cl_get_para(g_enterprise,g_site,'S-FIN-6015') = 'Y' THEN   #启用成本次要素 
      ...
      FOREACH axcp202_xcdr_cur INTO l_xcdr.*
         ...
         #180105-00029#1 add --(S)--
         #取得成本單價與金額小數位數
         CALL axcp202_get_ooaj(l_xcdr.xcdrcomp,l_xcdr.xcdrld) RETURNING l_success,l_ooaj006,l_ooaj007
         IF NOT l_success THEN
            CONTINUE FOREACH
         END IF           
         #180105-00029#1 add --(E)--
         ...
         IF l_xcdr.xcdr006 = '1' THEN 
            ...
            FOREACH xcbi_cur_11 INTO l_xcbi002,l_sum_xcbi201
               ...
               #fengmy150731-------e 
              #CALL s_ld_sel_glaa(l_xcdq.xcdqld,'glaa001') RETURNING l_success,l_curr          #180105-00029#1 mark
              #IF l_success THEN                                                               #180105-00029#1 mark
                 #LET l_xcdq.xcdq100 = s_curr_round(l_xcdq.xcdqcomp,l_curr,l_xcdq.xcdq100,'1') #170725-00076#4  mark
                 #170725-00076#7  mark --s
              ...
              #180105-00029#1 mark --(S)--                 
              #  #170725-00076#7   add --s
              #   LET l_xcdq.xcdq202 = s_curr_round(l_xcdq.xcdqcomp,l_curr,l_xcdq.xcdq202,'5') 
              #   LET l_xcdq.xcdq212 = s_curr_round(l_xcdq.xcdqcomp,l_curr,l_xcdq.xcdq212,'5') 
              #   LET l_xcdq.xcdq222 = s_curr_round(l_xcdq.xcdqcomp,l_curr,l_xcdq.xcdq222,'5')
              #  #170725-00076#7   add --e
              #ELSE
              #   CONTINUE FOREACH
              #END IF
              #180105-00029#1 mark --(E)--
               #180105-00029#1 add --(S)--
               LET l_xcdq.xcdq202 = s_num_round(l_roundtype,l_xcdq.xcdq202,l_ooaj007) 
               LET l_xcdq.xcdq212 = s_num_round(l_roundtype,l_xcdq.xcdq212,l_ooaj007) 
               LET l_xcdq.xcdq222 = s_num_round(l_roundtype,l_xcdq.xcdq222,l_ooaj007)
               #180105-00029#1 add --(E)--                
...
#               INSERT INTO xcdq_t VALUES(l_xcdq.*)  #161124-00048#21 mark
               ...
               #IF l_xcdq.xcdq202 > l_max_amt THEN   #170808-00032#1 mark
               IF s_math_abs(l_xcdq.xcdq202) > s_math_abs(l_max_amt) THEN #170808-00032#1 add               
...
#               INSERT INTO xcdq_t VALUES(l_xcdq.*)  #161124-00048#21 mark
   ...
   #尾差处理的范围是同一分摊方式不同工单
         IF l_xcdq.xcdq006 IS NOT NULL THEN    #没有工单，说明前面没有抓到xcbi资料，不该有尾差调整
           ...
           #180105-00029#1 mark --(S)--   
           #  #170725-00076#7  add --s              
           #   LET l_xcdq.xcdq202 = s_curr_round(l_xcdq.xcdqcomp,l_curr,l_sum_xcdq202,'5') 
           #   LET l_xcdq.xcdq212 = s_curr_round(l_xcdq.xcdqcomp,l_curr,l_sum_xcdq212,'5') 
           #   LET l_xcdq.xcdq222 = s_curr_round(l_xcdq.xcdqcomp,l_curr,l_sum_xcdq222,'5')
           #   LET l_xcdr.xcdr103 = s_curr_round(l_xcdq.xcdqcomp,l_curr,l_xcdr.xcdr103,'5') 
           #   LET l_xcdr.xcdr113 = s_curr_round(l_xcdq.xcdqcomp,l_curr,l_xcdr.xcdr113,'5') 
           #   LET l_xcdr.xcdr123 = s_curr_round(l_xcdq.xcdqcomp,l_curr,l_xcdr.xcdr123,'5')
           #  #170725-00076#7  add --e               
           #ELSE
           #   CONTINUE FOREACH
           #END IF
           #180105-00029#1 mark --(E)--
            #180105-00029#1 add --(S)--
            LET l_xcdq.xcdq202 = s_num_round(l_roundtype,l_sum_xcdq202,l_ooaj007) 
            LET l_xcdq.xcdq212 = s_num_round(l_roundtype,l_sum_xcdq212,l_ooaj007) 
            LET l_xcdq.xcdq222 = s_num_round(l_roundtype,l_sum_xcdq222,l_ooaj007)
            LET l_xcdr.xcdr103 = s_num_round(l_roundtype,l_xcdr.xcdr103,l_ooaj007) 
            LET l_xcdr.xcdr113 = s_num_round(l_roundtype,l_xcdr.xcdr113,l_ooaj007) 
            LET l_xcdr.xcdr123 = s_num_round(l_roundtype,l_xcdr.xcdr123,l_ooaj007)
            #180105-00029#1 add --(E)--            
   ...
   #处理尾差结束
      ...
      #170223-00004#3---s
      FOREACH axcp202_xcdr_cur3 INTO l_xcdr.xcdrent,l_xcdr.xcdrcomp,l_xcdr.xcdrld,l_xcdr.xcdr001,l_xcdr.xcdr002,l_xcdr.xcdr003,
                                     l_xcdr.xcdr004,l_xcdr.xcdr005,l_xcdr.xcdr006,l_xcdr.xcdr104,l_xcdr.xcdr114,l_xcdr.xcdr124
         IF SQLCA.sqlcode THEN
            INITIALIZE g_errparam TO NULL
            LET g_errparam.code = SQLCA.sqlcode
            LET g_errparam.extend = "FOREACH:"
         ...
         LET l_xcdq.xcdq004 = l_xcdr.xcdr004
         LET l_xcdq.xcdq005 = l_xcdr.xcdr005
         LET l_xcdq.xcdq006 = 'OH-FIXED'  #'OH+SUB'         #工單單號  #170915-00057 MODI  
         LET l_xcdq.xcdq007 = l_xcdr.xcdr006 
         LET l_xcdq.xcdq101 = l_xcdr.xcdr104   #單位成本
         LET l_xcdq.xcdq111 = l_xcdr.xcdr114   #單位成本
         LET l_xcdq.xcdq121 = l_xcdr.xcdr124   #單位成本
         
         LET l_xcdq.xcdq202 = l_xcdr.xcdr104
         LET l_xcdq.xcdq212 = l_xcdr.xcdr114
         LET l_xcdq.xcdq222 = l_xcdr.xcdr124
         
         LET l_xcdq.xcdq100 = 0   #分攤數
...
#      LET g_errno = 'axc-00303'     #170828-00014#1 mark
      LET g_errno = 'axr-00162'     #170828-00014#1 add
   END IF

   #IF l_yy <= p_xcbk002 AND l_pp > p_xcbk003 THEN   #161117-00031#1 mark
   IF l_yy = p_xcbk002 AND l_pp >= p_xcbk003 THEN   #161117-00031#1 #170828-00014#1 add =
#      LET g_errno = 'axc-00304'     #170828-00014#1 mark
      LET g_errno = 'axc-00883'     #170828-00014#1 add
   END IF
END FUNCTION
...
#   LET g_sql = "SELECT * FROM xcbj_t ",  #161124-00048#21 mark
   ...
   FOREACH axcp202_xcbj_cur2 INTO l_xcbj.*
      ...
      #實際工時
      #IF l_xcbj.xcbj006 = '1' THEN #170617-00503#1 mark
      IF l_xcbj.xcbj006 = '1' OR l_xcbj.xcbj006 = '7' THEN #170617-00503#1 mark      
         ...
         FOREACH xcbr_cur_1 INTO l_xcbr002,l_xcbr003,l_xcbr004,l_sum_xcbr201
           ...
           #CALL s_ld_sel_glaa(l_xcbs.xcbsld,'glaa001') RETURNING l_success,l_curr          #180105-00029#1 mark 
           #IF l_success THEN                                                               #180105-00029#1 mark
              #LET l_xcbs.xcbs100 = s_curr_round(l_xcbs.xcbscomp,l_curr,l_xcbs.xcbs100,'1') #170725-00076#4  mark
              #170725-00076#7  mark --s
           ...
           #180105-00029#1 mark --(S)--    
           #  #170725-00076#7   add --s
           #   LET l_xcbs.xcbs101 = s_curr_round(l_xcbs.xcbscomp,l_curr,l_xcbs.xcbs101,'4')    
           #   LET l_xcbs.xcbs111 = s_curr_round(l_xcbs.xcbscomp,l_curr,l_xcbs.xcbs111,'4')    
           #   LET l_xcbs.xcbs121 = s_curr_round(l_xcbs.xcbscomp,l_curr,l_xcbs.xcbs121,'4')    
           #   LET l_xcbs.xcbs202 = s_curr_round(l_xcbs.xcbscomp,l_curr,l_xcbs.xcbs202,'5') 
           #   LET l_xcbs.xcbs212 = s_curr_round(l_xcbs.xcbscomp,l_curr,l_xcbs.xcbs212,'5') 
           #   LET l_xcbs.xcbs222 = s_curr_round(l_xcbs.xcbscomp,l_curr,l_xcbs.xcbs222,'5')
           #  #170725-00076#7   add --e
           #ELSE
           #   CONTINUE FOREACH
           #END IF
           #180105-00029#1 mark --(E)--
            #180105-00029#1 add --(S)--
            LET l_xcbs.xcbs101 = s_num_round(l_roundtype,l_xcbs.xcbs101,l_ooaj006)    
            LET l_xcbs.xcbs111 = s_num_round(l_roundtype,l_xcbs.xcbs111,l_ooaj006)    
            LET l_xcbs.xcbs121 = s_num_round(l_roundtype,l_xcbs.xcbs121,l_ooaj006)    
            LET l_xcbs.xcbs202 = s_num_round(l_roundtype,l_xcbs.xcbs202,l_ooaj007) 
            LET l_xcbs.xcbs212 = s_num_round(l_roundtype,l_xcbs.xcbs212,l_ooaj007) 
            LET l_xcbs.xcbs222 = s_num_round(l_roundtype,l_xcbs.xcbs222,l_ooaj007)
            #180105-00029#1 add --(E)--
...
#            INSERT INTO xcbs_t VALUES(l_xcbs.*)  #161124-00048#21 mark
            ...
            #IF l_xcbs.xcbs202 > l_max_amt THEN   #170808-00032#1 mark
            IF s_math_abs(l_xcbs.xcbs202) > s_math_abs(l_max_amt) THEN #170808-00032#1 add            
...
#            INSERT INTO xcbs_t VALUES(l_xcbs.*)  #161124-00048#21 mark
      ...
      #標準機時
      #IF l_xcbj.xcbj006 = '3' THEN #170617-00503#1 mark
      IF l_xcbj.xcbj006 = '3' OR l_xcbj.xcbj006 = '8' THEN #170617-00503#1 add
...
#尾差处理的范围是同一分摊方式不同工单
      IF l_xcbs.xcbs006 IS NOT NULL THEN   #没有工单，说明前面没有抓到xcbr资料，不该有尾差调整
        ...
        #   LET l_xcbs.xcbs111 = s_curr_round(l_xcbs.xcbscomp,l_curr,l_xcbs.xcbs111,'4')    
        #   LET l_xcbs.xcbs121 = s_curr_round(l_xcbs.xcbscomp,l_curr,l_xcbs.xcbs121,'4')            
        #   LET l_xcbs.xcbs202 = s_curr_round(l_xcbs.xcbscomp,l_curr,l_sum_xcbs202,'5') 
        #   LET l_xcbs.xcbs212 = s_curr_round(l_xcbs.xcbscomp,l_curr,l_sum_xcbs212,'5') 
        #   LET l_xcbs.xcbs222 = s_curr_round(l_xcbs.xcbscomp,l_curr,l_sum_xcbs222,'5')
        #   LET l_xcbj.xcbj103 = s_curr_round(l_xcbs.xcbscomp,l_curr,l_xcbj.xcbj103,'5') 
        #   LET l_xcbj.xcbj113 = s_curr_round(l_xcbs.xcbscomp,l_curr,l_xcbj.xcbj113,'5') 
        #   LET l_xcbj.xcbj123 = s_curr_round(l_xcbs.xcbscomp,l_curr,l_xcbj.xcbj123,'5')
        #  #170725-00076#7   add --e           
        #ELSE
        #   CONTINUE FOREACH
        #END IF
        #180105-00029#1 mark --(E)--
         ...
         LET l_xcbs.xcbs111 = s_num_round(l_roundtype,l_xcbs.xcbs111,l_ooaj006)    
         LET l_xcbs.xcbs121 = s_num_round(l_roundtype,l_xcbs.xcbs121,l_ooaj006)            
         LET l_xcbs.xcbs202 = s_num_round(l_roundtype,l_sum_xcbs202,l_ooaj007) 
         LET l_xcbs.xcbs212 = s_num_round(l_roundtype,l_sum_xcbs212,l_ooaj007) 
         LET l_xcbs.xcbs222 = s_num_round(l_roundtype,l_sum_xcbs222,l_ooaj007)
         LET l_xcbj.xcbj103 = s_num_round(l_roundtype,l_xcbj.xcbj103,l_ooaj007) 
         LET l_xcbj.xcbj113 = s_num_round(l_roundtype,l_xcbj.xcbj113,l_ooaj007) 
         LET l_xcbj.xcbj123 = s_num_round(l_roundtype,l_xcbj.xcbj123,l_ooaj007)
         #180105-00029#1 add --(E)--         
...
#处理尾差结束
   ...
   #170223-00004#3--s
   #當 閒置費用金額大於0時，產生一筆工單單號為 'OH+SUB'的資料，記錄閒置費用 (依年度、期別、成本中心、費用分類、分攤方式)
   LET g_sql = "SELECT xcbjent,xcbjcomp,xcbjld,xcbj001,xcbj002,xcbj003,xcbj004,xcbj005,", 
               "       xcbj006,SUM(xcbj104),SUM(xcbj114),SUM(xcbj124)",
               "     FROM xcbj_t ", 
               " WHERE xcbjent = '",g_enterprise,"'",
               ...
               "   AND (xcbj104 <> 0 OR xcbj114 <> 0 OR xcbj124 <> 0 ) ",
               "  GROUP BY xcbjent,xcbjcomp,xcbjld,xcbj001,xcbj002,xcbj003,xcbj004,xcbj005,xcbj006 "
   PREPARE axcp202_xcbj_pre4 FROM g_sql
   DECLARE axcp202_xcbj_cur4 CURSOR FOR axcp202_xcbj_pre4

   IF cl_get_para(g_enterprise,g_site,'S-FIN-6015') = 'Y' THEN   #启用成本次要素
      CALL s_chr_replace(g_sql,'xcbj','xcdr',0) RETURNING g_sql
      PREPARE axcp202_xcdr_pre4 FROM g_sql
      DECLARE axcp202_xcdr_cur4 CURSOR FOR axcp202_xcdr_pre4
   END IF
   
   FOREACH axcp202_xcbj_cur4 INTO l_xcbj.xcbjent,l_xcbj.xcbjcomp,l_xcbj.xcbjld,l_xcbj.xcbj001,l_xcbj.xcbj002,l_xcbj.xcbj003,
                                  l_xcbj.xcbj004,l_xcbj.xcbj005,l_xcbj.xcbj006,l_xcbj.xcbj104,l_xcbj.xcbj114,l_xcbj.xcbj124
      IF SQLCA.sqlcode THEN
         INITIALIZE g_errparam TO NULL
         LET g_errparam.code = SQLCA.sqlcode
      ...
      LET l_xcbs.xcbs004 = l_xcbj.xcbj004
      LET l_xcbs.xcbs005 = l_xcbj.xcbj005
      LET l_xcbs.xcbs006 = 'OH+SUB'         #工單單號  
      LET l_xcbs.xcbs007 = l_xcbj.xcbj006
      LET l_xcbs.xcbs008 = ' '
      LET l_xcbs.xcbs009 = ' '
      LET l_xcbs.xcbs101 = l_xcbj.xcbj104
      LET l_xcbs.xcbs111 = l_xcbj.xcbj114
      LET l_xcbs.xcbs121 = l_xcbj.xcbj124
      LET l_xcbs.xcbs202 = l_xcbj.xcbj104
      LET l_xcbs.xcbs212 = l_xcbj.xcbj114
      LET l_xcbs.xcbs222 = l_xcbj.xcbj124
      LET l_xcbs.xcbs100 = 0   #分攤數
      ...
      FOREACH axcp202_xcdr_cur2 INTO l_xcdr.*
         ...
         IF l_xcdr.xcdr006 = '1' THEN 
            ...
            FOREACH xcbr_cur_11 INTO l_xcbr002,l_xcbr003,l_xcbr004,l_sum_xcbr201
              ...
              #CALL s_ld_sel_glaa(l_xcdy.xcdyld,'glaa001') RETURNING l_success,l_curr          #180105-00029#1 mark
              #IF l_success THEN                                                               #180105-00029#1 mark
                 #LET l_xcdy.xcdy100 = s_curr_round(l_xcdy.xcdycomp,l_curr,l_xcdy.xcdy100,'1') #170725-00076#4  mark
                 #170725-00076#7  mark --s
              ...
              #180105-00029#1 mark --(S)--    
              #  #170725-00076#7   add --s
              #   LET l_xcdy.xcdy202 = s_curr_round(l_xcdy.xcdycomp,l_curr,l_xcdy.xcdy202,'5') 
              #   LET l_xcdy.xcdy212 = s_curr_round(l_xcdy.xcdycomp,l_curr,l_xcdy.xcdy212,'5') 
              #   LET l_xcdy.xcdy222 = s_curr_round(l_xcdy.xcdycomp,l_curr,l_xcdy.xcdy222,'5')
              #  #170725-00076#7   add --e                 
              #ELSE
              #   CONTINUE FOREACH
              #END IF
              #180105-00029#1 mark --(E)--
               #180105-00029#1 add --(S)--
               LET l_xcdy.xcdy202 = s_num_round(l_roundtype,l_xcdy.xcdy202,l_ooaj007) 
               LET l_xcdy.xcdy212 = s_num_round(l_roundtype,l_xcdy.xcdy212,l_ooaj007) 
               LET l_xcdy.xcdy222 = s_num_round(l_roundtype,l_xcdy.xcdy222,l_ooaj007)
               #180105-00029#1 add --(E)--               
...
#               INSERT INTO xcdy_t VALUES(l_xcdy.*)  #161124-00048#21 mark
               ...
               #IF l_xcdy.xcdy202 > l_max_amt THEN    #170808-00032#1 mark
               IF s_math_abs(l_xcdy.xcdy202) > s_math_abs(l_max_amt) THEN #170808-00032#1 add
...
#               INSERT INTO xcdy_t VALUES(l_xcdy.*)  #161124-00048#21 mark
   ...
   #尾差处理的范围是同一分摊方式不同工单
         IF l_xcdy.xcdy006 IS NOT NULL THEN    #没有工单，说明前面没有抓到xcbr资料，不该有尾差调整
           ...
           #180105-00029#1 mark --(S)--    
           #  #170725-00076#7   add --s
           #   LET l_xcdy.xcdy202 = s_curr_round(l_xcdy.xcdycomp,l_curr,l_sum_xcdy202,'5') 
           #   LET l_xcdy.xcdy212 = s_curr_round(l_xcdy.xcdycomp,l_curr,l_sum_xcdy212,'5') 
           #   LET l_xcdy.xcdy222 = s_curr_round(l_xcdy.xcdycomp,l_curr,l_sum_xcdy222,'5')
           #   LET l_xcdr.xcdr103 = s_curr_round(l_xcdy.xcdycomp,l_curr,l_xcdr.xcdr103,'5') 
           #   LET l_xcdr.xcdr113 = s_curr_round(l_xcdy.xcdycomp,l_curr,l_xcdr.xcdr113,'5') 
           #   LET l_xcdr.xcdr123 = s_curr_round(l_xcdy.xcdycomp,l_curr,l_xcdr.xcdr123,'5')
           #  #170725-00076#7   add --e               
           #ELSE
           #   CONTINUE FOREACH
           #END IF
           #180105-00029#1 mark --(E)--
            #180105-00029#1 add --(S)--
            LET l_xcdy.xcdy202 = s_num_round(l_roundtype,l_sum_xcdy202,l_ooaj007) 
            LET l_xcdy.xcdy212 = s_num_round(l_roundtype,l_sum_xcdy212,l_ooaj007) 
            LET l_xcdy.xcdy222 = s_num_round(l_roundtype,l_sum_xcdy222,l_ooaj007)
            LET l_xcdr.xcdr103 = s_num_round(l_roundtype,l_xcdr.xcdr103,l_ooaj007) 
            LET l_xcdr.xcdr113 = s_num_round(l_roundtype,l_xcdr.xcdr113,l_ooaj007) 
            LET l_xcdr.xcdr123 = s_num_round(l_roundtype,l_xcdr.xcdr123,l_ooaj007)
            #180105-00029#1 add --(E)--            
    ...
    #处理尾差结束
      END FOREACH 
      FOREACH axcp202_xcdr_cur4 INTO l_xcdr.xcdrent,l_xcdr.xcdrcomp,l_xcdr.xcdrld,l_xcdr.xcdr001,l_xcdr.xcdr002,l_xcdr.xcdr003,
                                     l_xcdr.xcdr004,l_xcdr.xcdr005,l_xcdr.xcdr006,l_xcdr.xcdr104,l_xcdr.xcdr114,l_xcdr.xcdr124
         IF SQLCA.sqlcode THEN
            INITIALIZE g_errparam TO NULL
            LET g_errparam.code = SQLCA.sqlcode
            ...
            EXIT FOREACH
         END IF 
         CALL axcp202_date_chk(l_xcdr.xcdrcomp,lc_param.xcbk002,lc_param.xcbk003)
         IF NOT cl_null(g_errno) THEN
            #LET g_errparam.extend = lc_param.xcbk002  #170509-00035#1 mark
            LET g_errparam.extend = lc_param.xcbk002||'.'||lc_param.xcbk003   #170509-00035#1 add
            LET g_errparam.code   = g_errno
         ...
         LET l_xcdy.xcdy004 = l_xcdr.xcdr004
         LET l_xcdy.xcdy005 = l_xcdr.xcdr005
         LET l_xcdy.xcdy006 = 'OH+SUB'         #工單單號  
         LET l_xcdy.xcdy007 = l_xcdr.xcdr006
         LET l_xcdy.xcdy008 = ' '
         LET l_xcdy.xcdy009 = ' '
         LET l_xcdy.xcdy101 = l_xcdr.xcdr104
         LET l_xcdy.xcdy111 = l_xcdr.xcdr114
         LET l_xcdy.xcdy121 = l_xcdr.xcdr124
         LET l_xcdy.xcdy202 = l_xcdr.xcdr104
         LET l_xcdy.xcdy212 = l_xcdr.xcdr114
         LET l_xcdy.xcdy222 = l_xcdr.xcdr124
         LET l_xcdy.xcdy100 = 0   #分攤數
...
################################################################################
# Descriptions...: 取得成本單價小數位數與成本金額小數位數
# Memo...........:
# Usage..........: CALL axcp202_get_ooaj(p_ld)
#                  RETURNING r_success,r_ooaj006,r_ooaj007,r_roundtype
# Input parameter: p_comp         法人
#                : p_ld           帳套
# Return code....: r_success      成功否
#                : r_ooaj006      成本單價小數位數
#                : r_ooaj007      成本金額小數位數
# Date & Author..: 18/01/15 By 07423  (#180105-00029#1 add)
# Modify.........:
################################################################################
PRIVATE FUNCTION axcp202_get_ooaj(p_comp,p_ld)
DEFINE p_comp         LIKE glaa_t.glaacomp     #法人
DEFINE p_ld           LIKE glaa_t.glaald       #帳套編號   
DEFINE r_success      LIKE type_t.num5         #成功否
DEFINE r_ooaj006      LIKE ooaj_t.ooaj006      #成本單價小數位數
DEFINE r_ooaj007      LIKE ooaj_t.ooaj007      #成本金額小數位數
DEFINE l_success      LIKE type_t.num5         
DEFINE l_curr         LIKE ooai_t.ooai001      #幣別
DEFINE l_ooef014      LIKE ooef_t.ooef014      #幣別參照表
   ...
   #取得該帳套的使用幣別
   CALL s_ld_sel_glaa(p_ld,'glaa001') RETURNING l_success,l_curr
   ...
   IF SQLCA.sqlcode THEN
      ...
      LET g_errparam.popup = TRUE
      CALL cl_err()
      RETURN r_success,r_ooaj006,r_ooaj007
   END IF
   
   #2.取币种设定档
   SELECT ooaj006,ooaj007
     INTO r_ooaj006,r_ooaj007
     FROM ooaj_t
   ...
   IF SQLCA.sqlcode THEN
      IF SQLCA.sqlcode = 100 THEN
         INITIALIZE g_errparam TO NULL
         LET g_errparam.code = 'aoo-00629'
         LET g_errparam.extend = l_curr
         LET g_errparam.popup = TRUE
         CALL cl_err()
         RETURN r_success,r_ooaj006,r_ooaj007
      ELSE
