...
#該程式非freestyle程式!
{<section id="axcp163.description" type="s" >}
#應用 a00 樣板自動產生(Version:3)
#+ Standard Version.....: SD版次:0011(2016-11-28 14:14:23), PR版次:0011(2018-01-04 13:45:44)
...
{<section id="axcp163.global" type="s" >}
...
#170615-00004#1     2017/06/15 By 02295    抓取上站作业为INIT时,runcard值给0
#170818-00006#1     2017/08/23 By zhujing  1.工单开窗不应过滤g_site,而是过滤法人下的资料；2.处理段也是应该过滤法人底下的工单
#170808-00051#1     2017/09/15 By 02294    初始制程就是平行工艺时，第一道制程的製程序都是1
#180103-00025#1     2018/01/04 By 02295    去除作业编号asft340的条件
#end add-point
...
{<section id="axcp163.main" type="s" >}
MAIN
...
{<section id="axcp163.init" type="s" >}
#+ 初始化作業
...
{<section id="axcp163.ui_dialog" type="s" >}
#+ 選單功能實際執行處
...
#         #應用 a03 樣板自動產生(Version:3)
         ...
         ON ACTION controlp INFIELD sfaadocno
            ...
            LET g_qryparam.where = " sfaa003 <> '3' AND (sfaastus = 'F' OR sfaastus = 'M' ) "
                                   ," AND sfaasite IN (SELECT ooef001 FROM ooef_t WHERE ooefent = ",g_enterprise," AND ooef017 = '",g_master.xcbtcomp,"' AND ooef201 = 'Y') " #170818-00006#1 add
#            CALL q_sfcadocno()                           #呼叫開窗 #170818-00006#1 mark
            CALL q_sfcadocno_4()                   #170818-00006#1 取消营运据点的限定
...
{<section id="axcp163.transfer_argv" type="s" >}
...
{<section id="axcp163.process" type="s" >}
...
#                  "   WHERE sfcaent = sfaaent AND sfcadocno = sfaadocno AND sfcasite = sfaasite AND sfaasite = '",g_site,"' ",   #170818-00006#1 mark
                  "   WHERE sfcaent = sfaaent AND sfcadocno = sfaadocno AND sfcasite = sfaasite  ",   #170818-00006#1 mod
                  "     AND sfaasite IN (SELECT ooef001 FROM ooef_t WHERE ooefent = ",g_enterprise," AND ooef017 = '",g_master.xcbtcomp,"' AND ooef201 = 'Y') ", #170818-00006#1 add
...
{<section id="axcp163.get_buffer" type="s" >}
...
{<section id="axcp163.msgcentre_notify" type="s" >}
...
{<section id="axcp163.other_function" readonly="Y" type="s" >}
...
DEFINE l_sfcb005    LIKE sfcb_t.sfcb005  #170808-00051#1 add
DEFINE l_xcbt009    LIKE xcbt_t.xcbt009  #170808-00051#1 add
DEFINE l_sfcb005_n  LIKE sfcb_t.sfcb005  #170808-00051#1 add
    ...
    SELECT sfcb016,sfcb005 INTO l_sfcb016_t,l_sfcb005 FROM sfcb_t #170808-00051#1 add sfcb005
    ...
    WHILE TRUE
             ...
             IF l_sfaa057 = '1' THEN #厂内
                SELECT SUM(inaj011*inaj014) INTO l_xcbt.xcbt010 FROM inaj_t 
                    WHERE inajent = g_enterprise AND inajsite = g_site AND inaj020 = p_sfcbdocno 
                      ...
                      #AND inaj035 = 'asft340'   #异动作业编号  #180103-00025#1 mark
             ...
             ELSE
                ...
                SELECT SUM(inaj011*inaj014) INTO l_xcbt.xcbt010 FROM inaj_t 
                    WHERE inajent = g_enterprise AND inajsite = g_site AND inaj020 = p_sfcbdocno 
                      ...
                      #AND (inaj035 = 'apmt571' OR inaj035 = 'apmt531')    #异动作业编号  #180103-00025#1 mark
                      AND inaj022 <= g_edate AND inaj022 >= g_bdate
             END IF
       ...
       FOR l_i = 1 TO l_sfcb.getLength()
          ...
          IF l_sfcb016_t = 'Y' THEN   #161121-00018#13
             ...
             LET g_n = g_n + 1
             #170808-00051#1---add---s
             #当前站的上站资料是INIT，且只有INIT这一笔上站来源时，当前的工艺顺序g_n=1
             LET l_n = 0
             SELECT COUNT(1) INTO l_n FROM sfcb_t 
                 WHERE sfcbent = g_enterprise AND sfcbdocno = p_sfcbdocno AND sfcb003 = p_sfcc003 
                   AND sfcb004 = p_sfcc004 AND sfcb007 = 'INIT'
             IF l_n = 1 THEN
                LET g_n = 1
             END IF
             #170808-00051#1---add---e
             
             #170808-00051#1---mark---s
             #SELECT MAX(xcbt008) INTO l_xcbt008 FROM xcbt_t 
             #      WHERE xcbtent = g_enterprise AND xcbtcomp = g_master.xcbtcomp AND xcbt001 = g_master.xcbt001
             #        AND xcbt002 = g_master.xcbt002 AND xcbt003 = p_sfcbdocno AND xcbt004 = p_sfcc003 AND xcbt005 = p_sfcc004
             #        AND xcbt006 = l_sfcb003 AND xcbt007 = l_sfcb004
             #170808-00051#1---mark---e
             #170808-00051#1---add---s
             SELECT xcbt008,xcbt009 INTO l_xcbt008,l_xcbt009 FROM xcbt_t 
                   WHERE xcbtent = g_enterprise AND xcbtcomp = g_master.xcbtcomp AND xcbt001 = g_master.xcbt001
                     AND xcbt002 = g_master.xcbt002 AND xcbt003 = p_sfcbdocno AND xcbt004 = p_sfcc003 AND xcbt005 = p_sfcc004
                     AND xcbt006 = l_sfcb003 AND xcbt007 = l_sfcb004        
             #170808-00051#1---add---e
                ...
                IF l_xcbt008 < g_n THEN
                   ...
                   UPDATE xcbt_t SET xcbt008 = g_n
                        WHERE xcbtent = g_enterprise AND xcbtcomp = g_master.xcbtcomp AND xcbt001 = g_master.xcbt001
                          AND xcbt002 = g_master.xcbt002 AND xcbt003 = p_sfcbdocno AND xcbt004 = p_sfcc003 AND xcbt005 = p_sfcc004
                          AND xcbt006 = l_sfcb003 AND xcbt007 = l_sfcb004
                   IF l_sfcb005 = '2' THEN  #替代群组，则上站转入数量累加，否则则是取转入数量小的值
                      UPDATE xcbt_t SET xcbt009 = xcbt009 + l_xcbt.xcbt009
                           WHERE xcbtent = g_enterprise AND xcbtcomp = g_master.xcbtcomp AND xcbt001 = g_master.xcbt001
                             AND xcbt002 = g_master.xcbt002 AND xcbt003 = p_sfcbdocno AND xcbt004 = p_sfcc003 AND xcbt005 = p_sfcc004
                             AND xcbt006 = l_sfcb003 AND xcbt007 = l_sfcb004
                   ELSE
                      IF l_xcbt.xcbt009 < l_xcbt009 THEN
                         UPDATE xcbt_t SET xcbt009 = l_xcbt.xcbt009
                           WHERE xcbtent = g_enterprise AND xcbtcomp = g_master.xcbtcomp AND xcbt001 = g_master.xcbt001
                             AND xcbt002 = g_master.xcbt002 AND xcbt003 = p_sfcbdocno AND xcbt004 = p_sfcc003 AND xcbt005 = p_sfcc004
                             AND xcbt006 = l_sfcb003 AND xcbt007 = l_sfcb004
                       END IF
          ...
          IF l_sfcb009 = 'END' THEN
             IF l_sfcb[l_i].sfcb016 = 'Y' THEN  #161121-00018#13
                ...
                #170418-00026#1---add---e
                   ...
                   #LET l_xcbt.xcbt012 = l_sffb019  #170618-00158#1 mark
                   LET l_xcbt.xcbt012 = l_sffb020   #170618-00158#1 add
                END IF   #170418-00026#1
                #LET l_xcbt.xcbt013 = l_sffb020  #170618-00158#1 mark
                LET l_xcbt.xcbt013 = l_sffb019   #170618-00158#1 add
                ...
                SELECT COUNT(1),xcbt009 INTO l_n,l_xcbt009 FROM xcbt_t   #170808-00051#1 add xcbt009
                   WHERE xcbtent = g_enterprise AND xcbtcomp = g_master.xcbtcomp AND xcbt001 = g_master.xcbt001
                ...
                ELSE
                   ...
                   UPDATE xcbt_t SET xcbt008 = l_xcbt.xcbt008
                        WHERE xcbtent = g_enterprise AND xcbtcomp = g_master.xcbtcomp AND xcbt001 = g_master.xcbt001
                          AND xcbt002 = g_master.xcbt002 AND xcbt003 = p_sfcbdocno AND xcbt004 = l_sfcb003 AND xcbt005 = l_sfcb004
                          AND xcbt006 = l_sfcb009 AND xcbt007 = l_sfcb010
                   SELECT sfcb005 INTO l_sfcb005_n FROM sfcb_t 
                         WHERE sfcbent = g_enterprise AND sfcbdocno = p_sfcbdocno AND sfcb003 = l_sfcb003 AND sfcb004 = l_sfcb004
                   IF l_sfcb005_n = '2' THEN  #替代群组，则上站转入数量累加，否则则是取转入数量小的值
                      UPDATE xcbt_t SET xcbt009 = xcbt009 + l_xcbt.xcbt009
                        WHERE xcbtent = g_enterprise AND xcbtcomp = g_master.xcbtcomp AND xcbt001 = g_master.xcbt001
                          AND xcbt002 = g_master.xcbt002 AND xcbt003 = p_sfcbdocno AND xcbt004 = l_sfcb003 AND xcbt005 = l_sfcb004
                          AND xcbt006 = l_sfcb009 AND xcbt007 = l_sfcb010
             ...
             ELSE
                ...
                UPDATE xcbt_t SET xcbt008 = l_xcbt.xcbt008
                  WHERE xcbtent = g_enterprise AND xcbtcomp = g_master.xcbtcomp AND xcbt001 = g_master.xcbt001
                    ...
                    AND xcbt006 = l_xcbt.xcbt006 AND xcbt007 = l_xcbt.xcbt007
                SELECT sfcb005 INTO l_sfcb005_n FROM sfcb_t 
                      WHERE sfcbent = g_enterprise AND sfcbdocno = p_sfcbdocno AND sfcb003 = l_xcbt.xcbt004 AND sfcb004 = l_xcbt.xcbt005
                IF l_sfcb005_n = '2' THEN  #替代群组，则上站转入数量累加，否则则是取转入数量小的值
                   UPDATE xcbt_t SET xcbt009 = xcbt009 + l_xcbt.xcbt009
                     WHERE xcbtent = g_enterprise AND xcbtcomp = g_master.xcbtcomp AND xcbt001 = g_master.xcbt001
                       AND xcbt002 = g_master.xcbt002 AND xcbt003 = p_sfcbdocno AND xcbt004 = l_xcbt.xcbt004 AND xcbt005 = l_xcbt.xcbt005
...
DEFINE l_success    LIKE type_t.num5
    ...
    ELSE
      ...
      #170808-00051#1--mark---e
      #170808-00051#1--add---s
      #规格调整，平行工艺、无顺序群组等，当站转入抓上站转出的最小值
      SELECT MIN(xcbt010) INTO l_sfdb007 FROM xcbt_t 
         WHERE xcbtent = g_enterprise AND xcbtcomp = g_master.xcbtcomp AND xcbt001 = g_master.xcbt001
           AND xcbt002 = g_master.xcbt002 AND xcbt003 = p_sfcbdocno AND xcbt006 = p_sfcc003 AND xcbt007 = p_sfcc004
      #170808-00051#1--add---s
    END IF
    ...
    #170418-00026#1---add---e
       ...
       #LET r_xcbt.xcbt012 = l_sffb019  #170618-00158#1 mark
       LET r_xcbt.xcbt012 = l_sffb020  #170618-00158#1 add
    END IF   ##170418-00026#1
    #LET r_xcbt.xcbt013 = l_sffb020  #170618-00158#1 mark
    LET r_xcbt.xcbt013 = l_sffb019  #170618-00158#1 add
...
DEFINE l_sfcb005    LIKE sfcb_t.sfcb005  #170808-00051#1 add
    ...
    SELECT sfcb005 INTO l_sfcb005 FROM sfcb_t
          WHERE sfcbent = g_enterprise AND sfcbdocno = p_sfcbdocno AND sfcb003 = p_sfcc003 AND sfcb004 = p_sfcc004
    #170808-00051#1---add---e
