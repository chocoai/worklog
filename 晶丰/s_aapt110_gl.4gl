# Prog. Version..: '5.30.13-14.06.19(00010)'     #
#
...
# Modify.........: No:MOD-180928 18/09/27 By lixwz  BPM抛转的有6位小数的金额,分录底稿只处理了本币的位数,先增加了原币的保留小数位数
...
FUNCTION t110_g_gl_1(p_aptype,p_apno,p_npptype) #No.FUN-680029 新增p_npptype
   DEFINE l_nmh26   LIKE nmh_file.nmh26  #add by zhouxm160105
   DEFINE l_n       LIKE type_file.num5  #add by zhouxm160105
   DEFINE l_nmd23   LIKE nmd_file.nmd23  #add by zhouxm160105
   DEFINE p_aptype	LIKE apa_file.apa00
   DEFINE p_apno	LIKE apa_file.apa01
   DEFINE p_npptype     LIKE npp_file.npptype   #No.FUN-680029
   ...
   DEFINE ll_nmh11      LIKE nmh_file.nmh11,
          ll_nmh30      LIKE nmh_file.nmh30  #add by zhouxm160105
   #  DEFINE l_nmd23         LIKE nmd_file.nmd23    #FUN-C90122
   DEFINE l_nmd231        LIKE nmd_file.nmd23    #FUN-C90122
   DEFINE z_occ37       LIKE occ_file.occ37    #zhouxm160301 add
   DEFINE l_wc          STRING #add renjj161221
         ...
         FOREACH t110_gl_c1 INTO l_api.*
           ...
           #FUN-D10065---mark--end
            ...
            IF g_apz.apz27 = 'Y' THEN   #有做重評價
                    LET l_count = 1
                    FOREACH t110_oox07_cl USING l_api.api26,l_api.api40
      ...
      IF l_apa.apa511 = 'MISC' OR l_apa.apa511 = 'UNAP' THEN
         ...
         FOREACH t110_gl_c11 INTO l_api.*
           ...
           #FUN-D10065---mark---end
            ...
            IF g_apz.apz27 = 'Y' THEN   #有做重評價
                LET l_count = 1
                FOREACH t110_oox07_cl_1 USING l_api.api26,l_api.api40
                                      INTO l_oox07[l_count].*
                    IF STATUS THEN
                       CALL cl_err('forach:',STATUS,1)
                       EXIT FOREACH
         ...
         FOREACH t110_gl_c41 INTO l_apb29,l_npq.npq03,l_npq.npq05,   #l_npq.npq04, #MOD-C80065 mark
          ...
          END IF
            ...
            IF l_npq.npq07!=0 THEN
              ...
              #add renjj161221--str
              IF l_apb26=' ' OR cl_null(l_apb26) THEN
                  LET l_wc= " (apb26= ' ' or apb26 is null )"
              ELSE
               	  LET l_wc= " apb26= '",l_apb26,"'"
              END IF
              IF l_apb31=' ' OR cl_null(l_apb31) THEN
                  LET l_wc=l_wc CLIPPED ," and  (apb31= ' ' or apb31 is null )"
               ELSE
               	  LET l_wc= l_wc CLIPPED, " and apb31= '",l_apb31,"'"
              END IF

              IF l_apb35=' ' OR cl_null(l_apb35) THEN
                  LET l_wc=l_wc CLIPPED ," and  (apb35= ' ' or apb35 is null )"
               ELSE
               	  LET l_wc= l_wc CLIPPED ," and apb35= '",l_apb35,"'"
              END IF

              IF l_apb36=' ' OR cl_null(l_apb36) THEN
                  LET l_wc=l_wc CLIPPED ," and (apb36= ' ' or apb36 is null )"
               ELSE
               	  LET l_wc= l_wc CLIPPED, " and apb36= '",l_apb36,"'"
              END IF

               IF l_apb930=' ' OR cl_null(l_apb930) THEN
                  LET l_wc=l_wc CLIPPED ," and (apb930= ' ' or apb930 is null )"
                ELSE
               	  LET l_wc= l_wc CLIPPED ," and apb930= '",l_apb930,"'"
               END IF
               #add renjj161221--end
               ...
               LET l_sql = " SELECT apb02 ",
                          ...
                          #"    AND apb26 = '",l_npq.npq05,"'",  #CHI-CC0022 mark
                           "    AND ",l_wc CLIPPED   #add renjj161221
                          { "    AND apb26 = '",l_apb26,"'",      #CHI-CC0022 add
                           "    AND apb31 = '",l_apb31,"'",
                           "    AND apb35 = '",l_apb35,"'",
                           "    AND apb36 = '",l_apb36,"'",
                           "    AND apb930 = '",l_apb930,"'" } #mark renjj161221
               ...
               #add by pane 180829 begin#
               IF g_aptype ='12' THEN
                  LET l_sql = cl_replace_str(l_sql,"apb27","apb33")
               END IF
               #MOD-D40069 add end-----
                                                         #MOD-D20101 add
              ...
              #CALL s_def_npq(l_npq.npq03,g_prog,l_npq.*,l_npq.npq01,'','',g_bookno)        #No.FUN-730064 #MOD-CC0253 mark
               ...
               SELECT azi04 INTO t_azi04 FROM azi_file
                 WHERE azi01 = l_npq.npq24
               LET l_npq.npq07f = cl_digcut(l_npq.npq07f,t_azi04) #No:MOD-180928 add by lixwz 180928
               LET l_npq.npq07 = cl_digcut(l_npq.npq07,g_azi04)
             # IF l_aag371 MATCHES '[23]' THEN                 #FUN-950053 mark
               IF l_aag371 MATCHES '[234]' THEN                #FUN-950053 add
                  #-->for 合併報表-關係人
   ...
   IF p_npptype = '0' THEN  #No.FUN-680029
      IF NOT cl_null(l_apa.apa51) AND l_apa.apa51 != 'UNAP' AND
        ...
        #LET l_npq.npq04 = l_apa.apa25        #FUN-D10065  mark
         ...
         SELECT SUM(apb10),SUM(apb24) INTO l_api05,l_api05f FROM apb_file
...
#-----------------------------------( Dr/Cr: Difference  )---------------------
  ...
  #IF l_apa.apa57>0 AND l_apa.apa33 != 0 AND l_apa.apa56 != '3' THEN #CHI-DC0023 mark
   IF l_apa.apa33 != 0 AND l_apa.apa56 != '3' THEN #CHI-DC0023
      LET l_npq.npq02 = l_npq.npq02 + 1
...
#-----------------------------------( Cr: 直接付款       )---------------------
   ...
   FOREACH t110_gl_c5 INTO l_aph.*
     ...
     #FUN-D10065--mark--end---
      LET l_npq.npq06 = '2'
      #add by zhouxm160105--str
        IF g_prog='aapt150' THEN
         SELECT COUNT(*) INTO l_n FROM nmh_file
         WHERE nmh01=l_npq.npq03
         IF l_n>=1 THEN
          SELECT nmh26 INTO l_nmh26 FROM nmh_file
          WHERE nmh01=l_aph.aph04
          LET l_npq.npq03=l_nmh26
         END IF
        END IF
        #add by zhouxm160105--end
        #zhouxm160129 add start
        IF l_aph.aph03='F' THEN
              SELECT nmd23,nmd231 INTO l_nmd23,l_nmd231 FROM nmd_file
              WHERE nmd01=l_aph.aph04
              IF p_npptype = '0' THEN
                  LET l_aph.aph04 = l_nmd23
               ELSE
                  LET l_aph.aph04 = l_nmd231
               END IF
               IF cl_null(l_aph.aph04) THEN
                  IF g_apz.apz13 = 'Y' THEN
                     IF p_npptype = '0' THEN
                        SELECT aps24 INTO l_aph.aph04 FROM aps_file WHERE aps01 = l_apa.apa22
                     ELSE
                        SELECT aps241 INTO l_aph.aph04 FROM aps_file WHERE aps01 = l_apa.apa22
                     END IF
                  ELSE
                     IF p_npptype = '0' THEN
                        SELECT aps24 INTO l_aph.aph04 FROM aps_file WHERE (aps01 = ' ' OR aps01 IS NULL)
                     ELSE
                        SELECT aps241 INTO l_aph.aph04 FROM aps_file WHERE (aps01 = ' ' OR aps01 IS NULL)
                     END IF
                  END IF
               END IF
               LET l_npq.npq03 = l_aph.aph04
           END IF
        #zhouxm160129 add end
...
#No.FUN-AC0001 --end
        #FUN-D40118 ---Add--- Start
         ...
         END IF
        #FUN-D40118 ---Add--- End
            ###################add by maoyy20160919
             IF g_prog = 'aapt150' AND l_npq.npq06='2' AND  l_aph.aph03='D' THEN
                SELECT  nmh11,nmh30 INTO l_npq.npq21,l_npq.npq22 from nmh_file,npo_file
                    where nmh01=npo03
                    and npo03=l_aph.aph04
                     LET l_npq.npq11=l_npq.npq21
               END IF
             ###################add by maoyy20160919
         INSERT INTO npq_file VALUES (l_npq.*)
         IF STATUS THEN
      ...
      IF l_apa.apa51 = 'MISC' OR l_apa.apa51 = 'UNAP' THEN
         ...
         FOREACH t110_gl_c12 INTO l_api.*
           ...
           #FUN-D10065--mark--end--
            ...
            IF g_apz.apz27 = 'Y' THEN   #有做重評價
                LET l_count = 1
                FOREACH t110_oox07_cl1 USING l_api.api26,l_api.api40
                                      INTO l_oox07[l_count].*
                    IF STATUS THEN
                       CALL cl_err('forach:',STATUS,1)
                       EXIT FOREACH
      ...
      IF l_apa.apa511 = 'MISC' OR l_apa.apa511 = 'UNAP' THEN
         ...
         FOREACH t110_gl_c13 INTO l_api.*
           ...
           #FUN-D10065---mark--end
            ...
            IF g_apz.apz27 = 'Y' THEN   #有做重評價
                LET l_count = 1
                FOREACH t110_oox07_cl2 USING l_api.api26,l_api.api40
                                      INTO l_oox07[l_count].*
                    IF STATUS THEN
                       CALL cl_err('forach:',STATUS,1)
                       EXIT FOREACH
  ...
  DEFINE l_flag        LIKE type_file.chr1    #No.FUN-D40118   Add
     ...
     IF p_npptype = '0' THEN
        LET g_sql = " SELECT api02,api04,SUM(api05),SUM(api05f),api06,api07,api930,api21,",   #MOD-8C0067 add api930 #MOD-B10093 add api02
                   ...
                   #"    AND (api04 IS NOT NULL AND api04 <> ' ')",              #TQC-B10104 mark
                    ...
                    "           api36,api37,api40 "  #MOD-930244 add api40
     ELSE
     ...
     FOREACH t110_gl_c1x INTO l_api.*
       ...
       #FUN-D10065--mark--end--
        ...
        IF g_apz.apz27 = 'Y' THEN   #有做重評價
                LET l_count = 1
                FOREACH t110_oox07_cl3 USING l_api.api26,l_api.api40
                                      INTO l_oox07[l_count].*
                    IF STATUS THEN
                       CALL cl_err('forach:',STATUS,1)
                       EXIT FOREACH
       ...
       #-TQC-B10104-add-
        ELSE
          ...
          #LET l_npq.npq25 = 1
           LET l_npq.npq24 = p_apa.apa13  #mod by pane 170825
           LET l_npq.npq25 = p_apa.apa14  #mod by pane 170825
        END IF
       #-TQC-B10104-end-
        LET l_npq.npq07 = l_api.api05
        LET l_npq.npq07f= l_api.api05f
