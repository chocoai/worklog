...
DEFINE  lc_qbe_sn       LIKE    gbm_file.gbm01    #No.FUN-580031  HCN
   ...
   INITIALIZE g_oqa.* TO NULL    #No.FUN-750051
    CONSTRUCT BY NAME g_wc ON                     # 螢幕上取單頭條件
        ...
        ,ta_oqa01,ta_oqa02 # add by lixwz 20170930
               BEFORE CONSTRUCT
                  CALL cl_qbe_init()
...
DEFINE l_chk        LIKE type_file.chr1 # add by lixwz 20170822
    ...
    INPUT  g_oqa.oqaoriu,g_oqa.oqaorig,
        ...
        g_oqa.oqaud13,g_oqa.oqaud14,g_oqa.oqaud15,g_oqa.ta_oqa01,g_oqa.ta_oqa02
        WITHOUT DEFAULTS
        ...
        oqaud13,oqaud14,oqaud15,ta_oqa01,ta_oqa02
...
#FUN-AA0059 ---------------------end-------------------------------
        ...
        #  add by lixwz 29170816 e
           ...
           # IF NOT cl_validate() THEN NEXT FIELD CURRENT END IF
            IF NOT cl_null(g_oqa.oqaud02) THEN
                ...
                ELSE
                    ...
                    CALL  t310_get_oqaud1(g_oqa.*,g_oqb,'0') RETURNING g_oqa.oqaud04,g_oqa.oqaud05,g_oqa.oqaud06,g_oqa.oqaud07,g_oqa.oqaud08,g_oqa.oqaud09
                    DISPLAY BY NAME g_oqa.oqaud04,g_oqa.oqaud05,g_oqa.oqaud06,g_oqa.oqaud07,g_oqa.oqaud08,g_oqa.oqaud09
                    #UPDATE oqb_file SET oqbud03 = oqb11 * ,g_oqa.oqaud02,  # add by lixwz 171127
                    #    WHERE oqb01=g_oqa.oqa01
                    LET g_sql = " UPDATE oqb_file SET oqbud03 =oqb11/",g_oqa.oqaud02 CLIPPED," WHERE oqb01='",g_oqa.oqa01 CLIPPED,"'"
                    PREPARE t310_upd_oqb FROM g_sql
                    EXECUTE t310_upd_oqb
                    IF SQLCA.sqlcode THEN
         ...
         AFTER FIELD ta_oqa01
            ...
            END IF
            IF NOT cl_validate() THEN NEXT FIELD CURRENT END IF
         # ad by lixwz 171113 s
         AFTER FIELD ta_oqa02
            IF NOT cl_null(g_oqa.ta_oqa02) THEN
                IF g_oqa.ta_oqa02 <= 0 THEN
                    CALL cl_err('纸箱每平方米单价不可小于0','!','0')
                    NEXT FIELD ta_oqa02
                ELSE
                #****************
                DISPLAY BY NAME g_oqa.ta_oqa02
                END IF
            END IF
            IF NOT cl_validate() THEN NEXT FIELD CURRENT END IF
        ON CHANGE ta_oqa02
            IF NOT cl_null(g_oqa.ta_oqa02) AND g_oqa.ta_oqa02 <= 0 THEN
                    CALL cl_err('纸箱每平方米单价不可小于0','!','0')
                    NEXT FIELD ta_oqa02
            END IF
            IF NOT cl_null(g_oqa.ta_oqa02) AND g_action_choice ='modify' THEN
            IF cl_confirm('axm_668') THEN
                #******************
                #更新单身中外箱的单价
                FOR  l_cnt = 1 TO g_oqb.getLength()
                    IF g_oqb[l_cnt].oqbud01='3' THEN #包装件
                        IF g_oqb[l_cnt].oqb03[1,4] ='MISC' AND g_oqb[l_cnt].oqb031 LIKE "外箱%" THEN
                            CALL t310_get_box_oqb09(g_oqb[l_cnt].oqb03,g_oqb[l_cnt].oqb032,g_oqa.ta_oqa02)
                                RETURNING g_oqb[l_cnt].oqb09
                            LET g_oqb[l_cnt].oqb10 = g_oqb[l_cnt].oqb09 * g_oqb[l_cnt].oqb08
                            LET g_oqb[l_cnt].oqb11 = g_oqb[l_cnt].oqb10 * g_oqb[l_cnt].oqbud07
                            LET g_oqb[l_cnt].oqbud03 = g_oqb[l_cnt].oqb11 / g_oqa.oqaud02  #　add by lixwz 171127
                            LET g_oqb[l_cnt].oqbud03 = cl_digcut(g_oqb[l_cnt].oqbud03,5)
                            #LET g_oqb[l_cnt].oqb10 = cl_digcut(g_oqb[l_cnt].oqb10,5)
                            LET g_oqb[l_cnt].oqb11 = cl_digcut(g_oqb[l_cnt].oqb11,5)
                            UPDATE oqb_file SET oqb09 = g_oqb[l_cnt].oqb09 ,
                                                oqb10 = g_oqb[l_cnt].oqb10 ,
                                                oqb11 = g_oqb[l_cnt].oqb11
                                                ,oqbud03 = g_oqb[l_cnt].oqbud03 # add by lixwz 171127
                                            WHERE oqb01 = g_oqa.oqa01 AND oqb02 = g_oqb[l_cnt].oqb02
                            IF SQLCA.sqlcode THEN
                               CALL cl_err('',SQLCA.sqlcode,0)
                            END IF
                        END IF
...
# add by lixwz 20170817 s
    ...
    DISPLAY BY NAME g_oqa.oqaoriu,g_oqa.oqaorig,                              # 顯示單頭值
        ...
        ,g_oqa.ta_oqa02   # add by lixwz 171117
        #   add by lixwz 20170817 s
...
#FUN-AA0059 ---------------------end-------------------------------
               ...
               IF g_oqb[l_ac].oqb03[1,4] !='MISC' THEN   #NO:6808
                  ...
                  #yemy 20130513  --End
                     ...
                     IF g_oqb[l_ac].oqb11 IS NULL THEN LET g_oqb[l_ac].oqb11=0 END IF
                     # LET g_oqb[l_ac].oqb11 = cl_digcut(g_oqb[l_ac].oqb11,t_azi04) mark by lixwz 171109 股价金额小数保留5位
                     LET g_oqb[l_ac].oqb11 = cl_digcut(g_oqb[l_ac].oqb11,5) # add by lixwz 171109
                     LET g_oqb[l_ac].oqbud03 = g_oqb[l_ac].oqb11 / g_oqa.oqaud02  #　add by lixwz 171127
                     LET g_oqb[l_ac].oqbud03 = cl_digcut(g_oqb[l_ac].oqbud03,5)
                     IF g_oqb[l_ac].oqb11 IS NULL THEN LET g_oqb[l_ac].oqb11=0 END IF
                     DISPLAY BY NAME g_oqb[l_ac].oqbud03  #　add by lixwz 171127
       ...
       AFTER FIELD oqb032
            IF NOT cl_null(g_oqb[l_ac].oqb032) AND g_oqb[l_ac].oqb031 LIKE "外箱%" THEN
                ...
                WHILE tok.hasMoreTokens()
                  ...
                  LET l_tok = l_tok + 1
                END WHILE
                IF NOT cl_null(g_oqa.ta_oqa02) AND g_oqa.ta_oqa02 > 0THEN # mod g_boxprice to g_oqa.ta_oqa02 lixwz 171127
                  IF g_oqb[l_ac].oqb03[1,4] ='MISC' AND  ( cl_null(g_oqb[l_ac].oqb09) OR g_oqb[l_ac].oqb09 = 0) THEN
                    ...
                    LET l_str ="SELECT ","((",g_box[1] CLIPPED,"*0.0254+",g_box[2] CLIPPED,"*0.0254)*2+0.07)*(",
                                g_box[2] CLIPPED,"*0.0254+",g_box[3] CLIPPED,"*0.0254+0.03)*",g_oqa.ta_oqa02 CLIPPED, # mod g_boxprice to g_oqa.ta_oqa02 lixwz 171127
                                " FROM dual"
       ...
       AFTER FIELD oqb07
        ...
        ON ROW CHANGE
            IF INT_FLAG THEN
               ...
               EXIT INPUT
            END IF
            # add by lixwz 171115 s
            # 修改单身纸箱价格
            IF g_oqb[l_ac].oqb09 != g_oqb_t.oqb09 THEN
                IF g_oqb[l_ac].oqbud01 = '3' AND g_oqb[l_ac].oqb031 LIKE "外箱%" THEN
                    IF  cl_confirm('axm_669') THEN
                        #*************************
                        CALL t310_get_boxprice(g_oqb[l_ac].oqb03,g_oqb[l_ac].oqb032,g_oqb[l_ac].oqb09)
                            RETURNING g_oqa.ta_oqa02
                        IF NOT cl_null(g_oqa.ta_oqa02) THEN

                            UPDATE oqa_file SET ta_oqa02 = g_oqa.ta_oqa02
                                WHERE oqa01=g_oqa.oqa01
                            IF SQLCA.sqlcode THEN
                               CALL cl_err('',SQLCA.sqlcode,0)
                            END IF
...
DEFINE l_chk       LIKE type_file.chr1
  ...
  IF l_chk='0' THEN
      ...
      IF l_oqa.oqa042 > 0 THEN
          ...
          LET l_oqa.oqaud05 = l_oqa.oqaud05 * l_oqa.oqa042 # add by lixwz 171124
      END IF
  ELSE
      #无利润率
      IF l_sum != 0 THEN
...
# add by lixwz 171113 s
# # 根据 原纸平方米单价和规格计算纸箱单价
FUNCTION t310_get_box_oqb09(p_oqb03,p_oqb032,p_boxprice)
DEFINE p_oqb03  LIKE oqb_file.oqb03
DEFINE p_oqb032 LIKE oqb_file.oqb032
DEFINE p_boxprice LIKE oqa_file.ta_oqa02
DEFINE l_oqb09 LIKE oqa_file.oqa09
DEFINE tok          base.StringTokenizer
DEFINE l_tok        LIKE type_file.num5
DEFINE l_str    STRING

    IF NOT cl_null(p_oqb032) THEN
        LET tok = base.StringTokenizer.create(p_oqb032,"X")
        LET l_tok = 1
        WHILE tok.hasMoreTokens()
          LET g_box[l_tok] = tok.nextToken()
          LET g_box[l_tok] = cl_replace_str(g_box[l_tok],'"',"")
          LET l_tok = l_tok + 1
        END WHILE
        IF NOT cl_null(p_boxprice) AND p_boxprice > 0 THEN
          IF p_oqb03[1,4] ='MISC' THEN
              #（（长*0.0254+宽*0.0254*2）+0.07）*（宽*0.0254+高*0.0254+0.03）*原纸的平方米单价带出纸箱单价
            LET l_str ="SELECT ","((",g_box[1] CLIPPED,"*0.0254+",g_box[2] CLIPPED,"*0.0254)*2+0.07)*(",
                        g_box[2] CLIPPED,"*0.0254+",g_box[3] CLIPPED,"*0.0254+0.03)*",p_boxprice CLIPPED,
                        " FROM dual"
            PREPARE t310_oqb09_s FROM l_str
            EXECUTE t310_oqb09_s INTO l_oqb09
            IF cl_null(l_oqb09) THEN LET l_oqb09 = 0 END IF
            #DISPLAY BY NAME g_oqb[l_ac].oqb09
          END IF
        END IF
    END IF
...
END FUNCTION
# 根据 纸箱单价和规格回写原纸平方米单价
FUNCTION t310_get_boxprice(p_oqb03,p_oqb032,p_oqb09)
DEFINE p_oqb03  LIKE oqb_file.oqb03
DEFINE p_oqb032 LIKE oqb_file.oqb032
DEFINE p_oqb09  LIKE oqb_file.oqb09
DEFINE p_boxprice LIKE oqa_file.ta_oqa02
DEFINE tok          base.StringTokenizer
DEFINE l_tok        LIKE type_file.num5
DEFINE l_str    STRING
    IF NOT cl_null(p_oqb032) THEN
        ...
        WHILE tok.hasMoreTokens()
          ...
          LET l_tok = l_tok + 1
        END WHILE
        IF NOT cl_null(p_oqb09) AND p_oqb09>0 THEN
            IF p_oqb03[1,4] ='MISC' THEN
                LET l_str = "SELECT ",p_oqb09 CLIPPED,"/((",g_box[1] CLIPPED,"*0.0254+",g_box[2] CLIPPED,"*0.0254)*2+0.07)/(",
                        g_box[2] CLIPPED,"*0.0254+",g_box[3] CLIPPED,"*0.0254+0.03) FROM dual"
                PREPARE t310_get_boxprice FROM l_str
                EXECUTE t310_get_boxprice INTO p_boxprice
            END IF
        END IF
    END IF
RETURN p_boxprice
END FUNCTION
# add by lixwz 171113 e