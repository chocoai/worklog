...
DEFINE g_b_xx  DYNAMIC ARRAY OF RECORD
         ...
         b_bz    LIKE  tz_aag_file.tz_aag04
       END RECORD
...
FUNCTION q931_curs()
 DEFINE l_wc  STRING
  ...
  #PREPARE q931_sysMon_prep5 FROM g_sql
   #-->2.6 应收票据
   LET g_sql = " select sum(a) from ( ",
               "  SELECT SUM(nmh02) a FROM nmh_file",
               " WHERE nmh26 = ? ",
               "   AND nmh38 = 'Y' and nmh24 in ('1')  ",
               "   AND YEAR(nmh04)*12+ MONTH(nmh04)<= ",tm.yy," *12+ ",tm.mm,
               "  union all ",
               " select sum(npo04) a ",
               "  from npn_file, npo_file, nmh_file ",
               "    where npn01 = npo01 and nmh01=npo03 and nmh26= ? ",
               "  and npnconf = 'Y' ",
               "  AND YEAR(nmh04)*12+ MONTH(nmh04)<= ",tm.yy," *12+ ",tm.mm,
               "  AND YEAR(npn02)*12+ MONTH(npn02)> ",tm.yy," *12+ ",tm.mm,")"
   ...
   #-->2.7 应付票据
   LET g_sql = " select sum(a) from ( ",
               "  SELECT SUM(nmd04) a FROM nmd_file",
               " WHERE nmd23 = ? ",
               "   AND nmd30 = 'Y' and nmd12 in ('1','X')  ",
               "   AND YEAR(nmd07)*12+ MONTH(nmd07)<= ",tm.yy," *12+ ",tm.mm,
               "  union all ",
               " select sum(npm06) a ",
               "  from npl_file, npm_file, nmd_file ",
               "    where npl01 = npm01 and nmd01=npm03 and nmd23= ? ",
               "  and nplconf = 'Y' ",
               "  AND YEAR(nmd07)*12+ MONTH(nmd07)<= ",tm.yy," *12+ ",tm.mm,
               "  AND YEAR(npl02)*12+ MONTH(npl02)> ",tm.yy," *12+ ",tm.mm,")"
  ...
  #-----------------3.子系统单据金额-------------------------------
   ...
   #抓取所有科目
   LET l_wc = cl_replace_str(tm.wc,"km","tz_aag01")
   LET g_sql = "SELECT tz_aag01,tz_aag02,tz_aag03 FROM tz_aag_file ",
               " WHERE tz_aag00 = '",tm.a,"' AND tz_aag04 ='Y' ",
               "   AND ",l_wc CLIPPED,
               " ORDER BY tz_aag01 "
...
DEFINE l_wc           STRING
DEFINE l_tz_aag01     LIKE tz_aag_file.tz_aag01
DEFINE l_tz_aag02     LIKE tz_aag_file.tz_aag02
DEFINE l_tz_aag03     LIKE tz_aag_file.tz_aag03
DEFINE l_cs           LIKE npr_file.npr01
DEFINE l_c_s        DYNAMIC  ARRAY OF RECORD
          tz_aag01   LIKE tz_aag_file.tz_aag01,
          tz_aag02   LIKE tz_aag_file.tz_aag02,
          c_s        LIKE occ_file.occ01,
          tz_aag03   LIKE tz_aag_file.tz_aag03
               END RECORD
DEFINE l_xx  RECORD
         km    LIKE  aag_file.aag01,
         kmm   LIKE  aag_file.aag02,
...
DEFINE l_cnt   LIKE  type_file.num5
   ...
   FOREACH q931_sel_cs1 INTO l_tz_aag01,l_tz_aag02,l_tz_aag03
      IF SQLCA.sqlcode THEN EXIT FOREACH END IF

      IF l_tz_aag02 MATCHES '[78]' THEN
         FOREACH q931_sel_cs2 USING l_tz_aag01,l_tz_aag01,l_tz_aag01 INTO l_cs
            CALL l_c_s.appendElement()
            LET l_c_s[l_c_s.getLength()].tz_aag01 = l_tz_aag01
            LET l_c_s[l_c_s.getLength()].tz_aag02 = l_tz_aag02
            LET l_c_s[l_c_s.getLength()].c_s = l_cs
            LET l_c_s[l_c_s.getLength()].tz_aag03 = l_tz_aag03
         END FOREACH

        #添加一个判断，78类型的总账也可能不做核算项，这里核算项给个'*'星号
        #下面抓数据时，碰到这种核算项直接抓aah,tah统计档
         LET l_aag15 = NULL
         SELECT aag15 INTO l_aag15 FROM aag_file WHERE aag01 = l_tz_aag01
            AND aag00 = tm.a
        #SELECT COUNT(*) INTO l_cnt FROM aag_file WHERE aag01 = l_tz_aag01
        #   AND aag00 = tm.a
   ...
   FOR i =1 TO l_c_s.getLength()
       ...
       #1.抓总账金额
       IF l_c_s[i].tz_aag02 MATCHES '[78]' THEN
          IF l_c_s[i].c_s = 'XXX' THEN
       	     EXECUTE q931_z_prep3 USING l_c_s[i].tz_aag01 INTO l_zbd,l_zbc
       	     EXECUTE q931_z_prep4 USING l_c_s[i].tz_aag01 INTO l_zyd,l_zyc
       	     EXECUTE q931_z_prep7 USING l_c_s[i].tz_aag01 INTO l_zdb,l_zdy
       	     EXECUTE q931_z_prep8 USING l_c_s[i].tz_aag01 INTO l_zcb,l_zcy
          ELSE
       	     EXECUTE q931_z_prep1 USING l_c_s[i].tz_aag01,l_c_s[i].c_s INTO l_zbd,l_zbc
       	     EXECUTE q931_z_prep2 USING l_c_s[i].tz_aag01,l_c_s[i].c_s INTO l_zyd,l_zyc
       	     EXECUTE q931_z_prep5 USING l_c_s[i].tz_aag01,l_c_s[i].c_s INTO l_zdb,l_zdy
       	     EXECUTE q931_z_prep6 USING l_c_s[i].tz_aag01,l_c_s[i].c_s INTO l_zcb,l_zcy
          END IF
       END IF
       IF l_c_s[i].tz_aag02 MATCHES '[123456]' THEN
       	  EXECUTE q931_z_prep3 USING l_c_s[i].tz_aag01 INTO l_zbd,l_zbc
       	  EXECUTE q931_z_prep4 USING l_c_s[i].tz_aag01 INTO l_zyd,l_zyc
       	  EXECUTE q931_z_prep7 USING l_c_s[i].tz_aag01 INTO l_zdb,l_zdy
       	  EXECUTE q931_z_prep8 USING l_c_s[i].tz_aag01 INTO l_zcb,l_zcy
       ...
       IF l_c_s[i].tz_aag02 MATCHES '[234]' THEN
          LET l_zyd = l_zbd
          LET l_zyc = l_zbc
          LET l_zdy = l_zdb
          LET l_zcy = l_zcb
       ...
       CASE tm.c
          WHEN '1'    #子系统月结
             CASE l_c_s[i].tz_aag02
                WHEN '1'  #货币资金
                   EXECUTE q931_sysMon_prep2 USING l_c_s[i].tz_aag01 INTO l_hbzj_db,l_hbzj_cb,l_hbzj_dy,l_hbzj_cy
                WHEN '2'  #固定资产
                  #EXECUTE q931_sysMon_prep3 USING l_c_s[i].tz_aag01 INTO l_gdzc
                   CALL afar201(l_c_s[i].tz_aag01,'2') RETURNING l_gdzc
                WHEN '3'  #资产折旧
                  #EXECUTE q931_sysMon_prep4 USING l_c_s[i].tz_aag01 INTO l_zczj
                   CALL afar201(l_c_s[i].tz_aag01,'3') RETURNING l_zczj
                WHEN '4'  #资产减值
                  #EXECUTE q931_sysMon_prep5 USING l_c_s[i].tz_aag01 INTO l_zcjz
                   CALL afar201(l_c_s[i].tz_aag01,'4') RETURNING l_zcjz
                WHEN '5'  #应付票据
                   EXECUTE q931_sysMon_prep7 USING l_c_s[i].tz_aag01,l_c_s[i].tz_aag01 INTO l_yfpj
                WHEN '6'  #应收票据
                   EXECUTE q931_sysMon_prep6 USING l_c_s[i].tz_aag01,l_c_s[i].tz_aag01 INTO l_yspj
                WHEN '7'  #应收款项
                   IF l_c_s[i].c_s = 'XXX' THEN
                     #EXECUTE q931_sysMon_prep11 USING l_c_s[i].tz_aag01 INTO l_yskx_db,l_yskx_dy,l_yskx_cb,l_yskx_cy  #No:131101  Mark
                      LET l_yskx_db = 0
                      LET l_yskx_dy = 0
                      LET l_yskx_cb = 0
                      LET l_yskx_cy = 0
                   ELSE
                      EXECUTE q931_sysMon_prep1 USING l_c_s[i].tz_aag01,l_c_s[i].c_s INTO l_yskx_db,l_yskx_dy,l_yskx_cb,l_yskx_cy
                   END IF
                WHEN '8'  #应付款项
                   IF l_c_s[i].c_s = 'XXX' THEN
                     #EXECUTE q931_sysMon_prep11 USING l_c_s[i].tz_aag01 INTO l_yfkx_db,l_yfkx_dy,l_yfkx_cb,l_yfkx_cy
                      LET l_yfkx_db = 0
                      LET l_yfkx_dy = 0
                      LET l_yfkx_cb = 0
                      LET l_yfkx_cy = 0
                   ELSE
                      EXECUTE q931_sysMon_prep1 USING l_c_s[i].tz_aag01,l_c_s[i].c_s INTO l_yfkx_db,l_yfkx_dy,l_yfkx_cb,l_yfkx_cy
                   END IF
             END CASE
          WHEN '2'    #子系统单据
             CASE l_c_s[i].tz_aag02
                ...
                WHEN '4'  #资产减值
                  #EXECUTE q931_sysMon_prep5 USING l_c_s[i].tz_aag01 INTO l_zcjz
                   CALL afar201(l_c_s[i].tz_aag01,'4') RETURNING l_zcjz
                WHEN '5'  #应付票据
                   #EXECUTE q931_sysMon_prep7 USING l_c_s[i].tz_aag01 INTO l_yfpj
                   EXECUTE q931_sysMon_prep7 USING l_c_s[i].tz_aag01,l_c_s[i].tz_aag01 INTO l_yfpj  #mod by cjy 20171016
                WHEN '6'  #应收票据
                   #EXECUTE q931_sysMon_prep6 USING l_c_s[i].tz_aag01 INTO l_yspj
                   EXECUTE q931_sysMon_prep6 USING l_c_s[i].tz_aag01,l_c_s[i].tz_aag01 INTO l_yspj  #mod by cjy 20171016
                WHEN '7'  #应收款项
                   CALL axrr151(l_c_s[i].tz_aag01,l_c_s[i].c_s) RETURNING l_ys_sysb,l_ys_sysy
                WHEN '8'  #应付款项
                   CALL aapr150(l_c_s[i].tz_aag01,l_c_s[i].c_s) RETURNING l_yf_sysb,l_yf_sysy
             END CASE
       END CASE
       ...
       SELECT aag02,aag06 INTO l_xx.kmm,l_aag06 FROM aag_file WHERE aag01 = l_c_s[i].tz_aag01
          AND aag00 = tm.a
       IF l_aag06 = '1' THEN
  ...
  #LET e_date =  MDY(tm.mm+1,1,tm.yy)-1
   ...
   FOREACH r151_curs1 INTO sr.*,l_omc02,l_oma16
     ...
     #No:120920   End  <<--yangjian---
      ...
      IF g_ooz.ooz07 = 'Y' AND l_count <> 0 AND sr.oma54t <> 0 THEN
#      IF g_ooz.ooz07 = 'Y' AND NOT cl_null(l_oma24_1) AND sr.oma54t <> 0 THEN  #TQC-B10083
         LET sr.oma56t = sr.oma54t * l_oma24_1                                 #CHI-830003
         CALL cl_digcut(sr.oma56t,g_azi04) RETURNING sr.oma56t                 #MOD-BB0050 add
      END IF                                                                   #CHI-830003
      ...
      IF g_ooz.ooz07 = 'Y' AND NOT cl_null(l_oma24_1) THEN                     #TQC-B10083 mod
         LET l_oob10 = l_oob09 * l_oma24_1                                     #CHI-830003
         CALL cl_digcut(l_oob10,g_azi04) RETURNING l_oob10                     #MOD-BB0050 add
      END IF                                                                   #CHI-830003
   ...
   IF g_apz.apz27 = 'N' THEN
      LET l_sql = "SELECT apa00,apa01,apa02,apa06,apa07,apc12,apa58,apc02,apc04,apa13,apa25,",   #MOD-880086 add apa08,apa58 #CHI-AB0019 add apc04 #CHI-AB0004 add apc02
                 ...
                 #" WHERE apa42 = 'N' AND apa74='N' ",   #非作廢&外購付款  #No.FUN-730064 #MOD-AB0159 mark
                  " WHERE apa42 = 'N' AND apa75='N' ",   #非作廢&外購付款  #No.FUN-730064 #MOD-AB0159
                  "   AND apa02 <='",e_date,"'",
                 ...
                 #"   AND ( (apa34f-apa35f) >0 OR ",   #TQC-610098 #MOD-720128 #MOD-C20191 mark
                  ...
                  "                   WHERE apf01=apg01 AND (apf00='33' OR apf00='34') ",   #MOD-820116
                  "                     AND apf41 = 'Y' ",
                  "                     AND apf02 >'",e_date,"' )  OR ",
      ...
      LET l_sql = l_sql," UNION ALL ",
                  "SELECT apa00,apa01,apa02,apa06,apa07,apc12,apa58,apc02,apc04,apa13,apa25,",   #MOD-880086 add apa08,apa58 #CHI-AB0019 add apc04 #CHI-AB0004 add apc02
                  "       apa44,apa54,'',(apc08-apc10-apc14),",   #TQC-610098   #MOD-720128  #TQC-B70203 add apc14
                  "       (apc09-apc11-apc15),apa74 ",    #TQC-610098   #MOD-720128 #CHI-850030 add apa74  #TQC-B70203 add acp15
                  ...
                  "   AND apa41 = 'Y'",   #已確認
                  "   AND ", l_wc CLIPPED,
                  "   AND apa01 IN ( SELECT UNIQUE apg04 FROM apf_file,apg_file WHERE apg01=apf01",
                  "   AND apf41 ='Y' AND apf00 = '36' AND apf02 > '",e_date,"')"
   ...
   ELSE
      LET l_sql = "SELECT apa00,apa01,apa02,apa06,apa07,apc12,apa58,apc02,apc04,apa13,apa25,",   #MOD-880086 add apa08,apa58 #CHI-AB0019 add apc04 #CHI-AB0004 add apc02
                 ...
                 #" WHERE apa42 = 'N' AND apa74='N' ",   #非作廢&外購付款 #MOD-AB0159 mark
                  " WHERE apa42 = 'N' AND apa75='N' ",   #非作廢&外購付款 #MOD-AB0159
                  "   AND apa02 <='",e_date,"'",
                  "   AND apa01 =apc01",   #No.MOD-9B0115
                  "   AND apa41 = 'Y'",   #已確認
                  "   AND ", l_wc CLIPPED,
                 #"   AND ( (apa34f-apa35f) >0 OR ",   #TQC-610098 #MOD-720128 #MOD-C20191 mark
                 #"   AND ( (apa34-apa35) >0 OR ",     #MOD-C20191 add
                  "   AND ( (apa34-apa35) !=0 OR ",    #No:140304   Modify  <<--yangjian--
                  "        apa01 IN (SELECT apg04 FROM apf_file,apg_file ",
                  "                   WHERE apf01=apg01 AND (apf00='33' OR apf00='34') ",   #MOD-820116
                  "                     AND apf41 = 'Y' ",
                  "                     AND apf02 >'",e_date,"' )  OR ",
      ...
      LET l_sql = l_sql," UNION ALL ",
                  ...
                  "       apa44,apa54,'',(apc08-apc10-apc14),",  #TQC-B70203 add apc14
                  "       apc13,apa74 ", #CHI-850030 add apa74
                  " FROM apa_file,apc_file", #CHI-850030 add apa74
                  " WHERE apa42 = 'N' AND apa75='N' ",                                           #非作廢&外購付款 #MOD-AB0159 #MOD-CA0154 mark
                  "   AND apa02 <='",e_date,"'",
                  "   AND apa01 =apc01",   #No.MOD-9B0115
                  "   AND apa41 = 'Y'",   #已確認
...
#----------------------#MOD-B60199 end-----------------------------------
     ...
     #IF sr.apa34f =0 THEN CONTINUE FOREACH END IF  #CHI-850030 add
      IF sr.apa34 =0 THEN CONTINUE FOREACH END IF  #CHI-850030 add
...
#       CALL r201_cal(sr.faj02,sr.faj022)
       ...
       #add by liumin 170214 str--
       #----推算至截止日期之資產數量
       #----調整(金額)
       SELECT SUM(fap661),SUM(fap10) INTO g_fap661,g_fap10 FROM fap_file
           WHERE fap03 IN ('9') AND fap02=sr.faj02
            AND fap021=sr.faj022 AND fap04 > g_edate
       ...
       IF cl_null(g_fap10) THEN LET g_fap10 = 0 END IF
         ...
         SELECT SUM(fap56) INTO g_fap561 FROM fap_file
          WHERE fap03 IN ('4','5','6','7','8','9') AND fap02=sr.faj02
            AND fap021=sr.faj022 AND fap04 > g_edate
       ...
       #----出售、報廢、銷帳(成本)
         ...
         #  INTO g_fap54,g_fap22,g_fap45,g_fap09             #No.TQC-680025  #CHI-850036 add fap09  #MOD-980171 #MOD-C90147 mark
          SELECT fap22+fap56,fap45,fap09                     #MOD-C90147 del fap54
            ...
            FROM fap_file
           #WHERE fap03 IN ('2','4','5','6') AND fap02=sr.faj02   #No.MOD-540112 #MOD-B70258 mark
            WHERE fap03 IN ('4','5','6') AND fap02=sr.faj02       #MOD-B70258
             AND fap021=sr.faj022
        ...
        #      WHERE fap03 IN ('4','5','6') AND fap02=sr.faj02   #No.MOD-540112  #MOD-A10024 mod
        #       AND fap021=sr.faj022
        ...
        #-------------------------MOD-C90147-----------------mark
         #本月尚未折舊時,抓取此財產之前的銷貨累折
          IF l_cnt = 0 THEN
             SELECT SUM(fap57) INTO g_fap57 FROM fap_file
               WHERE fap03 IN ('4','5','6') AND fap02=sr.faj02
                AND fap021=sr.faj022
                AND fap04 < g_bdate
          END IF
         #-MOD-B70258-end-
