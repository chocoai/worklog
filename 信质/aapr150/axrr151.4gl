...
# Modify.........: No.160817     16/08/17 By pulf 定金预收把之后出货冲账的数据剔除
# Modify.........: No.161118     16/11/18 By pulf 预收汇差本币显示问题
# Modify.........: No.170914     17/09/14 By pulf 调整No.160817时间取值问题
...
#No.170914 add begin----------
DEFINE ll_year    like type_file.chr10
DEFINE ll_month   like type_file.chr10
DEFINE ll_date    like type_file.dat
#No.170914 add end------------                                
...
#FUN-A10098---end 
   ...
   #No.170914 add begin----------
   LET ll_year = year(tm.e_date)
   LET ll_month = month(tm.e_date)
   LET ll_date=MDY(ll_month,1,ll_year)
   #No.170914 add end------------
   ...
   IF g_ooz.ooz07 = 'N' THEN
      LET l_sql = "SELECT oma00,oma01,oma02,oma03,oma032,oma23,",
             ...
             #     " FROM oma_file, OUTER aag_file,omc_file", #FUN-A10098  #No.160817 mark
                  " FROM oma_file,aag_file,omc_file",  #FUN-A10098         #No.160817 add
               ...
               #  "       oma01 IN (SELECT oob06 FROM ",l_dbs CLIPPED,"ooa_file,",l_dbs CLIPPED,"oob_file ",    #FUN-A10098
                ...
                # "       oma01 IN (SELECT oma19 FROM oma_file ",            #CHI-860040 add
                  ...
                  #No.160817 add begin---------
                  " UNION ",
                  "SELECT OMA00,OMA01,OMA02,OMA03,OMA032,OMA23,",
                  "       OMA33,OMA18,AAG02,oma51f,",
                  "      oma51,OMC02,OMA16 ",
                  " FROM oma_file,aag_file,omc_file",
                  " WHERE oma_file.oma18=aag_file.aag01",                                                                                  
                  "   AND aag_file.aag00 = '",g_bookno,"'",               #No.FUN-730073 
                  "   AND oma01 = omc01",                        #No.MOD-9B0115
                  "   AND omavoid = 'N'",                        #作廢否='N'                                                                           
                  "   AND omaconf = 'Y'",                                                                                      
              #    "   AND oma02 >'",tm.e_date,"' ",                 #No.170914 mark
                  "   AND oma02 >='",ll_date,"' ",                   #No.170914 add
                  "   AND OMAUD02 IN (SELECT DISTINCT OMAUD02   ",
                  "    FROM OMA_FILE WHERE OMAUD02 IN ",
                  "         (SELECT OMAUD02 FROM OMA_FILE WHERE OMA00 = '23' ",
              #    "             AND OMA02 <= '",tm.e_date,"' ",     #No.170914 mark
                  "             AND OMA02 < '",ll_date,"' ",         #No.170914 add
                  "             AND ", tm.wc CLIPPED,
                  "             ))",
     ...
     #已沖金額需加回這些超出截止日期單據的oma52,oma53
      IF sr.oma00 = '23' THEN                              
        ...
        #END IF                                                   #MOD-BB0120 add #MOD-C10005 mark
         #No.161118 add begin-------
         let l_oma53 = l_oma56t   #加入汇差后的本币应收
         #No.161118 add end---------
