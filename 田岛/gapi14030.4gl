# Prog. Version..: '5.30.27-18.04.25(00010)'     #
#
...
# Modify.........: No.MOD-GA0048 16/10/20 By Sabrina 在AFTER FIELD rvw07應判斷發票號碼是否存在apk_file
# Modify.........: No.MOD-GC0058 16/12/29 By Sabrina 修改FUN-E10053，將每段抓取ima021移到status後面抓取 
# Modify.........: No.FUN-I40010 18/04/16 By yihsuan 功能回追，調整：如果滿足aooi151 稅別差異不報錯；根據稅前金額一致更新金額稅額等數據
# Modify.........: No.MOD-I40011 18/04/25 By yihsuan s_compute_amt增加回傳p_price(單價)
# Modify.........: No.TQC-I40003 18/04/25 By yihsuan 輸入完新稅別後，若不存在aooi151對照表中，僅提示訊息不卡死
...
#                 LET g_cmd = "aapp111 '' '3' '4' '' '",g_head_1.rvw19,"' '' '' '",g_user,"' '",g_clas,"' '' 'N' '",g_head_1.rvw01,"'" #No.FUN-CB0053 Add #TQC-D90025 mark
        ...
        WHEN "change_cal_type" 
         IF cl_chk_act_auth() THEN
            CALL s_showmsg_init()
            CALL i140_change_cal_type()
            CALL s_showmsg()
         END IF 
        #--FUN-I40010 add end--
      ...
      AFTER FIELD rvw09
         IF NOT cl_null(g_rvw[l_ac].rvw09) THEN
            IF (p_cmd = 'a') OR
               ...
               CALL i140_rvw09()
               IF NOT cl_null(g_errno) THEN
                 #CALL cl_err(g_rvw[l_ac].rvw09,g_errno,0)   #TQC-I40003 mark
                  CALL cl_err(g_rvw[l_ac].rvw09,g_errno,1)   #TQC-I40003 add
                  IF g_errno <> 'aoo-114' THEN               #TQC-I40003 add
                     NEXT FIELD rvw09
                  END IF   #TQC-I40003 add
               END IF
               IF NOT cl_null(g_rvw[l_ac].rvw08) THEN                                                                               
                 #---------------------MOD-C90185-------------------(S)
      ...
      AFTER FIELD rvw10
	   ...
	   #               CALL cl_parse_qry_sql(g_sql,g_plant_new) RETURNING g_sql       #FUN-A50102            
           ...
           #CHI-EB0021---add---str---
            IF l_rvw10_t <> g_rvw[l_ac].rvw10 THEN   #MOD-F40062 add
               CALL s_compute_amt(g_rvw[l_ac].rvw10,g_rvw[l_ac].rvw17,g_rvv38t,g_rvv39,g_rvv39t,   #MOD-F10140 add g_rvv39
                        ...
                        RETURNING g_rvw[l_ac].rvw05f,g_rvw[l_ac].rvw06f,g_rvw[l_ac].rvw05,g_rvw[l_ac].rvw06,g_rvw[l_ac].sum1,g_rvw[l_ac].sum2
                                 ,g_rvw[l_ac].rvw17  #MOD-I40011 add
           ...
           #CHI-EB0021---add---end---
            ...
            DISPLAY BY NAME g_rvw[l_ac].sum1       #FUN-C80027  ADD
            DISPLAY BY NAME g_rvw[l_ac].sum2       #FUN-C80027  ADD
            DISPLAY BY NAME g_rvw[l_ac].rvw17      #MOD-I40011 add
         END IF
#MOD-B70073--end       
...
FUNCTION i140_rvw09()
   ...
   DEFINE l_n      LIKE type_file.num5    #FUN-I40010 add
   DEFINE l_rvv03  LIKE rvv_file.rvv03    #No.TQC-760013
      ...
      IF g_rvw03 != g_head_1.rvw03 THEN
        #--FUN-I40010 add start--
         ...
         SELECT COUNT(1) INTO l_n FROM hsc_file 
          WHERE hsc01=g_rvw03 
            AND hsc02 = g_head_1.rvw03 
            AND hsc03 <= g_head_1.rvw19 
            AND hscacti = 'Y'
         IF l_n > 0 THEN
         ELSE
        #--FUN-I40010 add end--
           #LET g_errno = 'tis-203' RETURN   #TQC-I40003 mark
            LET g_errno = 'aoo-114'          #TQC-I40003 add
         END IF   #FUN-I40010 add
      END IF
...
DEFINE
   ...
   DISPLAY ARRAY g_rvw TO s_rvw.* ATTRIBUTE(COUNT=g_rec_b,UNBUFFERED)
     ...
     #No.FUN-CB0048 ---end  --- Add
      ...
      #--FUN-I40010 add start--
      ON ACTION change_cal_type 
         LET g_action_choice= "change_cal_type"
         EXIT DISPLAY
      #--FUN-I40010 add end--
...
DEFINE l_rvw RECORD LIKE rvw_file.*,
       ...
       t_pmm04      LIKE pmm_file.pmm04,    #FUN-I40010 add 
       l_n          LIKE type_file.num5,    #FUN-I40010 add
   ...
   FOREACH i140_sel_rvb INTO l_rvw.rvw17,l_rvw.rvw10,l_rvw.rvw05f,
     ...
     #MOD-A30163---add---end---
      ...
      IF t_rvw03 != l_rvw.rvw03 THEN
         ...
         SELECT COUNT(1) INTO l_n FROM hsc_file 
          WHERE hsc01 = t_rvw03
            AND hsc02 = l_rvw.rvw03
            AND hsc03 <= l_rvw.rvw19 
            AND hscacti = 'Y'
         IF l_n > 0 THEN
         ELSE       
         #--FUN-I40010 add end--
           #LET g_errno = 'tis-203' CONTINUE FOREACH   #TQC-I40003 mark
            LET g_errno = 'aoo-114' CONTINUE FOREACH   #TQC-I40003 add
         END IF   #FUN-I40010 add
      END IF
...
#--FUN-I40010 add start--
FUNCTION i140_change_cal_type()
   DEFINE l_n      LIKE type_file.num5  
   DEFINE l_rvw    RECORD LIKE rvw_file.* 
   DEFINE l_rvw04  LIKE rvw_file.rvw04
   DEFINE l_rvw10  LIKE rvw_file.rvw10      
   DEFINE l_rvw05f LIKE rvw_file.rvw05f
   DEFINE l_rvw05  LIKE rvw_file.rvw05
   DEFINE l_rvw06f LIKE rvw_file.rvw06f
   DEFINE l_rvw06  LIKE rvw_file.rvw06
   ...
   ELSE
      FETCH i140_cl INTO g_head_1.rvw01,g_head_1.rvw07,g_head_1.rvw02,    #鎖住將被更改或取消的數據
                         g_head_1.rvw18,g_head_1.rvw19,                  
                         g_head_1.rvw03,g_head_1.rvw04,g_head_1.rvw11,
                         g_head_1.rvw12   #MOD-640405
                        ,g_head_1.rvwacti,g_head_1.rvwuser,g_head_1.rvwgrup,g_head_1.rvwmodu,
                         g_head_1.rvwdate,g_head_1.rvworiu,g_head_1.rvworig  
      IF SQLCA.sqlcode THEN                                         #資料被他人LOCK
      ...
      DECLARE i140_curs_c CURSOR FOR
         SELECT * FROM rvw_file
          WHERE rvw01 = g_head_1.rvw01 ORDER BY rvw08,rvw09
      LET g_success = 'Y'
      FOREACH i140_curs_c INTO l_rvw.*
         IF STATUS THEN
            CALL cl_err('i140_curs',STATUS,0)
            LET g_success='N'
            EXIT FOREACH
         ...
         #str 
         #因为 No.MOD-A70133       只取入库
         IF g_sma.sma116 MATCHES '[13]' THEN
            LET g_sql = "SELECT rvv87,rvu12,rvv38,rvv39,rvv38t,rvv39t", 
                        "  FROM ",cl_get_target_table(g_plant_new,'rvv_file'),",", 
                                  cl_get_target_table(g_plant_new,'rvu_file'),     
                        " WHERE rvv01 = '",l_rvw.rvw08,"' ",
                        "   AND rvv02 = '",l_rvw.rvw09,"' ",
                        "   AND rvv01 = rvu01 AND rvuconf != 'X' "  #排除作廢入庫單
            CALL cl_replace_sqldb(g_sql) RETURNING g_sql                   							
            CALL cl_parse_qry_sql(g_sql,g_plant_new) RETURNING g_sql                          
            PREPARE sel_rvu03_pre6c FROM g_sql #计价数量 税率 单价 金额 含税单价 含税金额
            EXECUTE sel_rvu03_pre6c INTO l_rvw10,l_rvw04,l_rvw17,l_rvw05f,l_rvv38t,l_rvv39t
         ELSE
            LET g_sql = "SELECT rvv17,rvu12,rvv38,rvv39,rvv38t,rvv39t  ", 
                        "  FROM ",cl_get_target_table(g_plant_new,'rvv_file'),",", 
                                  cl_get_target_table(g_plant_new,'rvu_file'),     
                        " WHERE rvv01 = '",l_rvw.rvw08,"' ",
                        "   AND rvv02 = '",l_rvw.rvw09,"' ",
            ...
            CALL cl_replace_sqldb(g_sql) RETURNING g_sql          				
            CALL cl_parse_qry_sql(g_sql,g_plant_new) RETURNING g_sql                          
            PREPARE sel_rvu03_pre7c FROM g_sql #数量 税率 单价 金额 含税单价 含税金额
            EXECUTE sel_rvu03_pre7c INTO l_rvw10,l_rvw04,l_rvw17,l_rvw05f,l_rvv38t,l_rvv39t
           
         END IF
         IF STATUS THEN
            LET g_errno = l_rvw.rvw08,l_rvw.rvw09
            CALL cl_err(g_errno,'asf-700',0)
            CONTINUE FOREACH              
         END IF
                 
         
         LET l_rvw05f  = l_rvw05f / l_rvw10  * l_rvw.rvw10
         LET l_rvw05f = cl_digcut(l_rvw05f,t_azi04)
         LET l_rvw05  = l_rvw05f * g_head_1.rvw12
         LET l_rvw05  = cl_digcut(l_rvw05,g_azi04)
         LET l_rvw06f = l_rvw05f * g_head_1.rvw04/100
         LET l_rvw06f = cl_digcut(l_rvw06f,t_azi04)
         LET l_rvw06  = l_rvw05 * g_head_1.rvw04/100
         LET l_rvw06  = cl_digcut(l_rvw06,g_azi04)
         LET l_rvw17= l_rvw05f/l_rvw.rvw10
         LET l_rvw17 = cl_digcut(l_rvw17,t_azi03)              
         
         UPDATE rvw_file SET rvw05=l_rvw05,
                           rvw06=l_rvw06,
                           rvw05f=l_rvw05f,
                           rvw06f=l_rvw06f,
                           rvw17=l_rvw17
      ...
      LET g_head_1.rvw05f_s = g_head_1.rvw05f_sum                   
      LET g_head_1.sum3=g_head_1.rvw05f_sum+g_head_1.rvw06f_sum     
      DISPLAY BY NAME g_head_1.rvw05f_s                             
      DISPLAY BY NAME g_head_1.rvw06f_s
      DISPLAY BY NAME g_head_1.rvw05f_sum,g_head_1.rvw06f_sum
