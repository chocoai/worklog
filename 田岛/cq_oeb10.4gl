...
DEFINE   ma_qry   DYNAMIC ARRAY OF RECORD
         ...
         oea10    LIKE oea_file.oea10,    #ADD by andy 2015/9/7 12:54:43
         oeb03    LIKE oeb_file.oeb03,
         oeb04    LIKE oeb_file.oeb04,
         oeb06    LIKE oeb_file.oeb06,          #TQC-C60093 add
         ima021   LIKE ima_file.ima021,         #TQC-C60093 add
         tc_hsc02 LIKE tc_hsc_file.tc_hsc02,    #add by andy 2015/12/28 9:17:53
         ima10    LIKE ima_file.ima10,          #add by andy 2015/11/24 22:14:42
...
FUNCTION cq_oeb10(pi_multi_sel,pi_need_cons,ps_cus,ps_oga01,ps_oga213,ps_oga211,
   ...
   OPEN WINDOW w_qry WITH FORM "cqry/42f/cq_oeb" ATTRIBUTE(STYLE="create_qry")  #No.FUN-660161
   CALL cl_ui_init()
   ...
   WHILE TRUE
      ...
      IF (li_reconstruct) THEN
         ...
         IF (mi_need_cons) THEN
            CONSTRUCT BY NAME ms_cons_where ON oea01,oea02,oea10,oeb03,oeb04,oeb06,ima021,ima10,oeb091,oeb12,  #TQC-C60093 add oeb06,ima021#add oea10 by andy 2015/9/7 12:56:21  #add ima10 by andy 2015/11/24 22:16:14
         ...
         CALL oeb_qry_prep_result_set_oeb10_c() 
      ...
      CALL oeb_qry_set_display_data_oeb10_c(li_start_index, li_end_index)
      ...
      IF (mi_multi_sel) THEN
         CALL oeb_qry_input_array_oeb10_c(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      ELSE
         CALL oeb_qry_display_array_oeb10_c(ls_hide_act, li_start_index, li_end_index) RETURNING li_continue,li_reconstruct,li_start_index
      END IF
...
# Modify        :
##################################################
FUNCTION oeb_qry_prep_result_set_oeb10_c()
   ...
   DEFINE   lr_qry     RECORD
            ...
            ima021     LIKE ima_file.ima021,         #TQC-C60093 add
            tc_hsc02 LIKE tc_hsc_file.tc_hsc02,   #add by andy 2015/12/28 9:17:53
            ima10      LIKE ima_file.ima10,          #add by andy 2015/11/24 22:17:46
            oeb091     LIKE oeb_file.oeb091,
            oeb12      LIKE oeb_file.oeb12,
            oeb24      LIKE oeb_file.oeb24,          #TQC-780032 add
            oeb25      LIKE oeb_file.oeb25,          #TQC-780032 add
   ...
   IF cl_null(ms_oga16) THEN   #MOD-8C0110
     #LET ls_sql = "SELECT 'N',oea01,oea02,oeb03,oeb04,oeb091,(oeb12-oeb24+oeb25),oeb24,oeb15",              #TQC-780032 mark #MOD-780285 modify -oeb24  #MOD-7B0195 +oeb25
      ...
      END IF
      #End:FUN-980030
      LET ls_sql = "SELECT 'N',oea01,oea02,oea10,oeb03,oeb04,oeb06,ima021,'',ima10,oeb091,oeb12,oeb24,oeb25,(oeb12-oeb24+oeb25),oeb15",  #TQC-780032 mod  #MOD-780285 modify -oeb24  #MOD-7B0195 +oeb25   #TQC-C60093 add oeb06,ima021#add oea10 by andy 2015/9/7 12:57:30
...
#FUN-990069---end  
   ...
   FOREACH lcurs_qry INTO lr_qry.*
      IF (SQLCA.SQLCODE) THEN
         ...
         EXIT FOREACH
      END IF
      SELECT tc_ima03 INTO lr_qry.tc_hsc02 FROM tc_ima_file WHERE tc_ima02 = lr_qry.oeb04 #add by andy 2015/12/28 9:30:19
      #-MOD-940130-add
...
# Modify        :
##################################################
FUNCTION oeb_qry_set_display_data_oeb10_c(pi_start_index, pi_end_index)
...
# Modify   	:
##################################################
FUNCTION oeb_qry_input_array_oeb10_c(ps_hide_act, pi_start_index, pi_end_index)
   ...
   INPUT ARRAY ma_qry_tmp WITHOUT DEFAULTS FROM s_oeb.* ATTRIBUTE(INSERT ROW=FALSE, DELETE ROW=FALSE,APPEND ROW=FALSE, UNBUFFERED) #FUN-880082
      ...
      ON ACTION prevpage
         ...
         CALL oeb_qry_reset_multi_sel_oeb10_c(pi_start_index, pi_end_index)
      ...
      ON ACTION accept
         IF ARR_CURR()>0 THEN        #CHI-9C0048
            ...
            CALL oeb_qry_reset_multi_sel_oeb10_c(pi_start_index, pi_end_index)
            IF cl_sure(0,0) THEN
               CALL oeb_qry_accept_oeb10_c(pi_start_index+ARR_CURR()-1)
            ELSE
               CLOSE WINDOW w_qry 
            END IF
         END IF                      #CHI-9C0048
...
# Modify   	:
##################################################
FUNCTION oeb_qry_reset_multi_sel_oeb10_c(pi_start_index, pi_end_index)
   DEFINE   pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_i             LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            li_j             LIKE type_file.num10 	#No.FUN-680131 INTEGER
...
# Modify   	:
##################################################
FUNCTION oeb_qry_display_array_oeb10_c(ps_hide_act, pi_start_index, pi_end_index)
   DEFINE   ps_hide_act      STRING,
            pi_start_index   LIKE type_file.num10, 	#No.FUN-680131 INTEGER
            pi_end_index     LIKE type_file.num10 	#No.FUN-680131 INTEGER
   DEFINE   li_continue      LIKE type_file.num5,  	#No.FUN-680131 SMALLINT
...
# Modify        :
##################################################
FUNCTION oeb_qry_refresh_oeb10_c()
   DEFINE   li_i   LIKE type_file.num10 	#No.FUN-680131 INTEGER
...
# Modify   	:
##################################################
FUNCTION oeb_qry_accept_oeb10_c(pi_sel_index)
   ...
   BEGIN WORK
      ...
      CALL s_showmsg_init() 
      #CALL q_oeb_chk_oeb10_c()  #mark by andy 2015/12/24 10:42:35 不检查订单的合理性
      IF g_success = 'Y' THEN 
      #-----END MOD-8C0110-----
         FOR li_i = 1 TO ma_qry.getLength()   
            IF (ma_qry[li_i].check = 'Y') THEN
...
FUNCTION oeb_qry_detail_oeb10_c()
...
#MOD-780285-begin-add
FUNCTION oeb_set_ogb917_oeb10_c()
...
#          END IF
          ...
          IF l_oga.oga909 = 'Y' THEN
             ...
             END IF
             #檢查流程代碼
             CALL q_oeb_chkpoz_oeb10_c(l_oga.*,ma_qry3[l_i].oea01) 
             ...
             IF ms_argv0 MATCHES '[456]' THEN #多角出貨    
                ...
                IF l_poz.poz011 = '2' THEN  #反拋方式
                   IF NOT q_oeb_last_oeb10_c(l_flow,ma_qry3[l_i].oea01) THEN
                      LET g_success = 'N'
                   END IF
                END IF
                #判斷銷售段 OR 代採段之訂單
...
FUNCTION q_oeb_chkpoz_oeb10_c(l_oga,p_ogb31)
...
FUNCTION q_oeb_last_oeb10_c(l_flow,l_oea01)
