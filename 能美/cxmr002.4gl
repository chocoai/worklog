...
################################################################################
   ...
   LET g_copies = '1'
WHILE TRUE
   CONSTRUCT BY NAME tm.wc ON oma01,oma02,oma03,omb04
       ON ACTION locale
  ...
  END CONSTRUCT
   ...
   INPUT BY NAME tm.b,tm.c,tm.d,tm.e
         WITHOUT DEFAULTS
      AFTER FIELD b
...
FUNCTION r002()
   DEFINE l_name    VARCHAR(20),        # External(Disk) file name
     ...
     #-->傳票分錄資料(npq_file)
      ...
      PREPARE r002_pnpq FROM l_sql
      IF SQLCA.sqlcode != 0 THEN
         CALL cl_err('r002_pnpq:',SQLCA.sqlcode,1)
      END IF
      DECLARE r002_cnpq CURSOR FOR r002_pnpq
...
REPORT r002_rep(sr)
 ...
 #No.MOD-560120 --begin
      ...
      # HEAD LINE 3
      CASE WHEN '11' PRINT g_x[57] CLIPPED,':',sr.oma.oma16;
