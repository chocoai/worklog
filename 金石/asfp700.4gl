...
DEFINE
    ...
    g_pmn           DYNAMIC ARRAY OF RECORD
         ...
         tc_cmy03   LIKE tc_cmy_file.tc_cmy03,    #add by qianyuan160905
...
#        subqty     LIKE ima_file.ima26,      #No.FUN-680121 DEC(12,3),
         ...
         amount     LIKE type_file.num15_3,   #add by aiqq160707  单重
...
#MOD-D40019---end   
      ...
      LET tm.pmm42=NULL
      LET tm.pmm01=NULL
      LET tm.pmm22='RMB'  #modify by aiqq160707
...
#        l_qty      LIKE ima_file.ima26     #No.FUN-680121 DEC(12,3)
  ...
  DEFINE l_sfb27    LIKE sfb_file.sfb27     #add by qianyuan160905
    ...
    #  LET l_sql = " SELECT ecm01,ecm45,ecm012,'',ecm03,sgd05,sga02,ecm03_par,ima02,ima021,'',0,ecm67,pmc03,0,0,",  #No.FUN-A60011  #FUN-A80102  #TQC-C70143 mark
       LET l_sql = " SELECT 'N',ecm01,ecm45,ecm012,'',ecm03,sgd05,sga02,ecm03_par,ima02,ima021,'','',0,ecm67,pmc03,0,0,",              #TQC-C70143     #,'' add by qianyuan160905
                     "0,",     #add by aiqq160707  
    ...
    #  LET l_sql = " SELECT ecm01,ecm45,ecm012,'',ecm03,'','',ecm03_par,ima02,ima021,'',0,ecm67,pmc03,0,0,",   #No.FUN-840240   #No.FUN-A60011  #FUN-A80102   #TQC-C70143 mark
       LET l_sql = " SELECT 'N',ecm01,ecm45,ecm012,'',ecm03,'','',ecm03_par,ima02,ima021,'','',0,ecm67,pmc03,0,0,",                #TQC-C70143    #,'' add by qianyuan160905
                     "0,",     #add by aiqq160707  
                     "   0,'',0,ecm321,ecm54,ecm322,ecm04,sfb06",   #No.FUN-550109    #No.FUN-A60011  #FUN-9A0047 add ecm04
    ...
    FOREACH p700_cur INTO g_pmn[g_cnt].*,l_ecm321,l_ecm54,l_ecm322,l_ecm04,l_sfb06   #No.FUN-A60011  #FUN-9A0047 add l_ecm04
       ...
       #----str----add by qianyuan160905-----------
       SELECT sfb27 INTO l_sfb27 FROM sfb_file WHERE sfb01=g_pmn[g_cnt].ecm01
       SELECT tc_cmy03 INTO g_pmn[g_cnt].tc_cmy03 FROM tc_cmy_file WHERE tc_cmy01=l_sfb27 AND tc_cmy02=g_pmn[g_cnt].ecm03_par
       #----end----add by qianyuan160905-----------
            ...
            IF g_pmn[l_ac].chk = 'Y' THEN    #TQC-C70143
               ...
               IF l_ac = 1 THEN 
                 FOR l_i = 1 TO g_pmn.getLength()
                     IF g_pmn[l_i].ecm45 = g_pmn[l_ac].ecm45 THEN
                       LET g_pmn[l_i].pmm09 = g_pmn[l_ac].pmm09
                       CALL p700_pmm09(l_i)
                        IF NOT cl_null(g_errno) THEN
                             CALL cl_err(g_pmn[l_i].pmm09,g_errno,0)
                             NEXT FIELD pmm09
                      END IF
                     END IF
                 END FOR 
...
FUNCTION p700_process()
  ...
  FOREACH p700_ins_pmm INTO g_tmp.*
    ...
    LET b_pmn.pmnud07 = g_tmp.amount   #add by aiqq160707
...
FUNCTION p700_pmm09(l_i)   #供應商check
   ...
   SELECT ima915 INTO l_ima915 FROM ima_file
    WHERE ima01 = g_pmn[l_i].ecm03_par
   ...
   IF l_ima915 = '2' OR l_ima915 = '3' THEN
      SELECT COUNT(*) INTO l_n FROM  pmh_file
       WHERE pmh01 = g_pmn[l_i].ecm03_par
         AND pmh02 = g_pmn[l_i].pmm09
         AND pmh22 = '2'
         #TQC-D70044--add--str--
         AND pmh05 = '0' 
         AND pmh13 = tm.pmm22
   ...
   END IF
   #TQC-D50092--add--end--
   LET g_pmn[l_i].pmc03 = g_pmc03
   DISPLAY g_pmn[l_i].pmc03 TO s_pmn[l_i].pmc03
 
   SELECT gec04,gec07 INTO g_pmn[l_i].gec04,g_gec07
     FROM gec_file WHERE gec01 = g_pmn[l_i].pmc47
...
FUNCTION p700_cre_tmp()
    ...
    CREATE TEMP TABLE asfp700_tmp(
       ...
       pmc03        LIKE pmc_file.pmc03,
       pmn20        LIKE pmn_file.pmn20,
       amount       LIKE type_file.num15_3,   #add by aiqq160707
       pmn31        LIKE pmn_file.pmn31,
