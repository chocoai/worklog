...
#   #LET g_sql = "SELECT SUM(nmh32-nmh17) FROM nmh_file",
#               " WHERE nmh26 = ? ",
#               "   AND nmh38 = 'Y' ",
#               "   AND YEAR(nmh04) =",tm.yy,
#               "   AND MONTH(nmh04) <=",tm.mm
   LET g_sql= " SELECT nvl(f.e,0)-nvl(a.c,0)-nvl(b.d,0) FROM  ",
             " (select sum(aph05) c from aph_file,apf_file where aph01=apf01 and year(apf02)= ",tm.yy,"  and month(apf02)<=",tm.mm,"  and  aph03='D') a , ",
             " ( select sum(npo06) d from npo_file,npn_file where npo01=npn01 and year(npn02)= ",tm.yy," and month(npn02)<=",tm.mm,"  and  npn03 in('4','6','7','8') ) b, ",
             " (select sum(nmh32) e  from nmh_file  WHERE nmh26 =  ?   ",
             " AND nmh38 = 'Y' ",
             " AND YEAR(nmh04) =",tm.yy,
             " AND MONTH(nmh04) <=",tm.mm,"）f "
   PREPARE q931_sysMon_prep6 FROM g_sql
   #-->2.7 应付票据
   IF tm.mm<10 THEN
   LET  l_date  =  tm.yy,"0",tm.mm
   else
   LET  l_date  =  tm.yy,tm.mm
   END IF
   LET l_date =  cl_replace_str(l_date,' ','')
   LET g_sql = "SELECT SUM(nmd26) FROM nmd_file ",
               " WHERE nmd23 = ? ",
               "   AND nmd30 = 'Y' ",
               #"   AND nmd12 <> '8' ",
               # add by lixwz180319 s---
               " AND ( nmd29 >last_day(to_date('",l_date,"','yyyymm')) OR nmd29 IS NULL)",
               # add by lixwz180319 e---
   ...
   LET g_sql = "SELECT SUM(nmd26)  FROM nmd_file ",
               ...
               " AND nmd30 = 'Y'",
               " AND ( nmd29 >last_day(to_date('",l_date,"','yyyymm')) OR nmd29 IS NULL)",
               " AND YEAR(nmd07) = ",tm.yy-1,
               " AND MONTH(nmd07) <= 12"
   prepare q931_sysMon_last_yer FROM g_sql
...
DEFINE l_qc    LIKE type_file.num20_6 # add by lixwz180319
          ...
          WHEN '1'    #子系统月结
             CASE l_c_s[i].tc_aag02
                ...
                WHEN '5'  #应付票据
                  ...
                  # add by lixwz180319 s---
                  EXECUTE q931_sysMon_last_yer USING l_c_s[i].tc_aag01 INTO l_qc
                  IF cl_null(l_qc) THEN LET l_qc =0  END IF
                  LET l_yfpj = l_yfpj + l_qc
                  LET l_qc =0
                  # add by lixwz180319 e---
                ...
                WHEN '6'  #应收票据
                 #  EXECUTE q931_sysMon_prep6 USING l_c_s[i].tc_aag01 INTO l_yspj
                  ...
                  SELECT nvl(f.e,0)-nvl(a.c,0)-nvl(b.d,0) into l_yspj FROM
                   (select sum(aph05) c from aph_file,apf_file where aph01=apf01 and year(apf02)= tm.yy
                   and month(apf02)<=    tm.mm  and  aph03='D') a ,
                   ( select sum(npo06) d from npo_file,npn_file where npo01=npn01 and year(npn02)=   tm.yy
                   and month(npn02)<=     tm.mm  and  npn03 in('4','6','7','8') ) b,
                   (select sum(nmh32) e  from nmh_file  WHERE nmh26 =  l_c_s[i].tc_aag01    AND nmh38 = 'Y'
                   AND YEAR(nmh04) =  tm.yy AND MONTH(nmh04) <=   tm.mm)f
                  # add by lixwz180319 s---
                  SELECT
                      SUM(AAH04)-SUM(AAH05)
                  INTO l_qc
                  ...
                  WHERE
                      AAH00   =   tm.a
                  AND AAH01   =   l_c_s[i].tc_aag01
                  AND AAH02   =   tm.yy-1
                  AND AAH03   <=   12
                  IF cl_null(l_qc) THEN LET l_qc =0  END IF
                  LET l_yspj = l_yspj + l_qc
                  LET l_qc =0
                  # add by lixwz180319 e---
          ...
          WHEN '2'    #子系统单据
             CASE l_c_s[i].tc_aag02
                ...
                WHEN '6'  #应收票据
                   ...
                   and month(apf02)<=    tm.mm  and  aph03='D') a ,
                   ( select sum(npo06) d from npo_file,npn_file where npo01=npn01 and year(npn02)=   tm.yy
                   and month(npn02)<=     tm.mm  and  npn03 in('6','7','8') ) b,
                   (select sum(nmh32) e  from nmh_file  WHERE nmh26 =  l_c_s[i].tc_aag01    AND nmh38 = 'Y'
                   AND YEAR(nmh04) =  tm.yy AND MONTH(nmh04) <=   tm.mm)f
                   # add by lixwz180319 s---
                   SELECT
...
IF g_plant='GLTP' THEN #150904wudj
ELSE #150904wudj
   ...
   IF g_apz.apz27 = 'N' THEN
      LET l_sql = "SELECT apa00,apa01,apa02,apa06,apa07,apc12,apa58,apc02,apc04,apa13,apa25,",   #MOD-880086 add apa08,apa58 #CHI-AB0019 add apc04 #CHI-AB0004 add apc02
                 ...
                 #"   AND ( (apa34f-apa35f) >0 OR ",   #TQC-610098 #MOD-720128 #MOD-C20191 mark
                  ...
                  "                   WHERE apf01=apg01 AND (apf00='32' OR apf00='33' OR apf00='34') ",   #MOD-820116
                  "                     AND apf41 = 'Y' ",
                  "                     AND apf02 >'",e_date,"' )  OR ",
