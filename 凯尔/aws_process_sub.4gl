# Prog. Version..: '5.30.23-16.12.05(00005)'     #
#
...
# Modify.........: No.FUN-F50005 15/05/13 By Lisa 新增傳入參數:傳輸的plm隊列主key
# Moidfy.........: No:FUN-FC0007 15/12/15 By Jessica 修正PLM優化程式
# Modify.........: No:FUN-G20003 16/02/18 By Jessica 修正Transation控制程式
# Modify.........: No:FUN-G90019 16/09/30 By Mars  修正PLM優化程式
...
DEFINE g_seqkey  LIKE wcg_file.wcg13     #傳輸的plm隊列主key   #FUN-F50005 add
DEFINE g_datakey LIKE wcf_file.wcf06     #傳輸的datakey        #FUN-F50005 add 
...
FUNCTION aws_process_p2(p_wcf06)              #FUN-FC0007 add p_wcf06 
  DEFINE p_wcf06    LIKE wcf_file.wcf06       #FUN-FC0007 add
  ...
  DEFINE l_cmd       STRING                   
  DEFINE l_status       STRING                #FUN-F50005 add
  DEFINE l_wcf17_cnt    LIKE type_file.num5   #FUN-F50005 add
  DEFINE l_wcf_process  LIKE type_file.num5   #FUN-F50005 add
  DEFINE l_msg1         LIKE type_file.chr100 #FUN-FC0007 add
  DEFINE l_msg2         LIKE type_file.chr100 #FUN-FC0007 add
  DEFINE l_tot_cnt      LIKE type_file.num5   #FUN-FC0007 add   
  DEFINE l_sql          STRING                #FUN-FC0007 add
  DEFINE l_wcf19        LIKE wcf_file.wcf19   #FUN-FC0007 add

   LET g_success = 'Y'   #FUN-FC0007 add
  ...
  #FUN-FC0007 add str---
   LET l_wcf19 = ''
   SELECT DISTINCT wcf19 INTO l_wcf19
     FROM wcf_file
    WHERE wcf06 = p_wcf06
   IF NOT cl_null(l_wcf19) THEN
      LET g_wc = g_wc CLIPPED," AND wcf19 = '",l_wcf19,"'"
   END IF
  #FUN-FC0007 add end---
  ...
  #FUN-FC0007 add str---
  #捉取目前欲處理的總筆數
   LET l_sql = "SELECT COUNT(wcf06) FROM wcf_file",
               ...
               "   AND ",g_wc,
               " ORDER BY wcf06"
   PREPARE data_cnt_pre FROM l_sql
   EXECUTE data_cnt_pre INTO l_tot_cnt
  ...
  #FUN-FC0007 add end---
   ...
   #LET g_seqkey =' '
   #LET g_seqkey = aws_ttsrv_getParameterGroup("seqkey")   #取由呼叫端呼叫時給予的 SQL Condition
   #IF cl_null(g_seqkey) THEN
   #   LET g_seqkey =' '
   #END IF
   #FUN-F50005 add end ---
   #FUN-G90019 mark end ---
   ...
   IF g_bgjob = 'Y' THEN
     ...
     #CALL aws_syslog("PLM","","3","aws_process_sub","",l_msg,"N",g_seqkey)  #執行紀錄寫入syslog   #FUN-F50005 add  #FUN-G90019 mark
      CALL aws_syslog("PLM","","3","aws_process_sub","",l_msg,"N",l_wcf19)   #執行紀錄寫入syslog   #FUN-G90019 add
   ELSE
     #CALL aws_syslog("PLM","","4","aws_process_sub","",l_msg,"N")  #執行紀錄寫入syslog
     #CALL aws_syslog("PLM","","4","aws_process_sub","",l_msg,"N",g_seqkey)  #執行紀錄寫入syslog   #FUN-F50005 add  #FUN-G90019 mark
      CALL aws_syslog("PLM","","4","aws_process_sub","",l_msg,"N",l_wcf19)   #執行紀錄寫入syslog   #FUN-G90019 add 
   END IF
   ...
   IF STATUS THEN 
      ...
      RETURN
   END IF
  #DECLARE aws_process_datakeycurs SCROLL CURSOR WITH HOLD FOR aws_process_datakeypre #FUN-FC0007 mark
   DECLARE aws_process_datakeycurs CURSOR WITH HOLD FOR aws_process_datakeypre        #FUN-FC0007 add
   IF STATUS THEN 
   ...
   CALL g_wcf.clear()
   LET l_cnt = 1
   IF l_tot_cnt > 0 THEN  #欲執行的總筆數大於0才跑，否則以下程式都不執行  #FUN-FC0007 add
 ...
 #FUN-FC0007 add str----------------- 
   ...
   IF g_success = 'Y' THEN
      FOR l_cnt = 1 TO l_tot_cnt
         LET l_msg1 = g_wcf[l_cnt].wcf06,'  ',g_wcf[l_cnt].wcf09,'  ','aws_process_p2_START:',CURRENT HOUR TO FRACTION(3)
         CALL plm_deb_logfile(l_msg1)
 #FUN-FC0007 add end------------------
         IF l_cnt = 1 THEN
             CALL aws_process_upd() 
             IF g_success = 'N' THEN 
               #EXIT FOREACH  #FUN-FC0007 mark
                EXIT FOR      #FUN-FC0007 add
             END IF
         END IF
         
        #依DataKey值找出相關wcf_file資料
         LET g_sql = "SELECT wcf01,wcf02,wcf03,wcflegal,wcfplant,wcf06,wcf07,wcf08,wcf09,wcf11,wcf17,wcf19 ",  #FUN-F50005 add wcf19
                     "  FROM wcf_file ",
                     " WHERE wcf06 = '",g_wcf[l_cnt].wcf06,"'",
                     " ORDER BY wcf19,wcf21,wcf08"  #FUN-F50005 add wcf19,wcf21
         ...
         FOREACH aws_process_curs INTO l_wcf[g_cnt1].wcf01,l_wcf[g_cnt1].wcf02,l_wcf[g_cnt1].wcf03,l_wcf[g_cnt1].wcflegal,l_wcf[g_cnt1].wcfplant,l_wcf[g_cnt1].wcf06,l_wcf[g_cnt1].wcf07,l_wcf[g_cnt1].wcf08,l_wcf[g_cnt1].wcf09,l_wcf[g_cnt1].wcf11,l_wcf[g_cnt1].wcf17,l_wcf[g_cnt1].wcf19  #FUN-F50005 add wcf19
           #FUN-F50005 add str--------
            IF STATUS THEN                                                              
            ...
            IF g_seqkey = " " OR cl_null(g_seqkey) THEN
               LET g_seqkey = l_wcf[g_cnt1].wcf19
            END IF   
            IF g_seqkey <> " " AND NOT(cl_null(g_seqkey)) THEN
              #同一隊列中若有WS已處理失敗則跳出不往下執行
               LET l_wcf_process = 0 
               SELECT COUNT(*) INTO l_wcf_process
                 FROM wcf_file
                WHERE wcf19 = g_seqkey
                  AND wcf17 = 'F'
               IF l_wcf_process > 0 THEN
                  CALL aws_process_sync_wcf17()
                  RETURN
               END IF
            END IF
           #FUN-F50005 add end--------
        ...
        #FUN-F50005 add start ---
         IF cl_null(l_description) AND g_status.code <> '0' THEN
            CALL cl_getmsg(g_wcf[l_cnt].wcf12,g_lang) RETURNING l_description
         END IF
        #FUN-F50005 add end ---
        
         LET g_transaction = 'N'   #呼叫syslog前資料庫是否為交易狀態
         LET l_msg = g_wcf[l_cnt].wcf09,":",l_description
        #CALL aws_syslog("PLM",g_wcf[l_cnt].wcf06,"3","aws_process_sub",g_status.code,l_msg,"N")  #執行紀錄寫入syslog           #FUN-F50005 mark
         CALL aws_syslog("PLM",g_wcf[l_cnt].wcf06,"3","aws_process_sub",g_status.code,l_msg,"N",g_seqkey)  #執行紀錄寫入syslog  #FUN-F50005 add
         ...
         IF g_success = 'Y' THEN 
            ...
            IF g_wcf[l_cnt].wcf12 <> '-1106' AND  g_wcf[l_cnt].wcf12 <> '-263'  THEN  #FUN-FC0007 add
           #FUN-F50005 add start ---
            IF NOT cl_null(g_seqkey) and g_seqkey <> " " THEN
               IF g_success = "Y" THEN
                  IF g_wcf[l_cnt].wcf17 = 'Y' THEN
                     LET l_status = "3" #執行成功
                  ELSE
                     LET l_status = "4" #執行失敗
                  END IF
              ...
              #捉取此隊列目前未處理完成的筆數
               LET l_wcf17_cnt = 0
               SELECT COUNT(*) INTO l_wcf17_cnt
                 FROM wcf_file
                WHERE wcf19 = g_seqkey
                  AND wcf17 IN ('S','N')
               IF l_wcf17_cnt = 0 OR g_wcf[l_cnt].wcf17 = 'F' THEN #此隊列皆已完成才呼叫PLM
                  LET g_datakey = g_wcf[l_cnt].wcf06               #此值在FUNCTION aws_process_plmcli裡用來寫入aws_syslog
                  CALL aws_syslog("PLM",g_datakey,"3","aws_process_p2","",'Before call aws_process_plmcli',"N",g_seqkey)
                  LET l_msg1 = g_wcf[l_cnt].wcf06,'  ',g_wcf[l_cnt].wcf09,'  ','aws_plmcli START    :',CURRENT HOUR TO FRACTION(3) #FUN-G90019 add
                  CALL plm_deb_logfile(l_msg1)  #FUN-G90019 add

                  CALL aws_process_plmcli(l_status,l_description
)
                  LET l_msg2 = g_wcf[l_cnt].wcf06,'  ',g_wcf[l_cnt].wcf09,'  ','aws_plmcli   END    :',CURRENT HOUR TO FRACTION(3) #FUN-G90019 add
                  CALL plm_deb_logfile(l_msg2)  #FUN-G90019 add
               END IF
            END IF
           #FUN-F50005 add end ---
            END IF  #FUN-FC0007 add
        
            CLOSE wcf_cl
         END IF
        
        #FUN-FC0007 add str---
        #LET l_msg2 = g_wcf[l_cnt].wcf06,'  ',g_wcf[l_cnt].wcf09,'  ','aws_process_p2_END  :',CURRENT HOUR TO FRACTION(3) #FUN-G90019 mark
         LET l_msg2 = g_wcf[l_cnt].wcf06,'  ',g_wcf[l_cnt].wcf09,'  ','aws_process_p2_END  :',CURRENT HOUR TO FRACTION(3),' wcf17:',g_wcf[l_cnt].wcf12  #FUN-G90019 add
         CALL plm_deb_logfile(l_msg2)
        #FUN-FC0007 add end---
        
        #LET l_cnt = l_cnt + 1 #FUN-FC0007 mark 
     #END FOREACH      #FUN-FC0007 mark
      END FOR       #FUN-FC0007 add
   END IF           #FUN-FC0007 add
 ...
 DEFINE l_parameter  om.DomNode
  ...
  #串接XML header/trailer
   IF l_wcf[g_cnt1].wcf07 <> 0 THEN  #判斷是否為最後一包
     ...
     #CALL aws_syslog("PLM",l_wcf[g_cnt1].wcf06,"3","aws_process_wcf10","",l_request.request,"N")           #FUN-F50005 mark
      CALL aws_syslog("PLM",l_wcf[g_cnt1].wcf06,"3","aws_process_wcf10","",l_request.request,"N",g_seqkey)  #FUN-F50005 add
...
FUNCTION aws_process_xmlbody()
  ...
  #CALL aws_syslog("PLM",l_wcf[g_cnt1].wcf06,"3","aws_process_xmlbody","",l_request.xmlbody,"N") #Lilan 0514           #FUN-F50005 mark
   CALL aws_syslog("PLM",l_wcf[g_cnt1].wcf06,"3","aws_process_xmlbody","",l_request.xmlbody,"N",g_seqkey) #Lilan 0514  #FUN-F50005 add
END FUNCTION
...
#FUN-F50005 add start ---
#[
# Description....: 呼叫PLM WS
# Date & Author..: 2015/05/13 by Lisa
# Parameter......: p_status - STRING
# Memo...........:
# Modify.........:
#]
FUNCTION  aws_process_plmcli(p_status,p_msg)
   DEFINE p_status       STRING
   DEFINE p_msg          STRING
   DEFINE l_msg          STRING
   DEFINE l_status       LIKE type_file.num10
   DEFINE l_sql          STRING  #FUN-F50005 add

   LET l_msg = ''
   LET l_status = -1

   IF p_status = '3' THEN
      CALL cl_getmsg('aps-525',g_lang)     #執行成功
      RETURNING l_msg
   ELSE
       IF p_status = '4' THEN
          CALL cl_getmsg('aps-528',g_lang) #執行失敗
          RETURNING l_msg
          IF NOT(cl_null(p_msg)) THEN 
             LET l_msg = l_msg ,',',p_msg
          END IF 
       END IF
   END IF

   CALL cl_replace_str(l_msg, ",","，") RETURNING l_msg
   CALL aws_syslog("PLM",g_datakey,"3","aws_process_plmcli","",'Before call aws_plmcli',"N",g_seqkey)
   CALL aws_plmcli(g_seqkey,'tiptop',p_status,l_msg)
   RETURNING l_status
   CALL aws_syslog("PLM",g_datakey,"3","aws_process_plmcli","",'After call aws_plmcli',"N",g_seqkey)
   CALL aws_syslog("PLM",g_datakey,"3","aws_process_plmcli_status","",l_status,"N",g_seqkey)

   IF l_status = 0 THEN   #PLM return true
      CALL aws_syslog("PLM",g_datakey,"3","aws_process_plmcli","",'Before call aws_del_plm_temptable_data',"N",g_seqkey)
      CALL aws_del_plm_temptable_data(g_seqkey)  #FUN-G90019 add
      CALL aws_syslog("PLM",g_datakey,"3","aws_process_plmcli","",'After call aws_del_plm_temptable_data',"N",g_seqkey) 
   END IF

END FUNCTION

#若有一筆WS失敗則同步更新wcf_file的其它未被處理的資料狀態碼為F 
FUNCTION aws_process_sync_wcf17()
    DEFINE l_sql    STRING
    BEGIN WORK  #FUN-G20003 add

   #執行作業時，會先將全部的資料狀態update成S
    LET l_sql = "UPDATE wcf_file",
                "   SET wcf17 = 'F' ",
                " WHERE wcf17 in ('S','N') ",
                "   AND wcflegal = '",g_legal,"'",
                "   AND wcfplant = '",g_plant,"'",
