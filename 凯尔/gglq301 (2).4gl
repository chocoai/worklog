...
# Modify.........: No.MOD-D40017 13/04/27 By yinhy 本期本幣餘額修正
   ...
   IF g_bgjob = 'Y' THEN
     ...
     #LET mm = MONTH(tm.bdate)
      IF NOT cl_null(tm.bookno) THEN 
         LET yy = s_get_aznn(g_plant,tm.bookno,tm.bdate,1)
         LET mm = s_get_aznn(g_plant,tm.bookno,tm.bdate,3)
      ELSE
         CALL s_get_bookno(YEAR(tm.bdate)) RETURNING g_flag,g_bookno1,g_bookno2
         LET yy = s_get_aznn(g_plant,g_bookno1,tm.bdate,1)
         LET mm = s_get_aznn(g_plant,g_bookno1,tm.bdate,3)
      END IF 
     #FUN-D70119--mod--end--
   END IF
   #TQC-CC0122--add--end--
...
FUNCTION gglq301_tm()
   ...
   DEFINE l_bookno1        LIKE aza_file.aza81   #FUN-D70119
   DEFINE l_bookno2        LIKE aza_file.aza82   #FUN-D70119
   DEFINE l_bookno3        LIKE aza_file.aza81   #FUN-D70119
   DEFINE l_bookno4        LIKE aza_file.aza82   #FUN-D70119
       ...
       INPUT BY NAME yy,tm.bdate,tm.edate,tm.a,tm.d1,tm.e,tm.n,tm.h,tm.lvl,   #FUN-C80102 add yy del t,k,curr
         ...
         AFTER FIELD edate
            ...
            ELSE 
              ...
              #IF YEAR(tm.bdate) <> YEAR(tm.edate) THEN NEXT FIELD edate END IF
               IF NOT cl_null(tm.bookno) THEN
                  IF s_get_aznn(g_plant,tm.bookno,tm.bdate,1) <> s_get_aznn(g_plant,tm.bookno,tm.edate,1) THEN
                     NEXT FIELD edate 
                  END IF  
               ELSE 
                  CALL s_get_bookno(YEAR(tm.bdate)) RETURNING g_flag,l_bookno1,l_bookno2
                  CALL s_get_bookno(YEAR(tm.edate)) RETURNING g_flag,l_bookno3,l_bookno4
                  IF s_get_aznn(g_plant,l_bookno1,tm.bdate,1) <> s_get_aznn(g_plant,l_bookno3,tm.edate,1) THEN
                     NEXT FIELD edate 
                  END IF
               END IF
              #FUN-D70119--mod--end-- 
...
#No.FUn-A40009 --end
     ...
     #LET nn1 = MONTH(tm.edate)
      IF NOT cl_null(tm.bookno) THEN
         LET mm1 = s_get_aznn(g_plant,tm.bookno,tm.bdate,3)
         LET nn1 = s_get_aznn(g_plant,tm.bookno,tm.edate,3)
      ELSE
         CALL s_get_bookno(YEAR(tm.bdate)) RETURNING g_flag,g_bookno1,g_bookno2
         LET mm1 = s_get_aznn(g_plant,g_bookno1,tm.bdate,3)
         CALL s_get_bookno(YEAR(tm.edate)) RETURNING g_flag,g_bookno1,g_bookno2
         LET nn1 = s_get_aznn(g_plant,g_bookno1,tm.edate,3)
      END IF
      #FUN-D70119--mod--end--
...
#FUN-D10072-130123--mod
   ...
   FOREACH q301v_aag1_dec2 INTO sr1.*
     ...
     #LET l_abb07f = sr0.qc_mc + sr0.qc_yc  #MOD-E20099 mark
      ...
      INSERT INTO q301_temp2
         VALUES(sr0.aea05,sr0.aag02,sr0.curr,sr0.aba04,'1',tm.bdate,'','','',sr0.abb24,
      ...
      FOR i = mm1 TO nn1
         ...
         INSERT INTO q301_temp2
            VALUES(sr0.aea05,sr0.aag02,sr0.curr,sr0.aba04,'3',l_date1,'','','',sr0.abb24,
         ...
         INSERT INTO q301_temp2
            VALUES(sr0.aea05,sr0.aag02,sr0.curr,sr0.aba04,'4',l_date1,'','','',sr0.abb24,
  ...
  #IF tm.k = '3' THEN  #FUN-D10072--130123
   IF tm.k = '2' AND tm.b = 'N' THEN  #FUN-D10072--130123
     #IF tm.d1 = 'Y' THEN   #MOD-E20099--mark
      IF tm.n = 'Y' THEN   #MOD-E20099--add
         LET l_sql = l_sql,"SELECT aag01,aag02,abb24_temp,aba04,'2',aea02,aea03,aea04,abb04,",
      ...
      ELSE
         LET l_sql = l_sql,"SELECT aag01,aag02,'",g_aza.aza17,"',aba04,'2',aea02,aea03,aea04,abb04,",
                          #"       abb24,abb06,abb07,abb07f,0,0,aag07",  #MOD-E20099 mark
                           "       abb24,abb06,abb07,abb07f,0,0,0,0,aag07",  #MOD-E20099 add
   ...
   ELSE
     #IF tm.d1 = 'Y' THEN   #MOD-E20099--mark
      IF tm.n = 'Y' THEN   #MOD-E20099--add
         LET l_sql = l_sql,"SELECT aag01,aag02,abb24_temp,aba04,'2',aba02,abb01,abb02,",
      ...
      ELSE
         LET l_sql = l_sql,"SELECT aag01,aag02,'",g_aza.aza17,"',aba04,'2',aba02,abb01,abb02,",
                          #"       abb04,abb24,abb06,abb07,abb07f,0,0,aag07",  #MOD-E20099 mark
                           "       abb04,abb24,abb06,abb07,abb07f,0,0,0,0,aag07",  #MOD-E20099 add
...
#MOD-E20099---end----------
#FUN-D10072--130123--mod
   PREPARE gglq301_prepare3 FROM l_sql
   ...
   IF tm.a = 'N' THEN
      IF tm.d1 = 'N' THEN
        #LET g_sql = "SELECT aea05,aag02,aea02,aea03,abb04,abb24,abb07f,0,abb07,abb07f,0,abb07,'',balf,0,bal,abb06,type FROM q301_temp2",  #MOD-E20099 mark
         LET g_sql = "SELECT aea05,aag02,aea02,aea03,abb04,abb24,abb07f_d,0,abb07_d,abb07f_c,0,abb07_c,'',balf,0,bal,abb06,type FROM q301_temp2",  #MOD-E20099 add
...
#MOD-E20099--end-----mark----
         ...
         #LET g_abbv[g_cnt].bal = l_d - l_c + l_n       #MOD-D40017 mark
         LET g_abbv[g_cnt].bal = l_d1 - l_c1 + l_n      #MOD-D40017
         LET l_d1 = 0
         LET l_c1 = 0
         #MOD-E20099---begin
     ...
     ELSE #No:FUN-C70061 add
        IF tm.d1 = 'Y' THEN 
          #LET g_sql = "SELECT UNIQUE abb24 FROM gglq301_tmp ", #No:FUN-C70061 add  #MOD-D60128 mark
           LET g_sql = "SELECT UNIQUE abb24 FROM q301_temp2 ",  #MOD-D60128
                       " WHERE ",tm.wc1 CLIPPED,
                       "   AND ",tm.wc2 CLIPPED,  #FUN-C80102
                       "  INTO TEMP x "  #No:FUN-C70061 add
        END IF
...
FUNCTION q301_out_2()
   DEFINE l_name              LIKE type_file.chr20
   DEFINE l_cnt   LIKE type_file.num5   #170821 add
  ...
  #FUN-C70061--mod--str
   LET g_sql = " SELECT aea05, aag02, aba04, type , aea02, ",
               "        aea03, aea04, abb04, abb24, abb06,abb07, abb07f,  d , df, abb25_d, ",
