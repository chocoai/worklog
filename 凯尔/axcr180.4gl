...
# Modify.........: No:MOD-D60117 13/06/14 By bart mark IF sr1.ccc92 = 0 THEN CONTINUE FOREACH END IF
# Modify.........: No:FUN-D70127 13/08/07 By wangrr 會計年期會不按照自然年月設置,年期的判斷需要按照aooq011的設置來
# Modify.........: No:MOD-F30086 15/03/16 By Alberti 報表新增ima12(成本分群)
...
GLOBALS "../../config/top.global"
   ...
   DEFINE tm  RECORD				# Print condition RECORD
             ...
             #p9            LIKE cre_file.cre08,           #No.FUN-680122CHAR(10)           # 工廠別編號    #FUN-A70084
              ...
              a             LIKE type_file.chr1,           # Prog. Version..: '5.30.27-19.04.12(01)           # 排列順序
              b             LIKE type_file.chr1,           # Prog. Version..: '5.30.27-19.04.12(01)           # 是否產生呆滯料檔(N/Y)
              ...
              more  LIKE type_file.chr1,           # Prog. Version..: '5.30.27-19.04.12(01)                    # 特殊列印條件
...
DEFINE g_flag       LIKE type_file.chr1,#FUN-D70127
...
MAIN
  ...
  ## *** 與 Crystal Reports 串聯段 - <<<< 產生Temp Table >>>> CR11 *** ##
   LET g_sql = "ima01.ima_file.ima01,", 
               "ima12.ima_file.ima01,",   #MOD-F30086 add
   ...
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,
               #" VALUES(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?)"   #MOD-F30086 mark
                " VALUES(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?)" #MOD-F30086 add
...
#No.FUN-A10098 ----add start
    ...
    ima12   LIKE ima_file.ima12,            #MOD-F30086 add
   ...
   DEFINE 
	  ...
	  l_date     LIKE type_file.dat,           #No.FUN-680122DATE
          sr         RECORD
                     ...
                     ima12    LIKE  ima_file.ima12, #成本分群 #MOD-F30086 add
...
DEFINE l_unt     LIKE cck_file.cck06   #NO.MOD-720042 07/02/14 By TSD.Sideny
    ...
    #FOR i=1 TO 9    #FUN-A70084
     ...
     # 假設 同一料件的單位相同
    #LET l_sql="SELECT ima01,sum(imk09),sum(ccc92), ",            #MOD-F30086 mark
     LET l_sql="SELECT ima01,MAX(ima12),sum(imk09),sum(ccc92), ", #MOD-F30086 add
     ...
     FOREACH tmp_cl INTO sr.*
      ...
      #   sr.delay >= tm.q and sr.delay <= tm.r                          #MOD-BC0040 mark
       IF (sr.delay >= tm.e AND sr.delay <= tm.i) OR                     #MOD-BC0040 add
        ...
        #EXECUTE insert_prep USING sr.ima01,sr.imk09,m_ccc92,sr.d_date,           #MOD-F30086 mark
         EXECUTE insert_prep USING sr.ima01,sr.ima12,sr.imk09,m_ccc92,sr.d_date,  #MOD-F30086 add
...
FUNCTION r180_foreach(l_plant_new)  #FUN-A70084 
     DEFINE
      ...
      #CALL s_yp(tm.s_date) RETURNING l_year,l_mon #FUN-D70127 mark
      CALL s_get_bookno(YEAR(tm.s_date)) RETURNING g_flag,g_bookno1,g_bookno2 #FUN-D70127
      LET l_year=s_get_aznn(l_plant_new,g_bookno1,tm.s_date,1) #FUN-D70127
      LET l_mon =s_get_aznn(l_plant_new,g_bookno1,tm.s_date,3) #FUN-D70127
     ...
     #LET l_sql = " SELECT ima01,ccc91,ima9021,ccc92,ccc91,ima02,ima021,ccc08,ima901 ", #MOD-D40147  #MOD-F30086 mark
      LET l_sql = " SELECT ima01,ima12,ccc91,ima9021,ccc92,ccc91,ima02,ima021,ccc08,ima901 ",        #MOD-F30086 add
#No.FUN-A10098 ---------mark start
...
#No.FUN-A10098 ---------add end 
      ...
      END IF 
       ...
       LET l_sql= "INSERT INTO ",
                 #"axcr180_temp(ima01,imk09,d_date,ccc92,ccc91,ima02,ima021,ccc08 )", #FUN-7C0101 ADD ccc08   #No.MOD-8B0229 add ima021   #MOD-F30086 mark
                 #     "VALUES ( ?    ,?    ,?    ,?    ,?    ,?   ,?   ,?)"       #FUN-7C0101    #No.MOD-8B0229 add ?                    #MOD-F30086 mark
                  "axcr180_temp(ima01,ima12,imk09,d_date,ccc92,ccc91,ima02,ima021,ccc08 )", #MOD-F30086 add
                       "VALUES ( ?    ,?   ,?    ,?     ,?    ,?    ,?   ,?   ,?)"          #MOD-F30086 add 
...
#No.FUN-A10098 ----add end   
      ...
      #CALL cl_parse_qry_sql(l_sql,l_plant_new) RETURNING l_sql                 #FUN-A70084  #TQC-BB0182 mark
       ...
       #求異動量
       #LET l_datex = MDY(MONTH(tm.s_date),1,YEAR(tm.s_date)) #FUN-D70127 mark
       LET l_datex = MDY(l_mon,1,l_year) #FUN-D70127
...
#                 "  FROM ",l_dbs_new clipped,"ccc_file ",              #FUN-A10098 ---mark
                   ...
                   " WHERE ccc01 = ? ",
                  #"   AND ccc02 = ",YEAR(tm.s_date), #FUN-D70127 mark
                  #"   AND ccc03= ",MONTH(tm.s_date), #FUN-D70127 mark
                   "   AND ccc02= ",l_year,           #FUN-D70127
                   "   AND ccc03= ",l_mon,            #FUN-D70127
...
#                   '|',sr1.ccc92                                                  #No.FUN-A10098 ---mark
        CALL ui.Interface.refresh()
        #IF sr1.ccc92 = 0 THEN CONTINUE FOREACH END IF  #MOD-D60117
