...
# Modify.........: NO:yinhy140507 14/05/07 By yinhy apz27為Y並且賬款幣種不為本位幣時，apc13賦值錯誤
...
DEFINE  l_zz           LIKE type_file.num10  #dengsy150914
...
#No.FUN-A60024 --end
       END IF
          ...
          IF g_aptype = '32' OR g_aptype = '33' THEN
             ...
             #CALL cl_set_comp_att_text("aph03_2","银行账户")  #add by dengsy150914
            ...
            CASE
               ...
               WHEN INFIELD(apf03) #PAY TO VENDOR
                  ...
                  ELSE #CALL q_gen(0,0,g_apf.apf03) RETURNING g_apf.apf03
                     ...
                     CALL cl_create_qry() RETURNING g_qryparam.multiret
                  END IF
                  #END IF   #FUN-DA0051 add  #FUN-DA0094 mark
...
FUNCTION t310_menu()
   ...
   DEFINE l_cmd       LIKE type_file.chr1000     #dengsy150914 add
...
#str--- add by dengsy150914
        ...
        WHEN "yingfu"
          ...
          IF l_zz = 0 THEN
              LET l_zz = 1
          END IF
            IF g_aph[l_zz].aph03 ='1' THEN
              LET l_cmd = "anmt100 '' '' '",g_apf.apf01,"' "
        ...
        WHEN "yingshou"
          ...
          IF l_zz = 0 THEN
              LET l_zz = 1
          END IF
            IF g_aph[l_zz].aph03 ='D' THEN
              LET l_cmd = "anmt200 '",g_aph[l_zz].aph04,"' "
              CALL cl_cmdrun(l_cmd)
            END IF
          END IF
#end--- add by dengsy150914
      END CASE
   END WHILE
END FUNCTION
...
FUNCTION t310_a()
   ...
   LET g_apf.apf14='1101'  #dengsy150914 add
   ...
   WHILE TRUE
      ...
      IF g_apf_t.apf02 != g_apf.apf02 THEN
         ...
         IF g_nmz.nmz70 = '3' THEN
            LET l_tic01=YEAR(g_apf.apf02)
            LET l_tic02=MONTH(g_apf.apf02)
...
#TQC-C50108--add--end
#DEFINE l_occ02_pmc03 LIKE occ_file.occ02  #FUN-DA0051#FUN-DA0094 mark
      ...
      ELSE
         LET g_sql = "SELECT 0,apa01,apc02,apa02,apa13,apa14,", #TQC-9B0119
                   ...
                   #           g_dbs_new CLIPPED,"apc_file ",                                                                                         #FUN-A50102 mark
                     ...
                     #"   AND (apc08 >(apc10+apc16+apc14) OR apc13>(apc16*apc07))", #MOD-B90084 # mark by lixwz180628
                     "  AND (apc08 > (apc10 + apc16 + apc14) OR apc09 +(select oox10 from oox_file,apz_file where oox03 =apa01 and oox01=apz21 and oox02 =apz22) > (apc11 + apc15) )",# add by lixwz180628
#                     "   AND apa00 LIKE '1%'",         #No.FUN-A40003
                     "   AND apa00 <> '16'", #No.MOD-7B0115
                     "   AND apa41 = 'Y' AND apa42 = 'N'",
                     "   AND apa13 = azi_file.azi01"
...
#                  "    AND apa00[1,1] = '1'",                         #No.FUN-A40003
   ...
   ELSE
      ...
      LET g_sql = #"SELECT apa02,apa13,apa72,apc08-apc10-apc16-apc14,apc13-apc16*apa72,",  #MOD-B90084  # amrk by lixwz180628
                  "SELECT apa02,apa13,apa72,apc08-apc10-apc16-apc14,apc13-(select oox10 from oox_file,apz_file where oox03 =apa01 and oox01=apz21 and oox02 =apz22)-apc16*apa72,", # add by lixwz180628
                  " apa06,apa07,apa21,apa22,apa13,apa00,apa41,apa42,apa08,apa44 ",
                # "  FROM ",g_dbs_new,"apa_file, ",                           #FUN-A50102 mark
                #           g_dbs_new,"apc_file  ",                           #FUN-A50102 mark
                  "  FROM ",cl_get_target_table(g_plant_new,'apa_file'),",",  #FUN-A50102
...
DEFINE l_npo04  LIKE npo_file.npo04 #add by wy20180522
DEFINE l_npo05  LIKE npo_file.npo05 #add by wy20180522
            ...
            #No.MOD-BC0242 --end
             ...
             #IF (g_aptype <> '32' AND g_aptype !='36') OR (g_aptype = '36' AND g_apz.apz68 = 'Y') THEN   #CHI-CA0044 mark  #FUN-C90027 add
              IF (g_aptype !='36') OR (g_aptype = '36' AND g_apz.apz68 = 'Y') THEN   #CHI-CA0044
                 #IF g_aph[l_ac].aph03 NOT MATCHES "[6789ABDFGZX]"  AND g_aph[l_ac].aph03 != g_aph_t.aph03 THEN      #FUN-B40011 add D #FUN-C90122 Add F #FUN-CB0065 add G
...
#No.FUN-B40011--begin
             ...
             #       AND aph01!= g_apf.apf01
                  ...
                  #add by wy20180522 b 要减去转付退票的金额
                  SELECT sum(npo04),sum(npo05) INTO l_npo04,l_npo05 FROM npn_file,npo_file
                  where npn01=npo01 and npn03='9' and npo03=g_aph[l_ac].aph04 and npnconf='Y' #add bby wy20180522 e
                  IF cl_null(l_npo04) THEN LET l_npo04=0 END IF
                  IF cl_null(l_npo05) THEN LET l_npo05=0 END IF
                  #add bby wy20180522 e
                  IF cl_null(l_sum_aph05f) THEN LET l_sum_aph05f= 0 END IF
                  IF cl_null(l_sum_aph05)  THEN LET l_sum_aph05 = 0 END IF
                  IF cl_null(g_aph_t.aph05f) THEN LET g_aph_t.aph05f = 0 END IF

                  #LET l_sum_aph05f = l_sum_aph05f - g_aph_t.aph05f #mark by wy20180522
                  LET l_sum_aph05f = l_sum_aph05f - g_aph_t.aph05f -l_npo04 #add by wy20180522
...
#No.TQC-B80079 --end
  ...
  #LET l_apa.apa34 = m_apf.apf10 - m_apf.apf08   #MOD-B10116 mark
   ...
   LET l_apa.apa12=l_apa.apa02 #muping151021
   LET l_apa.apa64=l_apa.apa02 #muping151021
...
#No.FUN-A30106 --end
      ...
      ON ACTION handling_differences
         ...
         EXIT DIALOG
      #CHI-AA0003 add --end--#str--- add by dengsy150914
      ON ACTION yingshou
         LET g_action_choice="yingshou"
         EXIT DIALOG

      ON ACTION yingfu
         LET g_action_choice="yingfu"
         EXIT DIALOG
#end--- add by dengsy150914
   END DIALOG
...
FUNCTION t310_out(p_cmd)
   ...
   LET l_cmd = "aapr311",  #FUN-C30085 mark  #radd by pulf 141225
   #LET l_cmd = "aapg311",  #FUN-C30085 add  #rmk by pulf 141225
               " '",g_today CLIPPED,"' ''",
               " '",g_lang CLIPPED,"' 'Y' '' '1' '",
         ...
         # LET g_sql = "UPDATE ",g_dbs_new CLIPPED,"apc_file ",              #FUN-A50102 mark
          ...
          #yinhy131210   --Begin
           ...
           ELSE
              ...
              LET l_apc13 = l_apc13 - l_apc15                        #yinhy140507
           END IF
           #LET l_apc13 = l_apc13 - l_apc15                        #MOD-C20202 add  #yinhy140507 mark
...
FUNCTION t310_set_entry_b(p_cmd)
  ...
  #    CALL cl_set_comp_entry("aph09",TRUE)   #FUN-B40011  #TQC-B70152 mark
     # CALL cl_set_comp_entry("aph09",TRUE)    #yinhy130923   #mark by dengsy150914
...
#No.FUN-A40003 --begin
      IF g_aptype ='32' THEN
        #CALL cl_set_comp_entry("aph07,aph13,aph14",FALSE) #MOD-DC0067     #mark by dengsy151012
...
#No.FUN-B40011 --begin
    IF g_aptype='33' THEN
       #LET comb_value = '1,2,3,4,5,6,7,8,9,A,B,C,D,E,F,G,H,Z'   #mark by dengsy150914
      # LET comb_value = 'D,1,2,4,5,6,7,8,9,G'#add by dengsy150914  #mark by liyjf170829
       LET comb_value = 'D,1,2,4,5,6,7,8,9,G,F'#add by liyjf170829
       SELECT ze03 INTO comb_item FROM ze_file
        #WHERE ze01='aap-355' AND ze02=g_lang  #mark by dengsy150914
        WHERE ze01='cap-355' AND ze02=g_lang  #add by dengsy150914
