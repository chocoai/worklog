...
FUNCTION t620sub_y_chk(p_argv,p_sfu01,p_action_choice) #CHI-C30118 add p_action_choice
  ...
  #DEFINE g_ecm_out  LIKE type_file.num5  #MOD-BB0295
   ...
   DEFINE l_slip     LIKE smy_file.smyslip    #add by lili 170622
   DEFINE l_smyud04  LIKE smy_file.smyud04    #add by lili 170622
   DEFINE l_shb114   LIKE shb_file.shb114     #add by lili 170622
  ...
  #TQC-C90031--add--end--
      ...
      SELECT SUM(sfv09) INTO tmp_qty FROM sfv_file,sfu_file
       WHERE sfv11 = l_sfv.sfv11
         ...
         AND sfuconf <> 'X'
         AND sfv01 not like (select smyslip from smy_file where smyud04 = 'Y')||'%'   #add by lili 170627

      IF tmp_qty IS NULL OR SQLCA.sqlcode THEN LET tmp_qty=0 END IF
      LET g_min_set = 0
    # CALL s_minp(l_sfv.sfv11,g_sma.sma73,0,'','','')               #FUN-C70037 mark
      IF g_prog = 'asft620' THEN LET g_sma.sma73 = 'Y' END IF    #add by lili 170531
      ...
      END IF
      #No.TQC-BB0236  --End
      #IF l_sfb.sfb93 !='Y' THEN  #TQC-C20372    #mark by lili 170531
         IF g_sma.sma73='Y' AND (l_sfv.sfv09) > (g_min_set-tmp_qty) THEN
            #CALL cl_err(l_sfv.sfv09,'asf-675',1)
            CALL cl_err_msg("","csf-675",l_sfv.sfv03||"|"||l_sfv.sfv09||"|"||g_min_set-tmp_qty,1)  # mod by lixwz180627
            LET g_success = 'N'
            EXIT FOREACH
         END IF
     ...
     #IF l_sfb.sfb93='Y' THEN # 走製程 check 轉出量
      #ELSE   #mark by lili 170531
      #add by lili 170622 ---s---
      CALL s_get_doc_no(l_sfu.sfu01) RETURNING l_slip
      LET l_smyud04 = ''
      SELECT smyud04 INTO l_smyud04 FROM smy_file
       WHERE smyslip = l_slip
      #add by lili 170622 ---e---
      IF l_sfb.sfb93 ='Y' THEN
         IF (l_smyud04 <> 'Y' OR l_smyud04 IS NULL) THEN #add by lili 170531
...
#TQC-C10126 mark end
      #add by lili 170622 ---s---
         ELSE
           SELECT NVL(SUM(shb114),0) INTO l_shb114 FROM shb_file
            WHERE shb01 = l_sfv.sfvud02
           IF l_sfv.sfv09 > l_shb114 THEN
              CALL cl_err(l_sfv.sfv09,'csf-006',1)
              LET g_success = 'N'
              EXIT FOREACH
           END IF
         END IF
...
FUNCTION t620sub_s1(p_sfu01,p_argv)
  ...
  DEFINE l_smyud04  LIKE smy_file.smyud04    #add by lili 170622
  DEFINE l_shb114   LIKE shb_file.shb114     #add by lili 170622
  DEFINE l_cnt_b    LIKE type_file.num5      # add by lixwz180627
     ...
     IF p_argv = '1' THEN   #入庫
       ...
       #應抓取該張工單在別張完工入庫量已過帳的數量並加上此張要過帳的數量後再與最小套數相比
        SELECT SUM(sfv09) INTO l_sfv09 FROM sfv_file,sfu_file
         ...
         LET l_cnt_b = 0
         SELECT SUM(sfv09) INTO l_cnt_b FROM sfv_file,sfu_file
         WHERE sfv11 = l_sfv.sfv11
           AND sfv01 = sfu01
           AND sfu00 = '1'           #完工入庫
           AND sfuconf <> 'X' #FUN-660137
        ...
        IF l_cnt_b is null THEN LET l_cnt_b = 0 END IF
           #add by lixwz180627 e---
        ...
        #  CALL s_minp(l_sfv.sfv11,g_sma.sma73,l_ima153,'','','')    #FUN-A60027 #FUN-C70037 mark
           ...
           CALL s_minp(l_sfv.sfv11,g_sma.sma73,l_ima153,'','','',l_sfu.sfu02)    #FUN-C70037
            ...
            IF l_cnt !=0  THEN
               ...
               CONTINUE FOREACH
            END IF
            LET l_min_set = l_min_set-l_cnt_b # add by lixwz180627  最小发料套数要减去不良品入库数
 #W/O 總入庫量大於最小套數 --
      ...
      IF l_sfb.sfb93='Y' # 製程否
        #IF l_sfb.sfb93='Y'
           ...
           THEN      #
         IF (l_smyud04 <> 'Y' OR l_smyud04 IS NULL) THEN #add by lili 170626
