...
# Modify.........: No:FUN-E30043 14/03/28 By Nina  修正FUN-C90107給予欄位預設值的錯誤
# Modify.........: No:FUN-E30053 14/03/28 By Nina  鞋服業新增欄位影響SERVICE寫入處理,增加給予預設值
# Modify.........: No:FUN-EB0043 14/11/18 By Rayhu 新增欄位ima943,ima165影響寫入處理,增加給予預設值
# Modify.........: No:FUN-F50005 15/05/15 By Lisa 預防取單號失敗的Retry機制
# Modify.........: No:FUN-FB0001 15/11/02 By Jessica PLM：將原本多筆料號被LOCK後統一COMMIT機制，改為一筆LOCK後COMMIT再Lock下一筆
# Modify.........: No:FUN-G20003 16/02/18 By Jessica 修正Table Lock Retry機制未清空狀態碼
# Modify.........: No:FUN-G90019 16/09/30 By Mars  將retry次數改為10次 ,且SLEEP秒數改為亂數

IMPORT util              #FUN-G90019 add
DATABASE ds
...
DEFINE l_second  LIKE type_file.num5     #FUN-G90019 add
...
FUNCTION aws_create_itemmaster_data_process()
    ...
    DEFINE l_ima140  LIKE type_file.chr4     #add by lili 170601
    DEFINE l_azf     RECORD LIKE azf_file.*  #add by lili 171114
    DEFINE l_azf01   LIKE azf_file.azf01     #add by lili 171114
   ...
   #TQC-A90047 add----end---
    #FUN-C70008 add----str---
     ...
     LET l_sql = "INSERT INTO ima_tmp_def(",
                 ...
                 #",ima943,ima165",                                          #FUN-EB0043 add ima943,ima165#mark by gujq 20170303
                 ") VALUES (",
                 "'@TEST',0,0,0,'0',' ',' ',' ',' ',",
                 "' ','N','0',' ',' ','N',' ','3',' ','N','Y')" #FUN-A70106 110628 add #FUN-C20084 給ima159默認值'3' #FUN-C90107 add ima928, ima160預設值'','N'  #FUN-D10122 add ima934  #FUN-EB0043 add ima943,ima165預設值#mark 943/165
   ...
   #BEGIN WORK  #FUN-FB0001 mark
    ...
    FOR l_i = 1 TO l_cnt1
       ...
       #此範圍內的變數l_i全改成l_j,因為和外圈的FOR l_i相衝
        FOR l_j = 1 TO l_list.getLength() #TQC-A90047 add
           ...
           #FUN-D10122 add str-----------------
            ...
            END IF
           #FUN-D10122 add end-----------------
            #add by lili 170601 ---s---
            IF l_name = 'ima140' THEN
               IF l_value = 'OBS' THEN
                  LET l_value = 'Y'
               ELSE
                  LET l_value = 'N'
               END IF
            END IF
            #add by lili 170601 ---e---
            #add by lili 170608 ---s---
            IF l_name = 'ima915' THEN
               IF l_value NOT MATCHES '[0123]' THEN
                  LET l_value = '2'
               END IF
            END IF
            #add by lili 170608 ---e---
       ...
       #FUN-A70106--add----end---
        ...
        IF NOT cl_null(g_ima.ima10) THEN
          ...
          IF l_cnt = 0 THEN
             ...
             LET l_azf.azf01 = g_ima.ima10
             LET l_azf.azf02 = 'E'
             LET l_azf.azf03 = g_ima.imaud03
             LET l_azf.azf04 = ''
             LET l_azf.azfacti = 'Y'
             LET l_azf.azfuser = g_ima.imauser
             LET l_azf.azfgrup = g_ima.imagrup
             LET l_azf.azfmodu = ''
             LET l_azf.azfdate = g_ima.imadate
             LET l_azf.azf08 = 'N'
             LET l_azf.azf09 = '1'
             LET l_azf.azf10 = 'N'
             LET l_azf.azf11 = 'N'
             ...
             LET l_azf.azf13 = 'N'
             LET l_azf.azfpos = 'N'
             LET l_azf.azforig = g_ima.imauser
             LET l_azf.azforiu = g_ima.imagrup

             INSERT INTO azf_file(azf01,azf02,azf03,azf04,azfacti,azfuser,azfgrup,azfmodu,azfdate,
                                  azf08,azf09,azf10,azf11,azf12,azf13,azfpos,azforig,azforiu)
                  VALUES(l_azf.azf01,l_azf.azf02,l_azf.azf03,l_azf.azf04,l_azf.azfacti,l_azf.azfuser,
                         l_azf.azfgrup,l_azf.azfmodu,l_azf.azfdate,l_azf.azf08,l_azf.azf09,l_azf.azf10,
                         l_azf.azf11,l_azf.azf12,l_azf.azf13,l_azf.azfpos,l_azf.azforig,l_azf.azforiu)
          END IF

          #其他分群码三
          LET l_cnt = 0
          LET l_azf01 = ''
          LET l_azf01 = g_ima.ima10[1,2]
          LET g_ima.ima11 = l_azf01
          SELECT COUNT(*) INTO l_cnt FROM azf_file
           WHERE azf01 = l_azf01
             AND azf02 = 'F'
             AND azfacti = 'Y'
       ...
       #CALL aws_create_itemmaster_field_update(l_ima.*)     #FUN-A70106 mark
        ...
        END IF
        #add by lili 171115 ---s---
        IF cl_null(g_ima.ima11) THEN
           LET g_ima.ima11 = g_ima.ima10[1,2]
        END IF
        #add by lili 171115 ---e---
       ...
       #FUN-C90107 add str---
        IF cl_null(g_ima.ima926) THEN
           LET g_ima.ima926= 'N'
          #CALL aws_ttsrv_setRecordField(l_node, "ima926", " ")  #FUN-E30043 mark
           CALL aws_ttsrv_setRecordField(l_node, "ima926", "N")  #FUN-E30043 add
        END IF
        ...
        IF cl_null(g_ima.ima160) THEN
           LET g_ima.ima160= 'N'
          #CALL aws_ttsrv_setRecordField(l_node, "ima160", " ")  #FUN-E30043 mark
           CALL aws_ttsrv_setRecordField(l_node, "ima160", "N")  #FUN-E30043 add
        END IF
       #FUN-C90107 add end---

       #add by lili 170601 ---s---
       IF cl_null(g_ima.ima140) THEN
           LET g_ima.ima140= 'N'
           LET l_ima140 = aws_ttsrv_setRecordField(l_node, "ima140", " ")
           IF l_ima140 = 'OBS' THEN
              LET g_ima.ima140 = 'Y'
           ELSE
              LET g_ima.ima140 = 'N'
           END IF
       ...
       #add by lili 170601 ---e---
       #add by lili 170608 ---s---
       IF cl_null(g_ima.ima915) THEN
           LET g_ima.ima915= '0'
           LET g_ima.ima915 = aws_ttsrv_setRecordField(l_node, "ima915", " ")
           IF g_ima.ima915 NOT MATCHES '[0123]' THEN
              LET g_ima.ima915 = '2'
           END IF
        END IF
       #add by lili 170608 ---e---
       #FUN-E30053 add str----
       # IF cl_null(g_ima.ima943) THEN #mark by gujq 20170303
       #    LET g_ima.ima943= ' '
       #    CALL aws_ttsrv_setRecordField(l_node, "ima943", " ")
       # END IF
       #FUN-EB0043 mark---str---
       #IF cl_null(g_ima.ima160) THEN
       #   LET g_ima.ima160= 'N'
       ...
       #FUN-EB0043 add---str---
       # IF cl_null(g_ima.ima165) THEN #mark by gujq 20170303
       #    LET g_ima.ima165= ' '
       #    CALL aws_ttsrv_setRecordField(l_node, "ima165", " ")
       # END IF
       #FUN-EB0043 add---end---
        ...
        IF SQLCA.SQLCODE THEN
           ...
           EXIT FOR
        END IF
        #add by lili 171115 ---s---其他分群码三
        UPDATE ima_file SET ima11 = g_ima.ima11
...
DEFINE l_str   STRING
       ...
       #FUN-C70008 mark---end---
        ...
        #FUN-AB0038--add----str---
        # add by lixwz180505 s---
        # ima601 赋值
        SELECT imz601 INTO g_ima.ima601
          FROM imz_file
         WHERE imz01 = g_ima.ima06
        # add by lixwz180505 e---
        IF SQLCA.SQLCODE OR SQLCA.sqlerrd[3] = 0 THEN
           LET g_status.code = SQLCA.SQLCODE
           LET g_status.sqlcode = SQLCA.SQLCODE
        END IF
  ...
  DEFINE
   ...
   #SELECT *  INTO g_ima.ima06,l_imz02,g_ima.ima03,g_ima.ima04,
    SELECT imz01,imz02,imz03 ,imz04,
           ...
           imz72,imz159   #add imz159 by lili 170524
           ,imz163,imz1631  # add by lixwz180712
   #FUN-710060---mod---end---
      ...
      INTO g_ima.ima06,l_imz02,g_ima.ima03,g_ima.ima04,
           ...
           g_ima.ima915,g_ima.ima159                             #FUN-710060 add   #add ima159 by lili 170524
           ,g_ima.ima163,g_ima.ima1631  # add by lixwz180712
           FROM  imz_file
           WHERE imz01 = g_ima.ima06
...
#     FOR l_i = 1 to 3  #FUN-F50005 add    #FUN-G90019 mark
      FOR l_i = 1 to 10    #FUN-G90019 add
         ...
         IF SQLCA.sqlcode THEN
           ...
           #FUN-G90019 add start ---
           IF l_i < 10 THEN
              LET l_second = util.Math.rand(4)  #取亂數0~4
              LET l_second = l_second + 1       #因最多允許5秒，亂數取到0~4的值，故直接+1，使秒數範圍為1~5s
              SLEEP l_second
              CLOSE i100_cl
              CONTINUE FOR
              END IF
         ...
         IF SQLCA.sqlcode THEN
           ...
           #FUN-F50005 add end ---
            RETURN FALSE
         END IF
         LET gi_err_code = '0'         #FUN-G90019 add
...
#        LET gi_err_code   = ''                       #FUN-G20003 add #FUN-G90019 mark
         LET gi_err_code   = 0                        #FUN-G90019 add
         LET g_status.description = ''                #FUN-G20003 add
         RETURN TRUE
...
FUNCTION i100_default()
   ...
   LET g_ima.ima18 = 0
   LET g_ima.ima09 =' '
   #LET g_ima.ima10 =' '   #mark by lili 171115
   #LET g_ima.ima11 =' '   #mark by lili 171115
   LET g_ima.ima12 =' '
   LET g_ima.ima23 = ' '
   #-----No.FUN-810036-----
   LET g_ima.ima918= 'N'
  ...
  #LET g_ima.ima1010 = '1' #No.FUN-610013 #FUN-690060
   ...
   LET g_ima.ima120 = '1'
   #FUN-A70106---add---end---
   #add by lili 170524 ---s---
   LET g_ima.ima930 = 'N'
   LET g_ima.ima931 = 'N'
   #add by lili 170524 ---e---
END FUNCTION
