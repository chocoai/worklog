...
# Moidfy.........: No:FUN-D60049 13/06/10 By Nina  因應pmh_file增加key值欄位(pmh07)調整程式
# Modify.........: No:TQC-DA0011 13/10/15 By fengrui 新增時賦bmy38默認值
# Modify.........: No:FUN-EA0020 14/11/17 By Rayhu 調整:1.當變異別bmy03為2.新元件新增生效，若工單開立展開選項(bmy20)為null，導致aws-607錯誤
#                                                       2.新增bmb34 服飾業顏色、尺寸屬性設定
# Modify.........: No:FUN-F10010 15/01/13 By Lisa 調整s_bomchk判斷前，需重新取得g_ima08_h的值
# Modify.........: No:FUN-EA0004 15/01/22 By Rayhu 變異別判斷(aws_diff_bmb)需增加生效日(bmb04)判斷
#                                                  (aws_diff_bmb_new)舊元件失效/修改判斷給值需增加生效日(bmb04)與失效日(bmb05)判斷
...
# Modify.........: No:FUN-GA0012 16/11/08 By Mars  修正DEV-C40005(調整CreatePLMBOMData,刪除整個BOM時的處理)的處理邏輯
...
FUNCTION aws_create_plm_bom_data_process()
    ...
    FOR l_i = 1 TO l_cnt1       
        ...
        IF g_ecn = 'N' THEN
            ...
            IF g_del_bom = 'N' THEN #DEV-C40005 add if 判斷
                ...
                FOR l_j = 1 TO l_cnt2
                    ...
                    #CALL aws_ttsrv_setRecordField(l_node2, "bmb34",g_bmb.bmb34)           #FUN-EA0020 add #mark by gujq 20170303
        ...
        ELSE
           ...
           IF g_del_bom = 'N'  THEN           #FUN-GA0012 add
              #FUN-GA0012 add str ------------------
              #先判斷是否為無效BOM,如果是無效BOM,先還原:更改有效碼=直接變有效
              IF g_bma.bmaacti = 'N' THEN              
                 ...
                 UPDATE bma_file                   
                    SET bmaacti = 'Y',    
                        bmamodu = g_user,
                        bmadate = g_today
                  WHERE bma01 = g_bma.bma01     #主件料號
                    AND bma06 = g_bma.bma06     #特性代碼
                 IF SQLCA.SQLCODE OR SQLCA.SQLERRD[3]=0 THEN
                    LET g_status.code = "aws-777"             #BOM還原作廢失敗
                    EXIT FOR  
                 END IF                    
              END IF
              #FUN-GA0012 add end ------------------
           ...
           ELSE         
              ...
              UPDATE bma_file                   
               ...
               WHERE bma01 = g_bma.bma01     #主件料號
                 AND bma06 = g_bma.bma06     #特性代碼
              IF SQLCA.SQLCODE OR SQLCA.SQLERRD[3]=0 THEN
                 LET g_status.code = "aws-776"             #BOM作廢失敗
                 EXIT FOR  
              END IF                               
           END IF       
           #FUN-GA0012  add end ----------
        ...
        CASE g_msg_flag
             ...
             WHEN 'bmb_file'
                   LET g_msg = cl_mut_get_feldname('bmb01',g_bmb.bmb01,
                                                   ...
                                                   'bmb03',g_bmb.bmb03,
                                                   'bmb04',g_bmb.bmb04,
                                                   #'','',                #FUN-D60049 add #mark by gujq 20170303
                                                   #'','',                #FUN-D60049 add #mark by gujq 20170303
                                                   '','')
             WHEN 'bmt_file'
...
DEFINE l_i        LIKE type_file.num10      #FUN-F50005  add   
  ...
  #FOR l_i = 1 to 3  #FUN-F50005 add   #FUN-G90019 mark
   FOR l_i = 1 to 10     #FUN-G90019 add
     ...
     #LET g_status.code    = ''   #FUN-G20003 add     #FUN-G90019 mark
     #LET g_status.sqlcode = ''   #FUN-G20003 add     #FUN-G90019 mark
...
FUNCTION aws_create_plm_bom_data_default(p_node,p_cnt)
    ...
    #LET g_bma.bmauser = aws_ttsrv_getRecordField(p_node,"bmauser") #資料所有者
    #LET g_bma.bmagrup = aws_ttsrv_getRecordField(p_node,"bmagrup") #資料所有部门
    #LET g_bma.bmaoriu = g_bma.bmauser
    #LET g_bma.bmaorig = g_bma.bmagrup
    ...
    IF p_cnt = 0 THEN
       ...
       LET g_bma.bma05=''
       LET g_bma.bma03=''
       LET g_bma.bmauser=g_user   #mod by lili 170524
       LET g_bma.bmagrup=g_grup   #mod by lili 170524
       LET g_bma.bmadate=g_today
       LET g_bma.bmaacti='Y'              #資料有效
       LET g_bma.bma09=0                  #拋轉次數            
       LET g_bma.bma10 = '0'              #狀態碼
       LET g_bma.bmaoriu = g_user #FUN-C20081 add  #mod by lili 170524
       LET g_bma.bmaorig = g_grup #FUN-C20081 add  #mod by lili 170524
...
FUNCTION aws_create_plm_bom_b_default(p_node,p_cnt)
    ...
    DEFINE l_ima70     LIKE ima_file.ima70   #add by chenke170630
    ...
    #LET g_bmb.bmb34 = aws_ttsrv_getRecordField(p_node,"bmb34")  #FUN-EA0020 add #mark by gujq 20170303
   ...
   #FUN-F10010 add(E)
	...
	#str---add by jixf 170316
	SELECT ima08 INTO g_ima08_b
       FROM ima_file WHERE ima01 = g_bmb.bmb03
	#end---add by jixf 170316
    ...
    IF g_ecn = 'N' THEN
       ...
       #IF cl_null(g_bmb.bmb15) THEN LET g_bmb.bmb15 = 'N' END IF  #mark by chenke170630
        LET g_bmb.bmb15 = l_ima70                                  #add by chenke170630
        ...
        #add by lili 170508 ---s---
        IF cl_null(g_bmb.bmb19) THEN
           SELECT ima110 INTO g_bmb.bmb19 FROM ima_file 
            WHERE ima01 = g_bmb.bmb03
        END IF    
        #add by lili 170508 ---e---
        ...
        #IF cl_null(g_bmb.bmb34) THEN LET g_bmb.bmb34 = ' ' END IF  #FUN-EA0020 add #mark by gujq 20170303
...
FUNCTION aws_ins_bmx() #建立 ECN 單頭資料
  ...
  DEFINE l_i        LIKE type_file.num10   #FUN-G90019 add  
  DEFINE l_bmx01    LIKE bmx_file.bmx01    #FUN-G90019 add
   ...
   LET l_bmx01       = g_smyslip     #FUN-G90019 add
   ...
   FOR l_i = 1 TO 10   #FUN-G90019 add
      ...
      IF SQLCA.sqlcode OR SQLCA.sqlerrd[3] = 0 THEN
         ...
         #FUN-G90019 add start ---
          IF l_i < 10 THEN
             ...
             LET l_second = l_second + 1       #因最多允許5秒，亂數取到0~4的值，故直接+1，使秒數範圍為1~5s
             SLEEP l_second
             LET g_bmx.bmx01 = l_bmx01         #但插入失敗時，需要重新獲取單號
             CONTINUE FOR
          END IF
         #FUN-G90019 add end ---
     ...
     #FUN-G90019 add end ---
      ...
      RETURN TRUE
      EXIT FOR   #FUN-G90019 add
   END FOR       #FUN-G90019 add
END FUNCTION
...
DEFINE l_cnt      LIKE type_file.num10
    ...
    SELECT COUNT(*) INTO l_cnt
     ...
     WHERE bmb01 = g_bmb.bmb01
       ...
       AND (bmb04 <= g_today OR bmb04 IS NULL)  #生效日 #FUN-EA0004 add
...
DEFINE l_cnt      LIKE type_file.num10
    ...
    SELECT COUNT(*) INTO l_cnt
     ...
     WHERE bmb01 = g_bmb.bmb01 #主件料件編號
       ...
       AND (bmb04 <= g_today OR bmb04 IS NULL)  #生效日 #FUN-EA0004 add
       AND (bmb05 >  g_today OR bmb05 IS NULL)  #失效日 #FUN-EA0004 add
    IF l_cnt = 0 THEN
        RETURN '2' #新元件新增生效
    ELSE
        INITIALIZE l_bmb.* TO NULL
   ...
   #DEV-C40005 --add----end---
    FOREACH get_old_item_void_cur INTO l_bmb.*
       ...
       #LET g_bmy.bmy36 = l_bmb.bmb36 #FUN-C20081 add   #FUN-D20018 mark
        ...
        #LET g_bmy.bmy38 = l_bmb.bmb34 #TQC-DA0011 add #mark by gujq 20170303
        #IF cl_null(g_bmy.bmy38) THEN LET g_bmy.bmy38 = ' ' END IF  #TQC-DA0011 add #mark by gujq 20170303
