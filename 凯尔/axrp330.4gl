...
# Modify.........: No:MOD-D90014 13/09/03 By SunLM oha09='5'時候產生應收金額錯誤 #140113
...
# Modify.........: No.MOD-DC0159 13/12/25 By yinhy 出貨包含銷退，檢查銷退單也不可跨月
# Modify.........: No.MOD-E10056 14/01/08 By yinhy oma68應取oha1001
# Modify.........: No.MOD-E10074 14/01/10 By yinhy oma63专案编号赋值为oga46
# Modify.........: No:FUN-D40106 13/05/22 By fengmy 30追,axmt670增加出貨客戶字段，拋轉時賬款客戶賦值
# Modify.........: No:2015060128 15/06/24 By kuangxj 回收p_sser2015060128走开票流程，出货和消退合并立账，axmt670数量本来就是负数，所以不用乘以-1                                                           
# Modify.........: No.2015080290 15/09/14 By renjj 回收p_sser-流水号:2015080290 
...
DEFINE g_omf27      LIKE omf_file.omf27
DEFINE g_oma68      LIKE oma_file.oma68
     ...
     IF g_ooz.ooz65 = 'Y' THEN                          #CHI-C10018 add
        LET g_sql = g_sql CLIPPED," UNION ",
          #   "SELECT DISTINCT omf01,omf02,oha94,ohb33,'','',oha01,ohaplant,''    ,oha03,oha03, ",   #minpp--mark
              ...
              "SELECT DISTINCT omf00,omf01,omf02,oha94,ohb33,'','',oha01,omf09,''    ,oha03,oha1001, ",   #MOD-E10056
   ...
   #IF g_aza.aza26 = '2' AND g_ooz.ooz65 = 'Y' THEN    #CHI-C10018 mark 
    IF g_ooz.ooz65 = 'Y' THEN                          #CHI-C10018 add
       LET g_sql = g_sql CLIPPED," UNION ",
             ...
             "SELECT DISTINCT '','','',oha94,ohb33,'','',oha01,ohaplant,''    ,oha03,oha1001, ", #MOD-E10056 
              #"       ''   ,oha21,oha15,oha14,oha23,oha02,oha02,''   ,oha08,'3' type ", #MOD-DA0161 mark
              #"       ''   ,oha21,oha15,oha14,oha23,oha02,to_date('','yyyymmdd'),'',oha08,'3' type ",    #MOD-DA0161 add
              "       ''   ,oha21,oha15,oha14,oha23,oha02,CAST('' AS DATE),'',oha08,'3' type ",    #MOD-DA0161 add
...
#No.MOD-DA0161 --begin   
   IF l_oga03 != sr.oga03  OR l_order1 != sr.order1            
           ...
           # OR (s_get_aznn(g_plant,g_aza.aza81,l_oga11,1) != s_get_aznn(g_plant,g_aza.aza81,sr.oga11,1) AND g_oaz92<>'Y')           #yinhy140515
           # OR (s_get_aznn(g_plant,g_aza.aza81,l_oga11,3) != s_get_aznn(g_plant,g_aza.aza81,sr.oga11,3) AND g_oaz92<>'Y') OR        #yinhy140515
#No.MOD-DA0161 --end
      ...
      #l_oga27 != sr.oga27 OR  #FUN-640191 add  #mark by quan 20170802
...
#    IF l_sum <= 0 THEN  #MOD-AA0156
  ...
  #生成分錄底稿
   ...
   FOREACH p330_cs01 INTO g_oma.*
    ...
    #CALL saxrp310_bu(g_oma.*,g_ogb.*,l_oea01,'') RETURNING g_oma.*     #qiull140820 mark
     INITIALIZE g_ogb.* TO NULL  #yinhy131217
     #FUN-D90048---add---str--
     #IF g_oma.oma54 <=0 OR g_oma.oma56 <= 0 THEN     #MOD-DC0159 mark
#qiull140820---mark---begin------
#    IF g_oma.oma54 < 0 OR g_oma.oma56 < 0 OR g_oma.oma54t<0 OR g_oma.oma56<0 THEN  #MOD-DC0159
...
FUNCTION p330_g_oma()
   ...
   #FUN-D40106--add--begin---
   IF g_oaz92 = 'Y' AND g_aza.aza26 = '2' THEN
      IF NOT cl_null(g_omf27) THEN
         LET g_oma.oma68 = g_oma68
         LET g_oma.oma03 = g_oga03
         LET g_oma.oma04 = g_oma.oma68
      END IF
   END IF
  #FUN-D40106--add--end---
   ...
   LET g_oma.oma63 = g_oga.oga46   #MOD-E10074
...
#FUN-AC0027 --end--
  ...
  #-------------------------MOD-CC0028-----------(E)
   ...
   END IF 
   #FUN-CA0084--add--end 
   SELECT occ37 INTO g_oma.oma40 FROM occ_file WHERE occ01=g_oma.oma03  #add by renjj 150914
    IF g_oma.oma40 IS NULL THEN LET g_oma.oma40 ='N' END IF               #add by renjj 150914
...
#No.MOD-B10206 --end
   ...
   #  IF g_type = '2' THEN     #mark by  2015060128        
     IF g_type = '2' OR  (g_oaz92='Y' AND g_aza.aza26='2' and g_oma.oma00='12') THEN  #add by 2015060128                               
        LET g_omb.omb12 = g_ogb.ogb917  #No.TQC-7B0144  #已去掉已用量           
     ELSE                                                                       
...
#No.MOD-B10206 --begin  
     ...
     IF g_type ='3' THEN 
       ...
       #-MOD-B70125-end-
        IF l_oha09 = '5' THEN  
           IF g_prog_type = 'axmt670' THEN #140113 add
              ...
              #SELECT SUM(ABS(omb14)),SUM(ABS(omb14t))  #MOD-B70278 
               ...
               WHERE omb33 = g_ogb.ogb01 
                 ...
                 AND oma01 = omb01
                 AND omavoid <> 'N' 
           ELSE #140113 add begin-----------------
              SELECT SUM(ABS(omb14)),SUM(ABS(omb14t))  #MOD-B70278 MOD-D90014
                INTO l_omb14,l_omb14t 
                FROM omb_file,oma_file 
               WHERE omb33 = g_ogb.ogb01 
                 AND omb01 <> g_oma.oma01 
           ...
           ELSE #140113 add begin-----------------
              #No.MOD-B70278  --Begin
              LET g_omb.omb13 = l_omb13 * -1    
              LET g_omb.omb14 = g_omb.omb14 * -1    
              LET g_omb.omb14t = g_omb.omb14t * -1  
              #No.MOD-B70278  --End              
           END IF 
           #140113 add end-------------------           
        END IF  
...
FUNCTION p330_chkdate()
   ...
   DEFINE l_where3  STRING                      #MOD-DC0159
   DEFINE l_wc      STRING                      #MOD-DC0159
...
FUNCTION p330_omf(p_azp01,p_sr)
   ...
   DEFINE l_omf27      LIKE omf_file.omf27   #FUN-D40106 add
   LET l_omf07 = g_oga.oga23
   LET g_oga.oga23 = ''
   LET l_omf27 = NULL   #FUN-D40106 add
  #SELECT DISTINCT omf03,omf01,omf02,omf05,omf06,omf07,omf22  #FUN-D50008 mark
   SELECT DISTINCT omf31,omf01,omf02,omf05,omf06,omf07,omf22,omf27  #FUN-D50008 add #FUN-D40106 add omf27  
     INTO g_oma.oma09,g_oma.oma10,g_oma.oma75,g_oga.oga03,g_oga.oga21,g_oga.oga23
          ,g_oma.oma24,l_omf27  #FUN-D40106 add omf27
     FROM omf_file
    WHERE omf01 = p_sr.omf01 AND omf02 = p_sr.omf02
     ...
     #AND omf09 = p_azp01
   #FUN-D40106--add---begin---
   IF NOT cl_null(l_omf27) THEN
      LET g_oma68 = g_oga.oga03
      LET g_oga03 = l_omf27
      LET g_omf27 = l_omf27
   END IF
   #FUN-D40106--add---end---
