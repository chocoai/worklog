...
# Modify.........: No:MOD-E60010 14/06/04 By fengmy 轉FQC單時qcf09、qcf091賦默認值
# Modify.........: No.MOD-DB0026 14/06/25 By fengmy 30追单,修正 t700_chk_input() 此段控卡應是指控卡最後一站
# Modify.........:               17/06/22 By lili  修改不在报工时做当站入库，在asft620做不良品完工入库
     ...
     CONSTRUCT g_wc3 ON shc03,shc04,shc05,shc012,shc06   #FUN-A60095
        ...
        ON ACTION controlp
           CASE WHEN INFIELD(shc04)
                     CALL cl_init_qry_var()
                     LET g_qryparam.state    = "c"
                     #LET g_qryparam.form     = "q_qce"    #mark by lili 170529
                     LET g_qryparam.form     = "q_qce01"   #add by lili 170529
                     CALL cl_create_qry() RETURNING g_qryparam.multiret
...
#FUN-A70095 --------End--------------
         ...
         WHEN "output"
            IF cl_chk_act_auth() THEN
              ...
              LET l_cmd ='asfr732 '," '",g_today CLIPPED,"' ''",  #FUN-C30085 mark   #radd by pulf 141225
              # LET l_cmd ='asfg732 '," '",g_today CLIPPED,"' ''",  #FUN-C30085 add  #rmk by pulf 141225
                           " '",g_lang CLIPPED,"' 'Y' '' '1' ",
                           " '",l_wc CLIPPED,"' "
        ...
        WHEN "trans_store"
         ...
         #add by lili 170618 ---s---
         WHEN "stock_post"
            IF cl_chk_act_auth() THEN
               CALL t700_stock_post()
            END IF
         WHEN "undo_stock_post"
            IF cl_chk_act_auth() THEN
               CALL t700_unstock_post()
            END IF
         #add by lili 170618 ---e---

         #add by jolly 170627 ---s---
         WHEN "run_p620"
            IF cl_chk_act_auth() THEN
               LET  l_cmd = "csfp620"
               CALL cl_cmdrun(l_cmd)
            END IF
         #add by jolly 170627 ---e---
      END CASE
   END WHILE
...
FUNCTION t700_confirm()
   ...
   DEFINE l_ecm53     LIKE ecm_file.ecm53  #add by lili 171110
   DEFINE l_qcm091    LIKE qcm_file.qcm091 #add by lili 171110
   DEFINE l_shb111    LIKE shb_file.shb111 #add by lili 171110
   DEFINE l_shb114    LIKE shb_file.shb114 #add by lili 171110
  ...
  #MOD-C80032-程式搬移-S-
   IF cl_null(g_shb.shb14) THEN                     #CHI-C50054 add
      ...
      IF t700_accept_qty2('c') THEN RETURN END IF   #mod by lili 171110
      #add by lili 171110 ---s---
      SELECT ecm53 INTO l_ecm53 FROM ecm_file
       WHERE ecm01=g_shb.shb05
      ...
      IF l_ecm53 = 'Y' THEN
         SELECT SUM(qcm091) INTO l_qcm091 FROM qcm_file
          WHERE qcm02=g_shb.shb05   #工單編號
            ...
            AND qcm14='Y'  #確認
            AND qcm012=g_shb.shb012
            AND (qcm09='4')  #判定結果(4.不良品合格)
         ...
         IF g_shb.shb114 > l_qcm091 - l_shb114 THEN
            CALL cl_err(g_shb.shb114,'csf-011',1)
            LET g_success = 'N'
            RETURN
          END IF
          LET l_qcm091 = 0
          ...
          WHERE qcm02=g_shb.shb05   #工單編號
            ...
            AND qcm14='Y'  #確認
            AND qcm012=g_shb.shb012
            AND (qcm09='1' OR qcm09='3')  #判定結果(1.合格 3.特采)
         ...
         IF cl_null(l_qcm091) THEN LET l_qcm091=0 END IF
         IF g_shb.shb111 > l_qcm091 - l_shb111 THEN
            CALL cl_err(g_shb.shb111,'csf-010',1)
            LET g_success = 'N'
            RETURN
          END IF
       END IF
      #add by lili 171110 ---e---
   ELSE                                             #CHI-C50054 add
...
FUNCTION t700_unconfirm()
   ...
   # add by lixwz 180627 s---
   SELECT COUNT(1)
     INTO l_cnt
     FROM sfv_file,sfu_file
    WHERE sfv01 = sfu01
      AND sfuconf <>'X'
      AND sfvud01 = g_shb.shb01
   IF l_cnt > 0 THEN CALL cl_err(g_shb.shb01,'cxm-015',1) RETURN END IF
   # add by lixwz 180627 e---
...
FUNCTION t700_a(l_flag,l_flag1)
  ...
  DEFINE l_sfb08    LIKE sfb_file.sfb08  #MOD-D20173
    ...
    IF l_flag1 = 'F' THEN
       ...
       LET g_shb.shbud03 = 'N'   #add by lili 170618
    ...
    END IF 	          #NO.FUN-940113
        ...
        IF cl_null(g_shb.shb14) THEN        #CHI-C50054 add
           ...
           IF t700_accept_qty2('c') THEN EXIT WHILE END IF   #add by lili 171110
       #CHI-C50054 str add-----
        ELSE
...
#FUN-A70095 --------------------End------------------------
        ...
        IF g_success='Y' THEN  #FUN-A80060
           #IF g_shb.shb114 > 0 THEN CALL t701(g_shb.shb01) END IF    #mark by lili 170622 报工不做当站入库
           CALL g_shc.clear()
           LET g_rec_b = 0
        ...
        AFTER FIELD shb111
           ...
           #No.FUN-BB0086--add--end--
            IF NOT cl_null(g_shb.shb111) THEN
               ...
               ## 檢查可報工數量
               IF cl_null(g_shb.shb14) THEN        #CHI-C50054 add
                  #IF t700_accept_qty('c') THEN NEXT FIELD shb111 END IF    #mark by lili 171110
                  #mod by lili 171110 ---s---
                  IF t700_accept_qty2('c') THEN NEXT FIELD shb111 END IF
                  SELECT ecm53 INTO l_ecm53 FROM ecm_file
                   WHERE ecm01=g_shb.shb05
                     AND ecm03=g_shb.shb06
                     AND ecm012=g_shb.shb012
           ...
           #No.FUN-BB0086--add--end--
            IF NOT cl_null(g_shb.shb112) THEN
               ...
               IF cl_null(g_shb.shb14) THEN        #CHI-C50054 add
                  ...
                  IF t700_accept_qty2('c') THEN NEXT FIELD shb112 END IF     #mod by lili 171110
              #CHI-C50054 str add-----
               ELSE
           ...
           #No.FUN-BB0086--add--end--
            IF NOT cl_null(g_shb.shb113) THEN
               ...
               IF cl_null(g_shb.shb14) THEN        #CHI-C50054 add
                  ...
                  IF t700_accept_qty2('c') THEN NEXT FIELD shb113 END IF     #mod by lili 171110
              #CHI-C50054 str add-----
               ELSE
           ...
           #No.FUN-BB0086--add--end--
            IF NOT cl_null(g_shb.shb114) THEN
               ...
               IF cl_null(g_shb.shb14) THEN        #CHI-C50054 add
                  ...
                  IF t700_accept_qty2('c') THEN NEXT FIELD shb114 END IF   #mod by lili 171110
                  #add by lili 171110 ---s---
                  SELECT ecm53 INTO l_ecm53 FROM ecm_file
                   WHERE ecm01=g_shb.shb05
                     AND ecm03=g_shb.shb06
           ...
           #No.FUN-BB0086--add--end--
            IF NOT cl_null(g_shb.shb17) THEN
               ...
               IF cl_null(g_shb.shb14) THEN        #CHI-C50054 add
                  ...
                  IF t700_accept_qty2('c') THEN NEXT FIELD shb17 END IF   #mod by lili 17110
              #CHI-C50054 str add-----
               ELSE
...
#FUN-B20097 ------------------End---------------------
         ...
         #TQC-B60219 -------------End-----------------
          ...
          #CHI-C50054 str add------
           IF cl_null(g_shb.shb14) THEN
              #IF t700_accept_qty('c') THEN NEXT FIELD shb111 END IF
              IF t700_accept_qty2('c') THEN NEXT FIELD shb111 END IF   #mod by lili 171110
           ELSE
...
#@    ON ACTION 工單轉出查詢
      ...
      ON ACTION stock_post
         ...
         EXIT DISPLAY
      ON ACTION undo_stock_post
         LET g_action_choice="undo_stock_post"
         EXIT DISPLAY
      #add by lili 170618 ---e---

      #add by jolly 170627 ---s---
      ON ACTION run_p620
         LET g_action_choice = 'run_p620'
         EXIT DISPLAY
      #add by jolly 170627 ---e---
...
FUNCTION t700_x(p_type)  #CHI-D20010
   ...
   DEFINE l_cnt     LIKE type_file.num5  # add by lixwz180627
...
DEFINE l_pqc      LIKE shb_file.shb111
DEFINE l_tot_pqc  LIKE shb_file.shb111
#add by lili 171110 ---e---

   CALL t700sub_accept_qty(p_cmd,g_shb.*) RETURNING l_flag,g_wip,g_pqc,g_ecm53,g_ecm54,g_tot_pqc
   DISPLAY g_wip,g_pqc,g_ecm53,g_ecm54,g_tot_pqc
            TO FORMONLY.wip,FORMONLY.pqc,ecm53,ecm54, FORMONLY.tot_pqc

   #add by lili ---s---
   CALL t700sub_accept_qty2(p_cmd,g_shb.*) RETURNING l_flag1,l_wip,l_pqc,g_ecm53,g_ecm54,l_tot_pqc
   DISPLAY l_pqc,l_tot_pqc TO FORMONLY.pqc,FORMONLY.tot_pqc
   #add by lili ---e---
...
#add by lili 170617 ---s---
FUNCTION t700_stock_post()
   DEFINE l_cnt         LIKE type_file.num10
   DEFINE l_shb01       LIKE shb_file.shb01
   DEFINE l_flag        LIKE type_file.num5
   DEFINE l_n           LIKE type_file.num5
   DEFINE l_ecm65       LIKE ecm_file.ecm65
   DEFINE l_shbud03     LIKE shb_file.shbud03
   DEFINE l_shbconf     LIKE shb_file.shbconf
   ...
   SELECT shbconf INTO l_shbconf FROM shb_file
    WHERE shb01 = g_shb.shb01
   IF l_shbconf <> 'Y' THEN
      CALL cl_err(g_shb.shb01,'aba-100',1)
      RETURN
   END IF

   SELECT shbud03 INTO l_shbud03 FROM shb_file
    WHERE shb01 = g_shb.shb01
   IF l_shbud03 = 'Y' THEN
      CALL cl_err(g_shb.shb01,'aim-318',1)
      RETURN
   END IF
   ...
   IF g_success = 'Y' THEN
      ...
      CALL t700sub_add(g_shb.shb01)
      IF g_success = 'Y' THEN
         UPDATE shb_file SET shbud03 = 'Y',
                             shbmodu = g_user,
                             shbdate = g_today
                       WHERE shb01 = g_shb.shb01
...
FUNCTION t700_unstock_post()
   ...
   SELECT shbud03 INTO l_shbud03 FROM shb_file
    WHERE shb01 = g_shb.shb01
   IF l_shbud03 <> 'Y' THEN
      CALL cl_err(g_shb.shb01,'mfg0178',1)
      RETURN
   END IF
   ...
   IF g_success = 'Y' THEN
      ...
      CALL t700sub_del(g_shb.shb01)
      IF g_success = 'Y' THEN
         UPDATE shb_file SET shbud03 = 'N',
                             shbmodu = g_user,
                             shbdate = g_today
                       WHERE shb01 = g_shb.shb01
         IF STATUS OR SQLCA.SQLERRD[3]=0 THEN
...
END FUNCTION
#add by lili 170617 ---e---

#add by lili 171110 ---s---
FUNCTION t700_accept_qty2(p_cmd)
DEFINE l_flag     LIKE type_file.num5
DEFINE p_cmd      LIKE type_file.chr1
DEFINE l_flag1    LIKE type_file.num5
DEFINE l_wip      LIKE shb_file.shb111
...
DEFINE l_tot_pqc  LIKE shb_file.shb111
   ...
   CALL t700sub_accept_qty2(p_cmd,g_shb.*) RETURNING l_flag,g_wip,g_pqc,g_ecm53,g_ecm54,g_tot_pqc
   DISPLAY g_pqc,g_tot_pqc TO FORMONLY.pqc,FORMONLY.tot_pqc
