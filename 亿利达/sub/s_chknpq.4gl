...
# Modify.........: No:FUN-D70118 13/08/07 By lujh 會計年期會不按照自然年月設置,修改全系統邏輯，年期的判斷需要按照aooq011的設置來
# Modify.........: No:CHI-DC0054 13/01/08 By Reanna 增加固資出售(FA)檢查關係人條件
# Modify.........: No:MOD-E40072 14/04/14 By yihsuan 由於 MOD-C60022 屬於特殊案例,不建議作為標準程式判斷因此將相關判斷 p_sys 不為 'NM' 條件移除
# Modify.........: No:FUN-E70058 14/07/31 By doris 於 aap-058 增加以下處理,當 npq05 not null,對應 gem06 為 Y 時,group by npq05,npq06 將 sum(npq07),判斷借貸合計的 npq07 需相等,否則提示 aap-058 錯誤
# Modify.........: No:TQC-E80005 14/08/05 By doris s_chknpq 中 TRIM 改為 LTRIM
# Modify.........: No.MOD-G30055 16/03/09 By Mandy 成本分錄類別為CA,以管控的時點來說，在分錄產生後進行預算控管感覺沒有什麼明確的管理效益,
#                                                  故調整即使該會科有設定預算控管，亦不應進入預算檢核的程式段
# Modify.........: No.MOD-G60146 16/06/27 By ywtsai 調整判斷無分錄底稿或不須產生會計分錄跳出程式時，沒有將g_success = 'N'即跳出的問題
# Modify.........: No.MOD-G60124 16/06/30 By yihsuan 針對ANM模組，在分錄檢核時，若是原幣*匯率造成本幣金額為零的情況，則不作aap-401/aap-402的檢核。
...
FUNCTION s_chknpq(p_no,p_sys,p_npq011,p_npptype,p_bookno) # 檢查分錄底稿正確否  #No.FUN-670047 新增p_npptype  #No.FUN-730020
   ...
   DEFINE p_sys       LIKE npq_file.npqsys    # Prog. Version..: '5.30.27-19.04.12(02)   # 系統別:AP/AR/NM/FA
   ...
   DEFINE l_t1        LIKE apy_file.apyslip        # Prog. Version..: '5.30.27-19.04.12(02)  #No.FUN560060
   DEFINE l_dmy3      LIKE type_file.chr1          #No.FUN-680147 VARCHAR(01)
  #DEFINE l_sql       LIKE type_file.chr1000       #No.FUN-680147 VARCHAR(1000)   #FUN-E70058 mark
   DEFINE l_sql       STRING                                                      #FUN-E70058 add 
...
#No.TQC-B80079 --end
  ...
  #FUN-E70058---add---str---
   DEFINE l_gem06         LIKE gem_file.gem06
   DEFINE l_npq05         LIKE npq_file.npq05
   DEFINE l_amtd,l_amtc   LIKE npq_file.npq07
   DEFINE l_flag2         LIKE type_file.chr1
  #FUN-E70058---add---end---
   DEFINE l_npq07_tot     LIKE npq_file.npq07    #MOD-G60124 add
   DEFINE l_npq07f        LIKE npq_file.npq07f   #MOD-G60124 add
   DEFINE l_npq07         LIKE npq_file.npq07    #MOD-G60124 add
   DEFINE l_npq25         LIKE npq_file.npq25    #MOD-G60124 add
...
#NO.FUN-720003-------end
   ...
   #--MOD-G60124 add start--
   #檢查本幣金額為零是否為原幣*匯率造成
   LET l_sql = "SELECT npq07f,npq25",
               "  FROM npq_file",
               " WHERE npq01 = '",p_no,"'",
               "   AND npqsys = '",p_sys,"'",
               "   AND npq011 = '",p_npq011,"'",
               "   AND npqtype = '",p_npptype,"' ",
               "   AND (npq06  = '1' AND npq07f > 0 OR npq06 = '2' AND npq07f < 0 )"
   PREPARE chk_npq07f_p FROM l_sql
   DECLARE chk_npq07f_c CURSOR FOR chk_npq07f_p

   LET l_npq07_tot = 0
   FOREACH chk_npq07f_c INTO l_npq07f,l_npq25
      LET l_npq07 = l_npq07f * l_npq25
      CALL cl_digcut(l_npq07,g_azi04) RETURNING l_npq07
      LET l_npq07_tot = l_npq07_tot + l_npq07   
   END FOREACH
   #--MOD-G60124 add end--
  ...
  IF cl_null(amtd) THEN                     #MOD-C60022 mark  #TQC-C60209  ummark
   ...
   #IF p_sys <> 'NM' THEN                   #MOD-E40072 mark  #TQC-C60209  add
    IF NOT(p_sys = 'NM' AND l_npq07_tot = 0) THEN   #MOD-G60124 add
...
#NO.FUN-720003-------end
     #-----No.FUN-640178 END-----
   #END IF   #TQC-C60209  add   #MOD-E40072 mark
    END IF   #MOD-G60124 add
   END IF
   ...
   PREPARE chk_npq07f_p2 FROM l_sql
   DECLARE chk_npq07f_c2 CURSOR FOR chk_npq07f_p2

   LET l_npq07_tot = 0
   FOREACH chk_npq07f_c2 INTO l_npq07f,l_npq25
      LET l_npq07 = l_npq07f * l_npq25
      CALL cl_digcut(l_npq07,g_azi04) RETURNING l_npq07
      LET l_npq07_tot = l_npq07_tot + l_npq07
   END FOREACH
...
#NO.FUN-720003-------end
  ...
  #借貸平衡檢核
   LET l_flag2 = "N"
   LET l_sql = "SELECT npq05",
               "  FROM npq_file",
               " WHERE LTRIM(npq05) IS NOT NULL",   #TQC-E80005 mod,TRIM改為LTRIM
               "   AND npq01 = '",p_no,"'",
               "   AND npqsys = '",p_sys,"'",
               "   AND npq011 = '",p_npq011,"'",
               "   AND npqtype = '",p_npptype,"' ",
               "GROUP BY npq05"
   PREPARE chk_npq05_p FROM l_sql
   DECLARE chk_npq05_c CURSOR FOR chk_npq05_p
   FOREACH chk_npq05_c INTO l_npq05
    #取出資產負債否
     SELECT gem06 INTO l_gem06
       FROM gem_file
      WHERE gem01 = l_npq05
     ...
     IF l_gem06 = "Y" THEN
       ...
       #同分錄同部門借貸平衡檢核
        LET l_amtd = 0
        SELECT SUM(ABS(npq07)) INTO l_amtd 
          FROM npq_file 
         WHERE npq01 = p_no
           AND npqsys = p_sys
           AND npq011 = p_npq011
        ...
        LET l_amtc = 0
        SELECT SUM(ABS(npq07)) INTO l_amtc 
          FROM npq_file
         WHERE npq01 = p_no
           AND npqsys = p_sys
           AND npq011 = p_npq011
        ...
        IF l_amtd != l_amtc THEN
           ...
           ELSE
              IF g_bgerr THEN
                 LET g_showmsg=p_no,"/",'2',"/",p_sys,"/",p_npq011,"/",p_npptype
                 CALL s_errmsg('npq01,npq06,npqsys,npq011,npqtype',g_showmsg,g_plant,'aap-058',1)
              ELSE
...
#FUN-E70058---add---end---
   ...
   IF l_flag2 = "N" THEN   #FUN-E70058 add
      #-->借貸要平
      IF amtd != amtc THEN
        #-----No.FUN-640178-----
     ...
     #No.B203..end
      ...
      IF l_aag.aag21 = 'Y' THEN
         ...
         IF p_bookno IS NULL OR l_npq.npq36 IS NULL OR
           #l_npq.npq03 IS NULL OR YEAR(l_npp.npp02) IS NULL OR    #FUN-D70118 mark
            l_npq.npq03 IS NULL OR s_get_aznn(g_plant,p_bookno,l_npp.npp02,1) IS NULL OR  #FUN-D70118 add
           ...
           #l_npq.npq05 IS NULL OR MONTH(l_npp.npp02) IS NULL THEN  #FUN-D70118 mark
            l_npq.npq05 IS NULL OR s_get_aznn(g_plant,p_bookno,l_npp.npp02,3) IS NULL THEN  #FUN-D70118 add
...
#FUN-950053 -----------------add end------------------------------------
      #CHI-DC0054 add---------------------------------------------------
      IF p_sys = 'FA' AND l_npp00 = 4 THEN
         SELECT occ37 INTO g_occ37 FROM occ_file,fbe_file
          WHERE occ01 = fbe03
            AND fbe01 = l_npq.npq01
         LET l_pmc903 = g_occ37
      END IF
      #CHI-DC0054 add end-----------------------------------------------
