...
# Modify.........: No.MOD-D70176 13/07/26 By apo 還原MOD-C70022段
# Modify.........: No.FUN-D70120 13/07/31 By yangtt 會計年期會不按照自然年月設置,年期的判斷需要按照aooq011的設置
# Modify.........: No.MOD-D80103 13/08/16 By suncx 工作量法折舊時，控制最後一期折舊後纍計折舊不能大於成本
# Modify.........: No.CHI-D90007 13/09/05 By apo 增加判斷 m_amt = 0 時 CONTINUE FOREACH
...
# Modify.........: No.MOD-E60021 14/06/05 By yihsuan :873 行計算 m_amt 時,增加 -(g_faj.faj101-g_faj.faj102) 此段處理
# Modify.........: No.MOD-F30026 15/03/09 By doris q_faj開窗條件增加 fan041 為 2 的資料
# Modify.........: No.CHI-E10030 15/05/13 By doris 若為多部門分攤且分攤方式固定,其折舊金額(fan07)計算完後應檢查累計折舊是否大於成本,如果已超過成本應分攤到其他部門
# Modify.........: No.MOD-H20029 17/02/14 By Sabrina 程式變成無窮迴圈  
     ...
     ON ACTION CONTROLP
         CASE
            ...
            WHEN INFIELD(faj02)
              ...
              #LET g_qryparam.where = " faj02 NOT IN (SELECT fan01 FROM fan_file WHERE fan03='",g_yy,"' AND fan04='",g_mm,"')"   #MOD-CA0224 add   #MOD-F30026 mark
               LET g_qryparam.where = " faj02 NOT IN (SELECT fan01 FROM fan_file WHERE fan03='",g_yy,"' AND fan04='",g_mm,"' AND fan041='2')"      #MOD-F30026 add
               CALL cl_create_qry() RETURNING g_qryparam.multiret
   ...
   FOREACH p300_cur INTO g_type,g_faj.*
     ...
     #IF g_faj.faj30 = 0 and ( g_faj.faj57 < m_yy or           #MOD-CC0065 mark
      ...
      IF m_faj30=1 THEN   
         ...
         ELSE  
            LET m_amt=g_faj.faj14+g_faj.faj141-g_faj.faj59
                                 ...
                                 -(g_faj.faj101-g_faj.faj102)   #MOD-E60021 add
         END IF
      ELSE
         IF m_faj30 = 0 THEN 
                  ...
                  ELSE           
                     ...
                     IF l_date_new <= l_date_today THEN
                        ...
                        #LET l_year_new = YEAR(l_date_new)+1
                         CALL s_get_bookno(YEAR(l_date_new)+1) RETURNING g_flag,g_bookno1,g_bookno2
                         IF g_flag = '1' THEN
                            CALL cl_err(YEAR(l_date_new)+1,'aoo-081',1)
                         END IF
                         LET l_year_new = s_get_aznn(g_plant,g_bookno1,l_date_new,1) + 1
                        #FUN-D70120---mod---end--
                        ...
                        #LET l_month2 = MONTH(l_date2) USING '&&'
                         CALL s_get_bookno(YEAR(l_date2)) RETURNING g_flag,g_bookno1,g_bookno2
                         IF g_flag = '1' THEN
                            CALL cl_err(YEAR(l_date2),'aoo-081',1)
                         END IF
                         LET l_year2 = s_get_aznn(g_plant,g_bookno1,l_date2,1)
                         LET l_month2 = s_get_aznn(g_plant,g_bookno1,l_date2,3) USING '&&'
                        #FUN-D70120---mod---end--
      ...
      IF m_amt < 0 THEN    
         ...
         CONTINUE FOREACH
      END IF
      IF m_amt = 0 THEN CONTINUE FOREACH END IF   #CHI-D90007   
        ...
        #IF l_over = 'N' THEN                                               #MOD-E30084 mark
         IF l_over = 'N' OR (g_faj.faj28='4' AND l_over = 'Y' ) THEN        #MOD-E30084
            LET l_faj33 = (g_faj.faj14 + g_faj.faj141) - g_faj.faj59 
                      ...
                      # - (g_faj.faj101 - g_faj.faj102)                   #MOD-D70176 mark   #MOD-C70022 add
...
#                     累積折舊=全期本期折舊(fan07)的加總
           ...
           #CHI-E10030---add---str--
            IF m_fad05 = '1' THEN
               CALL p300_fan07_diff(g_fan.fan01,g_fan.fan02,g_fan.fan03,g_fan.fan04)
            END IF
           #CHI-E10030---add---end--
      ...
      IF g_faa.faa15 != '4' THEN
         IF l_over = 'N' THEN
            LET l_faj332 = (g_faj.faj142 + g_faj.faj1412) - g_faj.faj592
                       ...
                       # - (g_faj.faj1012 - g_faj.faj1022)                    #MOD-D70176 mark   #MOD-C70022 add
...
#CHI-E10030---add---str--
FUNCTION p300_fan07_diff(p_fan01,p_fan02,p_fan03,p_fan04)
   DEFINE p_fan01   LIKE fan_file.fan01,
          p_fan02   LIKE fan_file.fan02,
          p_fan03   LIKE fan_file.fan03,
          p_fan04   LIKE fan_file.fan04
   DEFINE l_sql     STRING
   DEFINE l_amt     LIKE type_file.num20_6
   DEFINE l_d,l_a   LIKE type_file.num5
   DEFINE l_cnt     LIKE type_file.num5
   DEFINE l_len     LIKE type_file.num5
   DEFINE l_diff DYNAMIC ARRAY OF RECORD
                    fan01   LIKE fan_file.fan01,
                    fan02   LIKE fan_file.fan02,
                    fan03   LIKE fan_file.fan03,
                    fan04   LIKE fan_file.fan04,
                    fan041  LIKE fan_file.fan041,
                    fan05   LIKE fan_file.fan05,
                    fan06   LIKE fan_file.fan06,
                    diff    LIKE type_file.num20_6,
                    residual LIKE type_file.num20_6
                 END RECORD
   DEFINE l_adjj DYNAMIC ARRAY OF RECORD
                    fan01   LIKE fan_file.fan01,
                    fan02   LIKE fan_file.fan02,
                    fan03   LIKE fan_file.fan03,
                    fan04   LIKE fan_file.fan04,
   ...
   LET l_a = 1 
   #累計折舊大於成本
   LET l_sql = "SELECT fan01,fan02,fan03,fan04,fan041, ",
               "       fan05,fan06,fan14 - fan15,ABS(fan14 - fan15) ",
               "  FROM fan_file ",
               " WHERE fan01 = '",p_fan01,"' ",
               "   AND fan02 = '",p_fan02,"' ",
               "   AND fan03 = ",p_fan03,
               ...
               "   AND fan05 = '3' ",
               "   AND fan14 - fan15 < 0 "
   PREPARE p300_diff_p FROM l_sql
   DECLARE p300_diff_c CURSOR FOR p300_diff_p

   #累計折舊未大於成本
   LET l_sql = "SELECT fan01,fan02,fan03,fan04,fan041, ",
               "       fan05,fan06,fan14 - fan15,fan14 - fan15 ",
               "  FROM fan_file ",
               " WHERE fan01 = '",p_fan01,"' ",
               "   AND fan02 = '",p_fan02,"' ",
               "   AND fan03 = ",p_fan03,
               ...
               "   AND fan05 = '3' ",
               "   AND fan14 - fan15 > 0 "
   PREPARE p300_adj_p FROM l_sql
   DECLARE p300_adj_c CURSOR FOR p300_adj_p
   FOREACH p300_adj_c INTO l_adjj[l_a].*
      LET l_a = l_a + 1  
   END FOREACH
   CALL l_adjj.deleteElement(l_a)
   LET l_len = l_adjj.getLength()
   LET l_a = 1

   FOREACH p300_diff_c INTO l_diff[l_d].*
      #每次以1為單位調整,最後再調整小數
      WHILE l_diff[l_d].residual > 0      #表示還超過成本(diff)
         FOR l_a = 1 TO l_len 
            IF l_diff[l_d].residual < 1 THEN
               LET l_amt = l_diff[l_d].residual
            ELSE
               LET l_amt = 1
            END IF
            IF l_diff[l_d].residual > 0 THEN
               IF l_adjj[l_a].residual > 0 THEN  #表示還未超過成本(adj)
                  UPDATE fan_file
                     SET fan07 = fan07 - l_amt, 
                         fan08 = fan08 - l_amt,
                         fan15 = fan15 - l_amt
                   WHERE fan01 = l_diff[l_d].fan01 
                     AND fan02 = l_diff[l_d].fan02 
                     AND fan03 = l_diff[l_d].fan03 
                     AND fan04 = l_diff[l_d].fan04 
                     AND fan041 = l_diff[l_d].fan041    
                     AND fan05 = l_diff[l_d].fan05
                     AND fan06 = l_diff[l_d].fan06
               
                  UPDATE fan_file
                     SET fan07 = fan07 + l_amt,
                         fan08 = fan08 + l_amt,
                         fan15 = fan15 + l_amt
                   WHERE fan01 = l_adjj[l_a].fan01 
                     AND fan02 = l_adjj[l_a].fan02
                     AND fan03 = l_adjj[l_a].fan03
                     AND fan04 = l_adjj[l_a].fan04
                     AND fan041 = l_adjj[l_a].fan041
                     AND fan05 = l_adjj[l_a].fan05
                     AND fan06 = l_adjj[l_a].fan06
               
                  LET l_diff[l_d].residual = l_diff[l_d].residual - l_amt
                  LET l_adjj[l_a].residual = l_adjj[l_a].residual - l_amt
               ELSE
                  CONTINUE FOR
               END IF
            END IF
         ...
         SELECT COUNT(*) INTO l_cnt
          ...
          WHERE fan01 = p_fan01
            ...
            AND fan041 = '1' 
            AND fan05 = '3' 
           #AND fan14 - fan15 > 0     #MOD-H20029 mark
            AND fan14 - fan15 < 0     #MOD-H20029 add
         IF cl_null(l_cnt) THEN LET l_cnt = 0 END IF
         IF l_cnt = 0 THEN
            EXIT WHILE
         END IF
