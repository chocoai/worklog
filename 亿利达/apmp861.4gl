...
# Modify.........: No:MOD-DC0150 13/12/25 By SunLM  多角抛转应付尾差调整(140224新增部分逻辑)
# Modify.........: No:140801     14/08/01 By cjy   单身ombud02根据aimi100其他分群码一取值
...
DEFINE p_apa34f     LIKE apa_file.apa34f #MOD-DC0150 记录采购单含税金额原币
...
DEFINE l_cnt    LIKE type_file.num5   #CHI-F20001 add
  ...
  SELECT ima906 INTO g_ima906 FROM ima_file
    ...
    #MOD-B30012 add --end--
     ...
     #add by cjy 20140801
     IF p_omb.ombud02 is null THEN
        # mod by lixwz 180403 s---
        Let l_sql = " SELECT ima09 FROM yld005.ima_file ",
                    " WHERE ima01 = '",p_omb.omb04 CLIPPED,"'"
        PREPARE sel_ima09 FROM l_sql
        EXECUTE sel_ima09 INTO p_omb.ombud02
        #SELECT ima09 INTO p_omb.ombud02 FROM yld005.ima_file WHERE ima01 = p_omb.omb04
        # mod by lixwz 180403 e---
     END IF
     #end add by cjy 20140801
...
FUNCTION p861_ins_omb()
   DEFINE l_legal   LIKE azw_file.azw02 #FUN-980006 add
    ...
    LET l_sql1="INSERT INTO ",cl_get_target_table(p_plant,'omb_file'), #FUN-A50102
               ...
               " omb35,omb36,omb37,omb930,omblegal,ombud02) ",   #,ombplant) ", #A060 #MOD-920266 add omb930 #FUN-960141 add ombplant   #FUN-960141 090824 del ombplant #FUN-980006 add omblegal #add ombud02 by cjy 20140801
               " VALUES( ?,?,?,?, ?,?,?,?, ?,?,?,?, ?,?,?,?,",
                        "?,?,?,?,? ,?,? ,?,?,?,?,?  ,?) "  #A060  #No.FUN-670026   #No.FUN-6A0026 #MOD-920266 #FUN-960141 add 1 ?   #FUN-960141 090824 del 1 ? #FUN-980006 add ? for omblegal   #add ? by cjy 20140801
     ...
     CALL cl_parse_qry_sql(l_sql1,p_plant) RETURNING l_sql1 #FUN-A50102
         CALL s_getlegal(p_plant) RETURNING l_legal #FUN-980006 add
           ...
           EXECUTE ins_omb USING
                ...
                ,p_omb.ombud02   #add by cjy 20140801
  ...
  END IF
     ...
     IF p_rvv.rvv25='Y' THEN  #No.9421
        ...
        #LET p_apa32= p_apa32 + 0  #MOD-C70105  #TQC-C90045 mark
         LET p_apa34f = p_apa34f + 0 #MOD-DC0150
     ELSE
        #----------------計算單頭金額-----------------------------
           #記錄採購出貨金額(原幣)
...
#MOD-C30815 add 不用含稅單價 * 計價數量 來算稅額,避免產生尾差,改用含稅推未稅(或者直接從rvv39,rvv39t取值)
          ...
          LET p_apa34f = p_apa34f + l_sum #MOD-DC0150
   IF l_gec.gec07='Y' THEN
     IF l_gec.gec05 = 'T' THEN
...
#MOD-B30012 add --start--
   ...
   DEFINE l_ombud02  LIKE omb_file.ombud02 #ADD by cjy 20140801
...
#MOD-B30012 add --end--
  ...
  CALL p861_occ37(p_npq.npq03,g_bookno1,p_oma.oma03,p_npq.npq37,p_dbs,p_plant)  #FUN-A50102
   RETURNING p_npq.npq37
     ...
     LET l_sql = "SELECT omb33,nvl(ombud02,'0'),SUM(omb14),SUM(omb16) ",            #MOD-C60074 add #add ombud02 by cjy 20140801
                 "  FROM ",cl_get_target_table( p_plant, 'omb_file' ),
                 " WHERE omb01='",p_oma.oma01,"'",
                 #" GROUP BY omb33"
                 " GROUP BY omb33,ombud02"   #add ombud02 by cjy 20140801
     CALL cl_replace_sqldb(l_sql) RETURNING l_sql
     CALL cl_parse_qry_sql(l_sql,p_plant) RETURNING l_sql
     ...
     FOREACH omb_c1 INTO l_omb33,l_ombud02,l_omb14,l_omb16 #MOD-C60074 add   #add ombud02 by cjy 20140801
     ...
     #MOD-B30012 add --end--
       ...
       #CALL p861_occ37(p_npq.npq03,g_bookno1,p_oma.oma03,p_npq.npq37,p_dbs)
        ...
        RETURNING p_npq.npq37
         ...
         #add by cjy 20140801
        IF p_npq.npq11 IS NULL THEN
        #mod by pane 150901 begin#
        # SELECT COUNT(*) INTO l_n FROM aag_file WHERE aag00 = g_bookno1
        #    AND aag01 = p_npq.npq03
        #    AND aag15 = '00'
        #mod by pane 150901 end#
        #mod by pane 150901 begin#
         LET l_sql = " SELECT COUNT(*) ",
                     ...
                     "   AND aag01 = '",p_npq.npq03,"'",
                     "   AND aag15 = '00' "
         PREPARE p FROM l_sql
         EXECUTE p INTO l_n
        #mod by pane 150901 end#
          IF l_n > 0 AND not cl_null(l_ombud02)THEN
             LET p_npq.npq11 = l_ombud02
          END IF
        END IF
      #end add by cjy 20140801
        #新增一筆分錄底稿單身檔
    ...
    LET l_sql1="INSERT INTO ",cl_get_target_table(p_plant,'npq_file'), #FUN-A50102
     ...
     CALL cl_parse_qry_sql(l_sql1,p_plant) RETURNING l_sql1 #FUN-A50102
        ...
        IF SQLCA.sqlcode<>0 THEN
              ...
              LET g_success = 'N'
        END IF
       # CALL s_flows('3','',p_npq.npq01,p_npp.npp02,'N',p_npq.npqtype,TRUE)   #No.TQC-B70021    #mark by zhousw 131012
END FUNCTION
...
FUNCTION p861_ap_apa()
  ...
  DEFINE g_t1    LIKE oay_file.oayslip #CHI-C60004 add
   DEFINE l_diff_apa34f  LIKE apa_file.apa34f, #MOD-DC0150 add
         l_diff_apa34   LIKE apa_file.apa34,   #MOD-DC0150 add
         l_diff_apa31   LIKE apa_file.apa31,   #MOD-DC0150 add
         l_diff_apa31f  LIKE apa_file.apa31f   #MOD-DC0150 add
   DEFINE l_apb10,l_apb24 like apb_file.apb10 #140224
  ...
  #讀取
       ...
       #LET p_apa.apa17= '3'            #扣抵區分 3:不可扣抵進貨及費用 #CHI-B50022 mark
        ...
        LET p_apa.apa34 = p_apa.apa31+p_apa.apa32  #本幣合計
         #MOD-DC0150 add begin----------------------
        IF p_gec.gec07 = 'Y' THEN
            #MOD-DC0150 add beg-----------------
            IF p_apa.apa34f <> p_apa34f   THEN #含税金额有小数尾差
                LET l_diff_apa34f = p_apa34f - p_apa.apa34f #原币小数尾差
                LET l_diff_apa34 = l_diff_apa34f * p_apa.apa14 #本币小数尾差
                ...
                #CALL  p861_up_apb_2(p_apa.apa01,l_diff_apa34f,l_diff_apa34) #140224 mark

                LET p_apa.apa34f = p_apa34f  #原币含税金额
                LET p_apa.apa31f = p_apa.apa34f / (1+p_apa.apa16/100) #原幣未稅
...
#140224 mark begin----------
#               #IF p_apa.apa31f <> p_apa31f THEN #未税金额尾差
#                   LET l_diff_apa31f = p_apa.apa31f - p_apa31f
#                   LET l_diff_apa31  = l_diff_apa31f *  p_apa.apa14
...
#                   CALL  p861_up_apb_2(p_apa.apa01,l_diff_apa31f,l_diff_apa31)
#140224 mark end------------
                   LET p_apa.apa31 = p_apa.apa31f * p_apa.apa14  #本币未税金额
                   CALL cl_digcut(p_apa.apa31 ,s_azi.azi04) RETURNING p_apa.apa31
                   LET p_apa.apa34 =  p_apa.apa34f * p_apa.apa14 #本币含税金额
                   CALL cl_digcut(p_apa.apa34 ,s_azi.azi04) RETURNING p_apa.apa34
                   LET p_apa.apa32 = p_apa.apa34 - p_apa.apa31 #本币税额
                #END IF
                #140224 add begin-------
                ...
                CALL cl_replace_sqldb(l_sql) RETURNING l_sql
                CALL cl_parse_qry_sql( l_sql, p_plant ) RETURNING l_sql
                PREPARE p861_prep841 FROM l_sql
                DECLARE p861_curs841 CURSOR FOR p861_prep841
                OPEN p861_curs841
                FETCH p861_curs841 INTO l_apb24,l_apb10
                IF  l_apb24 <> p_apa.apa31f THEN  #
                   LET l_diff_apa31f = p_apa.apa31f - l_apb24
                   LET l_diff_apa31  = l_diff_apa31f *  p_apa.apa14
                   CALL cl_digcut(l_diff_apa31f,l_azi.azi04) RETURNING l_diff_apa31f
                   CALL cl_digcut(l_diff_apa31 ,s_azi.azi04) RETURNING l_diff_apa31
                   CALL  p861_up_apb_2(p_apa.apa01,l_diff_apa31f,l_diff_apa31)
...
# copy from axr/s_up_omb.4gl
  ...
  IF m_oma.oma213='N' THEN
     ...
     IF l_oma59t <> m_oma.oma56t THEN     						#MOD-9A0163
        #IF cl_confirm("axr-007") THEN                #mark by zhousw 131012
     #外加稅
...
FUNCTION p861_up_apb_2(p_apa01,l_diff_apa34f,l_diff_apa34)
