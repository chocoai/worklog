...
# Modify.........: No:MOD-D80057 13/08/09 By bart 改為ima01= l_cdj.cdj06
# Modify.........: No:FUN-D90054 13/09/17 By yangtt cdj10賦值為tlf10*tlf60
# Modify.........: No:MOD-E10042 14/01/06 By suncx 销退时，数量应该为负数
# Modify.........: No:MOD-E80084 14/08/14 By qiull 修正MOD-E10042的問題
...
DEFINE l_flag     LIKE type_file.chr1,  #FUN-D70127
       l_bookno1  LIKE aza_file.aza81,  #FUN-D70127
       l_bookno2  LIKE aza_file.aza82   #FUN-D70127

   CALL s_get_bookno(YEAR(g_today)) RETURNING l_flag,l_bookno1,l_bookno2 #FUN-D70127
   #FUN-BB0038 --begin
  ...
  #IF g_oaz.oaz92 = 'N' OR (g_oaz.oaz92 = 'Y' AND g_oaz.oaz93 = 'Y') THEN  #FUN-C90002   #CHI-EC0033 mark
   IF g_oaz.oaz92 = 'N' OR g_oaz.oaz92 IS NULL OR (g_oaz.oaz92 = 'Y' AND g_oaz.oaz93 = 'Y') THEN  #FUN-C90002   #CHI-EC0033 add
      SELECT tlfclegal,tlfctype,tlfccost,tlf01,tlf14,-(tlf907*tlfc21) a,oga03,oga15,tlf15,tlf16,oga032,(tlf10*tlf60) b   #FUN-D90054 add (tlf10*tlf60) b
        FROM tlf_file,tlfc_file,oga_file,aznn_file #FUN-D70127 add aznn_file                                           
       WHERE tlf905=oga01 AND tlfc01=tlf01 AND tlfc06=tlf06 AND tlfc02=tlf02        
         AND tlfc03=tlf03 AND tlfc13=tlf13 AND tlfc902=tlf902 AND tlfc903=tlf903     
         AND tlfc904=tlf904 AND tlfc907=tlf907 AND tlfc905=tlf905 AND tlfc906=tlf906
        #AND YEAR(tlf06)=tm.yy AND MONTH(tlf06)=tm.mm #FUN-D70127 mark
         AND aznn02=tm.yy AND aznn04=tm.mm     #FUN-D70127
         AND aznn01=tlf06 AND aznn00=l_bookno1 #FUN-D70127                                              
         AND tlfctype=tm.tlfctype          
         AND oga57<>'2' #过滤代收                                
...
#     SELECT oaz92 INTO g_oaz.oaz92 FROM oaz_file              #FUN-C90002
      IF g_oaz.oaz92 = 'Y' AND g_oaz.oaz93 = 'Y' THEN           #FUN-C90002
         SELECT tlfclegal,tlfctype,tlfccost,tlf01,tlf14,tlfc21 a,omf05 oga03,oga15,tlf15,tlf16,omf051 oga032,(tlf10*tlf60) b   #FUN-D90054 add (tlf10*tlf60) b
           FROM tlf_file,tlfc_file,omf_file,oga_file,aznn_file #FUN-D70127 add aznn_file                                           
  ...
  #No.FUN-D70068 ---Mark--- Start
   #FUN-C90002  --end
   IF g_oaz.oaz92 = 'N' OR cl_null(g_oaz.oaz92) OR (g_oaz.oaz92 = 'Y' AND g_oaz.oaz93 = 'Y') THEN  #FUN-C90002   #CHI-EC0033 add
  #IF g_oaz.oaz92 = 'N' OR (g_oaz.oaz92 = 'Y' AND g_oaz.oaz93 = 'Y') THEN  #FUN-C90002   #CHI-EC0033 mark
  #SELECT tlfclegal,tlfctype,tlfccost,tlf01,tlf14,-(tlf907*tlfc21) a,oha03 oga03,oha15 oga15,tlf15,tlf16,oha032 oga032,(tlf10*tlf60) b   #FUN-D90054 add (tlf10*tlf60) b                                      
   SELECT tlfclegal,tlfctype,tlfccost,tlf01,tlf14,-(tlf907*tlfc21) a,oha03 oga03,oha15 oga15,tlf15,tlf16,oha032 oga032,-1 * (tlf10*tlf60) b   #FUN-D90054 add (tlf10*tlf60) b  #MOD-E10042 add -1 *                                     
     FROM tlf_file,tlfc_file,oha_file,aznn_file #FUN-D70127 add aznn_file                                           
...
#   IF g_oaz.oaz92 = 'Y' THEN                       #FUN-C90002
    IF g_oaz.oaz92 = 'Y' AND g_oaz.oaz93 = 'Y' THEN  #FUN-C90002
      SELECT tlfclegal,tlfctype,tlfccost,tlf01,tlf14,-tlfc21 a,omf05 oga03,oha15 oga15,tlf15,tlf16,omf051 oga032,(tlf10*tlf60) b   #FUN-D90054 add (tlf10*tlf60) b                                      
        FROM tlf_file,tlfc_file,omf_file,oha_file,aznn_file #FUN-D70127 add aznn_file                                           
  ...
  #No.FUN-D70068 ---Add--- End
   #FUN-C80009 --end
   #将相同客户，部门，理由码，科目汇总 
   LET l_sql = " SELECT tlfclegal,tlfctype,tlfccost,tlf01,tlf14,SUM(a),oga03,oga15,tlf15,tlf16,oga032,b ",  #FUN-D90054 add b #MOD-E10042 mark   #MOD-E80084 remark
  #LET l_sql = " SELECT tlfclegal,tlfctype,tlfccost,tlf01,tlf14,SUM(a),oga03,oga15,tlf15,tlf16,oga032,SUM(b) ",  #MOD-E10042 SUM(b)              #MOD-E80084 mark
               "   FROM cdj_temp ",
               "  GROUP BY tlfclegal,tlfctype,tlfccost,tlf01,tlf14,oga03,oga15,tlf15,tlf16,oga032,b "   #FUN-D90054 add b #MOD-E10042 mark       #MOD-E80084 remark 
              #"  GROUP BY tlfclegal,tlfctype,tlfccost,tlf01,tlf14,oga03,oga15,tlf15,tlf16,oga032 "     #MOD-E10042 add                          #MOD-E80084 mark
      ...
      IF cl_null(l_cdj.cdj08) THEN   #借方
         ...
         IF cl_null(actno_d1) THEN 
           SELECT oba16,oba161 INTO actno_d1,actno_d2 FROM oba_file 
            #WHERE oba01 IN (SELECT ima131 FROM ima_file WHERE ima01=tlf01)  #MOD-D80057
            WHERE oba01 IN (SELECT ima131 FROM ima_file WHERE ima01=l_cdj.cdj06)  #MOD-D80057
         END IF
         IF cl_null(actno_d1) THEN 
