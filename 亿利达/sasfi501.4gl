...
# Modify.........: No:TQC-B60136 11/06/17 By jan取替代料的退料 退料數量檢查錯誤
# Modify.........: No:FUN-C80107 12/09/19 By suncx 增可按倉庫進行負庫存判斷
# Modify.........: NO.120502     12/05/02 by xiayan sfq单身工单增加多选功能
# Modify.........: No:150715     15/07/15 By yangjian 关联交易自动生成的领料单不允许操作
# Modify.........: No:151012     15/10/12 By yangjian WMS集成仓库手工发料审核、取消审核通知WMS
# Modify.........: No:160318     16/03/18 By liming 新增发料料号增加管控，发料量管控
...
DEFINE g_sma894     LIKE type_file.chr1           #FUN-C80107
#add by sunar170825-------begin--------
DEFINE   w    ui.Window
DEFINE   f    ui.Form
DEFINE   page om.DomNode
#add by sunar170825-------end---------
...
#MOD-B30148 --------------------End----------------------------
    ...
    IF NOT cl_null(g_argv3) THEN
       CASE g_argv4
          ...
          WHEN "efconfirm"
             ...
             CALL i501_q()
             CALL i501sub_y_chk(g_sfp.sfp01)                           #CALL 原確認的 check 段
             IF g_success = 'Y' THEN
          ...
          WHEN "auto"
             LET g_action_choice = "efconfirm"
             #add by liudong 161222
             LET l_str = "date +'%Y/%m/%d %H:%M:%S' |awk '{print \"Start i501_q:",p_argv3," \"$0}' >> /u1/out/aws_chktime.log "
             RUN l_str
             #end by liudong 161222
             #IF l_str = '1' THEN
               LET g_sfp.sfp01=p_argv3
             #ELSE
             #  CALL i501_q()
             #END IF
             #add by liudong 161222
             LET l_str = "date +'%Y/%m/%d %H:%M:%S' |awk '{print \"Start subchk:",p_argv3," \"$0}' >> /u1/out/aws_chktime.log "
             RUN l_str
             #end by liudong 161222
             CALL i501sub_y_chk(g_sfp.sfp01)                           #CALL 原確認的 check 段
             IF g_success = 'Y' THEN
             #add by liudong 161222
             LET l_str = "date +'%Y/%m/%d %H:%M:%S' |awk '{print \"Start subupd:",p_argv3," \"$0}' >> /u1/out/aws_chktime.log "
             RUN l_str
             #end by liudong 161222
                CALL i501sub_y_upd(g_sfp.sfp01,g_action_choice,FALSE) #CALL 原確認的 update 段
                      RETURNING g_sfp.*
             ...
             IF g_success = 'Y' THEN
             #add by liudong 161222
             LET l_str = "date +'%Y/%m/%d %H:%M:%S' |awk '{print \"Start subpst:",p_argv3," \"$0}' >> /u1/out/aws_chktime.log "
             RUN l_str
             #end by liudong 161222
                CALL i501sub_s(g_argv1,g_sfp.sfp01,FALSE,'N') #FUN-740187
             END IF
             EXIT PROGRAM
          OTHERWISE
             CALL i501_q()
    ...
    CALL aws_efapp_flowaction("insert, modify, delete, detail, query, locale, void, confirm, undo_confirm, stock_post, undo_post, easyflow_approval, sets, gen_transfer_note, reproduce, regen_detail, s_icdout, exporttoexcel, modi_lot, warahouse_modify")
...
DEFINE l_length,l_i,l_cc1 LIKE type_file.num5    #No.FUN-680121 SMALLINT
...
####
    ...
    CONSTRUCT BY NAME g_wc ON                                 # 螢幕上取單頭條件
       #sfp01,sfp02,sfp03,sfp06,sfp07,sfp08,sfp05,sfpconf,    #FUN-660106  #FUN-AB0001 mark
        ...
        sfp04,sfp09,sfp10,sfp11,sfp15,                        #FUN-630084 add sfp11  #FUN-AB0001 add:sfp15
        sfpuser,sfpgrup,sfpmodu,sfpdate
        ,sfpud01,sfpud02,sfpud03,sfpud04,sfpud05,
        sfpud06,sfpud07,sfpud08,sfpud09,sfpud10,
        sfpud11,sfpud12,sfpud13,sfpud14,sfpud15
    ...
    END CONSTRUCT
    CONSTRUCT g_wc3 ON sfs02,sfs26,sfs03,sfs04,sfs27,sfs012,sfs013,sfs06,sfs10,  ##FUN-940039 add sfs27  #FUN-A60028 add sfs012,sfs013
                       sfs07,sfs08,sfs09,sfs05,sfs21,sfs33,sfs34,
                       sfs35,sfs30,sfs31,sfs32,sfs930,sfs36  #FUN-670103  #FUN-950088 add sfs36
                       ,sfsud01,sfsud02,sfsud03,sfsud04,sfsud05,
                       sfsud06,sfsud07,sfsud08,sfsud09,sfsud10,
                       sfsud11,sfsud12,sfsud13,sfsud14,sfsud15
                  FROM s_sfs[1].sfs02,s_sfs[1].sfs26,s_sfs[1].sfs03,
                       s_sfs[1].sfs04,s_sfs[1].sfs27,s_sfs[1].sfs012,s_sfs[1].sfs013,s_sfs[1].sfs06,s_sfs[1].sfs10,
    ...
    END CONSTRUCT
        ...
        ON ACTION controlp
          CASE WHEN INFIELD(sfp01) #查詢單据
                    ...
                    ##組合拆解的工單發料不顯示出來!
                   #LET g_qryparam.where = " substr(sfp01,1,",g_doc_len,") NOT IN (SELECT smy70 FROM smy_file WHERE smy70 IS NOT NULL) "
                    LET g_qryparam.where = " sfp01[1,",g_doc_len,"] NOT IN (SELECT smy70 FROM smy_file WHERE smy70 IS NOT NULL) "  #FUN-B40029
                    CALL cl_create_qry() RETURNING g_qryparam.multiret
                    DISPLAY g_qryparam.multiret TO sfp01
                    NEXT FIELD sfp01
               WHEN INFIELD(sfp07)
                    CALL cl_init_qry_var()
                    LET g_qryparam.state  = "c"
                    LET g_qryparam.form   ="cq_gem15"  #xiaoyxq_gem"
                    CALL cl_create_qry() RETURNING g_qryparam.multiret
                    DISPLAY g_qryparam.multiret TO sfp07
                    NEXT FIELD sfp07
               WHEN INFIELD(sfp08)   #料表批號
...
#        WHEN 'sfs01'  LET l_buf[l_i,l_i+4] ='sfe02' #FUN-A60028
         WHEN 'sfs01'  LET l_buf[l_i,l_i+4] ='sfe01' #FUN-A60028
  ...
  ELSE
    # LET g_wc = " sfp01 = '",g_argv3,"'" #add 160719001
   LET l_cc1 = 0
   SELECT count(*) INTO l_cc1 FROM sfp_file WHERE sfpud04 = g_argv3 #add 160719001
   IF l_cc1 >= 1  THEN
    LET g_wc = " sfpud04 = '",g_argv3,"'"
   ELSE
...
FUNCTION i501_menu()
   ...
   DEFINE l_cnt        LIKE type_file.num5      #FUN-B40082
      DEFINE ls_sfs04     LIKE sfs_file.sfs04
     DEFINE ls_sfa05     LIKE sfa_file.sfa05
     DEFINE ls_sfa06a    LIKE sfa_file.sfa05  #yxxiao
     DEFINE l_sfpconf    LIKE sfp_file.sfpconf  # add by lixwz180329
   ...
   #xiaoyx 2011/10/11
       ...
       #@WHEN "取消確認"
         WHEN "undo_confirm"
            ...
            END IF
         WHEN "stock_post"
               # add by lixwz180329 s---
               IF cl_chk_act_auth() THEN
                  SELECT sfpconf INTO l_sfpconf FROM sfp_file
		...
		CALL s_showmsg()
         ...
         WHEN "void"
            IF cl_chk_act_auth() THEN
               CALL i501sub_x(g_sfp.sfp01,g_action_choice,TRUE)   #FUN-920175
               CALL i501sub_refresh(g_sfp.sfp01) RETURNING g_sfp.*  #FUN-920175
               CALL i501_show()  #FUN-920175
            END IF
         ...
         #                             base.TypeInfo.create(g_sfq),            #mark by sunar170825
         #                             base.TypeInfo.create(g_sfs),'')
            #modify by sunar170825---------begin-----------
...
#End Mark TQC-AC0125
    ...
    IF g_plant='YLD007' AND g_prog = 'asfi511' THEN
      	 LET g_sfp.sfp07='CZCJ'
    ELSE
   ...
   END IF
      ...
      #FUN-AC0074 -----------End--------------
        ...
        IF g_sfp.sfp06 MATCHES '[136AB]' AND l_cnt1 = 0 THEN CALL i501_g_b() END IF   #FUN-5B0037 #FUN-5C0114 add 'AB'
...
FUNCTION i501_i(p_cmd)
  ...
  DEFINE ls_cnt          LIKE type_file.num5   #xiaoyx
  DEFINE l_sfk02         LIKE sfk_file.sfk02   #add by liming 160318
  DEFINE l_cnt           LIKE type_file.num5   #add by liming 160318
    CALL cl_set_head_visible("","YES")  #NO.FUN-6B0031
...
#liming 160318 add----------------------
         IF p_cmd='u'THEN
           LET l_cnt=1
         SELECT MIN(sfk02) INTO l_sfk02 FROM sfl_file,sfk_file WHERE sfk01=sfl01  AND sfl02=g_sfs[l_cnt].sfs03 AND sfl03=g_sfs[l_cnt].sfs04
         IF l_sfk02>g_sfp.sfp02 THEN
         CALL cl_err('','csf1022',0)#报错并提示：所填超领时间不可以小于报废时间
        NEXT FIELD sfp02
        END IF
       ELSE
#liming 160318 end---------------------
	       ...
	       IF g_sfp.sfp03 IS NULL THEN
               ...
               END IF
            END IF
        END IF # liming 160318 add

        AFTER FIELD sfp03
#liming 160318 add----------------------------------------------------
           IF g_sfp.sfp02>g_sfp.sfp03 THEN
           CALL cl_err('','csf1026',0)#报错并提示：所填扣账时间不可以小于录入时间
          NEXT FIELD sfp03
        ELSE
#liming 160318 end-------------------------------------------------
 ...
 END IF # liming 160318 add ---------------------------------------
        ...
        AFTER FIELD sfp07
 #xiaoyx
  SELECT COUNT(*) INTO ls_cnt FROM gem_file
         WHERE gem01=g_sfp.sfp07
      #xiaoyx    AND gem01 in ('JG','FW','JC','GC','JJ','SP','DF','XF','DCY','XCY','DYL','XYL','DJ','ZSCJ')
    AND gem01 IN  ('JG','FW','JC','GC','JJ','SP','DF','XF','DCY','XCY','DYL','XYL','DJ','SC',
             'ZSCJ','SP','JJ','GC','DJ','FYZX','BJZX','CCZX','JJZX','DJZX','WZZX','XCY','ZLFJ','ZJFJ','LXFJ',
             'ZJZX','WZZZX','ZJZP','ZJBZX','BJCJ','FP','ZSCJ','ZLFJ','ZPF','ZPO','ZPR','ZPS','ZPW',
             'CZCJ','HJ1','HJ2','QQFJ','HQFJ','QQYL','JCBJ','FZCJ','1003','1007','1009')
         IF ls_cnt IS NULL THEN LET ls_cnt =0 END IF
         IF ls_cnt=0 THEN
           CALL cl_err('','cim-002',1)
          NEXT FIELD sfp07
         END IF
...
FUNCTION i501_show()
    ...
    DISPLAY BY NAME g_sfp.sfporiu,g_sfp.sfporig,
        ...
        g_sfp.sfp15,                                       #FUN-AB0001 add:sfp15
        g_sfp.sfpuser,g_sfp.sfpgrup,                       #NO:6908
        g_sfp.sfpmodu,g_sfp.sfpdate                        #NO:6908
        ,g_sfp.sfpud01,g_sfp.sfpud02,g_sfp.sfpud03,g_sfp.sfpud04,
        g_sfp.sfpud05,g_sfp.sfpud06,g_sfp.sfpud07,g_sfp.sfpud08,
...
# 當有耗材產生時,應將asft620之耗材單號清為NULL
       ...
       #退料單=> UPDATE sfm07 = NULL
        ...
        FOR l_i = 1 TO g_rec_b
            IF NOT s_del_rvbs("1",g_sfp.sfp01,g_sfs[l_i].sfs02,0)  THEN        #FUN-880129  #MOD-890271 modify 2->1
               ROLLBACK WORK
               RETURN
            END IF
        END FOR
...
FUNCTION i501_d()
 DEFINE l_cnt,l_cnt_1  LIKE type_file.num5    #No.FUN-680121 SMALLINT
   ...
   IF g_sfp.sfp06 NOT MATCHES '[136ABC]' THEN RETURN END IF #FUN-5C0114 add ABC
   IF g_sfp.sfpconf = 'Y' THEN CALL cl_err('','9023',1) RETURN END IF #FUN-660106
   IF g_sfp.sfp04 = 'Y' THEN RETURN END IF
   IF g_sfp.sfpconf = 'X' THEN CALL cl_err('','9024',1) RETURN END IF #FUN-660106
...
#  DEFINE l_per         LIKE type_file.num5   #TQC-9B0061         #MOD-AB0047
   DEFINE l_per         LIKE sfq_file.sfq03                       #MOD-AB0047
   ...
   DEFINE ls_sfb08       LIKE sfb_file.sfb08   #xiaoyx
   DEFINE ls_sfq03       LIKE sfq_file.sfq03   #xiaoyx
   DEFINE ls_sfa05       LIKE sfa_file.sfa05   #xiaoyx
   DEFINE l_chk          LIKE type_file.num5
   DEFINE ls_sfs05       LIKE sfs_file.sfs05   #xiaoyx
   DEFINE ls_sfs05a      LIKE sfs_file.sfs05   #xiaoyx
   DEFINE  l_exit_sw       LIKE type_file.num5   #NO.120502 add
   DEFINE l_sfb98        LIKE sfb_file.sfb98

  #NO.120502 add-------------------------begin
   IF g_sfp.sfp06 = '1' OR g_sfp.sfp06 = '3' THEN
      CALL cl_set_act_visible("select_sfq02",TRUE)
   ELSE
      CALL cl_set_act_visible("select_sfq02",FALSE)
   END IF
  #NO.120502 add------------------------end
   ...
   IF NOT cl_null(g_sfp.sfp08) THEN
      IF i=0 THEN
         DECLARE i501_s_sfq_c CURSOR FOR
           SELECT sfd03,'',sfb05,'','','',sfb95,0,sfb08,sfb081   #FUN-870097 add sfb95 #FUN-5C0114 add sfq05
  ...
  #NO.120502 add--------------begin
  WHILE TRUE
    IF NOT l_exit_sw THEN CALL i501_d_fill(' 1=1') END IF
    LET l_exit_sw = TRUE
  #NO.120502 add----------------end
   ...
   INPUT ARRAY g_sfq WITHOUT DEFAULTS FROM s_sfq.*
      ...
      AFTER FIELD sfq02
        #mark by pane 150603客户要求拿掉#
        ##add by cjy 20150601根据客户需求添加判断是否已经存在工单
        #LET l_chk = 0
        #SELECT count(*) into l_chk FROM sfq_file WHERE sfq01 <> g_sfp.sfp01 AND sfq02 = g_sfq[i].sfq02
        #IF l_chk > 0 THEN
        #   CALL cl_err('发料单已经存在重复的工单,请核查','!',2)
        #END IF
        ##END add  by cjy 20150601
        #mark by pane 150603 end#
...
#FUN-AA0059 ---------------------end-------------------------------
       ...
       #add by cjy 20141011
         IF NOT cl_null(g_sfq[i].sfq02) and g_prog = 'asfi511' THEN
            LET l_sfb98 = ''
            SELECT sfb98 INTO l_sfb98 FROM sfb_file WHERE sfb01 = g_sfq[i].sfq02
            IF l_sfb98 <> g_sfp.sfp07 THEN
              CALL cl_err('部门和工单不一致','i',0)
              NEXT FIELD sfq02
            END IF
         END IF
        #end add by cjy
         ...
         IF g_sfp.sfp06 MATCHES '[1]' THEN #MOD-640419 '[16]' -> '[1]'
            ...
            ELSE
             ...
             #    AND sfq04=g_sfq[i].sfq04                   #MOD-A30070
                 #AND (sfq04=g_sfq[i].sfq04 OR sfq04 = ' ')  #MOD-A30070  #FUN-AC0074
                  AND sfq01=sfp01 AND sfp06='1' AND sfp04='Y'
                  AND sfq04=g_sfq[i].sfq04   #FUN-AC0074
...
#MOD-A30070 --BEGIN--
 ...
 #MOD-A30070 --END
            ...
            IF g_sfq[i].sfq03 IS NULL OR
               (l_sfq02_t IS NULL OR l_sfq02_t != g_sfq[i].sfq02) THEN
               IF g_argv1='1' THEN
            ...
            IF g_sfq[i].sfq04 != ' ' THEN
               SELECT SUM(sfq03) INTO l_qty1 FROM sfq_file, sfp_file
                WHERE sfq02=g_sfq[i].sfq02
                 #AND (sfq04=g_sfq[i].sfq04 OR sfq04 = ' ')        #FUN-AC0074
            ...
            IF cl_null(l_sfq03_o) THEN
               LET l_sfq03_o =0
            END IF
            IF l_sfq03+g_sfq[i].sfq03-l_sfq03_o > l_sfb08 THEN     #No.MOD-860012
               CALL cl_err(g_sfq[i].sfq02,'asf-704',0)
         ...
         IF (g_sfp.sfp06 NOT MATCHES '[16ABC]') THEN  #FUN-5C0114 add ABC
            LET g_sfq[i].sfq03=0
         END IF
...
#MOD-A30070 --END--
      ...
      AFTER FIELD sfq03
    {  #xiaoyx
      #当已开单但未过帐,此时sfq03累计会大于sfb08,否则回写sfb081会>sfb08
      #但如果sfq03=0则展不开下阶.
      SELECT sfb08-sfb081 INTO ls_sfb08 FROM sfb_file
       WHERE sfb01=g_sfq[i].sfq02
      SELECT SUM(sfq03) INTO ls_sfq03 FROM sfq_file
       WHERE sfq01<>g_sfp.sfp01
        AND sfq02=g_sfq[i].sfq02
      IF ls_sfq03 IS NULL THEN LET ls_sfq03=0 END IF
      IF ls_sfb08-ls_sfq03 <0 THEN
        LET g_sfq[i].sfq03=0
      ELSE
       LET g_sfq[i].sfq03=ls_sfb08-ls_sfq03
      END IF
      DISPLAY BY NAME g_sfq[i].sfq03
     }
         IF NOT cl_null(g_sfq[i].sfq03) THEN
            ...
            END IF
          { #xiaoyx
           #当不检查发料套数时,此时也要CHECK开立未过帐的数量,xiaoyx
            SELECT sfa05-sfa06  INTO ls_sfa05
                    FROM sfa_file
               ...
               #nouse      AND sfa012=g_sfs[l_ac].sfs012
               #nouse      AND sfa013=g_sfs[l_ac].sfs013
             SELECT SUM(sfs05) INTO ls_sfs05 FROM sfs_file
             WHERE sfs01 <>g_sfp.sfp01
              AND sfs03=g_sfs[l_ac].sfs03
              AND sfs04=g_sfs[l_ac].sfs04
             LET ls_sfs05a=ls_sfa05-ls_sfs05
             IF ls_sfs05a >=0 THEN
               LET g_sfs[l_ac].sfs05=ls_sfs05a
             ELSE
           ...
           #新增時不在此段自動產生單身
            ...
            ELSE
               ...
               IF g_sfp.sfp06 ='1' AND g_sfq[i].sfq03 <> l_sfq03_t THEN
                  LET g_flag_sfq03 = 1
               END IF
            END IF   #MOD-A30065 add

            IF g_sfp.sfp06 ='6' AND g_sfq[i].sfq03 > (qty1-qty2) THEN
            ...
            IF g_sfp.sfp06 = '6' THEN    #成套退料
               IF g_sfq[i].sfq03 <= 0 THEN NEXT FIELD sfq03 END IF
...
#MOD-A30070 --END--
      ...
      AFTER INPUT
         ...
         IF (g_sfq[i].sfq02 IS NOT NULL) AND (NOT g_sfp.sfp06 MATCHES '[ABC]') THEN #FUN-5C0114
            ...
            IF g_sfp.sfp06 MATCHES '[16]' THEN
               SELECT SUM(sfq03) INTO qty1 FROM sfq_file, sfp_file
            ...
            IF g_sfp.sfp06 NOT MATCHES '[16]' THEN
               LET g_sfq[i].sfq03=0
            END IF
      ...
      ON ACTION controlp
         CASE WHEN INFIELD(sfq02)
                  ...
                  #          8.工單一般退料單維護作業
                  #          9.工單領退料維護作業
	           IF g_argv2 MATCHES '[13]' THEN
                      IF g_argv2 = 1 THEN              #FUN-940039 add
                         CALL cl_init_qry_var()
                        ...
                        #LET g_qryparam.where = " substr(sfb01,1,",g_doc_len,") NOT IN (SELECT smy69 FROM smy_file WHERE smy69 IS NOT NULL) "
                         ...
                         LET g_qryparam.where = " sfb01[1,",g_doc_len,"] NOT IN (SELECT smy69 FROM smy_file WHERE smy69 IS NOT NULL) "    #FUN-B40029   #MOD-B60107
                     ...
                     ELSE
                       ...
                       LET li_where = li_where CLIPPED," AND sfb01[1,",g_doc_len,"] NOT IN (SELECT smy69 FROM smy_file WHERE smy69 IS NOT NULL) "   #CHI-9B0006 mod
...
#FUN-AA0059---------mod------------end-----------------
         ...
         END CASE
      #NO.120502 add ---------------------------------begin
       ON ACTION select_sfq02
           IF g_sfp.sfp06 = '1' THEN
              CALL cq_sfq02(FALSE,TRUE,g_sfp.sfp06,g_sfp.sfp01,g_sfp.sfp02,"","","")
           END IF
           IF g_sfp.sfp06 = '3' THEN
              CALL cq_short_qty(TRUE,TRUE,g_sfp.sfp01,g_sfp.sfp02,li_where,'1')
           END IF
           LET l_exit_sw = FALSE
           EXIT INPUT
      #NO.120502 add-----------------------------------end
...
FUNCTION i501_set_no_entry_d(p_cmd)
  DEFINE p_cmd   LIKE type_file.chr1    #No.FUN-680121 VARCHAR(1)
    ...
    IF g_sfp.sfp06 NOT MATCHES '[16AB]' THEN #FUN-5C0114 add AB #FUN-740232 將3拿掉
...
FUNCTION i501_g_b()
 ...
 #DEFINE l_qty           LIKE sfq_file.sfq03    #NO.FUN-A20048  #FUN-AC0074 mark
  ...
  DEFINE l_sma894    STRING                     #No.TQC-A50063
  #No.FUN-A70034  --Begin
              ...
              #No.FUN-A70034  --End
                ...
                #FUN-AC0074 (S)
                 SELECT ima35,ima36,ima136,ima137
                   INTO l_main_ware,l_main_loc,l_wip_ware,l_wip_loc FROM ima_file
                  WHERE ima01=g_sfa2.sfa03
                #FUN-AC0074 (E)
                CALL i501_chk_img(l_main_ware,l_main_loc,l_wip_ware,l_wip_loc,lot_no,FALSE)		# 依 issue_qty1 尋找 img_file可用資料           #FUN-AC0074
...
FUNCTION i501_g_b1() 		# 依套數發料/退料(When sfp06=1/6)
  ...
  LET l_sql = "SELECT sfa_file.*,ima108 FROM sfa_file, ima_file",
              " WHERE sfa01='",b_sfq.sfq02,"'",
              "   AND sfa26 IN ('0','1','2','3','4','5','T','7','8')",    #bugno:7111 add '5T'  #FUN-A20037 add '7,8'
...
FUNCTION i501_chk_img(l_main_ware,l_main_loc,l_wip_ware,l_wip_loc,l_lot_no,l_sie_flag)	# 依 issue_qty1 尋找 img_file可用資料  #FUN-AC0074
    DEFINE l_sql		LIKE type_file.chr1000 #No.FUN-680121 VARCHAR(600)
    ...
    IF cl_null(l_main_loc ) THEN LET l_main_loc =loc_no  END IF
...
DEFINE l_count LIKE type_file.num5   #NO.FUN-A40053 add
    ...
    LET b_sfs.sfs05=cl_digcut(issue_qty2,l_gfe03) #MOD-640364
    ...
    IF g_sfa2.sfa26 MATCHES '[SUTZ]' THEN  #FUN-A20037 add 'Z'
...
DEFINE ls_sfa05,ls_sfs05,ls_sfs05a LIKE sfs_file.sfs05 #xiaoyx
#TQC-B60036--add--end--
DEFINE ls_imn10,ls_img10         LIKE img_file.img10  #xiaoyx
DEFINE l_a1,l_a2        LIKE sfs_file.sfs05   #add by pane 150602
DEFINE l_sfa05_1         LIKE sfa_file.sfa05   #add by pane 150602
DEFINE l_num1           LIKE type_file.num20   #add by liming 160318
DEFINE l_num2           LIKE type_file.num20  #add by liming 160318
DEFINE l_num03           LIKE type_file.num20 #add by liming 160318
DEFINE l_sum_sfl07      LIKE type_file.num20  #add by liming 160318
DEFINE l_num4           LIKE type_file.num20  #add by liming 160318
DEFINE l_sfe16          LIKE sfe_file.sfe16   #add by liming 160318
...
#FUN-AA0059 ---------------------end-------------------------------
           ...
           IF NOT cl_null(g_sfs[l_ac].sfs27) AND
              ...
              g_sfs[l_ac].sfs012 IS NOT NULL AND      #FUN-A60028  ADD
              NOT cl_null(g_sfs[l_ac].sfs013) THEN    #FUN-A60028 ADD
...
#FUN-A60028 --end--
        ...
        AFTER FIELD sfs04
#liming 160318 (s)----------------------------------------------------------------
        IF g_sfs[l_ac].sfs03 IS NULL THEN  NEXT FIELD sfs03 END IF
        IF g_sfp.sfp06='2' THEN

           SELECT count(*) INTO l_num1 FROM sfl_file,sfk_file WHERE sfk01=sfl01  AND sfl02=g_sfs[l_ac].sfs03 AND sfl03=g_sfs[l_ac].sfs04 AND sfk02<=g_sfp.sfp02
          IF l_num1 IS NULL THEN LET l_num1=1  END IF
          IF l_num1=0 THEN
               CALL cl_err('','csf1020',0)#报错并提示：该工单没有报废料
              NEXT FIELD sfs03
          END IF
          ELSE
#No:160318  liming-edd---------------------------------------------------------
   #xiaoyx
           #当不检查发料套数时,此时也要CHECK开立未过帐的数量,xiaoyx
            SELECT sfa05-sfa06  INTO ls_sfa05
                    FROM sfa_file
...
#FUN-AA0059 ---------------------start----------------------------
              IF NOT s_chk_item_no(g_sfs[l_ac].sfs04,"") THEN
                 ...
                 NEXT FIELD sfs04
              END IF
              #add by pane 150602 begin输入料号的时候检查该料件已经扣帐发料量+已审核量+未审核量#
             # SELECT SUM(sfe16) INTO l_a1 FROM sfe_file
             #  WHERE sfe01 = g_sfs[l_ac].sfs03
             #    AND sfe07 = g_sfs[l_ac].sfs04

             # SELECT SUM(sfs05) INTO l_a2 FROM sfs_file,sfp_file
             #  WHERE sfs01 = sfp01 AND sfs01 <> g_sfp.sfp01
             #    AND sfs03 = g_sfs[l_ac].sfs03
             #    AND sfs04 = g_sfs[l_ac].sfs04
             #    AND sfpconf <> 'X'

             # SELECT sfa05 INTO l_sfa05_1 FROM sfa_file
             #  WHERE sfa01 = g_sfs[l_ac].sfs03
             #    AND sfa03 = g_sfs[l_ac].sfs04
             ...
             #    LET l_a2 = 0
             # END IF
             # IF cl_null(l_sfa05_1) THEN
             #    LET l_sfa05_1 = 0
             # END IF
             # IF l_a1 + l_a2 >= l_sfa05_1 THEN
             #    CALL cl_err(g_sfs[l_ac].sfs03/g_sfs[l_ac].sfs04,'csf-017',1)
             #    NEXT FIELD sfs04
             # END IF
              #add by pane 150602 end#
#FUN-AA0059 ---------------------end-------------------------------
 IF g_user <>'tiptop' THEN
        #NO.111031 add by xiayan-----------------------begin
              IF g_sfp.sfp06 = '2' THEN    #超領單
                SELECT COUNT(*) INTO g_cnt FROM sfa_file
                 WHERE sfa01 = g_sfs[l_ac].sfs03   #工單
                   AND sfa03 = g_sfs[l_ac].sfs04   #料號
                   AND sfa05 > sfa06               #應發 > 已發
                IF g_cnt > 0 THEN
                   CALL cl_err(g_sfs[l_ac].sfs04,'asf-707',0)
                   NEXT FIELD sfs04
                END IF
             END IF
        #NO.111031 add by xiayan-------------------------end
  END IF #xiaoyx NO.130306
              # 新增時 Default倉庫,儲位
...
#作業編號
           ...
           IF g_sfp.sfp06='4' THEN
              SELECT sfa11 FROM sfa_file
               WHERE sfa01=g_sfs[l_ac].sfs03
                 AND sfa03=g_sfs[l_ac].sfs04   #No.3351
                 AND sfa12=g_sfs[l_ac].sfs06
                 AND sfa08=g_sfs[l_ac].sfs10
                 AND sfa27=g_sfs[l_ac].sfs27  #FUN-9B0149
                 AND sfa012=g_sfs[l_ac].sfs012   #FUN-A60028
           ...
           IF (g_sfp.sfp06 = '1') OR (g_sfp.sfp06 = '2') OR (g_sfp.sfp06 = '4' ) THEN #MOD-6C0050 #帶應發已發欠料的資料,超領和成套發一樣 #MOD-8C0239 modify
              IF cl_null(g_sfs[l_ac].sfs26) THEN
                  SELECT (sfa05-sfa065),sfa06  # 扣除代買部分        #FUN-940039 add   #FUN-B50059
                 ...
                 #計算欠料量g_short_qty(原g_sfa07)
                  CALL s_shortqty(g_sfs[l_ac].sfs03,g_sfs[l_ac].sfs04,g_sfs[l_ac].sfs10,
                                  ...
                                  g_sfs012,g_sfs013)   #FUN-A50066 add
                       RETURNING g_short_qty
                  IF cl_null(g_short_qty) THEN LET g_short_qty = 0 END IF
              ELSE
                  SELECT SUM(sfa05-sfa065),SUM(sfa06)  # 扣除代買部分                #FUN-940039 add #FUN-B50059
                    INTO g_sfs[l_ac].sfa05,g_sfs[l_ac].sfa06                         #FUN-940039 add
                    FROM sfa_file
                   WHERE sfa01=g_sfs[l_ac].sfs03
                     AND sfa03=g_sfs[l_ac].sfs04  #MOD-630129
                     AND sfa12=g_sfs[l_ac].sfs06
                     AND sfa08=g_sfs[l_ac].sfs10
                     AND sfa27=g_sfs[l_ac].sfs27 #FUN-940039 add
                     AND sfa012=g_sfs[l_ac].sfs012   #FUN-A60028
           ...
           IF g_sfp.sfp06 = '3' THEN
              LET l_n = 0
              SELECT (sfa05-sfa065),sfa06        # 扣除代買部分  #FUN-B50059
                INTO s_sfa05,s_sfa06
                FROM sfa_file
               WHERE sfa01=g_sfs[l_ac].sfs03
                 AND sfa03=g_sfs[l_ac].sfs04
                 #AND sfa27=g_sfs[l_ac].sfs04     #MOD-B10201 add   #NO.111031 mark by xiayan
                 AND sfa12=g_sfs[l_ac].sfs06
                 AND sfa08=g_sfs[l_ac].sfs10
                 AND sfa27=g_sfs[l_ac].sfs27     #FUN-940039 add
                 AND sfa012=g_sfs[l_ac].sfs012   #FUN-A60028
             ...
             #計算欠料量g_short_qty(原g_sfa07)
              ...
              IF g_sfs[l_ac].sfs26 MATCHES '[SUZ]' THEN                  #FUN-A40058 add 'Z'
                 SELECT SUM(sfa05-sfa065),SUM(sfa06)  # 扣除代買部分 #FUN-940039 add  #FUN-B50059
                   INTO t_sfa05,t_sfa06           #FUN-940039 add
                   FROM sfa_file
                  WHERE sfa01=g_sfs[l_ac].sfs03
...
#                 IF cl_null(t_short_qty) THEN LET t_short_qty = 0 END IF  #TQC-A50101
 ...
 #超領單不控制  --so '2' 要拿掉
           IF g_sfp.sfp06 MATCHES '[134]' AND     #NO:7075,補2,4
              g_sfs[l_ac].sfa05<=g_sfs[l_ac].sfa06 THEN
        ...
        AFTER FIELD sfs07
           IF NOT cl_null(g_sfs[l_ac].sfs07) THEN
              ...
              #End Add No.FUN-AB0018
            #xiaoyx
                SELECT imd02 INTO g_sfs[l_ac].imd02 FROM imd_file
                WHERE imd01=g_sfs[l_ac].sfs07
                  AND imdacti = 'Y'
                DISPLAY g_sfs[l_ac].imd02 TO FORMONLY.imd02
             #xiaoend
           END IF
        AFTER FIELD sfs08
        ...
        AFTER FIELD sfs05
 ----------     #No:160318  liming----str-------------------
    IF p_cmd='u' THEN
     SELECT SUM(sfl07) INTO l_num2 FROM sfl_file,sfk_file WHERE sfl01=sfk01 AND sfl02=g_sfs[l_ac].sfs03 AND sfl03=g_sfs[l_ac].sfs04 AND sfk02<=g_sfp.sfp02

     SELECT SUM(sfs05) INTO l_num03 FROM (SELECT DISTINCT sfs01,sfs05 FROM sfs_file,sfp_file,sfk_file,sfl_file WHERE sfs01=sfp01 AND sfk01=sfl01 AND  sfs03=sfl02  AND sfs04=sfl03 AND sfs03=g_sfs[l_ac].sfs03 AND sfs04=g_sfs[l_ac].sfs04 AND sfpconf !='X'
     AND sfk02<=g_sfp.sfp02 AND sfp02<=g_sfp.sfp02)


     SELECT  sfs05      INTO l_num4 FROM sfs_file,sfp_file WHERE sfs01=sfp01 AND sfp01=g_sfp.sfp01 AND sfs02=g_sfs[l_ac].sfs02 AND sfs03=g_sfs[l_ac].sfs03 AND sfs04=g_sfs[l_ac].sfs04

     SELECT SUM(sfe16) INTO l_sfe16 FROM sfp_file,sfe_file WHERE sfp06 ='2' AND sfp01=sfe02 AND sfe01=g_sfs[l_ac].sfs03 AND sfe07=g_sfs[l_ac].sfs04 AND sfpconf ='Y' AND sfp04 = 'Y'

     #SELECT sfa062 INTO l_num4 FROM sfa_file,sfs_file WHERE sfa01=sfs03 sfa01=g_sfs[l_ac].sfs03 AND sfa03=g_sfs[l_ac].sfs04
      IF l_num2 IS NULL   THEN  LET l_num2=0   END IF
      IF l_num03 IS NULL   THEN  LET l_num03=0  END IF
      IF l_num4 IS NULL   THEN  LET l_num4=0   END IF
      IF l_sfe16 IS NULL   THEN  LET l_sfe16=0  END IF

   #  IF l_num2-l_sfe16-l_num03+l_num4<g_sfs[l_ac].sfs05 THEN        #mark by xujw160921
   #  CALL cl_err('','csf1023',0)#报错并提示：所填领数量大于可领数量
   #     NEXT FIELD sfs05
   #     END IF
   #   ELSE
     ...
     SELECT SUM(sfs05) INTO l_num03 FROM sfs_file,sfp_file WHERE sfs01=sfp01   AND sfs03=g_sfs[l_ac].sfs03 AND sfs04=g_sfs[l_ac].sfs04 AND sfpconf !='X'
   ...
   # IF g_sfs[l_ac].sfs05+l_sfe16+l_num03>l_num2 THEN

   #   CALL cl_err('','csf1023',0)#报错并提示：所填领数量大于可领数量
   #     NEXT FIELD sfs05
   # END IF
 ---------    #No:160318  liming-mode--edd-----------------------

  #xiaoyx  --begin
 #xiaoyx NO.130116
  SELECT SUM(imn10) INTO ls_imn10
 FROM imn_file,imm_file
 WHERE imm01=imn01
  AND imm04='Y' AND imm03='N'
  AND imn03=g_sfs[l_ac].sfs04
  IF ls_imn10 IS NULL THEN LET ls_imn10=0 END IF

 SELECT SUM(sfs05) INTO g_sfs[l_ac].sfsud07
 FROM sfs_file,sfp_file
 WHERE sfp04='N'
 AND sfpconf='Y'
 AND sfs01=sfp01
 AND sfp01 <> g_sfp.sfp01
 AND sfs04=g_sfs[l_ac].sfs04 AND sfs07=g_sfs[l_ac].sfs07 AND sfs08=g_sfs[l_ac].sfs08 AND sfs09=g_sfs[l_ac].sfs09
 LET g_sfs[l_ac].sfsud07=g_sfs[l_ac].sfsud07+ls_imn10
 SELECT img10 INTO ls_img10
 FROM sfs_file,sfp_file,img_file
 WHERE sfp04='N'
 AND sfpconf='Y'
 AND sfs01=sfp01
 AND sfp01 = g_sfp.sfp01
 ...
 AND img03=sfs08
 AND img04=sfs09
 LET g_sfs[l_ac].sfsud08= ls_img10 - g_sfs[l_ac].sfsud07
 IF g_sfs[l_ac].sfsud08>0 THEN LET g_sfs[l_ac].sfsud08=0 END IF
 DISPLAY BY NAME g_sfs[l_ac].sfsud07
 DISPLAY BY NAME g_sfs[l_ac].sfsud08
  #xiaoyx --end
     #xiaoyx
         IF g_sfs[l_ac].sfs05> ls_sfs05a THEN
          CALL cl_err('','csf-001',1)
          NEXT FIELD sfs05
         END IF
          IF NOT cl_null(g_sfp.sfp06) THEN
...
#TQC-A40085
            ...
            IF g_sma.sma129='N' THEN
             ...
             IF g_sfp.sfp06 = '2' THEN
               ...
               END IF
             END IF
 #xiaoyx 2012/9/26 yld反应库存不够也可过帐。
 #查为l_factor 空，跳过了此段CHECK.
#xiaoyx2013/6/22 for vmi  LET l_factor=1 #xiaoyx

            IF g_sfp.sfp06 MATCHES '[1234]' THEN
                #BUGNO:3264 sfs05發料量 * l_factor > img10庫存量 01/08/10mandy
                IF (g_sfs[l_ac].sfs05 * l_factor) > g_sfs[l_ac].img10 THEN
             ...
             IF g_sfp.sfp06 MATCHES '[6789]' THEN   #NO:7075
                ...
                IF (g_sfp.sfp06 = '6') AND (l_sfa11 <> 'S') THEN    #成套退料
                  ...
                  IF g_sfs[l_ac].sfs05 >l_sfa06 THEN
                     ...
                     NEXT FIELD sfs05
                  END IF
                  SELECT SUM(sfa06*sfa28),SUM(sfa161) INTO l_sfa06,l_sfa161 FROM sfa_file  #FUN-B50059
                   WHERE sfa01 = g_sfs[l_ac].sfs03
                 ...
                 #MOD-B20062---add---end---
                  LET l_qty = l_sfa06 - (l_sfb09 * l_sfa161) - l_sfs05         #MOD-B20062 add l_sfs05
                  CALL i501_totsfs05(g_sfp.sfp01,g_sfp.sfp06,g_sfs[l_ac].sfs02,
                                     g_sfs[l_ac].sfs03,g_sfs[l_ac].sfs04,
                                     g_sfs[l_ac].sfs06,g_sfs[l_ac].sfs10,
                                     g_sfs[l_ac].sfs27)  #FUN-9B0149
                       RETURNING l_totsfs05    #No.MOD-790175 modify
                  IF g_sfs[l_ac].sfs05*l_sfa28 > l_qty - l_totsfs05*l_sfa28 THEN
                ...
                IF (g_sfp.sfp06 = '9') AND (l_sfa11 <> 'S') THEN    #領料退料      #FUN-9C0040
                   SELECT (sfa062+sfa06) INTO l_sfa062     #No.MOD-940164 add  #FUN-B50059
                     FROM sfa_file
                    WHERE sfa01 = g_sfs[l_ac].sfs03
                      AND sfa03 = g_sfs[l_ac].sfs04
                      AND sfa08 = g_sfs[l_ac].sfs10
                ...
                IF (g_sfp.sfp06 = '8') AND (l_sfa11 <> 'S') THEN    #一般退料      #FUN-9C0040
                  ...
                  #MOD-B20062---add---start---
                   ...
                   IF g_sfs[l_ac].sfs05 > ((l_sfa06 + l_sfa062)-b_sfs05) THEN
            ...
            IF (g_ima918 = "Y" OR g_ima921 = "Y") AND
               ...
               IF g_sfp.sfp06 MATCHES '[6789B]' THEN
                  CALL s_lotout(g_prog,g_sfp.sfp01,g_sfs[l_ac].sfs02,0,
                                ...
                                g_sfs[l_ac].sfs08,g_sfs[l_ac].sfs09,
                                g_sfs[l_ac].sfs06,g_img09,l_fac,
                                g_sfs[l_ac].sfs05,'','SEL')  #CHI-9A0022 add ''
                        RETURNING l_r,g_qty
               ELSE
#No.CHI-9A0022 --Begin
                  LET l_bno = ''
...
#No.CHI-9A0022 --End
                  CALL s_lotout(g_prog,g_sfp.sfp01,g_sfs[l_ac].sfs02,0,
                                ...
                                g_sfs[l_ac].sfs08,g_sfs[l_ac].sfs09,
                                g_sfs[l_ac].sfs06,g_img09,l_fac,
                                g_sfs[l_ac].sfs05,l_bno,'MOD')#CHI-9A0022 add l_bno
                        RETURNING l_r,g_qty
               END IF
           ...
           IF NOT cl_null(g_sfs[l_ac].sfs35) THEN
             ...
             #str MOD-A60062 add
              IF g_sfp.sfp06 MATCHES '[13]' THEN
                 ...
                 IF g_sfs[l_ac].sfs35>(g_sfs[l_ac].sfa05-g_sfs[l_ac].sfa06-l_sfs35x) THEN
              ...
              IF NOT cl_null(g_sfs[l_ac].sfs33) THEN
                 IF g_sfp.sfp06 MATCHES '[1234]' THEN
                    IF g_sfs[l_ac].sfs35 > g_imgg10_2 THEN
                       #IF g_sma.sma894[3,3]='N' OR g_sma.sma894[3,3] IS NULL THEN                               #FUN-C80107 mark
                       INITIALIZE g_sma894 TO NULL                                                               #FUN-C80107
                       CALL s_inv_shrt_by_warehouse(g_sma.sma894[3,3],g_sfs[l_ac].sfs07) RETURNING g_sma894      #FUN-C80107
                       IF g_sma894 = 'N' THEN                                                                    #FUN-C80107
...
#No.CHI-9A0022 --End
           ...
           IF NOT cl_null(g_sfs[l_ac].sfs32) THEN
             ...
             #str MOD-A60062 add
              ...
              IF NOT cl_null(g_sfs[l_ac].sfs30) THEN
                 IF g_sfp.sfp06 MATCHES '[1234]' THEN
                    IF g_ima906 = '2' THEN
                       IF g_sfs[l_ac].sfs32 > g_imgg10_1 THEN
                          #IF g_sma.sma894[3,3]='N' OR g_sma.sma894[3,3] IS NULL THEN                               #FUN-C80107 mark
                          INITIALIZE g_sma894 TO NULL                                                               #FUN-C80107
                          CALL s_inv_shrt_by_warehouse(g_sma.sma894[3,3],g_sfs[l_ac].sfs07) RETURNING g_sma894      #FUN-C80107
                          IF g_sma894 = 'N' THEN                                                                    #FUN-C80107
			     CALL cl_err(g_sfs[l_ac].sfs32,'mfg1303',0)
                    ...
                    ELSE
                      ...
                      #IF g_sfs[l_ac].sfs31 * g_sfs[l_ac].sfs32 > g_imgg10_1 THEN  #MOD-A50001 mark
                       IF l_factor * g_sfs[l_ac].sfs32 > g_imgg10_1 THEN           #MOD-A50001
                          #IF g_sma.sma894[3,3]='N' OR g_sma.sma894[3,3] IS NULL THEN                               #FUN-C80107 mark
                          INITIALIZE g_sma894 TO NULL                                                               #FUN-C80107
                          CALL s_inv_shrt_by_warehouse(g_sma.sma894[3,3],g_sfs[l_ac].sfs07) RETURNING g_sma894      #FUN-C80107
                          IF g_sma894 = 'N' THEN                                                                    #FUN-C80107
        ...
        BEFORE DELETE                            #是否取消單身
            IF g_sfs_t.sfs02 > 0 AND g_sfs_t.sfs02 IS NOT NULL THEN
                ...
                IF g_ima918 = "Y" OR g_ima921 = "Y" THEN
                   IF NOT s_lotout_del(g_prog,g_sfp.sfp01,g_sfs[l_ac].sfs02,0,g_sfs[l_ac].sfs04,'DEL') THEN   #No.FUN-860045
        ...
        AFTER ROW
            ...
            SELECT (sfa05-sfa065),sfa06   #FUN-B50059
               ...
               FROM sfs_file LEFT OUTER JOIN sfa_file ON sfs03=sfa01 AND sfs04=sfa03 AND sfs06=sfa12 AND sfs10=sfa08 AND sfs27=sfa27 #09/10/21 xiaofeizhu Add
                                                     AND sfs012=sfa012 AND sfs013=sfa013  #FUN-A60095
            ...
            DISPLAY BY NAME g_sfs[l_ac].sfa05,g_sfs[l_ac].sfa06,
                            g_sfs[l_ac].short_qty
            CLOSE i501_bcl
            COMMIT WORK
                display "commit work after row"

            CALL g_sfs.deleteElement(g_rec_b+1)
...
#FUN-A40058 --end--
              ...
              #FUN-A60028 --end--
                          ...
                          WHEN g_argv2 MATCHES '[2478]'
                               ...
                               ELSE
                                  LET g_qryparam.where = " sfa26 IN ('0','1','2','3','4','5','6','7','8') "   #FUN-A20037 add '7,8'
                               END IF
...
#FUN-A60028 --end--
                WHEN INFIELD(sfs07) OR INFIELD(sfs08) OR INFIELD(sfs09)
                      CALL q_img4(FALSE,TRUE,g_sfs[l_ac].sfs04,g_sfs[l_ac].sfs07,g_sfs[l_ac].sfs08,g_sfs[l_ac].sfs09,'A') #MOD-470501 modify  ##NO.FUN-660085
               ...
               WHEN INFIELD(sfs36)
                    ...
                    NEXT FIELD sfs36
               WHEN INFIELD(sfs27)
                  LET li_where = " AND sfb04 IN ('2','3','4','5','6','7') AND sfa01 = '",g_sfs[l_ac].sfs03,"' AND sfa03 = '",g_sfs[l_ac].sfs04,"' "
                  IF NOT cl_null(g_sfs[l_ac].sfs26) THEN
                 #   LET li_where = " AND sfa26 = '",g_sfs[l_ac].sfs26,"' "                    #FUN-A40058 mark
                     LET li_where = li_where CLIPPED," AND sfa26 = '",g_sfs[l_ac].sfs26,"' "   #FUN-A40058
                  ELSE
...
#No.CHI-9A0022 --End
   ...
   #FUN-AB0001 add str-------------------
    IF g_cnt>0 AND g_smy.smydmy4='Y' AND g_smy.smyapr <> 'Y'THEN  #單據需自動確認且不需簽核
       LET g_action_choice = "insert"
   #FUN-AB0001 add end-------------------
       CALL i501sub_y_chk(g_sfp.sfp01)
       IF g_success = "Y" THEN
...
FUNCTION i501_set_entry_b(p_cmd)
   ...
   IF p_cmd = 'a' OR p_cmd = 'u' THEN
      CALL cl_set_comp_entry("sfs26",TRUE)
   END IF
...
FUNCTION i501_d_fill(p_wc2)              #BODY FILL UP
 ...
 DEFINE l_ima01 LIKE ima_file.ima01
    ...
    IF g_sfp.sfp06 MATCHES '[ABC]' THEN
       LET g_sql =
           "SELECT sfq02,sfq04,'','','',sfq05,sfq07,sfq08,sfq03", #No.FUN-870097 add sfq07 #FUN-940008 add sfq08
           ",sfqud01,sfqud02,sfqud03,sfqud04,sfqud05,",
           "sfqud06,sfqud07,sfqud08,sfqud09,sfqud10,",
           "sfqud11,sfqud12,sfqud13,sfqud14,sfqud15",
    ...
    ELSE
       LET g_sql =
           "SELECT sfq02,sfq04,sfb05,'','',sfq05,sfq07,sfq08,sfq03", #No.FUN-870097 add sfq07 #FUN-5C0114 add sfq05 #FUN-940008 add sfq08
           ",sfqud01,sfqud02,sfqud03,sfqud04,sfqud05,",
           "sfqud06,sfqud07,sfqud08,sfqud09,sfqud10,",
           "sfqud11,sfqud12,sfqud13,sfqud14,sfqud15",
...
FUNCTION i501_b_fill(p_wc2)              #BODY FILL UP
    ...
    DEFINE ls_img10b       LIKE img_file.img10   #xiaoyx
 #xiaoyx,在sfs07后加入imd02
     IF g_sfp.sfp04='N' THEN  #FUN-660106
       LET g_sql =
        "SELECT sfs02,sfs26,sfs03,sfs04,sfs27,ima02,ima021,sfs012,'',sfs013,sfs06,sfs10,(sfa05-sfa065),sfa06,'',",    #FUN-940039 add sfs27,''(欠料量) #FUN-B50059
                                                                           #FUN-A60028 add sfs01,'',sfs013
       "       sfs07,'',sfs08,sfs09,sfs05,sfs33,sfs34,sfs35,sfs30,sfs31,sfs32,",
       "       sfs21,img10,0,sfs930,'',sfs36,''",     #No.MOD-790113 mark sfa07   #FUN-670103 #FUN-950088 add sfs36,''
        "       ,sfsud01,sfsud02,sfsud03,sfsud04,sfsud05,",
        "       sfsud06,sfsud07,sfsud08,sfsud09,sfsud10,",
        "       sfsud11,sfsud12,sfsud13,sfsud14,sfsud15",
        ",sfa07",  #No.MOD-790113 add sfa07
        " FROM sfs_file LEFT OUTER JOIN sfa_file ON sfs03=sfa01 AND sfs04=sfa03 AND sfs06=sfa12 AND sfs10=sfa08 AND sfs27=sfa27",   #09/10/21 xiaofeizhu Add
        "  AND sfs012 = sfa012 AND sfs013 = sfa013 ",  #FUN-A60028
        " LEFT OUTER JOIN img_file ON sfs04=img01 AND sfs07=img02 AND sfs08=img03 AND sfs09=img04",                                 #09/10/21 xiaofeizhu Add
        " LEFT OUTER JOIN ima_file ON sfs04=ima01 ",                                                                                #09/10/21 xiaofeizhu Add
        " WHERE sfs01 ='",g_sfp.sfp01,"'",
        "   AND ",p_wc2 CLIPPED,
    ...
    ELSE
       LET g_sql =
        "SELECT sfe28,sfe26,sfe01,sfe07,sfe27,ima02,ima021,sfe012,'',sfe013,sfe17,sfe14,(sfa05-sfa065),sfa06,'',",      #FUN-940039 add '',''(欠料量)  #TQC-9B0049 add sfe27  #FUN-B50059
                                                                #FUN-A60028 add sfe012,'',sfe013
 #xiaoyx       "       sfe08,sfe09,sfe10,sfe16,sfe33,sfe34,sfe35,sfe30,sfe31,sfe32,",
   "       sfe08,'',sfe09,sfe10,sfe16,sfe33,sfe34,sfe35,sfe30,sfe31,sfe32,",
        "       sfe11,img10,'',sfe930,'',sfe36,''",    #No.MOD-790113 mark sfa07  #FUN-670103 #FUN-950088 add sfe36,''
        "       ,'','','','','',",
        "       '','','','','',",
        "       '','','','','' ",
        ",''",                                   #FUN-940039 add
        " FROM sfe_file LEFT OUTER JOIN sfa_file ON sfe01=sfa01 AND sfe07=sfa03 AND sfe17=sfa12 AND sfe14=sfa08 AND sfe27=sfa27",   #09/10/21 xiaofeizhu Add
        "  AND sfe012 = sfa012 AND sfe013 = sfa013 ",  #FUN-A60028
    ...
    FOREACH sfs_curs INTO g_sfs[g_cnt].*, g_short_qty #單身 ARRAY 填充#FUN-940039 add
        IF STATUS THEN CALL cl_err('foreach:',STATUS,1) EXIT FOREACH END IF
  #xiaoyx  --begin
 SELECT SUM(sfs05) INTO g_sfs[g_cnt].sfsud07
 FROM sfs_file,sfp_file
 WHERE sfp04='N'
 AND sfpconf='Y'
 AND sfs01=sfp01
 AND sfs04=g_sfs[g_cnt].sfs04 AND sfs07=g_sfs[g_cnt].sfs07 AND sfs08=g_sfs[g_cnt].sfs08 AND sfs09=g_sfs[g_cnt].sfs09
 SELECT img10 INTO ls_img10b
 FROM sfs_file,sfp_file,img_file
 WHERE sfp04='N'
 AND sfpconf='Y'
 AND sfs01=sfp01
 ...
 AND img04=sfs09
 AND sfs04=g_sfs[g_cnt].sfs04 AND sfs07=g_sfs[g_cnt].sfs07 AND sfs08=g_sfs[g_cnt].sfs08 AND sfs09=g_sfs[g_cnt].sfs09
 LET g_sfs[g_cnt].sfsud08= ls_img10b - g_sfs[g_cnt].sfsud07
 IF g_sfs[g_cnt].sfsud08>0 THEN LET g_sfs[g_cnt].sfsud08=0 END IF
#xiaoyx --end

#bygaozk ---150613
SELECT SUM(img10) INTO g_sfs[g_cnt].sfsud09 FROM img_file
WHERE img01=g_sfs[g_cnt].sfs04 and img02='1003' and img23='Y'

IF g_sfs[g_cnt].sfsud09<>''  THEN LET g_sfs[g_cnt].sfsud09=0 END IF
#bygaozk ---150613
       ...
       #xiaoyx
          SELECT imd02 INTO g_sfs[g_cnt].imd02 FROM imd_file
               WHERE imd01=g_sfs[g_cnt].sfs07
                  AND imdacti = 'Y'
       #xiaoend
        IF g_sfp.sfp04='N' THEN
...
FUNCTION i501_out()
    ...
    IF g_sfp.sfp01 IS NULL THEN RETURN END IF
  #NO.111017 add---------------begin
    IF g_prog = 'asfi510' OR g_prog = 'asfi511' OR g_prog = 'asfi512'
       OR g_prog = 'asfi513' OR g_prog = 'asfi514' THEN
       CALL cl_set_act_visible("tui_csfr501",TRUE)
    ELSE
       CALL cl_set_act_visible("tui_csfr501",FALSE)
    END IF
    IF g_prog = 'asfi520' OR g_prog = 'asfi526' OR g_prog = 'asfi527'
       OR g_prog = 'asfi528' OR g_prog = 'asfi529' THEN
       CALL cl_set_act_visible("tui_csfr502",TRUE)
    ELSE
       CALL cl_set_act_visible("tui_csfr502",FALSE)
     END IF
  #NO.111017 add-----------------end
    ...
    MENU ""
     ...
     #NO.111017 add-------------------begin
       ON ACTION tui_csfr501
      IF g_plant='YLD008' THEN
        LET l_wc2 = 'sfp01= "',g_sfp.sfp01,'" AND sfp02 BETWEEN "',g_sfp.sfp02,'" AND "',g_sfp.sfp02,'"'
          LET g_msg = "csfr503",
                      " '",g_today,"' '",g_user,"' '",g_lang,"' ",
                      " 'Y' ' ' '1' ",
                      " '",l_wc2,"' "
      ...
      END IF
       ...
       ON ACTION tui_csfr502
          LET g_msg = "csfr502",
                      " '",g_today,"' '",g_user,"' '",g_lang,"' ",
                      " 'Y' ' ' '1' ",
                      " '",g_sfp.sfp01 CLIPPED,"' "
          CALL cl_cmdrun_wait(g_msg)
     ON ACTION tui_csfr504
         LET g_msg = "csfr504"," '",g_sfp.sfp01 CLIPPED,"' "
          CALL cl_cmdrun_wait(g_msg)
     #NO.111017 add---------------------end
       ON ACTION exit
          EXIT MENU
...
FUNCTION i501_t()
   ...
   DEFINE l_sfs     DYNAMIC ARRAY OF RECORD    #程式變數(Prinram Variables)
                    ...
                    ima021    LIKE ima_file.ima021,  #xiaoyx
   ...
   OPEN WINDOW i501_t_w AT 4,3 WITH FORM "csf/42f/asfi511c"
         ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN
   ...
   DECLARE i501_t_c CURSOR FOR
          SELECT sfs04,sfs07,sfs08,sfs09,img09,img10,ima02,ima021,sfs06,SUM(sfs05)   #No:MOD-980003 modify
            FROM sfs_file,ima_file, OUTER img_file
           WHERE sfs01=g_sfp.sfp01
             AND sfs04=img01 AND sfs07=img02 AND sfs08=img03 AND sfs09=img04
             AND sfs04=ima01
           GROUP BY sfs04,sfs07,sfs08,sfs09,img09,img10,ima02,ima021,sfs06    ##No:MOD-980003 modify
   LET i=1
          ...
          CASE WHEN INFIELD(tr_no) #查詢單据
               ...
               WHEN INFIELD(to_loc)
                   ...
                   #CALL cl_create_qry() RETURNING tm.to_loc
                    CALL q_ime_1(FALSE,TRUE,tm.to_loc,tm.to_w,"",g_plant,"","","")
                         RETURNING tm.to_loc
                   #End Mod No.FUN-AB0018
                    NEXT FIELD to_loc
          END CASE
   ...
   ELSE
      ...
      FOREACH sfs_upd INTO l_sfs01,l_sfs02                                    #09/10/21 xiaofeizhu Add
         IF STATUS <> 0 THEN EXIT FOREACH END IF
            UPDATE sfs_file SET sfs07=tm.to_w, sfs08=tm.to_loc, sfs09=' '
...
DEFINE  l_cnt        LIKE type_file.num5    #No.FUN-680121 SMALLINT
  ...
  FORMAT
   PAGE HEADER
      LET g_x[1]="Unuual Store Transfer Not Enough Report"
         ...
         IF tm.tr_no IS NOT NULL THEN
           ...
           #IF (l_qty-from_qty) > 0 THEN   #No:MOD-A20007 mark
              PRINT COLUMN g_c[31],sr.ima23[1,6],
                    ...
                    COLUMN g_c[35],(l_qty-from_qty)  USING '---,---,--&'
   ...
   IF NOT cl_null(g_sfp.sfp06) THEN
      ...
      IF g_sfp.sfp06 MATCHES '[6789]' THEN   #NO:7075
         ...
         IF g_sfp.sfp06 = '8' THEN    #一般退料
            ...
            SELECT sfa06,sfa161,sfa26,sfa27,sfa28,sfa05,sfa100  #MOD-8B0230 add sfa100  #FUN-B50059
            INTO l_sfa06,l_sfa161,l_sfa26,l_sfa27,l_sfa28,l_sfa05,l_sfa100 FROM sfa_file  #MOD-8B0230 add l_sfa100
            WHERE sfa01 = g_sfs[l_ac].sfs03
              AND sfa03 = g_sfs[l_ac].sfs04
              AND sfa08 = g_sfs[l_ac].sfs10
              AND sfa27 = g_sfs[l_ac].sfs27    #FUN-A60095
...
FUNCTION i501_y_chk()
  DEFINE ls_sfa05,ls_sfs05 LIKE sfa_file.sfa05  #xiaoyx
 DEFINE ls_sfs03 LIKE sfs_file.sfs03,
        ls_sfs04 LIKE sfs_file.sfs04,
        ls_sfs10 LIKE sfs_file.sfs10,
        l_gcnt01 LIKE type_file.num5,
        l_gcnt03 LIKE type_file.num5,
        l_gcnt02 LIKE type_file.num5,
        l_gsfs07 LIKE sfs_file.sfs07,
        l_gsfs08 LIKE sfs_file.sfs08

DEFINE l_sfe16   LIKE sfe_file.sfe16 #160318 liming -add
DEFINE l_num3 LIKE sfl_file.sfl07   #记录已审核并过账的报废数  160318 liming -add
DEFINE l_num4 LIKE sfs_file.sfs05    #160318 liming -add
#xiaoyx IF g_user <>'tiptop' THEN
#IF g_plant='YLD004' AND g_user <>'110713003' THEN

####再次填坑，检查单身仓库必须为“成本仓”
   LET l_gcnt02=0
IF g_sfp.sfp06 ='1' THEN
   SELECT COUNT(*) INTO l_gcnt02 from imd_file
       where imd01 in (SELECT distinct sfs07 from sfs_file where sfs01=g_sfp.sfp01) and imd09='N'
     IF l_gcnt02>0 THEN
     	   CALL cl_err('','csf1027','1')   #报错并提示：所录入仓库存在非成本仓不允许审核。
         LET g_success='N'
         RETURN
     END IF
END IF

# bygaozk 160511 控制 VMI仓库的 库位不为 空格

IF g_sfp.sfp06 ='1' THEN

	 DECLARE i511_sfs07 CURSOR FOR
     SELECT distinct substr( sfs07,1,3),sfs08 from sfs_file where sfs01=g_sfp.sfp01

     FOREACH i511_sfs07 INTO l_gsfs07,l_gsfs08
         IF l_gsfs07='VMI' AND l_gsfs08=' ' THEN
         	    CALL cl_err(l_gsfs07,'csf1028',1)
     	        LET g_success='N'
              EXIT FOREACH
          END IF
      END FOREACH
...
# bygaozk 161121 控制 如果单头部门为：DJ ，仓库不能选择：1034、1023、1024、1025、1009
IF g_sfp.sfp06 ='1' AND g_sfp.sfp07 ='DJ' THEN

LET l_gcnt03=0
     SELECT count(*) INTO l_gcnt03 from sfs_file where sfs01=g_sfp.sfp01
               AND sfs07 IN ('1034','1023','1024','1025','1009')

         IF l_gcnt03>0 THEN
         	    CALL cl_err(l_gcnt03,'csf1029',1)
     	        LET g_success='N'
     	        RETURN
          END IF
...
LET l_gcnt01=0
IF g_sfp.sfp06 ='2' THEN
        SELECT COUNT(*) INTO l_gcnt01 from sfb_file
          where sfb01 in ( SELECT sfs03 FROM sfs_file
                      WHERE sfs01=g_sfp.sfp01 ) and sfb09-sfb081=0
#160318 liming -begin ---------
       LET l_sfe16 =0

    SELECT SUM(sfe16) INTO l_sfe16 FROM sfp_file,sfe_file
      WHERE sfp06 ='2'
      AND sfp01=sfe02
      AND sfe01=g_sfs[g_cnt].sfs03
      AND sfe07=g_sfs[g_cnt].sfs04
      AND sfpconf ='Y'
      AND sfp04 = 'Y'
     IF STATUS THEN LET l_sfe16=0 END IF

    IF cl_null(l_sfe16) THEN LET l_sfe16 = 0 END IF   #超领發料數
     SELECT sum(sfl07) INTO l_num3
    FROM sfl_file,sfk_file
    WHERE sfl01 = sfk01
    AND sfl02=g_sfs[g_cnt].sfs03
    AND sfl03=g_sfs[g_cnt].sfs04
   AND sfk02<=g_sfp.sfp02


     SELECT SUM(sfs05) INTO l_num4
     FROM (SELECT DISTINCT sfs01,sfs05 FROM  sfs_file,sfp_file,sfk_file,sfl_file
     WHERE sfs01=sfp01
     AND sfk01=sfl01
     AND  sfs03=sfl02
     AND sfs04=sfl03
     AND sfs03=g_sfs[g_cnt].sfs03
     AND sfs04=g_sfs[g_cnt].sfs04
     AND sfpconf !='X'
     AND sfk02<=g_sfp.sfp02
     AND sfp02<=g_sfp.sfp02 )


    IF l_sfe16 + l_num4 > l_num3 THEN #判断超领总数是否小于报废总数
    CALL cl_err('','csf1023','0')   #报错并提示：所填领数量大于可领数量
    LET g_success='N'
     RETURN
    END IF
...
IF g_sfp.sfp06 <>'2' THEN  #超领放开,xiaoyx 2012/8/11
  DECLARE i501_y_sfb1 CURSOR FOR
SELECT sfs03,sfs04,sfs10 FROM sfs_file
 WHERE sfs01=g_sfp.sfp01
 FOREACH i501_y_sfb1 INTO ls_sfs03,ls_sfs04
  SELECT sfa05-sfa06 INTO ls_sfa05
  FROM sfs_file,sfa_file
  WHERE sfa01=sfs03
  AND sfa03=sfs04
  AND sfs03=ls_sfs03
  AND sfs04=ls_sfs04
  SELECT SUM(sfs05) INTO ls_sfs05
   FROM sfs_file,sfa_file
  WHERE sfa01=sfs03
  AND sfa03=sfs04
  AND sfs03=ls_sfs03
  AND sfs04=ls_sfs04
  AND sfa08=ls_sfs10
  IF ls_sfa05 <ls_sfs05 THEN
     CALL cl_err(ls_sfs04 CLIPPED,'csf-001',0)
     LET g_success='N'
    RETURN
EXIT FOREACH
  END IF
...
FUNCTION i501_out2()
  DEFINE    l_cmd       LIKE type_file.chr1000        #NO.FUN-810099
    IF cl_null(g_wc2) THEN
       ...
       RETURN
    END IF
    LET l_cmd = ' p_query "csfr504" "',g_wc3 CLIPPED,'"'
