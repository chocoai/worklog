...
DEFINE l_n              LIKE type_file.num5     # add by lixwz 180416
...
#MOD-AB0124 --end--
              ...
              WHEN INFIELD(oga04)
                   ...
                   CALL cl_create_qry() RETURNING g_oga.oga04
                    DISPLAY BY NAME g_oga.oga04        #No.MOD-490371
                   NEXT FIELD oga04
              WHEN INFIELD(oga13)
                   CALL cl_init_qry_var()
                   LET g_qryparam.state = "c"
...
#                  CALL cl_create_qry() RETURNING g_ogb[1].ogb04
                   CALL q_sel_ima(TRUE, "q_ima","",g_ogb[1].ogb04,"","","","","",'')
                     RETURNING g_ogb[1].ogb04
#FUN-AA0059---------mod------------end------------------
                    DISPLAY BY NAME g_ogb[1].ogb04     #No.MOD-490371
                   NEXT FIELD ogb04
              WHEN INFIELD(ogb05)
                   CALL cl_init_qry_var()
                   LET g_qryparam.state = "c"
                   ...
                   CALL cl_create_qry() RETURNING g_ogb[1].ogb05
                    DISPLAY BY NAME g_ogb[1].ogb05
                   NEXT FIELD ogb05
              WHEN INFIELD(ogb908)
...
DEFINE l_n      LIKE type_file.num5          # add by lixwz18416
   WHILE TRUE
      CALL t650_bp("G")
      CASE g_action_choice
         ...
         WHEN "detail"
            ...
            END IF
         WHEN "output"
             IF cl_chk_act_auth() AND NOT cl_null(g_oga.oga01) AND g_plant='YLD008' AND g_oga.ogaconf='N' THEN
               CALL cl_err('','cxm-018',1)
            END IF

            IF cl_chk_act_auth() AND NOT cl_null(g_oga.oga01) AND g_plant='YLD008' AND g_oga.ogaconf='Y' THEN
                 LET g_no=g_oga.oga01

                        LET l_wc='oga01="',g_no,'"'
                    #xiaoyx    LET g_msg = "axmr600",
                   LET g_msg = "cxmr004",
                                     " '",g_today CLIPPED,"' ''",
                                     " '",g_lang CLIPPED,"' 'Y' '' '1'",
                                     " '",l_wc CLIPPED,"' "
           ...
           IF cl_chk_act_auth() AND NOT cl_null(g_oga.oga01) AND g_plant<>'YLD008' THEN
                LET g_no=g_oga.oga01
            ...
            IF cl_chk_act_auth() THEN
               ...
               ELSE
                 IF NOT cl_null(g_oga.oga10) THEN
                    ...
                    CONTINUE WHILE
                 END IF
                  # add by lixwz180416 s---
                  #已开票不可取消确认
                  LET l_n = 0
                  SELECT COUNT(1) INTO l_n FROM omf_file WHERE omf11=g_oga.oga01 AND omf08 !='X'
                  IF l_n>0 THEN
                     CALL cl_err('','cxm-135',1)
                     ROLLBACK WORK
                     RETURN
                  END IF
                  # add by lixwz180416 e---
...
FUNCTION t650_i(p_cmd)
  ...
  DEFINE l_oaytype       LIKE oay_file.oaytype  #130108jiangxt
    ...
    INPUT BY NAME g_oga.ogaoriu,g_oga.ogaorig,
      ...
      AFTER INPUT
         #addbywangww161107-str
          IF NOT cl_null(g_oga.oga01) THEN
             IF g_dbs='yld001' THEN
                IF g_oga.oga01 LIKE '2304%' OR g_oga.oga01 LIKE '2307%' OR g_oga.oga01 LIKE '2306%' THEN
                  IF g_oga.oga00<>'2' THEN
                  call cl_err('','axm-612',1)
                  NEXT FIELD oga00
                  END IF
               END IF
             END IF
          END IF
         #addbywangww161107-end
        ...
        #start FUN-950062 add
         #addbywangww161107-str
          IF NOT cl_null(g_oga.oga01) THEN
             IF g_dbs='yld001' THEN
                IF g_oga.oga01='2304' OR g_oga.oga01='2307' OR g_oga.oga01='2306' THEN
                  IF g_oga.oga00<>'2' THEN
                  call cl_err('','axm-612',1)
                  NEXT FIELD oga00
                  END IF
...
FUNCTION t650_show()
    ...
    DISPLAY BY NAME g_oga.ogaoriu,g_oga.ogaorig,
        ...
        g_oga.oga10,g_oga.oga03,g_oga.oga032,g_oga.oga04,g_oga.oga18,  #No.TQC-740308 add oga18
       ...
       AFTER FIELD ogb091
        ...
        IF g_ogb[l_ac].ogb091 IS NOT NULL AND NOT cl_null(g_ogb[l_ac].ogb09) THEN     #MOD-AB0033
         ...
         IF NOT cl_null(g_ogb[l_ac].ogb09) AND
            ...
            IF SQLCA.sqlcode THEN
              ...
              #NEXT FIELD ogb092
               LET l_flag = NULL
               CALL s_inv_shrt_by_warehouse(g_sma.sma894[2,2],g_ogb[l_ac].ogb09) RETURNING l_flag
        ...
        AFTER FIELD ogb12
           ...
           END IF
            IF NOT cl_null(g_ogb[l_ac].ogb12) THEN
              ...
              IF g_ogb[l_ac].ogb12 > g_img10 THEN
                 ...
                 CALL s_inv_shrt_by_warehouse(g_sma.sma894[2,2],g_ogb[l_ac].ogb09) RETURNING l_flag   #FUN-C80107 add
           ...
           IF g_oga.oga213 = 'N' THEN
              LET g_ogb[l_ac].ogb14 =g_ogb[l_ac].ogb917*g_ogb[l_ac].ogb13
              LET g_ogb[l_ac].ogb14t=g_ogb[l_ac].ogb14*(1+g_oga.oga211/100)
              CALL cl_digcut(g_ogb[l_ac].ogb14,g_azi04)
                   RETURNING g_ogb[l_ac].ogb14
              CALL cl_digcut(g_ogb[l_ac].ogb14t,g_azi04)
                   RETURNING g_ogb[l_ac].ogb14t
           ELSE
              LET g_ogb[l_ac].ogb14t=g_ogb[l_ac].ogb917*g_ogb[l_ac].ogb13
              LET g_ogb[l_ac].ogb14 =g_ogb[l_ac].ogb14t/(1+g_oga.oga211/100)
           ...
           END IF
            ...
            CALL s_fetch_price_new(g_oga.oga03,g_ogb[l_ac].ogb04,g_ogb[l_ac].ogb05,g_oga.oga02,g_oga.oga00,
        ...
        AFTER FIELD ogb912  #第一數量
           ...
           IF NOT cl_null(g_ogb[l_ac].ogb912) THEN
              ...
              END IF
              IF g_ogb[l_ac].ogb17 = 'N' AND g_sma.sma117 = "N" THEN  #No.FUN-610090
                 IF g_ima906 = '2' THEN
                 ...
                 ELSE
                    ...
                    IF g_cnt = 1 THEN
                       CALL cl_err('ogb910/img09','mfg3075',1)
                       NEXT FIELD ogb910
                    END IF
...
#                END IF     #FUN-A60035 ---add    #FUN-A60035 ---MARK
	       ...
	       #FUN-810045 begin
                IF s_chk_rvbs(g_ogb[l_ac].ogb41,g_ogb[l_ac].ogb04) THEN
                  IF NOT s_del_rvbs("1",g_oga.oga01,g_ogb[l_ac].ogb03,0)  THEN                       #FUN-880129
                    ROLLBACK WORK
                    CANCEL DELETE
                  END IF
               END IF
...
FUNCTION t650_b_ogc_1()			# 庫存異動明細(ogc_file)輸入
  ...
  DEFINE i,k,s,l_i    LIKE type_file.num5,    #No.FUN-680137 SMALLINT
   ...
   DECLARE t6006_c_1 CURSOR FOR
           ...
           SELECT ogc09,ogc091,ogc092,ogc12,img10,ogc15,ogc15_fac,ogc16,ogc18   #MOD-910173加上ogc18   #CHI-9C0024 move ogc12
...
FUNCTION t650_b_ogc()			# 庫存異動明細(ogc_file)輸入
  ...
  DEFINE l_flag           LIKE type_file.chr1   #FUN-C80107 add 121024
   ...
   DECLARE t6506_c CURSOR FOR
        ...
        SELECT ogc09,ogc091,ogc092,ogc12,img10,ogc15,ogc15_fac,ogc16,ogc18   #MOD-8B0161   #CHI-9C0024 move ogc12
          FROM ogc_file LEFT OUTER JOIN img_file ON ogc09 = img02 AND ogc091 = img03 AND ogc092 = img04  #No.TQC-9A0132 mod
         WHERE ogc01 = g_oga.oga01 AND ogc03 = g_ogb[l_ac].ogb03
     ...
     AFTER ROW
        IF l_ogc[i].ogc12 IS NOT NULL AND l_ogc[i].ogc12 != 0 THEN
           ...
           IF STATUS THEN
             ...
             #NEXT FIELD ogc09
              LET l_flag = NULL
              CALL s_inv_shrt_by_warehouse(g_sma.sma894[2,2],l_ogc[i].ogc09) RETURNING l_flag
              IF l_flag = 'N' OR l_flag IS NULL THEN
...
#No.FUN-AA0062  --Begin
   ...
   FOR i = 1 TO l_ogc.getLength()
       ...
       INSERT INTO ogc_file(ogc01,ogc03,ogc09,ogc091,ogc092,
                             ogc12,ogc15,ogc15_fac,ogc16,ogc17,ogc18,ogcplant,ogclegal)  #No.MOD-470041   #MOD-8B0161 #FUN-980010 add ogcplant,ogclegal
                     VALUES(g_oga.oga01,g_ogb[l_ac].ogb03,
	      	            ...
	      	            l_ogc[i].ogc15, l_ogc[i].ogc15_fac,
                            l_ogc[i].ogc16,g_ogb[l_ac].ogb04,l_ogc[i].ogc18,g_plant,g_legal)   #MOD-8B0161 #FUN-980010 g_plant,g_legal
       IF SQLCA.sqlcode THEN
...
FUNCTION t650_b_ogg_1()			# 庫存異動明細(ogg_file)輸入
  ...
  DEFINE l_msg2           STRING
   ...
   DECLARE t6006_ogg_c_1 CURSOR FOR
    ...
    SELECT ogg20,ogg09,ogg091,ogg092,ogc12,ogg10,0,ogg15,ogg15_fac,ogg16,ogg18     #FUN-8A0030 Add ''   #No.FUN-8B0021 #MOD-940275 move ogg20   #CHI-9C0024 move ogg12
      FROM ogg_file
     WHERE ogg01 = g_oga.oga01 AND ogg03 = g_ogb[l_ac].ogb03
        ...
        IF l_ogg[i].ogg12 IS NOT NULL AND l_ogg[i].ogg12 != 0 THEN
           ...
           IF l_ima906 = '1' THEN
              ...
              IF STATUS THEN
                ...
                #NEXT FIELD ogg09
                 LET l_flag1 = NULL
                 CALL s_inv_shrt_by_warehouse(g_sma.sma894[2,2],l_ogg[i].ogg09) RETURNING l_flag1
           ...
           IF l_ima906 = '3' THEN
              #IF l_ogg[i].ogg20 = '1' THEN #FUN-C80107 mark
                 ...
                 IF STATUS THEN
                   ...
                   #NEXT FIELD ogg09
                    LET l_flag = NULL
                    CALL s_inv_shrt_by_warehouse(g_sma.sma894[2,2],l_ogg[i].ogg09) RETURNING l_flag
                    IF l_flag = 'N' OR l_flag IS NULL THEN
                       CALL cl_err3("sel","img_file",g_ogb[l_ac].ogb04,"",STATUS,"","sel img",1)
                       NEXT FIELD ogg09
                    ELSE
 ...
 #No.FUN-AA0062  --End
   ...
   FOR i = 1 TO l_ogg.getLength()
       IF l_ogg[i].ogg12 IS NULL OR l_ogg[i].ogg12=0 THEN
          #-----MOD-A20117---------
          IF s_lotout_del(g_prog,g_oga.oga01,g_ogb[l_ac].ogb03,l_ogg[i].ogg18,g_ogb[l_ac].ogb04,'DEL') THEN
             CONTINUE FOR
          END IF
          #-----END MOD-A20117-----
       END IF
       INSERT INTO ogg_file(ogg01,ogg03,ogg09,ogg091,ogg092,ogg10,
                            ogg12,ogg15,ogg15_fac,ogg16,ogg20,ogg18,oggplant,ogglegal)   #MOD-910173 增加ogg18 #FUN-980010 add oggplant,ogglegal
                     VALUES(g_oga.oga01,g_ogb[l_ac].ogb03,
	      	            l_ogg[i].ogg09, l_ogg[i].ogg091,
                            l_ogg[i].ogg092,l_ogg[i].ogg10,
                            l_ogg[i].ogg12, l_ogg[i].ogg15,
                            l_ogg[i].ogg15_fac,l_ogg[i].ogg16,
                            l_ogg[i].ogg20,l_ogg[i].ogg18,g_plant,g_legal)   #MOD-910173 #FUN-980010 add g_plant,g_legal
       IF SQLCA.sqlcode THEN
...
#FUN-A60035 ---MARK END
    ...
    LET g_sql =
        ...
        "       ogb910,ogb911,ogb912,ogb916,ogb917,ogb41,ogb42,ogb43,ogb37,ogb13,ogb14,",  #FUN-810045 add ogb41/42/43#FUN-AB0061
