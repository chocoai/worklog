...
# Modify.........: No:150715     15/07/15 By jedi 关联交易自动生成的发料单不允许做任何操作

DATABASE ds
 
GLOBALS "../../config/top.global"
GLOBALS "../../../topcust/cws/4gl/aws_wms_global.4gl"   #No.FUN-7C0010
GLOBALS "../../../topcust/cws/4gl/aws_ttsrv2_global.4gl" 
...
DEFINE l_sfe013        LIKE sfe_file.sfe013
          ...
          AFTER FIELD sfp03
             IF NOT cl_null(l_sfp.sfp03) THEN
                ...
                IF l_sfp.sfp06 = '4' THEN                                                                                           
                   ...
                   IF l_cnt > 0 THEN                                                                                                
                      CALL cl_err(l_sfp.sfp03,'asf-140',0)                                                                          
                      NEXT FIELD sfp03                                                                                              
                   END IF                                                                                                           
                END IF                                                                                                              
             END IF
   ...
   FOREACH i501_s1_c3 INTO l_sfs.*
      ...
      IF g_prog = 'asfi511' THEN
         IF l_sfs.sfs26 MATCHES '[SUT]' THEN
     ...
     CALL s_showmsg()   
     IF g_success = 'N' THEN RETURN END IF  #No.MOD-8A0088
     ...
     IF l_sfp.sfp06 = '6' THEN    #成套退料                                                                                           
        LET l_sql = "SELECT DISTINCT sfq02 FROM sfq_file",                                                                            
        " WHERE sfq01 = '",l_sfp.sfp01,"'"                                                                                
        PREPARE i526_prepare FROM l_sql                                                                                               
        DECLARE i526_sfq02 CURSOR FOR i526_prepare                                                                                    
        FOREACH i526_sfq02 INTO l_sfq02                                                                                               
  ...
  #FUN-9A0095 add MES ---- mark by liudong 161222
  {
  ...
  {     #FUN-B40053 add begin--------------------------------- 
        IF p_sfp.sfp06 MATCHES '[1234AC]' THEN
           CALL i501sub_VMI_insrvarvb(p_sfp.sfp06,l_sfs.*,l_pmc.*)  RETURNING l_rvb01,l_rvb02
        END IF
       #FUN-B40053 add -end----------------------------------
        IF g_success = 'N' THEN RETURN END IF
        CALL i501sub_VMI_insrvurvv(l_rvb01,l_rvb02,p_sfp.sfp06,l_sfs.*,l_pmc.*)
        IF g_success = 'N' THEN RETURN END IF
    } #xiaoyx,月底再汇总
  END FUNCTION
  ...
  FUNCTION i501sub_VMI_insinainb(p_sfp06,p_sfs,p_pmc,p_sfp)
   ...
   DEFINE l_imgg21   LIKE imgg_file.imgg21     #No.FUN-A10001
     ...
     CALL s_auto_assign_no("aim",g_ina_vmi.ina01,g_today,'1',"ina_file","ina01","","","")
     ...
     CALL s_auto_assign_no("aim",g_ina_vmi.ina01,g_today,'2',"ina_file","ina01","","","")
        RETURNING li_result,g_ina_vmi.ina01
     IF (NOT li_result) THEN
        LET g_success = 'N'
        RETURN
     END IF
        END IF
       #LET g_ina_vmi.ina02 = g_today
       #LET g_ina_vmi.ina03 = g_today   # mark by xufeng 131106
        LET g_ina_vmi.ina02 = p_sfp.sfp03
        LET g_ina_vmi.ina03 = p_sfp.sfp03
    ...
    ##NO.FUN-A10001   add--begin
     ...
     IF g_ina_vmi.ina00 = '1' THEN 
       CALL i501_vmi_tlf(g_inb_vmi.inb04,g_inb_vmi.inb05,g_inb_vmi.inb06,g_inb_vmi.inb07,
             ...
             g_inb_vmi.inb08,g_inb_vmi.inb08_fac,g_ina_vmi.ina04)
      END IF 
...
#TQC-B40227 --end--
  ...
  FUNCTION i501_vmi_tlff(p_part,p_ware,p_loca,p_lot,p_qty,p_sta,p_no,p_no1,p_no2,p_no3,p_unit,p_fac,p_gov,p_tlff219)
     DEFINE
        p_part  LIKE img_file.img01,   ##料件編號(p_part)
        p_ware  LIKE img_file.img02,   ##倉庫 #MOD-580001
        p_loca  LIKE img_file.img03,   ##儲位 #MOD-580001
  ...
  FUNCTION i501sub_VMI_insrvarvb(p_sfp06,p_sfs,p_pmc)
   ...
   DEFINE l_chr      LIKE type_file.chr1    #No.FUN-A10001 
     ...
     IF STATUS = 100 OR l_chr = 'Y' THEN
        ...
        CALL s_auto_assign_no("apm",l_rva.rva01,g_today,"3","rva_file","rva01","","","")
  ...
  #     LET l_rva.rvaconf = 'N'   #No.FUN-A10001
     ...
     LET l_rva.rva114 = 1
        ELSE
     CALL s_curr3(l_rva.rva113,g_today,g_sma.sma904)
  ...
  #  LET l_rvb.rvb03 = ''         #No.FUN-A10001
    ...
    #                l_rva.rva111)
     CALL s_defprice_new(l_rvb.rvb05,l_rva.rva05,l_rva.rva113,g_today,l_rvb.rvb87,'',
         l_rva.rva115,l_rva.rva116,'1',l_rvb.rvb86,'',l_rva.rva112,
    ...
    ##NO.FUN-A10001   add--begin
     CALL i501_vmi_tlf(l_rvb.rvb05,l_rvb.rvb36,l_rvb.rvb37,l_rvb.rvb38,
           ...
           l_rvb.rvb90,l_rvb.rvb90_fac,l_rva.rva05)
  ...
  FUNCTION i501sub_VMI_z(p_sfp01)
   DEFINE p_sfp01  LIKE sfp_file.sfp01
   DEFINE l_cnt    LIKE type_file.num5
   ...
   DEFINE l_cnt2   LIKE type_file.num5
     ...
     SELECT COUNT(*) INTO l_cnt2
      ...
      WHERE apb21 = rvu01
        AND rvu117 = p_sfp01
    #xiaoyx IF l_cnt > 0 THEN
        IF l_cnt1 > 0 OR l_cnt2 > 0 THEN
     CALL cl_err('','asf-199',1)
     LET g_success = 'N'
     RETURN
        ELSE
     CALL i501sub_VMI_z1(p_sfp01)
     IF g_success = 'N' THEN RETURN END IF
        END IF
    # END IF
  END FUNCTION
   
  FUNCTION i501sub_VMI_z1(p_sfp01)
   DEFINE p_sfp01      LIKE sfp_file.sfp01
   DEFINE l_rva        RECORD LIKE rva_file.*
   DEFINE l_rvu        RECORD LIKE rvu_file.*
 ...
 {    DECLARE i501sub_VMI_rva2 CURSOR FOR                                           
     ...
     SELECT * INTO l_rvu.* FROM rvu_file WHERE rvu117=p_sfp01 } #xiaoyx 2013/10/16
  ...
  FUNCTION i501sub_s2(p_argv1,l_sfp)
    ...
    IF l_sfp.sfp06  NOT MATCHES '[49]' THEN  #TQC-AC0077
       ...
       FOREACH i501_s3_c INTO l_sfe01,l_sfe07,l_sfe14, l_sfe012, l_sfe013, l_sfb.*          #FUN-A60028 add l_sfe012,l_sfe013 by vealxu
          CALL i501sub_upd_ecm301('s',l_sfe01,l_sfe07,l_sfe14,l_sfb.*,l_sfe012,l_sfe013)    #FUN-A60028 add l_sfe012,l_sfe013 by vealxu 
          RETURNING l_flag
          IF NOT l_flag THEN
             LET g_success = 'N'
          END IF
  ...
  #        l_imaqty LIKE ima_file.ima262,
    ...
    DEFINE l_sfp RECORD LIKE sfp_file.* #FUN-740187
    DEFINE l_sfs RECORD LIKE sfs_file.* #FUN-740187
    DEFINE l_imd07       LIKE imd_file.imd11  #xiaoyx
     ...
     WHEN p_argv1='2' LET u_type=+1
      ...
      END CASE
    #xiaoyx
     SELECT imd07 INTO l_imd07 FROM imd_file
   WHERE imd01= l_sfs.sfs07
  IF l_imd07 IS NULL THEN LET l_imd07='N' END IF
  IF l_imd07='Y' THEN
  ...
  FUNCTION i501sub_u_sfa(p_code,p_argv1,l_sfp,l_sfs,l_sfb)
    ...
    DECLARE i501_tlff_c CURSOR FOR
      SELECT tlff13,SUM(tlff10) FROM tlff_file  #FUN-9B0149
       WHERE tlff62=l_sfs.sfs03 AND tlff01=l_sfs.sfs04
         AND tlff11=l_sfs.sfs06 AND tlff05=l_sfs.sfs10
    ...
    FOREACH i501_tlff_c INTO l_tlff13,l_tlff10
      IF STATUS THEN
         ...
         EXIT FOREACH   
      END IF
      CASE WHEN l_tlff13='asfi511' LET l_sfaiicd04_t = l_sfaiicd04_t + l_tlf10
           WHEN l_tlff13='asfi512' LET l_sfaiicd05_t = l_sfaiicd05_t + l_tlf10
           WHEN l_tlff13='asfi513' LET l_sfaiicd04_t = l_sfaiicd04_t + l_tlf10
           WHEN l_tlff13='asfi514' LET l_sfaiicd04_t = l_sfaiicd04_t + l_tlf10
           WHEN l_tlff13='asfi526' LET l_sfaiicd04_t = l_sfaiicd04_t - l_tlf10
           WHEN l_tlff13='asfi527' LET l_sfaiicd05_t = l_sfaiicd05_t - l_tlf10
           WHEN l_tlff13='asfi528' LET l_sfaiicd04_t = l_sfaiicd04_t - l_tlf10
           WHEN l_tlff13='asfi529' LET l_sfaiicd04_t = l_sfaiicd04_t - l_tlf10
           OTHERWISE CONTINUE FOREACH
      END CASE
    END FOREACH
       END IF
    ...
    CASE WHEN SQLCA.SQLCODE=0
        ...
        END IF
        EXIT CASE
         WHEN SQLCA.SQLCODE=100 AND l_sfs.sfs26 MATCHES '[SU]'
...
#FUN-A70125 --end--
  ...
  #FUN-A20037 --end--
    ...
    IF g_sma.sma129='N' THEN
        IF l_sfp.sfp06 MATCHES '[134689]' THEN #sfp06=超領(2),超退(7),
                 #重復性生產(ABC)的發料量
                 #不會計入sfa06,所以不會
                 #影響sfb081的結果
    ...
    IF g_plant='YLD008' THEN LET l_flag=1 END IF #xiaoyx
     IF NOT l_flag THEN
     ...
     ELSE
       ...
       #xiaoyx    CALL cl_err('sfb081 > sfb08','asf-447',1) 
       #    LET g_success='N' 
        ELSE
  ...
  FUNCTION i501sub_upd_ecm301(p_code,l_sfs03,l_sfs04,l_sfs10,l_sfb,l_sfs012,l_sfs013)   #FUN-A60028 add l_sfs012,l_sfs013 by vealxu
     DEFINE p_code        LIKE type_file.chr1
     DEFINE l_sfb RECORD  LIKE sfb_file.*
...
#FUN-A60028 --end--    
     ...
     IF (l_ecm04 = l_sfs10) OR (l_sfs10=' ') THEN
        ...
        CALL s_minp_routing(l_sfs03,g_sma.sma73,l_ima153,l_sfs10,l_sfs012,l_sfs013)  #No.FUN-9B0140     #FUN-A60028 add sfs012,sfs013 by vealxu
...
FUNCTION i501sub_y_chk(p_sfp01)
     ...
     DEFINE l_gsfs04     LIKE sfs_file.sfs04 
     DEFINE l_gsfs07     LIKE sfs_file.sfs07 
     DEFINE l_gsfs08     LIKE sfs_file.sfs08
     DEFINE l_gsfs01_1   LIKE sfs_file.sfs01 
     DEFINE l_gsfs04_1   LIKE sfs_file.sfs04 
     DEFINE l_gsfs07_1   LIKE sfs_file.sfs07
   ...
   DEFINE l_a1         LIKE sfa_file.sfa05   #add by pane 150602
   DEFINE l_a4         LIKE sfa_file.sfa05   #add by pane 150709
   DEFINE l_a2         LIKE sfa_file.sfa05   #add by pane 150602
   DEFINE l_a3         LIKE sfa_file.sfa05   #add by pane 150602
   DEFINE l_sfs03      LIKE sfs_file.sfs03   #add by pane 150602
   DEFINE l_sfs04      LIKE sfs_file.sfs04   #add by pane 150602
   DEFINE l_sfs05      LIKE sfs_file.sfs05   #add by pane 150813
   DEFINE l_sfs10      LIKE sfs_file.sfs10   #add by pane 150817
   ...
   #TQC-A50122--end--add--------------
   DEFINE l_b          LIKE type_file.num5   #add by pane 150723
   DEFINE l_sfq02      LIKE sfq_file.sfq02   #add by pane 150723
  ...
  DEFINE l_cmd    STRING 
     ...
     #add by cjy 20140909
     IF l_sfp.sfp07 is null THEN
        CALL cl_err('制造部门为空','!',0)
        LET g_success = 'N'
        RETURN
     END IF
     ...
     IF l_cnt = 0 THEN
        ...
        RETURN
     END IF
     #add by pane 150602 检查发料量begin#
     IF g_prog = 'asfi511' THEN

        CALL s_showmsg_init()
        DECLARE  i501_sfs04 CURSOR  FOR 
         #SELECT sfs03,sfs04 FROM sfs_file
          SELECT sfs03,sfs04,sfs05,sfs10 FROM sfs_file    #mod by pane 150813  #add sfs10 by pane 150817
           WHERE sfs01= p_sfp01
             ORDER BY sfs01,sfs02


       #FOREACH i501_sfs04 INTO l_sfs03,l_sfs04
        FOREACH i501_sfs04 INTO l_sfs03,l_sfs04,l_sfs05,l_sfs10   #mod by pane 150803  #add sfs10 by pane 150817
          #add by pane 150602 begin输入料号的时候检查该料件已经扣帐发料量+已审核量+未审核量# 
              SELECT SUM(sfe16) INTO l_a1 FROM sfe_file
               WHERE sfe01 = l_sfs03
                 AND sfe07 = l_sfs04
                 AND sfe06 = '1' #add by pane 150709
                 AND sfe14 = l_sfs10   #add by pane 150817

              #add by pane 150709 being#
              SELECT SUM(sfe16) INTO l_a4 FROM sfe_file
               WHERE sfe01 = l_sfs03
                 AND sfe07 = l_sfs04
                 AND sfe06 = '4' 
                 AND sfe14 = l_sfs10   #add by pane 150817
              #add by pane 150709 end#
        
              SELECT SUM(sfs05) INTO l_a2 FROM sfs_file,sfp_file
               WHERE sfs01 = sfp01 AND sfs01 <> p_sfp01
                 AND sfs03 = l_sfs03
                 AND sfs04 = l_sfs04
                 AND sfs10 = l_sfs10    #add by pane 150817
                 AND sfpconf <> 'X'
                 AND sfp06='1'  #add by pane 150714
        
              #SELECT sfa05 INTO l_a3 FROM sfa_file   #mark by sunar170905
              SELECT SUM(sfa05) INTO l_a3 FROM sfa_file
               WHERE sfa01 = l_sfs03
                 AND sfa03 = l_sfs04
                 AND sfa08 = l_sfs10   #add by pane 150817
              ...
              IF cl_null(l_a3) THEN
                 LET l_a3 = 0 
              END IF
              #add by pane 150709 begin#
              IF cl_null(l_a4) THEN
                 LET l_a4 = 0 
              END IF
              #add by pane 150709 end#
              #add by pane 150813 begin#
              IF cl_null(l_sfs05) THEN
                 LET l_sfs05 = 0 
              END IF
              #add by pane 150813 end#
             #IF l_a1 + l_a2 - l_a4 >= l_a3 THEN  #add l_a4 by pane 150709 
              IF l_sfs05 + l_a1 + l_a2 - l_a4 > l_a3 THEN  #add l_a4 by pane 150709 #mod by pane 150813
                 LET g_success = 'N'
                 LET g_showmsg = l_sfs03,"/",l_sfs04
                 CALL s_errmsg('sfs03,sfs04',g_showmsg,'','csf-017',1)
              END IF
        END FOREACH 
        #add by pane 150723 begin如果发料单中有工单，明细中没有发料信息，报错不让审核#
        DECLARE i501_y_chk_sfq02 CURSOR FOR
          SELECT sfq02 FROM sfq_file
            WHERE sfq01 = p_sfp01
             ORDER BY sfq02
       
        FOREACH i501_y_chk_sfq02 INTO l_sfq02
          SELECT COUNT(sfs03) INTO l_b FROM sfs_file,sfq_file
            WHERE sfs01 = sfq01
              AND sfs03 = sfq02
              AND sfs01 = p_sfp01
              AND sfs03 = l_sfq02
           ...
           IF l_b = 0 THEN
              LET g_success = 'N'
              LET g_showmsg = p_sfp01,"/",l_sfq02
              CALL s_errmsg('sfq01,sfq02',g_showmsg,'','csf-018',1)
           END IF
        END FOREACH 
        #add by pane 150723 end#
        CALL s_showmsg()
        IF g_success = 'N' THEN
           RETURN
        END IF
     ...
     IF l_sfp.sfp06='3' OR l_sfp.sfp06='1' THEN 
        ...
        FOREACH i501_y_chk_sfs05 INTO l_sfs03a,l_sfs04a,l_sfs27a,
            ...
            SELECT sum(sfa05),sum(sfa06) INTO l_sfa05a,l_sfa06a FROM sfa_file
             WHERE sfa01=l_sfs03a AND sfa03=l_sfs04a #FUN-B20070
               AND sfa08= l_sfs10a   #130509xh add
   ...
   #xiaoyx
      IF l_sfp.sfp06='3' OR l_sfp.sfp06='1' THEN
        DECLARE slip_cnt1 CURSOR FOR
         SELECT sfs03,sfs04  FROM sfs_file  #TQC-B30039
                WHERE sfs01=p_sfp01
        FOREACH slip_cnt1 INTO l_sfs03a,l_sfs04a 
         SELECT  COUNT(*) INTO l_sfa05a
          from sfp_file,sfs_file
          where sfp04='N' AND sfs01=sfp01
          and sfs03=l_sfs03a AND sfs04=l_sfs04a
          and sfs01 != p_sfp01
           IF l_sfa05a IS NULL THEN LET l_sfa05a=0 END IF
           IF l_sfa05a>1 THEN                                                                                
               CALL cl_err(l_sfs04a,'csf-005',0)
               LET g_success='N'
               EXIT FOREACH
            END IF
        END FOREACH
      ...
      # bygaozk 150706  添加 1003 仓发料时 ，不允许审核。
     DECLARE i501_y_chk_sfs08  CURSOR FOR SELECT substr(sfs01,1,4),sfs04,sfs07 FROM sfs_file
             WHERE sfs01=p_sfp01
      FOREACH  i501_y_chk_sfs08 INTO  l_gsfs01_1,l_gsfs04_1,l_gsfs07_1    
        IF g_plant='YLD005' AND NOT (l_gsfs01_1='W014' OR l_gsfs01_1 ='W013') AND l_gsfs07_1='1003' THEN   # 等于1003 仓库时，不允许审核 
        	  
        	          CALL cl_err(l_gsfs04_1,'cim-007',1)
                    LET g_success = "N"
                    EXIT FOREACH 
  ...
  # end 150411   
     ...
     # bygaozk 150411  添加 VMI 仓发料时 ，库位不允许为空。
     DECLARE i501_y_chk_sfs07  CURSOR FOR SELECT sfs04,sfs07,sfs08 FROM sfs_file
             WHERE sfs01=p_sfp01
      FOREACH  i501_y_chk_sfs07 INTO  l_gsfs04,l_gsfs07,l_gsfs08    
        IF l_gsfs07='VMI-10' THEN
        	  IF l_gsfs08='' OR l_gsfs08 is null OR l_gsfs08=' ' THEN
        	          CALL cl_err(l_gsfs04,'cim-006',1)
                    LET g_success = "N"
                    EXIT FOREACH 
        	  END IF
 ...
 #xiaoyx --end
     ...
     FOREACH i501_y_chk_c INTO l_sfs.*
       ...
       #MOD-AC0409---add---end---
        ...
        #FUN-A40022--begin--add------------
        IF s_industry('icd') THEN 
           IF l_sfp.sfp06 MATCHES '[6789B]' AND cl_null(l_sfs.sfs09) 
              AND NOT cl_null(l_sfs.sfs04)
            THEN
                 LET l_imaicd13 = ''
   ...
   {xiaoyx 2011/10/17 
     ...
     亿利达这边可以过0,特此MARK  }
        ...
        IF l_cnt=0 THEN
           ...
           EXIT FOREACH
        END IF
        LET l_sfa05 = NULL         #MOD-B20080 add
        LET l_sfa06 = NULL         #MOD-B20080 add
        SELECT sfa05,sfa06,sfa11 into l_sfa05,l_sfa06,l_sfa11 FROM sfa_file  #FUN-B50059 #TQC-B60001 add sfa11
         WHERE sfa01=l_sfs.sfs03 AND sfa03=l_sfs.sfs04
           AND sfa12=l_sfs.sfs06 AND sfa08=l_sfs.sfs10
           AND sfa27 = l_sfs.sfs27  #FUN-9B0149 
           AND sfa012= l_sfs.sfs012   #FUN-A60028
...
FUNCTION i501sub_y_upd(p_sfp01,p_action_choice,p_inTransaction)  
  ...
  #No:151012  Add  <<--yangjian--
   #做WMS集成的仓库，发料审核时要通知WMS处理
      IF g_success = 'Y' AND l_sfp.sfp06 = '1' THEN
        #从非集成仓库发的料要通知WMS
         SELECT COUNT(*) INTO l_cnt FROM sfs_file WHERE sfs01 = l_sfp.sfp01
            AND sfs07 NOT IN('1003')
         SELECT * INTO g_tc_gws.* FROM tc_gws_file WHERE tc_gws00 = '0'
         IF g_tc_gws.tc_gws01 = 'Y' AND l_cnt > 0 THEN
            LET l_cmd = "aws_erptrans 'wms' 'asfi511_confirm' '"||l_sfp.sfp01||"' "
            CALL cl_cmdrun_wait(l_cmd)
               SELECT tc_log00 ,tc_log051,tc_log052
                 INTO l_tc_log.tc_log00,l_tc_log.tc_log051,l_tc_log.tc_log052
                 FROM tc_log_file 
                WHERE tc_log01 = 'WMS'
                  AND tc_log031 = l_sfp.sfp01
                  AND tc_log07 = 'asfi511_con'
               ...
               IF l_status != 0 OR l_tc_log.tc_log051 != '0' THEN 
                  ...
                  #LET g_success = 'N'      # mark 151012                    	 
               ELSE 
                  CALL cl_err('','cws-006',1)
               END IF 
         END IF
      END IF
  #No:151012  End  <<--yangjian--
...
FUNCTION i501sub_qty(p_cmd,p_sfb)
  ...
  IF p_sfb.sfb39 != '2' THEN
       #工單完工方式為'2' pull 不check min_set
        ...
        CALL s_minp(p_sfb.sfb01,g_sma.sma73,l_ima153,'','','')    #FUN-910053   #FUN-A60028 add 2'' by vealxu 
...
#          IF l_min_set < 0 THEN                        #FUN-960037 --- mark
          IF (l_min_set - l_qty) < 0 THEN               #FUN-960037 --- add
             IF p_cmd = 's' THEN 
...
FUNCTION i501sub_w(p_sfp01,p_action_choice,p_call_transaction)
  ...
  #FUN-AB0001 add end--- 
   ...
   #做WMS集成的仓库，发料审核时要通知WMS处理
      IF g_success = 'Y' AND l_sfp.sfp06 = '1' THEN
        #从非集成仓库发的料要通知WMS
         SELECT COUNT(*) INTO l_cnt FROM sfs_file WHERE sfs01 = p_sfp01
            AND sfs07 NOT IN('1003')
         SELECT * INTO g_tc_gws.* FROM tc_gws_file WHERE tc_gws00 = '0'
         IF g_tc_gws.tc_gws01 = 'Y' AND l_cnt > 0 THEN
            LET l_cmd = "aws_erptrans 'wms' 'asfi511_undo_confirm' '"||p_sfp01||"' "
            CALL cl_cmdrun_wait(l_cmd)
               SELECT tc_log00 ,tc_log051,tc_log052
                 INTO l_tc_log.tc_log00,l_tc_log.tc_log051,l_tc_log.tc_log052
                 FROM tc_log_file 
                WHERE tc_log01 = 'WMS'
                  AND tc_log031 = p_sfp01
                  AND tc_log07 = 'asfi511_undo_con'
                  AND tc_logdate = g_today
                  ORDER BY tc_log00 DESC 
               LET l_str = l_tc_log.tc_log052
               LET l_len = l_str.getIndexOf("<Status code='",1)
...
FUNCTION i501sub_z(p_argv1,p_sfp01,p_action_choice,p_call_transaction)
 ...
 #DEFINE l_sie10   LIKE sie_file.sie10    #No.FUN-A20048 #FUN-AC0074 mark
  ...
  #No:150715  Add  <<--jedi--
   IF NOT cl_null(l_sfp.sfpud10) THEN 
      SELECT COUNT(*) INTO l_cnt FROM tc_rec_file WHERE tc_recid = l_sfp.sfpud10
         AND tc_rec07 = l_sfp.sfp01
      IF l_cnt > 0 THEN 
         CALL cl_err('关联交易自动生成的发料单不允许异动,要取消请撤消关联交易','!',1)
         LET g_success = 'N'
         RETURN 
      END IF 
   END IF 
  ...
  ##NO.FUN-A10001   add--begin   
   ...
   SELECT COUNT(*) INTO l_cnt
    ...
    WHERE pmc917 = l_rvv.rvv32
      AND pmc918 = l_rvv.rvv33 } #xiaoyx
  # IF l_cnt > 0 THEN          
      CALL i501sub_VMI_z(p_sfp01)
      IF g_success = 'N' THEN
         IF p_call_transaction THEN
            ROLLBACK WORK
         END IF
...
FUNCTION i501sub_z2(p_argv1,l_sfp)
  ...
  FOREACH i501_z2_c INTO l_sfe.*, l_sfb.*
      ...
      IF l_sfb.sfb02 != '11' THEN           #No:MOD-9B0079 add
         CALL i501sub_qty('z',l_sfb.*)         #CHI-870030  add
  ...
  IF l_sfp.sfp06  NOT MATCHES '[49]' THEN  #TQC-AC0077
     ...
     FOREACH i501_z3_c INTO l_sfs03,l_sfs04,l_sfs10,l_sfs012,l_sfs013,l_sfb.*            #FUN-A60028 add l_sfs012.l_sfs013 by vealxu 
        CALL i501sub_upd_ecm301('z',l_sfs03,l_sfs04,l_sfs10,l_sfb.*,l_sfs012,l_sfs013)   #FUN-A60028 add l_sfs012.l_sfs013 by vealxu 
             RETURNING l_flag
        IF NOT l_flag THEN
           LET g_success = 'N'
        END IF
...
FUNCTION i501sub_sfs(l_sfp06,l_sfe)
  ...
  LET l_sfs.sfsud01  = l_sfe.sfeud01  #FUN-A60028
  LET l_sfs.sfsud02  = l_sfe.sfeud02  #FUN-A60028
  LET l_sfs.sfsud03  = l_sfe.sfeud03  #FUN-A60028
  LET l_sfs.sfsud04  = l_sfe.sfeud04  #FUN-A60028
  LET l_sfs.sfsud05  = l_sfe.sfeud05  #FUN-A60028
  LET l_sfs.sfsud06  = l_sfe.sfeud06  #FUN-A60028
  LET l_sfs.sfsud07  = l_sfe.sfeud07  #FUN-A60028
  LET l_sfs.sfsud08  = l_sfe.sfeud08  #FUN-A60028
  LET l_sfs.sfsud09  = l_sfe.sfeud09  #FUN-A60028
  LET l_sfs.sfsud10  = l_sfe.sfeud10  #FUN-A60028
  LET l_sfs.sfsud11  = l_sfe.sfeud11  #FUN-A60028
  LET l_sfs.sfsud12  = l_sfe.sfeud12  #FUN-A60028
  LET l_sfs.sfsud13  = l_sfe.sfeud13  #FUN-A60028
  LET l_sfs.sfsud14  = l_sfe.sfeud14  #FUN-A60028
  LET l_sfs.sfsud15  = l_sfe.sfeud15  #FUN-A60028
...
FUNCTION i501sub_x(p_sfp01,p_action_choice,p_call_transaction)
   DEFINE p_sfp01 LIKE sfp_file.sfp01
   DEFINE p_action_choice STRING
   DEFINE p_call_transaction LIKE type_file.num5 #WHEN TRUE -> CALL BEGIN/ROLLBACK/COMMIT WORK
   DEFINE l_sfp   RECORD LIKE sfp_file.*
   ...
   IF cl_null(l_sfp.sfp01) THEN CALL cl_err('',-400,0) LET g_success='N' RETURN END IF
   #-->確認不可作廢
   ...
   IF s_industry('icd') THEN
     ...
     #CASE WHEN l_sfp.sfp06 = '1' #CHI-A70060 mark
      ...
      IF l_cnt > 0 THEN
        ...
        #CASE WHEN l_sfp.sfp06 = '1' #CHI-A70060 mark
         CASE WHEN l_sfp.sfp06 MATCHES '[1234]' #CHI-A70060
                   CALL s_icdinout_del(-1,l_sfp.sfp01,'')
                        RETURNING l_flag
             #WHEN l_sfp.sfp06 = '6' #CHI-A70060 mark
              WHEN l_sfp.sfp06 MATCHES '[6789]' #CHI-A70060
                   CALL s_icdinout_del(1,l_sfp.sfp01,'')
   ...
   IF NOT cl_null(p_action_choice) THEN
      IF NOT cl_void(0,0,l_sfp.sfpconf) THEN #FUN-660106
         CLOSE i501sub_cl
         IF p_call_transaction THEN
            ROLLBACK WORK
         END IF
   ...
   IF l_sfp.sfpconf ='N' THEN  #FUN-660106
