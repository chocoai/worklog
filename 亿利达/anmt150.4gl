...
# Modify.........: No.by shij 130425 增加复制功能
...
DEFINE   g_apa01        LIKE apa_file.apa01    #No.MOD-840530
 ...
 MAIN
   ...
   IF (NOT cl_setup("CNM")) THEN #mod by shij 130425
      EXIT PROGRAM
   END IF
...
DEFINE  lc_qbe_sn       LIKE    gbm_file.gbm01    #No.FUN-580031  HCN
   ...
   INITIALIZE g_npl.* TO NULL    #No.FUN-750051
      ...
      CONSTRUCT g_wc1 ON npm02,npm03,npm04,npm05,npm06
           ...
           FROM s_npm[1].npm02,s_npm[1].npm03,s_npm[1].npm04,s_npm[1].npm05,s_npm[1].npm06
...
FUNCTION t150_menu()
   ...
   WHILE TRUE
      ...
      CASE g_action_choice
         ...
         #add by shij 130425--str
         WHEN "reproduce"
            IF cl_chk_act_auth() THEN
               CALL t150_copy()
            END IF
         #add by shij 130425--end
        ...
        WHEN "entry_sheet1"
         ...
         WHEN "output"
            IF cl_chk_act_auth() THEN
               ...
               LET l_cmd =  "anmr150 '' '' '",g_lang,"' 'Y' '' '' "," '",g_wc CLIPPED,"' '3'" CLIPPED
               CALL cl_cmdrun(l_cmd CLIPPED)
            END IF
...
FUNCTION t150_g_b1()                 	#由應付票據產生單身
   DEFINE l_wc    LIKE type_file.chr1000#No:9568  #No.FUN-680107 VARCHAR(1000)
   ...
   FOREACH t150_b_c INTO g_nmd.*
      ...
      SELECT COUNT(*) INTO g_cnt FROM npl_file,npm_file
       WHERE npl01 = npm01 AND npm03 = g_nmd.nmd01
         AND nplconf = 'N'   #未確認單據  #MOD-910237
      IF g_cnt > 0 THEN
         CONTINUE FOREACH
      END IF
      ...
      LET b_npm.npm04=g_nmd.nmd04
      LET b_npm.npm05=g_nmd.nmd26
      CALL cl_digcut(b_npm.npm05,g_azi04) RETURNING b_npm.npm05
      LET b_npm.npm06=g_nmd.nmd04 * g_npl.npl05
        ...
        AFTER FIELD npl05
           IF NOT cl_null(g_npl.npl05) OR g_npl.npl05 <> 0 THEN
              IF cl_null(g_npl_t.npl05) OR g_npl_t.npl05 <> g_npl.npl05 THEN
                 UPDATE npm_file SET npm06=npm04 * g_npl.npl05
...
DEFINE
    ...
    INPUT ARRAY g_npm WITHOUT DEFAULTS FROM s_npm.*
	   ...
	   BEGIN WORK
        ...
        AFTER FIELD npm03
           IF NOT cl_null(g_npm[l_ac].npm03) THEN
             ...
             #同一張異動,不允許有相同的開票單
              ...
              SELECT COUNT(*) INTO g_cnt FROM npl_file,npm_file  #check是否重覆開立
               WHERE npl01 = npm01 AND npm03 = g_npm[l_ac].npm03
                 AND nplconf='N' AND npl01 <> g_npl.npl01
              IF g_cnt > 0 THEN
              ...
              LET g_npm[l_ac].npm06=g_npm[l_ac].npm04 * g_npl.npl05
               DISPLAY BY NAME g_npm[l_ac].npm06            #No.MOD-4B0244
              CALL cl_digcut(g_npm[l_ac].npm06,g_azi04) RETURNING g_npm[l_ac].npm06
           END IF


        AFTER FIELD npm05,npm06
...
FUNCTION t150_x()
   ...
   END IF
   CALL t150_show()
   IF cl_void(0,0,g_npl.nplconf) THEN
      IF g_npl.nplconf='N' THEN    #切換為作廢
         DELETE FROM npp_file
          WHERE nppsys = 'NM' AND npp00=1 AND npp01 = g_npl.npl01
           ...
           ELSE
              DECLARE npl03_c CURSOR FOR
                 SELECT npl03 FROM npl_file,npm_file
                    WHERE npl01 = npm01 AND npm03 = g_npm[l_cnt].npm03 AND
                          npl02 = l_npl02
              LET l_cnt2 = 1
              FOREACH npl03_c INTO l_npl03[l_cnt2]
                 ...
                 IF g_npl.npl03 = l_npl03[l_cnt2] THEN
                    ...
                    SELECT COUNT(*) INTO l_count FROM npl_file,npm_file
                       WHERE npl01 = npm01 AND npl02 = l_npl02 AND
                             npl03 = l_npl03[l_cnt2] AND
                             npm03 = g_npm[l_cnt].npm03
                    IF l_count > 1 THEN
         ...
         IF NOT cl_null(g_nmz.nmz31) THEN
            IF g_aza.aza73 = 'Y' THEN
               ...
               FOREACH nme24_cs1 INTO l_nme24
                  #IF l_nme24 != '9' THEN  #mark by sunar151019
...
FUNCTION t150_copy()
   ...
   DEFINE l_npl01      LIKE ina_file.ina02
   ...
   IF g_npl.npl01 IS NULL THEN
      CALL cl_err('',-400,0)
      RETURN
   END IF

   LET g_before_input_done = FALSE
   CALL t150_set_entry('a')
   CALL cl_set_head_visible("","YES")

   INPUT l_newno FROM npl01
      ...
      AFTER FIELD npl01
            IF NOT cl_null(l_newno) THEN
               SELECT count(*) INTO g_cnt FROM npl_file
                WHERE npl01 = l_newno
               IF g_cnt > 0 THEN
   ...
   SELECT * FROM npl_file
    ...
    UPDATE y
        SET npl01=l_newno,    #資料鍵值
            npl03='1',        #目前票况
            nplconf='N',        #审核状况
            npluser=g_user,   #資料所有者
            nplgrup=g_grup,   #資料所有者所屬群
            nplmodu=NULL,     #資料修改日期
            nploriu=g_user,     #資料所有者
            npldate=g_today  #資料建立日期

      INSERT INTO npl_file SELECT * FROM y
      IF SQLCA.sqlcode THEN
         CALL cl_err3("ins","npl_file","","",SQLCA.sqlcode,"","",1)
         LET g_success = 'N'
         RETURN
      END IF
      ...
      UPDATE x SET npm01=l_newno

      INSERT INTO npm_file
          SELECT * FROM x
      IF SQLCA.sqlcode THEN
         CALL cl_err3("ins","npm_file","","",SQLCA.sqlcode,"","",1)
         LET g_success = 'N'
         RETURN
      END IF
      IF g_success = 'Y' THEN
   ...
   LET l_oldno = g_npl.npl01
   SELECT npl_file.* INTO g_npl.* FROM npl_file WHERE npl01 = l_newno
   CALL t150_u()
   CALL t150_b()
   SELECT npl_file.* INTO g_npl.* FROM npl_file WHERE npl01 = l_oldno
   CALL t150_show()
END FUNCTION
#add by shij 130425--end
