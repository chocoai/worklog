# Prog. Version..: '5.25.07-12.04.26(00010)'     #
#
...
# Modify.........: No:TQC-B80079 11/08/16 By wujie 检查现金变动码时排除借贷相抵的情况
# Modify.........: No:MOD-B80302 11/08/30 By Dido 由於固資 10/12 類別單號相同,檢核方式也應相同
# Modify.........: No:FUN-BB0165 11/11/28 By Abby 若背景執行，則不開窗顯示錯誤訊息
...
# Modify.........: No:FUN-C30313 12/04/17 By Sakura 原使用fahdmy3處理的地方加npptype判斷
...
#NO.FUN-720003------begin
           ...
           #FUN-920077---end---
              IF  g_bgerr THEN
                  CALL s_errmsg('apyslip',l_t1,g_plant,SQLCA.SQLCODE,1)  #No.TQC-740094
               ELSE
...
#NO.FUN-720003-------end
   ...
   #NO.FUN-720003------begin
              ...
              #FUN-920077---end
                 IF  g_bgerr THEN
                     CALL s_errmsg('ooyslip',l_t1,g_plant,SQLCA.SQLCODE,1)  #No.TQC-740094
                 ELSE
                     CALL s_chknpq_err(g_plant,SQLCA.SQLCODE,'')
                 END IF
              END IF
...
#NO.FUN-720003------begin
                  ...
                  #FUN-920077---end---
                     IF g_bgerr THEN
                        CALL s_errmsg('nmyslip',l_t1,g_plant,SQLCA.SQLCODE,1)  #No.TQC-740094
                     ELSE
                         CALL s_chknpq_err(g_plant,SQLCA.SQLCODE,'')
                     END IF
                  END IF
...
#NO.FUN-720003------begin
               ...
               #FUN-920077---end
                  IF g_bgerr THEN
                     CALL s_errmsg('fahslip',l_t1,g_plant,SQLCA.SQLCODE,1)  #No.TQC-740094
                  ELSE
                     CALL s_chknpq_err(g_plant,SQLCA.SQLCODE,'')
                  END IF
               END IF
...
#NO.FUN-720003------begin
      ...
      IF g_bgjob = 'Y' THEN
        #CALL cl_err(p_no,'aap261',1)        #MOD-B30054 mark
         CALL cl_err(p_no,'aap-261',1)       #MOD-B30054 add
         LET g_success = 'N'
         RETURN
      ELSE
      #FUN-920077---end
...
#NO.FUN-720003------begin
      ...
      IF g_bgjob = 'Y' THEN
        #CALL cl_err(p_no,'aap-262',1) #FUN-BB0165 mark
         CALL cl_err(p_no,'aap-262',0) #FUN-BB0165 add
         LET g_success = 'N'
         RETURN
      ELSE
      #FUN-920077---end
   ...
   FOREACH npq_cur INTO l_npp.*,l_npq.*,l_aag.*  #No.FUN-830161
     ...
     #-MOD-B30051-add-
      IF cl_null(l_npq.npq24) THEN
         LET g_totsuccess = "N"
         IF g_bgjob = 'Y' THEN
            CALL cl_err(l_npq.npq03,'anm-040',1)
            LET g_success = 'N'
            RETURN
         ELSE
            IF g_bgerr THEN
               CALL s_errmsg('npq03',l_npq.npq03,g_plant,'anm-040',1)
            ELSE
            ...
            #FUN-920077---end
               IF g_bgerr THEN
                 #CALL s_errmsg('','',g_plant,'aap-287',1)  #No.TQC-740094               #MOD-AB0215 mark
                  CALL s_errmsg('npq03',l_npq.npq03,g_plant,'aap-287',1)  #No.TQC-740094 #MOD-AB0215
               ELSE
...
#NO.FUN-720003------begin
           #FUN-920077---start
           IF g_bgjob = 'Y' THEN
              CALL cl_err(l_npq.npq03,'aap-039',1)
              LET g_success = 'N'
              RETURN
           ELSE
           #FUN-920077---end
              IF g_bgerr THEN
                 LET g_showmsg=l_npq.npq05,"/",'Y'
      ...
      IF l_aag.aag21 = 'Y' THEN
         ...
         IF p_bookno IS NULL OR l_npq.npq36 IS NULL OR
           ...
           #No.MOD-920001---end--- modify
            ...
            #FUN-920077---start
            IF g_bgjob = 'Y' THEN
               CALL cl_err(l_npq.npq03,'aap-293',1)
               LET g_success = 'N'
               RETURN
            ELSE
            #FUN-920077---end
               IF g_bgerr THEN
                 #CALL s_errmsg('','',g_plant,'aap-293',1)          #MOD-860091 mod aap-297->aap-293               #MOD-AB0215 mark
                  CALL s_errmsg('npq03',l_npq.npq03,g_plant,'aap-293',1)          #MOD-860091 mod aap-297->aap-293 #MOD-AB0215
               ELSE
 	     ...
 	     # CALL cl_replace_sqldb(l_sql) RETURNING l_sql        #FUN-920032
             ...
             # OPEN aznn_c1
               ...
               IF SQLCA.sqlcode THEN
                  ...
                  CALL s_errmsg('aznn00,aznn01',g_showmsg,'','mfg3078',1)
...
#            SELECT afb04,afb15 INTO l_afb04,l_afb15                          #FUN-810069
            SELECT afb04,afb041,afb042 INTO l_afb04,l_afb041,l_afb042         #FUN-810069
              FROM afb_file
              ...
              #FUN-920077---end
                 IF g_bgerr THEN
                    ...
                    LET g_showmsg = p_bookno,'/',l_npq.npq36,'/',l_npq.npq03,'/',
                                    ...
                                    l_npq.npq08
                    CALL s_errmsg('afb00,afb01,afb02,afb03,afb04,afb041,afb042',g_showmsg,'','agl-139',1)
                    #No.FUN-830161  --End
                 ELSE
...
#                    LET g_showmsg=g_bookno,"/",l_npq.npq15,"/",l_npq.npq03,"/",g_azn02,"/",l_dept    #No.FUN-830139
                     LET g_showmsg=p_bookno,"/",l_npq.npq36,"/",l_npq.npq03,"/",g_azn02,"/",l_npq.npq35,'/',l_npq.npq05,'/',l_npq.npq08    #No.FUN-830139
...
#                 #   AND npq15 = l_npq.npq15 AND npq06 = '2' #貸方     #No.FUN-830139
      ...
      IF p_sys = 'AP' OR p_sys ='NM' OR p_sys ='AR' THEN
         IF l_npq.npq07f < 0 OR l_npq.npq07 < 0 THEN
            ...
            #IF l_minus = 'Y' OR l_aag42 = 'Y' THEN         #FUN-D20023
           #mark by cjy 20131213 分录底稿不在管控是否为负数
           # IF l_minus = 'Y' OR l_aag42 = 'Y' OR (g_aza.aza26 = '2' AND p_sys = 'NM') THEN   #FUN-D20023
           # ELSE
           #    IF l_npq.npq07f < 0 THEN
           #       LET g_totsuccess = "N"
           #       IF g_bgjob = 'Y' THEN
           #          CALL cl_err(l_npq.npq07f,'aec-992',1)
           #          LET g_success = 'N'
           #          RETURN
           #       ELSE
           #          IF g_bgerr THEN
           #             CALL s_errmsg('npq07f',l_npq.npq07f,g_plant,'aec-992',1)
           #          ELSE
           ...
           #       LET g_totsuccess = "N"
           #       IF g_bgjob = 'Y' THEN
           #          CALL cl_err(l_npq.npq07,'aec-992',1)
           #          LET g_success = 'N'
           #          RETURN
           #       ELSE
           #          IF g_bgerr THEN
            ...
            #FUN-920077---end
               IF g_bgerr THEN
                 #CALL s_errmsg('','',g_plant,'agl-922',1)  #No.TQC-740094              #MOD-AB0215 mark
                  CALL s_errmsg('npq03',l_npq.npq03,g_plant,'agl-922',1) #No.TQC-740094 #MOD-AB0215
               ELSE
...
#NO.FUN-720003------begin
               #FUN-920077---start
               IF g_bgjob = 'Y' THEN
                  CALL cl_err(l_npq.npq08,'apj-005',1)
                  LET g_success = 'N'
                  RETURN
               ELSE
               #FUN-920077---end
                  IF g_bgerr THEN
                     LET g_showmsg=l_npq.npq08,"/",'Y'
         ...
         #FUN-920077---end
            IF g_bgerr THEN
              ...
              #CALL s_errmsg('pja01,pjaacti',g_showmsg,g_plant,'aap-288',1)  #No.TQC-740094  #FUN-810045 gja->pja #MOD-980116
               LET g_showmsg=l_npq.npq02,"/",l_npq.npq03,"/",'1'                #MOD-980116
...
#NO.FUN-720003------begin
         ...
         #FUN-920077---end
            IF g_bgerr THEN
              #LET g_showmsg=l_npq.npq08,"/",'Y'   #MOD-980116
              #CALL s_errmsg('pja01,pjaacti',g_showmsg,g_plant,'aap-288',1)  #No.TQC-740094   #FUN-810045 gja->pja #MOD-980116
               LET g_showmsg=l_npq.npq02,"/",l_npq.npq03,"/",'2' #MOD-980116
...
#NO.FUN-720003------begin
         ...
         #FUN-920077---end
            IF g_bgerr THEN
              #LET g_showmsg=l_npq.npq08,"/",'Y'       #MOD-980116
              #CALL s_errmsg('pja01,pjaacti',g_showmsg,g_plant,'aap-288',1)  #No.TQC-740094   #FUN-810045 gja->pja #MOD-980116
               LET g_showmsg=l_npq.npq02,"/",l_npq.npq03,"/",'3' #MOD-980116
...
#NO.FUN-720003------begin
         ...
         #FUN-920077---end
            IF g_bgerr THEN
              #LET g_showmsg=l_npq.npq08,"/",'Y'     #MOD-980116
              #CALL s_errmsg('pja01,pjaacti',g_showmsg,g_plant,'aap-288',1)  #No.TQC-740094   #FUN-810045 gja->pja #MOD-980116
               LET g_showmsg=l_npq.npq02,"/",l_npq.npq03,"/",'4'  #MOD-980116
      ...
      IF (l_aag.aag311 = '2' OR     #異動碼-1控制方式
         ...
         #FUN-920077---end
            IF g_bgerr THEN
              #LET g_showmsg=l_npq.npq08,"/",'Y'      #MOD-980116
              #CALL s_errmsg('pja01,pjaacti',g_showmsg,g_plant,'aap-288',1)  #No.TQC-740094   #FUN-810045 gja->pja #MOD-980116
               LET g_showmsg=l_npq.npq02,"/",l_npq.npq03,"/",'5' #MOD-980116
      ...
      IF (l_aag.aag321 = '2' OR     #異動碼-1控制方式
         ...
         #FUN-920077---end
            IF g_bgerr THEN
              #LET g_showmsg=l_npq.npq08,"/",'Y'   #MOD-980116
              #CALL s_errmsg('pja01,pjaacti',g_showmsg,g_plant,'aap-288',1)  #No.TQC-740094   #FUN-810045 gja->pja #MOD-980116
               LET g_showmsg=l_npq.npq02,"/",l_npq.npq03,"/",'6' #MOD-980116
      ...
      IF (l_aag.aag331 = '2' OR     #異動碼-1控制方式
         ...
         #FUN-920077---end
            IF g_bgerr THEN
              #LET g_showmsg=l_npq.npq08,"/",'Y'        #MOD-980116
              #CALL s_errmsg('pja01,pjaacti',g_showmsg,g_plant,'aap-288',1)  #No.TQC-740094   #FUN-810045 gja->pja #MOD-980116
               LET g_showmsg=l_npq.npq02,"/",l_npq.npq03,"/",'7' #MOD-980116
      ...
      IF (l_aag.aag341 = '2' OR     #異動碼-1控制方式
         ...
         #FUN-920077---end
            IF g_bgerr THEN
              #LET g_showmsg=l_npq.npq08,"/",'Y'  #MOD-980116
              #CALL s_errmsg('pja01,pjaacti',g_showmsg,g_plant,'aap-288',1)  #No.TQC-740094    #FUN-810045 gja->pja #MOD-980116
               LET g_showmsg=l_npq.npq02,"/",l_npq.npq03,"/",'8' #MOD-980116
      ...
      IF (l_aag.aag351 = '2' OR     #異動碼-1控制方式
         ...
         #FUN-920077---end
            IF g_bgerr THEN
              #LET g_showmsg=l_npq.npq08,"/",'Y'    #MOD-980116
              #CALL s_errmsg('pja01,pjaacti',g_showmsg,g_plant,'aap-288',1)  #No.TQC-740094    #FUN-810045 gja->pja #MOD-980116
               LET g_showmsg=l_npq.npq02,"/",l_npq.npq03,"/",'9' #MOD-980116
      ...
      IF (l_aag.aag361 = '2' OR     #異動碼-1控制方式
         ...
         #FUN-920077---end
            IF g_bgerr THEN
              #LET g_showmsg=l_npq.npq08,"/",'Y'    #MOD-980116
              #CALL s_errmsg('pja01,pjaacti',g_showmsg,g_plant,'aap-288',1)  #No.TQC-740094  #FUN-810045 gja->pja #MOD-980116
               LET g_showmsg=l_npq.npq02,"/",l_npq.npq03,"/",'10' #MOD-980116
...
#FUN-950053 -----------------add end------------------------------------
      ...
      IF p_sys = 'NM' THEN
         SELECT pmc903 INTO g_pmc903 FROM pmc_file
          WHERE pmc01 = l_npq.npq11
          LET l_pmc903 = g_pmc903
         IF STATUS THEN
            SELECT occ37 INTO g_occ37 FROM occ_file
             WHERE occ01 = l_npq.npq11
            LET l_pmc903 = g_occ37
         END IF
      END IF
      #yinhy130407  --End
            ...
            #FUN-920077---end
               IF g_bgerr THEN
                 ...
                 #CALL s_errmsg('pja01,pjaacti',g_showmsg,g_plant,'aap-288',1)  #No.TQC-740094   #FUN-810045 gja->pja #MOD-980116
                  ...
                  LET g_showmsg=l_npq.npq02,"/",l_npq.npq03,"/",ls_ze03  #MOD-980116
     ...
     #異動碼1
      IF NOT cl_null(l_npq.npq11) THEN
         ...
         IF NOT cl_null(g_errno) THEN
            LET g_totsuccess = "N"
            IF g_bgerr THEN
               LET g_showmsg=l_npq.npq03,":",l_npq.npq11
     ...
     #異動碼2
      IF NOT cl_null(l_npq.npq12) THEN
         ...
         IF NOT cl_null(g_errno) THEN
            LET g_totsuccess = "N"
            IF g_bgerr THEN
               LET g_showmsg=l_npq.npq03,":",l_npq.npq12
     ...
     #異動碼3
      IF NOT cl_null(l_npq.npq13) THEN
         ...
         IF NOT cl_null(g_errno) THEN
            LET g_totsuccess = "N"
            IF g_bgerr THEN
               LET g_showmsg=l_npq.npq03,":",l_npq.npq13
     ...
     #異動碼4
      IF NOT cl_null(l_npq.npq14) THEN
         ...
         IF NOT cl_null(g_errno) THEN
            LET g_totsuccess = "N"
            IF g_bgerr THEN
               LET g_showmsg=l_npq.npq03,":",l_npq.npq14
     ...
     #異動碼5
      IF NOT cl_null(l_npq.npq31) THEN
         ...
         IF NOT cl_null(g_errno) THEN
            LET g_totsuccess = "N"
            IF g_bgerr THEN
               LET g_showmsg=l_npq.npq03,":",l_npq.npq31
     ...
     #異動碼6
      IF NOT cl_null(l_npq.npq32) THEN
         ...
         IF NOT cl_null(g_errno) THEN
            LET g_totsuccess = "N"
            IF g_bgerr THEN
               LET g_showmsg=l_npq.npq03,":",l_npq.npq32
     ...
     #異動碼7
      IF NOT cl_null(l_npq.npq33) THEN
         ...
         IF NOT cl_null(g_errno) THEN
            LET g_totsuccess = "N"
            IF g_bgerr THEN
               LET g_showmsg=l_npq.npq03,":",l_npq.npq33
     ...
     #異動碼8
      IF NOT cl_null(l_npq.npq34) THEN
         ...
         IF NOT cl_null(g_errno) THEN
            LET g_totsuccess = "N"
            IF g_bgerr THEN
               LET g_showmsg=l_npq.npq03,":",l_npq.npq34
     ...
     #異動碼-關係人
      IF NOT cl_null(l_npq.npq37) THEN
         ...
         IF NOT cl_null(g_errno) THEN
            LET g_totsuccess = "N"
            IF g_bgerr THEN
               LET g_showmsg=l_npq.npq03,":",l_npq.npq37
