...
# Modify.........: NO.110919     11/09/19 by xiayan (1)将sfbud01放到“工单资料一”页签中
#                                                   (2)打印时增加按钮"打印明细资料"和"打印单头资料"
# Modify.........: No:140513     14/05/13 By yangjian 采购单审核时下发给WMS产商
...
MAIN
   ...
   #IF (NOT cl_setup("ASF")) THEN  #NO.110919 modify
   IF (NOT cl_setup("CSF")) THEN  #NO.110919 modify
      EXIT PROGRAM
   END IF
   ...
   IF g_bgjob = "N" OR cl_null(g_bgjob) THEN
      #OPEN WINDOW t301_w AT p_row,p_col WITH FORM "asf/42f/asfi301"  #NO.110919 modify
      OPEN WINDOW t301_w AT p_row,p_col WITH FORM "csf/42f/asfi301"  #NO.110919 modify
...
#                             sub_p_o, s_n, run_card, gen_m_iss, aps_related_data, aps_route_subc_data")         
   CALL aws_efapp_flowaction("insert, modify, delete, detail, query, locale, void, confirm, undo_confirm, easyflow_approval,gen_allotment,delete_allotment,sub,gen_allotment,delete_allotment,sub,gen_routing, mold_wtdw, mold_return, m_iss, mat_shift, sub_p_o, s_n, run_card, gen_m_iss, aps_related_data, aps_route_subc_data")
#FUN-A50032 mod end ----
...
#FUN-AA0059---------mod------------end------------------
               ...
               WHEN INFIELD(sfb98)
                    ...
                    ELSE
                      #LET g_qryparam.form = "q_gem"    #mark by zhangcba20131023
                      LET g_qryparam.form = "cq_gem15"    #add by zhangcba20131023
                    END IF
                    CALL cl_create_qry() RETURNING g_qryparam.multiret
...
#FUN-9C0119--add--end            
         ...
         WHEN "routing"
           IF g_plant='YLD009' THEN
             LET g_msg="ceci701", " '",g_sfb.sfb01 CLIPPED,"' ''"
           ELSE
...
#  並將所有合拼的工單及PBI資料做確認
          ...
          # bygaozk 160910 修改 sfb02=7 时，工艺不能勾选
           IF g_sfb.sfb02='7' AND g_sfb.sfb93='Y' THEN
                 UPDATE sfb_file SET sfb93='N',sfb06='' WHERE sfb01=g_sfb.sfb01 
                
           END IF

         # END 
         
       #171010  没有工艺资料生成 时，工艺不能勾选
         IF g_plant='YLD005' THEN
           LET l_gcnt02=0
           select count(*) into l_gcnt02 from ecm_file where ecm01=g_sfb.sfb01
           IF l_gcnt02=0  THEN
           UPDATE sfb_file SET sfb93='N',sfb06='' WHERE sfb01=g_sfb.sfb01   
           END IF
          END IF 
...
#FUN-A80054--end--add---------------------------------------------------------------------------
      ...
      #bygaozk 150902 yld004 中 ,update sfb98,sfb82='GC' 
                  IF g_success='Y' THEN
                    IF g_plant='YLD004' THEN
                 UPDATE sfb_file SET sfb98='GC',sfb82='GC' WHERE sfb01=g_sfb.sfb01 
                    END IF
                  END IF
      #bygaozk 150902
             
               #No:140513  Add  <<--yangjian--
                  IF g_success = 'Y' THEN
                     SELECT * INTO g_tc_gws.* FROM tc_gws_file WHERE tc_gws00 = '0'
                     IF g_tc_gws.tc_gws01 = 'Y' THEN
                        LET l_cmd = "aws_erptrans 'wms' 'asfi301' '"||g_sfb.sfb01||"' "
                        CALL cl_cmdrun_wait(l_cmd)
                        SELECT tc_log00 ,tc_log051,tc_log052
                          INTO l_tc_log.tc_log00,l_tc_log.tc_log051,l_tc_log.tc_log052
                          FROM tc_log_file 
                         WHERE tc_log01 = 'WMS'
                           AND tc_log031 = g_sfb.sfb01
                           AND tc_log07 = 'asfi301'
                        ...
                        ELSE 
                           CALL cl_err('','cws-006',0)
                        END IF 
                     END IF
                  END IF
               #No:140513  End  <<--yangjian--
            END IF
            CALL i301_show()
            ...
            IF cl_chk_act_auth() THEN
              ...
              #No:140513  Add  <<--yangjian--
                  IF g_success = 'Y' THEN
                     SELECT * INTO g_tc_gws.* FROM tc_gws_file WHERE tc_gws00 = '0'
                     IF g_tc_gws.tc_gws01 = 'Y' THEN
                        LET l_cmd = "aws_erptrans 'wms' 'asfi301_undo' '"||g_sfb.sfb01||"' "
                        CALL cl_cmdrun_wait(l_cmd)
                        SELECT tc_log00 ,tc_log051,tc_log052
                          INTO l_tc_log.tc_log00,l_tc_log.tc_log051,l_tc_log.tc_log052
                          FROM tc_log_file 
                     ...
                     END IF
                  END IF
               IF g_success = 'N' THEN CONTINUE WHILE END IF 
               #No:140513  End  <<--yangjian--
               IF NOT cl_null(g_sfb.sfb85) AND l_cnt > 0 THEN  #MOD-B30414
         ...
         WHEN "output"
            IF cl_chk_act_auth() THEN
            	 IF g_plant='YLD009' THEN        #160908pj
                   IF (g_sfb.sfb04='1'  or g_sfb.sfb87='X') THEN
            	          CALL cl_err('','aqc-025 ',5)
                       COMMIT WORK
                    ELSE   
                        CALL i301_out()  
   ...
   DEFINE l_oeaud10     LIKE oea_file.oeaud10
    ...
    INPUT BY NAME g_sfb.sfboriu,g_sfb.sfborig,g_sfb.sfb01,g_sfb.sfb101,g_sfb.sfb44,g_sfb.sfb81,g_sfb.sfb02,g_sfb.sfb39,   #FUN-8C0081 add sfb44
                 ...
                 #g_sfb.sfb13,g_sfb.sfb15,g_sfb.sfb25,g_sfb.sfb081,                         #FUN-9A0029 mark
                  ...
                  g_sfb.sfb34,g_sfb.sfb96,g_sfb.sfb40,g_sfb.sfb97, #FUN-6A0007 add sfb1001 #CHI-970012 add sfb96
        ...
        AFTER FIELD sfb01
          ...
          #xiaoyx          LET g_sfb.sfb98=g_smy.smy60
            #xiaoyx
       select smymemo1 INTO g_sfb.sfb98 FROM smy_file
      WHERE  smyslip =l_slip
      DISPLAY BY NAME g_sfb.sfb98
        ...
        #xiaoyx    IF cl_null(g_sfb.sfb98) THEN LET g_sfb.sfb98 = ' '
        IF cl_null(g_sfb.sfb98) THEN
          CALL cl_err('stop','csf-003',0)
          NEXT FIELD sfb98
            ELSE
           #xiaoyx
            SELECT COUNT(*) INTO ls_cnt FROM gem_file
          #xiaoyx  WHERE gem01 in ('JG','FW','JC','GC','JJ','SP','DF','XF')
           WHERE gem01 IN (SELECT smymemo1 FROM smy_file WHERE smymemo1 IS NOT NULL)
            AND gem01=g_sfb.sfb98
     ...
     {   AFTER FIELD sfb102
        ...
        AFTER FIELD sfb82
          ...
          # bygaozk 171114 
          LET  g_sfb.sfb102=g_sfb.sfb82  
           DISPLAY BY NAME g_sfb.sfb102
           END IF
       
       #180417
            IF g_plant='YLD007' AND g_sfb.sfb02='7'AND cl_null(g_sfb.sfb82)  THEN
                   CALL cl_err(g_sfb.sfb01,'csf-046',1)
                     NEXT FIELD sfb82
            END IF 
        ...
        AFTER FIELD sfb22
           ...
           IF NOT cl_null(g_sfb.sfb22) AND g_sfb.sfb22[1,g_doc_len]<>'MRP' THEN   #No.FUN-550067
            ...
            #No:150715  Add  <<--jedi--
              ...
              LET g_sfb.sfbud01 = l_oea01||' '||l_oea032||' '||l_oea10||' '||l_oeaud01
              LET g_sfb.sfbud10 = l_oeaud10
              DISPLAY BY NAME g_sfb.sfbud01,g_sfb.sfbud10
            #No:150715  End  <<--jedi--
           END IF
        ...
        AFTER FIELD sfb221
             ...
             #str MOD-A40133 add
              ...
              IF g_sfb.sfb08=0 OR g_sfb.sfb08 IS NULL THEN
                ...
                #LET g_sfb.sfb08 = l_qty             #MOD-A50195 mark
                 LET g_sfb.sfb08 = l_qty - l_woqty   #MOD-A50195
              END IF
           ...
           IF NOT cl_null(g_sfb.sfb22) AND NOT cl_null(g_sfb.sfb221) THEN
             ...
             #IF l_woqty>= g_sfb.sfb08 THEN               #MOD-A50016 mark
              IF l_woqty + g_sfb.sfb08 > l_qty THEN       #MOD-A50016
              ...
              ELSE
                #LET g_sfb.sfb08 = g_sfb.sfb08 - l_woqty  #MOD-A50016 mark
                 LET g_sfb.sfb08 = l_qty - l_woqty        #MOD-A50016
                #str MOD-A50195 add
...
#FUN-AA0059 ---------------------end-------------------------------
            ...
            DISPLAY BY NAME g_sfb.sfb15
           CALL i301_set_no_entry(p_cmd)
     ## by 180417  添加：yld007，的委外工单 厂商部门不允许为空
        IF g_plant='YLD007' AND g_sfb.sfb02='7' THEN
...
#FUN-A50066--end--mark--------------------
        ...
        AFTER FIELD sfb15
            ...
            IF NOT cl_null(g_sfb.sfb22) AND NOT cl_null(g_sfb.sfb221) THEN
               ...
               END IF
            END IF
          {xiaoyx IF NOT cl_null(l_sfb15_t) AND g_sfb.sfb15 != l_sfb15_t THEN
      ...
      #   END IF
       ...
       AFTER FIELD sfbud07
       #   IF NOT cl_validate() THEN NEXT FIELD CURRENT END IF
       UPDATE sfa_file SET sfa05=sfa05*g_sfb.sfbud07,sfa161=sfa161*g_sfb.sfbud07
       WHERE sfa03 IS NOT NULL AND sfa01=g_sfb.sfb01
        ...
        AFTER INPUT #判斷必要欄位之值是否有值,若無則反白顯示,並要求重新輸入
           ...
           END IF
            ...
            IF g_cnt = 0 THEN
             ...
             IF NOT cl_null(g_sfb.sfb95) THEN
                LET g_cnt = 0
                IF g_sma.sma52 = 'Y' AND g_sfb.sfb93 = 'Y' THEN
                   SELECT COUNT(*) INTO g_cnt FROM bra_file
...
FUNCTION i301_show()
   ...
   DEFINE l_imc03      LIKE imc_file.imc03
   DEFINE l_imc04      LIKE imc_file.imc04
    ...
    DISPLAY BY NAME g_sfb.sfboriu,g_sfb.sfborig,
       ...
       g_sfb.sfbud13,g_sfb.sfbud14,g_sfb.sfbud15 
  #xiaoyx
   IF CL_NULL(g_sfb.sfb86) AND CL_NULL(g_sfb.sfbud04) AND NOT CL_NULL(g_sfb.sfb22) THEN
      SELECT occ02 INTO g_sfb.sfbud04
      FROM oea_file,occ_file,oeb_file
      WHERE oea01=oeb01
      AND oea04=occ01
      AND oeb01=g_sfb.sfb22
      AND oeb03=g_sfb.sfb221
      IF g_sfb.sfbud04 IS NULL THEN LET g_sfb.sfbud04=' ' END IF
        
       update sfb_file set sfbud04=g_sfb.sfbud04 where sfb01=g_sfb.sfb01  
       DISPLAY BY NAME g_sfb.sfbud04    
    END IF
    
   
# bygaozk 14-11-15  更新客户简称
    IF NOT CL_NULL(g_sfb.sfb86)  THEN  # bygaozk14-11-12  
      	
     SELECT occ02 INTO l_gsfbud04
      FROM oea_file,occ_file,oeb_file
      WHERE oea01=oeb01
      AND oea04=occ01
      AND oeb01=(select sfb22 from sfb_file where sfb01=g_sfb.sfb86)
      AND oeb03=(select sfb221 from sfb_file where sfb01=g_sfb.sfb86)
    IF l_gsfbud04 IS NULL THEN LET l_gsfbud04='' END IF
      
      update sfb_file set sfbud04=l_gsfbud04 where sfb01=g_sfb.sfb01 
       select sfbud04 into g_sfb.sfbud04 from sfb_file where sfb01=g_sfb.sfb01
      DISPLAY BY NAME g_sfb.sfbud04
    END IF  
    
    
    # bypj  更新订单单身备注
   { IF NOT CL_NULL(g_sfb.sfb86)  AND CL_NULL(g_sfb.sfbud02)  THEN  # bypj16-11-12  
      	
     SELECT oebud01 INTO l_gsfbud04
      FROM oeb_file
      WHERE
       oeb01=(select sfb22 from sfb_file where sfb01=g_sfb.sfb86)
      AND oeb03=(select sfb221 from sfb_file where sfb01=g_sfb.sfb86)
    IF l_gsfbud04 IS NULL THEN LET l_gsfbud04='' END IF
      
      update sfb_file set sfbud02=l_gsfbud04 where sfb01=g_sfb.sfb01 
       select sfbud02 into g_sfb.sfbud02 from sfb_file where sfb01=g_sfb.sfb01
      DISPLAY BY NAME g_sfb.sfbud02
    END IF}
   # bygaozk 141226
    IF g_plant='YLD009'  THEN 
        # 添加 sfb96 备注 -料件说明 
      LET l_gcnt01=0
      select count(*) into l_gcnt01 from imc_file where imc01= g_sfb.sfb05
    IF l_gcnt01>0 AND CL_NULL(g_sfb.sfb96) THEN 
      DECLARE t540_pmn20_cs CURSOR FOR                                                                                                                                                                                     
       SELECT imc03,imc04 FROM imc_file                                                                                           
        WHERE imc01=g_sfb.sfb05                                                                                                   
                                                                               
      FOREACH t540_pmn20_cs INTO l_imc03,l_imc04   #MOD-A10033                                                                                 
         IF NOT CL_NULL(l_imc03) THEN                                                                                                          
           LET g_sfb.sfb96=g_sfb.sfb96,l_imc03,'、',l_imc04                                                                                                                                                                                                          
         END IF  
      END FOREACH
 
    DISPLAY BY NAME g_sfb.sfb96
    END IF 
  END IF 
 
   IF NOT CL_NULL(g_sfb.sfb22)  THEN  # bygaozk14-11-12  
...
DEFINE l_flag       LIKE type_file.num5    #MOD-AC0336
    ...
    END IF
    # bygaozk 170105
     IF (g_plant='YLD001' OR g_plant='YLD004') AND NOT CL_NULL(g_sfb.sfbud10) THEN
         CALL cl_err('','csf-037',1)
         RETURN
     END IF   # end if 
    ...
    IF g_rec_b=0 THEN CALL g_sfa.clear() END IF
...
#FUN-AA0059 ---------------------end-------------------------------
    ...
    AFTER FIELD sfa26
        IF NOT cl_null(g_sfa[l_ac].sfa26) THEN
           ...
           IF g_sfa[l_ac].sfa26 NOT MATCHES '[0123456SUT78Z]' THEN  #No:8765   #FUN-A20037 add '7,8,Z'
              NEXT FIELD sfa26
           END IF
           IF g_sfa[l_ac].sfa26 MATCHES '[012345678]' THEN  #No:8765  #FUN-A20037 add '7,8'
              LET g_sfa[l_ac].sfa03=g_sfa[l_ac].sfa27
              SELECT * INTO g_ima.* FROM ima_file
               WHERE ima01=g_sfa[l_ac].sfa03
              IF STATUS THEN
...
FUNCTION i301_sfa05()   #no.5173 add
  ...
  # WHERE ima01=g_sfa[l_ac].sfa27   #MOD-A70105 mark
   ...
   IF STATUS THEN LET l_ima64 = 0 LET l_ima641 = 0 END IF
   #check最少發料數量
   IF l_ima641 <>  0 AND g_sfa[l_ac].sfa05 < l_ima641 THEN
      LET g_errno = 'asf-100'
   END IF
...
FUNCTION i301_out()
    ...
    DEFINE l_date LIKE sfb_file.sfb81    #NO.111013 add
 
    IF g_sfb.sfb01 IS NULL THEN RETURN END IF
    LET l_no=g_sfb.sfb01
 
    MENU ""   #NO.110919 add by xiayan
      ON ACTION print_all  #NO.110919 add
         LET g_cnt =0 
   ...
   #NO.110919 add by xiayan---------------------begin
     ON ACTION dantou_sfb
         LET g_wc = 'sfb01 = "',l_no,'"'
    ...
    #NO.111013 add--------------------------begin
      ON ACTION gen_card
       IF g_plant='YLD009' THEN 
       	  IF  cl_null(g_sfb.sfbud11)  THEN
       	  	 LET g_cnt=0
       	  ELSE
       	      LET g_cnt=g_sfb.sfbud11
       	  END IF
         
         
       	 LET g_wc = 'sfb01 = "',l_no,'" AND sfb81 BETWEEN "',g_sfb.sfb81,'" AND "',g_sfb.sfb81,'"'
         LET g_sql = "csfr303 ",
                     " '",g_today,"'",
                     " '",g_user,"'",
                     " '",g_lang,"'", #  " '0'",  #MOD-530280-11
                     ...
                     " ' '",
                     " '",g_wc clipped,"'"
                    # " '",g_wc CLIPPED AND sfb81 BETWEEN '",g_sfb.sfb81,"' AND '"g_sfb.sfb81,"'"'"  
   ...
   # 15-01-15 
    ...
    ON ACTION tell_cxmr100    # 充电机车间 标签
         LET g_wc = 'sfb01 = "',l_no,'"'
    ...
    # END 
     ...
     # by gaozk 180117  空调风机灰色横向标签
    ON ACTION tell_cxmr110    # 充电机车间 标签
         LET g_wc = 'sfb01 = "',l_no,'"'
  ...
  # 添加 cxmr019 报表 
    ...
    #NO.111017 add----------------------------end
      ON ACTION o_csfr706
        SELECT NVL(sfbud09,0) INTO g_sfb.sfbud09 FROM sfb_file WHERE sfb01=g_sfb.sfb01
        IF g_sfb.sfbud09=1 THEN
          CALL cl_err('','mr-005',0)
        ELSE
         LET g_wc = 'sfb01 = "',l_no,'" AND sfb81 BETWEEN "',g_sfb.sfb81,'" AND "',g_sfb.sfb81,'"'
         LET g_sql = "csfr706",
                     " ' '",
...
#               =new[i].ima262 + new[i].new_qty5 - new[i].new_qty6   #NO.FUN-A20044
   ...
   ###折和量累加
         ...
         AFTER INPUT
            ...
            FOR i = 1 TO new.getLength()
               IF new[i].new_part IS NOT NULL AND
                  #new[i].new_yes = "Y"                        THEN  #MOD-490390  #MOD-A50107 mark
                   new[i].new_yes = "Y" AND new[i].new_qty !=0 THEN  #MOD-490390  #MOD-A50107
                #重新抓取發料單位與轉換率
                  SELECT ima63,ima63_fac,ima86,ima86_fac INTO l_ima63,
                    l_ima63_fac,l_ima86,l_ima86_fac
                    FROM ima_file
    ...
    #MOD-AC0336--end--modify----------
      ...
      ELSE
         UPDATE sfb_file SET sfb13=g_sfb.sfb13,
          ...
          WHERE sfb01=g_sfb.sfb01
      END IF
      #CALL cl_getmsg('asf-386',g_lang) RETURNING g_msg  #mark test
      #CALL cl_msgany(0,0,g_msg)   #mark test
      CALL cl_err('','asf-386',0)  #add test
 #str MOD-A40026 add
  ELSE
  ...
  #表示有尚未確認的子工單需要一起清除
   IF l_sfb86_cnt > 0 THEN
      ...
      FOREACH i301_dealc_cs INTO l_sfb01
        ...
        DELETE FROM she_file WHERE she01 = g_sfb.sfb01
        IF SQLCA.sqlcode THEN
           CALL cl_err3("del","she_file",g_sfb.sfb01,"",STATUS,"","del she_sub",1)  #No.FUN-660128
           LET g_success = 'N'
        END IF
 
        DELETE FROM sfw_file WHERE sfw01 = g_sfb.sfb01
        IF SQLCA.sqlcode THEN
...
#    如已產生過雜發單不可再重新產生
     ...
     IF l_cnt < 1 THEN
        ...
        IF NOT cl_null(g_sfb.sfb51) THEN
          ...
          SELECT azf03 INTO tm.azf03 FROM azf_file
...
FUNCTION i301_m_iss()
  ...
  IF SQLCA.sqlcode THEN
     IF SQLCA.sqlerrd[3]=0 THEN
        CALL cl_err3("sel","ina_file",g_sfb.sfb01,"","asf-020","","",1)  #No.FUN-660128
        RETURN
     ELSE
     RETURN
     END IF
...
FUNCTION i301_ui_default()
  ...
  #CALL cl_set_comp_visible("sfb1001,sfb1002,sfb1003",g_sma.sma79='Y') #FUN-6A0007
   CALL cl_set_comp_visible("sfb1002,sfb1003",g_sma.sma79='Y') #FUN-6A0007
