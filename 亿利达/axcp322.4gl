# Prog. Version..: '5.30.02-12.08.07(00005)'     #
#
...
# Modify.........: No:FUN-C80094 12/08/27 By xuxz add ---->#add by xuxz 20120827
# Modify.........: No:FUN-C90002 12/09/04 By minpp 成本类型给默认值ccz28
# Modify.........: No.FUN-CA0108 12/10/19 By zhangweib 增加接收背景參數邏輯段
# Modify.........: No:131009     13/10/09 By yangjian  cdjud02
...
MAIN
  ...
  #No.FUN-CA0108   ---satrt--- Add
   LET g_bgjob = ARG_VAL(1)
   LET tm.tlfctype = ARG_VAL(2)
   LET tm.yy = ARG_VAL(3)
   LET tm.mm = ARG_VAL(4)
   LET tm.b = ARG_VAL(5)
...
DEFINE l_ooz08    LIKE ooz_file.ooz08   #FUN-BB0038
  ...
  #            "   AND ",g_wc CLIPPED 
   ...
   IF g_oaz.oaz92 = 'N' OR (g_oaz.oaz92 = 'Y' AND g_oaz.oaz93 = 'Y') THEN  #FUN-C90002
      #SELECT tlfclegal,tlfctype,tlfccost,tlf01,tlf14,-(tlf907*tlfc21) a,oga03,oga15,tlf15,tlf16,oga032
      #SELECT tlfclegal,tlfctype,tlfccost,tlf01,tlf14,-(tlf907*tlfc21) a,oga03,oga15,tlf15,tlf16,oga032,oga032,(tlf10*tlf60) b   #FUN-D90054 add (tlf10*tlf60) b
      #SELECT tlfclegal,tlfctype,tlfccost,tlf01,tlf14,-(tlf907*tlfc21) a,oga03,oga15,tlf15,tlf16,oga032,(tlf10*tlf60) b   #140104jiangxt
      SELECT tlfclegal,tlfctype,tlfccost,tlf01,tlf14,-(tlf907*tlfc21) a,oga03,oga15,tlf15,tlf16,oga032,(tlf10*tlf60*tlf907) b   #140104jiangxt  #mod by cjy 20140313数量和金额都需要判断是否是入库还是出库
        FROM tlf_file,tlfc_file,oga_file                                           
      ...
      IF g_oaz.oaz92 = 'Y' AND g_oaz.oaz93 = 'Y' THEN             #FUN-C90002
         #SELECT tlfclegal,tlfctype,tlfccost,tlf01,tlf14,tlfc21 a,omf05 oga03,oga15,tlf15,tlf16,omf051 oga032
   ...
   IF g_oaz.oaz92 = 'Y' AND g_oaz.oaz93 = 'N' THEN  
      ...
      SELECT tlfclegal,tlfctype,tlfccost,tlf01,tlf14,ccc23*cfc15 a,oga03,oga15,tlf15,tlf16,oga032,(tlf10*tlf60) b   #FUN-D90054 add (tlf10*tlf60)
   ...
   IF g_oaz.oaz92 = 'N' OR (g_oaz.oaz92 = 'Y' AND g_oaz.oaz93 = 'Y') THEN  #FUN-C90002
      #SELECT tlfclegal,tlfctype,tlfccost,tlf01,tlf14,-(tlf907*tlfc21) a,oha03 oga03,oha15 oga15,tlf15,tlf16,oha032 oga03
      SELECT tlfclegal,tlfctype,tlfccost,tlf01,tlf14,-(tlf907*tlfc21) a,oha03 oga03,oha15 oga15,tlf15,tlf16,oha032 oga032,-1*(tlf907*tlf10*tlf60) b  
                 #FUN-D90054 add (tlf10*tlf60) #140106 suncx oga03 -> oga32 ,(tlf10*tlf60) -> -1*(tlf10*tlf60)  #Exia add (tlf907*tlf10*tlf60)
  ...
  #IF g_oaz.oaz92 = 'Y' THEN  #FUN-C90002
      IF g_oaz.oaz92 = 'Y' AND g_oaz.oaz93 = 'Y' THEN  #FUN-C90002
         SELECT tlfclegal,tlfctype,tlfccost,tlf01,tlf14,-tlfc21 a,omf05 oga03,oha15 oga15,tlf15,tlf16,omf051 oga032,-1*(tlf10*tlf60) b                                      
   ...
   #FUN-C90002  --begin
   IF g_oaz.oaz92 = 'Y' AND g_oaz.oaz93 = 'N' THEN  
      SELECT tlfclegal,tlfctype,tlfccost,tlf01,tlf14,ccc23*cfc15 a,oha03 oga03,oha15 oga15,tlf15,tlf16,oha032 oga032,-1*(tlf10*tlf60) b                                      
   ...
   #FUN-C80009 --end
   #将相同客户，部门，理由码，科目汇总 
   #LET l_sql = " SELECT tlfclegal,tlfctype,tlfccost,tlf01,tlf14,SUM(a),oga03,oga15,tlf15,tlf16,oga032 ",
   LET l_sql = " SELECT tlfclegal,tlfctype,tlfccost,tlf01,tlf14,SUM(a),oga03,oga15,tlf15,tlf16,oga032,SUM(b) ", #FUN-D90054 add b #140106 b->SUM(b)
               "   FROM cdj_temp ",
               "  GROUP BY tlfclegal,tlfctype,tlfccost,tlf01,tlf14,oga03,oga15,tlf15,tlf16,oga032 "  #140106 suncx remark 
              # "  GROUP BY tlfclegal,tlfctype,tlfccost,tlf01,tlf14,oga03,oga15,tlf15,tlf16,oga032,b " #FUN-D90054 add b  #140106 suncx mark
   ...
   FOREACH p322_c1 INTO l_cdj.cdjlegal,l_cdj.cdj04,l_cdj.cdj05,l_cdj.cdj06,l_cdj.cdj07,l_cdj.cdj12,
      ...
      #LET g_cdj.cdj07 = l_cdj.cdj07    #mark by lanhang 121114 料件成本分群
      SELECT ima12 INTO g_cdj.cdj07 FROM ima_file WHERE ima01=l_cdj.cdj06 #add by lanhang 121114 料件成本分群抓取 ima12
      SELECT ima09 INTO g_cdj.cdjud02 FROM ima_file WHERE ima01=l_cdj.cdj06 #No:131009  Add  <<--yangjian--
      ...
      LET g_cdj.cdj11 = cl_digcut(g_cdj.cdj11,g_azi03)   #TQC-BB0046
      LET g_cdj.cdj12 = cl_digcut(g_cdj.cdj12,g_azi04)   #TQC-BB0046
      ...
      let l_cdj.cdj08 =null
      let l_cdj.cdj09 =null
      let actno_d1 = null
      let actno_d2 = null 
      ...
      IF cl_null(l_cdj.cdj08) THEN   #借方
         ...
         IF cl_null(actno_d1) THEN 
           ...
           SELECT oba111,'' INTO actno_d1,actno_d2 FROM oba_file   #mod by wangzf 130509 
            WHERE oba01 IN (SELECT ima131 FROM ima_file WHERE ima01=l_cdj.cdj06) #mod by wangzf 130509
         END IF
         IF cl_null(actno_d1) THEN 
      ...
      #FUN-BB0038 --end
      LET g_cdj.cdj00 = '1' #add by xuxz 20120827FUN-C80094 
...
#FUN-BB0038 --begin
  ...
  # LET l_wc = cl_replace_str(l_wc,"ccc03","cdj03")
   ...
   LET l_sql = "SELECT count(*) ",
            ...
            #   "   AND ",l_wc CLIPPED  
               "   AND cdj00 = 1 ",   #add by luttb 20120904FUN-C80094
