...
# Modify.........: No.130725     13/07/25 By tangyh未付金額應包含 apc14/apc15
# Modify.........: No:20130930     13/09/30 By cjy “转账” “票据” “收票转付.单据日期不可大于票据关帐日期
# Modify.........: No:20131014   13/10/14 By leizj 审核时再次管控 “转账” “票据” “收票转付.单据日期不可大于票据关帐日期
# Modify.........: No:20131015   13/10/15 By leizj 取消审核时管控 “转账” “票据” “收票转付.单据日期不可大于票据关帐日期
...
# Modify.........: No.FUN-C90027 15/05/12 By pane 新增apf47
# Modify.........: No.MOD-CC0054 15/05/12 By pane aapt335單身二賬款日期默認為單頭沖帳日期
# Modify.........: No:CHI-B70031 15/05/13 By pane 差異視窗開啟判斷調整
# Modify.........: No:FUN-D30032 15/05/14 By pane 修改單身新增時按下放棄鍵未執行AFTER INSERT的問題
# Modify.........: No:MOD-B90204 15/05/14 By pane 轉銷部分單身輸入完后金額与賬款部分的金額一致，但是系統會提示：借貸不合，請選擇差异處理方式
# Modify.........: No.MOD-BB0133 11/11/22 By yinhy apt335 产生apa时，预设apa73=apa34  #addbywangw161228
...
DEFINE  g_nmz10         LIKE nmz_file.nmz10
...
DEFINE l_count1 LIKE type_file.num5 #added by leizj20131015
...
#No.FUN-A60024 --begin
             IF g_aptype = '36' THEN
               ...
               #OPEN WINDOW t310_w33 AT 2,2 WITH FORM "cap/42f/aapt335"  #mod by pane 150512
                ATTRIBUTE (STYLE = g_win_style CLIPPED)
             END IF
#No.FUN-A60024 --end
       END IF
...
#add by leizj 20131015 “转账” “票据” “收票转让”转付”单据日期不可大于票据关帐日期
         IF g_aptype = '33' THEN
         LET l_count1=0
...
#                  CALL s_check_no(g_sys,g_apf.apf01,g_apf01_t,g_aptype,"apf_file","apf01","") RETURNING li_result,g_apf.apf01
        ...
        AFTER FIELD apf03
         IF g_aptype='34' THEN
            IF p_cmd = 'a' OR g_apf.apf03 != g_apf_o.apf03 THEN
               CALL t310_apf03('a')
            ...
            CALL t310_set_no_entry(p_cmd)
        ELSE
            IF p_cmd = 'a' OR g_apf.apf03 != g_apf_o.apf03
              OR g_apf.apf03 != g_apf_o.apf03 THEN
              CALL t310_apf03('a')
              IF NOT cl_null(g_errno) THEN
                 CALL cl_err(g_apf.apf03,g_errno,0)
                 LET g_apf.apf03 = g_apf_o.apf03
...
#No.FUN-CB0066 --end
      IF g_apz.apz27 = 'N' THEN
         LET g_sql = "SELECT 0,apa01,apc02,apa02,apa13,apa14,",   #TQC-9B0119
                     "       apc08-apc10-apc16-apc14,apc09-apc11-apc16*apa14-apc15, ",
      ...
      ELSE
         LET g_sql = "SELECT 0,apa01,apc02,apa02,apa13,apa14,", #TQC-9B0119
                     #"       apc08-apc10-apc16,apc13-apc16*apc07, ",   #130724 mark by tangyh
                     "       apc08-apc10-apc16-apc14,apc13-apc16*apc07,", #130725 mod by tangyh
                     "apaud01,apaud02,apaud03,apaud04,apaud05,apaud06,apaud07,apaud08,",
                     "apaud09,apaud10,apaud11,apaud12,apaud13,apaud14,apaud15,",
                     "       azi01,apa44",
                   # "  FROM ",g_dbs_new CLIPPED,"apa_file LEFT OUTER JOIN ",g_dbs_new CLIPPED,"azi_file ON apa_file.apa13 = azi_file.azi01, ",       #FUN-A50102 mark
            ...
            IF l_cnt > 0  THEN
               ...
               SELECT COUNT(*) INTO l_cnt FROM apa_file
                WHERE (apa00 = '23' OR apa00 = '25')
                  ...
                  AND apa02 <= g_apf.apf02          #MOD-A30146 add
               IF l_cnt > 0 THEN
                  CALL cl_err(g_apg[l_ac].apg04,'aap-753',1)
                  NEXT FIELD apg04
               END IF
            END IF
...
FUNCTION t310_b2_tot()
  ...
  #CHI-A10034---add---end---
      ...
      IF g_aptype <> '36' THEN    #No.FUN-A90007
        ...
        #-MOD-B10116-end-
           #LET g_apf.apf09 = g_apf.apf10 -  l_apf08       #MOD-A90078 #TQC-B10199 mark
         END IF    #MOD-B10116
...
FUNCTION t310_apg04(p_cmd)
   ...
   DEFINE g_t1            LIKE oay_file.oayslip   #MOD-580041  #No.FUN-690028 VARCHAR(5)
   DEFINE l_apydmy3       LIKE apy_file.apydmy3   #MOD-580041
   DEFINE l_apc14         LIKE apc_file.apc14    #add by zhangym 130329


   LET g_errno = ' '
   IF g_apz.apz27 = 'N' THEN
      #LET g_sql = "SELECT apa02,apa13,apa14,apc08-apc10-apc16,apc09-apc11-apc16*apa14,",
      LET g_sql = "SELECT apa02,apa13,apa14,apc08-apc10-apc16-apc14,apc09-apc11-apc16*apa14-apc15,",    #mod by zhangym 130306
...
#                  "    AND apa00[1,1] = '1'",                         #No.FUN-A40003
   ...
   ELSE
      #LET g_sql = "SELECT apa02,apa13,apa72,apc08-apc10-apc16,apc13-apc16*apa72,",  #No.TQC-780034
      LET g_sql = "SELECT apa02,apa13,apa72,apc08-apc10-apc16-apc14,apc13-apc16*apa72,",  #No.TQC-780034   #mod by zhangym 130306
                  " apa06,apa07,apa21,apa22,apa13,apa00,apa41,apa42,apa08,apa44 ",
                # "  FROM ",g_dbs_new,"apa_file, ",                           #FUN-A50102 mark
                #           g_dbs_new,"apc_file  ",                           #FUN-A50102 mark
                  "  FROM ",cl_get_target_table(g_plant_new,'apa_file'),",",  #FUN-A50102
...
#No.FUN-A40003 --end
   ...
   END IF
   #LET g_sql = " SELECT apa20,apc08 ,apc10 ,apc09,apc11,apa14,apa72,apc13 ",   #No.FUN-680027 add
   LET g_sql = " SELECT apa20,apc08 ,apc10 ,apc09,apc11,apa14,apa72,apc13,apc14 ",   #No.FUN-680027 add  #mod by zhangym 130329
...
#No.FUN-A40003 --end
   ...
   FETCH t310_z1 INTO l_apc16,l_apc08 ,l_apc10 ,l_apc09,l_apc11,l_apa14,l_apa72,l_apc13,l_apc14   #NO.FUN-680027 add  #mod by zhangym 130329
   CLOSE t310_z1
   ...
   #      (l_apc09-l_apc11-(l_apc16*l_apa14)-l_apg05) > l_amt THEN #No.FUN-680027 add
      IF (l_apc08 -l_apc10 -l_apc16-l_apg05f-l_apc14) > l_amtf OR         #No.FUN-680027 add
         (l_apc09-l_apc11-(l_apc16*l_apa14)-l_apg05-l_apc14) > l_amt THEN #No.FUN-680027 add
   #mod by zhangym 130329 end----
         LET g_errno='aap-250'
      END IF
   ELSE
   #mod by zhangym 130329 begin----
   ...
   #      (l_apc13-(l_apc16*l_apa72)-l_apg05) > l_amt THEN #No.FUN-680027 add
      IF (l_apc08 -l_apc10 -l_apc16-l_apg05f-l_apc14) > l_amtf OR #No.FUN-680027 add
         (l_apc13-(l_apc16*l_apa72)-l_apg05-l_apc14) > l_amt THEN #No.FUN-680027 add
   #mod by zhangym 130329 end-----
         LET g_errno='aap-250'
      END IF
   END IF
...
#No.FUN-A90007 --end
        ...
        BEFORE INSERT
           ...
           LET g_aph_o.aph08 = NULL                                    #MOD-A80076
...
#No.FUN-A90007 --end
        ...
        AFTER FIELD aph03
        #add by cjy 20130930 “转账” “票据” “收票转让”转付”单据日期不可大于票据关帐日期
         IF g_aptype = '33' THEN
             ...
             #IF g_aptype <>'32' THEN     #No.FUN-A40003
              IF (g_aptype !='36') OR (g_aptype = '36' AND g_apz.apz68 = 'Y') THEN   #CHI-CA0044
                 IF g_aph[l_ac].aph03 NOT MATCHES "[6789ABDZ]" THEN      #FUN-B40011 add D
                    ...
                    IF g_aza.aza63 = 'Y' THEN
                       SELECT apw03 INTO g_aph[l_ac].aph041 FROM apw_file
                        WHERE apw01=g_aph[l_ac].aph03
                    END IF
                 END IF
              END IF         #No.FUN-A40003
...
#No.FUN-A40003 --begin
            IF NOT cl_null(g_aph[l_ac].aph04) AND g_aptype = '32' THEN
               ...
               END IF
            END IF
            LET g_aph[l_ac].aph041 = g_aph[l_ac].aph04
#No.FUN-A40003 --end
#No.FUN-B40011 --begin
        ...
        AFTER FIELD aph08
           ...
           IF g_aph[l_ac].aph03 = '2' THEN   #TQC-630158
             #IF cl_null(g_aph[l_ac].aph04) AND cl_null(g_aph[l_ac].aph041) THEN  #MOD-990189      #MOD-A80076 mark
              IF g_aph_o.aph08 IS NULL OR g_aph_o.aph08 != g_aph[l_ac].aph08 THEN                  #MOD-A80076
                 SELECT nma05,nma051 INTO g_aph[l_ac].aph04,g_aph[l_ac].aph041 FROM nma_file    #No.FUN-680029
                  WHERE nma01 = g_aph[l_ac].aph08
                 DISPLAY BY NAME g_aph[l_ac].aph04
                 LET g_aph_o.aph08 = g_aph[l_ac].aph08                                             #MOD-A80076
              END IF #MOD-990189
           END IF
       ...
       #FUN-970077--End
        ...
        AFTER FIELD aph13,aph14
            ...
            IF g_aph[l_ac].aph03 NOT MATCHES "[6789]" AND
               ...
               DISPLAY BY NAME g_aph[l_ac].aph14
            END IF
            IF p_cmd = 'a' OR g_aph[l_ac].aph13 != g_aph_t.aph13 THEN
        ...
        AFTER FIELD aph05f
            ...
            IF g_aph[l_ac].aph03 NOT MATCHES "[45]" THEN
               ...
               IF g_apz.apz27 = 'N' THEN
                  SELECT apc08-apc10,apc09-apc11
                    INTO g_qty1,g_qty2
                    FROM apa_file,apc_file
                   WHERE apa01 = g_aph[l_ac].aph04 AND apa00 = l_type
                     AND apc01 = apa01 AND apc02 = g_aph[l_ac].aph17
      ...
      AFTER FIELD aph23                  #帳款編號
         IF NOT cl_null(g_aph[l_ac].aph23) THEN   #No.FUN-690080
            IF p_cmd = 'a' OR (p_cmd = 'u' AND g_aph[l_ac].aph23 != g_aph_t.aph23) THEN
               IF g_apf.apf47 = '2' THEN #FUN-C90027
...
#No.FUN-A90007 --end
        ...
        #   LET l_ac_t = l_ac   #FUN-D30032
            IF INT_FLAG THEN
               ...
               #FUN-D30032--add--end--
               END IF
               LET g_aph_o.aph03 = ''  #No.FUN-680029
               CLOSE t310_bcl
               ROLLBACK WORK
               EXIT INPUT
            END IF
              ...
              WHEN INFIELD(aph08)
                 ...
                 ELSE
                    IF g_aph[l_ac].aph03 = '2' THEN
                       ...
                       END IF
                    ELSE
                       LET g_qryparam.arg1 = g_aph[l_ac].aph03   #No.TQC-5B0114
                    END IF
                 END IF   #MOD-620055
...
###mod by wangzf 130511
  # LET l_apa.apa13 = m_apf.apf06
  # LET l_apa.apa14 = m_apf.apf09/m_apf.apf09f

    select distinct aph13,aph14 into l_apa.apa13,l_apa.apa14 from aph_file
      where aph01 = g_apf.apf01
#end add

   SELECT azi07,azi04 INTO t_azi07,t_azi04 FROM azi_file
...
FUNCTION t310_firm1_chk()
   ...
   define l_cntw  smallint

   #LET g_success = 'Y'           #TQC-B10069
 #marked by lzj 130711 取消本次付款金额不可为0管控 start
#   IF g_apf.apf08+g_apf.apf09 = 0 AND g_apf.apf10 = 0 THEN
 ...
 #     RETURN
 #  ELSE
#marked by lzj 130711 取消本次付款金额不可为0管控	 end


#add by leizj 20131014 “转账” “票据” “收票转让”转付”单据日期不可大于票据关帐日期
         IF g_aptype = '33' THEN
         LET l_cnt=0
      ...
      IF g_apf.apf08+g_apf.apf09 != 0 THEN   #CHI-850001
         ...
         END IF
      END IF   #CHI-850001
#   END IF #marked by lzj 130711 取消本次付款金额不可为0管控	 end
...
###add by wangzf 130511
   let l_cntw = 0
   select count(distinct aph13) into l_cntw from aph_file where aph01 = g_apf.apf01
   if l_cntw > 1 then
      CALL cl_err('','cap-098',1)
      LET g_success = 'N'
      RETURN
   end if
#end add
...
FUNCTION t310_firm1_chk1()
 ...
 DEFINE  l_amt     LIKE type_file.num20_6     #MOD-AB0197
  ...
  #-MOD-AB0197-end-
   ...
   END IF
   IF only_one != '1' THEN
      LET g_sql = "SELECT * FROM apf_file WHERE ",g_wc CLIPPED
...
#No.FUN-A40003 --end
       ...
       #LET g_plant_new = m_apg.apg03   #MOD-B10147 mark
        ...
        CLOSE z3_curs
        IF NOT cl_null(m_apg.apg06) THEN
           #LET g_sql = "SELECT apc16,apc08 ,apc10 ,apc09,apc11,apc06  ",
           ...
           #FETCH z3_curs_apc INTO l_apc16,l_apc08 ,l_apc10 ,l_apc09,l_apc11,l_apc06
...
#   LET l_nme.nme14=''
   ...
   END IF
   #LET l_nme.nme24='9'
   LET l_nme.nme24='99'  #modif by sunar151019
   ...
   IF l_aza73 = 'Y' THEN
    # LET g_sql="SELECT nme24 FROM ",g_dbs_nm CLIPPED,"nme_file",                  #FUN-A50102 mark
      ...
      FOREACH t310_z_nme_c1 INTO l_nme24
         #IF l_nme24 <> '9' THEN
          IF l_nme24 <> '99' THEN   #modify by sunar151019
...
FUNCTION t310_set_entry_b(p_cmd)
  DEFINE p_cmd   LIKE type_file.chr1    #No.FUN-690028 VARCHAR(1)
      ...
      CALL cl_set_comp_entry("aph09",TRUE)   #FUN-B40011
END FUNCTION
...
#FUN-B40011 --begin
         IF g_aph[l_ac].aph03 ='D' THEN
            ...
            CALL cl_set_comp_entry("aph07,aph09,aph14,aph041",FALSE)
         END IF
#FUN-B40011 --end
      END IF
...
#No.FUN-A60024 --begin
      IF g_aptype ='36' THEN
         CALL cl_set_comp_entry("aph03",FALSE)
      END IF
#No.FUN-A60024 --end
...
DEFINE  i            LIKE type_file.num5   #No.FUN-A90007
   ...
   SELECT * INTO m_apf.* FROM apf_file WHERE apf01 = g_apf.apf01   #No.TQC-B20112
#No.FUN-A80111 --begin
   CALL t310_chk_apa35(m_apf.apf01)
   IF g_success = 'N' THEN RETURN END IF
#No.FUN-A80111 --end
...
#No.FUN-A90007 --end
         ...
         LET l_apa.apa73  = l_apa.apa34          #No.MOD-BB0133  #addbywangww161228
         LET l_apa.apa56  = 0
...
#No.TQC-B20112 --begin
   SELECT * INTO m_apf.* FROM apf_file WHERE apf01 = g_apf.apf01
   LET l_cnt = 0
   #FUN-C90027 --str
...
#No.FUN-A90007 --begin
      IF g_aptype ='32' THEN
         SELECT * INTO l_apa.* FROM apa_file WHERE apa01 = m_apf.apf01   #No.TQC-B20112
      ELSE
...
#No.FUN-A90007 --begin
      IF g_aptype ='32' THEN
         ...
         IF SQLCA.sqlcode AND SQLCA.sqlerrd[3] = 0 THEN
            ...
            EXIT FOREACH
         ELSE
           UPDATE aph_file SET aph04 = g_apz.apz64 WHERE aph01 = l_aph.aph01 AND aph02 = l_aph.aph02
   ...
   IF diff_flag MATCHES "[345]" THEN
      ...
      IF diff_flag ='3' THEN
        ...
        #-MOD-B10116-add-
         ...
         END IF
         CALL t310_b2_tot()
         CALL t310_tmp_pay(g_apf.*)
        #-MOD-B10116-end-
      END IF
      IF diff_flag='4' THEN
         CALL t310_ins_apa()           #轉溢退
      END IF
      IF diff_flag ='5' THEN
         CALL t310_diff()
