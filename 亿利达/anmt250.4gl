...
# Modify.........: No:MOD-D50227 13/05/28 By Polly 增加控卡，已存在應付憑單, 不可取消確認
# Modify.........: No:FUN-D40034 13/07/11 By yangtt 取消作廢需控卡關帳日期
# Modify.........: No:MOD-D70045 13/07/08 By yinhy 應給omb48賦值
# Modify.........: No:MOD-D70200 13/08/01 By apo 原幣手續費依原幣幣別取位
# Modify.........: No:FUN-D70122 13/08/02 by yuhuabao 會計年期會不按照自然年月設置,修改全系統邏輯，年期的判斷需要按照aooq011的設置來
# Modify.........: No:MOD-D80121 13/08/19 By yinhy 狀態為質押，審核時應更新anmt200票況為質押
# Modify.........: No:MOD-D90007 13/09/04 By yinhy 暫收的收票單據可以做轉付
# Modify.........: No:CHI-D90023 13/09/14 By apo 1.增加npn03為6時且nmh41為Y,oma55不可大於0之控卡,開窗、自動產生單身一併調整
#                                                2.確認及取消確認時,若npn03為6,應更新oma/omc_file的金額
# Modify.........: No:MOD-DA0176 13/10/28 By yuhuabao 調用anmp400的時候用戶範圍傳g_npn.npnuser
# Modify.........: No.FUN-DA0125 13/11/4  By Sulamite add g_data_keyvalue
# Modify.........: No.FUN-DA0126 13/11/05 By bart 移除azo_file程式段
# Modify.........: No.MOD-DC0176 13/12/27 By yuhuabao 不區分版別,異動類型為4：票帖或者8：兌現時,單身手續費欄位顯示以供用戶輸入 寫入nme_file時，若npn03 = 4 OR 8，則金額扣減手續費
# Modify.........: No.MOD-E10025 14/01/03 By apo 1.調整抓取oma18/oma181邏輯:先抓取ool13/ool131,若無資料才取oca03/oca04
#                                                2.調整anm-997至開立帳視窗以前
# Modify.........: No.MOD-E30106 14/03/20 By yihsuan 原判斷異動別為 5 時,將本幣異動不可維護,改為 5/6/7 皆不可維護
# Modify.........: No.FUN-E50028 14/05/30 By Reanna 還原CR
# Modify.........: No.FUN-940114 14/06/03 By yihsuan 1.異動別(npn03)增加一個"質押撤回"的選項
#                                                    2.確認時將anmt200的目前票況(nmh24)改為1.未異動
#                                                    3.取消確認時將anmt200的目前票況(nmh24)改為3.質押
# Modify.........: No.MOD-E70081 14/07/16 By Sabrina 不使用gnmg250，改用anmr250
# Modify.........: No.MOD-E70101 14/07/22 By yihsuan anm-662 增加 1.質押撤回,與程式段一併調整
# Modify.........: NO:FUN-E80012 14/08/04 By qiull 修改付款日期之後,tic現金流量明細重複
# Modify.........: No.CHI-E80025 14/08/26 By doris _delHeader()，在詢問是否保留單據前，需重新 SELECT 實際單身剩餘總比數，並改為其他變數儲存，不用 g_rec_b 判斷是否保留單據
# Modify.........: No.MOD-F40057 15/04/16 By yihsuan oma64|oma70 給予 1
# Modify.........: No:FUN-F50015 15/06/01 By Mio 解決【新增】、【複製】之後無法紀錄key值問題
# Modify.........: No:MOD-G40089 16/04/21 By yihsuan 單身輸入完後，增加詢問axr-309是否產生分錄底稿
# Modify.........: No:MOD-G60001 16/06/02 By edison 應撤/退/作廢票立帳還原加上是否還原控卡
# Modify.........: No:MOD-G90086 16/09/28 By yihsuan 1.新增/修改時，若單別須拋轉總帳，詢問是否產生分錄
#                                                    2.確認時控卡帳款金額與分錄底稿金額需一致                           
# Modify.........: No:MOD-GA0008 16/10/07 By Sabrina 該單別若不需拋轉傳票，則不需產生分錄底稿
...
MAIN
  ...
  #TQC-B70197 --begin
    IF g_aza.aza26 = '2' THEN
      #LET comb_value = '3,4,5,6,7,8,9' #FUN-C70129 add 9   #MOD-E70101 mark
       LET comb_value = '1,3,4,5,6,7,8,9'                   #MOD-E70101 add                                                                                      
   ...
   IF NOT cl_null(g_argv1) THEN
      CASE g_argv2
         ...
         WHEN "insert"
            ...
            IF cl_chk_act_auth() THEN
               CALL t250_a()
               LET g_data_keyvalue = g_npn.npn01         #FUN-F50015 add
            END IF
         OTHERWISE
                CALL t250_q()
      END CASE
      ...
      CASE g_action_choice
         ...
         WHEN "output"
            IF cl_chk_act_auth() THEN
               ...
               LET l_cmd =  "anmr250 '' '' '",g_lang,"' 'Y' '' '' "," '",g_wc CLIPPED,"' '3'" CLIPPED #FUN-C30085 mark #FUN-E50028 remark
...
FUNCTION t250_fetch(p_flnpn)
   ...
   ELSE
      ...
      LET g_data_keyvalue = g_npn.npn01    #No.FUN-DA0125  
      CALL t250_show()                      # 重新顯示
   END IF
END FUNCTION
...
FUNCTION t250_show()
   ...
   LET g_data_keyvalue = g_npn.npn01         #FUN-F50015 add
   #FUN-C80083--ADD--STR
  #IF g_npn.npn03='4' THEN #MOD-DC0176 mark
   IF g_npn.npn03='4' OR g_npn.npn03='8' THEN  #MOD-DC0176 add
      CALL cl_set_comp_visible('npo09,npo10',TRUE)
   ELSE
      CALL cl_set_comp_visible('npo09,npo10',FALSE) #MOD-DC0176 .->,
   END IF
   #FUN-C80083--ADD--END
...
FUNCTION t250_g_b1()                       #由應付票據產生單身
   ...
   DEFINE l_nmh41 LIKE nmh_file.nmh41  #CHI-D90023
   DEFINE l_oma55 LIKE oma_file.oma55  #CHI-D90023
   ...
   #--MOD-E70101 add start--
      IF g_npn.npn03 = '1' THEN
         LET g_sql = g_sql CLIPPED," AND nmh24 = 3 "
      END IF
   #--MOD-E70101 add end--
  #IF g_npn.npn03 MATCHES '[1235]' THEN           #NO.FUN-B40003  #TQC-B70197 add 2 #FUN-C80083 del 4 #MOD-C90252 mark
  #IF g_npn.npn03 MATCHES '[123]' THEN            #MOD-C90252 del 5   #MOD-E70101 mark
   IF g_npn.npn03 MATCHES '[23]' THEN                                 #MOD-E70101 add
  ...
  #-----------------MOD-C90252---------------(E)
   ...
   FOREACH t250_b_c INTO g_nmh.*
      ...
      END IF
     #-CHI-D90023-str--
      IF g_npn.npn03 = '6' THEN
         LET l_nmh41 = ' '
         SELECT nmh41 INTO l_nmh41 FROM nmh_file
          WHERE nmh01 = g_nmh.nmh01
         IF l_nmh41 = 'Y' THEN
            LET l_oma55 = 0
            SELECT oma55 INTO l_oma55 FROM oma_file
             WHERE oma01 = g_nmh.nmh01
            IF l_oma55 > 0 THEN
               CONTINUE FOREACH
            END IF
         END IF
      ...
      BEFORE INPUT
        ...
        AFTER FIELD npn03
           IF NOT cl_null(g_npn.npn03) THEN
              IF g_npn.npn03 NOT MATCHES "[123456789]" THEN      #NO.FUN-B40003  #TQC-B70197 add 2 #FUN-C70129 add 9 #FUN-940114 add 1 
                 NEXT FIELD npn03
              END IF
           END IF
           #FUN-C70129--add--str
...
#    l_qty           LIKE ima_file.ima26,    #No.FUN-680107 DECIMAL(15,3) #FUN-A20044
    ...
    l_nmh41         LIKE nmh_file.nmh41,    #CHI-D90023
    l_oma55         LIKE oma_file.oma55     #CHI-D90023
DEFINE l_nmydmy3    LIKE nmy_file.nmydmy3   #MOD-G40089 add 
   ...
   #No.TQC-C30121   ---start---   Add
   #IF g_npn.npn03 = '5' THEN             #MOD-E30106 mark
    IF g_npn.npn03 MATCHES '[567]' THEN   #MOD-E30106 add
   ...
   #No.TQC-C30121   ---end---     Add
    ...
    INPUT ARRAY g_npo WITHOUT DEFAULTS FROM s_npo.*
          ...
          #-CHI-D90023-str--
           IF g_npn.npn03='6' THEN
              ...
              IF l_nmh41 = 'Y' THEN
                 LET l_oma55 = 0
                 SELECT oma55 INTO l_oma55 FROM oma_file
                  WHERE oma01 = g_npo[l_ac].npo03
                 IF l_oma55 > 0 THEN
                    CALL cl_err(g_npo[l_ac].npo03,'aco-228',1)
                    NEXT FIELD npo03
                 END IF
              END IF
           END IF
        ...
        AFTER FIELD npo10
           ...
           ELSE
              ...
              CALL cl_digcut(g_npo[l_ac].npo09,g_azi04) RETURNING g_npo[l_ac].npo09
              SELECT azi04 INTO t_azi04 FROM azi_file       #MOD-D70200
               WHERE azi01 = g_npn.npn04                    #MOD-D70200
             #CALL cl_digcut(g_npo[l_ac].npo10,g_azi04) RETURNING g_npo[l_ac].npo10   #MOD-D70200 mark
              CALL cl_digcut(g_npo[l_ac].npo10,t_azi04) RETURNING g_npo[l_ac].npo10   #MOD-D70200
                 ...
                 CASE
                   #WHEN g_npn.npn03 MATCHES '[2345]'                                    #NO.FUN-B40003  #TQC-B70197 #MOD-C80059 mark
                    WHEN g_npn.npn03 MATCHES '[234]'                                     #MOD-C80059 add
                     ...
                     #-CHI-D90023-mark--
                      #-CHI-D90023-str--
                       CALL q_nmh(FALSE,TRUE,g_npo[l_ac].npo03,g_npn.npn04) RETURNING g_npo[l_ac].npo03
                      #-CHI-D90023-end--
    ...
    #--MOD-G40089 add start--
   #IF g_t1 IS NOT NULL THEN                  #MOD-G90086 mark
    IF NOT cl_null(g_npn.npn01) THEN          #MOD-G90086 add
       LET g_t1 = s_get_doc_no(g_npn.npn01)   #MOD-G90086 add 
       SELECT nmydmy3 INTO l_nmydmy3 
         FROM nmy_file 
        WHERE nmyslip = g_t1
       IF SQLCA.sqlcode THEN
          CALL s_errmsg('nmyslip',g_t1,'sel nmy:',STATUS,0)
       END IF
       IF l_nmydmy3 = 'Y' THEN   #是否拋轉傳票
          IF cl_confirm('axr-309') THEN
             CALL s_t250_gl(g_npn.npn01,'0')    
   ...
   CASE
     ...
     #WHEN g_npo[l_ac].nmh05 < g_npn.npn02                                   #MOD-C80059 mark
      WHEN g_npo[l_ac].nmh05 < g_npn.npn02 AND g_npn.npn03 MATCHES '[5]'     #MOD-C80059 add
         LET g_errno = 'anm-664'
      #TQC-C50107--add--end
       WHEN g_npn.npn03 matches '[35]' AND l_nmh24 <> 1   #MOD-580071 #FUN-C80083 del--4   #FUN-940114 del 1
         LET g_errno = 'anm-142'
...
FUNCTION t250_r()
    ...
    IF cl_delete() THEN
       ...
       IF SQLCA.sqlcode THEN
          ...
          LET g_success='N'
       END IF
       #INSERT INTO azo_file(azo01,azo02,azo03,azo04,azo05,azo06,azoplant,azolegal)  #FUN-980005 add plant & legal  #FUN-DA0126
       #             VALUES ('anmt250',g_user,g_today,g_msg,g_npn.npn02,'Delete',g_plant,g_legal)  #FUN-DA0126
       INITIALIZE g_npn.* TO NULL
...
FUNCTION t250_v(p_cmd)
   ...
   IF g_npn.npnconf='Y' THEN
      ...
      RETURN
   END IF
  #MOD-GA0008---add---start---
   IF g_nmy.nmydmy3 = 'N' THEN
      CALL cl_err('','mfg9310',0)
      RETURN
   END IF
  #MOD-GA0008---add---end----
...
FUNCTION t250_firm1()
   ...
   DEFINE l_npq07f_t1 LIKE npq_file.npq07f   #MOD-G90086 add
   DEFINE l_npq07_t1  LIKE npq_file.npq07    #MOD-G90086 add
   DEFINE l_npq07f_t2 LIKE npq_file.npq07f   #MOD-G90086 add
   DEFINE l_npq07_t2  LIKE npq_file.npq07    #MOD-G90086 add
   DEFINE l_npp00     LIKE npp_file.npp00    #MOD-G90086 add
   DEFINE l_npp011    LIKE npp_file.npp011   #MOD-G90086 add
...
#  IF NOT cl_confirm('axm-108') THEN RETURN END IF  #CHI-C30107 mark
   ...
   IF g_nmy.nmydmy3 = 'Y' AND g_nmy.nmyglcr = 'N' THEN
     ...
     LET l_npq07f_t2 = 0
     LET l_npq07_t2 = 0
     SELECT SUM(npq07f),SUM(npq07) INTO l_npq07f_t1,l_npq07_t1
       FROM npq_file
      WHERE npqsys  = 'NM'
        AND npq00   = l_npp00 
     ...
     SELECT SUM(npq07f),SUM(npq07) INTO l_npq07f_t2,l_npq07_t2
       FROM npq_file
      WHERE npqsys  = 'NM'
        AND npq00   = l_npp00 
        AND npq01   = g_npn.npn01
     ...
     IF cl_null(l_npq07f_t1) THEN LET l_npq07f_t1 = 0 END IF
     IF cl_null(l_npq07_t1) THEN LET l_npq07_t1 = 0 END IF
     IF cl_null(l_npq07f_t2) THEN LET l_npq07f_t2 = 0 END IF
     IF cl_null(l_npq07_t2) THEN LET l_npq07_t2 = 0 END IF

     IF l_npq07f_t1 != g_npn.npn10 OR l_npq07f_t2 != g_npn.npn10 THEN
        CALL cl_err('npq07f','aap-065',1)
        LET g_success='N'
        RETURN
     END IF
     IF l_npq07_t1 != g_npn.npn11 OR l_npq07_t2 != g_npn.npn11 THEN
        CALL cl_err('npq07','aap-065',1)
        LET g_success='N'
        RETURN
     END IF
     #--MOD-G90086 add end--
   ...
   IF g_nmy.nmydmy3 = 'Y' AND g_nmy.nmyglcr = 'Y' AND g_success = 'Y' THEN
     ...
     #LET g_str="anmp400 '",g_wc_gl CLIPPED,"' '0' 'z' '",g_nmz.nmz02p,"' ",               #MOD-DA0176 mark
      LET g_str="anmp400 '",g_wc_gl CLIPPED,"' '",g_npn.npnuser CLIPPED,"' '",g_npn.npnuser CLIPPED,"' '",g_nmz.nmz02p,"' ", #MOD-DA0176 add
      ...
      IF g_npn.npn03 MATCHES '[6]' THEN
        ...
        #-CHI-D90023--str--
         ...
         IF l_nmh41 = 'Y' THEN
            LET l_oma55 = 0
            SELECT oma55 INTO l_oma55 FROM oma_file
             WHERE oma01 = m_npo.npo03
            IF l_oma55 > 0 THEN
               CALL s_errmsg('npo03',m_npo.npo03,m_npo.npo03,'aco-228',1)
               LET g_success='N'
               CONTINUE FOREACH
            ELSE
               UPDATE oma_file SET oma55 = oma54t,
                                   oma57 = oma56t,
                                   oma61 = 0
                WHERE oma01 = m_npo.npo03
               IF SQLCA.sqlcode THEN
                  CALL s_errmsg('oma01',m_npo.npo03,'upd oma',STATUS,1) 
                  LET g_success='N'
                  CONTINUE FOREACH
               END IF
               UPDATE omc_file SET omc10 = omc08,
                                   omc11 = omc09,
                                   omc13 = 0
                WHERE omc01 = m_npo.npo03
               IF SQLCA.sqlcode THEN
                  CALL s_errmsg('omc01',m_npo.npo03,'upd omc',STATUS,1) 
                  LET g_success='N'
                  CONTINUE FOREACH
               END IF
            END IF
...
FUNCTION t250_firm2()
   ...
   END IF
  #---------------MOD-C80263---------------------(S)
   IF g_npn.npn03 MATCHES '[5678]' THEN              #MOD-D50227 add 5
...
FUNCTION t250_z1()
   ...
   FOREACH t250_dcs2 INTO m_npo.*,l_nmh24
      ...
      IF l_cnt > 0 THEN
         ...
         CONTINUE FOREACH 
      END IF
      IF g_npn.npn03 <> '0' THEN   #FUN-940114 add
...
#--end-- FUN-B40011
     ...
     #-CHI-D90023-str-- 
      IF g_npn.npn03 = '6' THEN
         UPDATE oma_file SET oma55 = oma55 - oma54t,
                             oma57 = oma57 - oma56t,
                             oma61 = oma56t
          WHERE oma01 = m_npo.npo03
         IF SQLCA.sqlcode THEN
            ...
            CONTINUE FOREACH
         END IF
         UPDATE omc_file SET omc10 = omc10 - omc08,
                             omc11 = omc11 - omc09,
                             omc13 = omc09
          WHERE omc01 = m_npo.npo03
         IF SQLCA.sqlcode THEN
...
FUNCTION t250_x(p_type)                  #FUN-D20035
   ...
   IF g_npn.npnconf='X' AND NOT cl_null(g_nmz.nmz10) AND g_npn.npn02<= g_nmz.nmz10 THEN
   ...
   IF SQLCA.sqlcode THEN
      ...
      RETURN
   END IF
  # Prog. Version..: '5.30.27-19.04.12(0,0,g_npn.npnconf) THEN              #FUN-D20035
...
#No:FUN-C60006---add--star---
      LET g_npn.npnmodu = g_user
      LET g_npn.npndate = g_today
      DISPLAY BY NAME g_npn.npnmodu
      DISPLAY BY NAME g_npn.npndate
...
#FUN-B30166--add--end
 ...
 END IF  
   ...
   #FUN-C80083--ADD--STR
  #IF g_aza.aza26='2' AND g_npn.npn03 = '4' THEN #MOD-DC0176 mark
   IF g_npn.npn03 = '4' OR  g_npn.npn03 = '8' THEN #MOD-DC0176 add
...
FUNCTION t250_upd_nmh(p_sw)
   ...
   IF p_sw='+' THEN   #確認
      CASE
         ...
         #--FUN-940114 add start--
         WHEN g_npn.npn03 = '1'
            UPDATE nmh_file SET nmh24 = '1',nmh19 = ''   #MOD-E70101 add nmh19 
             WHERE nmh01 = m_npo.npo03   
         #--FUN-940114 add end--
      END CASE
   ELSE      #取消確認
      CASE
        ...
        #FUN-C80083--ADD--STR
         ...
         #--FUN-940114 add start--
         WHEN g_npn.npn03 = '1'
              UPDATE nmh_file SET nmh24 = '3',nmh19 = '3'   #MOD-E70101 add
               WHERE nmh01=m_npo.npo03
         #--FUN-940114 add end--
      END CASE
   END IF
...
FUNCTION t250_t()
   ...
   IF s_anmshut(0) THEN RETURN END IF
  #-MOD-E10025-str--
   LET g_wc = " npn01 = '",g_npn.npn01,"' AND npn02 = '",g_npn.npn02,"'"  
   #資料權限的檢查
  ...
  #-MOD-AC0019-end-
   ...
   IF INT_FLAG THEN
      ...
      RETURN
   END IF
  #LET g_wc = " npn01 = '",g_npn.npn01,"' AND npn02 = '",g_npn.npn02,"'"  #MOD-E10025 mark  #MOD-AC0019
   #資料權限的檢查
 
 
  #LET g_wc  = g_wc CLIPPED," AND npnconf='Y' "   # 已確認                #MOD-E10025 mark
...
#FUN-D70122 add ------- begin#获取帐套
         ...
         END IF
         LET g_oma.oma173 = s_get_aznn(g_plant,g_bookno1,g_today,1)
         LET g_oma.oma174 = s_get_aznn(g_plant,g_bookno1,g_today,3)
#FUN-D70122 add ------- end
#        LET g_oma.oma173 = YEAR(g_today)   #FUN-D70122 mark
#        LET g_oma.oma174 = MONTH(g_today)  #FUN-D70122 mark
         LET g_oma.oma18 = l_ool.ool13      #MOD-E10025
         IF cl_null(g_oma.oma18) THEN       #MOD-E10025
        ...
        #-MOD-E10025-str--
         IF g_aza.aza63 = 'Y' THEN  
            LET g_oma.oma181 = l_ool.ool131 
            IF cl_null(g_oma.oma181) THEN  
               SELECT oca04 INTO g_oma.oma181 FROM occ_file,oca_file
                WHERE occ01=g_oma.oma03 AND occ03=oca01
            END IF                        
         END IF
        #-MOD-E10025-end--
...
#No.FUN-AB0034 --end
   ...
   IF g_success = 'Y' THEN
      ...
      END IF
   ELSE
     #IF g_count > 0 THEN             #MOD-E10025 mark
     #   CALL cl_err('','anm-997',0)  #MOD-E10025 mark
#        MESSAGE '應收帳款已存在,無法重復產生 !!'
     #ELSE                            #MOD-E10025 mark
      IF g_cnt=0 THEN
         CALL cl_err('','axr-289',1)
      ELSE
   ...
   FORMAT
      AFTER GROUP OF p_oma.oma03
        ...
        #LET p_oma.oma58 = l_oma58               #MOD-AC0104 mark 
         ...
         LET p_oma.oma64 = '1'   #MOD-F40057 add
        ...
        END IF
         ...
         LET p_oma.oma70 = '1'   #MOD-F40057 add 
...
FUNCTION t250_t2()
   ...
   IF cl_confirm('anm-132') THEN     #MOD-G60001 add
      BEGIN WORK
      LET g_success='Y'
      ...
      IF g_success='Y' THEN
         ...
         COMMIT WORK
      ELSE
         CALL cl_err('','mfg1604',0)    #MOD-G60001 add
         ROLLBACK WORK
      END IF
   END IF                               #MOD-G60001 add
END FUNCTION
...
FUNCTION t250_carry_voucher()
   ...
   DEFINE l_n          LIKE type_file.num5     #No.FUN-680107 SMALLINT
    ...
    END IF
     ...
     LET g_wc_gl = 'npp01 = "',g_npn.npn01,'" AND npp011 = ',g_npn.npn03
    #LET g_str="anmp400 '",g_wc_gl CLIPPED,"' '0' 'z' '",g_nmz.nmz02p,"' ",     #MOD-DA0176 mark
     LET g_str="anmp400 '",g_wc_gl CLIPPED,"' '",g_npn.npnuser CLIPPED,"' '",g_npn.npnuser CLIPPED,"' '",g_nmz.nmz02p,"' ",     #MOD-DA0176
...
#FUN-D70122 add ------- begin#获取帐套
         ...
         IF g_flag = '1' THEN
            CALL cl_err(YEAR(g_today),'aoo-081',1)
         END IF
         LET g_apa.apa173 = s_get_aznn(g_plant,g_bookno1,g_today,1)
         LET g_apa.apa174 = s_get_aznn(g_plant,g_bookno1,g_today,3)
#FUN-D70122 add ------- end
#        LET g_apa.apa173=YEAR(g_today)    #FUN-D70122 mark
#        LET g_apa.apa174=MONTH(g_today)   #FUN-D70122 mark
