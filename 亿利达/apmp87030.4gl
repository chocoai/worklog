...
# Modify.........: No:FUN-CC0004 13/05/23 By jt_chen 拋轉還原刪除來源站的apk_file記錄
# Modify.........: No:MOD-DB0031 13/11/06 By Vampire 單據已分攤，故不可還原!
# Modify.........: No:MOD-DB0155 13/11/25 By Vampire 修正 MOD-DB0031 SQL 語法。
# Modify.........: No:FUN-E80005 14/08/11 By liliwen 營運中心屬於大陸版，而rvw_file有資料,清空rvw18帳款編號
# Modify.........: No:MOD-E80054 14/08/12 By liliwen 刪除apk_file時,條件只需對應到apk01
# Modify.........: No:MOD-E80162 14/08/28 By Summer 續MOD-E80054,若多筆項次,第二筆後會刪不到資料
# Modify.........: No:FUN-EA0009 15/06/16 By Alberti 畫面新增日期(rvu03)，部門(rvu06)，單號(rvu01)改為QBE
# Modify.........: No:MOD-F90068 15/09/14 By fionchen 修正FUN-EA0009錯誤,因為會造成拋轉還原時沒刪到npq_file 
# Modify.........: No:MOD-G60010 16/06/04 By catmoon MOD-E80162未考慮多張發票號碼(apb11)的情形
...
#DEFINE g_argv1  LIKE rvu_file.rvu01         #FUN-EA0009 mark
DEFINE g_argv1  STRING                       #FUN-EA0009 add
DEFINE g_oga01  LIKE oga_file.oga01
DEFINE g_rvu01  LIKE rvu_file.rvu01
DEFINE g_apa01  LIKE apa_file.apa01
DEFINE g_apa08  LIKE apa_file.apa08          #FUN-E80005
...
DEFINE p_aza26       LIKE aza_file.aza26     #FUN-E80005
...
DEFINE g_wc         STRING                   #FUN-EA0009 add
...
MAIN
  ...
  #LET tm.rvu01     = ARG_VAL(1)  #FUN-EA0009 mark
   LET g_wc         = ARG_VAL(1)  #FUN-EA0009 add
...
FUNCTION p870_p1()
 ...
 WHILE TRUE
    ...
    #INPUT BY NAME tm.rvu00,tm.rvu01,g_bgjob  WITHOUT DEFAULTS   #NO.FUN-570138  #FUN-EA0009 mark
     INPUT BY NAME tm.rvu00,g_bgjob  WITHOUT DEFAULTS                            #FUN-EA0009 add
        #no.7176
        AFTER FIELD rvu00
...
#NO.FUN-570138 start---
   ...
   IF g_bgjob = "Y" THEN
      ...
      ELSE
         LET g_wc=cl_replace_str(g_wc, "'", "\"")  #FUN-EA0009 Add
         LET lc_cmd = lc_cmd CLIPPED,
                     #" '",tm.rvu01 CLIPPED,"'",   #FUN-EA0009 mark
                      " '",g_wc CLIPPED ,"'",      #FUN-EA0009 add
                      " '",tm.rvu00 CLIPPED,"'",
                      " '",g_bgjob CLIPPED,"'"
...
FUNCTION p870_p2()
  ...
  DEFINE l_cnt  LIKE type_file.num5     #MOD-DB0031 add 
  DEFINE l_cnt1 LIKE type_file.num5     #FUN-EA0009 add
  DEFINE l_rvv87 LIKE rvv_file.rvv87    #FUN-EA0009 add
  DEFINE l_rvv23 LIKE rvv_file.rvv23    #FUN-EA0009 add
 ...
 #FUN-EA0009-Start-Add 
 #整段改成可用多筆資料
 #參考MOD-A20031(apmp860)補上條件需要有apb
  LET l_sql="SELECT * FROM rvu_file,rva_file ",
            " WHERE rvu02 = rva01 ",
             " AND rvu08  ='TAP' ",
             " AND rvu20  ='Y' ",
             " AND rvuconf='Y' ",     #已確認之入庫單
             " AND rvu00  ='", tm.rvu00, "'",
             " AND rvu99 IS NOT NULL ",                             
             " AND rvu01 IN( SELECT apb21 ",                    
  ...
  FOREACH p870_curs1 INTO g_rvu.*,g_rva.*
     IF SQLCA.SQLCODE THEN
        CALL cl_err('foreach p870_curs1',SQLCA.SQLCODE,1) 
        LET g_success = 'N'
        EXIT FOREACH
    ...
    #已請款匹配量
    #axm-994
     SELECT SUM(rvv23),SUM(rvv87) 
       INTO l_rvv23,l_rvv87 FROM rvv_file  
      WHERE rvv01 = g_rvu.rvu01 
     IF cl_null(l_rvv23) THEN LET l_rvv23 = 0 END IF
     IF cl_null(l_rvv87) THEN LET l_rvv87 = 0 END IF
     
     IF l_rvv23 = 0 AND l_rvv87<>0 THEN  
        CONTINUE FOREACH    
     END IF

    #算筆數，如果l_cnt=0 表示無符合資料(參考apmp860)  
     LET l_cnt1 = l_cnt1 +1               
#FUN-EA0009-End-Add
...
#        CALL cl_err(g_pmm.pmm904,'axm-318',1)   #No.FUN-660129
     ...
     FOR i = 0 TO p_last
        ...
        #INITIALIZE p_apy TO NULL       #MOD-F90068 mark
        ...
        #FUN-EA0009-End-Add
         ...
         #FUN-E80005 start
         LET l_sql = "SELECT aza26 ",
                     " FROM ",cl_get_target_table(l_plant_new,'aza_file'), 
                     " WHERE aza01 = '0' "
     ...
     CALL cl_parse_qry_sql(l_sql,l_plant_new) RETURNING l_sql #FUN-A50102
            ...
            #No.FUN-710030--End--
            END IF
           #MOD-DB0031 add start -----
            LET l_cnt = 0
           #MOD-DB0155 mark start -----
           #LET l_sql = "SELECT COUNT(*) FROM ",cl_get_target_table(l_plant_new,'apa_file'),
           #            "                    ,",cl_get_target_table(l_plant_new,'apc_file'),
           #            " WHERE apc01 = apa01 AND aqaconf<> 'X' AND aqc02 = '",g_apa01,"'"
           #MOD-DB0155 mark end   -----
           #MOD-DB0155 add start ----
            LET l_sql = "SELECT COUNT(*) FROM ",cl_get_target_table(l_plant_new,'aqa_file'),
                        "                    ,",cl_get_target_table(l_plant_new,'aqc_file'),
                        " WHERE aqc01 = aqa01 AND aqaconf<> 'X' AND aqc02 = '",g_apa01,"'"
           #MOD-DB0155 add end   ----
 	    CALL cl_replace_sqldb(l_sql) RETURNING l_sql 
            CALL cl_parse_qry_sql(l_sql,l_plant_new) RETURNING l_sql 
            PREPARE apa_pre1 FROM l_sql
           #DECLARE apa_cs1 CURSOR FOR apa_pre  #MOD-DB0155 mark
            DECLARE apa_cs1 CURSOR FOR apa_pre1 #MOD-DB0155 add
            OPEN apa_cs1 
            FETCH apa_cs1 INTO l_cnt
            IF SQLCA.SQLCODE <> 0 OR l_cnt IS NULL THEN
               LET l_cnt = 0
            END IF

            IF l_cnt > 0 THEN
               CALL s_errmsg('','apa01:',g_apa01,'axm1210',1)
               LET g_success='N' CONTINUE FOR
            END IF
           #MOD-DB0031 add end   -----
         END IF
...
FUNCTION p870_pro(l_plant,p_dbs,i)  #FUN-A50102
 ...
 DEFINE l_apb11  LIKE apb_file.apb11     #FUN-CC0004 add
     ...
     IF i != p_last OR     ##最後一家工廠不必拋AP
        ...
        #FUN-E80005 start
        LET l_sql="UPDATE ",cl_get_target_table(l_plant,'rvw_file'),
                  " SET rvw18 = '' ",
                  " WHERE rvw01 = ? AND rvw08||rvw09 IN ",
                  " (SELECT apb21||apb22 FROM ",
                     cl_get_target_table(l_plant,'apb_file'),
                     " WHERE apb01 = ?)"
        CALL cl_replace_sqldb(l_sql) RETURNING l_sql
        CALL cl_parse_qry_sql(l_sql,l_plant) RETURNING l_sql
        PREPARE upd_rvw18 FROM l_sql
        EXECUTE upd_rvw18 USING g_apa08,g_apa01
        IF SQLCA.sqlcode<>0 THEN
           CALL s_errmsg('','','upd rvw18',SQLCA.SQLCODE,1)
           LET g_success = 'N'
        END IF
        #FUN-E80005 end
        ...
        IF i = 0 THEN #來源站
          #LET l_sql="SELECT apb02,apb11 FROM ",cl_get_target_table(l_plant,'apb_file'),    #MOD-E80162 mark
           LET l_sql="SELECT DISTINCT apb11 FROM ",cl_get_target_table(l_plant,'apb_file'), #MOD-E80162
                     " WHERE apb01 = ? "
           CALL cl_replace_sqldb(l_sql) RETURNING l_sql
           CALL cl_parse_qry_sql(l_sql,l_plant) RETURNING l_sql
           PREPARE sel_apb11 FROM l_sql
           DECLARE apb11_cs CURSOR FOR sel_apb11
          #FOREACH apb11_cs USING g_apa01 INTO l_apb02,l_apb11  #MOD-E80162 mark
           FOREACH apb11_cs USING g_apa01 INTO l_apb11          #MOD-E80162
              IF NOT cl_null(l_apb11) THEN
                 LET l_sql="DELETE FROM ",cl_get_target_table(l_plant,'apk_file'),
                           " WHERE apk01 = ? "
                          #"   AND apk02 = ? ", #MOD-E80054 mark
                          ,"   AND apk03 = ? "  #MOD-E80054 mark  #MOD-G60010 remark
                 CALL cl_replace_sqldb(l_sql) RETURNING l_sql
                 CALL cl_parse_qry_sql(l_sql,l_plant) RETURNING l_sql
                 PREPARE del_apk FROM l_sql
                #EXECUTE del_apk USING g_apa01,l_apb02,l_apb11   #MOD-E80054 mark
                #EXECUTE del_apk USING g_apa01                   #MOD-E80054 add  #MOD-G60010 mark
                 EXECUTE del_apk USING g_apa01,l_apb11           #MOD-G60010 add
                 IF SQLCA.SQLCODE <> 0 OR SQLCA.SQLERRD[3]=0 THEN
                    CALL s_errmsg('','','del apk:',SQLCA.SQLCODE,1)
                    LET g_success='N'
                 END IF
              END IF
           END FOREACH
  ...
  DEFINE i     LIKE type_file.num5          #No.FUN-680136 SMALLINT
 	 ...
 	 CALL cl_replace_sqldb(l_sql) RETURNING l_sql        #FUN-920032
         CALL cl_parse_qry_sql(l_sql,l_plant_new) RETURNING l_sql #FUN-980092
            ...
            #LET l_sql = " SELECT apa01 FROM ",cl_get_target_table(l_plant_new,'apa_file'),      #FUN-A50102 #FUN-E80005 mark
            LET l_sql = " SELECT apa01,apa08 FROM ",cl_get_target_table(l_plant_new,'apa_file'), #FUN-A50102 #FUN-E80005 add
     ...
     CALL cl_parse_qry_sql(l_sql,l_plant_new) RETURNING l_sql #FUN-A50102
            ...
            OPEN apa01_cs 
           #FETCH apa01_cs INTO g_apa01                              #付款單 #FUN-E80005 mark
            FETCH apa01_cs INTO g_apa01,g_apa08                      #付款單 #FUN-E80005 add
            IF SQLCA.SQLCODE THEN
  ...
  DEFINE i     LIKE type_file.num5          #No.FUN-680136 SMALLINT
 	 ...
 	 CALL cl_replace_sqldb(l_sql) RETURNING l_sql        #FUN-920032
         CALL cl_parse_qry_sql(l_sql,l_plant_new) RETURNING l_sql #FUN-A50102
            ...
            OPEN apa01t_cs 
           #FETCH apa01t_cs INTO g_apa01                              #付款單 #FUN-E80005 mark
            FETCH apa01t_cs INTO g_apa01,g_apa08                      #付款單 #FUN-E80005 add
            IF SQLCA.SQLCODE THEN
