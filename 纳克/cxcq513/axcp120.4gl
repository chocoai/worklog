...
#+ Version..: T100-ERP-1.01.00(SD版次:13,PR版次:13) Build-000239
#+ 
#+ Filename...: axcp120
#+ Description: 成本階計算作業
#+ Creator....: 02291(2014-03-20 13:43:47)
#+ Modifier...: 00537(2015-07-05 16:36:18) -SD/PR- 00537(2016-01-06 10:07:09)
...
{<section id="axcp120.other_function" >}
   ...
   END IF
    ...
    IF g_master.order = '0' THEN       #依BOM低階碼
       ...
       IF SQLCA.sqlcode THEN
         ...
         LET g_errparam.extend = 'INS axcp120_xcbb_tmp'
         CALL cl_err() 
         RETURN
       END IF
       ...
       IF SQLCA.sqlcode THEN
         ...
         INITIALIZE g_errparam TO NULL
         LET g_errparam.code = SQLCA.sqlcode
         LET g_errparam.extend = 'UPD axcp120_xcbb_tmp'
         CALL cl_err() 
         RETURN
       END IF
...
#wujie 150731 --begin #order选项1，2 
    IF g_master.order ='1' OR g_master.order ='2' THEN       #依發料低階碼or发料+特性 
       IF NOT axcp120_gen_imac() THEN   #依发料计算低阶码，放入类似imac的临时表中   #APS改好的话这个if可以去掉了
         ...
         INITIALIZE g_errparam TO NULL
         LET g_errparam.code = SQLCA.sqlcode
         LET g_errparam.extend = 'INS axcp120_ins_xcbb_t'
         CALL cl_err() 
          RETURN
       END IF 
#把发料低阶码更新到每个料号
...
#按bom取出料件
       LET l_sql = " INSERT INTO axcp120_xcbb_tmp1 ",
                   " SELECT DISTINCT imagent,imagsite,",g_master.year,",",g_master.month,",imag001,NVL(inaj006,' '),imaa006,imag013,NVL(imac003,99),0,NVL(imac003,99),'N',imag011,imaa003,imaa004",   #160106-00003 imac003 -->NVL
...
#                   "   AND imaa001 = imac001",   #160106-00003 mark  
                   ...
                   "   AND imagsite = '",g_master.imagsite,"'",
...
#把前面发料低阶码计算出来的结果做对比，不存在的料件+特性就添加进去                   
       ...
       IF SQLCA.sqlcode THEN
         ...
         INITIALIZE g_errparam TO NULL
         LET g_errparam.code = SQLCA.sqlcode
         LET g_errparam.extend = 'MERGE INTO axcp120_xcbb_tmp'
         CALL cl_err() 
         RETURN
       END IF
...
#order选项0的情况，最后根据参数，启用特性的，关联inaj去找料件的特性
    IF cl_get_para(g_enterprise,g_master.imagsite,'S-FIN-6013') = 'Y' AND (g_master.order = '0') THEN   
       ...
       END IF 
    END IF
    UPDATE axcp120_xcbb_tmp1 SET xcbb004 = ' ' WHERE xcbb004 IS NULL
    UPDATE axcp120_xcbb_tmp SET xcbb004 = ' ' WHERE xcbb004 IS NULL
#wujie 150731 --end        
#按BOM和按发料都不需要抓特性
...
#运算临时表
   DROP TABLE axcp120_imaa_tmp2     
   CREATE TEMP TABLE axcp120_imaa_tmp2     
   (
...
#wujie 150801 --begin sfaa -->sfac
   ...
   INSERT INTO sfac_tmp 
        SELECT sfac_t.*
          FROM sfac_t,sfaa_t,axcp120_xcbb_tmp
...
#               "                       AND sfaadocno = sfec001)"
   ...
   IF g_master.order = '2' THEN #order是发料+特性的，需要考虑特性条件
      ...
      LET l_where2 = " AND sfac006 <> xcbb004 "
   ELSE
      LET l_where1 = " "
      LET l_where2 = " "
   END IF
...
#填充axcp120_sfaa_tmp
   ...
   SELECT DISTINCT sfac001,NVL(sfac006,' '),sfdd001,NVL(sfdd013,' ') FROM sfda_t,sfdd_t,sfdc_t,sfac_t
       WHERE sfdaent   = sfdcent 
         AND sfdaent   = sfacent
         AND sfdaent   = sfddent
         AND sfdasite  = sfdcsite 
...
#         AND (sfaa048 IS  NULL OR sfaa048 < l_bdate)    #已做成本结案的工单排除掉
   ...
   IF SQLCA.sqlcode THEN
      ...
      INITIALIZE g_errparam TO NULL
      LET g_errparam.code = SQLCA.sqlcode
      LET g_errparam.extend ='INS axcp120_sfaa_tmp'
      CALL cl_err()  
      RETURN r_success
   END IF
...
#不使用特性时，特性不参与计算，upd成空格  
   ...
   (
   SELECT DISTINCT sfaa010 imaa001,",l_field1," xcbb004 FROM axcp120_sfaa_tmp
   UNION
   SELECT DISTINCT sfdd001,",l_field2," FROM axcp120_sfaa_tmp
   )"
   ...
   IF SQLCA.sqlcode THEN
      ...
      INITIALIZE g_errparam TO NULL
      LET g_errparam.code = SQLCA.sqlcode
      LET g_errparam.extend ='INS axcp120_imaa_tmp'
      CALL cl_err()  
      RETURN r_success
   END IF
...
#剔除死循环
   LET l_sql="SELECT DISTINCT X1||Y1 FROM  ",                                
   					"( ",                                                               
   					"  SELECT * FROM " ,                                                
   					"  (SELECT sfaa010||sfaa011 x1,sfdd001||sfdd013 y1 FROM axcp120_sfaa_tmp ), ",      
   					"  (SELECT sfaa010||sfaa011 x2,sfdd001||sfdd013 y2 FROM axcp120_sfaa_tmp ) ",       
  					...
  					"SELECT DISTINCT X2||Y2 FROM  ",                                
   					...
   					"  WHERE x1=y2 AND y1=x2  ", 
 					") "   
   LET l_sql1 = "SELECT sfaa010,sfaa011 FROM axcp120_sfaa_tmp WHERE sfaa010||sfaa011||sfdd001||sfdd013 IN ( ",l_sql," ) " 	#这个sql是用来输出死循环的料号的	
   ...
   LET l_sql="DELETE FROM axcp120_sfaa_tmp WHERE sfaa010||sfaa011||sfdd001||sfdd013 IN ( ",l_sql," ) "  
   ...
   IF SQLCA.sqlcode THEN
      ...
      INITIALIZE g_errparam TO NULL
      LET g_errparam.code = SQLCA.sqlcode
      LET g_errparam.extend ='DEL FROM axcp120_sfaa_tmp'
      CALL cl_err()
      RETURN r_success
   END IF
...
#        END IF
        ...
        INSERT INTO axcp120_imaa_tmp1 
         SELECT imaa001,xcbb004 FROM axcp120_imaa_tmp
          WHERE NOT EXISTS ( SELECT 1 FROM axcp120_sfaa_tmp WHERE sfdd001 = imaa001 AND sfdd013 = xcbb004 )
            AND EXISTS ( SELECT 1 FROM axcp120_sfaa_tmp WHERE sfaa010 = imaa001 AND sfaa011 = xcbb004)          
        IF SQLCA.sqlcode THEN
           LET r_success = FALSE
           INITIALIZE g_errparam TO NULL
           LET g_errparam.code = SQLCA.sqlcode
           LET g_errparam.extend ='INS axcp120_imaa_tmp1'
           CALL cl_err()
           RETURN r_success
        END IF
...
#死循环的应该是既有单头又有单身的            
              LET l_sql = "SELECT imaa001,xcbb004 FROM axcp120_imaa_tmp ",
                          " WHERE EXISTS ( SELECT 1 FROM axcp120_sfaa_tmp WHERE sfdd001 = imaa001 AND sfdd013 = xcbb004 )",
                          "   AND EXISTS ( SELECT 1 FROM axcp120_sfaa_tmp WHERE sfaa010 = imaa001 AND sfaa011 = xcbb004)"
...
#没有单身的元件取当前跑到的低阶码              
              LET l_sql = "INSERT INTO axcp120_ins_xcbb_t ",
                          " SELECT DISTINCT imaa001,xcbb004,",l_level," FROM axcp120_imaa_tmp",
                          " WHERE NOT EXISTS ( SELECT 1 FROM axcp120_imaa_tmp2 WHERE imaa001 = imaa0012 AND xcbb004 = xcbb0042) "
...
#只有单身的，低价码+1              
              LET l_sql = "INSERT INTO axcp120_ins_xcbb_t ",
                          " SELECT DISTINCT imaa0012,xcbb0042,",l_level,"+1 FROM axcp120_imaa_tmp2"
              ...
              IF SQLCA.sqlcode THEN
                 ...
                 INITIALIZE g_errparam TO NULL
                 LET g_errparam.code = SQLCA.sqlcode
                 LET g_errparam.extend ='INS axcp120_ins_xcbb_t1'
                 CALL cl_err()
                 RETURN r_success
              END IF
           END IF
        ...
        IF SQLCA.sqlcode THEN
           ...
           INITIALIZE g_errparam TO NULL
           LET g_errparam.code = SQLCA.sqlcode
           LET g_errparam.extend ='MERGE axcp120_ins_xcbb_t'
           CALL cl_err()
           RETURN r_success
        END IF

        #把这些料号从基准表中剔除掉，同时从关系表中剔除掉以其为父料件的关系记录
        DELETE FROM axcp120_imaa_tmp WHERE EXISTS (SELECT 1 FROM axcp120_imaa_tmp1 WHERE imaa0011 = imaa001 AND xcbb0041 = xcbb004)
        DELETE FROM axcp120_sfaa_tmp WHERE EXISTS ( SELECT 1 FROM axcp120_imaa_tmp1 WHERE imaa0011 = sfaa010 AND xcbb0041 = sfaa011)
