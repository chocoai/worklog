...
# Modify.........: NO:MOD-E70117 14/07/24 By qiull 修改付款日期之後,tic現金流量明細重複
# Modify.........: NO:qiull140812 14/08/12 By qiull 第二单身类型oob04栏位值为Z:票据转付时，科目oob11栏位的值系统未自动带出
# Modify.........: No:FUN-EC0013 15/04/22 By yihsuan 增加 7.差異視為匯兌損益 處理  # add by lixwz 180322
DATABASE ds
       ...
       ON ACTION print_this_pay_slip
         ...
         LET g_msg = "axrr400 '' '' '",g_lang,"' 'Y' '' '' ",  #FUN-C30085   #radd by pulf 141225
          #LET g_msg = "axrg400 '' '' '",g_lang,"' 'Y' '' '' ",  #FUN-C30085   #TQC-C70127  mark
                      "'ooa01 =\"",g_ooa.ooa01,"\"' 'g_ooa.ooaconf' "         #TQC-C70127  mark #radd by pulf 141225
          #TQC-C70127--add--str--
...
DEFINE l_nmh02         LIKE nmh_file.nmh17   #FUN-C90092
    ...
    IF cl_null(g_wc3) THEN LET g_wc3 = ' 1=1' END IF # add by lixwz 180322
    CALL t410_g_b()                 #先由應收票據或TT自動產生借方單身
    CALL t410_b_fill_2(g_wc3)       #FUN-A90003 Add
    CALL cl_opmsg('b')
    ...
    DECLARE t410_bcl1 CURSOR FROM g_forupd_sql      # LOCK CURSOR
      ...
      INPUT ARRAY g_oob_d WITHOUT DEFAULTS FROM s_oob_d.*
        ...
        AFTER INSERT
           ...
           #IF g_ooz.ooz62 = 'Y' THEN                                   #yinhy140418 mark
           IF g_ooz.ooz62 = 'Y' AND g_oob_d[l_ac2].oob04_1='1' THEN     #yinhy140418
              IF cl_null(g_oob_d[l_ac2].oob15_1) THEN
                 NEXT FIELD oob15_1
              END IF
           END IF
