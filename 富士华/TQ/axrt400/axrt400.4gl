...
# Modify.........: No:MOD-B70242 11/08/27 By yinhy 單身參考單號欄位應可以錄入單據性質為27,28單據
# Modify.........: No:MOD-B70289 11/08/29 By yinhy 審核生成益收款時未對oma25賦值
# Modify.........: No.MOD-B80099 11/08/10 By johung 進單身時，omc_file有多筆才NEXT FIELD oob19
...
# Modify.........: No.MOD-B90075 11/09/09 By wujie 借方为待抵时，取消审核没有更新待抵账款已冲金额
...
# Modify.........: No.TQC-BA0089 11/10/17 By yinhy axrt401不提供打印
...
# Modify.........: No:TQC-C20026 12/02/02 By yinhy axrt400中第二單身oob11_1欄位應顯示科目
...
# Modify.........: No:MOD-C20199 12/02/23 By yinhy 當貸方單身選擇應收賬款單據後，出現報錯axr-193,原因是本幣金額比較沒有考慮oox10
...
# Modify.........: No:MOD-C30743 12/03/15 By yinhy BEFORE FIELD oob19_1段會產生死循環
# Modify.........: No:MOD-C30853 12/03/26 By Polly 貸方類別 9 時,匯率不可修改
# Modify.........: No:MOD-C40139 12/04/18 By Elise 增加判斷,若匯率為 1 時,則原幣與本幣如果不同則提示 agl-926 錯誤訊息
...
#No.FUN-AA0088 --begin
      IF g_prog ='axrt400' THEN
         CALL cl_set_comp_visible("oob24_1,oob25_1,oob26_1,oob27_1,occ02,ooc02",FALSE)
      END IF
...
#No.FUN-AA0088 --end
   ...
   # add by lixwz180201 s
   CALL cl_set_comp_visible("oobud02,oobud02_1",TRUE)
   # add by lixwz180201 e
...
FUNCTION t400_g_b1()
   ...
   DEFINE g_t1             LIKE type_file.chr5       # Prog. Version..: '5.25.08-12.05.10(03)   #MOD-730023
           ...
           AFTER FIELD ooa14
                 IF NOT cl_null(g_ooa.ooa14) THEN
                    ...
                    IF STATUS THEN
                       ...
                       NEXT FIELD ooa14
                    END IF
                    IF cl_null(g_ooa.ooa15) THEN                                  #MOD-BB0086 ADD
                       LET g_ooa.ooa15=g_buf  DISPLAY By NAME g_ooa.ooa15
                    END IF                                                        #MOD-BB0086 ADD
                 END IF
          AFTER FIELD ooa15
...
FUNCTION t400_i(p_cmd)
  ...
  DEFINE l_ooyacti       LIKE ooy_file.ooyacti             #No.TQC-770025
    ...
    INPUT BY NAME g_ooa.ooaoriu,g_ooa.ooaorig,
        ...
        g_ooa.ooa14,g_ooa.ooa15,g_ooa.ooa23,g_ooa.ooa13,
      ...
      INPUT ARRAY g_oob WITHOUT DEFAULTS FROM s_oob.*
        ...
        BEFORE ROW
            ...
            IF g_rec_b>=l_ac THEN
               ...
               # add by lixwz180201 s
               CALL cl_set_comp_entry("oobud02,oobud02_1",TRUE)
               # add by lixwz180201 e
            END IF
                ...
                IF p_cmd= 'a' OR g_oob_t.oob06 != g_oob[l_ac].oob06  THEN
                   IF g_oob[l_ac].oob04 = '1' THEN
                      ...
                      IF (l_oma00!='11') AND (l_oma00!='12') AND
                         ...
                         (l_oma00!='17') THEN   #FUN-9B0130 add 15   #TQC-A20009 add 17
...
#TQC-A40132 --end--
        ...
        AFTER FIELD oob11
           #IF g_oob[l_ac].oob11 IS NULL THEN NEXT FIELD oob11 END IF    #No.TQC-670085 #MOD-BA0225
           ...
           IF NOT cl_null(g_oob[l_ac].oob11) THEN
              ...
              IF NOT cl_null(g_oob[l_ac].oob13) THEN
                 SELECT tc_afc04
                   INTO g_oob[l_ac].oobud02
                   FROM tc_afc_file
                  WHERE tc_afc01 = g_oob[l_ac].oob13
                    AND tc_afc02 = g_oob[l_ac].oob11
                 IF cl_null(g_oob[l_ac].oobud02) THEN LET g_oob[l_ac].oobud02 = ' ' END IF
                 DISPLAY BY NAME  g_oob[l_ac].oobud02
              END IF
...
FUNCTION t400_oob09_19(l_sw,l_cmd,l_sw2)           # 借/貸方檢查 : A/P   #MOD-750063
   ...
   DEFINE l_oob09      LIKE oob_file.oob09
   DEFINE l_oob10      LIKE oob_file.oob09
   DEFINE l_apz27      LIKE apz_file.apz27
   ...
   ELSE
      ...
      IF l_apz27 = 'N' THEN      #不做月底重評時, 依原判斷
        ...
        #IF g_oob[l_ac].oob10 > (l_apc.apc09-l_apc.apc11) THEN   #MOD-750063  #MOD-880177        #MOD-BB0016 mark
         IF g_oob[l_ac].oob10 > (l_apc.apc09-l_apc.apc11-l_amt16) THEN #MOD-750063  #MOD-880177  #MOD-BB0016 mod
            CALL cl_err('','axr-185',1)
        ...
        #IF (l_oob10+g_oob[l_ac].oob10) > tot3 - p05 THEN  #MOD-750063            #MOD-BB0016 mark
         IF (l_oob10+g_oob[l_ac].oob10) > tot3 - p05 - l_amt16 THEN  #MOD-750063  #MOD-BB0016 mod
           ...
           #LET g_oob[l_ac].oob10 = tot3 - p05 - l_oob10   #MOD-750063            #MOD-BB0016 mark
            LET g_oob[l_ac].oob10 = tot3 - l_amt16 - p05 - l_oob10   #MOD-750063  #MOD-BB0016 mod
...
#No.FUN-AA0088 --begin
    ...
    IF g_prog ='axrt401' THEN LET l_n =2 END IF
           ...
           #-CHI-B20040-end-
            ...
            IF (l_apa00 != '11') AND (l_apa00 != '12') AND (l_apa00 != '15') AND (l_apa00 != '13') THEN   #MOD-A50047 add l_apa00!='13'
              #CALL cl_err(l_oob.oob06,'axr-993',0) #CHI-A80031 mark
               CALL s_errmsg('',l_oob.oob06,'','axr-993',1) #CHI-A80031
               LET g_success = 'N'
              #EXIT FOREACH #CHI-A80031 mark
            END IF
         END IF
      ...
      IF l_oob.oob03='2' THEN
         IF l_oob.oob04 = '1' THEN
            SELECT oma00 INTO l_oma00 FROM oma_file
             WHERE oma01 = l_oob.oob06
            IF (l_oma00 !='11') AND (l_oma00 !='12') AND
               (l_oma00 !='13') AND (l_oma00 !='14') AND (l_oma00 !='15') AND (l_oma00 !='17') THEN   #FUN-9B0130 add 15  #TQC-A20009 add 17
              #CALL cl_err(l_oob.oob06,'axr-994',0) #CHI-A80031 mark
               CALL s_errmsg('',l_oob.oob06,'','axr-994',1) #CHI-A80031
               LET g_success = 'N'
              #EXIT FOREACH #CHI-A80031 mark
            END IF
         END IF
      ...
      AFTER FIELD oof06
         IF NOT cl_null(g_oof[l_ac1].oof06) THEN
            IF g_oof_t.oof06 IS NULL OR g_oof[l_ac1].oof06 != g_oof_t.oof06 THEN
               ...
               END IF
               CALL s_get_bookno(year(g_oof[l_ac1].oof06))
...
FUNCTION t400_oob09_1_19_2(l_sw,l_cmd,l_sw2)           # 借/貸方檢查 : A/P
   ...
   DEFINE l_oob09_1      LIKE oob_file.oob09
   DEFINE l_oob10_1      LIKE oob_file.oob09
   DEFINE l_apz27        LIKE apz_file.apz27
   ...
   IF l_sw = '1' THEN
     #IF g_oob_d[l_ac2].oob09_1 > (l_apc.apc08-l_apc.apc10) THEN              #MOD-BB0016 mark
      IF g_oob_d[l_ac2].oob09_1 > (l_apc.apc08-l_apc.apc10-l_apc.apc16) THEN  #MOD-BB0016 mod
   ...
   ELSE
      ...
      ELSE    #有做月底重評時, 需判斷不可超過未沖金額
        ...
        #IF (l_oob10_1+g_oob_d[l_ac2].oob10_1) > tot3 - p05 THEN              #MOD-BB0016 mark
         IF (l_oob10_1+g_oob_d[l_ac2].oob10_1) > tot3 - p05 - l_amt16 THEN    #MOD-BB0016 mod
            CALL cl_err('','axr-185',1)
...
#No.FUN-AA0088 --begin
    ...
    IF g_prog ='axrt401' THEN LET l_n =1 END IF
...
FUNCTION t400_set_comb_oob04()
  ...
  DEFINE comb_item  LIKE type_file.chr1000
    ...
    LET comb_value = '1,2,3,4,5,6,7,8,9,E,F,Q'
...
#No.FUN-AA0088 --begin
                 IF g_prog = 'axrt401' THEN
                    IF g_oob_d[l_ac2].oob09_1  > l_oob09 - l_oob09_1 THEN
...
#No.FUN-AA0088 --begin
                 IF g_prog = 'axrt401' THEN
                    IF g_oob_d[l_ac2].oob10_1  > l_oob10 - l_oob10_1 THEN
                       CALL cl_err('','axr-323',1)
                       LET g_oob_d[l_ac2].oob10_1 =0
                       NEXT FIELD oob10_1
                    END IF
...
#No.FUN-AA0088 --end
              ...
              END IF
             #MOD-C40139---str---
              IF g_oob_d[l_ac2].oob08_1 = 1 THEN
...
#No.FUN-AA0088 --end
          # add by lixwz 180131 s
          IF NOT cl_null(g_oob_d[l_ac].oob13_1) AND NOT cl_null(g_oob_d[l_ac].oob11_1) THEN
             SELECT tc_afc04
               INTO g_oob[l_ac].oobud02
               FROM tc_afc_file
              WHERE tc_afc01 = g_oob_d[l_ac].oob11_1
                AND tc_afc02 = g_oob_d[l_ac].oob13_1
             IF cl_null(g_oob_d[l_ac].oobud02_1) THEN LET g_oob_d[l_ac].oobud02_1 = ' ' END IF
             DISPLAY BY NAME  g_oob_d[l_ac].oobud02_1
          END IF
          # add by lixwz 180131 e
        ...
        AFTER FIELD oob13_1
          ...
          END IF
            ...
            IF NOT cl_null(g_oob_d[l_ac].oob13_1) AND NOT cl_null(g_oob_d[l_ac].oob11_1) THEN
               SELECT tc_afc04
                 INTO g_oob_d[l_ac].oobud02_1
                 FROM tc_afc_file
                WHERE tc_afc01 = g_oob_d[l_ac].oob11_1
                  AND tc_afc02 = g_oob_d[l_ac].oob13_1
               IF cl_null(g_oob[l_ac].oobud02) THEN LET g_oob_d[l_ac].oobud02_1 = ' ' END IF
               DISPLAY BY NAME  g_oob_d[l_ac].oobud02_1
            END IF
            # add by lixwz 180131 e
#No.FUN-AA0088 --begin
        AFTER FIELD oob24_1
           IF NOT cl_null(g_oob_d[l_ac2].oob24_1) THEN
              SELECT occ02 INTO g_oob_d[l_ac2].occ02 FROM occ_file WHERE occ01 =g_oob_d[l_ac2].oob24_1
...
DEFINE l_oma181      LIKE oma_file.oma181
    ...
    END FOR
       ...
       LET g_oob_d[l_ac2].oob25_1  = g_today
...
DEFINE  l_ooy        RECORD LIKE ooy_file.*
   ...
   DECLARE t401_sel_oob CURSOR  FOR
      SELECT * FROM oob_file WHERE oob01 = g_ooa.ooa01 AND oob03 ='1'
   ...
   FOREACH t401_sel_oob INTO l_oob.*
      IF STATUS THEN CALL cl_err('sel oob',STATUS,1) EXIT FOREACH END IF
         ...
         LET l_oma.oma09  = l_oma.oma02
         LET l_oma.oma00  = '14'
         CALL s_auto_assign_no("AXR",l_oob.oob27,l_oob.oob25,"14","oma_file","oma01","","","")
...
FUNCTION t401_del_oma()
 ...
 DEFINE l_ooy      RECORD LIKE ooy_file.*
   ...
   DECLARE  t401_sel_oma1 CURSOR FOR SELECT oma_file.* FROM oma_file,oob_file WHERE oma01 = oob27 AND oob01 = g_ooa.ooa01 AND oob03 ='1'
