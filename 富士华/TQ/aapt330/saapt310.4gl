...
# Modify.........: No:TQC-B80069 11/08/04 By guoch aph041赋值的bug处理
# Modify.........: No:MOD-B80062 11/08/08 By johung 付款單身檢核幣別應用幣別筆數
# Modify.........: No:TQC-B80079 11/08/15 By wujie 溢付和溢退抛转改在审核段做
# Modify.........: No:MOD-B80144 11/08/16 By lixia 增加nma21檢查
...
# Modify.........: No.FUN-B90062 11/09/26 By wujie 产生nme_file时同时产生tic_file
...
# Modify.........: No:CHI-B90046 11/10/19 By Vampire ON ACTION 沒有接 LET g_action_choice 沒有做權限控管
# Modify.........: No.MOD-BA0189 11/10/30 By Dido 新增輸入段給予帳別改在 BEFORE INPUT
# Modify.........: No.MOD-BB0020 11/11/04 By yinhy aapt332,aapt330沖帳時，aapt220已沖金額回寫錯誤
# Modify.........: No.MOD-BB0039 11/11/06 By Dido aapt335 預設類別為apz65,限定選擇apw_file資料
# Modify.........: No.MOD-BB0091 11/11/07 By wujie aapt335 产生apa时，预设apa20 = 0
# Modify.........: No.MOD-BB0098 11/11/09 By Polly 調整判斷，apf03是空值或不等於舊值，才重抓apf12
# Modify.........: No.MOD-BB0133 11/11/15 By yinhy aapt335 产生apa时，预设apa73=apa34
...
# Modify.........: No.TQC-BC0062 11/12/09 By Dido 增加判斷，账款本币金额若为0报错提示
...
# Modify.........: No:TQC-BC0207 11/12/31 By wujie 调帐/溢退抛转的应付帐款自动抛凭证不能
# Modify.........: No:FUN-C10039 12/02/02 by Hiko 整批修改資料歸屬設定
# Modify.........: No:TQC-BC0197 12/02/02 By yinhy UPDATE npp_file报错更改
# Modify.........: No:TQC-C20037 12/02/03 By yinhy 立即審核并自動拋磚憑證時增加判斷
...
# Modify.........: No:TQC-C40194 12/04/10 By minpp 将点击[打印]时，对应于传入得打印额外备注的参数改为'Y'
# Modify.........: No:CHI-C50002 12/05/02 By yinhy 退款沖帳作業退款部份類型為轉帳時參考單號應判斷原幣出賬合計-原幣已沖金額>0
# Modify.........: No_120813_1   12/08/13 By Exia  修改退款单身参考单号的开窗
# Modify.........: No:121126     12/11/26 By yangjian 修改程序传递参数，被saapt110事务调用
DATABASE ds
...
#FUN-A20010 --End
    ...
    g_statu         LIKE type_file.chr1,        # Prog. Version..: '5.25.08-12.05.10(01),              #是否從新賦予等級
...
DEFINE g_inTruncation      BOOLEAN    #No:121126   Add <-yangjian-

#FUNCTION t310(p_aptype,p_plant,p_argv1,p_argv2)  #No:121126  Modify <-yangjian-
FUNCTION t310(p_aptype,p_plant,p_argv1,p_argv2,p_inTruncation)  #No.FUN-630010
DEFINE
   ...
   p_inTruncation  BOOLEAN       #No:121126  Add <-yangjian

   WHENEVER ERROR CALL cl_err_msg_log

   CALL s_get_bookno('') RETURNING g_flag,g_bookno1,g_bookno2    #TQC-7C0050

  #No:121126  Add  <<--yangjian--
   IF cl_null(p_inTruncation) THEN LET p_inTruncation = FALSE END IF
   LET g_inTruncation = p_inTruncation
  #No:121126  End  <<--yangjian--
   LET g_plant=p_plant
   LET g_argv1=p_argv1
   ...
   IF p_argv1 is null or p_argv1 = ' ' THEN
      SLEEP 0
   ELSE
     #No:121126  Modify <-yangjian-
     #根据传递参数来判断是否要切数据库，为了保证数据库已有的事务不被破坏
      IF NOT p_inTruncation THEN
         LET g_plant=p_plant
         LET g_argv1=p_argv1
         LET g_argv2=p_argv2                        #No.FUN-630010
   ...
   IF g_bgjob='N' OR cl_null(g_bgjob) THEN  #TQC-720009
       ...
       ELSE
             IF g_aptype = '33' THEN      #No.FUN-A40003
                OPEN WINDOW t310_w33 AT 2,2 WITH FORM "cap/42f/aapt330"
...
#No.FUN-A90007 --begin
          IF p_aptype <> '36' THEN
             CALL cl_set_comp_visible("aph21,pmc03,aph22,apr02,aph23,aph24",FALSE)
          END IF
#No.FUN-A90007 --end
...
#No.FUN-A80111 --end
  ...
  IF NOT cl_null(g_argv1) THEN
     CASE g_argv2
      ...
      #No:121126  Add  <<--yangjian--
      #专门被saapt110来调用，勿修改，其它作业如果有调用到的话，请拷贝，勿直接修改
        WHEN "outconfirm"
           CALL t310_q()
           CALL s_showmsg_init()          #TQC-B10069
           LET g_success = "Y"            #TQC-B10069
  ...
  CALL aws_efapp_flowaction("insert, modify, delete, reproduce, detail, query,locale, void, confirm, undo_confirm, easyflow_approval,account_detail,payment_detail,gen_entry,entry_sheet,entry_sheet2, carry_voucher, undo_carry_voucher,handling_differences")     #No.FUN-680029 #TQC-740281 #CHI-AA0003 add handling_differences
...
DEFINE   lc_qbe_sn   LIKE gbm_file.gbm01   #No.FUN-580031
   ...
   ELSE
      ...
      DIALOG ATTRIBUTES(UNBUFFERED)
         ...
         CONSTRUCT g_wc3 ON aph02,aph03,aph21,aph22,aph08,aph04,aph17,aph23,aph24,aph16,aph07,aph09,aph13,   #MOD-590054   #TQC-5B0021 #No.FUN-680027--add aph17  FUN-A90007 add aph21,aph22,aph23,aph24
               ...
               FROM s_aph[1].aph02,s_aph[1].aph03,s_aph[1].aph21,s_aph[1].aph22,s_aph[1].aph08,s_aph[1].aph04,s_aph[1].aph17,s_aph[1].aph23,s_aph[1].aph24,  #TQC-5B0021   #No.FUN-680027--add aph17  FUN-A90007 add aph21,aph22,aph23,aph24
               ...
               IF cl_confirm('aws-081') THEN  #詢問是否繼續下一筆資料的簽核
                  IF aws_efapp_getnextforminfo() THEN #取得下一筆簽核單號
                     ...
                     IF NOT cl_null(g_argv1) THEN     #自動 query 帶出資料
                       CALL t310_q()
                      #設定簽核功能及哪些 action 在簽核狀態時是不可被執行的
                       CALL aws_efapp_flowaction("insert, modify, delete, reproduce, detail, query,locale, void, confirm, undo_confirm, easyflow_approval,account_detail,payment_detail,gen_entry,entry_sheet,entry_sheet2, carry_voucher, undo_carry_voucher")     #No.FUN-680029 #TQC-740281
                            RETURNING g_laststage
                     ELSE
                         EXIT WHILE
                     END IF
...
FUNCTION t310_u()
   ...
   IF NOT s_chkpost(g_apf.apf44,g_apf.apf01) THEN RETURN END IF
...
FUNCTION t310_apf04(p_cmd)
   ...
   SELECT gen02,gen03,genacti INTO l_gen02,l_gen03,l_genacti
     FROM gen_file WHERE gen01 = g_apf.apf04
   LET g_errno = ' '
   CASE WHEN SQLCA.SQLCODE = 100 LET g_errno = 'aap-038'
        WHEN l_genacti = 'N'     LET g_errno = '9028'          #No.CHI-780046
        WHEN SQLCA.SQLCODE != 0  LET g_errno = SQLCA.SQLCODE USING '-----'
...
FUNCTION t310_show()
   ...
   DISPLAY BY NAME g_apf.apforiu,g_apf.apforig,
          ...
          g_apf.apfuser,g_apf.apfgrup,g_apf.apfmodu,g_apf.apfdate,g_apf.apfacti,
           ...
           g_apf.apfud09,g_apf.apfud10,g_apf.apfud11,g_apf.apfud12,
           g_apf.apfud13,g_apf.apfud14,g_apf.apfud15,
           g_apf.apf45    #FUN-870037 add
   CALL t310_apf03('d')
   CALL t310_apf04('d')     #No:130729  Add  <<--yangjian--
   CALL t310_apf05('d')     #No:130729  Add  <<--yangjian--
...
DEFINE l_cnt            LIKE type_file.num5       #MOD-870048 add
   ...
   IF NOT s_chkpost(g_apf.apf44,g_apf.apf01) THEN RETURN END IF #no.7277
   LET g_success = 'Y'
   BEGIN WORK
   OPEN t310_cl USING g_apf.apf01
   IF STATUS THEN
      ...
      ELSE
         LET g_sql = "SELECT 0,apa01,apc02,apa02,apa13,apa14,", #TQC-9B0119
                   ...
                   #           g_dbs_new CLIPPED,"apc_file ",                                                                                         #FUN-A50102 mark
                     ...
                     "   AND apa02 <= '",g_apf.apf02 CLIPPED,"'",         #MOD-A30146 add
                     "   AND apa01 = apc01 ",
                     #"   AND (apc08 >(apc10+apc16+apc14) OR apc13>(apc16*apc07+apc15))", #MOD-B70047 add apc14/apc15 #MOD-B90084 mark
...
#No.FUN-A40003 --begin
      ...
      ELSE
        LET g_sql = g_sql CLIPPED,"   AND apa00 LIKE '1%'"
      END IF
#No.FUN-A40003 --end
        ...
        AFTER FIELD apgud02
            IF g_prog = 'aapt330' THEN
               IF NOT cl_null(g_apb[l_ac].apb26) THEN
                   SELECT tc_afc04
                     INTO g_apb[l_ac].apbud03
                     FROM tc_afc_file
                    WHERE tc_afc01 = g_apb[l_ac].apb26
                      AND tc_afc02 = g_apb[l_ac].apb25
                   IF cl_null(g_apb[l_ac].apbud03) THEN LET g_apb[l_ac].apbud03 = ' ' END IF
                   DISPLAY BY NAME g_apb[l_ac].apbud03
               END IF
           END IF
           IF NOT cl_validate() THEN NEXT FIELD CURRENT END IF
        AFTER FIELD apgud03
...
FUNCTION t310_b2_tot()
  ...
  #CHI-A10034---add---start---
   ...
   IF l_cnt > 0 THEN
      ...
      DISPLAY BY NAME g_apf.apf10
      IF g_aptype <> '36' THEN    #No.FUN-A90007       #MOD-B90024 mark
         LET g_apf.apf09 = g_apf.apf10 -  l_apf08       #MOD-A90078
         DISPLAY BY NAME g_apf.apf09                    #MOD-A90078
      END IF        #No.FUN-A90007
#No.TQC-B80079 --begin
      LET g_apf.apf09f =0
      LET g_apf.apf10f =0
...
#No.TQC-B80079 --end
  ...
  #CHI-A10034---add---end---
      ...
      IF g_apf.apf10  IS NULL THEN LET g_apf.apf10 =0 END IF
      DISPLAY BY NAME g_apf.apf10f,g_apf.apf10
      IF g_aptype <> '36' THEN    #No.FUN-A90007
        ...
        #-MOD-B10116-end-
         ...
         END IF    #MOD-B10116
         DISPLAY BY NAME g_apf.apf09f,g_apf.apf09       #MOD-A90078
      END IF        #No.FUN-A90007
...
#                  "    AND apa00[1,1] = '1'",                         #No.FUN-A40003
   ...
   ELSE
      ...
      LET g_sql = "SELECT apa02,apa13,apa72,apc08-apc10-apc16-apc14,apc13-apc16*apa72,",  #No.MOD-B90084
                  " apa06,apa07,apa21,apa22,apa13,apa00,apa41,apa42,apa08,apa44 ",
                # "  FROM ",g_dbs_new,"apa_file, ",                           #FUN-A50102 mark
                #           g_dbs_new,"apc_file  ",                           #FUN-A50102 mark
                  "  FROM ",cl_get_target_table(g_plant_new,'apa_file'),",",  #FUN-A50102
...
#No.FUN-A40003 --end
   ...
   IF NOT cl_null(l_apa44) AND g_apz.apz05 = 'N' THEN
      ...
      OPEN t310_c3x USING l_apa44,g_apz.apz02b
...
DEFINE p_cmd           LIKE type_file.chr1,   #處理狀態  #No.FUN-690028 VARCHAR(1)
       ...
       l_t1            LIKE type_file.chr5,   # Prog. Version..: '5.25.08-12.05.10(03),     #No.MOD-6B0141
       ...
       l_nmh17         LIKE nmh_file.nmh17    #FUN-B40011 add
#FUN-A20010 --Begin
...
DEFINE l_sme02   LIKE sme_file.sme02              #CHI-C10043
    ...
    LET g_forupd_sql = "SELECT aph02,aph03,aph21,'',aph22,'',aph08,aph04,aph041,aph17,aph23,aph24,aph16,'',aph07,aph09,aph13,",   #MOD-590054   #TQC-5B0021  #No.FUN-680027 add  FUN-A90007 add aph21,'',aph22,''   aph23,aph24
...
#No.TQC-B30220 --Beign #去掉mark
           ...
           IF g_rec_b2 >= l_ac THEN
              ...
              END IF
#No.FUN-A90007 --begin
              SELECT pmc03 INTO g_aph[l_ac].pmc03 FROM pmc_file WHERE pmc01 = g_aph[l_ac].aph21
...
#No.FUN-A90007 --end
        ...
        BEFORE INSERT
          ...
          #LET g_aph_o.aph08 = NULL              #MOD-A80076 #MOD-BC0242 mark
           LET g_aph_t.aph08 = NULL              #MOD-BC0242 add                              #MOD-A80076
...
#No.FUN-A90007 --end
           INSERT INTO aph_file(aph01,aph02,aph03,aph08,aph18,aph19,aph20, #FUN-A20010 add aph18/19/20
                                ...
                                aph13,aph14,aph05f,aph05,aph16,aph21,aph22,aph23,aph24,       #No.FUN-A90007 add aph21,aph22,aph23,aph24
                                  aphud01,aphud02,aphud03,
        ...
        AFTER FIELD aph03
           ...
           IF NOT cl_null(g_aph[l_ac].aph03) THEN
              CASE WHEN g_aptype = '32'
                IF g_aph[l_ac].aph03 NOT MATCHES "[12]" THEN                #No.FUN-A40003
            ...
            #No.MOD-BC0242 --begin
             ...
             #No.FUN-A40003 --end
              IF p_cmd='a' OR
                (p_cmd='u' AND (g_aph_t.aph03 != g_aph[l_ac].aph03)) THEN   #TQC-5B0021
            #No.MOD-BC0242 --end
               ...
               #LET g_aph_o.aph03 = g_aph[l_ac].aph03   #TQC-5B0021 #MOD-BC0242 mark
                LET g_aph_t.aph03 = g_aph[l_ac].aph03   #MOD-BC0242 add
              ELSE
              #-MOD-A80076-mark-
              ...
              END IF
              IF g_aptype <>'32' THEN     #No.FUN-A40003
                 IF g_aph[l_ac].aph03 NOT MATCHES "[6789ABDZ]" THEN      #FUN-B40011 add D
           ...
           IF g_aza.aza63 = 'Y' AND g_aph[l_ac].aph03 NOT MATCHES "[6789]"  AND g_aptype <> '32' THEN       #No.FUN-A40003
        ...
        AFTER FIELD aph04
            IF NOT cl_null(g_aph[l_ac].aph04) AND g_aptype <> '32' THEN   #No.FUN-A40003 add aptype
               IF g_aph_t.aph04 IS NULL OR g_aph[l_ac].aph04!=g_aph_t.aph04
                   ...
                   IF g_aph[l_ac].aph03 NOT MATCHES "[6789D]" THEN            #No.FUN-B40011
            ...
            IF NOT cl_null(g_aph[l_ac].aph041) THEN
              ...
              #防止User只修改部門欄位時,未再次檢查會科與允許/拒絕部門關係
               IF g_aza.aza63='Y' THEN
                  ...
                  LET g_errno = ' '
                  IF l_aag05_1 = 'Y' AND g_aph[l_ac].aph03 NOT MATCHES "[6789]"
                     AND NOT cl_null(g_apf.apf05) THEN
                    #當使用利潤中心時(aaz90=Y),允許/拒絕部門的判斷請改用會科+成本中心
                     IF g_aaz.aaz90 !='Y' THEN
                        CALL s_chkdept(g_aaz.aaz72,g_aph[l_ac].aph041,g_apf.apf05,g_bookno2)
                ...
                IF g_apz.apz27 = 'N' THEN
                   SELECT apa00,apc08-apc10-apc14,apc09-apc11-apc15,apa41,apa42, #MOD-B70047 add apc14/apc15
                    ...
                    WHERE apa01 = g_aph[l_ac].aph04 AND apa00 = l_type
                      ...
                      AND apa02 <= g_apf.apf02          #MOD-A30146 add
                ELSE
                   #SELECT apa00,apc08-apc10-apc14,apc13-apc15,apa41,apa42, #MOD-B70047 add apc14/apc15  #MOD-B90084 mark
               ...
               IF g_apz.apz27 = 'N' THEN
                  SELECT apc08-apc10-apc14,apc09-apc11-apc15 #MOD-B70047 add apc14/apc15
                   ...
                   WHERE apa01 = g_aph[l_ac].aph04 AND apa00 = l_type
                     ...
                     AND apa02 <= g_apf.apf02          #MOD-A30146 add
               ELSE
                  #SELECT apc08-apc10-apc14,apc13-apc15,  #MOD-B70047 add apc14/apc15
                  SELECT apc08-apc10-apc14,apc13, #MOD-B90084
                    INTO g_qty1,g_qty2
                    FROM apa_file,apc_file
                   WHERE apa01 = g_aph[l_ac].aph04 AND apa00 = l_type
                     AND apc01 = apa01 AND apc02 = g_aph[l_ac].aph17
...
#No.FUN-B40011--begin
             ...
             #       AND aph01!= g_apf.apf01
                  ...
                  SELECT nmh17 INTO l_nmh17
                    FROM nmh_file
                   WHERE nmh01 = g_aph[l_ac].aph04
                     AND nmh24='5'
                     AND nmh38='Y'
                  IF cl_null(l_nmh17) THEN LET l_nmh17 = 0 END IF

                  IF l_sum_aph05f+g_aph[l_ac].aph05f > l_nmh17 THEN
...
#No.FUN-A90007 --end
        ...
        AFTER ROW
            ...
            IF INT_FLAG THEN
              ...
              #LET g_aph_o.aph03 = ''  #No.FUN-680029 #MOD-BC0242 mark
               LET g_aph_t.aph03 = ''  #No.MOD-BC0242 add
               CLOSE t310_bcl
               ROLLBACK WORK
               EXIT INPUT
            END IF
        ...
        ON ACTION mntn_freq_ac
           IF g_aph[l_ac].aph03 NOT MATCHES '[ABT]' THEN
              ...
              WHEN INFIELD(aph08)
                 ...
                 ELSE
                    IF g_aph[l_ac].aph03 = '2' THEN
                       ...
                       END IF
                    ELSE
                       #LET g_qryparam.arg1 = g_aph[l_ac].aph03   #No.TQC-5B0114 #TQC-B90208 mark
...
#No.FUN-A40003 --begin
                       ...
                       IF g_aph[l_ac].aph03 ='2' THEN
                          CALL cl_init_qry_var()
                          #LET g_qryparam.form = "q_nmg"   #CHI-C50002
                          # LET g_qryparam.form = "q_nmg3"   #CHI-C50002
                          LET g_qryparam.form = "q_nmg3_1"  #Exia 120813_1
...
#No.FUN-A90007 --end
   ...
   #-TQC-B10199-end-
    ...
    #CALL t3101() #CHI-AA0003 add  #MOD-B90204
...
#No.FUN-A80111 --end
   ...
   IF g_apf.apf00 = '33' AND g_apz.apz40 = 'N' THEN RETURN END IF
    ...
    IF g_add = 'Y' THEN
       ...
       RETURN
    END IF
    IF g_apf.apf10 <> (g_apf.apf08+g_apf.apf09 ) THEN     #No.FUN-A60024
...
FUNCTION t310_tmp_apf09()            # 轉入暫付
   DEFINE l_t1          LIKE type_file.chr5        # Prog. Version..: '5.25.08-12.05.10(03)     #No.MOD-6B0141
...
#No.FUn-A60024 --end
      ...
      ON ACTION ExportToExcel
         #LET g_action_choice="account_detail"     #CHI-B90046 mark
         LET g_action_choice="ExportToExcel"       #CHI-B90046 add
...
#               "       aph05f,aph05,",
    LET g_sql = "SELECT aph02,aph03,aph21,'',aph22,'',aph08,aph04,aph041,aph17,aph23,aph24,aph16,'',aph07,aph09,aph13,aph14,",   #MOD-590054   #TQC-5B0021    #No.FUN-FUN-970077  FUN-A90007 add aph21,'',aph22,''   aph23,aph24
                "       aph05f,aph05,aph18,aph19,aph20,",
#FUN-A20010 --End
                "       aphud01,aphud02,aphud03,aphud04,aphud05,",
                "       aphud06,aphud07,aphud08,aphud09,aphud10,",
                "       aphud11,aphud12,aphud13,aphud14,aphud15",
...
FUNCTION t310_firm1_chk()
  ...
  #FOREACH aph03_c INTO l_aph03                           #MOD-A90129 mark
   FOREACH aph03_c INTO l_aph03,l_aph16,l_aph21           #MOD-A90129     #TQC-B50029 add aph21
    ...
    #IF l_aph03 = '2' AND cl_null(g_apf.apf14) AND g_aptype<>'32' AND g_aptype<>'36' THEN  #TQC-B20092         #MOD-C20117 mark
  ...
  #-MOD-AB0197-end-
   ...
   IF only_one != '1' THEN
     #LET g_sql = "SELECT * FROM apf_file WHERE ",g_wc CLIPPED         #MOD-B70159 mark
      LET g_sql = "SELECT * FROM apf_file WHERE apf41='N' AND ",g_wc CLIPPED     #MOD-B70159 add
      ...
      FOREACH apf_curs INTO g_apf.*
         ...
         CALL t310_firm1_chk()
         IF g_success = 'N' THEN
         #  CLOSE WINDOW t310_w6    #MOD-AC0327  #TQC-B10069
   ...
   IF g_success = 'Y' THEN
      IF g_apf.apfmksg = 'Y' THEN #簽核模式
         CASE aws_efapp_formapproval()            #呼叫 EF 簽核功能
              WHEN 0  #呼叫 EasyFlow 簽核失敗
                   LET g_apf.apf41="N"
                   LET g_success = "N"
                 #  ROLLBACK WORK       #No:121126  Modify <-yangjian-
                   IF NOT g_inTruncation THEN
                      ROLLBACK WORK
                   END IF
                   RETURN
      ...
      END IF
       IF g_success='Y' THEN
         ...
         DISPLAY BY NAME g_apf.apf42
         #COMMIT WORK        #No:121126  Modify <-yangjian-
         IF NOT g_inTruncation THEN
            COMMIT WORK
         END IF
...
#  CALL s_showmsg() #CHI-A60034 add        #TQC-B10069
   ...
   #IF g_apy.apydmy3 = 'Y' AND g_apy.apyglcr = 'Y' AND g_success = 'Y' THEN   #No:121126 Modify <-yangjian-
   IF g_apy.apydmy3 = 'Y' AND g_apy.apyglcr = 'Y' AND g_success = 'Y' AND NOT g_inTruncation THEN
...
#No.FUN-A40003 --end
        ...
        IF u_sw='1' THEN #no.7372
           ...
           IF NOT cl_null(l_apa44) AND g_apz.apz05 = 'N' THEN
              ...
              OPEN t310_c3z USING l_apa44,g_apz.apz02b
         ...
         # LET g_sql = "UPDATE ",g_dbs_new CLIPPED,"apc_file ",              #FUN-A50102 mark
           ...
           IF g_aptype = '32' OR g_aptype='33' THEN
              IF NOT cl_null(m_apg.apg06) THEN
                 ...
                 IF STATUS THEN
                    IF u_sw = 1 THEN
                       CALL s_errmsg('','','sum apv04f,apv05f',STATUS,1)
                    ELSE
      ...
      DECLARE t310_bu_c2 CURSOR FOR
          SELECT * FROM aph_file
           WHERE aph01=m_apf.apf01 AND aph03 IN ('6','7','8','9')
      FOREACH t310_bu_c2 INTO m_aph.*
        ...
        #LET g_plant_new = m_aph.aph08   #MOD-B10147 mark
         ...
         ELSE
            IF u_sw=1 THEN
               IF (l_apa34f < (l_amtf+t1+t3+t10) ) OR (l_apa34  < (l_amt +t2+t4+t9)) THEN   #TQC-6C0044
       ...
       # LET g_sql = "UPDATE ",g_dbs_new CLIPPED,"apa_file ",               #FUN-A50102 mark
         ...
         IF STATUS THEN
           #CALL cl_err('s4_aph_pre',status,1) #CHI-A60034 mark
            ...
            EXIT FOREACH
         END IF
         LET l_amtf = l_amtf + t1 + t3 + t10   #MOD-8B0006 mark回復
         LET l_amt  = l_amt  + t2 + t4 + t9    #MOD-8B0006 mark回復
        ...
        #更新apc_file
         IF NOT cl_null(m_aph.aph17) THEN
            ...
            ELSE
               IF u_sw=1 THEN
                  IF (l_apc08  < (l_amtf5+t5+t7+t10) ) OR (l_apc09  < (l_amt5+t6+t8+t9)) THEN  #No.FUN-7B0055
                    #CALL cl_err('','aap-250',1) #CHI-A60034 mark
                     CALL s_errmsg('','','','aap-250',1) #CHI-A60034
                     LET g_success='N'
                  END IF
          ...
          # LET g_sql = "UPDATE ",cl_get_target_table(g_plant_new,'apc_file'),",", #FUN-A50102  #TQC-AC0292
            ...
            IF STATUS THEN
              #CALL cl_err('s4_aph_pre',status,1) #CHI-A60034 mark
               ...
               EXIT FOREACH
            END IF
            LET l_amtf5 = l_amtf5 + t5 + t7 + t10  #No.FUN-7B0055  add t10
            LET l_amt5  = l_amt5  + t6 + t8 + t9   #No.FUN-7B0055  add t9
            IF g_apz.apz27 = 'Y' AND m_aph.aph13 != g_aza.aza17 THEN
...
FUNCTION t310_x()
   ...
   IF SQLCA.sqlcode THEN
       ...
       RETURN
   END IF
   IF cl_void(0,0,g_apf.apf41) THEN
      IF g_apf.apf41 ='N' THEN    #切換為作廢
         LET l_cnt = 0
         SELECT COUNT(*) INTO l_cnt FROM npp_file
...
#No.FUN-B40011 --begin
    IF g_aptype='33' THEN
       LET comb_value = '1,2,3,4,5,6,7,8,9,A,B,C,D,Z'
       SELECT ze03 INTO comb_item FROM ze_file
        WHERE ze01='aap-355' AND ze02=g_lang
 #   END IF    #TQC-B70160 mark
#No.FUN-B40011 --end
   #-MOD-B70011-add-
...
FUNCTION t310_aph04()
   ...
   # Exia 120813_1 add  MARK
    #  SELECT nmg_file.* INTO l_nmg.*  FROM nmg_file
    #         WHERE nmg00= g_aph[l_ac].aph04
#   #            AND nmg23 > nmg24   #CHI-C50002
    #           AND nmg05 > nmg24    #CHI-C50002
    #           AND nmgconf <> 'X'
#   #            AND (nmg20='21' OR nmg20='22')
#   #            AND (nmg29 !='Y')    #NO:4181
      SELECT nmg_file.* INTO l_nmg.*  FROM nmg_file
             WHERE nmg00= g_aph[l_ac].aph04
               AND (nmg23 > nmg24 OR nmg05 > nmg24)
               AND nmgconf <> 'X'
      #Exia 120813_1 end
      IF STATUS  THEN
         LET g_errno=STATUS
      ...
      IF l_nmg.nmgconf='X' THEN
         ...
         RETURN  '','','','',''
      END IF
      IF l_nmg.nmg23-l_nmg.nmg24 = 0 THEN  #CHI-C50002 mark Exia 取消Mark
      #IF l_nmg.nmg05-l_nmg.nmg24 = 0 THEN   #CHI-C50002
         LET g_errno='axr-184'
         RETURN  '','','','',''
      END IF
      ...
      FOREACH t310_aph04_npk_c1 INTO l_npk.*
        ...
        IF l_nmz20 = 'Y' AND l_curr != g_aza.aza17 THEN
           ...
           IF (tot1+l_aph05f) = l_nmg.nmg23 THEN    #CHI-C50002 mark Exia 取消Mark
           #IF (tot1+l_aph05f) = l_nmg.nmg05 THEN     #CHI-C50002
...
#--begin-- FUN-B40011
   IF g_aph[l_ac].aph03 = 'D' THEN
      ...
      #---> TQC-C30293 add
      IF l_nmh.nmh22!=g_apf.apf03 THEN    #幣別與沖帳不一致
         LET g_errno='aap-040'
      END IF
      #---> TQC-C30293 add--end
...
FUNCTION t310_bu_12(p_sw)             #更新TT檔 (nmg_file)
  ...
  DEFINE l_nmg05        LIKE nmg_file.nmg05      #CHI-C50002
...
##No.2724 modify 1998/11/05 確認時,判斷此參考單號之單據是否已確認
  ...
  SELECT nmg25,nmgconf INTO l_nmg25,l_nmgconf
    FROM nmg_file WHERE nmg00= m_aph.aph04
...
# 同參考單號若有一筆以上僅沖款一次即可 --------------
  ...
  END IF
  LET l_nmg24 =0
  SELECT nmg23,nmg23-nmg24 INTO l_nmg23,l_nmg24  #CHI-C50002 mark Exia 120813_1 end
 # SELECT nmg05,nmg05-nmg24 INTO l_nmg05,l_nmg24   #CHI-C50002
    FROM nmg_file WHERE nmg00= m_aph.aph04
    IF STATUS THEN
    ...
    END IF
# check 是否沖過頭了 ------------
    IF tot1>l_nmg23  THEN  #CHI-C50002 mark Exia 取消Mark 120813_1
    #IF tot1>l_nmg05  THEN   #CHI-C50002
...
#No.FUN-A80111 --begin
            ...
            IF g_success ='N' THEN RETURN END IF
            IF g_aptype ='32' THEN EXIT FOREACH END IF
#No.FUN-A80111 --end
            CALL t310_b2_fill(" 1=1")
         END IF
   END FOREACH
...
FUNCTION t310_carry_voucher_apa()
  ...
  DEFINE l_apa        RECORD LIKE apa_file.*
    ...
    FOREACH t310_sel_apa INTO l_apa.*
       ...
       END IF
#       IF g_apy.apydmy3 = 'Y' AND g_apy.apyglcr = 'Y'  THEN
      #IF l_apy.apydmy3 = 'Y' AND l_apy.apyglcr = 'Y'  THEN       #No:121126  Modify <-yangjian-
       IF l_apy.apydmy3 = 'Y' AND l_apy.apyglcr = 'Y' AND NOT g_inTruncation  THEN    #No.TQC-BC0207
  ...
  #-MOD-B10116-end-
   ...
   INPUT BY NAME diff_flag
      AFTER FIELD diff_flag
         IF diff_flag NOT MATCHES "[12345E]" THEN NEXT FIELD diff_flag END IF
...
#           IF diff_flag MATCHES '[3]'  AND g_apf.apf10 > g_apf.apf08 THEN
   ...
   IF diff_flag MATCHES "[12345]" THEN
      BEGIN WORK
#No.TQC-B20112 --begin
