...
# Modify.........: No.MOD-BB0085 11/11/09 By jt_chen 增加檢核傳票編號存在於nppglno時,則不可以刪除
# Modify.........: No.MOD-BB0229 11/11/23 By Dido abb08/abb35 改用 aag23 判斷
# Modify.........: No.TQC-BB0211 11/11/24 By Sarah name09與name10的顯示與否,應該是判斷aaz119,而非aza48
# Modify.........: No:FUN-BB0123 11/12/01 By Lori 修改拋轉帳別方式
# Modify.........: No.TQC-BB0246 11/12/08 By yuhuabao 取位問題修改
...
# Modify.........: No:MOD-C40085 12/04/12 By Elise 增加控卡
# Modify.........: No.MOD-C40155 12/04/24 By Polly aglt1101畫面多加帳別aba00欄位
# Modify.........: No.TQC-C40017 12/04/24 By lujh  當來源碼不為GL RV AC時，複製時備註欄位的值清空
...
#No.FUN-A50058 --end
   ...
   IF g_aza48 = "N" THEN
     ...
     #CALL cl_set_comp_visible("page02,aba35,name09,aba36,name10",FALSE) #MOD-7B0006  #No.FUN-A50058  #TQC-BB0211 mark
      CALL cl_set_comp_visible("page02,aba35,aba36",FALSE)  #MOD-7B0006  #No.FUN-A50058               #TQC-BB0211
   END IF
   ...
   #delete switch_plant
   CALL aws_efapp_flowaction("insert, modify, delete, reproduce, detail, query,locale, void, confirm, undo_confirm, easyflow_approval,invalid,invoice,cashier_confirm,undo_cashier_confirm,contra_detail,enter_book_no,voucher_post,undo_post,source_query")  #TQC-740284   #FUN-920105
...
#No.TQC-A70036 --end
      ...
      IF SQLCA.sqlcode THEN               #置入資料庫不成功
         ...
         CALL cl_err3("ins","aba_file",g_aba.aba00,g_aba.aba01,SQLCA.sqlcode,"","",1)  #No.FUN-660123
         CONTINUE WHILE
      ELSE
         COMMIT WORK   #No.7875
      END IF
           ...
           LET g_aba.abagrup = s_get_data_group("aba_file") #FUN-C10039
            ...
            IF g_aaz.aaz81='Y' THEN #MOD-9A0081
               IF cl_null(g_aba.aba11) THEN
                  ...
                  IF cl_null(g_aba.aba11) THEN LET g_aba.aba11 = 1 END IF
    ...
    ELSE
       ...
       SELECT count(*) INTO l_n FROM aag_file
        WHERE aag19 = '1' AND aag01 IN (SELECT abb03 FROM abb_file
          ...
          AND aag00 = g_bookno    #No.FUN-740020
       IF l_n = 0 THEN
         #CALL cl_set_comp_visible("aba35,name09,aba36,name10",FALSE)  #No.FUN-A50058  #TQC-BB0211 mark
          CALL cl_set_comp_visible("aba35,aba36",FALSE)  #No.FUN-A50058                #TQC-BB0211
...
FUNCTION t110_show()
    ...
    DISPLAY BY NAME g_aba.abaoriu,g_aba.abaorig,                        # 顯示單頭值
        ...
        g_aba.abaoriu,g_aba.abaorig,                                  #TQC-B90255
        g_aba.abaud01,g_aba.abaud02,g_aba.abaud03,g_aba.abaud04,
        g_aba.abaud05,g_aba.abaud06,g_aba.abaud07,g_aba.abaud08,
        g_aba.abaud09,g_aba.abaud10,g_aba.abaud11,g_aba.abaud12,
        g_aba.abaud13,g_aba.abaud14,g_aba.abaud15
...
#FUN-B50116   ---start
        ...
        DISPLAY l_aba16,l_aba17 TO aba16,aba17
#FUN-B50116   ---end
...
FUNCTION t110_x()
   DEFINE l_cnt       LIKE type_file.num10    #MOD-C40085
   DEFINE l_aba00     LIKE aba_file.aba00     #MOD-C40085
   DEFINE l_aba16     LIKE aba_file.aba16     #MOD-C40085
  ...
  #MOD-C40085---str---
  #00帳作廢,但01帳已經過帳時,則不可作廢

  SELECT COUNT(*) INTO l_cnt FROM aba_file
   WHERE aba01 = g_aba.aba01
      IF l_cnt = 2 THEN
      SELECT aba00,aba16 INTO l_aba00,l_aba16 FROM aba_file
       WHERE aba01 = g_aba.aba01
      ...
      SELECT COUNT(*) INTO l_cnt
       ...
       WHERE aba01=g_aba.aba01
         ...
         AND abapost='Y'
         IF l_cnt > 0 THEN
            CALL cl_err(g_aba.aba01,'aap-622',1)
            RETURN
         END IF
      END IF
  #MOD-C40085---end---
         ...
         WHERE aba01 = g_aba.aba01
            ...
            SELECT COUNT(*) INTO l_cnt FROM aba_file
             WHERE aba01 = g_aba.aba01
               ...
               AND aba19 != 'Y'
                IF l_cnt > 0 THEN
                  UPDATE aba_file SET abaacti=g_aba.abaacti, aba20=g_aba.aba20
                   WHERE aba01 = g_aba.aba01
                     AND aba00 = l_aba16
                END IF
            END IF
...
FUNCTION t110_r()
   ...
   IF g_aba.abamksg = 'Y' AND  (g_aba.aba20 = '1' OR  g_aba.aba20 = 'S') THEN
      CALL cl_err(g_aba.aba01,'agl-160',0)
      RETURN
   END IF
   #-->非總帳輸入則不可刪除
   IF g_argv2 = '1' THEN
   ...
   END IF
   CALL t110_show()
# Prog. Version..: '5.25.08-12.05.10(0,0) THEN                   #確認一下   #No.FUN-9B0098 MARK 10/03/01
           ...
           IF g_rec_b>=l_ac THEN
              ...
              ELSE
                 ...
                 ELSE
                    ...
                    # add by lixwz 180131 s
                    IF g_aba.aba06 = 'GL' AND g_prog = 'aglt110' THEN
                       CALL cl_set_comp_entry("abb13",TRUE)
                    ELSE
                       CALL cl_set_comp_entry("abb13",FALSE)
                    END IF
                    # add by lixwz 180131 e
                    LET g_before_input_done=TRUE
                 END IF
              END IF
        ...
        AFTER FIELD abb03
            IF NOT cl_null(g_abb[l_ac].abb03) THEN
              #修改abb03時,先檢核沖帳檔abh_file,若已有資料則不可修改
               ...
               END IF
               # add by lixwz 180131 s
               IF g_aba.aba06 = 'GL' AND NOT cl_null(g_abb[l_ac].abb05) AND g_prog = 'aglt110' THEN
                   SELECT tc_afc04
                     INTO g_abb[l_ac].abb13
                     FROM tc_afc_file
                    WHERE tc_afc01 = g_abb[l_ac].abb05
                      AND tc_afc02 = g_abb[l_ac].abb03
                   IF cl_null(g_abb[l_ac].abb13) THEN LET g_abb[l_ac].abb13 = ' ' END IF
...
#No.FUN-A50092 --end
        AFTER FIELD abb05      #部門
            ...
            IF NOT cl_null(g_abb[l_ac].abb05) THEN
               IF m_aag05='Y' THEN
                   # add by lixwz 180131 s
                   IF g_aba.aba06 = 'GL' AND NOT cl_null(g_abb[l_ac].abb03) AND g_prog = 'aglt110' THEN
                       SELECT tc_afc04
                         INTO g_abb[l_ac].abb13
                         FROM tc_afc_file
                        WHERE tc_afc01 = g_abb[l_ac].abb05
...
#No.FUN-A50092 --end
        ...
        AFTER FIELD abb07f
            ...
            LET g_abb[l_ac].abb07 = cl_digcut(g_abb[l_ac].abb07,g_azi04)   #No.CHI-6A0004 g_azi-->t_azi #No.TQC-BB0246 modify:t_azi->g_azi
            CALL t110_bud(p_cmd,'0')
            IF NOT cl_null(g_errno) THEN
               NEXT FIELD abb07f
            END IF
...
FUNCTION t110_out()             # 查詢後整批列印, 印查詢出的數筆
   ...
   IF l_prog = 'gglr304' OR l_prog='aglr903'OR l_prog = 'aglr902' THEN #CHI-6C0018 add    #TQC-970225
  ...
  #LET l_wc = g_wc clipped," AND aba06!='RV' "  #MOD-AA0014 mark
   ...
   IF SQLCA.sqlcode OR l_wc2 IS NULL THEN
      IF g_msg = "aglr902" THEN
         LET l_wc2 = " '12' ' ' '3' '3' 'N' "
      ELSE
         IF g_msg = "aglr903" THEN
            LET l_wc2 = " '3' '3' 'N' '3' "
         ELSE
           #No:130711  Modify  <<--yangjian--
           #LET l_wc2 = " '3' '3' '3' 'N' "
            LET l_n = 0
   ...
   IF g_msg = 'aglr902' THEN
      LET l_cmd = l_prog CLIPPED,
                  ...
                  #" '",l_wc2 CLIPPED,"'"             #MOD-AA0014   #No.MO-B80009
                   "  ",l_wc2 CLIPPED                 #MOD-AA0014   #No.MO-B80009
   ELSE
      IF g_msg = 'aglr903' THEN   #MOD-6A0010
         LET l_cmd = l_prog CLIPPED,
                     #' "',g_bookno,'"',              #MOD-AA0014 mark
                      " '",g_bookno,"'",              #MOD-AA0014
                     #' "',g_dbs CLIPPED,'"',         #MOD-AA0014 mark
...
FUNCTION t110_z()
 ...
 #-------------MOD-C20031-----remark
  ...
  #FUN-BB0123--Begin--
  #被拋轉的共用單別的傳票不可取消確認
   LET l_sql = " SELECT COUNT(*) FROM aba_file,aac_file",
               "  WHERE aba16 = '",g_aba.aba00,"'",
               "    AND aba17 = '",g_aba.aba01,"'",
  ...
  #MOD-C40085---str-------
  #00帳有設定立沖科目(aag20='Y')且01帳已經確認時,則不可取消確認
  ...
  IF l_cnt = 2 THEN
    ...
    SELECT COUNT(*) INTO l_cnt FROM abb_file abb
     INNER JOIN aag_file aag ON abb.abb03=aag.aag01
     WHERE abb01=g_aba.aba01 AND aag20='Y' AND abb00=l_aba00

     IF l_cnt > 0 AND l_aba00 = g_bookno THEN
       SELECT COUNT(*) INTO l_cnt FROM aba_file aba
        WHERE abb01=g_aba.aba01
          AND aga19='Y' AND aba00=l_aba16
        IF l_cnt > 0 THEN
           CALL cl_err(g_aba.aba01,'aap-621',1)
           RETURN
        END IF
     END IF
  END IF
...
FUNCTION t110_y1()
   ...
   #  CALL cl_err('t110_yy1_pre',STATUS,0) LET g_success = 'N' RETURN    #TQC-B10069
      CALL s_errmsg(' ',' ','t110_yy1_pre',STATUS,1)   #TQC-B10069
      LET g_success = 'N'   #TQC-B10069
      RETURN                #TQC-B10069
   END IF
   ...
   FOREACH t110_yy1 INTO m_aba.*
      ...
      IF g_cnt = 0 THEN
         IF g_bgerr THEN
            CALL s_errmsg(m_aba.aba01,'','','arm-034',1)           #No.MOD-A20090
         ELSE
...
#No.FUN-AC0001 --end
   ...
   IF l_afb07  != '1' THEN
      ...
      IF m_aag06 = '1' THEN                     #借餘
         ...
         IF g_abb[l_ac].abb06 = '2' THEN
            LET l_fac = -1
         END IF
      ELSE                                      #貸餘
         LET total_t = l_tol1 - l_tol           #貸減借
         IF g_abb[l_ac].abb06 = '1' THEN
            LET l_fac = -1
         END IF
      END IF
  ...
  #IF g_action_choice != 'confirm' THEN                              #CHI-920071 #MOD-C20053 mark
   IF g_action_choice != 'confirm' OR cl_null(g_action_choice) THEN  #MOD-C20053 add
      IF p_cmd = 'a' THEN                                            #若本筆資料為新增則加上本次輸入的值
         LET total_t = total_t + g_abb[l_ac].abb07 * l_fac           #No.TQC-840044
      ELSE                                                           #若為更改則減掉舊值再加上新值
         LET total_t = total_t - g_abb_t.abb07 * l_fac + g_abb[l_ac].abb07 * l_fac  #No.TQC-840044
      END IF
   END IF    #CHI-920071
...
FUNCTION t110_set_no_entry_b(p_cmd)
  ...
  SELECT count(*) INTO l_n FROM aba_file
   ...
   ELSE
  #FUN-B50113---End---
     #IF g_aba.aba06 <> 'GL' AND g_aba.aba06 <> 'AC' AND g_aba.aba06 <> 'AD' AND g_aba.aba06 <> 'CE' THEN
      IF g_aba.aba06 <> 'GL' AND g_aba.aba06 <> 'AC' AND g_aba.aba06 <> 'AD' AND g_aba.aba06 <> 'CE' AND g_aba.aba06 <> 'FC' THEN
     ...
     IF NOT cl_null(l_ahe01) THEN
        ...
        IF NOT cl_null(l_ahe04) AND NOT cl_null(l_ahe05) AND
          #NOT cl_null(l_ahe07) THEN                       #MOD-B40231 mark
           NOT cl_null(l_ahe07) AND l_ahe03 = '1' THEN     #MOD-B40231
