...
#該程式非freestyle程式!
{<section id="afap310.description" type="s" >}
#應用 a00 樣板自動產生(Version:3)
#+ Standard Version.....: SD版次:0027(2014-12-01 14:14:50), PR版次:0027(2018-02-08 17:50:52)
#+ Customerized Version.: SD版次:0000(1900-01-01 00:00:00), PR版次:0000(1900-01-01 00:00:00)
#+ Build......: 000187
#+ Filename...: afap310
#+ Description: 固定資產月結作業
#+ Creator....: 05426(2014-09-08 20:25:32)
#+ Modifier...: 01531 -SD/PR- 00768
 
{</section>}
 
{<section id="afap310.global" type="s" >}
...
#170417-00045#1  2017/04/28 By 07900    筛选所有当月需要折旧的资产时，缺少主要类型和法人限定 导致 afap310报错：需折旧资产不存在折旧资料，不可月结！但afap230按此年期，查找不出需折旧的资料
#170704-00044#1  170710     By albireo  faan073  應存改良的預留殘值異動額
#170802-00009#1  2017/08/03 By 08729    當經由afap310產生完月結檔之後，用該畫面的年月回寫至aoos020
#170829-00006#1  2017/09/14 By 08729    資產狀態是折畢在提時，抓取faan020的地方從原本的faaj019改抓faaj012，本位幣二三也一併調整
#170927-00026#1  2017/10/27 By 07900    1.修改月结档, 当月出售、销账/报废(5,6)的也要写入
#                                       2.#12).如果数量都减完,标示已无此资产.不需要在汇入 月结档，#因此，删除faan006 = faan040 + faan050 + faan060 + faan080，此段逻辑mark掉
#171031-00035#1  2017/11/08 By 07900    现行年度、期别小于关账日时，年、期别报错有问题
#171219-00045#1  2017/12/20 By 05634    PREPARE afap310_faan015_pr的SQL補上x.faan007 = '7'條件
#171006-00010#12 2017/12/22 BY 10461    調整報錯做法，先結束交易再做報錯
#180108-00076#1  2018/01/09 By 05795    报废的资产afat507 没有产生月结资料
#180109-00073#1  2018/01/10 By 05634    帳面價值計算應該要納入異動處置累折
#180208-00025#1  2018/02/08 By 00768    需考虑afat523调整单
#end add-point
...
{<section id="afap310.main" type="s" >}
#+ 作業開始 
MAIN
...
{<section id="afap310.init" type="s" >}
#+ 畫面資料初始化
...
{<section id="afap310.ui_dialog" type="s" >}
#+ 選單功能實際執行處
...
#               END IF
               ...
               IF NOT cl_null(g_master.faan002) THEN
                  IF g_master.faan001 = l_yy  AND g_master.faan002 < l_mm THEN
...
{<section id="afap310.query" type="s" >}
#+ QBE資料查詢
...
{<section id="afap310.b_fill" type="s" >}
#+ 單身陣列填充
...
{<section id="afap310.fetch" type="s" >}
#+ 單身陣列填充2
...
{<section id="afap310.detail_show" type="s" >}
#+ 顯示相關資料
...
{<section id="afap310.filter" type="s" >}
#+ filter過濾功能
...
{<section id="afap310.filter_parser" type="s" >}
#+ filter欄位解析
...
{<section id="afap310.filter_show" type="s" >}
#+ Browser標題欄位顯示搜尋條件
...
{<section id="afap310.other_function" readonly="Y" type="s" >}
...
#170104-00050#1 mod e---
   ...
   IF NOT cl_null(p_ld) THEN 
      ...
      #170417-00045#1--add--s--xul      
      LET l_wc = " faah032 IN (SELECT glaacomp FROM glaa_t WHERE glaaent = ",g_enterprise," AND glaald = '",p_ld,"' ) "
      LET l_wc_1 = " SELECT faal001  FROM faal_t  WHERE faalent = ",g_enterprise," AND faalld = '",p_ld,"'"
      LET l_wc = l_wc," AND faah006 IN (",l_wc_1,")"
      #170417-00045#1--add--e--xul
      #筛选所有当月需要折旧的资产 
      LET l_sql = " SELECT faah015,faah006,faah003,faah004,faah001 ",   
                  ...
                  "    AND faaj003 != 5 ",         
                  "    AND faaj048 ='2' ",  #170417-00045#1 add ,
                  "    AND ",l_wc CLIPPED   #170417-00045#1 add xul
...
PRIVATE FUNCTION afap310_cycle_ld()
   ...
   IF g_success = 'N' THEN
     #  CALL cl_err_collect_show() #171006-00010#12 mark
        CALL s_transaction_end('N','0')
        CALL cl_err_collect_show() #171006-00010#12 add       
   ELSE
...
#重估/销账/报废/出售/调拨/改良/折毕再提
   ...
   LET g_sql = " SELECT SUM(fabh012-fabh011),SUM(fabh016-fabh015),SUM(fabh011),SUM(fabh015) ",
               ...
               "     ON fabgent = fabhent AND fabgld = fabhld AND fabgdocno =  fabhdocno ",
               "  WHERE fabhent = '",g_enterprise,"' ",
               "    AND fabg005 IN('8','9','6','21','39') ", #180208-00025#1 add '39'
               "    AND fabg005 = ? AND fabhld = ? AND fabh000 = ? AND fabh001 = ? AND fabh002 = ? ",
               "    AND fabgdocdt <= '",p_edate,"' AND fabgdocdt >= '",p_bdate,"' ", #20150203 add by chenying 
               "    AND fabgstus = 'S' " 
...
#afat503/508异动未折减额= 异动成本fabh010-异动累折(fabh012-fabh011)
   ...
   LET g_sql = " SELECT SUM(fabh006),SUM(fabh010),SUM(fabh010-fabh012+fabh011),SUM(fabh014-fabh013) FROM fabg_t LEFT OUTER JOIN fabh_t ", #20150129 原抓成本fabh008,未折减额fabh004 改成异动后的
               "     ON fabgent = fabhent AND fabgld = fabhld AND fabgdocno =  fabhdocno ",
               "  WHERE fabhent = '",g_enterprise,"' ",
               "    AND fabg005 IN('8','9','39') AND fabg005 = ? AND fabhld = ? AND fabh000 = ? AND fabh001 = ? AND fabh002 = ? ", #180208-00025#1 add '39'
               "    AND fabgdocdt <= '",p_edate,"' AND fabgdocdt >= '",p_bdate,"' ", #20150203 add by chenying 
               "    AND fabgstus = 'S' " 
...
#######累计折旧#预留残值#######################################################################
      ...
      LET l_fabh011_503 = 0   
      LET l_fabh015_503 = 0      
      FOREACH afap310_cs_yd_fabh_6 USING '39',l_faaj.faajld,l_faah.faah001,l_faah.faah003,l_faah.faah004 
                                    INTO l_fabh011,l_fabh015,l_fabh011_1,l_fabh015_1
         IF SQLCA.sqlcode THEN
            INITIALIZE g_errparam TO NULL
            LET g_errparam.code = SQLCA.sqlcode
...
#######数量#成本#年限#未折减额#################################################################   
      ...
      LET l_fabh004_503 = 0 
      LET l_fabh013_503 = 0    
      FOREACH afap310_cs_yd_fabh_3 USING '39',l_faaj.faajld,l_faah.faah001,l_faah.faah003,l_faah.faah004 
                                  INTO l_fabh007,l_fabh008,l_fabh004,l_fabh013 
         IF SQLCA.sqlcode THEN
            INITIALIZE g_errparam TO NULL
            LET g_errparam.code = SQLCA.sqlcode
         ...
         LET l_fabh018 = NULL  
         EXECUTE afap310_cs_yd_fabh018 USING '39',l_faaj.faajld,l_faah.faah001,l_faah.faah003,l_faah.faah004 
                                  INTO l_fabh018                     
         IF cl_null(l_fabh018) THEN LET l_fabh018 = 1 END IF                       
         IF l_fabh018 = 2 THEN 
            LET l_fabh008 =  l_fabh008 * (-1)
...
PRIVATE FUNCTION afap310_get_faan2(p_glaald,p_glaacomp,p_bdate,p_edate)
   ...
   #170927-00026#1--add--s-xul
   DEFINE l_sql1             STRING 
   DEFINE l_sql2             STRING 
   #170927-00026#1--add--e--xul
   ...
   LET g_sql = "INSERT INTO faan_t (faanent,faancomp,faansite,faan001,faan002,",
               ...
               #账面余额\账面净值\账面价值\累计折旧\减值准备\预留残值
               #"       faaj016+faaj020,faaj016+faaj020-faaj017-faaj034,0,faaj017,faaj021,faaj019,",      #170207-00026#1 change faaj016 to faaj016+faaj020 #170829-00006#1 mark
               "        faaj016+faaj020,faaj016+faaj020-faaj017-faaj034,0,faaj017,faaj021,", #170829-00006#1 add  
               #當資產狀態是折畢再提時，改抓折畢再提預留殘值欄位
               "        CASE WHEN faah015 = '7' THEN faaj012 ELSE faaj019 END ,",            #170829-00006#1 add       
               #本位币二账面余额\本位币二账面净值\本位币二账面价值\本位币二累计折旧\本位币二减值准备\本位币二预留残值
               #"       faaj103+faaj117,faaj103+faaj117-faaj104-faaj113,0,faaj104,faaj112,faaj105,",      #170207-00026#1 change faaj103 to faaj103+faaj117  #170829-00006#1 mark
               "        faaj103+faaj117,faaj103+faaj117-faaj104-faaj113,0,faaj104,faaj112,", #170829-00006#1 add  
               #當資產狀態是折畢再提時，改抓折畢再提預留殘值欄位
               "        CASE WHEN faah015 = '7' THEN faaj106 ELSE faaj105 END ,",            #170829-00006#1 add  
               #本位币三账面余额\本位币三账面净值\本位币三账面价值\本位币三累计折旧\本位币三减值准备\本位币三预留残值
               #"       faaj153+faaj167,faaj153+faaj167-faaj154-faaj163,0,faaj154,faaj162,faaj155,",      #170207-00026#1 change faaj153 to faaj153+faaj167  #170829-00006#1 mark
               "        faaj153+faaj167,faaj153+faaj167-faaj154-faaj163,0,faaj154,faaj162,", #170829-00006#1 add 
               #當資產狀態是折畢再提時，改抓折畢再提預留殘值欄位
               "        CASE WHEN faah015 = '7' THEN faaj156 ELSE faaj155 END ,",            #170829-00006#1 add                 
                        l_sql_faan091,",",
               #从此处栏位往下，金额默认=0
               "        0,0,",
               "        0,0,0,0,0,0,",
   ...
   #170927-00026#1--add--s--xul
   #出售
   LET l_sql1 =  #企业编号\法人\资产中心\年度\期别
               " SELECT faahent,'",p_glaacomp,"','",g_master.faansite,"',",g_master.faan001,",",g_master.faan002,",",
               #卡片编号\财产编号\附号\账套\资产状态\（列账/列管）
               "        faah001,faah003,faah004,faajld,faah015,faaj048,", 
               #数量\币别\汇率\耐用年限\未使用年限
               "        faah018,faaj014,faaj015,faaj004,faaj005,", 
               #本位币二币别\本位币二汇率\本位币三币别\本位币三汇率
               "        faaj101,NVL(faaj102,0),faaj151,NVL(faaj152,0),",
               #账面余额\账面净值\账面价值\累计折旧\减值准备\预留残值
               "        faaj016+faaj020,faaj016+faaj020-faaj017-faaj034,0,faaj017,faaj021,",  
               #當資產狀態是折畢再提時，改抓折畢再提預留殘值欄位
               "        CASE WHEN faah015 = '7' THEN faaj012 ELSE faaj019 END ,",                
               #本位币二账面余额\本位币二账面净值\本位币二账面价值\本位币二累计折旧\本位币二减值准备\本位币二预留残值
               "        faaj103+faaj117,faaj103+faaj117-faaj104-faaj113,0,faaj104,faaj112,",   
               #當資產狀態是折畢再提時，改抓折畢再提預留殘值欄位
               "        CASE WHEN faah015 = '7' THEN faaj106 ELSE faaj105 END ,",              
               #本位币三账面余额\本位币三账面净值\本位币三账面价值\本位币三累计折旧\本位币三减值准备\本位币三预留残值
               "        faaj153+faaj167,faaj153+faaj167-faaj154-faaj163,0,faaj154,faaj162,",  
               #當資產狀態是折畢再提時，改抓折畢再提預留殘值欄位
               "        CASE WHEN faah015 = '7' THEN faaj156 ELSE faaj155 END ,",                          
                        l_sql_faan091,",",
               #从此处栏位往下，金额默认=0
               "        0,0,",
               "        0,0,0,0,0,0,",
               ...
               "    AND faah003 = faaj001 AND faah004 = faaj002 ",
               "    AND faah001 = faaj037 ",
               "   LEFT JOIN fabo_t ON faboent = faahent AND fabo001 =faah003  ",
               "    AND fabo003 = faah001 AND fabo002 = faah004  ",
               "   LEFT JOIN fabg_t ON fabgent = faboent AND fabgld = fabold  ",
               "    AND  fabodocno = fabgdocno AND fabg005 = '4' ",
               "  WHERE faahent = ",g_enterprise," AND faajld = '",p_glaald,"'",
               "    AND faah015 = '5' AND  fabgstus = 'S' ",
               "    AND fabgdocdt BETWEEN '",p_bdate,"' AND '",p_edate,"'",
               "    AND faahstus = 'Y' AND faajstus = 'Y' "
   #销账报废            
   LET l_sql2 =   #企业编号\法人\资产中心\年度\期别
               " SELECT faahent,'",p_glaacomp,"','",g_master.faansite,"',",g_master.faan001,",",g_master.faan002,",",
               #卡片编号\财产编号\附号\账套\资产状态\（列账/列管）
               "        faah001,faah003,faah004,faajld,faah015,faaj048,", 
               #数量\币别\汇率\耐用年限\未使用年限
               ...
               "        faaj101,NVL(faaj102,0),faaj151,NVL(faaj152,0),",
               #账面余额\账面净值\账面价值\累计折旧\减值准备\预留残值
               #"        faaj016+faaj020,faaj016+faaj020-faaj017-faaj034,0,faaj017,faaj021,",   #180109-00073#1 mark
               "        faaj016+faaj020,faaj016+faaj020-faaj017-(faaj034-faaj035),0,faaj017,faaj021,",    #180109-00073#1 add  
               #當資產狀態是折畢再提時，改抓折畢再提預留殘值欄位
               "        CASE WHEN faah015 = '7' THEN faaj012 ELSE faaj019 END ,",                
               #本位币二账面余额\本位币二账面净值\本位币二账面价值\本位币二累计折旧\本位币二减值准备\本位币二预留残值
               "        faaj103+faaj117,faaj103+faaj117-faaj104-faaj113,0,faaj104,faaj112,",   
               ...
               "    AND faah003 = faaj001 AND faah004 = faaj002 ",
               "    AND faah001 = faaj037 ",
#               #180108-00076#1---mark---str
#               "   LEFT JOIN fabo_t ON faboent = faahent AND fabo001 =faah003 ",
#               "    AND fabo003 = faah001 AND fabo002 = faah004 ",
#               "   LEFT JOIN fabg_t ON fabgent = faboent AND fabgld = fabold  ",
#               "    AND  fabodocno = fabgdocno AND (fabg005 = '6' OR fabg005 = '21')",
#               #180108-00076#1---mark---end
               #180108-00076#1---add---str---->报废afat507用的是fabh_t
               "   LEFT JOIN fabh_t ON fabhent = faahent AND fabh001 =faah003 ",
               "    AND fabh000 = faah001 AND fabh002 = faah004 ",
               "   LEFT JOIN fabg_t ON fabgent = fabhent AND fabgld = fabhld  ",
               "    AND  fabhdocno = fabgdocno AND (fabg005 = '6' OR fabg005 = '21')",
               #180108-00076#1---add---end
               "  WHERE faahent = ",g_enterprise," AND faajld = '",p_glaald,"'",
               "    AND faah015 = '6' AND  fabgstus = 'S' ",
               "    AND fabgdocdt BETWEEN '",p_bdate,"' AND '",p_edate,"'",
               "    AND faahstus = 'Y' AND faajstus = 'Y' "
               
   LET g_sql = g_sql," UNION ",l_sql1," UNION ",l_sql2       
   #170927-00026#1--add--e--xul
   ...
   LET g_sql = "MERGE INTO faan_t x",
               ...
               " WHERE x.faanent=",g_enterprise," AND x.faan001=",g_master.faan001,
               "   AND x.faan002=",g_master.faan002," AND x.faanld='",p_glaald,"'"
   PREPARE afap310_faan030_pr1 FROM g_sql
   EXECUTE afap310_faan030_pr1 USING '39'
   IF SQLCA.SQLCODE THEN
      INITIALIZE g_errparam TO NULL
      LET g_errparam.extend = "update faan030" 
      LET g_errparam.code   = SQLCA.sqlcode 
   ...
   #改良異動預留殘值faan073/faan161/faan261 = SUM(fabh016-fabh015)    #170704-00044#1 modify fabh016-fabh015
   ...
   LET g_sql = "MERGE INTO faan_t x",
               ...
               #"            x.faan073 = NVL(y.fabh015,0),",    #170704-00044#1 mark
               "            x.faan073 = NVL(y.fabh016_15,0),",  #170704-00044#1
...
#   END IF 
   ...
   #170829-00006#1 add(s)
   #當月資產狀態是7.折畢在提，上個月不是時，faan020要抓faaj012的值，並重新UPDATE faan020
   #當月資產狀態是7.折畢再提，上個月也是時，faan020就用原來上面的邏輯
   LET g_sql = "MERGE INTO faan_t x",
               " USING( SELECT  faan003,faan004,faan005,faan007,faan020 ",
               "          FROM faan_t ",
               "         WHERE faanent=",g_enterprise," AND faan001=",l_yy," AND faan002=",l_mm,
               "           AND faanld='",p_glaald,"'",
               "       ) y",
               #" ON (x.faan003=y.faan003 AND x.faan004=y.faan004 AND x.faan005=y.faan005 AND x.faan007 <> y.faan007) ",   #171219-00045#1 mark
               " ON (x.faan003=y.faan003 AND x.faan004=y.faan004 AND x.faan005=y.faan005 AND x.faan007 <> y.faan007 AND x.faan007 = '7') ",   #171219-00045#1 add
               " WHEN MATCHED THEN ",
               " UPDATE SET x.faan020 = (SELECT faaj012 FROM faaj_t WHERE faajent=",g_enterprise," AND faaj001=x.faan004 AND faajld= x.faanld AND faaj002=x.faan005),",
               #本位币二
               "            x.faan108 = (SELECT faaj106 FROM faaj_t WHERE faajent=",g_enterprise," AND faaj001=x.faan004 AND faajld= x.faanld AND faaj002=x.faan005),",
               #本位币三
               "            x.faan208 = (SELECT faaj156 FROM faaj_t WHERE faajent=",g_enterprise," AND faaj001=x.faan004 AND faajld= x.faanld AND faaj002=x.faan005)",
               " WHERE x.faanent=",g_enterprise," AND x.faan001=",g_master.faan001,
               "   AND x.faan002=",g_master.faan002," AND x.faanld='",p_glaald,"'"
   PREPARE afap310_faan015_pr FROM g_sql
   EXECUTE afap310_faan015_pr
   IF SQLCA.SQLCODE THEN
      INITIALIZE g_errparam TO NULL
      LET g_errparam.extend = "update faan020" 
      LET g_errparam.code   = SQLCA.sqlcode 
      LET g_errparam.popup  = TRUE 
      CALL cl_err()
      LET g_success = 'N'
   END IF             
   #170829-00006#1 add(e)
   ...
   IF g_glaa.glaa014 = 'Y' THEN   
      LET l_yy = NULL
      LET l_mm = NULL
      #LET l_yy = cl_get_para(g_enterprise,p_glaacomp,'S-FIN-9018') #170802-00009#1 mark
      #LET l_mm = cl_get_para(g_enterprise,p_glaacomp,'S-FIN-9019') #170802-00009#1 mark
      LET l_yy = g_master.faan001 #170802-00009#1 add
      LET l_mm = g_master.faan002 #170802-00009#1 add
      IF l_mm = 12 THEN 
         LET l_yy = l_yy + 1
         LET l_mm = 1
      ELSE
