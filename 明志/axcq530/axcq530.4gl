...
#該程式非freestyle程式!
{<section id="axcq530.description" type="s" >}
#應用 a00 樣板自動產生(Version:3)
#+ Standard Version.....: SD版次:0030(2017-11-10 16:42:44), PR版次:0030(2017-12-07 16:57:01)
#+ Customerized Version.: SD版次:0000(1900-01-01 00:00:00), PR版次:0000(1900-01-01 00:00:00)
#+ Build......: 000156
#+ Filename...: axcq530
#+ Description: 庫存成本多筆查詢作業
#+ Creator....: 03297(2014-12-25 10:06:49)
#+ Modifier...: 09767 -SD/PR- 10500

{</section>}

{<section id="axcq530.global" type="s" >}
#應用 i07 樣板自動產生(Version:68)
...
#170511-00021#1   2017/05/11  By 02295    走批次成本时,批号为空格的不需要检查aini010是否存在
#170427-00041#4   2017/05/12  By 08734    axcq查询作业，取平均值、小计及总计等再计算栏位的金额取位，需按照aooi150处的【成本金额小数字数】栏位的设置取位
#170606-00042#1   2017/06/08  By lixiang  【期初单位成本】栏位不用做合计
#170616-00088#1   2017/06/26  By 02111    重工領出 xccc207、xccc208 要算在本期出庫。
#170522-00111#1   2017/07/17  By zhujing  效能优化
#170914-00003#1   2017/09/26  By 02111    調整將 imag_t 組到 xccc_t 。
#170927-00015#1   2017/11/10  by 09767    成本单位、期初单位成本改成查询时可录入查询条件
#171201-00043#5   2017/12/07  By 10500     还原170427-00041的取位调整
#end add-point
...
#DEFINE g_glaa001             LIKE glaa_t.glaa001   #170427-00041#4 add #171201-00043#5 mask
#end add-point
...
{<section id="axcq530.main" type="s" >}
...
{<section id="axcq530.init" type="s" >}
#+ 瀏覽頁簽資料初始化
...
{<section id="axcq530.ui_dialog" type="s" >}
#+ 功能選單
               ...
               CASE
                  WHEN lb_get_type = "poi"   #使用poi樣板
                     ...
                     ELSE
                        ...
                        LET l_arr_len = g_xccc2_d.getLength()
                        LET g_export_tag[2] = rec_arr_type.create(l_arr_len)
                        LET g_export_tabname[2] = "s_detail2"
                        FOR l_arr_cnt = 1 TO l_arr_len
                           LET g_export_tag[2][l_arr_cnt] = g_xccc2_d[l_arr_cnt]
                        END FOR
         ...
         ON ACTION queryplansel
            ...
            IF NOT cl_null(ls_wc) THEN
               ...
               FOR li_idx = 1 TO la_wc.getLength()
                  CASE
                     WHEN la_wc[li_idx].tableid = "xccc_t"
...
{<section id="axcq530.browser_search" type="s" >}
#+ 瀏覽頁簽資料搜尋
...
{<section id="axcq530.browser_fill" type="s" >}
#+ 瀏覽頁簽資料填充
...
# #                   ", xccc005 ",    #fengmy150812  mark
  ...
  #151130-00003#1--add--(s)
   ...
   #170927-00015#1 add by 09767-->s
   IF g_wc2.getIndexOf("l_unit",1)THEN
      LET g_wc2 = cl_replace_str(g_wc2,'l_unit','DECODE(xccc101,0,0,xccc102/xccc101)')
   END IF
   #170927-00015#1 add by 09767-->e
   IF (g_wc2 <> " 1=1" AND NOT cl_null(g_wc2)) OR NOT cl_null(g_wc_filter) THEN
      ...
      LET l_sub_sql = l_sub_sql CLIPPED , " LEFT OUTER JOIN xcbb_t t2 ON t2.xcbbent = xcccent AND t2.xcbbcomp = xccccomp AND t2.xcbb001 = xccc004 AND t2.xcbb002 = xccc005 AND t2.xcbb003 = xccc006 "    #170927-00015#1 add by 09767
   END IF
...
#wujie 150604 --end
   ...
   #170927-00015#1 add by 09767-->s
   IF l_wc2.getIndexOf("l_unit",1)THEN
      LET l_wc2 = cl_replace_str(l_wc2,'l_unit','DECODE(xccc101,0,0,xccc102/xccc101)')
   END IF
   #170927-00015#1 add by 09767-->e
   LET g_sql  = "SELECT DISTINCT t0.xcccld,t0.xccc001,t0.xccc003,'','' ",  #,t0.xccc004,t0.xccc005",   #fengmy150812  mark
               ...
               #" LEFT OUTER JOIN imag_t t1 ON t1.imagent = '"||g_enterprise||"' AND t1.imagsite = '"||g_xccc_m.xccccomp||"' AND t1.imag001 = xccc006 ",  #151130-00002#1  mark
                ...
                " LEFT OUTER JOIN xcbb_t t2 ON t2.xcbbent = xcccent AND t2.xcbbcomp = xccccomp AND t2.xcbb001 = xccc004 AND t2.xcbb002 = xccc005 AND t2.xcbb003 = xccc006 ",    #170927-00015#1 add by 09767
                " WHERE t0.xcccent = '" ||g_enterprise|| "' AND ", l_wc," AND ",l_wc2, cl_sql_add_filter("xccc_t")
...
{<section id="axcq530.ui_headershow" type="s" >}
#+ 單頭資料重新顯示
...
{<section id="axcq530.ui_detailshow" type="s" >}
#+ 單身資料重新顯示
...
{<section id="axcq530.ui_browser_refresh" type="s" >}
#+ 瀏覽頁簽資料重新顯示
...
{<section id="axcq530.construct" type="s" >}
#+ QBE資料查詢
...
#         #應用 a03 樣板自動產生(Version:3)
      ...
      CONSTRUCT g_wc2_table1 ON imag011,xccc002,xccc006,xccc007,xccc008,xcbb005,xccc101,l_unit,xccc102,
          xccc201,xccc202,xccc280,xccc280a,xccc280b,xccc280c,xccc280d,xccc280e,xccc280f,xccc280g,xccc280h,
          xccc301,xccc302,xccc901,xccc902,xccc903,xccc203,xccc204,xccc205,xccc206,xccc207,xccc208,xccc209,
          xccc210,xccc211,xccc212,xccc213,xccc214,xccc215,xccc216,xccc217,xccc218,xccc303,xccc304,xccc305,
          xccc306,xccc307,xccc308,xccc309,xccc310,xccc311,xccc312,xccc313,xccc314
           FROM s_detail1[1].imag011,s_detail1[1].xccc002,s_detail1[1].xccc006,s_detail1[1].xccc007,
               s_detail1[1].xccc008,s_detail1[1].xcbb005,s_detail1[1].xccc101,s_detail1[1].l_unit,s_detail1[1].xccc102,
               s_detail1[1].xccc201,s_detail1[1].xccc202,s_detail1[1].xccc280,s_detail1[1].xccc280a,
               s_detail1[1].xccc280b,s_detail1[1].xccc280c,s_detail1[1].xccc280d,s_detail1[1].xccc280e,
               s_detail1[1].xccc280f,s_detail1[1].xccc280g,s_detail1[1].xccc280h,s_detail1[1].xccc301,
               s_detail1[1].xccc302,s_detail1[1].xccc901,s_detail1[1].xccc902,s_detail1[1].xccc903,s_detail2[1].xccc203,
               s_detail2[1].xccc204,s_detail2[1].xccc205,s_detail2[1].xccc206,s_detail2[1].xccc207,s_detail2[1].xccc208,
               s_detail2[1].xccc209,s_detail2[1].xccc210,s_detail2[1].xccc211,s_detail2[1].xccc212,s_detail2[1].xccc213,
               s_detail2[1].xccc214,s_detail2[1].xccc215,s_detail2[1].xccc216,s_detail2[1].xccc217,s_detail2[1].xccc218,
               s_detail2[1].xccc303,s_detail2[1].xccc304,s_detail2[1].xccc305,s_detail2[1].xccc306,s_detail2[1].xccc307,
               s_detail2[1].xccc308,s_detail2[1].xccc309,s_detail2[1].xccc310,s_detail2[1].xccc311,s_detail2[1].xccc312,
               s_detail2[1].xccc313,s_detail2[1].xccc314
...
{<section id="axcq530.query" type="s" >}
#+ 資料查詢QBE功能準備
...
{<section id="axcq530.fetch" type="s" >}
#+ 指定PK後抓取單頭其他資料
...
{<section id="axcq530.insert" type="s" >}
#+ 資料新增
...
{<section id="axcq530.modify" type="s" >}
#+ 資料修改
...
{<section id="axcq530.input" type="s" >}
#+ 資料輸入
...
{<section id="axcq530.input.head" type="s" >}
...
{<section id="axcq530.input.body" type="s" >}
      #Page1 預設值產生於此處
               ...
               #add-point:單身修改前 name="input.body.b_update"
{
               ...
               CALL axcq530_update_b('xccc_t',gs_keys,gs_keys_bak,"'1'")
                     #資料多語言用-增/改
                        ...
                        #161130-00056
                     #修改歷程記錄(單身修改)
                     CALL axcq530_set_all_pk_array()   #(ver:50)
                     ...
                     IF NOT cl_log_modified_record(g_log1,g_log2) THEN
                        CALL s_transaction_end('N','0')
                     END IF
                     CALL axcq530_set_pk_array()   #(ver:50)
...
#         #應用 a03 樣板自動產生(Version:3)
      ...
      BEFORE DIALOG
         ...
         #新增時強制從單頭開始填
         IF p_cmd = 'a' THEN
            NEXT FIELD xcccld #170906-00011#1
         ELSE
            CASE g_aw
               WHEN "s_detail1"
                  NEXT FIELD imag011 #170906-00011#1
               WHEN "s_detail2"
                  NEXT FIELD l_unit_2 #170906-00011#1
...
{<section id="axcq530.show" type="s" >}
#+ 單頭資料重新顯示及單身資料重抓
...
{<section id="axcq530.ref_show" type="s" >}
#+ 帶出reference資料
...
{<section id="axcq530.reproduce" type="s" >}
#+ 資料複製
...
{<section id="axcq530.detail_reproduce" type="s" >}
#+ 單身自動複製
...
#  DROP TABLE axcq530_detail   #180110-00034 mark

END FUNCTION

{</section>}

{<section id="axcq530.delete" type="s" >}
#+ 資料刪除
PRIVATE FUNCTION axcq530_delete()
   ...
   IF cl_getField("xccc_t","") THEN
      ...
      PREPARE axcq530_chk_own_pre FROM g_sql
      DECLARE axcq530_chk_own_cus CURSOR FOR axcq530_chk_own_pre

      FOREACH axcq530_chk_own_cus INTO g_data_owner,g_data_dept
         IF SQLCA.SQLCODE THEN
            INITIALIZE g_errparam TO NULL
   ...
   IF cl_ask_del_master() THEN              #確認一下
      ...
      #(ver:50) 將mark拿掉 ---start---
      #頭先不紀錄
      LET g_log1 = util.JSON.stringify(g_xccc_m_t)   #(ver:50)
      #執行刪除時，不會有新值，所以新值欄位傳送空白             #(ver:50)
   #  LET g_log2 = util.JSON.stringify(g_xccc_m)     #(ver:50)
...
{<section id="axcq530.b_fill" type="s" >}
#+ 單身陣列填充
...
{<section id="axcq530.idx_chk" type="s" >}
#+ 顯示正確的單身資料筆數
...
{<section id="axcq530.b_fill2" type="s" >}
#+ 單身陣列填充2
...
{<section id="axcq530.before_delete" type="s" >}
#+ 單身db資料刪除
...
{<section id="axcq530.delete_b" type="s" >}
#+ 刪除單身後其他table連動
...
{<section id="axcq530.insert_b" type="s" >}
#+ 新增單身後其他table連動
...
{<section id="axcq530.update_b" type="s" >}
#+ 修改單身後其他table連動
...
{<section id="axcq530.key_update_b" type="s" >}
...
{<section id="axcq530.key_delete_b" type="s" >}
...
{<section id="axcq530.lock_b" type="s" >}
#+ 連動lock其他單身table資料
...
{<section id="axcq530.unlock_b" type="s" >}
#+ 連動unlock其他單身table資料
...
{<section id="axcq530.set_entry" type="s" >}
#+ 單頭欄位開啟設定
...
{<section id="axcq530.set_no_entry" type="s" >}
#+ 單頭欄位關閉設定
...
{<section id="axcq530.set_entry_b" type="s" >}
#+ 單身欄位開啟設定
...
{<section id="axcq530.set_no_entry_b" type="s" >}
#+ 單身欄位關閉設定
...
{<section id="axcq530.set_act_visible" type="s" >}
#+ 單頭權限開啟
...
{<section id="axcq530.set_act_no_visible" type="s" >}
#+ 單頭權限關閉
...
{<section id="axcq530.set_act_visible_b" type="s" >}
#+ 單身權限開啟
...
{<section id="axcq530.set_act_no_visible_b" type="s" >}
#+ 單身權限關閉
...
{<section id="axcq530.default_search" type="s" >}
#+ 外部參數搜尋
...
{<section id="axcq530.fill_chk" type="s" >}
#+ 單身填充確認
...
{<section id="axcq530.modify_detail_chk" type="s" >}
#+ 單身輸入判定
...
{<section id="axcq530.mask_functions" type="s" >}
&include "erp/axc/axcq530_mask.4gl"

{</section>}

{<section id="axcq530.state_change" type="s" >}


{</section>}

{<section id="axcq530.set_pk_array" type="s" >}
...
{<section id="axcq530.msgcentre_notify" type="s" >}
...
{<section id="axcq530.other_function" readonly="Y" type="s" >}
#單身g_xccc_d填充
...
#wujie 150604 --end
   ...
   LET g_sql = "SELECT UNIQUE imag011, ",
               ...
               #170927-00015#1 mark by 09767-->s
               ...
               #170927-00015#1 mark by 09767-->e
               " ,xcbb005 " ,     #170927-00015#1 add  by 09767
               " FROM ( ",
   ...
   #160520-00003#5--e
               ...
               #170616-00088#1 mark end   -----
               #170616-00088#1 add start -----
               "               (xccc201+xccc203+xccc205+xccc209+xccc211+xccc213+xccc215+xccc217) xccc201 ,",
               "               (xccc202+xccc204+xccc206+xccc210+xccc212+xccc214+xccc216+xccc218) xccc202 ,xccc280,",
               #170616-00088#1 add end   -----
               "               xccc280a,xccc280b,xccc280c,xccc280d,xccc280e,xccc280f,xccc280g,xccc280h,",   #fengmy150519 add
               #170616-00088#1 mark start -----
               ...
               #170616-00088#1 mark end   -----
               #170616-00088#1 add start -----
               "               (xccc301+xccc303+xccc305+xccc307+xccc309+xccc311+xccc313+xccc207) xccc301 ,",
               "               (xccc302+xccc304+xccc306+xccc308+xccc310+xccc312+xccc314+xccc208) xccc302 ,xccc901,xccc902,xccc903,",
               #170616-00088#1 add end   -----
...
#               "               t3.imaal003 ,t4.imaal004 ,t5.imaal004 FROM axcq530_tmp1 t0",   #fengmy150812
               ...
               #160520-00003#5--e
               #"   FROM axcq530_tmp1 t0"  #170927-00015#1 mark by 09767
...
#               " LEFT JOIN imaal_t t5 ON t5.imaalent='"||g_enterprise||"' AND t5.imaal001=t0.xccc006 AND t5.imaal002='"||g_dlang||"' "
              ...
              #
              #170927-00015#1 add by 09767-->s
              "   FROM axcq530_tmp1 t0 LEFT OUTER JOIN xcbb_t ON xcbbent='",g_enterprise,"' AND xcbbcomp='",g_xccc_m.xccccomp,"' AND xcbb001=",g_yy2," AND xcbb002=",g_mm2,
              "               AND xcbb003=t0.xccc006 AND xcbb004=t0.xccc007"
              #170927-00015#1 add by 09767-->e
...
#wujie 150604 --begin
   ...
   SELECT xcat005 INTO l_xcat005    #l_xcat005 =3表示是批次成本，单身填充时要去找出门店和供应商
    ...
    WHERE xcatent = g_enterprise
      AND xcat001 = g_xccc_m.xccc003
   IF l_xcat005 = '3' THEN
      #170522-00111#1 mod-S 将抓取说明的部分直接组合到sql中
...
#                   "  GROUP BY inadent,inad001,inad002,inad003"
      LET l_sql1 = " SELECT inadsite,(SELECT ooefl003 FROM ooefl_t WHERE ooefl001 = inadsite AND ooefl002 = '",g_dlang,"' AND ooeflent = inadent),",
                   "        inad010,(SELECT pmaal003 FROM pmaal_t WHERE pmaal001 = inad010 AND pmaal002 = '",g_dlang,"' AND pmaalent = inadent) ",
                   "   FROM ( ",
                   " SELECT inadent,inad001,inad002,inad003,MIN(inadsite),MIN(inad010) FROM inad_t ",
                   "  WHERE inadent = ? ",
                   "    AND inad001 = ? ",
                   "    AND inad002 = ? ",
                   "    AND inad003 = ? ",
                   "    AND ",g_wc3,
                   "  GROUP BY inadent,inad001,inad002,inad003",
                   " ) WHERE inadsite IS NOT NULL AND inad010 IS NOT NULL "
      #170522-00111#1 mod-E
...
#170309-00049#1 mark end   -----
   ...
   #170927-00015#1 add by 09767-->s
   IF g_wc2_table1.getIndexOf("l_unit",1)THEN
      LET g_wc2_table1 = cl_replace_str(g_wc2_table1,'l_unit','DECODE(xccc101,0,0,xccc102/xccc101)')
   END IF
   #170927-00015#1 add by 09767-->e
...
#   LET g_glaa001=''  #170427-00041#4 add #171201-00043#5 mask
#   SELECT glaa001 INTO g_glaa001 FROM glaa_t WHERE glaaent=g_enterprise AND glaald=g_xccc_m.xcccld  #170427-00041#4 add #171201-00043#5 mask
...
#wujie 150604 --begin
         IF l_xcat005 = '3' THEN
            IF NOT cl_null(g_xccc_d[l_ac].xccc008) THEN #170511-00021#1 add
               #170522-00111#1 mod-S
...
##               LET g_xccc2_d[l_ac].inad010_2_desc = s_desc_get_trading_partner_full_desc(g_xccc2_d[l_ac].inad010)   #mark--2015/10/07 By shiun
               EXECUTE axcq530_inad INTO g_xccc_d[l_ac].inadsite,g_xccc_d[l_ac].inadsite_desc,g_xccc_d[l_ac].inad010,g_xccc_d[l_ac].inad010_desc
                                      USING g_enterprise,g_xccc_d[l_ac].xccc006,g_xccc_d[l_ac].xccc007,g_xccc_d[l_ac].xccc008
               #170522-00111#1 mod-E
            END IF  #170511-00021#1 add
         END IF
#wujie 150604 --end
...
#               LET g_xccc2_d[l_ac].xccc006 = cl_getmsg("axc-00205",g_lang)
               ...
               #LET g_xccc_d[l_ac].l_unit = l_unit1_sum - g_xccc_d[l_ac + 1].l_unit      #dorislai-20151104-add  #170606-00042#1 mark
               LET g_xccc_d[l_ac].l_unit = NULL     #170606-00042#1 add
               ...
               #mark--2015/10/07 By shiun--(S)
               #IF cl_null(g_xccc_d[l_ac].l_unit) THEN LET g_xccc_d[l_ac].l_unit=0 END IF  #170606-00042#1 mark
               IF cl_null(g_xccc_d[l_ac].xccc102) THEN LET g_xccc_d[l_ac].xccc102=0 END IF
               IF cl_null(g_xccc_d[l_ac].xccc202) THEN LET g_xccc_d[l_ac].xccc202=0 END IF
               IF cl_null(g_xccc_d[l_ac].xccc280) THEN LET g_xccc_d[l_ac].xccc280=0 END IF
               IF cl_null(g_xccc_d[l_ac].xccc280a) THEN LET g_xccc_d[l_ac].xccc280a=0 END IF
               IF cl_null(g_xccc_d[l_ac].xccc280b) THEN LET g_xccc_d[l_ac].xccc280b=0 END IF
               IF cl_null(g_xccc_d[l_ac].xccc280c) THEN LET g_xccc_d[l_ac].xccc280c=0 END IF
               IF cl_null(g_xccc_d[l_ac].xccc280d) THEN LET g_xccc_d[l_ac].xccc280d=0 END IF
               IF cl_null(g_xccc_d[l_ac].xccc280e) THEN LET g_xccc_d[l_ac].xccc280e=0 END IF
               IF cl_null(g_xccc_d[l_ac].xccc280f) THEN LET g_xccc_d[l_ac].xccc280f=0 END IF
               IF cl_null(g_xccc_d[l_ac].xccc280g) THEN LET g_xccc_d[l_ac].xccc280g=0 END IF
               IF cl_null(g_xccc_d[l_ac].xccc280h) THEN LET g_xccc_d[l_ac].xccc280h=0 END IF
               IF cl_null(g_xccc_d[l_ac].xccc302) THEN LET g_xccc_d[l_ac].xccc302=0 END IF
               ...
               #CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,g_xccc_d[l_ac].l_unit,'5') RETURNING g_xccc_d[l_ac].l_unit  #170606-00042#1 mark
               #171201-00043#5 mask-S
#               CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,g_xccc_d[l_ac].xccc102,'5') RETURNING g_xccc_d[l_ac].xccc102
#               CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,g_xccc_d[l_ac].xccc202,'5') RETURNING g_xccc_d[l_ac].xccc202
#               CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,g_xccc_d[l_ac].xccc280,'5') RETURNING g_xccc_d[l_ac].xccc280
#               CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,g_xccc_d[l_ac].xccc280a,'5') RETURNING g_xccc_d[l_ac].xccc280a
#               CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,g_xccc_d[l_ac].xccc280b,'5') RETURNING g_xccc_d[l_ac].xccc280b
#               CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,g_xccc_d[l_ac].xccc280c,'5') RETURNING g_xccc_d[l_ac].xccc280c
#               CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,g_xccc_d[l_ac].xccc280d,'5') RETURNING g_xccc_d[l_ac].xccc280d
#               CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,g_xccc_d[l_ac].xccc280e,'5') RETURNING g_xccc_d[l_ac].xccc280e
#               CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,g_xccc_d[l_ac].xccc280f,'5') RETURNING g_xccc_d[l_ac].xccc280f
#               CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,g_xccc_d[l_ac].xccc280g,'5') RETURNING g_xccc_d[l_ac].xccc280g
#               CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,g_xccc_d[l_ac].xccc280h,'5') RETURNING g_xccc_d[l_ac].xccc280h
#               CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,g_xccc_d[l_ac].xccc302,'5') RETURNING g_xccc_d[l_ac].xccc302
#               CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,g_xccc_d[l_ac].xccc902,'5') RETURNING g_xccc_d[l_ac].xccc902
#               CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,g_xccc_d[l_ac].xccc903,'5') RETURNING g_xccc_d[l_ac].xccc903
                #171201-00043#5 mask-E
...
#      CALL g_xccc2_d.deleteElement(g_xccc2_d.getLength())
   ...
   IF l_ac > 1 THEN
      #最后一组小计
      #170427-00041#4 add(S) #171201-00043#5 mask-S
#      IF cl_null(l_unit1_total) THEN LET l_unit1_total=0 END IF
#      IF cl_null(l_xccc1021_total) THEN LET l_xccc1021_total=0 END IF
#      IF cl_null(l_xccc2021_total) THEN LET l_xccc2021_total=0 END IF
#      IF cl_null(l_xccc2801_total) THEN LET l_xccc2801_total=0 END IF
#      IF cl_null(l_xccc280a1_total) THEN LET l_xccc280a1_total=0 END IF
#      IF cl_null(l_xccc280b1_total) THEN LET l_xccc280b1_total=0 END IF
#      IF cl_null(l_xccc280c1_total) THEN LET l_xccc280c1_total=0 END IF
#      IF cl_null(l_xccc280d1_total) THEN LET l_xccc280d1_total=0 END IF
#      IF cl_null(l_xccc280e1_total) THEN LET l_xccc280e1_total=0 END IF
#      IF cl_null(l_xccc280f1_total) THEN LET l_xccc280f1_total=0 END IF
#      IF cl_null(l_xccc280g1_total) THEN LET l_xccc280g1_total=0 END IF
#      IF cl_null(l_xccc280h1_total) THEN LET l_xccc280h1_total=0 END IF
#      IF cl_null(l_xccc3021_total) THEN LET l_xccc3021_total=0 END IF
#      IF cl_null(l_xccc9021_total) THEN LET l_xccc9021_total=0 END IF
#      IF cl_null(l_xccc9031_total) THEN LET l_xccc9031_total=0 END IF
#      IF cl_null(l_unit1_sum) THEN LET l_unit1_sum=0 END IF
#      IF cl_null(l_xccc1021_sum) THEN LET l_xccc1021_sum=0 END IF
#      IF cl_null(l_xccc2021_sum) THEN LET l_xccc2021_sum=0 END IF
#      IF cl_null(l_xccc2801_sum) THEN LET l_xccc2801_sum=0 END IF
#      IF cl_null(l_xccc280a1_sum) THEN LET l_xccc280a1_sum=0 END IF
#      IF cl_null(l_xccc280b1_sum) THEN LET l_xccc280b1_sum=0 END IF
#      IF cl_null(l_xccc280c1_sum) THEN LET l_xccc280c1_sum=0 END IF
#      IF cl_null(l_xccc280d1_sum) THEN LET l_xccc280d1_sum=0 END IF
#      IF cl_null(l_xccc280e1_sum) THEN LET l_xccc280e1_sum=0 END IF
#      IF cl_null(l_xccc280f1_sum) THEN LET l_xccc280f1_sum=0 END IF
#      IF cl_null(l_xccc280g1_sum) THEN LET l_xccc280g1_sum=0 END IF
#      IF cl_null(l_xccc280h1_sum) THEN LET l_xccc280h1_sum=0 END IF
#      IF cl_null(l_xccc3021_sum) THEN LET l_xccc3021_sum=0 END IF
#      IF cl_null(l_xccc9021_sum) THEN LET l_xccc9021_sum=0 END IF
#      IF cl_null(l_xccc9031_sum) THEN LET l_xccc9031_sum=0 END IF
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_unit1_total,'5') RETURNING l_unit1_total
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc1021_total,'5') RETURNING l_xccc1021_total
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc2021_total,'5') RETURNING l_xccc2021_total
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc2801_total,'5') RETURNING l_xccc2801_total
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc280a1_total,'5') RETURNING l_xccc280a1_total
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc280b1_total,'5') RETURNING l_xccc280b1_total
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc280c1_total,'5') RETURNING l_xccc280c1_total
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc280d1_total,'5') RETURNING l_xccc280d1_total
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc280e1_total,'5') RETURNING l_xccc280e1_total
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc280f1_total,'5') RETURNING l_xccc280f1_total
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc280g1_total,'5') RETURNING l_xccc280g1_total
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc280h1_total,'5') RETURNING l_xccc280h1_total
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc3021_total,'5') RETURNING l_xccc3021_total
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc9021_total,'5') RETURNING l_xccc9021_total
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc9031_total,'5') RETURNING l_xccc9031_total
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_unit1_sum,'5') RETURNING l_unit1_sum
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc1021_sum,'5') RETURNING l_xccc1021_sum
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc2021_sum,'5') RETURNING l_xccc2021_sum
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc2801_sum,'5') RETURNING l_xccc2801_sum
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc280a1_sum,'5') RETURNING l_xccc280a1_sum
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc280b1_sum,'5') RETURNING l_xccc280b1_sum
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc280c1_sum,'5') RETURNING l_xccc280c1_sum
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc280d1_sum,'5') RETURNING l_xccc280d1_sum
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc280e1_sum,'5') RETURNING l_xccc280e1_sum
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc280f1_sum,'5') RETURNING l_xccc280f1_sum
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc280g1_sum,'5') RETURNING l_xccc280g1_sum
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc280h1_sum,'5') RETURNING l_xccc280h1_sum
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc3021_sum,'5') RETURNING l_xccc3021_sum
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc9021_sum,'5') RETURNING l_xccc9021_sum
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc9031_sum,'5') RETURNING l_xccc9031_sum
      #170427-00041#4 add(E) #171201-00043#5 mask-E
...
#      LET g_xccc2_d[l_ac].xccc006 = cl_getmsg("axc-00205",g_lang)   #mark--2015/10/07 By shiun
      ...
      #LET g_xccc_d[l_ac].l_unit  = l_unit1_sum     #dorislai-20151104-add  #170606-00042#1 mark
      LET g_xccc_d[l_ac].l_unit = NULL     #170606-00042#1 add
...
#      LET g_xccc2_d[l_ac].xccc006 = cl_getmsg("axc-00204",g_lang)   #mark--2015/10/07 By shiun
      ...
      #LET g_xccc_d[l_ac].l_unit  = l_unit1_total     #dorislai-20151104-add  #170606-00042#1 mark
      LET g_xccc_d[l_ac].l_unit = NULL     #170606-00042#1 add
...
#                       "  GROUP BY inadent,inad001,inad002,inad003"
          ...
          #170522-00111#1 mod-E
          PREPARE axcq530_inad1 FROM l_sql1
#            EXECUTE axcq530_inad1 INTO sr.inadsite,sr.inad010 USING g_enterprise,sr.xccc006,sr.xccc007,sr.xccc008 #170522-00111#1 mark
            EXECUTE axcq530_inad1 INTO sr.inadsite,sr.inadsite_desc,sr.inad010,sr.inad010_desc USING g_enterprise,sr.xccc006,sr.xccc007,sr.xccc008 #170522-00111#1 add
            #170522-00111#1 mark-S
#            IF sr.inadsite IS NULL AND sr.inad010 IS NULL THEN
#               CONTINUE FOR
#            END IF
            #170522-00111#1 mark-E
       END IF
#wujie 150702 --end
       #170522-00111#1 mark-S
##wujie 150611 --begin
...
DEFINE l_wc2          STRING    #151130-00002#1--add
  ...
  #151130-00002#1--add--(s)
   ...
   IF g_wc2 <> " 1=1" AND NOT cl_null(g_wc2) THEN
      LET g_sql =   " INSERT INTO axcq530_tmp1 ",
                    ...
                    " LEFT OUTER JOIN xcbb_t t1 ON t1.xcbbent = xcccent AND t1.xcbbcomp = xccccomp AND t1.xcbb001 = xccc004 AND t1.xcbb002 = xccc005 AND t1.xcbb003 = xccc006 ",    #170927-00015#1 add by 09767
  ...
  #-151130-00002#1--add--(e)
   ...
   #170522-00111#1 add-S prepare的部分移到公共区，foreach的部分直接一句sql就可以做完
   LET l_merge_qm =
          " MERGE INTO axcq530_tmp1 t0 ",
          "      USING (SELECT a.xccc002,a.xccc006,a.xccc007,a.xccc008,a.xccc901,a.xccc902 ",
          "               FROM xccc_t a,axcq530_tmp1 b ",
          "              WHERE a.xcccent = ",g_enterprise," AND a.xcccld = '",g_xccc_m.xcccld,"' AND a.xccc001 = '",g_xccc_m.xccc001,"' ",
          "                AND a.xccc002 = b.xccc002 AND a.xccc003 = '",g_xccc_m.xccc003,"' ",
          "                AND a.xccc004 = ",g_yy2," AND a.xccc005 = ",g_mm2," AND a.xccc006 = b.xccc006 AND a.xccc007 = b.xccc007 AND a.xccc008 = b.xccc008 ) tqm",
   ...
   EXECUTE axcq530_merge_qm
   #170522-00111#1 add-E
   #170522-00111#1 mark-S  以下计算可以通过一句sql执行完成，不用foreach来实现。
   #modify--2015/10/08 By shiun--(S)
...
#      SELECT xccc004,MAX(xccc005) INTO l_xccc004_max,l_xccc005_max
#        FROM xccc_t
#       WHERE xcccent = g_enterprise
#         AND xcccld = g_xccc_m.xcccld
#         AND xccc001 = g_xccc_m.xccc001
...
#         AND xccc007 = l_xccc007
#         AND xccc008 = l_xccc008
#         AND xccc004 = (SELECT MAX(xccc004)
#                          FROM xccc_t
#                         WHERE xcccent = g_enterprise
#                           AND xcccld = g_xccc_m.xcccld
#                           AND xccc001 = g_xccc_m.xccc001
...
DEFINE  l_xcat005         LIKE xcat_t.xcat005
   ...
   LET g_sql = "SELECT UNIQUE xccc004,xccc005,imag011,xccc002,xccc006,xccc007,xccc008,xccc101,xccc102,xccc201,",
               ...
               #"               (SELECT imag011 FROM imag_t WHERE imagent = '"||g_enterprise||"' AND imagsite = '"||g_xccc_m.xccccomp||"' AND imag001 = xccc006 ) imag011, ", #170914-00003#1 mark
               "               imag011, ", #170914-00003#1 add
               "               xccc002,xccc006,xccc007,xccc008,xccc101,xccc102,xccc201,",
   #160520-00003#5----e
               "               xccc202,xccc203,xccc204,xccc205,xccc206,xccc207,xccc208,xccc209,xccc210,xccc211,xccc212,xccc213,",
               "               xccc214,xccc215,xccc216,xccc217,xccc218,xccc301,xccc302,xccc303,xccc304,xccc305,xccc306,xccc307,",
               ...
               #160520-00003#5----e
               #"      FROM xccc_t",        #170914-00003#1 mark
               "       FROM xccc_t, imag_t", #170914-00003#1 add
               #160520-00003#5----s
               ...
               #160520-00003#5----e
               "    WHERE xcccent='"||g_enterprise||"' AND ",g_wc,  #170224-00017#1 #170914-00003#1 add ,
               "      AND xcccent = imagent AND xccccomp = imagsite AND imag001 = xccc006 " #170914-00003#1 add
...
#wujie 150604 --begin
         IF l_xcat005 = '3' THEN
            IF NOT cl_null(g_xccc2_d[l_ac].xccc008) THEN #170511-00021#1 add
               #170522-00111#1 mod-S
...
#               LET g_xccc2_d[l_ac].inad010_2_desc = s_desc_get_trading_partner_full_desc(g_xccc2_d[l_ac].inad010)
               EXECUTE axcq530_inad_2 INTO g_xccc2_d[l_ac].inadsite,g_xccc2_d[l_ac].inadsite_2_desc,g_xccc2_d[l_ac].inad010,g_xccc2_d[l_ac].inad010_2_desc
                                      USING g_enterprise,g_xccc2_d[l_ac].xccc006,g_xccc2_d[l_ac].xccc007,g_xccc2_d[l_ac].xccc008
               #170522-00111#1 mod-E
            END IF  #170511-00021#1 add
         END IF
         ...
         IF l_ac > 1 THEN
           #IF g_xccc2_d[l_ac].xccc006 != g_xccc2_d[l_ac - 1].xccc006 OR g_xccc2_d[l_ac].xccc007 != g_xccc2_d[l_ac - 1].xccc007 THEN #151013-00019#7 mark
            IF g_xccc2_d[l_ac].imag011 != g_xccc2_d[l_ac - 1].imag011 THEN  #151013-00019#7
               ...
               #LET g_xccc2_d[l_ac].l_unit_2= l_unit_22_sum - g_xccc2_d[l_ac+ 1].l_unit_2#dorislai-20151104-add  #170606-00042#1 mark
               LET g_xccc2_d[l_ac].l_unit_2 = NULL     #170606-00042#1 add
               LET g_xccc2_d[l_ac].xccc102= l_xccc1022_sum - g_xccc2_d[l_ac+ 1].xccc102
               ...
               #IF cl_null(g_xccc2_d[l_ac].l_unit_2) THEN LET g_xccc2_d[l_ac].l_unit_2=0 END IF   #170606-00042#1 mark
               IF cl_null(g_xccc2_d[l_ac].xccc102) THEN LET g_xccc2_d[l_ac].xccc102=0 END IF
               IF cl_null(g_xccc2_d[l_ac].xccc202) THEN LET g_xccc2_d[l_ac].xccc202=0 END IF
               IF cl_null(g_xccc2_d[l_ac].xccc204) THEN LET g_xccc2_d[l_ac].xccc204=0 END IF
               IF cl_null(g_xccc2_d[l_ac].xccc206) THEN LET g_xccc2_d[l_ac].xccc206=0 END IF
               IF cl_null(g_xccc2_d[l_ac].xccc208) THEN LET g_xccc2_d[l_ac].xccc208=0 END IF
               IF cl_null(g_xccc2_d[l_ac].xccc210) THEN LET g_xccc2_d[l_ac].xccc210=0 END IF
               IF cl_null(g_xccc2_d[l_ac].xccc212) THEN LET g_xccc2_d[l_ac].xccc212=0 END IF
               IF cl_null(g_xccc2_d[l_ac].xccc214) THEN LET g_xccc2_d[l_ac].xccc214=0 END IF
               IF cl_null(g_xccc2_d[l_ac].xccc216) THEN LET g_xccc2_d[l_ac].xccc216=0 END IF
               IF cl_null(g_xccc2_d[l_ac].xccc218) THEN LET g_xccc2_d[l_ac].xccc218=0 END IF
               IF cl_null(g_xccc2_d[l_ac].xccc302) THEN LET g_xccc2_d[l_ac].xccc302=0 END IF
               IF cl_null(g_xccc2_d[l_ac].xccc304) THEN LET g_xccc2_d[l_ac].xccc304=0 END IF
               IF cl_null(g_xccc2_d[l_ac].xccc306) THEN LET g_xccc2_d[l_ac].xccc306=0 END IF
               IF cl_null(g_xccc2_d[l_ac].xccc308) THEN LET g_xccc2_d[l_ac].xccc308=0 END IF
               IF cl_null(g_xccc2_d[l_ac].xccc310) THEN LET g_xccc2_d[l_ac].xccc310=0 END IF
               IF cl_null(g_xccc2_d[l_ac].xccc312) THEN LET g_xccc2_d[l_ac].xccc312=0 END IF
               IF cl_null(g_xccc2_d[l_ac].xccc314) THEN LET g_xccc2_d[l_ac].xccc314=0 END IF
               IF cl_null(g_xccc2_d[l_ac].xccc902) THEN LET g_xccc2_d[l_ac].xccc902=0 END IF
               IF cl_null(g_xccc2_d[l_ac].xccc903) THEN LET g_xccc2_d[l_ac].xccc903=0 END IF
               IF cl_null(g_xccc2_d[l_ac].xccc903a) THEN LET g_xccc2_d[l_ac].xccc903a=0 END IF
               IF cl_null(g_xccc2_d[l_ac].xccc903b) THEN LET g_xccc2_d[l_ac].xccc903b=0 END IF
               IF cl_null(g_xccc2_d[l_ac].xccc903c) THEN LET g_xccc2_d[l_ac].xccc903c=0 END IF
               IF cl_null(g_xccc2_d[l_ac].xccc903d) THEN LET g_xccc2_d[l_ac].xccc903d=0 END IF
               IF cl_null(g_xccc2_d[l_ac].xccc903e) THEN LET g_xccc2_d[l_ac].xccc903e=0 END IF
               IF cl_null(g_xccc2_d[l_ac].xccc903f) THEN LET g_xccc2_d[l_ac].xccc903f=0 END IF
               IF cl_null(g_xccc2_d[l_ac].xccc903g) THEN LET g_xccc2_d[l_ac].xccc903g=0 END IF
               IF cl_null(g_xccc2_d[l_ac].xccc903h) THEN LET g_xccc2_d[l_ac].xccc903h=0 END IF
               #CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,g_xccc2_d[l_ac].l_unit_2,'5') RETURNING g_xccc2_d[l_ac].l_unit_2  #170606-00042#1 mark
               #171201-00043#5 mask-S
#               CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,g_xccc2_d[l_ac].xccc102,'5') RETURNING g_xccc2_d[l_ac].xccc102
#               CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,g_xccc2_d[l_ac].xccc202,'5') RETURNING g_xccc2_d[l_ac].xccc202
#               CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,g_xccc2_d[l_ac].xccc204,'5') RETURNING g_xccc2_d[l_ac].xccc204
#               CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,g_xccc2_d[l_ac].xccc206,'5') RETURNING g_xccc2_d[l_ac].xccc206
#               CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,g_xccc2_d[l_ac].xccc208,'5') RETURNING g_xccc2_d[l_ac].xccc208
#               CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,g_xccc2_d[l_ac].xccc209,'5') RETURNING g_xccc2_d[l_ac].xccc209
#               CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,g_xccc2_d[l_ac].xccc210,'5') RETURNING g_xccc2_d[l_ac].xccc210
#               CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,g_xccc2_d[l_ac].xccc212,'5') RETURNING g_xccc2_d[l_ac].xccc212
#               CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,g_xccc2_d[l_ac].xccc214,'5') RETURNING g_xccc2_d[l_ac].xccc214
#               CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,g_xccc2_d[l_ac].xccc216,'5') RETURNING g_xccc2_d[l_ac].xccc216
#               CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,g_xccc2_d[l_ac].xccc218,'5') RETURNING g_xccc2_d[l_ac].xccc218
#               CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,g_xccc2_d[l_ac].xccc302,'5') RETURNING g_xccc2_d[l_ac].xccc302
#               CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,g_xccc2_d[l_ac].xccc304,'5') RETURNING g_xccc2_d[l_ac].xccc304
#               CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,g_xccc2_d[l_ac].xccc306,'5') RETURNING g_xccc2_d[l_ac].xccc306
#               CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,g_xccc2_d[l_ac].xccc308,'5') RETURNING g_xccc2_d[l_ac].xccc308
#               CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,g_xccc2_d[l_ac].xccc310,'5') RETURNING g_xccc2_d[l_ac].xccc310
#               CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,g_xccc2_d[l_ac].xccc312,'5') RETURNING g_xccc2_d[l_ac].xccc312
#               CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,g_xccc2_d[l_ac].xccc314,'5') RETURNING g_xccc2_d[l_ac].xccc314
#               CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,g_xccc2_d[l_ac].xccc902,'5') RETURNING g_xccc2_d[l_ac].xccc902
#               CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,g_xccc2_d[l_ac].xccc903,'5') RETURNING g_xccc2_d[l_ac].xccc903
#               CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,g_xccc2_d[l_ac].xccc903a,'5') RETURNING g_xccc2_d[l_ac].xccc903a
#               CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,g_xccc2_d[l_ac].xccc903b,'5') RETURNING g_xccc2_d[l_ac].xccc903b
#               CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,g_xccc2_d[l_ac].xccc903c,'5') RETURNING g_xccc2_d[l_ac].xccc903c
#               CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,g_xccc2_d[l_ac].xccc903d,'5') RETURNING g_xccc2_d[l_ac].xccc903d
#               CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,g_xccc2_d[l_ac].xccc903e,'5') RETURNING g_xccc2_d[l_ac].xccc903e
#               CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,g_xccc2_d[l_ac].xccc903f,'5') RETURNING g_xccc2_d[l_ac].xccc903f
#               CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,g_xccc2_d[l_ac].xccc903g,'5') RETURNING g_xccc2_d[l_ac].xccc903g
#               CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,g_xccc2_d[l_ac].xccc903h,'5') RETURNING g_xccc2_d[l_ac].xccc903h
               #171201-00043#5 mask-E
   ...
   IF l_ac > 1 THEN
      #最后一组小计
      #170427-00041#4 add(S) #171201-00043#5 mask-S
#      IF cl_null(l_unit_22_total) THEN LET l_unit_22_total=0 END IF
#      IF cl_null(l_xccc1022_total) THEN LET l_xccc1022_total=0 END IF
#      IF cl_null(l_xccc2022_total) THEN LET l_xccc2022_total=0 END IF
#      IF cl_null(l_xccc2042_total) THEN LET l_xccc2042_total=0 END IF
#      IF cl_null(l_xccc2062_total) THEN LET l_xccc2062_total=0 END IF
#      IF cl_null(l_xccc2082_total) THEN LET l_xccc2082_total=0 END IF
#      IF cl_null(l_xccc2102_total) THEN LET l_xccc2102_total=0 END IF
#      IF cl_null(l_xccc2122_total) THEN LET l_xccc2122_total=0 END IF
#      IF cl_null(l_xccc2142_total) THEN LET l_xccc2142_total=0 END IF
#      IF cl_null(l_xccc2162_total) THEN LET l_xccc2162_total=0 END IF
#      IF cl_null(l_xccc2182_total) THEN LET l_xccc2182_total=0 END IF
#      IF cl_null(l_xccc3022_total) THEN LET l_xccc3022_total=0 END IF
#      IF cl_null(l_xccc3042_total) THEN LET l_xccc3042_total=0 END IF
#      IF cl_null(l_xccc3062_total) THEN LET l_xccc3062_total=0 END IF
#      IF cl_null(l_xccc3082_total) THEN LET l_xccc3082_total=0 END IF
#      IF cl_null(l_xccc3102_total) THEN LET l_xccc3102_total=0 END IF
#      IF cl_null(l_xccc3122_total) THEN LET l_xccc3122_total=0 END IF
#      IF cl_null(l_xccc3142_total) THEN LET l_xccc3142_total=0 END IF
#      IF cl_null(l_xccc9022_total) THEN LET l_xccc9022_total=0 END IF
#      IF cl_null(l_xccc9032_total) THEN LET l_xccc9032_total=0 END IF
#      IF cl_null(l_xccc903a2_total) THEN LET l_xccc903a2_total=0 END IF
#      IF cl_null(l_xccc903b2_total) THEN LET l_xccc903b2_total=0 END IF
#      IF cl_null(l_xccc903c2_total) THEN LET l_xccc903c2_total=0 END IF
#      IF cl_null(l_xccc903d2_total) THEN LET l_xccc903d2_total=0 END IF
#      IF cl_null(l_xccc903e2_total) THEN LET l_xccc903e2_total=0 END IF
#      IF cl_null(l_xccc903f2_total) THEN LET l_xccc903f2_total=0 END IF
#      IF cl_null(l_xccc903g2_total) THEN LET l_xccc903g2_total=0 END IF
#      IF cl_null(l_xccc903h2_total) THEN LET l_xccc903h2_total=0 END IF
#
#      IF cl_null(l_unit_22_sum) THEN LET l_unit_22_sum=0 END IF
#      IF cl_null(l_xccc1022_sum) THEN LET l_xccc1022_sum=0 END IF
#      IF cl_null(l_xccc2022_sum) THEN LET l_xccc2022_sum=0 END IF
#      IF cl_null(l_xccc2042_sum) THEN LET l_xccc2042_sum=0 END IF
#      IF cl_null(l_xccc2062_sum) THEN LET l_xccc2062_sum=0 END IF
#      IF cl_null(l_xccc2082_sum) THEN LET l_xccc2082_sum=0 END IF
#      IF cl_null(l_xccc2102_sum) THEN LET l_xccc2102_sum=0 END IF
#      IF cl_null(l_xccc2122_sum) THEN LET l_xccc2122_sum=0 END IF
#      IF cl_null(l_xccc2142_sum) THEN LET l_xccc2142_sum=0 END IF
#      IF cl_null(l_xccc2162_sum) THEN LET l_xccc2162_sum=0 END IF
#      IF cl_null(l_xccc2182_sum) THEN LET l_xccc2182_sum=0 END IF
#      IF cl_null(l_xccc3022_sum) THEN LET l_xccc3022_sum=0 END IF
#      IF cl_null(l_xccc3042_sum) THEN LET l_xccc3042_sum=0 END IF
#      IF cl_null(l_xccc3062_sum) THEN LET l_xccc3062_sum=0 END IF
#      IF cl_null(l_xccc3082_sum) THEN LET l_xccc3082_sum=0 END IF
#      IF cl_null(l_xccc3102_sum) THEN LET l_xccc3102_sum=0 END IF
#      IF cl_null(l_xccc3112_sum) THEN LET l_xccc3112_sum=0 END IF
#      IF cl_null(l_xccc3122_sum) THEN LET l_xccc3122_sum=0 END IF
#      IF cl_null(l_xccc3132_sum) THEN LET l_xccc3132_sum=0 END IF
#      IF cl_null(l_xccc3042_sum) THEN LET l_xccc3142_sum=0 END IF
#      IF cl_null(l_xccc9022_sum) THEN LET l_xccc9022_sum=0 END IF
#      IF cl_null(l_xccc9032_sum) THEN LET l_xccc9032_sum=0 END IF
#      IF cl_null(l_xccc903a2_sum) THEN LET l_xccc903a2_sum=0 END IF
#      IF cl_null(l_xccc903b2_sum) THEN LET l_xccc903b2_sum=0 END IF
#      IF cl_null(l_xccc903c2_sum) THEN LET l_xccc903c2_sum=0 END IF
#      IF cl_null(l_xccc903d2_sum) THEN LET l_xccc903d2_sum=0 END IF
#      IF cl_null(l_xccc903e2_sum) THEN LET l_xccc903e2_sum=0 END IF
#      IF cl_null(l_xccc903f2_sum) THEN LET l_xccc903f2_sum=0 END IF
#      IF cl_null(l_xccc903g2_sum) THEN LET l_xccc903g2_sum=0 END IF
#      IF cl_null(l_xccc903h2_sum) THEN LET l_xccc903h2_sum=0 END IF
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_unit_22_total,'5') RETURNING l_unit_22_total
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc1022_total,'5') RETURNING l_xccc1022_total
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc2022_total,'5') RETURNING l_xccc2022_total
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc2042_total,'5') RETURNING l_xccc2042_total
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc2062_total,'5') RETURNING l_xccc2062_total
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc2082_total,'5') RETURNING l_xccc2082_total
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc2102_total,'5') RETURNING l_xccc2102_total
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc2122_total,'5') RETURNING l_xccc2122_total
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc2142_total,'5') RETURNING l_xccc2142_total
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc2162_total,'5') RETURNING l_xccc2162_total
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc2182_total,'5') RETURNING l_xccc2182_total
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc3022_total,'5') RETURNING l_xccc3022_total
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc3042_total,'5') RETURNING l_xccc3042_total
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc3062_total,'5') RETURNING l_xccc3062_total
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc3082_total,'5') RETURNING l_xccc3082_total
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc3102_total,'5') RETURNING l_xccc3102_total
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc3122_total,'5') RETURNING l_xccc3122_total
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc3142_total,'5') RETURNING l_xccc3142_total
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc9022_total,'5') RETURNING l_xccc9022_total
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc9032_total,'5') RETURNING l_xccc9032_total
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc903a2_total,'5') RETURNING l_xccc903a2_total
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc903b2_total,'5') RETURNING l_xccc903b2_total
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc903c2_total,'5') RETURNING l_xccc903c2_total
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc903d2_total,'5') RETURNING l_xccc903d2_total
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc903e2_total,'5') RETURNING l_xccc903e2_total
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc903f2_total,'5') RETURNING l_xccc903f2_total
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc903g2_total,'5') RETURNING l_xccc903g2_total
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc903h2_total,'5') RETURNING l_xccc903h2_total
#
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_unit_22_sum,'5') RETURNING l_unit_22_sum
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc1022_sum,'5') RETURNING l_xccc1022_sum
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc2022_sum,'5') RETURNING l_xccc2022_sum
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc2042_sum,'5') RETURNING l_xccc2042_sum
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc2062_sum,'5') RETURNING l_xccc2062_sum
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc2082_sum,'5') RETURNING l_xccc2082_sum
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc2102_sum,'5') RETURNING l_xccc2102_sum
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc2122_sum,'5') RETURNING l_xccc2122_sum
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc2142_sum,'5') RETURNING l_xccc2142_sum
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc2162_sum,'5') RETURNING l_xccc2162_sum
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc2182_sum,'5') RETURNING l_xccc2182_sum
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc3022_sum,'5') RETURNING l_xccc3022_sum
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc3042_sum,'5') RETURNING l_xccc3042_sum
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc3062_sum,'5') RETURNING l_xccc3062_sum
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc3082_sum,'5') RETURNING l_xccc3082_sum
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc3102_sum,'5') RETURNING l_xccc3102_sum
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc3122_sum,'5') RETURNING l_xccc3122_sum
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc3142_sum,'5') RETURNING l_xccc3142_sum
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc9022_sum,'5') RETURNING l_xccc9022_sum
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc9032_sum,'5') RETURNING l_xccc9032_sum
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc903a2_sum,'5') RETURNING l_xccc903a2_sum
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc903b2_sum,'5') RETURNING l_xccc903b2_sum
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc903c2_sum,'5') RETURNING l_xccc903c2_sum
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc903d2_sum,'5') RETURNING l_xccc903d2_sum
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc903e2_sum,'5') RETURNING l_xccc903e2_sum
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc903f2_sum,'5') RETURNING l_xccc903f2_sum
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc903g2_sum,'5') RETURNING l_xccc903g2_sum
#      CALL s_curr_round(g_xccc_m.xccccomp,g_glaa001,l_xccc903h2_sum,'5') RETURNING l_xccc903h2_sum
      #170427-00041#4 add(E) #171201-00043#5 mask-E
      #LET g_xccc2_d[l_ac].xccc006 = cl_getmsg("axc-00205",g_lang)   #151029-00010#3 20151029 mark by beckxie
      LET g_xccc2_d[l_ac].imag011_2_desc = g_xccc2_d[l_ac-1].imag011,cl_getmsg("axc-00205",g_lang)   #151029-00010#3 20151029 add by beckxie

      LET g_xccc2_d[l_ac].xccc101 = l_xccc1012_sum #dorislai-20151104-add
      #LET g_xccc2_d[l_ac].l_unit_2 = l_unit_22_sum #dorislai-20151104-add  #170606-00042#1 mark
      LET g_xccc2_d[l_ac].l_unit_2 = NULL     #170606-00042#1 add
      LET g_xccc2_d[l_ac].xccc102 = l_xccc1022_sum
      ...
      #LET g_xccc2_d[l_ac].l_unit_2 = l_unit_22_total #dorislai-20151104-add  #170606-00042#1 mark
      LET g_xccc2_d[l_ac].l_unit_2 = NULL     #170606-00042#1 add
      LET g_xccc2_d[l_ac].xccc102 = l_xccc1022_total
...
{<section id="axcq530.show_ownid_msg" type="s" >}
##+ 判斷是否顯示只能修改自己權限資料的訊息
PRIVATE FUNCTION axcq530_show_ownid_msg()
...
PRIVATE FUNCTION axcq530_set_all_pk_array()
   ...
   LET g_pk_array[1].values = g_xccc_m.xcccld
   LET g_pk_array[1].column = 'xcccld'
   LET g_pk_array[02].values = g_xccc_m.xccc001
   LET g_pk_array[02].column = 'xccc001'
   LET g_pk_array[03].values = g_xccc_m.xccc003
   LET g_pk_array[03].column = 'xccc003'
   LET g_pk_array[04].values = g_xccc_m.xccc004
   LET g_pk_array[04].column = 'xccc004'
   LET g_pk_array[05].values = g_xccc_m.xccc005
   LET g_pk_array[05].column = 'xccc005'


   #設定單身pk
   LET ls_length = g_pk_array.getLength()+1
   LET g_pk_array[ls_length].values = g_xccc_d[g_detail_idx].xccc002
   LET g_pk_array[ls_length].column = 'xccc002'
   LET ls_length = g_pk_array.getLength()+1
   LET g_pk_array[ls_length].values = g_xccc_d[g_detail_idx].xccc006
   LET g_pk_array[ls_length].column = 'xccc006'
   LET ls_length = g_pk_array.getLength()+1
   LET g_pk_array[ls_length].values = g_xccc_d[g_detail_idx].xccc007
   LET g_pk_array[ls_length].column = 'xccc007'
   LET ls_length = g_pk_array.getLength()+1
   LET g_pk_array[ls_length].values = g_xccc_d[g_detail_idx].xccc008
   LET g_pk_array[ls_length].column = 'xccc008'


   #add-point:set_all_pk_array段之後 name="set_all_pk_array.after"
   {<point name="set_all_pk_array.after"/>}
   #end add-point
