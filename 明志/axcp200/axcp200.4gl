...
#+ Standard Version.....: SD版次:0057(2016-06-13 17:17:39), PR版次:0057(1900-01-01 00:00:00)
...
MAIN
   ...
   DROP TABLE reached_tmp;        #170908-00044#1 add
   DROP TABLE convert_by_so_tmp;  #170908-00044#1 add
   #end add-point
...
#  FOREACH axcp200_process_cs INTO
   ...
   SELECT COUNT(*) INTO l_cnt FROM xcea_t 
    WHERE xceaent = g_enterprise AND xceald = g_master.glaald
      AND xcea002 IN ('11','12') AND xcea004 = g_master.yy AND xcea005 = g_master.pp
...
#               "    AND (xmdl014 = ' ' OR EXISTS(SELECT 1 FROM inaa_t WHERE inaaent = xmdlent AND inaasite  = xmdlsite  AND inaa001 = xmdl014 AND inaa010 = 'Y')) ", #170214-00060#1 add )
               "    AND (xmdl014 = ' ' OR xmdl014 IS NULL OR EXISTS(SELECT 1 FROM inaa_t WHERE inaaent = xmdlent AND inaasite  = xmdlsite  AND inaa001 = xmdl014 AND inaa010 = 'Y')) ", 
   ...
   LET g_sql = " INSERT INTO convert_tmp ",
               ...
               " SELECT DISTINCT xmdl008,xmdl017,imaa006,1,'','' ", #170603-00014#1 add               
               "   FROM xmdk_t,xmdl_t,imaa_t,xmdm_t  ",
               "  WHERE xmdkent = xmdlent AND xmdkdocno = xmdldocno ",
               "    AND imaaent = xmdlent AND imaa001   = xmdl008 ",
               "    AND xmdment = xmdkent AND xmdmdocno = xmdkdocno AND xmdmseq = xmdlseq",
...
　       UPDATE convert_tmp
   ...
   LET g_sql = " INSERT INTO xckb_tmp(xckbent,xckbcomp,xckbld,xckb001,xckb002,",
               ...
               " SELECT ",g_enterprise,",'",g_comp,"','",g_master.glaald,"',(CASE WHEN xmdk000='1' OR xmdk000='2' OR xmdk000='3' OR xmdk000='7'  THEN '1' WHEN xmdk000='4' OR xmdk000='5' THEN '2' WHEN xmdk000='6' THEN '3' END) xckb001,'1',",  ##170504-00033#1 add xmdk000='7'
...
#                 "    AND (xmdl014 = ' ' OR EXISTS(SELECT 1 FROM inaa_t WHERE inaaent = xmdlent AND inaasite  = xmdlsite  AND inaa001 = xmdl014 AND inaa010 = 'Y')) ", #170214-00060#1 add )
   ...
   LET g_sql = " MERGE INTO xckb_tmp a",
               ...
               " USING (SELECT imaa001,from_unit,to_unit,qty,ooca002,ooca004 FROM convert_tmp ) b", #170603-00014#1 add
               ...
               "                                     WHEN b.ooca004 = '4' THEN CEIL(a.xckb014*b.qty)/POWER(10,b.ooca002) ",
               "                                END) "
               #170603-00014#1 add end   -----               
...
#               "    AND ",g_wc_ais CLIPPED
               ...
               #170908-00044#1-s add
               " SELECT DISTINCT isag009 as imaa001,xmdm008 as from_unit,imaa006 as to_unit,1 as qty,'' as ooca002,'' as ooca004", 
...
　       UPDATE convert_tmp
   ...
   IF l_xcat005 = '3' THEN    #170618-00239#1 add   
      LET g_sql = " INSERT INTO xckb2_tmp(xckbent,xckbcomp,xckbld,xckb001,xckb002,",
                  ...
                  #"        isag005,SUM(DECODE(isag005,NULL,isag004,isag004*isag015)) isag004,' ',' ',' ',",  #170220-00038#1 add   #170306-00053 wujie mod #170512-00092#1 mark
                   "        isag005,SUM(DECODE(isag005,NULL,isag004,isag004)) isag004,' ',' ',' ',",                 #170908-00044#1 add               
         ...
         END IF
         LET g_sql = g_sql,
                    "'','','',0,' ',",   #170512-00092#1 mark        #170908-00044#1 add  都先放項序0
                   ...
                   #"    AND isagent = xmdment AND isag002 = xmdmdocno AND isag003 = xmdmseq ",  #170512-00092#1  #170908-00044#1 mark
                    ...
                    #171011-00045#1---add---e
                    "    AND ",g_wc_ais CLIPPED,
                    " GROUP BY isaf011,isag002,isag003,isaf003,isaf005,isaf006,isag009,isag005,isaf010,imaa006"  #170306-00053 wujie add  #170908-00044#1 去掉,
   ...
   ELSE                    
      LET g_sql = " INSERT INTO xckb2_tmp(xckbent,xckbcomp,xckbld,xckb001,xckb002,",
               ...
               "                 xckb122e,xckb122f,xckb122g,xckb122h,xckb038) ",        
               " SELECT ",g_enterprise,",'",g_comp,"','",g_master.glaald,"','4','-1',",
               "        NVL(isaf011,' ') isaf011,'",g_site,"',isag002,isag003,",g_master.yy,",",  
               "        ",g_master.pp,",isaf003,isaf005,isaf006,isag009,",
               "        isag005,SUM(DECODE(isag005,NULL,isag004,isag004*isag015)) isag004,' ',' ',' ',",  
               "        '',isaf010,imaa006,'','',",  
               "        'N','','','",g_glaa001,"',SUM(isag115)," 
      IF g_glaa015 = 'Y' THEN                        
         LET g_sql = g_sql,"'",g_glaa016,"',0,"
      ELSE
         LET g_sql = g_sql,"'',0,"
      ...
      LET g_sql = g_sql,
                 ...
                 #171011-00045#1---add---e                    
                 "    AND ",g_wc_ais CLIPPED,
                 " GROUP BY isaf011,isag002,isag003,isaf003,isaf005,isaf006,isag009,isag005,isaf010,imaa006",  
#                 "          ,xmdm008,xmdm009,xmdm005,xmdm006,xmdm007,xmdmseq1 "    #170512-00092#1 add     #171012-00003#1 mark
   ...
   LET g_sql = " INSERT INTO convert_by_so_tmp ",
               ...
               "        xmdl017 as from_unit,xmdl021 as to_unit,(NVL(xmdl018,1)/NVL(xmdl022,1)) as qty,ooca002,ooca004 ",
   ...
   LET g_sql = " MERGE INTO xckb2_tmp a ",
               " USING (SELECT xckb005,xckb006,from_unit,to_unit,qty,ooca002,ooca004 FROM convert_by_so_tmp ) b",
   ...
   ##b.依a找出來的單號、項次去看這個月之前開多少數量的發票了
   LET g_sql = "SELECT a.xckbent,a.xckbcomp,a.xckbld,a.xckb003,a.xckb005,a.xckb006,a.xckb014,a.xckb021 ",
   ...
   LET g_sql = "SELECT xckb036,xckb014,xckb014_reached,xckb021,xckb021_reached,",
               ...
               "   AND xckb005 = ? AND xckb006 = ?  AND xckb021 > xckb021_reached ",
               " ORDER BY xckb036 "
   ...
   FOREACH xckb2_exe33 INTO l_wo_xckbent,l_wo_xckbcomp,l_wo_xckbld,l_wo_xckb003,l_wo_xckb005,l_wo_xckb006,l_wo_xckb014,l_wo_xckb021
      ...
      FOREACH xckb2_exe34 USING l_wo_xckbent,l_wo_xckbcomp,l_wo_xckbld,l_wo_xckb005,l_wo_xckb006
         ...
         IF l_so_xckb021 - l_so_xckb021_reached < l_wo_xckb021 THEN 
