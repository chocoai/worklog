...
#+ Standard Version.....: SD版次:0002(2017-01-03 09:55:19), PR版次:0002(2017-01-12 10:44:11)
#+ Customerized Version.: SD版次:0001(2017-06-27 14:45:37), PR版次:0002(2017-11-20 15:47:48)
#+ Build......: 000113
#+ Filename...: aapt930_01
#+ Description: 批次產生
...
PUBLIC FUNCTION aapt930_01(--)
   ...
   #畫面開啟 (identifier)
   OPEN WINDOW w_aapt930_01 WITH FORM cl_ap_formpath("cap","aapt930_01")
...
DEFINE l_docdt2          LIKE xrea_t.xrea008     #171120 add 
DEFINE l_apcadocdt       LIKE apca_t.apcadocdt   #171120 add
DEFINE l_apcb104_320     LIKE apcb_t.apcb104     #171120 add
DEFINE l_apcb113_320     LIKE apcb_t.apcb113     #171120 add
DEFINE l_apcb114_320     LIKE apcb_t.apcb114     #171120 add
DEFINE l_apcb115_320     LIKE apcb_t.apcb115     #171120 add
DEFINE l_xren005         LIKE xren_t.xren005     #171121 add
#170915-00038#1---add---end---
  ...
  #LET l_docdt = MDY(l_xrem.xrem002+1,1,l_xrem.xrem001) - 1 #年度期別第一天     #170915-00038#1 add     #171120 mark
   LET l_docdt = MDY(l_xrem.xrem002,1,l_xrem.xrem001)        #年度期別第一天#      #171120 add
   IF l_xrem.xrem002 != 12 THEN   #180104 add
   LET l_docdt2 = MDY(l_xrem.xrem002+1,1,l_xrem.xrem001) - 1 #年度期別最後一天     #171120 add
   #180104---add---str--
   ELSE
      LET l_docdt2 = MDY(l_xrem.xrem002,31,l_xrem.xrem001)
   END IF
   #180104---add---end--
   ...
   #170915-00038#1---add---end---
              ...
              #"    AND xrea005 = '",l_xren005,"'",   #171121 add
               "    AND xrea002 = '",l_xrem.xrem002,"'  "
               
   IF g_argv1 = '2' THEN
   #   LET g_sql = g_sql CLIPPED, " AND xrea008 < '",l_docdt,"' "#170915-00038#1 mark
     #LET g_sql = g_sql CLIPPED, " AND xrea008 <= '",l_docdt,"' "    #170915-00038#1 add    #171120 mark
       LET g_sql = g_sql CLIPPED, " AND xrea008 < '",l_docdt,"' "                           #171120 add
   END IF                          
   ...
   LET l_sql = "SELECT apcbseq,apcb002,apcb003,apcb007,apcb101,",
               ...
               "       ,apca004,apca005,apcb010,apcb011, ", #mod muping171204
              # "       ,apcb054,apcb055,apcb010,apcb011, ",         # 帳款對象 / 付款對象 / 業務部門 / 責任中心 
               ...
               "      ,apcadocdt,apcb104,apcb113,apcb114,apcb115 ",      #171120 add
               "  FROM apcb_t ",
   ...
   LET l_sql = "SELECT SUM(apcb007),SUM(apcb103),SUM(apcb104),SUM(apcb105), ",      #數量/原幣未稅/原幣稅額/原幣含稅
               ...
               "   AND apcadocdt BETWEEN ? AND '",l_docdt2,"'",       #171120 add
               "   AND apcb023 = 'Y'",                                #171120 add
               "   AND apcastus = 'Y'"
   ...
   FOREACH aapt931_ins_curs 
      INTO l_xrea.xreaent,l_xrea.xreacomp,l_xrea.xreasite,l_xrea.xreald,l_xrea.xrea001,
       ...
       #180104---add---str--
       IF l_xrea.xrea005 = '601-AP01-170900278' THEN
          DISPLAY ' '
       END IF
       #180104---add---end-- 
       ...
       FOREACH aapt930_sel_apcb_c USING l_xrea.xreald,l_xrea.xrea005
          INTO l_apcbseq,l_apcb002,l_apcb003,l_apcb007,l_apcb101,
               ...
               #170915-00038#10 --e add
              ,l_apcadocdt,l_apcb104_320,l_apcb113_320,l_apcb114_320,l_apcb115_320      #171120 add
...
#      #add muping170627 str
#    SELECT apcb103,apcb104,apcb105,apcb113,apcb114,apcb115  #税前 税额 税后
#      INTO l_xren.xren103,l_xren.xren104,l_xren.xren105,l_xren.xren113,l_xren.xren114,l_xren.xren115
#      FROM apcb_t  
#     WHERE apcbent=  g_enterprise 
#       AND apcbld =  g_xrem_m.xremld 
#       AND apcbdocno = l_xren.xren005
#       AND apcbseq =  l_xren.xren006
#      #add muping170627 end
