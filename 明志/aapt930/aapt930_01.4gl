...
#該程式非freestyle程式!
{<section id="aapt930_01.description" type="s" >}
#應用 a00 樣板自動產生(Version:3)
#+ Standard Version.....: SD版次:0019(2017-01-03 09:55:19), PR版次:0019(2018-01-30 09:03:39)
#+ Customerized Version.: SD版次:0000(1900-01-01 00:00:00), PR版次:0000(1900-01-01 00:00:00)
#+ Build......: 000229
#+ Filename...: aapt930_01
#+ Description: 批次產生
#+ Creator....: 05016(2014-11-03 14:45:17)
#+ Modifier...: 06821 -SD/PR- 10554

{</section>}

{<section id="aapt930_01.global" type="s" >}
...
#170301-00027#3  2017/03/03  by 08172     单别参照表调整
#170315-00043#2  2017/03/17  By 09257     AP單據作業增加關帳日期控卡整批調整
#170728-00030#1  2017/08/01  By Sabrina   產生單身時若帳款單性質為02則金額為負
#170915-00038#1  2017/10/16  By 05488     修改aapt930，aapt931入庫/雜項暫估金額邏輯
#170915-00038#10 2017/11/09  By 06821     延續#170915-00038#1, (1) aapt930/931 金額計算尾差時應也納入數量已沖完的金額來計算  (2) 寫入暫估帳務明細檔時, 核算項來源取用aapt320 暫估單的核算項資訊
#171204-00028#1  2017/12/05  By 05795     aapt930审核时提示没有客商核算项数据
#171116-00010#1  2017/12/11  By 05488     延續#170915-00038#1調整
#180110-00048#1  2018/01/10  By 05634     修正取年度期別最後一天的邏輯
#170606-00033#18 2018/01/29  By 10554     財務系統相關 帳務人員,業務人員 開窗規格調整,帳務人員: 依據點所屬法人的所有據點人員(含 ALL)
#end add-point
...
DEFINE g_xrem_m_t          type_g_xrem_m            #170315-00043#2 by 09257 --add
DEFINE g_sfin3007          LIKE xrem_t.xremdocdt    #170315-00043#2 by 09257 --add
DEFINE g_glaacomp          LIKE glaa_t.glaacomp     #170315-00043#2 by 09257 --add
DEFINE g_sfin3012          LIKE type_t.chr1         #170915-00038#1 add
#end add-point
...
{<section id="aapt930_01.input" type="s" >}
#+ 資料輸入
PUBLIC FUNCTION aapt930_01(--)
   ...
   DEFINE l_success       LIKE type_t.num5   #170606-00033#18 add
   #end add-point
   ...
   #170606-00033#18--add--s---
   #檢核該人員是否存在目前據點法人下
   CALL s_afa_ooag001_def_chk(g_xrem_m.xremsite,g_xrem_m.xrem004,'',1) RETURNING g_errno
   IF NOT cl_null(g_errno) THEN
      LET g_xrem_m.xrem004 = NULL
   END IF
   #170606-00033#18--add--e---
      ...
      INPUT BY NAME g_xrem_m.xremsite,g_xrem_m.xremld,g_xrem_m.xrem004,g_xrem_m.xremdocno,g_xrem_m.xrem001,
         ...
         AFTER FIELD xrem004
            ...
            IF NOT cl_null(g_xrem_m.xrem004) THEN
               ...
               #檢查該人員是否存在目前據點法人下
               ELSE
                  CALL s_afa_ooag001_chk(g_xrem_m.xremsite,g_xrem_m.xrem004,'',1) RETURNING l_success
         ...
         AFTER FIELD xremdocno
            ...
            #add-point:AFTER FIELD xremdocno name="input.a.xremdocno"
             IF NOT cl_null(g_xrem_m.xremdocno) THEN
               ...
               END IF
               #161104-00046#8 --s add
#               CALL s_control_chk_doc('1',g_xrem_m.xremdocno,'4',g_user,g_dept,'','') RETURNING g_sub_success,l_flag #170301-00027#3 20170303 mark by 08172
               CALL s_control_chk_doc('1',g_xrem_m.xremdocno,'4',g_user,g_dept,g_xrem_m.xremld,'') RETURNING g_sub_success,l_flag #170301-00027#3 20170303 add by 08172
         ...
         AFTER FIELD xremdocdt
            ...
            IF NOT cl_null(g_xrem_m.xremdocdt) THEN
               CALL cl_get_para(g_enterprise,g_glaacomp,'S-FIN-3007') RETURNING g_sfin3007
               IF NOT cl_null(g_sfin3007) THEN
                  IF g_xrem_m.xremdocdt < g_sfin3007 THEN
                     INITIALIZE g_errparam TO NULL
                     LET g_errparam.code = 'aap-00110'
                     LET g_errparam.extend = ''
                     LET g_errparam.popup = TRUE
                     CALL cl_err()
                     LET g_xrem_m.xremdocdt= g_xrem_m_t.xremdocdt
                     NEXT FIELD CURRENT
                  END IF
               END IF
            END IF
            #170315-00043#2 by 09257 --add_e
            #END add-point
...
{<section id="aapt930_01.other_dialog" readonly="Y" type="s" >}


{</section>}

{<section id="aapt930_01.other_function" readonly="Y" type="s" >}
...
DEFINE l_apcb007_2       LIKE apcb_t.apcb007
DEFINE l_apcb101         LIKE apcb_t.apcb101
DEFINE l_apcb103_320     LIKE apcb_t.apcb103
DEFINE l_apcb105_320     LIKE apcb_t.apcb105
...
DEFINE l_apcb103_2       LIKE apcb_t.apcb103
DEFINE l_apcb104_2       LIKE apcb_t.apcb104
DEFINE l_apcb105_2       LIKE apcb_t.apcb105
DEFINE l_apcb113_2       LIKE apcb_t.apcb113
DEFINE l_apcb114_2       LIKE apcb_t.apcb114
DEFINE l_apcb115_2       LIKE apcb_t.apcb115
DEFINE l_sum_apcb103     LIKE apcb_t.apcb103
DEFINE l_sum_apcb104     LIKE apcb_t.apcb104
DEFINE l_sum_apcb105     LIKE apcb_t.apcb105
DEFINE l_sum_apcb113     LIKE apcb_t.apcb113
DEFINE l_sum_apcb114     LIKE apcb_t.apcb114
DEFINE l_sum_apcb115     LIKE apcb_t.apcb115
DEFINE l_sum_apcb103_2   LIKE apcb_t.apcb103
DEFINE l_sum_apcb104_2   LIKE apcb_t.apcb104
DEFINE l_sum_apcb105_2   LIKE apcb_t.apcb105
DEFINE l_sum_apcb113_2   LIKE apcb_t.apcb113
DEFINE l_sum_apcb114_2   LIKE apcb_t.apcb114
DEFINE l_sum_apcb115_2   LIKE apcb_t.apcb115
...
DEFINE l_money           LIKE apcb_t.apcb105
#170915-00038#1---add---end---
#171116-00010#1---add---start---
DEFINE l_docdt2          LIKE xrea_t.xrea008
DEFINE l_apcadocdt       LIKE apca_t.apcadocdt
DEFINE l_apcb104_320     LIKE apcb_t.apcb104
DEFINE l_apcb113_320     LIKE apcb_t.apcb113
DEFINE l_apcb114_320     LIKE apcb_t.apcb114
DEFINE l_apcb115_320     LIKE apcb_t.apcb115
#171116-00010#1---add---end---
  ...
  #LET l_docdt = MDY(l_xrem.xrem002,1,l_xrem.xrem001) #年度期別第一天           #170915-00038#1 mark
  #LET l_docdt = MDY(l_xrem.xrem002+1,1,l_xrem.xrem001) - 1 #年度期別第一天     #170915-00038#1 add    #171116-00010#1 mark
   LET l_docdt = MDY(l_xrem.xrem002,1,l_xrem.xrem001)        #年度期別第一天#   #171116-00010#1 add
   #LET l_docdt2 = MDY(l_xrem.xrem002+1,1,l_xrem.xrem001) - 1 #年度期別最後一天  #171116-00010#1 add   #180110-00048#1 mark
   #180110-00048#1---add---str--
   IF l_xrem.xrem002 = 12 THEN
      LET l_docdt2 = MDY(l_xrem.xrem002,31,l_xrem.xrem001) #年度期別最後一天
   ELSE
      LET l_docdt2 = MDY(l_xrem.xrem002+1,1,l_xrem.xrem001) - 1 #年度期別最後一天
   END IF
   #180110-00048#1---add---end--
   #LET g_sql = " SELECT *                               ",   #161208-00026#15   mark
   #170728-00030#1---mark---start---
   ##161208-00026#15   add---s
   ...
   #170728-00030#1---mark---end---
   #170915-00038#1---mark---start---
   ##170728-00030#1---add---start---
   #   LET g_sql = " SELECT xreaent,xreacomp,xreasite,xreald,xrea001,",
   #               "     xrea002,xrea003,xrea004,xrea005,xrea006,",
   #               "     xrea007,xrea008,xrea009,xrea010,xrea011,",
   #               "     xrea012,xrea013,xrea014,xrea015,xrea016,",
   ...
   #               "     xrea041,xrea042,xrea043,xrea100,xrea101,",
   #               "     xrea102,",
   #               "     CASE WHEN xrea004 = '02' THEN xrea103 * -1 ELSE xrea103 END AS xrea103,",
   #               "     CASE WHEN xrea004 = '02' THEN xrea104 * -1 ELSE xrea104 END AS xrea104,",
   #               "     CASE WHEN xrea004 = '02' THEN xrea105 * -1 ELSE xrea105 END AS xrea105,",
   #               "     CASE WHEN xrea004 = '02' THEN xrea106 * -1 ELSE xrea106 END AS xrea106,",
   #               "     CASE WHEN xrea004 = '02' THEN xrea113 * -1 ELSE xrea113 END AS xrea113,",
   #               "     CASE WHEN xrea004 = '02' THEN xrea114 * -1 ELSE xrea114 END AS xrea114,",
   #               "     CASE WHEN xrea004 = '02' THEN xrea115 * -1 ELSE xrea115 END AS xrea115,",
   #               "     CASE WHEN xrea004 = '02' THEN xrea116 * -1 ELSE xrea116 END AS xrea116,",
   #               "     xrea122,",
   #               "     CASE WHEN xrea004 = '02' THEN xrea123 * -1 ELSE xrea123 END AS xrea123,",
   #               "     xrea132,",
   #               "     CASE WHEN xrea004 = '02' THEN xrea133 * -1 ELSE xrea133 END AS xrea133,",
   #               "     xreaud001,xreaud002,",
   #               "     xreaud003,xreaud004,xreaud005,xreaud006,xreaud007,",
   #               "     xreaud008,xreaud009,xreaud010,xreaud011,xreaud012,",
   #               "     xreaud013,xreaud014,xreaud015,xreaud016,xreaud017,",
   ...
   #               "     xreaud023,xreaud024,xreaud025,xreaud026,xreaud027,",
   #               "     xreaud028,xreaud029,xreaud030,xrea044,xrea045 ",
   ##170728-00030#1---add---end---
   #170915-00038#1---mark---end---
   #170915-00038#1---add---start---
   LET g_sql = " SELECT DISTINCT xreaent,xreacomp,xreasite,xreald,xrea001,",
               "     xrea002,xrea003,xrea004,xrea005,xrea008,",
               "     xrea009,xrea010,xrea011,xrea012,xrea013,",
               "     xrea014,xrea015,xrea016,xrea017,xrea018,",
               "     xreaorga,xrea020,xrea021,xrea022,",
               "     xrea023,xrea024,xrea025,xrea026,xrea027,",
               "     xrea028,xrea029,xrea030,xrea031,xrea032,",
               "     xrea033,xrea034,xrea035,xrea036,xrea037,",
               "     xrea038,xrea039,xrea040,xrea041,xrea042,",
               "     xrea043,xrea100,xrea101,xrea102,xrea122,",
               "     xrea132,xreaud001,xreaud002,xreaud003,xreaud004,",
               "     xreaud005,xreaud006,xreaud007,xreaud008,xreaud009,",
               "     xreaud010,xreaud011,xreaud012,xreaud013,xreaud014,",
               "     xreaud015,xreaud016,xreaud017,xreaud018,xreaud019,",
               "     xreaud020,xreaud021,xreaud022,xreaud023,xreaud024,",
               "     xreaud025,xreaud026,xreaud027,xreaud028,xreaud029,",
               "     xreaud030,xrea045 ",
   #170915-00038#1---add---end---
               "   FROM xrea_t                          ",
   ...
   IF g_argv1 = '2' THEN
     #LET g_sql = g_sql CLIPPED, " AND xrea008 < '",l_docdt,"' "     #170915-00038#1 mark
     #LET g_sql = g_sql CLIPPED, " AND xrea008 <= '",l_docdt,"' "    #170915-00038#1 add    #171116-00010#1 mark
       LET g_sql = g_sql CLIPPED, " AND xrea008 < '",l_docdt,"' "                           #171116-00010#1 add
   END IF
   ...
   #170915-00038#1---add---start---
   CALL cl_get_para(g_enterprise,l_xrem.xremcomp,'S-FIN-3012') RETURNING g_sfin3012
   #aapt320
#   #171204-00028#1---mark----str
#   LET l_sql = "SELECT apcbseq,apcb002,apcb003,apcb007,apcb101,",
#               "       apcb103,apcb105,apcb021,apcb029,apca013 ",
#               #170915-00038#10 --s add 核算項資料
#               "       ,apcb054,apcb055,apcb010,apcb011, ",         # 帳款對象 / 付款對象 / 業務部門 / 責任中心
#               "       apcb024,apcb036,apcb012,apcb051,apcb015, ",  # 區域 / 客群 / 產品類別 / 業務人員 / 專案編號
#               "       apcb016,apcb033,apcb034,apcb035,apcb037, ",  # WBS編號 / 經營方式 / 通路 / 品牌 / 自由核算項一
#               "       apcb038,apcb039,apcb040,apcb041,apcb042, ",  # 自由核算項二 / 自由核算項三 / 自由核算項四 / 自由核算項五 / 自由核算項六
#               "       apcb043,apcb044,apcb045,apcb046,apcb047  ",  # 自由核算項七 / 自由核算項八 / 自由核算項九 / 自由核算項十 / 摘要
#               #170915-00038#10 --e add
#               "  FROM apcb_t ",
#               "  LEFT JOIN apca_t ON apcaent = apcbent AND apcadocno = apcbdocno ",
#               " WHERE apcbent = ",g_enterprise,
#               "   AND apcbld = ?",
#               "   AND apcbdocno = ?",
#               "   AND apcastus = 'Y'"
#   #171204-00028#1---mark----end
   #171204-00028#1---add----str
   LET l_sql = "SELECT apcbseq,apcb002,apcb003,apcb007,apcb101,",
               "       apcb103,apcb105,apcb021,apcb029,apca013 ",
               "       ,apca004,apca005,apcb010,apcb011, ",         # 帳款對象 / 付款對象 / 業務部門 / 責任中心
               "       apcb024,apcb036,apcb012,apcb051,apcb015, ",  # 區域 / 客群 / 產品類別 / 業務人員 / 專案編號
               "       apcb016,apcb033,apcb034,apcb035,apcb037, ",  # WBS編號 / 經營方式 / 通路 / 品牌 / 自由核算項一
               "       apcb038,apcb039,apcb040,apcb041,apcb042, ",  # 自由核算項二 / 自由核算項三 / 自由核算項四 / 自由核算項五 / 自由核算項六
               "       apcb043,apcb044,apcb045,apcb046,apcb047  ",  # 自由核算項七 / 自由核算項八 / 自由核算項九 / 自由核算項十 / 摘要
               "      ,apcadocdt,apcb104,apcb113,apcb114,apcb115 ", #171116-00010#1 add
               "  FROM apcb_t,apca_t ",
               " WHERE apcaent = apcbent AND apcadocno = apcbdocno AND apcbent = ",g_enterprise,
               "   AND apcbld = ?",
               "   AND apcbdocno = ?",
               "   AND apcastus = 'Y'"
   #171204-00028#1---add----end
   PREPARE aapt930_sel_apcb_p FROM l_sql
   DECLARE aapt930_sel_apcb_c CURSOR FOR aapt930_sel_apcb_p

   #有來源單：入庫/倉退
   #aapt300
   LET l_sql = "SELECT SUM(apcb007),SUM(apcb103),SUM(apcb104),SUM(apcb105), ",      #數量/原幣未稅/原幣稅額/原幣含稅
               "       SUM(apcb113),SUM(apcb114),SUM(apcb115)",                     #原幣未稅/原幣稅額/原幣含稅
               "  FROM apcb_t ",
               "  LEFT JOIN apca_t ON apcaent = apcbent AND apcadocno = apcbdocno ",
               " WHERE apcbent = ",g_enterprise,
               "   AND apcb002 = ?",
               ...
               "   AND apcadocdt BETWEEN ? AND '",l_docdt2,"'",       #171116-00010#1 add
               "   AND apcb023 = 'Y'",                                #171116-00010#1 add
               "   AND apcastus = 'Y'"
   PREPARE aapt930_sel_apcb2 FROM l_sql

   #雜項
   #aapt301
   LET l_sql = "SELECT SUM(apcb103),SUM(apcb104),SUM(apcb105),SUM(apcb113),SUM(apcb114),SUM(apcb115) ",          #原幣含稅金額
               "  FROM apcb_t ",
               "  LEFT JOIN apca_t ON apcaent = apcbent AND apcadocno = apcbdocno ",
               " WHERE apcbent = ",g_enterprise,
               "   AND apcb002 = ?",
               "   AND apcb003 = ?",
               "   AND apca001 IN ('16','19','29')",
               "   AND apcadocdt BETWEEN ? AND '",l_docdt2,"'",       #171116-00010#1 add
               "   AND apcb023 = 'Y'",                                #171116-00010#1 add
               "   AND apcastus = 'Y'"
   PREPARE aapt930_sel_apcb3 FROM l_sql

   LET l_sql = "SELECT MAX(xrenseq)+1 ",
               "  FROM xren_t ",
               " WHERE xrenent = ",g_enterprise,
               "   AND xrendocno = '",p_xremdocno,"'"
   PREPARE aapt930_sel_xrenseq_c FROM l_sql
   ...
   FOREACH aapt931_ins_curs
      INTO l_xrea.xreaent,l_xrea.xreacomp,l_xrea.xreasite,l_xrea.xreald,l_xrea.xrea001,
           ...
           l_xrea.xrea038,l_xrea.xrea039,l_xrea.xrea040,l_xrea.xrea041,l_xrea.xrea042,
           l_xrea.xrea043,l_xrea.xrea100,l_xrea.xrea101,l_xrea.xrea102,l_xrea.xrea122,
           l_xrea.xrea132,l_xrea.xreaud001,l_xrea.xreaud002,l_xrea.xreaud003,l_xrea.xreaud004,
           l_xrea.xreaud005,l_xrea.xreaud006,l_xrea.xreaud007,l_xrea.xreaud008,l_xrea.xreaud009,
           l_xrea.xreaud010,l_xrea.xreaud011,l_xrea.xreaud012,l_xrea.xreaud013,l_xrea.xreaud014,
           l_xrea.xreaud015,l_xrea.xreaud016,l_xrea.xreaud017,l_xrea.xreaud018,l_xrea.xreaud019,
           l_xrea.xreaud020,l_xrea.xreaud021,l_xrea.xreaud022,l_xrea.xreaud023,l_xrea.xreaud024,
           l_xrea.xreaud025,l_xrea.xreaud026,l_xrea.xreaud027,l_xrea.xreaud028,l_xrea.xreaud029,
           l_xrea.xreaud030,l_xrea.xrea045
       ...
       SELECT glab005 INTO l_xren.xren042
        ...
        WHERE glabent = g_enterprise
          AND glab003 = '8504_08'
          AND glabld  = g_xrem_m.xremld
          AND glab002 = (SELECT apca007 FROM apca_t WHERE apcadocno = l_xrea.xrea005 AND apcaent = g_enterprise)
       ...
       LET l_apcb101 = 0
       LET l_apcb103_320 = 0
       LET l_sum_apcb103_2 = 0
       LET l_sum_apcb104_2 = 0
       LET l_sum_apcb105_2 = 0
       LET l_sum_apcb113_2 = 0
       LET l_sum_apcb114_2 = 0
       LET l_sum_apcb115_2 = 0
       #171116-00010#1---add---start---
       LET l_apcb104_320 = 0
       LET l_apcb105_320 = 0
       LET l_apcb113_320 = 0
       ...
       LET l_apcb115_320 = 0
       #171116-00010#1---add---end---
       FOREACH aapt930_sel_apcb_c USING l_xrea.xreald,l_xrea.xrea005
          INTO l_apcbseq,l_apcb002,l_apcb003,l_apcb007,l_apcb101,
               l_apcb103_320,l_apcb105_320,l_xren.xren043,l_xren.xren019,l_apca013
               #170915-00038#10 --s add 核算項欄位
               ,l_xrea.xrea009,l_xrea.xrea010,l_xrea.xrea011,l_xrea.xrea012,l_xrea.xrea013,
               l_xrea.xrea014,l_xrea.xrea015,l_xrea.xrea016,l_xrea.xrea017,l_xrea.xrea018,
               l_xrea.xrea020,l_xrea.xrea021,l_xrea.xrea022,l_xrea.xrea023,l_xrea.xrea024,
               l_xrea.xrea025,l_xrea.xrea026,l_xrea.xrea027,l_xrea.xrea028,l_xrea.xrea029,
               l_xrea.xrea030,l_xrea.xrea031,l_xrea.xrea032,l_xrea.xrea033
               #170915-00038#10 --e add
              ,l_apcadocdt,l_apcb104_320,l_apcb113_320,l_apcb114_320,l_apcb115_320      #171116-00010#1 add

             IF cl_null(l_apcb007) THEN LET l_apcb007 = 0 END IF
             IF cl_null(l_apcb101) THEN LET l_apcb101 = 0 END IF
             IF cl_null(l_apcb103_320) THEN LET l_apcb103_320 = 0 END IF
             #171116-00010#1---add---start---
             IF cl_null(l_apcb104_320) THEN LET l_apcb104_320 = 0 END IF
             IF cl_null(l_apcb105_320) THEN LET l_apcb105_320 = 0 END IF
             IF cl_null(l_apcb113_320) THEN LET l_apcb113_320 = 0 END IF
             IF cl_null(l_apcb114_320) THEN LET l_apcb114_320 = 0 END IF
             IF cl_null(l_apcb115_320) THEN LET l_apcb115_320 = 0 END IF
             #171116-00010#1---add---end---
             LET l_apcb007_2 = 0
             LET l_apcb103 = 0
             LET l_apcb104 = 0
             ...
             LET l_apcb114 = 0
             LET l_apcb115 = 0
             IF l_xrea.xrea004 = '01' OR l_xrea.xrea004 = '02' THEN       #入庫/倉退
                EXECUTE aapt930_sel_apcb2 USING l_apcb002,l_apcb003       #入庫/倉退單號，項次
                                               ,l_apcadocdt               #171116-00010#1 add
                   INTO l_apcb007_2,l_apcb103,l_apcb104,l_apcb105,l_apcb113,l_apcb114,l_apcb115
                IF cl_null(l_apcb007_2) THEN LET l_apcb007_2 = 0 END IF
                ...
                #171116-00010#1---mark---start---
                ##170915-00038#10 --s add
                #LET l_sum_apcb103_2 = l_sum_apcb103_2 + l_apcb103
                #LET l_sum_apcb104_2 = l_sum_apcb104_2 + l_apcb104
                #LET l_sum_apcb105_2 = l_sum_apcb105_2 + l_apcb105
                #LET l_sum_apcb113_2 = l_sum_apcb113_2 + l_apcb113
                #LET l_sum_apcb114_2 = l_sum_apcb114_2 + l_apcb114
                #LET l_sum_apcb115_2 = l_sum_apcb115_2 + l_apcb115
                ##170915-00038#10 --e add
                #171116-00010#1---add---end---
                IF l_apcb007 = l_apcb007_2 THEN
                   #171116-00010#1---add---start---
                   LET l_sum_apcb103_2 = l_sum_apcb103_2 + l_apcb103_320
                   LET l_sum_apcb104_2 = l_sum_apcb104_2 + l_apcb104_320
                   LET l_sum_apcb105_2 = l_sum_apcb105_2 + l_apcb105_320
                   LET l_sum_apcb113_2 = l_sum_apcb113_2 + l_apcb113_320
                   LET l_sum_apcb114_2 = l_sum_apcb114_2 + l_apcb114_320
                   LET l_sum_apcb115_2 = l_sum_apcb115_2 + l_apcb115_320
                   #171116-00010#1---add---end---
                   CONTINUE FOREACH
                ELSE
                   #171116-00010#1---add---start---
                   LET l_apcb103 = l_apcb101 * l_apcb007_2    #原幣暫估金額
                   IF l_apcb103 = 0 THEN
                      LET l_apcb103 = 0
                      LET l_apcb104 = 0
                      LET l_apcb105 = 0
                      ...
                      LET l_apcb115 = 0
                   ELSE
                      CALL s_tax_count(l_xrea.xreasite,l_xrea.xrea040,l_apcb103,l_apcb007_2,l_xrea.xrea100,l_xrea.xrea102)
                      RETURNING l_apcb103,l_apcb104,l_apcb105,l_apcb113,l_apcb114,l_apcb115
                      IF cl_null(l_apcb103) THEN LET l_apcb103 = 0 END IF
                      IF cl_null(l_apcb104) THEN LET l_apcb104 = 0 END IF
                      IF cl_null(l_apcb105) THEN LET l_apcb105 = 0 END IF
                      IF cl_null(l_apcb113) THEN LET l_apcb113 = 0 END IF
                   ...
                   LET l_sum_apcb114_2 = l_sum_apcb114_2 + l_apcb114
                   LET l_sum_apcb115_2 = l_sum_apcb115_2 + l_apcb115
                   IF l_apcb007_2 = 0 THEN
                      LET l_xren.xren103 = l_apcb103_320
                      LET l_xren.xren104 = l_apcb104_320
                      LET l_xren.xren105 = l_apcb105_320
                      LET l_xren.xren113 = l_apcb113_320
                      LET l_xren.xren114 = l_apcb114_320
                      LET l_xren.xren115 = l_apcb115_320
                   ELSE
                   #171116-00010#1---add---end---
                      LET l_xren.xren103 = l_apcb101 * (l_apcb007 - l_apcb007_2)    #原幣暫估金額
                      CALL s_tax_count(l_xrea.xreasite,l_xrea.xrea040,l_xren.xren103,l_apcb007-l_apcb007_2,l_xrea.xrea100,l_xrea.xrea102)
                      RETURNING l_xren.xren103,l_xren.xren104,l_xren.xren105,l_xren.xren113,l_xren.xren114,l_xren.xren115
                   END IF        #171116-00010#1 add
                   #170915-00038#10 --s mark
                   #LET l_sum_apcb103_2 = l_sum_apcb103_2 + l_apcb103
                   #LET l_sum_apcb104_2 = l_sum_apcb104_2 + l_apcb104
                   #LET l_sum_apcb105_2 = l_sum_apcbz105_2 + l_apcb105
                   #LET l_sum_apcb113_2 = l_sum_apcb113_2 + l_apcb113
                   #LET l_sum_apcb114_2 = l_sum_apcb114_2 + l_apcb114
                   #LET l_sum_apcb115_2 = l_sum_apcb115_2 + l_apcb115
             ...
             ELSE
                ...
                LET l_apcb114_2 = 0
                LET l_apcb115_2 = 0
                EXECUTE aapt930_sel_apcb3 USING l_xrea.xrea005,l_apcbseq    #暫估單號。項次
                                               ,l_apcadocdt                 #171116-00010#1 add
                   INTO l_apcb103_2,l_apcb104_2,l_apcb105_2,l_apcb113_2,l_apcb114_2,l_apcb115_2
                IF cl_null(l_apcb103_2) THEN LET l_apcb103_2 = 0 END IF
                IF cl_null(l_apcb104_2) THEN LET l_apcb104_2 = 0 END IF
                IF cl_null(l_apcb105_2) THEN LET l_apcb105_2 = 0 END IF
                IF cl_null(l_apcb113_2) THEN LET l_apcb113_2 = 0 END IF
                IF cl_null(l_apcb114_2) THEN LET l_apcb114_2 = 0 END IF
                IF cl_null(l_apcb115_2) THEN LET l_apcb115_2 = 0 END IF

                #170915-00038#10 --s add
                LET l_sum_apcb103_2 = l_sum_apcb103_2 + l_apcb103_2
                LET l_sum_apcb104_2 = l_sum_apcb104_2 + l_apcb104_2
                LET l_sum_apcb105_2 = l_sum_apcb105_2 + l_apcb105_2
                LET l_sum_apcb113_2 = l_sum_apcb113_2 + l_apcb113_2
                LET l_sum_apcb114_2 = l_sum_apcb114_2 + l_apcb114_2
                LET l_sum_apcb115_2 = l_sum_apcb115_2 + l_apcb115_2
                #170915-00038#10 --e add

                IF l_apcb103_320 = l_apcb103_2 THEN
                   CONTINUE FOREACH
                ELSE
                   #171116-00010#1---add---start---
                   IF l_apcb103_2 = 0 THEN
                      LET l_xren.xren103 = l_apcb103_320
                      LET l_xren.xren104 = l_apcb104_320
                      LET l_xren.xren105 = l_apcb105_320
                      LET l_xren.xren113 = l_apcb113_320
                   ...
                   #171116-00010#1---add---end---
                      IF l_apca013 = 'Y' THEN
                         LET l_money = l_apcb105_320-l_apcb105_2
                      ELSE
                         LET l_money = l_apcb103_320-l_apcb103_2
                      END IF
                      CALL s_tax_count(l_xrea.xreasite,l_xrea.xrea040,l_money,1,l_xrea.xrea100,l_xrea.xrea102)
                      RETURNING l_xren.xren103,l_xren.xren104,l_xren.xren105,l_xren.xren113,l_xren.xren114,l_xren.xren115
                   END IF          #171116-00010#1 add
                   #170915-00038#10 --s mark
                   #LET l_sum_apcb103_2 = l_sum_apcb103_2 + l_apcb103_2
   ...
   #170915-00038#1---add---end---
            ...
            #LET l_xren.xren006 = l_xrea.xrea006    #170915-00038#1 mark
            #LET l_xren.xren007 = l_xrea.xrea007    #170915-00038#1 mark
             LET l_xren.xren007 = 0                 #170915-00038#1
            ...
            #LET l_xren.xren019 = l_xrea.xrea019    #應收付科目   #170915-00038#1 mark
            ...
            #LET l_xren.xren103 = l_xrea.xrea104 #170915-00038#1 mark
            #LET l_xren.xren104 = l_xrea.xrea105 #170915-00038#1 mark
            #LET l_xren.xren105 = l_xrea.xrea106 #170915-00038#1 mark
            #LET l_xren.xren113 = l_xrea.xrea114 #170915-00038#1 mark
            #LET l_xren.xren114 = l_xrea.xrea115 #170915-00038#1 mark
            #LET l_xren.xren115 = l_xrea.xrea116 #170915-00038#1 mark
       ...
       SELECT SUM(apcb103),SUM(apcb104),SUM(apcb105),SUM(apcb113),SUM(apcb114),SUM(apcb115)
         INTO l_sum_apcb103,l_sum_apcb104,l_sum_apcb105,l_sum_apcb113,l_sum_apcb114,l_sum_apcb115
         FROM apcb_t
         LEFT JOIN apca_t ON apcaent = apcbent AND apcadocno = apcbdocno
        WHERE apcbdocno = l_xrea.xrea005
          AND apca001 = l_xrea.xrea004
          AND apcbent = g_enterprise

       IF cl_null(l_sum_apcb103) THEN LET l_sum_apcb103 = 0 END IF
       IF cl_null(l_sum_apcb104) THEN LET l_sum_apcb104 = 0 END IF
       IF cl_null(l_sum_apcb105) THEN LET l_sum_apcb105 = 0 END IF
       IF cl_null(l_sum_apcb113) THEN LET l_sum_apcb113 = 0 END IF
       IF cl_null(l_sum_apcb114) THEN LET l_sum_apcb114 = 0 END IF
       IF cl_null(l_sum_apcb115) THEN LET l_sum_apcb115 = 0 END IF
       ...
       LET l_diff_114 = 0
       LET l_diff_115 = 0
       LET l_diff_103 = l_sum_apcb103 - (l_sum_xren103 + l_sum_apcb103_2)
       LET l_diff_104 = l_sum_apcb104 - (l_sum_xren104 + l_sum_apcb104_2)
       LET l_diff_105 = l_sum_apcb105 - (l_sum_xren105 + l_sum_apcb105_2)
       LET l_diff_113 = l_sum_apcb113 - (l_sum_xren113 + l_sum_apcb113_2)
       LET l_diff_114 = l_sum_apcb114 - (l_sum_xren114 + l_sum_apcb114_2)
       LET l_diff_115 = l_sum_apcb115 - (l_sum_xren115 + l_sum_apcb115_2)

       IF g_sfin3012 = '1' THEN            #暫估不含稅
          UPDATE xren_t
