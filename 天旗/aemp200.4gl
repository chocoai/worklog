# Prog. Version..: '5.30.27-19.04.12(00009)'     #
#
...
#                                                   依成本計算類別取得該料的成本平均
# Modify.........: No.FUN-D70120 13/07/30 By yangtt 會計年期會不按照自然年月設置,年期的判斷需要按照aooq011的設置
...
DEFINE   g_flag1         LIKE type_file.chr1     #No.FUN-D70120
...
FUNCTION p200_tm()
   ...
   CALL cl_ui_init()
   CALL cl_opmsg('p')
  #LET tm.yy = YEAR(g_today)   #FUN-D70120
  #LET tm.mm = MONTH(g_today)  #FUN-D70120
  #FUN-D70120---add---str--
  CALL s_get_bookno(YEAR(g_today)) RETURNING g_flag1,g_bookno1,g_bookno2
  IF g_flag1 = '1' THEN
     CALL cl_err(YEAR(g_today),'aoo-081',1)
  END IF
  LET tm.yy = s_get_aznn(g_plant,g_bookno1,g_today,1)   
  LET tm.mm = s_get_aznn(g_plant,g_bookno1,g_today,3)   
  #FUN-D70120---add---end--
