...
#+ Standard Version.....: SD版次:0022(2015-09-01 11:44:22), PR版次:0022(2017-08-14 17:51:05)
...
#170302-00012#10 2017/03/24 By 08734  整批调整未产生数据时，提示消息修正。
#170429-00001#1  2017/05/09 By lixh   信用評等" 欄位開窗顯示应该是信用評等條件維護作業(axmi140)的資料
#170429-00002#1  2017/05/09 By wuxja  当下信用评等条件时应都以额度客户为基准
#170804-00048#1  2017/08/11 By 07024  若集團集的幣別有修改，集團幣別(xmab019)應該也要更新，也要更新集團匯率(xmab020)的資料
#170804-00048#3  2017/08/14 By 08171  回寫xmab012與xmab013時，目前是用xmab005(交易營運據點)當條件，但應該用xmab018(額度營運據點)當條件才對
#end add-point
...
DEFINE g_flag                LIKE type_t.num5  #170302-00012#10   2017/03/24  By 08734 add   
   ...
   WHILE TRUE
      ...
      DIALOG ATTRIBUTES(UNBUFFERED,FIELD ORDER FORM)
         ...
         ON ACTION controlp INFIELD pmab004
            ...
            LET g_qryparam.state = 'c'
            LET g_qryparam.reqry = FALSE
            #170429-00001#1-S  mark
            #LET g_qryparam.arg1 = '296'
            #CALL q_oocq002()  #呼叫開窗
            #170429-00001#1-E  mark
            CALL q_xmaj001()   #170429-00001#1 add  
...
DEFINE l_xmab018    LIKE xmab_t.xmab018           #170804-00048#3 add #額度交易據點
DEFINE l_xmab019    LIKE xmab_t.xmab019           #170804-00048#1-add
DEFINE l_xmab020    LIKE xmab_t.xmab020           #170804-00048#1-add
...
DEFINE l_xmaa004    LIKE xmaa_t.xmaa004           #160926-00001#1 add
  ...
  #END IF
   ...
   LET g_flag = FALSE  #170302-00012#10   2017/03/24  By 08734 add
   ...
   IF g_bgjob <> "Y" THEN
      ...
      LET ls_sql = "SELECT COUNT(1) ",   #160810-00026#1 mod COUNT(*)->COUNT(1)
                  ...
                  #"    AND ",lc_param.wc   #150930-00021#1 20151002  add by beckxie   #170429-00002#1 mark
                   #170429-00002#1 add  --begin--
                   "    AND pmab003 IN(SELECT DISTINCT pmab003 FROM pmab_t ",
                   "                    WHERE pmabent = ",g_enterprise,
                   "                      AND pmab002 NOT IN ('1','4') ",
                   "                      AND ",lc_param.wc,")"
                   #170429-00002#1 add  --end--
...
#  FOREACH axmp140_process_cs INTO
   ...
   LET l_sql = "SELECT DISTINCT pmabsite,pmab003",
               ...
               "  WHERE pmabent = ",g_enterprise,
               "    AND pmab002 NOT IN ('1','4') ",
              #"    AND ",lc_param.wc,     #170429-00002#1 mark
               #170429-00002#1 add  --begin--
               "    AND pmab003 IN(SELECT DISTINCT pmab003 FROM pmab_t ",
               "                    WHERE pmabent = ",g_enterprise,
               "                      AND pmab002 NOT IN ('1','4') ",
...
#效能考量,搬到FOREACH外
  ...
  #IF l_pmab.pmabsite <> 'ALL' THEN
   #LET l_sql1 = l_sql," AND xmab005 = ?" #170804-00048#3 mark
   LET l_sql1 = l_sql," AND xmab018 = ?"  #170804-00048#3 add
  #END IF
   ...
   #更新異動檔的匯率   
   #LET l_sql = "SELECT DISTINCT xmab005,xmab007,xmab012 FROM xmab_t", #170804-00048#3 mark
   LET l_sql = "SELECT DISTINCT xmab005,xmab007,xmab012,xmab018 FROM xmab_t", #170804-00048#3 add
               "  WHERE xmabent = ?",
               "    AND xmab005 = ?",             #交易營運據點
               "    AND xmab008 = ?"
   ...
   #取得集團級的資料(更新匯率用)
   LET l_sql = "SELECT DISTINCT xmab005,xmab007,xmab019 ",
               "  FROM xmab_t",
               " WHERE xmabent = ?",
               "   AND xmab008 = ?"
   PREPARE axmp140_sel_all_pmab005_pr FROM l_sql
   DECLARE axmp140_sel_all_pmab005_cs CURSOR FOR axmp140_sel_all_pmab005_pr
   ...
   LET l_sql = "UPDATE xmab_t",
               "   SET xmab012 = ?",
               " WHERE xmabent = ?",
               #"   AND xmab005 = ?", #170804-00048#3 mark
               "   AND xmab018 = ?",  #170804-00048#3 add
               "   AND xmab008 = ?"
   ...
   LET l_sql = "UPDATE xmab_t",
               ...
               " WHERE xmabent = ?",
               "   AND xmab008 = ?"
   PREPARE axmp140_update_xmab019_prep FROM l_sql
   #170804-00048#1-e-add
   ...
   LET l_sql = "UPDATE xmab_t",
               ...
               "   AND xmab007 = ?",
               "   AND xmab019 = ?"
   PREPARE axmp140_update_xmab020_prep FROM l_sql
   #170804-00048#1-e-add
   ...
   LET l_sql = "SELECT COUNT(1) ",
               "  FROM xmab_t ",
               " WHERE xmabent = ?",
               #"   AND xmab005 = ?",                       #交易營運據點 #170804-00048#3 mark
               "   AND xmab018 = ?",                        #額度營運據點 #170804-00048#3 add
               "   AND xmab006 = ?",                        #交易客戶
   ...
   LET l_sql = "SELECT COUNT(1) ",
               ...
               " WHERE xmabent = ?",
               "   AND xmab006 = ?",         #交易客戶
               "   AND EXISTS (SELECT pmab005 FROM pmab_t ",
               "                WHERE pmabent = xmabent AND pmab001 = xmab006 AND pmabsite='ALL' ",
               "                  AND (pmab005 <> xmab019  OR (pmab005 IS NULL AND xmab019 IS NOT NULL) OR (pmab005 IS NOT NULL AND xmab019 IS NULL) ) ",
               "               ) "
   PREPARE axmp140_process_sel_all_xmab_pre1 FROM l_sql
   #170804-00048#1-e-add
   ...
   LET l_sql = "SELECT COUNT(1) ",
               ...
               "   AND xmacsite = ?",
               "   AND xmac001 = ?",
               #"   AND xmac002 <> ?" #170804-00048#3 mark
               "   AND (xmac002 <> ? OR xmac002 IS NULL) " #170804-00048#3 add
      ...
      IF cl_null(l_cnt) OR l_cnt = 0 THEN
        ...
        #160810-00026#1-e mod
         ...
         IF SQLCA.sqlcode THEN
            ...
            EXIT FOREACH
         ELSE 
            LET g_flag = TRUE  #170302-00012#10   2017/03/24  By 08734 add(E)
         END IF         
     ...
     #160922-00029#1-e-add
      ...
      IF l_pmab.pmabsite <> 'ALL' THEN
         ...
         IF NOT cl_null(l_pmab.pmab003) THEN              #151125-00014#1 add
           ...
           #EXECUTE axmp140_process_sel_xmab_568cnt_pr2 USING g_enterprise,l_pmab.pmabsite,l_pmab.pmab001,l_pmab.pmab003 INTO l_cnt  #170429-00002#1 mark
            EXECUTE axmp140_process_sel_xmab_568cnt_pr1 USING g_enterprise,l_pmab.pmabsite,l_pmab.pmab001,l_pmab.pmab003 INTO l_cnt  #170429-00002#1 add
           #160810-00026#1-e mod
        #151125-00014#1--add--(s) 
         ...
         IF NOT cl_null(l_cnt) AND l_cnt > 0 THEN
           ...
           #60810-00026#1-e mod
            #170302-00012#10   2017/03/24  By 08734 add(S)      
            IF SQLCA.sqlerrd[3] > 0 THEN
               LET g_flag = TRUE
            END IF
            #170302-00012#10   2017/03/24  By 08734 add(E)
         END IF 
...
#160810-00026#1-e mark
           ...
           #FOREACH axmp140_sel_pmab005_cs INTO l_xmab005,l_xmab007,l_xmab012
            FOREACH axmp140_sel_pmab005_cs USING g_enterprise,l_pmab.pmabsite,l_pmab.pmab003
                                           #INTO l_xmab005,l_xmab007,l_xmab012 #170804-00048#3 mark
                                           INTO l_xmab005,l_xmab007,l_xmab012,l_xmab018 #170804-00048#3 add
           #160810-00026#1-e mod
             　LET l_xmab013 = 0
               LET l_exchange = 0
               IF NOT cl_null(l_xmab012) THEN
                 ...
                 #CALL s_aooi160_get_exrate('1',l_xmab005,g_today,l_xmab007,l_xmab012,0,l_para_data) RETURNING l_exchange   #160906-00029#1 mark
                  #CALL s_credit_get_exrate('1',l_xmab005,g_today,l_xmab007,l_xmab012,0,l_para_data) RETURNING l_exchange    #160906-00029#1 add #170804-00048#3 mark
                  CALL s_credit_get_exrate('1',l_xmab018,g_today,l_xmab007,l_xmab012,0,l_para_data) RETURNING l_exchange    #170804-00048#3 add 用額度交易對象抓匯率
      ...
      ELSE
         ...
         #檢查集團集的幣別是否相同
         LET l_cnt = 0
         EXECUTE axmp140_process_sel_all_xmab_pre1 USING g_enterprise,l_pmab.pmab001 INTO l_cnt
         
         IF l_cnt > 0 THEN 
            #更新幣別
            EXECUTE axmp140_update_xmab019_prep USING l_pmab.pmab005,g_enterprise,l_pmab.pmab003
            #更新匯率
            FOREACH axmp140_sel_all_pmab005_cs USING g_enterprise,l_pmab.pmab003
                                                INTO l_xmab005,l_xmab007,l_xmab019
               LET l_xmab020 = 0
               LET l_exchange = 0
               IF NOT cl_null(l_xmab019) THEN 
                  #取得參數值               
                  EXECUTE axmp140_sel_ooab002_pr USING g_enterprise,l_xmab005,l_para_id INTO l_para_data
                  #取得匯率
                  CALL s_credit_get_exrate('1',l_xmab005,g_today,l_xmab007,l_xmab019,0,l_para_data) RETURNING l_exchange 
                  
                  LET l_xmab020 = l_exchange
                  #更新匯率資料
                  EXECUTE axmp140_update_xmab020_prep
                    USING l_xmab020,g_enterprise,l_pmab.pmab003,l_xmab005,l_xmab007,l_xmab019
                  IF SQLCA.sqlerrd[3] > 0 THEN
                     LET g_flag = TRUE
                  END IF
               END IF          
