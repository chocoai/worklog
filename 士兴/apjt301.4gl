...
#+ Standard Version.....: SD版次:0008(2015-10-08 14:13:51), PR版次:0008(2017-10-06 11:28:08)
#+ Customerized Version.: SD版次:0000(1900-01-01 00:00:00), PR版次:0000(1900-01-01 00:00:00)
#+ Build......: 000074
#+ Filename...: apjt301
#+ Description: 專案人工製費收集維護作業
#+ Creator....: 02294(2015-10-08 10:30:30)
#+ Modifier...: 02294 -SD/PR- 10044
 
{</section>}
 
{<section id="apjt301.global" >}
#應用 i07 樣板自動產生(Version:65)
...
#170814-00012#25  2017/09/08 By 10044       開窗q_ooeg001_4與q_ooeg001 內容相同,整批修改
#170814-00012#10  2017/10/03 By 10044       因開窗增加法人限制條件.所以將有使用v_ooeg001_4的欄位開窗修改為q_ooeg001_8,且增加其參數值
#end add-point
            ...
            IF cl_auth_chk_act("exporttoexcel") THEN
               ...
               CASE
                  WHEN lb_get_type = "poi"   #使用poi樣板
                     ...
                     #非browser
                     ELSE
                        LET l_arr_len = g_pjbz_d.getLength()
                        LET g_export_tag[1] = rec_arr_type.create(l_arr_len)
                        LET g_export_tabname[1] = "s_detail1"
                        FOR l_arr_cnt = 1 TO l_arr_len
                           LET g_export_tag[1][l_arr_cnt] = g_pjbz_d[l_arr_cnt]
                        END FOR
 
                        LET l_arr_len = g_pjbz2_d.getLength()
                        LET g_export_tag[2] = rec_arr_type.create(l_arr_len)
                        LET g_export_tabname[2] = "s_detail2"
                        FOR l_arr_cnt = 1 TO l_arr_len
                           LET g_export_tag[2][l_arr_cnt] = g_pjbz2_d[l_arr_cnt]
                        END FOR
         ...
         ON ACTION queryplansel
            ...
            IF NOT cl_null(ls_wc) THEN
               ...
               FOR li_idx = 1 TO la_wc.getLength()
                  CASE
                     WHEN la_wc[li_idx].tableid = "pjbz_t"
...
#         #應用 a03 樣板自動產生(Version:3)
         ...
         ON ACTION controlp INFIELD pjbz012
            ...
            LET g_qryparam.state = 'c'
            LET g_qryparam.reqry = FALSE
            #CALL q_ooeg001_4()                           #呼叫開窗        #170814-00012#25  170908 By 10044 mark
            CALL q_ooeg001()                                              #170814-00012#25  170908 By 10044 add
         ...
         ON ACTION controlp INFIELD pjbz016
            ...
            LET g_qryparam.reqry = FALSE
            #CALL q_ooeg001_4()                           #呼叫開窗        #170814-00012#25  170908 By 10044 mark
            #CALL q_ooeg001()                                              #170814-00012#25  170908 By 10044 add    #170814-00012#10  By 10044 mark
            CALL q_ooeg001_8()                                             #170814-00012#10  By 10044 add
...
#此段落由子樣板a19產生
               ...
               LET g_chkparam.arg2 = g_today
               LET g_chkparam.arg3 = g_pjbz_m.pjbzcomp           #170814-00012#10  By 10044 add
               #160318-00025#8--add--str
               LET g_errshow = TRUE 
               LET g_chkparam.err_str[1] = "aoo-00029:sub-01302|aooi125|",cl_get_progname("aooi125",g_lang,"2"),"|:EXEPROGaooi125"
               #160318-00025#8--add--end   
...
#         #應用 a03 樣板自動產生(Version:3)
         ...
         ON ACTION controlp INFIELD pjbz016
            ...
            #LET g_qryparam.where = " ooeg003 = '3'"        #170814-00012#10  By 10044 mark
            #給予arg
            LET g_qryparam.arg1 = g_today
            LET g_qryparam.arg2 = g_pjbz_m.pjbzcomp                     #170814-00012#10  By 10044 add
            #CALL q_ooeg001_4()                                #呼叫開窗        #170814-00012#25  170908 By 10044 mark
            #CALL q_ooeg001()                                                  #170814-00012#25  170908 By 10044 add    #170814-00012#10  By 10044 mark
            CALL q_ooeg001_8()                                                 #170814-00012#10  By 10044 add
...
LET g_pjbz2_d[l_ac].pjbzmodid_desc = cl_get_username(g_pjbz2_d[l_ac].pjbzmodid)
               ...
               CALL apjt301_update_b('pjbz_t',gs_keys,gs_keys_bak,"'1'")
                     #資料多語言用-增/改
                        ...
                        #161130-00056
                     #修改歷程記錄(單身修改)
                     CALL apjt301_set_all_pk_array()   #(ver:50)
                     ...
                     IF NOT cl_log_modified_record(g_log1,g_log2) THEN 
                        CALL s_transaction_end('N','0')
                     END IF
                     CALL apjt301_set_pk_array()   #(ver:50)
      ...
      BEFORE DIALOG
        ...
        ELSE
         ...
         #新增時強制從單頭開始填
         IF p_cmd = 'a' THEN
            NEXT FIELD pjbzcomp #170906-00011#1
         ELSE
            CASE g_aw
               WHEN "s_detail1"
                  NEXT FIELD pjbzseq #170906-00011#1
...
PRIVATE FUNCTION apjt301_delete()
   ...
   IF cl_ask_del_master() THEN              #確認一下
      ...
      #(ver:50) 將mark拿掉 ---start---
      #頭先不紀錄
      LET g_log1 = util.JSON.stringify(g_pjbz_m_t)   #(ver:50)
      #執行刪除時，不會有新值，所以新值欄位傳送空白             #(ver:50)
   #  LET g_log2 = util.JSON.stringify(g_pjbz_m)     #(ver:50)
...
#  IF g_action_choice = "modify" OR g_action_choice = "modify_detail" THEN   #(ver:2) mark
   ...
   THEN
      ...
      #若有設定部門權限
      CALL cl_get_owndept_list("pjbz_t","modify") RETURNING ls_owndept_list
      IF NOT cl_null(ls_owndept_list) THEN
         LET g_sql = g_sql, " AND pjbzowndp IN (",ls_owndept_list,")"
      END IF
 
      #若有設定個人權限
      CALL cl_get_ownuser_list("pjbz_t","modify") RETURNING ls_ownuser_list
      IF NOT cl_null(ls_ownuser_list) THEN
         LET g_sql = g_sql," AND pjbzownid IN (",ls_ownuser_list,")"
      END IF
   END IF
...
{<section id="apjt301.show_ownid_msg" >}
##+ 判斷是否顯示只能修改自己權限資料的訊息
PRIVATE FUNCTION apjt301_show_ownid_msg()
...
{<section id="apjt301.set_all_pk_array" >}
#+ 因為是假雙檔，所以需將單頭與第一單身的key組合起來，才是該筆資料完整的key欄位
PRIVATE FUNCTION apjt301_set_all_pk_array()
   ...
   LET g_pk_array[1].values = g_pjbz_m.pjbzld
   LET g_pk_array[1].column = 'pjbzld'
   LET g_pk_array[02].values = g_pjbz_m.pjbz001
   LET g_pk_array[02].column = 'pjbz001'
   LET g_pk_array[03].values = g_pjbz_m.pjbz002
   LET g_pk_array[03].column = 'pjbz002'
   LET g_pk_array[04].values = g_pjbz_m.pjbz003
   LET g_pk_array[04].column = 'pjbz003'
   LET g_pk_array[05].values = g_pjbz_m.pjbz004
   LET g_pk_array[05].column = 'pjbz004'
 
 
   #設定單身pk
   LET ls_length = g_pk_array.getLength()+1
   LET g_pk_array[ls_length].values = g_pjbz_d[g_detail_idx].pjbzseq
   LET g_pk_array[ls_length].column = 'pjbzseq'
