...
# Modfiy.........: No:110527     11/05/27 By yangjian 报表添加显示主要业务员
# Modfiy.........: NO.151223     15/12/23 by lifang 增加制单人员nmeuser/gen02栏位
...
MAIN
   ...
   LET g_sql = " nme00.nme_file.nme00,",
               ...
               " occ18.occ_file.occ18,",   #No:110527 Add <<--yangjian--
               " gen02.type_file.chr100"   #No:110527 Add <<--yangjian--
              ,",gen02_1.gen_file.gen02,",   #NO.151223
               " nmeuser.nme_file.nmeuser "  #NO.151223
               ,",nmg13.nmg_file.nmg13"  # add by lixwz 180604
   ...
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,
               " VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ",
               "        ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ",
              #"        ?,?,?,?,?, ?)"           #FUN-8C0018 #FUN-990067 add ?  #No:110527
               "        ?,?,?,?,?, ?,?,?,?,? ,?)"           #FUN-8C0018 #FUN-990067 add ?   #NO.151223 add 2*? # add ？ by lixwz180604
...
#  INPUT BY NAME tm.b,tm.p1,tm.p2,tm.p3,tm.p4,tm.p5,tm.p6,tm.p7,tm.p8,           #FUN-8C0018 #No.FUN-A10098
   INPUT BY NAME                                                                 #No.FUN-A10098
                 ...
                 tm.bdate,tm.edate,tm.d,tm.date_sw,tm.more                       #FUN-8C0018
                 WITHOUT DEFAULTS
         #No.FUN-580031 --start--
         BEFORE INPUT
...
#         l_time    LIKE type_file.chr8           #No.FUN-6A0082
          ...
          l_nme25   LIKE nme_file.nme25,          #No:110527  Add <--yangjian--
          l_occ04   LIKE occ_file.occ04,
          l_occ18   LIKE occ_file.occ18,
          l_gen02   LIKE type_file.chr100,
...
##No.FUN-8C0018---End
#No.FUN-A10098 -END-------
 DEFINE     l_nmeuser  LIKE nme_file.nmeuser   #NO.151223
 DEFINE     l_gen02_1  LIKE gen_file.gen02     #NO.151223
 DEFINE     l_nmg13    LIKE nmg_file.nmg13     # add by lixwz180604
...
#No.FUN-A10098 -END-------
  ...
  LET l_sql = "SELECT ",
                 ...
                 " nme11, nme12, nme13, nme16, nme17,",
                #" nmc02, nmc03, nma03, nma10,'',''",       #No:110527  Modify <<-yangjian-
                 " nmc02, nmc03, nma03, nma10,'','','',nme25",
                 " ,nmeuser ",                              #NO.151223 add
                 " ,nmg13",                            # add by lixwz180604
                ...
                #"           nme11,nme12,nme13,nme16,nme17,nmc02,nmc03,nme01 ",
                 "           nme11,nme12,nme13,nme16,nme17,nmc02,nmc03,nme01,nme25,nmeuser ",   #No:110527  #NO.151223 add nmeuser
     ...
     LET l_sql = l_sql CLIPPED,"  ) tmp ",
                 " ON nma01 = tmp.nme01 ",
                 " LEFT JOIN nmg_file ON nmg00=tmp.nme17 ", # add by lixwz180604" LEFT JOIN nmg_file ON nmg00=nme17 ", # add by lixwz180604
                 " WHERE ", tm.wc CLIPPED
    ...
    #FOREACH r320_curs1 INTO sr.*
     FOREACH r320_curs1 INTO sr.*,l_nme25,l_nmeuser   #No:110527  Modify <<-yangjian-  #NO.151223 add l_nmeuser
                             ,l_nmg13                 # add by lixwz180604
       IF STATUS != 0 THEN
          CALL cl_err('foreach:',STATUS,1)
          EXIT FOREACH
       END IF
      #No:110527  Add <<--yangjian--
       #add by chenyu --110608--begin
       LET l_occ18 = NULL
       LET l_occ04 = NULL
       LET l_gen02 = NULL
       #add by chenyu --110608--end
       LET l_sql = " SELECT occ18,occ04,gen02 ",
                   "   FROM occ_file,gen_file ",
                   "  WHERE occ01 = '",l_nme25,"'",
                   "    AND occ04 = gen01(+) "
       PREPARE gen_prep FROM l_sql
       EXECUTE gen_prep INTO l_occ18,l_occ04,l_gen02
       IF cl_null(l_occ04) THEN LET l_occ04 = ' '  END IF
       IF cl_null(l_gen02) THEN LET l_gen02 = ' '  END IF
       LET l_gen02 = l_occ04||'|'||l_gen02
      #No:110527  End <<--yangjian--
      #NO.151223 add begin#
       SELECT gen02 INTO l_gen02_1
       FROM gen_file
       WHERE gen01 = l_nmeuser
      #NO.151223 add end#
...
#             LET l_sql = "SELECT nmo02 INTO FROM ",l_dbs CLIPPED," nmo_file WHERE nmo01='",sr.nmh10,"' "    #TQC-950127
     ...
     # EXECUTE insert_prep USING sr.*,l_restf,l_rest,t_azi04,t_azi05,m_dbs[l_i] #FUN-8C0018 #No.FUN-A10098
      #EXECUTE insert_prep USING sr.*,l_restf,l_rest,t_azi04,t_azi05,''  #No:110527  Modify<<-yangjian-
       EXECUTE insert_prep USING sr.*,l_restf,l_rest,t_azi04,t_azi05,'',l_occ18,l_gen02,l_gen02_1,l_nmeuser   #NO.151223 add l_gen02_1,l_nmeuser
                                 ,l_nmg13             # add by lixwz180604
       #No.FUN-780011  --End
     END FOREACH
