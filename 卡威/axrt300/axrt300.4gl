...
# Modify.........: 141011        14/10/11 By lifang 增加批号显示
# Modify.........: No.MOD-C60247 12/07/02 By Polly 銷退改用計價數量控卡  # add by lixwz 180404

DATABASE ds

#GLOBALS "../../config/top.global"            #mark by lifang  141011
GLOBALS "../../../tiptop/config/top.global"   #add by lifang 141011
...
#No.FUN-AB0034 --end
                               ...
                               pno       LIKE ogb_file.ogb092, #add by lifang 141011
...
MAIN
  ...
  # IF (NOT cl_setup("AXR")) THEN
   #mark by lifang 141011
   IF (NOT cl_setup("CXR")) THEN    #add by lifang 141011
      EXIT PROGRAM
   END IF
   ...
   IF g_bgjob='N' OR cl_null(g_bgjob) THEN
    ...
    #    WITH FORM "axr/42f/axrt300"  ATTRIBUTE (STYLE = g_win_style CLIPPED) #No.FUN-580092 HCN   #mark by lifang 141011
         WITH FORM "cxr/42f/axrt300"  ATTRIBUTE (STYLE = g_win_style CLIPPED)  #add by lifang 141011
...
FUNCTION t300_g_guino()
   ...
   END IF
   CALL t300_ins_omc('1')   #No.FUN-680022   ##TQC-750177  #MOD-9C0409
   SELECT * INTO g_oma.* FROM oma_file WHERE oma01=g_oma.oma01
   CALL t300_show()
...
FUNCTION t300_g_b4()                       #由銷退單產生單身
   ...
   FOREACH t300_g_b_c4 INTO g_ohb.*
     ...
     END IF
     INITIALIZE b_omb.* TO NULL
     LET b_omb.omb38 = '3'                       #為銷退單 #No.FUN-670026
   ...
   FOREACH t300_g_b_c6 INTO g_ohb.*
     ...
     END IF
     INITIALIZE b_omb.* TO NULL
     LET b_omb.omb38 = '5'                       #銷退返利 #No.FUN-670026
...
#No.FUN-AB0034 --end
     ...
     ELSE
        IF b_omb.omb12 = 0  AND l_oha09 ='5' THEN       #MOD-940155
           IF g_oma.oma00='21' THEN   #MOD-940155 add
...
## No.2681 modify 1998/10/29 無效客戶不可立帳
       ...
       AFTER FIELD oma24
          IF NOT cl_null(g_oma.oma24) THEN
             IF p_cmd='u' AND g_oma.oma24 != g_oma_o.oma24 THEN   #MOD-6B0121
...
FUNCTION t300_show()
    ...
    DEFINE l_occ18_1 LIKE occ_file.occ18  #add by lifang 141009
    DEFINE l_occ18_2 LIKE occ_file.occ18  #add by lifang 141009
  ...
  CALL cl_set_field_pic(g_oma.omaconf,g_chr2,"","",g_oma.omavoid,"")
    ...
    #add by lifang 141009 begin#
    IF g_plant='KW27' OR g_plant='KW28' OR g_plant='KW29' OR g_plant='KW20' OR
       g_plant='KW06' OR g_plant='KW07' OR g_plant='KW08' THEN
    SELECT occ18 INTO l_occ18_1 FROM occ_file WHERE occ01 = g_oma.oma03
    SELECT occ18 INTO l_occ18_2 FROM occ_file WHERE occ01 = g_oma.oma68
    DISPLAY l_occ18_1 TO oma032
    DISPLAY l_occ18_2 TO oma69
    END IF
    #add by lifang 141009 end #
...
FUNCTION t300_show0()
   ...
   CASE g_oma.oma00
      ...
      WHEN '12'
         ...
         CALL cl_set_comp_att_text("oma16,omb31",ls_msg CLIPPED || "," || ls_msg CLIPPED)
         CALL cl_set_comp_visible("oma74",TRUE)   #No.FUN-AB0034
      WHEN '13'
...
##-----------客戶,稅別,幣別,匯率要一致
        ...
        AFTER FIELD omb32
                ...
                #-MOD-A80073-end-
                 IF p_cmd='a' OR g_omb[l_ac].omb31!=g_omb_t.omb31
                   ...
                   #   IF l_omb12 = g_ohb.ohb12 THEN                                                      #MOD-A80073 mark
                    #IF (l_oha09 = '5' AND l_cnt > 0) OR (l_oha09!='5' AND l_omb12 = g_ohb.ohb12) THEN     #MOD-A80073 #MOD-C60247 mark
                    IF (l_oha09 = '5' AND l_cnt > 0) THEN                                                 #MOD-C60247 add
                          CALL cl_err("","axr-390",1)
                          NEXT FIELD omb32
                      #END IF                                                                             #MOD-A80073 mark
                    END IF                 #MOD-940155 add
                   #------------------------------MOD-C60247------------------------------(S)
                    IF (l_oha09 != '5' AND l_omb12 = g_ohb.ohb917) THEN
                       CALL cl_err("","axr-620",1)
                       NEXT FIELD omb32
                    END IF
                   #------------------------------MOD-C60247------------------------------(E)
                 END IF
	     ...
	     #-MOD-A80073-end-
              ...
              END IF
           END IF
         #add by lifang 141011 begin#
           IF NOT cl_null(g_omb[l_ac].omb31) THEN
              IF NOT cl_null(g_omb[l_ac].omb32) THEN
                 IF g_oma.oma00='12' THEN
                    SELECT ogb092 INTO g_omb[l_ac].pno
                     FROM ogb_file
                     WHERE ogb01 = g_omb[l_ac].omb31
                      AND ogb03 = g_omb[l_ac].omb32
                 ELSE
                   IF g_oma.oma00='21' THEN
                       SELECT ohb092 INTO g_omb[l_ac].pno
                       FROM ogb_file
                       WHERE ohb01 = g_omb[l_ac].omb31
                       AND ohb03 = g_omb[l_ac].omb32
                   END IF
       ...
       AFTER FIELD omb42
        ...
        AFTER FIELD omb33
           IF NOT cl_null(g_omb[l_ac].omb33) THEN
              ...
              IF STATUS THEN
                 CALL cl_err3("sel","aag_file",g_omb[l_ac].omb33,"",STATUS,"","sel aag",1)  #No.FUN-660116
                 NEXT FIELD omb33
              END IF
        ...
        AFTER FIELD omb331
           IF NOT cl_null(g_omb[l_ac].omb331) THEN
              ...
              IF STATUS THEN
                 CALL cl_err3("sel","aag_file",g_omb[l_ac].omb331,"",STATUS,"","sel aag",1)  #No.FUN-660116
                 NEXT FIELD omb331
              END IF
        ...
        ON ACTION CONTROLP   #ok
            CASE
              ...
              WHEN INFIELD(omb44)     #门店编号
               ...
               WHEN INFIELD(omb33)
                 CALL cl_init_qry_var()
                 CALL q_m_aag(FALSE,TRUE,g_omb[l_ac].omb44,g_omb[l_ac].omb31,'23',g_bookno1)
...
#              #                    AND omavoid = 'N'
#              #                    AND ogb31 = g_ogb.ogb31)
#               LET g_sql = " SELECT SUM(oma52),SUM(oma54),SUM(oma54t) ",
#                           " FROM oma_file ",
...
#               EXECUTE sel_ogb_pre04 INTO l_12_oma52,l_12_oma54,l_12_oma54t
#              #-MOD-B10012-end-
#               IF l_12_oma52  IS NULL THEN LET l_12_oma52 = 0 END IF    #No:FUN-A50103
...
#               #   AND ogb31 = g_ogb.ogb31)
#                LET g_sql = " SELECT SUM(oma53),SUM(oma56),SUM(oma56t) ",
#                            " FROM oma_file ",
#                            " WHERE oma01 IN (SELECT oma01 FROM oma_file,",cl_get_target_table(g_plant_new,'ogb_file'),
#                            " WHERE oma16 = ogb01 ",
#                            "   AND oma16 <> '",g_ogb.ogb01,"'",
...
#                EXECUTE sel_ogb_pre05 INTO l_12_oma53,l_12_oma56,l_12_oma56t
#               #-MOD-B10012-end-
#                IF l_12_oma53  IS NULL THEN LET l_12_oma53 = 0 END IF   #TQC-A60027
...
"       omb13,omb14,omb14t,omb15,omb16,omb16t,omb17,omb18,omb18t,omb930,'','', ", #FUN-680001  #add '',*1 by lifang 141011
"       ombud01,ombud02,ombud03,ombud04,ombud05,",
"       ombud06,ombud07,ombud08,ombud09,ombud10,",
"       ombud11,ombud12,ombud13,ombud14,ombud15 ",
" FROM omb_file ",
...
LET g_omb[g_cnt].gem02c=s_costcenter_desc(g_omb[g_cnt].omb930) #FUN-680006
    #add by lifang 141011 begin#
      IF NOT cl_null(g_omb[g_cnt].omb31) THEN
         IF NOT cl_null(g_omb[g_cnt].omb32) THEN
            IF g_oma.oma00='12' THEN
               SELECT ogb092 INTO g_omb[g_cnt].pno
               FROM ogb_file
               WHERE ogb01 = g_omb[g_cnt].omb31
                AND ogb03 = g_omb[g_cnt].omb32
            ELSE
               IF g_oma.oma00='21' THEN
                   SELECT ohb092 INTO g_omb[g_cnt].pno
                   FROM ogb_file
                   WHERE ohb01 = g_omb[g_cnt].omb31
                   AND ohb03 = g_omb[g_cnt].omb32
               END IF
            END IF
         END IF
     END IF
      ...
      IF NOT cl_null(g_oma.oma71) THEN
         ...
         SELECT COUNT(*) INTO l_cnt FROM ama_file
          WHERE ama02 = g_oma.oma71
           ...
           END IF
           IF g_aza.aza21 = 'Y' OR NOT s_chkban(g_oma.oma71) THEN #FUN-990053 mod
...
FUNCTION t300_2()
   ...
   DEFINE  l_oab02      LIKE oab_file.oab02      #No.TQC-7C0070
    ...
    IF g_oma.oma01 IS NULL THEN RETURN END IF
    IF g_oma.omavoid = 'Y' THEN RETURN END IF
    IF g_oma.oma70 = '1' THEN RETURN END IF   #FUN-960140
    IF g_oma.oma64 matches '[Ss]' THEN
        RETURN
    END IF
...
FUNCTION t300_y_chk()              # when g_oma.omaconf='N' (Turn to 'Y')
   ...
   DEFINE l_count_1 LIKE type_file.num5   #add by lifang 150210
   ...
   IF g_oma.omavoid = 'Y' THEN
     ...
     #RETURN #CHI-A80031 mark
   END IF
   #--add by lifang 150210 begin#
   SELECT COUNT(*) INTO l_count_1
   FROM omb_file
   WHERE omb01 = g_oma.oma01
   AND omb04 IS NULL
   AND omb06 IS NOT NULL
   IF l_count_1 > 0 THEN
      CALL cl_err('','cxr-004',1)
      LET g_success = 'N'
   END IF
   #--add by lifang 150210 end#
...
FUNCTION t300_y_upd()        # when g_oma.omaconf='N' (Turn to 'Y')
   ...
   DECLARE t300_firm1_c3 CURSOR FOR t300_firm1_p3
  ...
  #CALL s_showmsg_init()    #NO.FUN-710050 #CHI-A80031 mark
   ...
   FOREACH t300_firm1_c3 INTO m_oma.*  #-->逐筆確認   #FUN-530061
      ...
      SELECT COUNT(*),SUM(oot05t) INTO l_cnt,l_oot05t
        FROM oot_file
      ...
      IF g_ooy.ooydmy1 = 'Y' AND g_flag2 ='Y' THEN  #MOD-940069 #CHI-AC0046
         ...
         IF m_oma.oma00 MATCHES '2*' THEN   #FUN-530061 mod g_oma->m_oma
            ...
            IF g_aza.aza63 = 'Y' THEN
               ...
               SELECT SUM(ABS(npq07)) INTO l_npq07_1 FROM npq_file
                WHERE npq01 = m_oma.oma01
                  AND npqsys= 'AR' AND npq011 = 1
                  AND npqtype= '1'
                  AND npq03 != l_npq031
...
#              IF l_npq07_1 !=(m_oma.oma56t+m_oma.oma53) THEN   #FUN-530061  #MOD-860185 #FUN-9A0036 mark
               #--FUN-9A0036 start--
               CALL cl_digcut(l_npq07_1,g_azi04) RETURNING l_npq07_1
               LET l_npq07_amt = (m_oma.oma56t+m_oma.oma53)*l_rate
               CALL cl_digcut(l_npq07_amt,g_azi04) RETURNING l_npq07_amt
         ...
         ELSE
            IF m_oma.oma65 != '2' THEN   #FUN-570099   #FUN-530061 mod g_oma->m_oma
              ...
              #-MOD-AC0033-end-
               IF l_cnt > 0 THEN
                  #No.MOD-A90001  --Begin
                  IF l_npq07 != (m_oma.oma56t+m_oma.oma53-l_oot05t) THEN   #FUN-9A0036 mark  #No.MOD-A90001 unmark
#                 IF l_npq07_1 !=l_npq07_amt THEN                          #FUN-9A0036       #No.MOD-A90001 mark
               ...
               ELSE
                  #No.MOD-A90001  --Begin
                  IF l_npq07 !=(m_oma.oma56t+m_oma.oma53) THEN             #FUN-9A0036 mark  #No.MOD-A90002 unmark
#                 IF l_npq07_1 !=l_npq07_amt THEN                          #FUN-9A0036       #No.MOD-A90002 mark
               ...
               IF g_aza.aza63 = 'Y' THEN
                  ...
                  IF l_cnt > 0 THEN
                     #--No.TQC-B10142 start--
                     LET l_npq07_amt = (m_oma.oma56t+m_oma.oma53-l_oot05t)*l_rate
                     CALL cl_digcut(l_npq07_amt,g_azi04) RETURNING l_npq07_amt
...
### 需考慮一張發票可能同時有銷退和折扣
   DECLARE t300_by_omb CURSOR FOR
    SELECT omb31,omb44,SUM(omb14t),SUM(omb16t),SUM(omb18t) FROM omb_file #No.FUN-9C0014 Add omb44
     WHERE omb01 = g_oma.oma01
...
FUNCTION t300_s2()
   ...
   IF l_cnt > 0 THEN
      IF g_oma.oma00='12' THEN
         ...
         FOREACH t300_oga10_c3 INTO m_omb31
            IF STATUS THEN EXIT FOREACH END IF     #MOD-AC0233
            LET m_oma01 = ''  LET m_oma05 = ''
            SELECT MAX(oma01) INTO m_oma01 FROM oma_file,omb_file
             WHERE oma01 = omb01
...
FUNCTION t300_carry_voucher()
  ...
  # LET g_str="axrp590 '",g_wc_gl CLIPPED,"' '",l_buser,"' '",l_euser,"' '",g_ooz.ooz02p,"' '",g_ooz.ooz02b,"' '",l_ooygslp,"' '",g_oma.oma02,"' 'Y' '1' 'Y' '",g_ooz.ooz02c,"' '",g_ooy.ooygslp1,"'"   #No.MOD-840608 add  #No.MOD-860075  #MOD-910250  #No.MOD-920343
   LET g_str="cxrp590 '",g_wc_gl CLIPPED,"' '",l_buser,"' '",l_euser,"' '",g_ooz.ooz02p,"' '",g_ooz.ooz02b,"' '",l_ooygslp,"' '",g_oma.oma02,"' 'Y' '1' 'Y' '",g_ooz.ooz02c,"' '",g_ooy.ooygslp1,"'"   #No.MOD-840608 add  #No.MOD-860075  #MOD-910250  #No.MOD-920343 #mod by tangyi 111212
   CALL cl_cmdrun_wait(g_str)
   SELECT oma33 INTO g_oma.oma33 FROM oma_file
    WHERE oma01 = g_oma.oma01
   DISPLAY BY NAME g_oma.oma33
      ...
      CASE
         ...
         WHEN l_omb38='3' OR l_omb38='5'
            ...
            ELSE
              ...
              #-MOD-A80073-add-
               IF l_omb38 = '3' THEN
                  #LET l_sql = "SELECT ohb12  FROM ",cl_get_target_table(l_omb44,'ohb_file'),              #MOD-C60247 mark
                  LET l_sql = "SELECT ohb917  FROM ",cl_get_target_table(l_omb44,'ohb_file'),             #MOD-C60247 add
                             #" WHERE ohb01='",g_omb[l_ac].omb31,"' AND ohb03='",g_omb[l_ac].omb32,"' ",
                              " WHERE ohb01='",l_omb31,"' AND ohb03='",l_omb32,"' ",    #add by chenyu --110610
                              "   AND ohb1005 !='2' "
                 ...
                 #IF (l_oha09 = '5' AND l_cnt > 0) OR (l_oha09!='5' AND l_omb12 = l_ohb12) THEN     #MOD-A80163 mark
                  #IF (l_oha09 = '5' AND l_cnt > 0) OR (l_oha09!='5' AND l_omb12 > l_ohb12) THEN   #MOD-A80163 #MOD-C60247 mark
                  IF (l_oha09 = '5' AND l_cnt > 0) THEN                                           #MOD-C60247 add
                  ...
                  END IF
                 #------------------------------MOD-C60247------------------------------(S)
                  IF (l_oha09 != '5' AND l_omb12 > l_ohb12) THEN
                     LET l_msg = cl_getmsg('axr-620',g_lang),':',l_omb03 USING '#####&'
                     LET l_err = l_msg
                  END IF
                 #------------------------------MOD-C60247------------------------------(E)
                 #MOD-C40119---str---
