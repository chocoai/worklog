...
# Modify.........: No:110614     11/06/14 By yangjian 打印添加显示工单类型
# Modify.........: NO.111213     11/12/13 by xiayan 增加完工入库部门
# Modify.........: No:TQC-D10073 13/05/10 By lifang 將tlfctype的條件搬到left outer join的條件裡
# Modify.........: No:TQC-D20051 13/05/10 By lifang sql有写错，调整
# Modify.........: No:FUN-B90029 13/12/07 By lifang 單價要加製費
# Modify.........: No:FUN-C80092 13/12/07 By lifang 修改只列印當站下線資料包含製差問題

DATABASE ds

#GLOBALS "../../config/top.global"   #No:110614  Modify <<-yangjian--
...
MAIN
   ...
   LET g_sql = "tlf031.tlf_file.tlf031,tlf06.tlf_file.tlf06,",
               "tlf904.tlf_file.tlf904,",                         #add by wulisong 150521
              ...
              #"tlf930.tlf_file.tlf930"     #No:110614  Modify <<--yangjian--
               "tlf930.tlf_file.tlf930,sfb02.type_file.chr100,",
               "sfu04.sfu_file.sfu04,gem02.gem_file.gem02"  #NO.111213 add
               # add by lixwz180604 s---
               ,",sfvud03.sfv_file.sfvud03,",
               "sfvud04.sfv_file.sfvud04,",
               "sfvud13.sfv_file.sfvud03,",
               "sfvud14.sfv_file.sfvud03"
               # add by lixwz180604 e---
   LET l_table = cl_prt_temptable('axcr772',g_sql) CLIPPED
   IF l_table = -1 THEN EXIT PROGRAM END IF
   LET g_sql = "INSERT INTO ",g_cr_db_str CLIPPED,l_table CLIPPED,
               " VALUES(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?,?, ?,?,?,?,?,",
              #"        ?,?,?,?,?,?)"            #No.FUN-7C0101 add               #FUN-9A0050 Add ?,?,?   #No:110614  Add ?  <<-yangjian--
               "        ?,?,?,?,?,?,?,?,?,?,?,?,?)"            #No.FUN-7C0101 add #FUN-9A0050 Add ?,?,?  #NO.111213 add 2?
...
FUNCTION axcr772()
   DEFINE l_name    LIKE type_file.chr20,         #No.FUN-680122 VARCHAR(20)       # External(Disk) file name
          ...
          sr        RECORD                                #MOD-840342 mod
                     ...
                     tlf904   LIKE tlf_file.tlf904,    #add by wulisong 150521
                     ...
                     sfb02    LIKE type_file.chr100    #No:110614  Add <<-yangjian-工单性质
                    END RECORD
...
DEFINE     l_sfu04    LIKE sfu_file.sfu04      #NO.111213 add
DEFINE     l_gem02    LIKE gem_file.gem02      #NO.111213 add
DEFINE     l_tlf19    LIKE tlf_file.tlf19      #NO.111213 add
DEFINE     l_where    STRING                   #FUN-B90029
# add by lixwz180604 s---
DEFINE     l_sfvud03  LIKE sfv_file.sfvud03
DEFINE     l_sfvud04  LIKE sfv_file.sfvud04
DEFINE     l_sfvud13  LIKE sfv_file.sfvud03
DEFINE     l_sfvud14  LIKE sfv_file.sfvud03
# add by lixwz180604 e---
     ...
     LET l_sql = "SELECT '',ima12,ima01,ima02,ima021,tlfccost,",      #No.FUN-7C0101 add tlfccost   #MOD-840342 mod
                 " tlf021,tlf031,tlf06,tlf904,tlf036,tlf037,",     #add tlf904 by wulisong 150521
                 " tlf01,tlf10*tlf60,tlfc21,tlf13,tlf62,tlf907,", #No:9452   #No.FUN-7C0101 tlf21->tlfc21
                 "  tlfc221,tlfc222,tlfc2231,tlfc2232,tlfc224,tlfc2241,tlfc2242,tlfc2243,0,'',tlf032,tlf930,",   #FUN-5B0082   #No.FUN-7C0101 #FUN-9A0050 Add tlf032,tlf930
                 " tlf19 ",   #NO.111213 add
                 " ,sfvud03,sfvud04,sfvud13,sfvud14", # add by lixwz180604
                 "  FROM tlf_file left join sfv_file on sfv01 = TLF026 and sfv03 = tlf027",  # add by lixwz180604 leftjoin on sfv_file
                 "  LEFT JOIN tlfc_file ON (tlf01=tlfc01 AND tlf06=tlfc06 AND tlf02=tlfc02 AND tlf03 = tlfc03 AND tlf13=tlfc13 AND tlf13=tlfc13 AND tlf902=tlfc902 AND tlf903=tlfc903 AND tlf904=tlfc904 AND tlf907=tlfc907 AND tlf905=tlfc905 AND tlf906=tlfc906   ",          #No.FUN-7C0101  #TQC-D20051 remove ima_file,sfb_file
     ...
     FOREACH axcr772_curs1 INTO sr.*,l_tlf032,l_tlf930 #FUN-9A0050 Add l_tlf032,l_tlf930
                               ,l_tlf19     #NO.111213 add
                               ,l_sfvud03,l_sfvud04,l_sfvud13,l_sfvud14  #add by lixwz180604
      ...
      #No:110614  Add <<--yangjian---
       LET sr.sfb02 = ''
       SELECT sfb02 INTO sr.sfb02 FROM sfb_file WHERE sfb01 = sr.tlf62
          CASE sr.sfb02
            WHEN  '1'  LET sr.sfb02 = '一般工单'
            WHEN  '5'  LET sr.sfb02 = '返工工单'
            WHEN  '7'  LET sr.sfb02 = '委外工单'
            WHEN  '8'  LET sr.sfb02 = '委外返工工单'
            WHEN  '11'  LET sr.sfb02 = '拆件式工单'
            WHEN  '13'  LET sr.sfb02 = '预测工单'
            WHEN  '15'  LET sr.sfb02 = '试产工单'
            OTHERWISE LET sr.sfb02 = ''
          END CASE
    #No:110614   End <<--yangjian--
       ...
       END IF
          ...
          CLOSE stock_c1
         #NO.111213 add----------------------begin
          #SELECT sfu04 INTO l_sfu04 FROM sfu_file WHERE sfu01 = sr.tlf036
          INITIALIZE l_gem02 TO NULL
          SELECT gem02 INTO l_gem02 FROM gem_file WHERE gem01 = l_tlf19
          IF cl_null(l_gem02) THEN
             SELECT pmc03 INTO l_gem02 FROM pmc_file WHERE pmc01 = l_tlf19
          END IF
         #NO.111213 add------------------------end
       EXECUTE insert_prep USING
          sr.tlf031,sr.tlf06, sr.tlf904,sr.tlf036,  sr.tlf62,sr.tlf01,  #add sr.tlf904 by wulisong 150521
        ...
        # ,l_ima39,l_ima391,l_tlf930       #No:110614  Modify <<--yangjian--
          ,l_ima39,l_ima391,l_tlf930,sr.sfb02
          ,l_tlf19,l_gem02    #NO.111213 add
          ,l_sfvud03,l_sfvud04,l_sfvud13,l_sfvud14 # add by lixwz180604
     END FOREACH
  ...
  #ccz09 = 'Y' 時,在製差異轉出歸本月入庫
    ...
    #LET l_emm = YEAR(tm.edate) * 12 + MONTH(tm.edate)   #No:MOD-920161 add
     ...
     LET l_sql = "SELECT 'Z',ima12,ccg04,ima02,ima021, ",
                ...
                #"       ccg07,' ',' ',' ',ima12,' ', ",              #mark by lifang 150604
                 "       ccg07,' ',' ',' ','',ima12,' ', ",           #add by lifang 150604
    ...
    #No:110614   End <<--yangjian--
       ...
       #NO.111213 add------------------------end
       EXECUTE insert_prep USING
          sr.tlf031,sr.tlf06,'', sr.tlf036,  sr.tlf62,sr.tlf01,#add '' by wulisong 150522
          sr.ima02, sr.ima021,sr.tlfccost,sr.tlf10,sr.amt01,   #No.FUN-7C0101 add tlfccost
          sr.amt02, sr.amt03, sr.amt04,   sr.amt05,sr.amt07,   #No.FUN-7C0101 add amt7
          sr.amt08, sr.amt09, sr.amt06,   sr.code, sr.ima12,   #No.FUN-7C0101 add amt8,amt9
          tm.bdate, tm.edate, l_azf03
        # ,'','',''       #No:110614  Modify <<-yangjian--
          ,'','','',sr.sfb02
          ,l_tlf19,l_gem02    #NO.111213 add
          ,'','','',''                                          # add by lixwz180604
     END FOREACH
  ...
  #ccz09 = 'N' 時,在製差異轉出歸差異ccg42 (成本分群預設為'ZZZ')
    ...
    IF g_zz05 = 'Y' THEN
        CALL cl_wcchp(tm.wc,'ima12,ima08,sfb01,ima57,ima01')
            RETURNING tm.wc
    END IF
    LET g_str = tm.wc,";",tm.a,";",tm.type,";",g_azi03,";",g_ccz.ccz27 #MOD-960193 add azi03 #MOD-960283 add czz27
