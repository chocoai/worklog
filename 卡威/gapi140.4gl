# Prog. Version..: '5.25.01-11.01.17(00010)'     #
#
...
# Modify.........: No:110331     11/03/31 By yangjian 退货时，金额为负数则生成正数的发票金额
# Modify.........: NO.110428     11/04/28 By xiayan  应付账款单号show出来
# Modify.........: NO.110429     11/04/29 By xiayan  录入时单价、数量、金额不可修改,并将含税单价显示出来
# Modify.........: No:110527     11/05/27 By yangjian 添加显示料号规格和计价单位
# Modify.........: NO.130826     13/08/26 By lifang  增加ACTION"賬款查詢" ,根據賬款編號rvw13抓取apa00,分別串aapt110/aapt210
# Modify.........: No:MOD-B50016 15/10/08 By lifang rvw08開窗時,也應做MOD-990163的處理
# Modify.........: No.MOD-C60217 15/10/08 By lifang VMI仓的入库单不可请款
# Modify.........: no.151008     15/10/08 By lifang 样品的含税单价显示为0
...
DEFINE
   g_head_1      RECORD
                 ...
                 rvw13      LIKE rvw_file.rvw13                     #add 凭证编号 110327
                 END RECORD,
   g_head_t      RECORD
                 rvw01      LIKE rvw_file.rvw01,         #發票號碼
                 rvw07      LIKE rvw_file.rvw07,         #發票類型
   ...
   g_type        LIKE type_file.chr1,         # Prog. Version..: '5.25.01-11.01.17(01)   #類型
   ...
   g_rvw         DYNAMIC ARRAY OF RECORD                 #程式變數(Program Variables)
                 ...
                 rvv86      LIKE rvv_file.rvv86,         #计价单位  #No:110527 Add <<-yangjian-
                 rvw10      LIKE rvw_file.rvw10,         #數量
                 rvw17      LIKE rvw_file.rvw17,         #原幣未稅單價
                 rvv38t     LIKE rvv_file.rvv38t,        #含税单价          add 110401
                 rvw05f     LIKE rvw_file.rvw05f,
                 rvw06f     LIKE rvw_file.rvw06f,
                 jine1      LIKE rvw_file.rvw05f,        #原币含税金额      add 110401
                 rvw05      LIKE rvw_file.rvw05,
                 rvw06      LIKE rvw_file.rvw06,
                 jine2      LIKE rvw_file.rvw05          #本币含税金额      add 110401
                 END RECORD,
...
MAIN
   ...
   LET g_forupd_sql = "SELECT rvw01,rvw07,rvw02,rvw03,rvw04,rvw11,rvw12 FROM rvw_file WHERE rvw01=? FOR UPDATE"   #MOD-640405
 ...
 #    WITH FORM "gap/42f/gapi140"
      WITH FORM "cgap/42f/gapi140"                       #mod 110331
   CALL cl_ui_init()
   LET g_type='1'
   CALL i140_menu()                                      #中文
...
DEFINE   l_rvw13           LIKE rvw_file.rvw13
      ...
      CASE g_action_choice
         ...
         WHEN "Carry_Account"
            IF NOT cl_null(g_head_1.rvw01) THEN
               ...
               END IF
              #CALL cl_cmdrun(g_cmd)
               CALL cl_cmdrun_wait(g_cmd)             #mod 110327
               SELECT rvw13 INTO l_rvw13 FROM rvw_file
                  WHERE rvw01 = g_head_1.rvw01
               IF NOT cl_null(l_rvw13) THEN
                  LET g_head_1.rvw13 = l_rvw13
                # DISPLAY BY NAME g_head_1.rvw13              #add 110327
                  DISPLAY g_head_1.rvw13 TO FORMONLY.rvw13
               END IF
            END IF
         ...
         WHEN "related_document"  #相關文件
          ...
          END IF
        #--NO.130826 begin--#
         WHEN "gen_data_1"    #账款查询
            IF cl_chk_act_auth() THEN
               IF NOT cl_null(g_head_1.rvw13) THEN
                  CALL i140_gen()
               END IF
            END IF
        #--NO.130826 end--#
...
DEFINE  lc_qbe_sn       LIKE    gbm_file.gbm01   #No.FUN-580031  HCN
   ...
   CONSTRUCT BY NAME g_wc ON                               # 螢幕上取單頭條件
                rvw01,rvw02,rvw07,rvw03,rvw04,rvw11,rvw12,rvw13   #FUN-9B0130              #add rvw13 110327
     BEFORE CONSTRUCT
   ...
   # 螢幕上取單身條件
   CONSTRUCT g_wc2 ON rvw99,rvw08,rvw09,rvv09,rvw10,rvw05f,rvw06f        #MOD-640405      #CHI-980038   #FUN-9B0130 add rvw99
   ...
   END CONSTRUCT
   LET g_sql = "SELECT UNIQUE rvw01,rvw07,rvw02,rvw03,rvw04,rvw11,rvw12,rvw13",                  #add rvw13  110327
                  "  FROM rvw_file ",
                  " WHERE ",g_wc CLIPPED," AND ",g_wc2 CLIPPED,
                  " ORDER BY rvw02"
   PREPARE i140_prepare FROM g_sql
   IF SQLCA.sqlcode THEN   #No.TQC-790140
      CALL cl_err('i140_prepare',STATUS,1) EXIT PROGRAM
   END IF
   ...
   IF SQLCA.sqlcode THEN   #No.TQC-790140
      CALL cl_err('i140_precount',STATUS,1) EXIT PROGRAM
   END IF
...
DEFINE
   ...
   END IF
      ...
      SELECT UNIQUE rvw01,rvw07,rvw02,rvw03,rvw04,rvw11,rvw12,'',
             '',SUM(rvw05f),SUM(rvw06f),SUM(rvw05f),SUM(rvw06f),rvw13                      #add rvw13 110327
        INTO g_head_1.*
        FROM rvw_file             #FUN-9B0130
       WHERE rvw01 = g_head_1.rvw01
       GROUP BY rvw01,rvw07,rvw02,rvw03,rvw04,rvw11,rvw12,rvw13                             #add rvw13  110327
   #--add by lifang 140114 begin--#
      IF g_plant = 'KW02' OR g_plant = 'KW27' OR g_plant = 'KW28' OR
         g_plant = 'KW29' OR g_plant = 'KW20' OR g_plant = 'KW06' OR
         g_plant = 'KW07' OR g_plant = 'KW08' THEN
         SELECT pmc081 INTO g_head_1.pmc03
         FROM pmc_file
         WHERE pmc01 = g_head_1.rvv06
     ELSE
   #--add by lifang 140114 end --#
      SELECT pmc03 INTO g_head_1.pmc03
        FROM pmc_file
       WHERE pmc01 = g_head_1.rvv06
     END IF     #add by lifang 140114
...
#NO.110428 mark begin aapp110以供应商汇总方式产生的账款发票号码为apa08='MISC',不等于g_head_1.rvw01
{此时，rvw13会被赋成空值，不会显示出来。
   SELECT apa01 INTO l_apa01 FROM apa_file WHERE
          apa08 = g_head_1.rvw01
   IF l_apa01 IS NULL THEN
      LET g_head_1.rvw13 = NULL
   END IF }                                                 #add by pane 110328
#NO.110428 mark -------------end
...
DEFINE l_count_1   LIKE type_file.num5      #add by lifang 131230
   LET g_action_choice = ""
   ...
   CALL cl_set_comp_entry("rvw10,rvw17,rvw05f,rvw06f,rvw05,rvw06",FALSE)    #add by chenyu --110429

   CALL cl_opmsg('b')
#   LET g_forupd_sql = "SELECT rvw99,rvw08,rvw09,'','',rvw02,rvw10,rvw17,rvw05f,rvw06f,rvw05,rvw06",    #FUN-9B0130 add rvw99
   #LET g_forupd_sql = "SELECT rvw99,rvw08,rvw09,'','',rvw02,rvw10,rvw17,'',rvw05f,rvw06f,'',rvw05,rvw06,'' ",       #mod 110401 增加含税单价、含税金额
    LET g_forupd_sql ="SELECT rvw99,rvw08,rvw09,'','','','','',rvw10,rvw17,'',rvw05f,rvw06f,'',rvw05,rvw06,'' ",     #add by chenyu --110627
                      "  FROM rvw_file ",
                      " WHERE rvw01 = ? ",
                      "   AND rvw08 = ? ",
                      "   AND rvw09 = ? ",
   ...
   INPUT ARRAY g_rvw WITHOUT DEFAULTS FROM s_rvw.*
      ...
      BEFORE ROW
         ...
         #       LET g_sql = "SELECT rvv09,rvv31,rvv031 FROM ",cl_get_target_table(g_rvw[l_ac].rvw99,'rvv_file'), #FUN-A50102
                 LET g_sql = "SELECT rvv09,rvv31,rvv031,rvv38t FROM ",cl_get_target_table(g_rvw[l_ac].rvw99,'rvv_file'),                   #mod 110401
         ...
         #       EXECUTE sel_rvv09_pre INTO g_rvw[l_ac].rvv09,g_rvw[l_ac].ima01,g_rvw[l_ac].ima02
                 EXECUTE sel_rvv09_pre INTO g_rvw[l_ac].rvv09,g_rvw[l_ac].ima01,g_rvw[l_ac].ima02,g_rvw[l_ac].rvv38t     #mod 110401
               END IF
            IF NOT cl_null(g_rvw_t.rvw08) AND NOT cl_null(g_rvw_t.rvw09) THEN
              ...
              #str MOD-A20066 add
               ...
               ELSE
                  ...
                  CALL i140_set_no_entry_b('u')              #add 110331
                  MESSAGE " "
               END IF
            END IF
         END IF
      ...
      AFTER INSERT
         IF INT_FLAG THEN
            ...
            CANCEL INSERT
         END IF
         INSERT INTO rvw_file(rvw01,rvw02,rvw03,rvw04,rvw07,rvw11,rvw12,rvw13,   #MOD-640405        dd rvw13 110327
                       ...
                       VALUES(g_head_1.rvw01,g_head_1.rvw02,g_head_1.rvw03,
                              ...
                              g_head_1.rvw12,g_head_1.rvw13,   #MOD-640405                    #add g_head_1.rvw13 110327
      ...
      AFTER FIELD rvw05f
         IF NOT cl_null(g_rvw[l_ac].rvw05f) THEN
	       ...
	       CALL cl_parse_qry_sql(g_sql,g_plant_new) RETURNING g_sql       #FUN-A50102
            ...
            #add 110401 begin增加含税单价、金额#
            LET g_rvw[l_ac].jine1 = g_rvw[l_ac].rvw05f + g_rvw[l_ac].rvw06f
            LET g_rvw[l_ac].jine2 = g_rvw[l_ac].rvw05  + g_rvw[l_ac].rvw06
            #add 110401 end #
            ...
            DISPLAY g_rvw[l_ac].jine1 TO FORMONLY.jine1          #add 110401
            DISPLAY g_rvw[l_ac].jine2 TO FORMONLY.jine2          #add 110401
      ...
      AFTER FIELD rvw17
         IF NOT cl_null(g_rvw[l_ac].rvw17) THEN
           ...
           #LET g_rvw[l_ac].rvw06f = cl_digcut(g_rvw[l_ac].rvw06f,t_azi04)  #No.MOD-740008       #MOD-A80052 mark
            ...
            #含税单价=未税单价*(1+税率/100)
            LET g_rvw[l_ac].rvv38t = g_rvw[l_ac].rvw17 * (1+g_head_1.rvw04/100) #NO.110429 add
            DISPLAY BY NAME g_rvw[l_ac].rvv38t   #NO.110429 add
      ...
      AFTER ROW
         LET l_ac = ARR_CURR()
         LET l_ac_t = l_ac
         #--add by lifang 131230 begin--#
         #SELECT COUNT(*) INTO l_count_1 FROM rvw_file
         #WHERE rvw01 =g_head_1.rvw01
         #AND rvw10 = 0
         #IF l_count_1 >0 THEN
         IF g_rvw[l_ac].rvw10 = 0 THEN
                 ...
                 WHILE tok.hasMoreTokens()
                   ...
                   #若為樣品時,金額部份(rvw17,rvw05,rvw05f,rvw06,rvw06f)應帶0
                    ...
                    IF l_rvv25="Y" THEN
                       ...
                       LET g_rvw[l_ac].rvw06 =0
                       LET g_rvw[l_ac].rvw06f=0
                       LET g_rvw[l_ac].rvv38t=0        #add no.151008
                    END IF
                   #end MOD-B50016 add
                    INSERT INTO rvw_file(rvw01,rvw02,rvw03,rvw04,rvw07,rvw11,rvw12,rvw13,           #add rvw13 110327
                                         rvw05f,rvw06f,rvw05,rvw06,rvw08,rvw09,rvw10,rvw17,
                                         rvw99,rvwlegal)       #FUN-9B0130
                                  VALUES(g_head_1.rvw01,g_head_1.rvw02,g_head_1.rvw03,
                                         g_head_1.rvw04,g_head_1.rvw07,g_head_1.rvw11,
                                         g_head_1.rvw12,g_head_1.rvw13,                      #add rvw13 110327
                                         g_rvw[l_ac].rvw05f,g_rvw[l_ac].rvw06f,
                                         g_rvw[l_ac].rvw05,g_rvw[l_ac].rvw06,
                                         g_rvw[l_ac].rvw08,g_rvw[l_ac].rvw09,
                                         g_rvw[l_ac].rvw10,g_rvw[l_ac].rvw17,
...
FUNCTION i140_upd_rvb22(p_cmd)                #更新驗收單發票號碼
   DEFINE p_cmd    LIKE type_file.chr1        # Prog. Version..: '5.25.01-11.01.17(01)   #b:單身新增 d:刪除
...
#No.MOD-A20009 --end
 ...
 LET g_sql = "SELECT rvuconf FROM ",cl_get_target_table(g_plant_new,'rvu_file'), #FUN-A50102
             " WHERE rvu01 = '",g_rvw[l_ac].rvw08,"' "
 CALL cl_replace_sqldb(g_sql) RETURNING g_sql                   #FUN-A50102
 CALL cl_parse_qry_sql(g_sql,g_plant_new) RETURNING g_sql       #FUN-A50102
 PREPARE sel_rvuconf_pre FROM g_sql
 EXECUTE sel_rvuconf_pre INTO l_rvuconf
      ...
      ELSE
         IF NOT cl_null(l_rvv36) THEN              #FUN-940083--add
            ...
            EXECUTE sel_rvu03_pre3 INTO g_rvw[l_ac].rvv09,g_rvw[l_ac].ima01,
                                        g_rvw[l_ac].ima02,g_rvw[l_ac].rvw10,
                                        l_rvu12,g_rvw[l_ac].rvw17,g_rvw11,
         ...
         ELSE
            ...
            LET g_sql = "SELECT rvu03,rvv31,rvv031,rvv17,",
                        ...
                        "   AND rvv02 = '",g_rvw[l_ac].rvw09,"' ",
                        "   AND rvb01 = rvv04 AND rvb02 = rvv05 ",
                        "   AND rvb01 = rva01 AND gec01 = rvu115",
                        "   AND gec011= '1'   AND rvv01 = rvu01 ",
                        "   AND rvuconf = 'Y' AND rvu00 = '1'   ",
                        "   AND rvv89 <> 'Y'"
            CALL cl_replace_sqldb(g_sql) RETURNING g_sql                   #FUN-A50102
  ...
  #所以rvu00='3'時,不做數量(rvw10)的管控
   ...
   IF cl_null(g_rvw[l_ac].rvw05f) OR g_rvw[l_ac].rvw05f=0 OR (NOT cl_null(g_rvw[l_ac].rvw10)) THEN    #No.MOD-920384 add  #No.MOD-940412
     ...
     #EXECUTE sel_gec07_pre INTO g_gec07                             #MOD-A80052 mark
      ...
      IF g_gec07 = 'Y' THEN    #含稅
         #未稅金額=(含稅單價*數量)/(1+稅率/100)
      #  IF g_rvw[l_ac].rvw10 <> 0 THEN    #mark  by xufeng 120927
      #    #-MOD-A80052-add-
      #     IF g_gec05 = 'T' THEN
      #        LET g_rvw05f = g_rvv38t*g_rvw[l_ac].rvw10
      #        LET g_rvw05f = cl_digcut(g_rvw05f,t_azi04)
      ...
      #     LET g_rvw[l_ac].rvw05f = g_rvv39t/(1+g_head_1.rvw04/100)
      #  END IF
	  # add by xufeng 120927-----直接将入库单的单价，金额赋给rvw   避免进位误差
	  LET g_rvw[l_ac].rvw05f = cl_digcut(g_rvw[l_ac].rvw05f,t_azi04)  #未税金额
         #LET g_rvw[l_ac].rvw17 = cl_digcut(g_rvw[l_ac].rvw17,t_azi04)    #未税单价   #mark by lifang 131023 单价小数位应取azi03
          LET g_rvw[l_ac].rvw17 = cl_digcut(g_rvw[l_ac].rvw17,t_azi03)    #未税单价   #add by lifang 131023
	  # end
      ELSE                     #不含稅
         #未稅金額=未稅單價*數量
         IF g_rvw[l_ac].rvw10 <> 0 THEN
      ...
      END IF
   END IF   #MOD-870285 add
  #IF g_type = '3' AND g_rvw[l_ac].rvw05f > 0 THEN   #負值顯示  #No:110331 Modify <-yangjian-
  #IF g_type = '3' THEN      #如果是仓退的负数，也要取反值
   IF g_type = '3' AND g_rvw[l_ac].rvw05f > 0 THEN   #負值顯示  #add by chenyu --110627
      LET g_rvw[l_ac].rvw05f = g_rvw[l_ac].rvw05f * -1
   END IF
  #IF g_type = '2' AND g_rvw[l_ac].rvw05f > 0 THEN    #No:110331  Add <--yangjian-
  #IF g_type = '2' THEN
   IF g_type = '2' AND g_rvw[l_ac].rvw05f > 0 THEN    #add by chenyu --110626
      IF l_rvv03 = '3' THEN
         LET g_rvw[l_ac].rvw05f = g_rvw[l_ac].rvw05f * -1
      END IF
   END IF
   LET g_rvw[l_ac].rvw05f = cl_digcut(g_rvw[l_ac].rvw05f,t_azi04)  #No.MOD-740008
   IF g_flag = 'N' THEN                                            #MOD-A80052
      # mod by lixwz180330 s---
      LET g_rvw[l_ac].rvw06f = g_rvw[l_ac].rvw17*g_rvw[l_ac].rvw10*g_rvw04/100
      #LET g_rvw[l_ac].rvw06f = g_rvw[l_ac].rvw05f*g_rvw04/100        #原幣稅額
      LET g_rvw[l_ac].rvw06f = cl_digcut(g_rvw[l_ac].rvw06f,t_azi04)  #No.MOD-740008
      # mod by lixwz180330 e---
   END IF                                                          #MOD-A80052
   ...
   LET g_rvw[l_ac].rvw06  = g_rvw[l_ac].rvw17*g_rvw[l_ac].rvw10*g_rvw04/100*g_rvw12
   LET g_rvw[l_ac].rvw06  = cl_digcut(g_rvw[l_ac].rvw06,g_azi04)
   # mod by lixwz180330 e---
 # LET g_rvw[l_ac].rvv38t = g_rvw[l_ac].rvw17*(1+g_head_1.rvw04/100)    #NO.110429 add  #mark by xufeng 120927
   LET g_rvw[l_ac].rvv38t = cl_digcut(g_rvw[l_ac].rvw17*(1+g_head_1.rvw04/100),t_azi04)
   DISPLAY BY NAME g_rvw[l_ac].rvv38t            #NO.110429 add
...
DEFINE
   ...
   CONSTRUCT l_wc2 ON rvw08,rvw09,rvw10,rvw05f,rvw06f,rvw05,rvw06   #MOD-640405
                 FROM s_rvw[1].rvw08,s_rvw[1].rvw09,
                      s_rvw[1].rvw10,s_rvw[1].rvw05f,   #MOD-640405
                      s_rvw[1].rvw06f,s_rvw[1].rvw05,s_rvw[1].rvw06
...
DEFINE l_rvv25     LIKE rvv_file.rvv25    #no.151008
      IF p_wc2 = 'all' THEN
         LET l_sta = 'N'
      ELSE
         LET l_sta = 'Y'
      ...
      END IF
   LET g_sql =
#         "SELECT rvw99,rvw08,rvw09,'','','',rvw10,rvw17,rvw05f,rvw06f,rvw05,rvw06",   #FUN-9B0130 add rvw99
         #"SELECT rvw99,rvw08,rvw09,'','','',rvw10,rvw17,'',rvw05f,rvw06f,'',rvw05,rvw06,'' ",     #add 110401   #No:110527  Modify <<-yangjian-
          "SELECT rvw99,rvw08,rvw09,'','','','','',rvw10,rvw17,'',rvw05f,rvw06f,'',rvw05,rvw06,'' ",     #add 110401
          "  FROM rvw_file ",    #FUN-9B0130
   ...
   IF SQLCA.sqlcode THEN    #No.TQC-790140
      CALL cl_err('i140_pb:',STATUS,1) EXIT PROGRAM
   END IF
   ...
   FOREACH rvw_curs INTO g_rvw[g_cnt].*            #單身 ARRAY 填充
    ...
    # LET g_sql = "SELECT rvv31,rvv031,rvv38t FROM ",cl_get_target_table(g_plant_new,'rvv_file'),              #mod 110401  #No:110527  Modify <-yangjian-
    # LET g_sql = "SELECT rvv31,rvv031,ima021,rvv38t,rvv86 FROM                                                #mod no.151008 add rvv25
      LET g_sql = "SELECT rvv31,rvv031,ima021,rvv38t,rvv86,rvv25  FROM
                  ",cl_get_target_table(g_plant_new,'rvv_file'),              #mod 110401
                  ",",cl_get_target_table(g_plant_new,'ima_file'),  #No:110527  Add <<-yangjian--
                  " WHERE rvv01 = '",g_rvw[g_cnt].rvw08,"' ",  #No:110527 Add <<--yangjian-
                  "   AND ima01(+) = rvv31 ",     #No:110527  Add <<--yangjian-
                  "   AND rvv02 = '",g_rvw[g_cnt].rvw09,"' "
      CALL cl_replace_sqldb(g_sql) RETURNING g_sql                   #FUN-A50102
      CALL cl_parse_qry_sql(g_sql,g_plant_new) RETURNING g_sql       #FUN-A50102
      PREPARE sel_rvv031 FROM g_sql
    # EXECUTE sel_rvv031 INTO g_rvw[g_cnt].ima01,g_rvw[g_cnt].ima02
    # EXECUTE sel_rvv031 INTO g_rvw[g_cnt].ima01,g_rvw[g_cnt].ima02,g_rvw[g_cnt].rvv38t     #add g_rvw[g_cnt].rvv38t   110401   #No:110527 Modify <<-yangjian-
      EXECUTE sel_rvv031 INTO g_rvw[g_cnt].ima01,g_rvw[g_cnt].ima02,g_rvw[g_cnt].ima021,
                              g_rvw[g_cnt].rvv38t,g_rvw[g_cnt].rvv86,l_rvv25                                    #add l_rvv25 no.151008

      IF NOT cl_null(l_rvv25) AND l_rvv25='Y' THEN LET g_rvw[g_cnt].rvv38t=0 END IF                                 #no.151008
      ...
      #add 110401 增加含税金额#
      LET g_rvw[g_cnt].jine1 = g_rvw[g_cnt].rvw05f + g_rvw[g_cnt].rvw06f
      LET g_rvw[g_cnt].jine2 = g_rvw[g_cnt].rvw05  + g_rvw[g_cnt].rvw06
      DISPLAY g_rvw[g_cnt].jine1 TO FORMONLY.jine1
      DISPLAY g_rvw[g_cnt].jine2 TO FORMONLY.jine2
      #add 110401 end #
...
DEFINE
   ...
   DISPLAY ARRAY g_rvw TO s_rvw.* ATTRIBUTE(COUNT=g_rec_b,UNBUFFERED)
     ...
     #--NO.130826 begin--#
      ON ACTION gen_data_1
         LET g_action_choice= "gen_data_1"
         EXIT DISPLAY
     #--NO.130826 end--#
...
DEFINE  l_cnt   LIKE type_file.num5     #NO FUN-690009 SMALLINT
   ...
   ELSE
      FETCH i140_cl INTO
            g_head_1.rvw01,g_head_1.rvw07,g_head_1.rvw02,g_head_1.rvw03,
            g_head_1.rvw04,g_head_1.rvw11,g_head_1.rvw12,g_head_1.rvw13     #add rvw13 110327               # 鎖住將被更改或取消的資料   #MOD-640405
...
DEFINE
   ...
   INPUT BY NAME g_type,g_head_1.rvw01,g_head_1.rvw02,g_head_1.rvw07,
                 g_head_1.rvv06,g_head_1.rvw03,g_head_1.rvw11,
                 g_head_1.rvw05f_s,g_head_1.rvw06f_s,g_head_1.rvw12,g_head_1.rvw13         #add rvw13 110327   #MOD-640405
...
FUNCTION i140_rvv06()
   ...
   DEFINE l_pmc081  LIKE pmc_file.pmc081   #add by lifang 140114

   SELECT pmc03,pmc05,pmcacti,pmc22,pmc47,pmc081
     INTO l_pmc03,l_pmc05,l_pmcacti,l_pmc22,l_pmc47,l_pmc081
   ...
   SELECT gec04 INTO g_head_1.rvw04
     FROM gec_file WHERE gec01 = g_head_1.rvw03 AND gec011='1' #No.TQC-9A0091 mod
   DISPLAY BY NAME g_head_1.pmc03,g_head_1.rvw11,g_head_1.rvw03,g_head_1.rvw04
END FUNCTION
...
FUNCTION i140_r()
   ...
   ELSE
      FETCH i140_cl INTO g_head_1.rvw01,g_head_1.rvw07,g_head_1.rvw02,    #鎖住將被更改或取消的數據
                         ...
                         g_head_1.rvw12,g_head_1.rvw13   #MOD-640405                #add rvw13 110327
...
DEFINE l_apb09  LIKE apb_file.apb09   #TQC-6C0126
DEFINE l_rvv03  LIKE rvv_file.rvv03   #No.TQC-760013
DEFINE l_rvv04  LIKE rvv_file.rvv04   #No.FUN-940083
DEFINE l_rvv36  LIKE rvv_file.rvv36   #No.FUN-940083
   ...
   LET l_rvw.rvw13=g_head_1.rvw13                      #add rvw13 110327
   ...
   LET l_rvw.rvw09=g_rvw[l_ac].rvw09
      ...
      IF g_sma.sma116 MATCHES '[13]' THEN
         IF NOT cl_null(l_rvv36) THEN                                             #FUN-940083--add
            LET l_sql="SELECT rvv38,rvv87,rvv39,pmm22,pmm21,gec04,gec05,rvv03",   #TQC-6C0126 #No.TQC-760013 add rvv03
                      " ,rvv25 ",#MOD-B50016 add rvv25
                     ...
                     ," UNION SELECT rvv38,rvv87,rvv39,'','',rvu12,'',rvv03",
                      " ,rvv25 ",#MOD-B50016 add rvv25
         ...
         ELSE
            IF NOT cl_null(l_rvv04) THEN
               LET l_sql="SELECT rvv38,rvv87,rvv39,rva113,rva115,gec04,gec05,rvv03",
                         " ,rvv25 ",#MOD-B50016 add rvv25
                        ...
                        ," UNION SELECT rvv38,rvv87,rvv39,'','',rvu12,'',rvv03",
                         ...
                         "   AND rvv01 = rvu01 "
            ELSE
               LET l_sql="SELECT rvv38,rvv87,rvv39,rvu113,rvu115,gec04,gec05,rvv03",
                         " ,rvv25 ",#MOD-B50016 add rvv25
      ...
      ELSE
         IF NOT cl_null(l_rvv36) THEN                                             #FUN-940083--add
            LET l_sql="SELECT rvv38,rvv17,rvv39,pmm22,pmm21,gec04,gec05,rvv03",   #TQC-6C0126 #No.TQC-760013 add rvv03
                      " ,rvv25 ",#MOD-B50016 add rvv25
                      ...
                      "   AND pmm01 = pmn01 AND gec01 = pmm21 AND gec011='1' ", #No.TQC-9A0091 mod
                      "   AND rvv89 <>'Y'"                                        #FUN-940083--add
                     ," UNION SELECT rvv38,rvv17,rvv39,'','',rvu12,'',rvv03",
                      " ,rvv25 ",#MOD-B50016 add rvv25
                      #"  FROM ",li_dbs CLIPPED,"rvu_file,",li_dbs CLIPPED,"rvv_file ",   #FUN-9B0130
                      "  FROM ",cl_get_target_table(g_plant_new,'rvu_file'),",", #FUN-A50102
                                cl_get_target_table(g_plant_new,'rvv_file'),     #FUN-A50102
         ...
         ELSE
            IF NOT cl_null(l_rvv04) THEN
               LET l_sql="SELECT rvv38,rvv17,rvv39,rva113,rva115,gec04,gec05,rvv03",
                         " ,rvv25 ",#MOD-B50016 add rvv25
                        ...
                        ," UNION SELECT rvv38,rvv17,rvv39,'','',rvu12,'',rvv03",
                         ...
                         "   AND rvu01=rvv01 "
               ELSE
               LET l_sql="SELECT rvv38,rvv17,rvv39,rvu113,rvu115,gec04,gec05,rvv03",
                         " ,rvv25 ",#MOD-B50016 add rvv25
                         #"   FROM ",li_dbs CLIPPED,"rvv_file,",li_dbs CLIPPED,"rvu_file,",
                         #           "OUTER ",li_dbs CLIPPED,"gec_file ",
                         "   FROM ",cl_get_target_table(g_plant_new,'rvv_file'),",", #FUN-A50102
...
DEFINE p_cmd       LIKE type_file.chr1,    # Prog. Version..: '5.25.01-11.01.17(01)  #b:單身 u:單頭修改
...
#NO.110429 录入时不可修改数量，金额，价格--------begin
   IF p_cmd = 'a' THEN
      CALL cl_set_comp_entry("rvw10,rvw17,rvw05f,rvw06f,rvw05,rvw06",FALSE)
   END IF
#NO.110429-----------------------------------------end
   CALL cl_set_comp_entry("rvw09",TRUE)    #Mark by chenyu --110429
  #CALL cl_set_comp_entry("rvw09,rvw17,rvw05f,rvw06f",TRUE)    #Mark by chenyu --110429
END FUNCTION

FUNCTION i140_set_no_entry_b(p_cmd)
   DEFINE p_cmd  LIKE type_file.chr1     #NO FUN-690009 VARCHAR(01)

   #add 110331 不可修改数量，金额，价格#
   IF p_cmd = 'u' THEN
      CALL cl_set_comp_entry("rvw10,rvw17,rvw05f,rvw06f,rvw05,rvw06",FALSE)
   END IF
   #add 110331 end#
  ...
  #CALL cl_set_comp_entry("rvw09,rvw17,rvw05f,rvw06f",FALSE)
   CALL cl_set_comp_entry("rvw17,rvw05f,rvw06f,ima021,rvv86",FALSE)
END FUNCTION
...
FUNCTION i140_gen()
   ...
   LET g_argv2 = "query"
   LET g_no1 = s_get_doc_no(g_head_1.rvw13)
   SELECT * INTO g_apy.* FROM apy_file WHERE apyslip = g_no1
   IF g_apy.apykind = '11' THEN
      LET l_str = "aapt110 '",g_head_1.rvw13,"' '",g_argv2,"'"
   ELSE
      LET l_str = "aapt210 '",g_head_1.rvw13,"' '",g_argv2,"'"
   END IF
