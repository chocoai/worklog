...
DEFINE g_oea          DYNAMIC ARRAY OF RECORD
                      oea03       LIKE oea_file.oea03,   # add by  lixwz 171102
                      oea032      LIKE oea_file.oea032, # add by lixwz 171102
   ...
   DEFINE l_name    LIKE type_file.chr20,
    ...
    SELECT zo02 INTO g_company FROM zo_file
     ...
     IF l_term = " oea03 like '%'" THEN
          LET l_sql ="SELECT UNIQUE OEA03,occ02,'' FROM(SELECT UNIQUE OEA03 FROM  OEA_FILE , TC_NME_FILE , TC_NMG_FILE",
                                " WHERE TC_NME03 =OEA01 AND TC_NME01 = TC_NMG01 AND TC_NMGDATE  BETWEEN '",tm.bdate,"' AND '",tm.edate,"'",
                                " UNION SELECT  UNIQUE OMF05 FROM  OMF_FILE WHERE OMF03 BETWEEN '",tm.bdate,"' AND '",tm.edate,"'",
                                " UNION SELECT  UNIQUE  OHA03 FROM OHA_FILE WHERE OHA02 BETWEEN '",tm.bdate,"' AND '",tm.edate,"'",
                                " AND OHACONF =       'Y' AND OHAPOST =       'Y' UNION ",
                                " SELECT UNIQUE NMH11 FROM NMH_FILE WHERE NMH04 BETWEEN '",tm.bdate,"' AND '",tm.edate,"' AND NMH33 IS NOT NULL",
                                " UNION SELECT UNIQUE NMG18 FROM NMG_FILE WHERE NMG01 BETWEEN '",tm.bdate,"' AND '",tm.edate,"'AND NMG13 IS NOT NULL AND NMG18 IS NOT NULL",
                                " UNION SELECT UNIQUE OOA03 FROM OOA_FILE WHERE OOA02 BETWEEN '",tm.bdate,"' AND '",tm.edate,"' AND OOA37 = '3' AND OOA33 IS NOT NULL)",
...
FUNCTION cxmq110_cs()
   ...
   #No.FUN-A30009  --Begin
   IF tm.c = 'Y' THEN
     LET g_sql = "SELECT UNIQUE oea03t,oea032t,oea23 FROM cxmq110_tmp ",
                 " ORDER BY oea03t,oea032t,oea23 "
   ELSE

     LET g_sql = "SELECT UNIQUE oea03t,oea032t FROM cxmq110_tmp ",
...
FUNCTION  cxmq110_b_fill()                     #BODY FILL UP
  ...
  DEFINE  l_type     LIKE type_file.chr1
   ...
   IF tm.c = 'Y' THEN
      LET g_sql = "SELECT oea03t,oea032t, vdate,vno,oea01,type,tc_nmg01,df,d,cf,c,zzysye,chysye,exp,azi04,azi05,azi07 ",
                  " FROM cxmq110_tmp ",
