...
FUNCTION cxmq111()
   DEFINE l_name    LIKE type_file.chr20,
    ...
    SELECT zo02 INTO g_company FROM zo_file
     ...
     END IF
     IF l_term = " oea03 like '%'" THEN
          LET  l_sql = "SELECT UNIQUE OEA03,occ02,'' FROM(select unique oea03 from oea_file where oea02 BETWEEN '",tm.bdate,"' AND '",tm.edate,"' AND oea00 != 0",
                            " union select unique oea03 from oea_file,tc_nme_file,tc_nmg_file where tc_nme03 = oea01 AND tc_nme01 = tc_nmg01 AND tc_nmgdate BETWEEN  '",
                            tm.bdate,"' AND '",tm.edate,"' union select unique omf05 from omf_file where   omf03 BETWEEN '",tm.bdate,"' AND '",tm.edate,"'",
                            " union select unique  oha03 from oha_file where oha02 BETWEEN  '",tm.bdate,"' AND '",tm.edate,"' AND ohaconf='Y' AND ohapost='Y' ",
                            " union select unique oga03 from oga_file  where oga02 BETWEEN '",tm.bdate,"' AND '",tm.edate,"' and ogapost='Y' and ogaconf='Y'",
                            " union select unique nmh11 from nmh_file where nmh04 BETWEEN  '",tm.bdate,"' AND '",tm.edate,"'",
                            " union select unique nmg18 from nmg_file  where nmg01 BETWEEN '",tm.bdate,"' AND '",tm.edate,"' and nmg18 is not null)",
                            " LEFT JOIN occ_file on occ01 =oea03 "
     ELSE
         IF tm.c = 'Y' THEN
...
FUNCTION cxmq111_cs()
   ...
   #No.FUN-A30009  --Begin
   IF tm.c = 'Y' THEN
     LET g_sql = "SELECT UNIQUE oea03t,oea032t,oea23 FROM cxmq111_tmp ",
                 " ORDER BY oea03t,oea032t,oea23 "
   ELSE

     LET g_sql = "SELECT UNIQUE oea03t,oea032t FROM cxmq111_tmp ",
                 " ORDER BY oea03t,oea032t"
   END IF
...
FUNCTION cxmq111_b_fill()                     #BODY FILL UP
  ...
  DEFINE  l_type     LIKE type_file.chr1
   ...
   IF tm.c = 'Y' THEN
      LET g_sql = "SELECT oea03t,oea032t, vdate,oea01,type,tc_nmg01,a,af,b,bf,c,cf,d,df,l,g,h,hf,i,iff,j,jf,k,kf,m,exp,azi04,azi05,azi07 ",  # mod by lixwz  20170804 add l,g,m
                  " FROM cxmq111_tmp ",
                  " WHERE oea03t ='",g_oea03,"'",
                  "   AND oea032t ='",g_oea032,"'",
                  "   AND oea23 ='",g_oea23,"' "," order by seq "
   ELSE
      LET g_sql = "SELECT oea03t,oea032t, vdate,oea01,type,tc_nmg01,a,af,b,bf,c,cf,d,df,l,m,g,h,hf,i,iff,j,jf,k,kf,exp,azi04,azi05,azi07 ",  # mod by lixwz  20170804 add l,g,m
                  " FROM cxmq111_tmp",
                  " WHERE oea03t ='",g_oea03,"'",
                  "   AND oea032t ='",g_oea032,"' "," order by seq "
   END IF
...
FUNCTION cxmq111_table()
     ...
     CREATE TEMP TABLE cxmq111_tmp(
                    oea03t    LIKE oea_file.oea03,
                    oea032t   LIKE oea_file.oea032,
                    oea23    LIKE oea_file.oea23,
                    mm       LIKE type_file.num5,
                    vdate      LIKE npp_file.npp02,   #日期
                    oea01      LIKE oea_file.oea01,   #订单号
...
FUNCTION cxmq111_t()
   ...
   END IF
   CALL cl_set_comp_att_text("oea03t","客户编号")   # add by lixwz 20170901
   CALL cl_set_comp_att_text("oea032t","客户简称")   # add by lixwz 20170901
   IF tm.c = 'Y' THEN
