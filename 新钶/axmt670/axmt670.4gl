...
#add by renkl160204 start
DEFINE l_dummy01  LIKE type_file.chr1000
DEFINE l_gec05    LIKE gec_file.gec05
DEFINE l_gec011   LIKE gec_file.gec011
#add by renkl160204 end
   LET g_ss='Y'
...
#add by renkl160204 start
        IF NOT cl_null(g_omf_1.omf06) THEN
            SELECT gec05,gec011 INTO l_gec05,l_gec011 FROM gec_file
              WHERE gec01=g_omf_1.omf06
            IF l_gec011 = '1' THEN
              CASE l_gec05
                  WHEN 'A'
                    LET l_dummy01 = '农产品收购凭证'
                  WHEN 'T'
                    LET l_dummy01 = '运输发票'
                  WHEN 'W'
                    LET l_dummy01 = '废旧物资收购凭证'
                  WHEN 'Z'
                    LET l_dummy01 = '其它'
                  WHEN 'S'
                    LET l_dummy01 = '可抵扣专用发票'
                  WHEN 'N'
                    LET l_dummy01 = '不可抵扣专用发票'
                  WHEN 'G'
                    LET l_dummy01 = '海关代征完税凭证'
                  WHEN 'X'
                    LET l_dummy01 = '不申请'
              END CASE
            END IF
            ...
            IF l_gec011 = '2' THEN
              CASE l_gec05
                WHEN 'A'
                  LET l_dummy01 = '增值税专用发票'
                WHEN 'B'
                  LET l_dummy01 = '普通发票'
                WHEN 'C'
                  LET l_dummy01 = '无发票视同销售'
              END CASE
            END IF
        END IF
      DISPLAY l_dummy01 TO FORMONLY.dummy01
#add by renkl160204 end
...
DEFINE
   ...
   ELSE
    ...
    #   AND omf02 = g_omf_1.omf02   #FUN-C60033
      ...
      SELECT azi04 INTO t_azi04 FROM azi_file
             WHERE azi01 = g_omf_1.omf07
      #CAll t670_upd_show_diff()  #MOD-D60161 add # mark by lixwz 171206
      #FUN-C60036--add--str
   ...
   WHILE TRUE
     ...
     #LET g_omf02_t = g_omf_1.omf02   #FUN-C60033
      ...
      IF INT_FLAG THEN
         ...
         EXIT WHILE
      END IF
      IF g_omf_1.omf02 IS NULL THEN lET g_omf_1.omf02 = ' ' END IF
   ...
   #IF g_omf_1.omf01 IS NULL THEN
    ...
    LET g_forupd_sql = "SELECT omf21,omf09,omf10,omf11,omf12,omf25,omf26,omf13,omf14,", #FUN-D10082 add omf25,omf26
                      ...
                      #"       omf18,omf19,omf19x,omf19t,omf04 FROM omf_file",#FUN-C60036 add omf04#FUN-D20003 mark
                       "       omf18,omf19,omf19x,omf19t ",
                       "       ,'','' ",     #add by dengsy151231
                       "FROM omf_file",#FUN-D20003 add
              ...
              ELSE
                 ...
                 ELSE
                     ...
                     EXECUTE sel_ogb14_pre INTO g_omf[l_ac].ogb14
                                                ,g_omf[l_ac].ogb41   #add by dengsy151231
               #     DISPLAY BY NAME g_omf[l_ac].ogb14
                    #CALL t670_set_entry(p_cmd)     #FUN-C60036 mark
                    #str---- add by dengsy151231
                    IF NOT cl_null(g_omf[l_ac].ogb41) THEN
                     SELECT pja02 INTO g_omf[l_ac].pja02 FROM pja_file
                     WHERE pja01=g_omf[l_ac].ogb41
                     END IF
                     #end----- add by dengsy151231
                     CALL t670_set_no_entry(p_cmd)
                 END IF
...
DEFINE p_wc   STRING   #MOD-8B0084
   ...
   FOREACH omf_curs INTO g_omf[g_cnt].*   #單身 ARRAY 填充
        ...
        LET g_sql = "SELECT ogb14 ",
                    " ,ogb41 ", #add by dengsy151230
                   "  FROM ",cl_get_target_table(g_plant_new,'ogb_file'),
                   " WHERE ogb01 = '",g_omf[g_cnt].omf11,"' ",
                   "   AND ogb03 = '",g_omf[g_cnt].omf12,"' "
      ELSE
 	       LET g_sql = "SELECT ohb14,''  ",    #add '', by dengsy151231
                    "  FROM ",cl_get_target_table(g_plant_new,'ohb_file'),
      ...
      EXECUTE sel_ogb14_pre1 INTO g_omf[g_cnt].ogb14
                                  ,g_omf[g_cnt].ogb41           #add by dengsy151231
      DISPLAY BY NAME g_omf[g_cnt].ogb14
      #str---- add by dengsy151231
      IF NOT cl_null(g_omf[g_cnt].ogb41) THEN
         SELECT pja02 INTO g_omf[g_cnt].pja02 FROM pja_file
          WHERE pja01=g_omf[g_cnt].ogb41
      END IF
      DISPLAY BY NAME g_omf[g_cnt].ogb41
      DISPLAY BY NAME g_omf[g_cnt].pja02
      #end----- add by dengsy151231
...
FUNCTION t670_set_no_entry_b(p_cmd)
   ...
   IF l_ac>0 THEN
      IF g_omf[l_ac].omf10 = '9' THEN
         ...
         #CALL cl_set_comp_entry('omf14,omf15',FALSE)        #FUN-D10082 add #yinhy130510 # mark by lixwz 171226
         CALL cl_set_comp_entry('omf14,omf15',TRUE)               # add by lixwz 171226
      ELSE
