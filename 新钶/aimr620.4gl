...
# Modify.........: No:CHI-D50002 13/05/06 By Alberti 增加分群碼(ima06)、其它分群碼一(ima09)
#                                                    其它分群碼二(ima10)、其它分群碼三(ima11)開窗功能
# Modify.........: No:FUN-D70127 13/08/01 By wangrr 會計年期會不按照自然年月設置,年期的判斷需要按照aooq011的設置來
# Modify.........: No:MOD-FB0130 15/11/24 By catmoon 還原MOD-970243，平均庫存需除以起迄期間之期數
# Modify.........: No:MOD-H20050 17/02/21 By Mandy 平均存貨維護日數l_7及安全天數1_8 公式調整
...
FUNCTION r620_tm(p_row,p_col)
   ...
   LET tm.yy1= s_get_aznn(g_plant,g_bookno1,g_today,1)
   LET tm.yy2= tm.yy1
   LET tm.m1 = s_get_aznn(g_plant,g_bookno1,g_today,3)
   LET tm.m2 = tm.m1
   #FUN-D70127--MOD--end
   LET tm.d='1'
   LET tm.e='N'
   ...
   DECLARE r620_cs4 CURSOR FROM l_sql
     ...
     IF g_success = 'Y' THEN
        FOREACH r620_cs2 INTO sr2.* 
           ...
           LET l_3 = l_3 / g_during   #MOD-FB0130 add
           ...
           ELSE 
             #LET l_7=(l_5/l_4)/g_durday   #MOD-H20050 mark
              LET l_7= l_5/(l_4 /g_durday) #MOD-H20050 add  平均存貨維護日數
           END IF
           IF l_2=0 THEN 
              LET l_8=sr2.ima27/g_durday
           ELSE 
             #LET l_8=(sr2.ima27/l_2)/g_durday #MOD-H20050 mark
              LET l_8=sr2.ima27/(l_2/g_durday) #MOD-H20050 add  安全天數
           END IF
