...
FUNCTION p500_wipx()     # 處理 WIP 在製成本 (工單性質=1/7)
   ...
   CLOSE wipx_c1
   IF l_cnt > 0 THEN
      CALL p500_xca_to0() # add by lixwz  171212
      CALL p500_ccc_tot('2')    # 計算所有出庫成本及結存
      CALL p500_ccc_upd()       # Update ccc_file
   END IF
END FUNCTION
...
END FUNCTION
#FUN-D60130 add-----------------end
# add by lixwz 171212 s
FUNCTION p500_xca_to0()
  ...
  UPDATE cxa_file
   ...
   WHERE cxa01 =g_ima01
     ...
     AND cxa03 = mm
  IF SQLCA.sqlcode  THEN
      CALL cl_err('upd cxa',SQLCA.sqlcode,1)
  END IF
  DELETE FROM cxc_file
        WHERE cxc01 = g_ima01
          AND cxc02 = yy
          AND cxc03 = mm
  IF SQLCA.sqlcode  THEN
      CALL cl_err('del cxc',SQLCA.sqlcode,1)
  END IF
  DELETE FROM tlf_temp01
  IF SQLCA.sqlcode  THEN
      CALL cl_err('del cxa',SQLCA.sqlcode,1)
  END IF
END FUNCTION
# add by lixwz 171212 e