...
FUNCTION (p500_ccc_tot)(p_sw)     # 計算所有出庫成本及結存
  ...
  UPDATE cxa_file SET cxa10 = cxa10 + l_qty,
                      ...
                      cxa117= 0,
                      cxa118= 0
      WHERE cxa01= g_ccc.ccc01 AND cxa02 =g_ccc.ccc02
        AND cxa03= g_ccc.ccc03 AND cxa010=g_ccc.ccc07
...
　
    ...
    #IF p_flag = 1 THEN
     IF p_flag = 1 AND l_n = 0 THEN #MOD-DA0096 add l_n = 0
        ...
        EXECUTE cxc_ins_p1 USING l_cxa.cxa01,l_cxa.cxa02,l_cxa.cxa03,q_tlf.tlf905,
                  ...
                  l_up,l_seq,g_legal
        INSERT INTO cxc_file(cxc01,cxc02,cxc03,cxc04,cxc05,cxc06,cxc07,cxc08,
                             cxc09,cxc091,cxc092,cxc093,cxc094,cxc095,cxc096,
                             cxc097,cxc098,cxc010,cxc011,cxc10,cxc11,cxclegal)
            VALUES (l_cxa.cxa01,l_cxa.cxa02,l_cxa.cxa03,q_tlf.tlf905,
                  q_tlf.tlf906,l_cxa.cxa06,l_cxa.cxa07,l_qty,l_amt,
                  l_amt1,l_amt2,l_amt3,l_amt4,l_amt5,
                  l_amt6,l_amt7,l_amt8,type,q_tlf.tlfcost,
                  l_up,l_seq,g_legal)
        SELECT COUNT(*) l_chk INTO FROM cxc_file
         WHERE cxc01 = l_cxa.cxa01  AND cxc03 = l_cxa.cxa03
           AND cxc04 = q_tlf.tlf905 AND cxc05 = q_tlf.tlf906
           AND cxc06 = l_cxa.cxa06  AND cxc07 = l_cxa.cxa07
           AND cxc10 = l_amt8       AND cxc11 = type
        #CHI-C80002---end
   ...
   END FOREACH
   # add ****************
   SELECT sfb02 INTO l_sfb02 FROM sfb_file WHERE sfb01 = q_tlf.tlf036
   IF l_sfb02 = '11' AND p_amt=0 AND q_tlf.tlf10 <>0 THEN
      SELECT cdf09,cdf09a INTO p_amt,p_amt1
        FROM cdf_file
       WHERE cdf02 = yy AND cdf03 = mm
         AND cdf07 = p_part AND cdf06 = q_tlf.tlf036
      IF cl_null(p_amt) THEN LET p_amt = 0 END IF
      IF cl_null(p_amt1) THEN LET p_amt1 = 0 END IF
   END IF
   # add ****************
   IF p_flag = 0 THEN
...
FUNCTION wipx_2_21()   #  2-1. WIP-元件 本期投入拆件品(依=‘式工單發料/退料檔)
   DEFINE l_ima08            LIKE ima_file.ima08   #No.FUN-680122CHAR(1)
   DEFINE l_ima57            LIKE ima_file.ima57   #No.FUN-680122SMALLINT
   DEFINE l_tlf01            LIKE tlf_file.tlf01   #No.MOD-490217
   DEFINE l_tlf02,l_tlf03    LIKE tlf_file.tlf02   #No.FUN-680122SMALLINT
   ...
   FOREACH wipx_c3 INTO l_tlf01,l_ima08,l_ima57,l_tlf02,l_tlf03,l_tlfcost,l_tlf10,	 #TQC-970003 add l_tlfcost
     ...
     CASE                        # 取發料單價
      ...
      WHEN l_ima08 MATCHES '[R]' # (本階重工者取重工前庫存月平均)
           IF g_sma.sma43 = '1' THEN
              CALL p500_upd_cxa09(l_tlf01,l_tlf10,1)
                   RETURNING l_c23,l_c23,l_c23,l_c23,l_c23,l_c23
                                  ,l_c23,l_c23,l_c23   # add by lixwz 171211
              LET l_c23 = 0
           ELSE
              SELECT ccc12a+ccc22a, ccc12b+ccc22b, ccc12c+ccc22c,
                     ccc12d+ccc22d, ccc12e+ccc22e,
