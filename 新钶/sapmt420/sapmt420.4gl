...
DEFINE g_ima50       LIKE ima_file.ima50  #add by wangyan151021
DEFINE g_ima48       LIKE ima_file.ima48
...
DEFINE l_tot            LIKE pml_file.pmlud08 #add by wangyan151021
...
FUNCTION t420(p_argv1,p_argv2,p_argv3,p_argv4) #No.FUN-630010
   ...
   CREATE TEMP TABLE apm_p470(
       part      VARCHAR(40),
       sfa26     VARCHAR(1),
       sfb13     DATE,         #實際開工日 #MOD-740471 add   #MOD-A50021 sfb25-->sfb13
       req_qty   DECIMAL(15,3),
       al_qty    DECIMAL(15,3),
       pr_qty    DECIMAL(15,3),
       po_qty    DECIMAL(15,3),
       qc_qty    DECIMAL(15,3),
      ...
      ON ACTION CONTROLP
         CASE
            ...
            WHEN INFIELD(pmk05)  #專案代號
                 CALL cl_init_qry_var()
                 LET g_qryparam.state= "c"
                 LET g_qryparam.form ="q_pja2"     #mark by shids 151026
                 #LET g_qryparam.form ="cq_pjfa05"  #mark by zhaoxiangb 160103
                 CALL cl_create_qry() RETURNING g_qryparam.multiret
...
#FUN-AA0059--------mod------------str-----------------
                 ...
                 LET g_qryparam.state = 'c'
                 CALL cl_create_qry() RETURNING g_qryparam.multiret
                 CALL q_sel_ima(TRUE, "q_ima","","","","","","","",'')  RETURNING  g_qryparam.multiret  #mark by zhaoxiangb 151203
#FUN-AA0059--------mod------------end-----------------
#mark by zhangyu 151231---str
#                 CALL cl_init_qry_var()
#                 #LET g_qryparam.form = "cq_pjfb04"
#                 LET g_qryparam.form = "cq_pjfa03"  #add by zhaoxiangb 151229
#
#                 LET g_qryparam.state = 'c'
#                 CALL cl_create_qry() RETURNING g_qryparam.multiret
#mark by zhangyu 151231
            ...
            WHEN INFIELD(pml12)  #專案代號
                 CALL cl_init_qry_var()
                 LET g_qryparam.form ="q_pja2"     #mark by shids 151026
                 #LET g_qryparam.form ="cq_pjfa05"    #add by shids 151026  #mark by zhaoxiangb 160103
                 LET g_qryparam.state = "c"   #多選
                 CALL cl_create_qry() RETURNING g_qryparam.multiret
            ...
            WHEN INFIELD(pml90) #費用原因
                 ...
                 LET g_qryparam.form ="cq_azf01a"    #add by zhaoxiangb 151230
                 LET g_qryparam.state = "c"   #多選
...
#FUN-B90101--add--end--
  ...
  #LET g_pmk.pmk22 = g_aza.aza17 #本幣幣別  No.B358 #MOD-B60015 mark
   ...
   WHILE TRUE
       ...
       LET g_pmk.pmk21='J00'            # add by lixwz 171127 默认税别'J00'
       CALL t420_i("a")                      # 各欄位輸入
...
DEFINE l_pja02          LIKE pja_file.pja02 #add by wangyan151021
    ...
    INPUT BY NAME g_pmk.pmkoriu,g_pmk.pmkorig,
      ...
      AFTER FIELD pmk04       #請購日期(預設會計年度/期間)
          ...
          AFTER FIELD pmk05
           IF cl_null(g_pmk.pmk05) THEN NEXT FIELD pmk05 END IF #add by shids 151026
           IF NOT cl_null(g_pmk.pmk05) THEN
              ...
              SELECT COUNT(*) INTO g_cnt FROM pjb_file
                  WHERE pjb01 = g_pmk.pmk05
              IF g_cnt = 0 THEN
                 CALL cl_err(g_pmk.pmk05,'asf-984',0)
                 NEXT FIELD pmk05
              END IF
              #SELECT pja02 INTO l_pja02 FROM pja_file WHERE pja01 = g_pmk.pmk05  #add by wangyan151021
              SELECT pja02 INTO l_pja02 FROM pja_file WHERE pja01 = g_pmk.pmk05
              DISPLAY l_pja02 TO FORMONLY.pja02_1  #add by wangyan151021
              #str add by shids 151026
              IF p_cmd = 'u' THEN
                 LET g_cnt = 0
                 SELECT COUNT(*) INTO g_cnt FROM pml_file
                   WHERE pml01=g_pmk.pmk01 AND pml12<>g_pmk.pmk05
                    IF g_cnt > 0 THEN
                        CALL cl_err(g_pmk.pmk05,'apm1106',1)
                        NEXT FIELD pmk05
                    END IF
              END IF
              #DISPLAY ARRAY g_pml TO s_pml.*
              #end add by shids 151026
           END IF
        ...
        ON ACTION CONTROLP
            CASE
               ...
               WHEN INFIELD(pmk05)  #專案代號
                    CALL cl_init_qry_var()
                    LET g_qryparam.form ="q_pja2"     #mark by shids 151026
                    #LET g_qryparam.form ="q_pja002"    #add by shids 151026  #mark by xulj151229
                    #LET g_qryparam.form ="cq_pjfa05"    #add by xulj151229   #mark by zhaoxiangb 160103
...
DEFINE l_pja02          LIKE pja_file.pja02  #add by shids 151026
   ...
   SELECT pnz02 INTO l_buf FROM pnz_file WHERE pnz01=g_pmk.pmk41 #FUN-930113 oah-->pnz
   DISPLAY l_buf TO FORMONLY.pnz02 #FUN-930113 oah-->pnz
   SELECT pja02 INTO l_pja02 FROM pja_file WHERE pja01 = g_pmk.pmk05      #add by shids 151026
   DISPLAY l_pja02 TO FORMONLY.pja02_1                                    #add by shids 151026
...
#FUN-B90101--add--end--
      LET g_sql="SELECT pml02,pml24,pml25,'',pml47,pml04,'','','','','','','','','','','','','','','','','','','','','', ",  #NO.FUN-670007 ADD pml24/pml25 #No.FUN-870007-add-pml47
                ...
                "       pml83,pml84,pml85,pml80,pml81,pml82,pml86,pml87,",
                "       pml21,pml35,pmlud13,pml34,pml33,pml919,pml55,pml41,",   #FUN-570106   #No.TQC-640132 #No.FUN-870007-add-pml55  #FUN-A80150 add pml919
                "       pml190,pml191,pml192,",     #No.FUN-630040     #pmlud13 add by wangyan151021
                "       pml92,pml93,",  #FUN-9A0065 add pml92,pml93
                "       pml12,'',pml121,pml122,", #pmlud02 add by wangyan151021
                ...
                "       ,pmlud01,pmlud02,pmlud03,pmlud04,pmlud05,",
                "       pmlud06,pmlud07,pmlud08,pmlud09,pmlud10,",
                "       pmlud11,pmlud12,pmlud14,pmlud15",
...
##FUN-A50054---end
#FUN-A60035 ---MARK END
      SELECT pja02 INTO g_pml[l_cnt].pja02 FROM pja_file WHERE pja01 = g_pml[l_cnt].pml12  #add by wangyan151021
      ...
      END IF
   END FOREACH
   SELECT SUM (pmlud09) INTO l_tot FROM pml_file WHERE pml01 = g_pmk.pmk01  #add by wangyan 151021
   DISPLAY l_tot TO FORMONLY.l_tot  #add by wangyan151021
...
FUNCTION t420_out()
   ...
   DEFINE l_type        LIKE type_file.chr1 #add by zhaoxiangb add one new CR
    IF cl_null(g_pmk.pmk01) THEN RETURN END IF
  #  LET l_prog='apmr903' #FUN-C30085 mark
   # LET l_prog='apmg903' #FUN-C30085 add  #mark by zhangyu151201
     LET l_prog='cpmr400' # add by zhangyu 151201
...
DEFINE l_ac_t,l_cnt    LIKE type_file.num5,    #No.FUN-680136 SMALLINT              #未取消的ARRAY CNT
   ...
   DEFINE   li_i,l_cn1,i   LIKE type_file.num5    #No.FUN-680136 SMALLINT
   ...
   DEFINE l_y       LIKE pja_file.pja02 #add by wangyan151021
   DEFINE l_mm      LIKE type_file.num5
   DEFINE l_gec04 LIKE gec_file.gec04
   DEFINE l_gec07 LIKE gec_file.gec07
   DEFINE l_tc_pjfb06 LIKE type_file.num20_6
...
#FUN-B90101--mark--
          ...
          BEFORE ROW
             ...
             LET l_n  = ARR_COUNT()
             LET l_ins_flag = 'N'
             #add by zhaoxiangb 160105
             SELECT gec04,gec07 INTO l_gec04,l_gec07 FROM gec_file WHERE gec01 = g_pmk.pmk21
             #mark by zhaoxiangb 160113
             #IF l_gec07 = 'Y' THEN
             #   CALL cl_set_comp_entry('pmlud08',TRUE)
             #   CALL cl_set_comp_entry('pmlud07',FALSE)
             #ELSE
             #   CALL cl_set_comp_entry('pmlud07',TRUE)
             #   CALL cl_set_comp_entry('pmlud08',FALSE)
             #END IF
             #end add
             #end mark
             #add by zhaoxiangb 160113  #所有情况下只可以维护未税，根据未税推含税
             CALL cl_set_comp_entry('pmlud07',TRUE)
             CALL cl_set_comp_entry('pmlud08',FALSE)
             #end add
             BEGIN WORK
...
#FUN-A60035 ---MARK END
             ...
             #str add by shids 151026
             IF p_cmd='a' THEN
                LET g_pml[l_ac].pml12 = g_pmk.pmk05
                SELECT pja02 INTO g_pml[l_ac].pja02 FROM pja_file WHERE pja01 = g_pml[l_ac].pml12
             END IF
             #end add by shids 151026
...
#FUN-A60035 ---MARK END
             ...
             #MOD-AA0099---add---end---
         #str---add by huanglf161230
          SELECT ROUND(g_pml2.pmlud07,3) INTO g_pml2.pmlud07 FROM dual
         #str---end by huanglf161230
...
#FUN-A60035 ---MARK END
          BEFORE INSERT
             ...
             ELSE
                ...
                #add by zhaoxiangb 160105
                LET g_pml[l_ac].pmlud07 = NULL
                LET g_pml[l_ac].pmlud08 = NULL
                LET g_pml[l_ac].pmlud09 = NULL
                LET g_pml[l_ac].pmlud10 = NULL
                #end add
#FUN-A60035 ---MARK BEGIN
##FUN-A50054 --Begin
#&ifdef SLK
...
#使用多屬性時，過料件編號，只要料號存在，母料號也可過
      ...
      AFTER FIELD att10_c
        ...
        #FUN-A80150---add---end---
         ...
         AFTER FIELD pml35
            IF NOT cl_null(g_pml[l_ac].pml35) THEN
              SELECT ima49,ima491,ima50,ima48 INTO g_ima49,g_ima491,g_ima50,g_ima48 #add by wangyan151021
                FROM ima_file WHERE ima01 = g_pml[l_ac].pml04
                LET l_mm=  g_ima50+g_ima49+g_ima48+g_ima491   #add by wangyan151021
                CALL s_aday(g_pml[l_ac].pml35,1,l_mm) RETURNING g_pml[l_ac].pmlud13
         ...
         AFTER FIELD pml12   #專案代號
          IF NOT cl_null(g_pml[l_ac].pml12) THEN   #str add by shids 151012
              IF NOT cl_null(g_pmk.pmk05) AND g_pml[l_ac].pml12 <> g_pmk.pmk05 THEN
                    CALL cl_err(g_pml[l_ac].pml12,'apm1106',1)
                    NEXT FIELD pml12
              END IF
          ELSE
              NEXT FIELD pml12
          END IF                                   #end add by shids 151012
          IF NOT cl_null(g_pml[l_ac].pml12) THEN
             ...
             IF g_cnt = 0 THEN
                ...
                NEXT FIELD pml12
             END IF
            SELECT pja02 INTO g_pml[l_ac].pja02 FROM pja_file WHERE pja01 = g_pml[l_ac].pml12  #add by wangyan151021
             DISPLAY g_pml[l_ac].pja02 TO pja02
             CALL t420_bud(p_cmd,'3')
        ...
        BEFORE FIELD pml122
          IF cl_null(g_pml[l_ac].pml121) THEN
             #NEXT FIELD pml121    #mark by shids 151026
          ELSE
             SELECT pjb25 INTO l_pjb25 FROM pjb_file
              WHERE pjb02 = g_pml[l_ac].pml121
...
#MOD-B40202 --end-
                 NEXT FIELD pml90
              END IF
              #add by lidj161229-----str-----
              LET l_cn1 = 0
              SELECT COUNT(*) INTO l_cn1 FROM pjfa_file,pjb_file WHERE pjfa01=pjb02
               AND pjb01 = g_pml[l_ac].pml12 AND pjb02 = g_pml[l_ac].pml121
               AND ta_pjfa14 = g_pml[l_ac].pml90
              IF l_cn1 = 0 THEN
              	 CALL cl_err(g_pml[l_ac].pml90,'cpm-003',1)
              	 NEXT FIELD pml90
              END IF
              #add by lidj161229-----end-----
#TQC-B50066 --begin--
...
#No.TQC-B50094 --end
              ...
              DISPLAY BY NAME g_pml[l_ac].pml40,g_pml[l_ac].pml401
           END IF
           #add by zhaoxiangb 160106
           IF g_pml[l_ac].pml90 IS NOT NULL THEN
           SELECT sum(nvl(pml20,0)) INTO l_sum FROM pml_file,pmk_file
                                         WHERE pml01 = pmk01 AND pmk25 != 9
                                           AND pmk05 = g_pmk.pmk05
                                           AND pml90 = g_pml[l_ac].pml90  #add by zhaoxiangb 160106
                                           AND pml04 = g_pml[l_ac].pml04
             IF l_sum IS NULL THEN LET l_sum = 0 END IF
             IF p_cmd = 'a' THEN
                   LET l_sum = l_sum + g_pml[l_ac].pml20
              ELSE
                 LET l_sum = l_sum + g_pml[l_ac].pml20-g_pml_t.pml20
              END IF
                SELECT sum(nvl(pjfa05,0)) INTO l_tc_pjfb06 FROM  pjb_file,pjfa_file
                                 WHERE pjb02 = pjfa01
                                   AND pjb01 = g_pmk.pmk05
                                   AND ta_pjfa14 = g_pml[l_ac].pml90  #add by zhaoxiangb 160106
                                   AND pjfa03 = g_pml[l_ac].pml04
            IF l_sum > l_tc_pjfb06 THEN
                CALL cl_err('','cpj_005',1)
             END IF
            END IF
           #end add
...
#TQC-BB0167 add end
        ...
        AFTER FIELD pml191
         IF NOT cl_null(g_pml[l_ac].pml191) THEN
            ...
            END IF
         END IF
         #add by wangyan151021----str
         AFTER FIELD pmlud07
         IF NOT cl_null(g_pml[l_ac].pmlud07) THEN
           #LET g_pml[l_ac].pmlud08 = g_pml[l_ac].pmlud07
           IF l_gec04 IS NULL THEN LET l_gec04 = 0 END IF
           LET g_pml[l_ac].pmlud08 = g_pml[l_ac].pmlud07*(1+l_gec04*0.01)  #add by zhaoxiangb 160113
           LET g_pml[l_ac].pmlud10 = g_pml[l_ac].pmlud08 * g_pml[l_ac].pml20
           LET  g_pml[l_ac].pmlud09 = g_pml[l_ac].pmlud07 * g_pml[l_ac].pml20
           DISPLAY BY NAME g_pml[l_ac].pmlud09
         END IF
        #add by wangyan151021----end
        AFTER FIELD pmlud08
          IF NOT cl_null(g_pml[l_ac].pmlud08) THEN
           LET  g_pml[l_ac].pmlud07 = g_pml[l_ac].pmlud08/(1+l_gec04*0.01)
           LET  g_pml[l_ac].pmlud09 = g_pml[l_ac].pmlud07 * g_pml[l_ac].pml20
           LET  g_pml[l_ac].pmlud10 = g_pml[l_ac].pmlud08 * g_pml[l_ac].pml20
           DISPLAY BY NAME g_pml[l_ac].pmlud10
         END IF
...
#TQC-B50066 --end--
          ...
          AFTER INPUT
             FOR i = 1 TO g_rec_b
		             LET l_cn1 = 0
		             SELECT COUNT(*) INTO l_cn1 FROM pjfa_file,pjb_file WHERE pjfa01=pjb02
		              AND pjb01 = g_pml[i].pml12 AND pjb02 = g_pml[i].pml121
		              AND ta_pjfa14 = g_pml[i].pml90
		             IF l_cn1 = 0 THEN
		          	    CALL cl_err(g_pml[i].pml90,'cpm-003',1)
...
#                    #    CALL q_sel_ima(TRUE, "q_ima","",g_pml[l_ac].pml04,"","","","","",'')  RETURNING  g_pml[l_ac].pml04  #FUN-AA0059 add  #MOD-AC0309 mark
                     ...
                     #END IF  #No.FUN-870007
                      CALL q_sel_ima(FALSE, "q_ima","",g_pml[l_ac].pml04,"","","","","",'')  RETURNING  g_pml[l_ac].pml04  #mark by zhaoxiangb 151203
                     #FUN-B40042 End-----
...
#mark by zhangyu 151231---str
#                     CALL cl_init_qry_var()
#                     LET g_qryparam.form = "cq_pjfa03"
#                     LET g_qryparam.arg1 = g_pmk.pmk05
#                     CALL cl_create_qry() RETURNING g_pml[l_ac].pml04 #FUN-AA0059 add
#mark by zhangyu 151231---end
               ...
               WHEN INFIELD(pml90)  #費用原因
                  # CALL cl_init_qry_var()
                  #LET g_qryparam.form ="q_azf01a" #No.FUN-930104
                  #LET g_qryparam.form ="cq_azf01a"    #add by zhaoxiangb 151230   #mark by chenke160921
                  #LET g_qryparam.default1 = g_pml[l_ac].pml90
                  #LET g_qryparam.arg1 = '7'       #No.FUN-930104
                  #CALL cl_create_qry() RETURNING g_pml[l_ac].pml90
               #   CALL cq_pml90(g_plant,g_pml[l_ac].pml121)       #mark by chenke160922 #mark by liufang170720
                  CALL cq_pml90(g_plant,g_pml[l_ac].pml121,g_pml[l_ac].pml12,g_pml[l_ac].pml04)       #mark by chenke160922 #mark by liufang170720
                       RETURNING g_pml[l_ac].pml90
...
DEFINE  l_pja02     LIKE pja_file.pja02  #add by shids 151026
    ...
    AFTER FIELD a
       IF l_chr NOT MATCHES '[12345]' THEN
          NEXT FIELD a
       END IF
    ...
    CASE
       ...
       WHEN l_chr='4'
            CALL p500("G",g_pmk.pmk01,"")
            LET g_wc2=NULL
            CALL t420_show()     #add by shids 151026
            CALL t420_b_fill(g_wc2,' 1=1')       #第二個參數，服飾中母單身的條件
            LET g_action_choice="detail"
            IF cl_chk_act_auth() THEN
               CALL t420_b()
            ELSE
               RETURN
            END IF
     #add by zhangyu 151231---str
       WHEN l_chr='5'
            CALL t420_p(g_pmk.pmk05)
            LET g_wc2=NULL
            CALL t420_b_fill(g_wc2,' 1=1')       #第二個參數，服飾中母單身的條件
            LET g_action_choice="detail"
            IF cl_chk_act_auth() THEN
...
#FUN-A60035 ---MARK END
   ...
   #IF NOT cl_null(g_pml[l_ac].pml24) THEN # mod by lixwz180320
   IF NOT cl_null(g_pml[l_ac].pml24) AND NOT cl_null(g_pml[l_ac].pml21) AND g_pml[l_ac].pml21<>0 THEN
...
#    CALL cl_set_comp_required("pml67,pml40,pml31,pml31t",TRUE)        #No.FUN-830161    #TQC-D50082-mark--
     ...
     END IF
  END IF
 CALL cl_set_comp_required("pml90",TRUE)
END FUNCTION
...
##FUN-A50054---end
#FUN-A60035 ---MARK END
  #add by zhaoxiangb 151203
  #SELECT count(*) INTO l_cnt FROM tc_pjfb_file,tc_pjfa_file WHERE tc_pjfa01 = tc_pjfb01
  #   AND tc_pjfa05 = tc_pjfb02 AND tc_pjfa05 = g_pmk.pmk05 AND tc_pjfb04 = g_pml[p_ac].pml04
  #mark by zhangyu 151231---str
  #SELECT COUNT(*) INTO l_cnt FROM pjb_file,pjfa_file WHERE pjb02 = pjfa01 AND pjb01 = g_pmk.pmk05 AND pjfa03 = g_pml[p_ac].pml04
  #IF l_cnt = 0 THEN
  #   CALL cl_err('','cpj_006',1)
  #   RETURN FALSE
  #END IF
  #SELECT ta_pjfa14 INTO g_pml[p_ac].pml90 FROM pjb_file,pjfa_file WHERE pjb02 = pjfa01 AND pjb01 = g_pmk.pmk05 AND pjfa03 = g_pml[p_ac].pml04
  #DISPLAY BY NAME  g_pml[p_ac].pml90
  #mark by zhangyu 151231---end
  #end add
...
FUNCTION t420_pml20_check(p_cmd)
   DEFINE p_cmd  LIKE type_file.chr1
   DEFINE l_tc_pjfb06 LIKE tc_pjfb_file.tc_pjfb06
   DEFINE l_sum LIKE tc_pjfb_file.tc_pjfb06
   DEFINE l_gec07  LIKE gec_file.gec07
   DEFINE l_gec04  LIKE gec_file.gec04
   ...
   IF NOT cl_null(g_pml[l_ac].pml20) THEN
      ...
      ELSE
         LET l_sum = l_sum + g_pml[l_ac].pml20-g_pml_t.pml20
      END IF
       #SELECT sum(nvl(tc_pjfb06,0)) INTO l_tc_pjfb06 FROM  tc_pjfb_file,tc_pjfa_file
       #                          WHERE tc_pjfb01 = tc_pjfa01
       #                            AND   tc_pjfa05 = tc_pjfb02
       #                            AND tc_pjfa05 = g_pmk.pmk05
       #                            AND tc_pjfb04 = g_pml[l_ac].pml04
       SELECT sum(nvl(pjfa05,0)) INTO l_tc_pjfb06 FROM  pjb_file,pjfa_file
                                 WHERE pjb02 = pjfa01
                                   AND pjb01 = g_pmk.pmk05
                                   AND ta_pjfa14 = g_pml[l_ac].pml90  #add by zhaoxiangb 160106
         ...
         SELECT gec04,gec07 INTO l_gec04,l_gec07 FROM gec_file WHERE gec01 = g_pmk.pmk21
         #IF l_gec07 = 'Y' THEN
         #   SELECT tc_pjfb09,tc_pjfb09 * g_pml[l_ac].pml20 INTO g_pml[l_ac].pmlud07,g_pml[l_ac].pmlud08
         #           FROM tc_pjfb_file,tc_pjfa_file
         #           WHERE tc_pjfa01 = tc_pjfb01
         #           AND   tc_pjfa05 = tc_pjfb02
         #           AND   tc_pjfa05 = g_pmk.pmk05
         #           AND   tc_pjfb04 = g_pml[l_ac].pml04
         #ELSE
         #   SELECT tc_pjfb08,tc_pjfb08 * g_pml[l_ac].pml20 INTO g_pml[l_ac].pmlud07,g_pml[l_ac].pmlud08
         #           FROM tc_pjfb_file,tc_pjfa_file
         #           WHERE tc_pjfa01 = tc_pjfb01
         #           AND   tc_pjfa05 = tc_pjfb02
         #           AND   tc_pjfa05 = g_pmk.pmk05
         #           AND   tc_pjfb04 = g_pml[l_ac].pml04
         #END IF
            IF g_pml[l_ac].pmlud08 IS NULL OR g_pml[l_ac].pmlud07 IS NULL THEN

            #SELECT ta_pjfa05,ta_pjfa05 * g_pml[l_ac].pml20 INTO g_pml[l_ac].pmlud08,g_pml[l_ac].pmlud10
            #       FROM pjb_file,pjfa_file
            #        WHERE pjb02 = pjfa01
            #       AND pjb01 = g_pmk.pmk05
            #       AND pjfa03 = g_pml[l_ac].pml04
            SELECT ta_pjfa04,ta_pjfa04 * g_pml[l_ac].pml20 INTO g_pml[l_ac].pmlud07,g_pml[l_ac].pmlud09
                    FROM pjb_file,pjfa_file
                    WHERE pjb02 = pjfa01
                                   AND pjb01 = g_pmk.pmk05
                                   AND pjfa03 = g_pml[l_ac].pml04
            #add by zhaoxiangb 160113   #只抓未税，含税依据单头数据和未税带出来
            IF l_gec07 = 'Y' THEN
               LET  g_pml[l_ac].pmlud08 = g_pml[l_ac].pmlud07 * (1+l_gec04*0.01)
            ELSE
               LET  g_pml[l_ac].pmlud08 = g_pml[l_ac].pmlud07
            END IF
            LET g_pml[l_ac].pmlud10 = g_pml[l_ac].pmlud08 * g_pml[l_ac].pml20
            ...
            END IF
         DISPLAY BY NAME g_pml[l_ac].pmlud07,g_pml[l_ac].pmlud08,g_pml[l_ac].pmlud09,g_pml[l_ac].pmlud10
   END IF
   #end add
   IF g_pml_t.pml20 IS NULL AND g_pml[l_ac].pml20 IS NOT NULL OR
      g_pml_t.pml20 IS NOT NULL AND g_pml[l_ac].pml20 IS NULL OR
...
#將prompt改開窗滑鼠選
          ...
          AFTER FIELD a
            IF l_chr NOT MATCHES '[12345]' THEN  #No.FUN-A20039 add 4
               NEXT FIELD a
            END IF
...
#FUN-C20006--mark--end--
 ...
 #FUN-A80016 --modify
               #    CALL p500("G",g_pmk.pmk01)
                    CALL p500("G",g_pmk.pmk01,"")    #FUN-A80016
                    LET g_wc2=NULL
                    SELECT pmk05 INTO g_pmk.pmk05 FROM pmk_file WHERE pmk01= g_pmk.pmk01
                    DISPLAY g_pmk.pmk05 TO pmk05
                    CALL t420_b_fill(g_wc2,' 1=1')       #FUN-B90101 add 第二個參數，服飾中母單身的條件
                    LET g_action_choice="detail"
                    IF cl_chk_act_auth() THEN
                       CALL t420_b()
...
#add by zhangyu 151231---str
#依项目编号整批生成单身
FUNCTION t420_p(p_pjb01)
DEFINE p_pjb01    LIKE pjb_file.pjb01
DEFINE l_ac,l_i,l_x,l_cnt   LIKE type_file.num5
DEFINE l_sql      STRING
DEFINE l_sum      LIKE type_file.num20_6
DEFINE l_chr      LIKE type_file.num5
DEFINE l_gec07 LIKE gec_file.gec07
DEFINE l_pml  RECORD LIKE pml_file.*,
       ...
       ta_pjfa04  LIKE pjfa_file.ta_pjfa04,
       ta_pjfa05  LIKE pjfa_file.ta_pjfa05,
       ta_pjfa06  LIKE pjfa_file.ta_pjfa06,
       ta_pjfa07  LIKE pjfa_file.ta_pjfa07,
       pjfaacti   LIKE pjfa_file.pjfaacti,
       ta_pjfa14  LIKE pjfa_file.ta_pjfa14
...
DEFINE   l_ima49    LIKE ima_file.ima49,
         ...
         g_date    DATE
   LET l_sql = "SELECT pjfa01,pjfa02,pjfa03,pjfa04,ima44,ima25,ima44_fac,pjfa05,pjfa06,",
               "ta_pjfa04,ta_pjfa05,ta_pjfa06,ta_pjfa07,",
               "       pjfaacti ,ta_pjfa14 FROM pjfa_file",
               " LEFT JOIN ima_file ON ima01=pjfa03 "  ,
               " WHERE pjfa01 IN (select pjb02 from pjb_file where pjb01='",p_pjb01,"')",
               " ORDER by pjfa02 "

   PREPARE t420_pjfap FROM l_sql
   DECLARE t420_pjfacl CURSOR FOR t420_pjfap
   IF SQLCA.sqlcode THEN CALL cl_err('P1:',SQLCA.sqlcode,1) RETURN  END IF
   SELECT gec07 INTO l_gec07 FROM gec_file WHERE gec01 = g_pmk.pmk21
    WHILE TRUE
        LET l_ac = 1
        FOREACH t420_pjfacl  INTO sr[l_ac].*
           LET l_ac = l_ac + 1
        ...
        FOR l_i = 1 TO l_x
           ...
           LET l_pml.pml04 =sr[l_i].pjfa03
           LET l_pml.pml041=sr[l_i].pjfa04
           LET l_pml.pml05=NULL
           LET l_pml.pml42 = '0'
           LET l_pml.pml25 =' '
           LET l_pml.pml07=sr[l_i].ima44
           LET l_pml.pml08=sr[l_i].ima25
           LET l_pml.pml09=sr[l_i].ima44_fac
           LET l_pml.pml12=p_pjb01
           LET l_pml.pml121=sr[l_i].pjfa01
           LET l_pml.pmlplant = g_plant
           LET l_pml.pmllegal = g_legal
           IF cl_null(l_pml.pml91) THEN LET l_pml.pml91 = 'N' END IF
           LET l_pml.pml49 = '1'
           LET l_pml.pml50 = '1'
           LET l_pml.pml54 = '2'
           LET l_pml.pml56 = '1'
           LET l_pml.pml92 = 'N'
           LET l_pml.pml20 = sr[l_i].pjfa05
           LET l_pml.pml90 = sr[l_i].ta_pjfa14
           LET l_pml.pmlud07 = sr[l_i].ta_pjfa04
           #LET l_pml.pmlud08 = sr[l_i].ta_pjfa05
           LET l_pml.pmlud09 = sr[l_i].ta_pjfa06
           #LET l_pml.pmlud10 = sr[l_i].ta_pjfa07
           #add by zhaoxiangb 160113
           IF l_gec07='Y' THEN
             LET l_pml.pmlud08 = l_pml.pmlud07 * (1+g_pmk.pmk43 * 0.01)
             LET l_pml.pmlud10 = l_pml.pmlud09 * (1+g_pmk.pmk43 * 0.01)
           ELSE
             LET l_pml.pmlud08 = l_pml.pmlud07
             LET l_pml.pmlud10 = l_pml.pmlud09
           END IF
           #end add
           LET l_pml.pml38 = 'Y'
           LET l_pml.pml11 = 'N'
           ...
           LET l_pml.pml192 = 'N'
           LET l_pml.pml91  = 'N'
           LET l_pml.pml21 = 0  #add by zhaoxiangb 160105
           LET l_pml.pml16 = 0  #add by zhaoxiangb 160107
           SELECT sum(nvl(pml20,0)) INTO l_sum FROM pml_file,pmk_file
                            WHERE pml01 = pmk01
                              AND pmk05 = p_pjb01
                              AND pml04 = sr[l_i].pjfa03
                              AND pml90 = sr[l_i].ta_pjfa14 #add by zhaoxiangb 160106
                              AND pmk18 <>'X'
           IF l_sum IS NULL THEN LET l_sum = 0 END IF #add by zhaoxiangb 160106
           LET l_pml.pml20 = sr[l_i].pjfa05  - l_sum
           IF  l_pml.pml20 < = 0 THEN
               CONTINUE FOR
           END IF
           ### 寫進 pml_file
           #mark by zhaoxiangb 160106
           #SELECT COUNT(*) INTO l_cnt FROM pml_file
           #WHERE pml01=l_pml.pml01 AND pml04=l_pml.pml04
           #IF l_cnt>0 THEN
           #   CONTINUE FOR
           #END IF
           # end mark
            LET l_chr = l_chr + 1
           LET l_pml.pml02 =l_chr
           #tianry add 160101
           LET l_pml.pml33=g_today
           SELECT ima49,ima491,ima50,ima48 INTO g_ima49,g_ima491,g_ima50,g_ima48
           FROM ima_file WHERE ima01 = l_pml.pml04
           LET l_pml.pml34=l_pml.pml33+g_ima49
           LET l_pml.pml35=l_pml.pml34+g_ima491
           LET l_mm=  g_ima50+g_ima49+g_ima48+g_ima491   #add by wangyan151021
           CALL s_aday(l_pml.pml35,1,l_mm) RETURNING l_pml.pmlud13
            CALL s_wkday(l_pml.pml35) RETURNING g_flag,g_date
            IF cl_null(g_date) THEN
            #   NEXT FIELD pml35
            ELSE
               LET l_pml.pml35 = g_date
            END IF
         #str---add by huanglf161230
          SELECT ROUND(l_pml.pmlud07,3) INTO l_pml.pmlud07 FROM dual
         #str---end by huanglf161230
            #tianry add
           INSERT INTO pml_file VALUES(l_pml.*)
           IF SQLCA.SQLCODE THEN
