...
DEFINE g_tot51   LIKE alz_file.alz09
  ...
  LET g_sql=" SELECT apa06,apa07,apa34f,apa35f,apa13,apa14,",
            ...
            "         num06,num6,num07,num7,num08,num8,num09,num9,",
            "         num010,num10,num011,num11,num012,num12,num013,num13,",
            "         num014,num14,num015,num15,num016,num16,num06,num17,",
            # mod  by lixwz 171229 e
            "        sum5,apa15,apa16,net1,net2,apa21,gen02,apa22,gem02,apa08,",
            "        apa00,apa01,apa02,alz12,alz07,apa11,pma02,apa36,apr02,apa44,",
            "        apa54,aag02,apa05,pmc03,pmy01,pmy02,apa41 ",
            "  FROM q191_tmp ",
   ...
   CASE tm.u
     ...
     LET g_sql = g_sql CLIPPED,"  SUM(apa34),SUM(apa35),SUM(amt_2),SUM(sum2),SUM(sum4),SUM(num01),SUM(num1),SUM(num02),",
         ...
         FOREACH q191_curs1 INTO g_alz_1[g_cnt].*
            ...
            END IF
          # add by lixwz 171229 s
          LET g_alz_1[g_cnt].amt17  = g_alz_1[g_cnt].amt6
          # add by lixwz 171229 e
	  LET g_cnt = g_cnt + 1
...
FUNCTION q191_set_title_1()
   ...
   FOREACH q191_curs INTO l_aly.*
      LET l_zl  = l_aly.aly03 USING '<<&','-',l_aly.aly04 USING '<<<',l_msg
      LET l_zl1 = l_aly.aly03 USING '<<&','-',l_aly.aly04 USING '<<<',l_msg1
   ...
   END FOREACH
   IF l_i2 > 1 THEN
      LET l_zl  = '>',l_aly.aly04 USING '<<<',l_msg
      LET l_zl1 = '>',l_aly.aly04 USING '<<<',l_msg1
      LET l_til= "amt17_1"
      LET l_til1= "bamt17_1"
      CALL cl_set_comp_visible(l_til,TRUE)
      CALL cl_set_comp_visible(l_til1,TRUE)
...
FUNCTION q191_get_datas()
   ...
   FOREACH aly_curs1 INTO g_aly[l_i].*
      ...
      LET l_str1 = g_aly[l_i].aly03 USING "<<&",'-',g_aly[l_i].aly04 USING "<<&",l_gae04_1
      CALL cl_set_comp_att_text(l_field1,l_str1)
      LET l_str2 = g_aly[l_i].aly03 USING "<<&",'-',g_aly[l_i].aly04 USING "<<&",l_gae04_2
   ...
   CALL cl_set_comp_visible(l_field1,TRUE)
   CALL cl_set_comp_visible(l_field2,TRUE)
   LET l_str1 = ">=",l_aly04 USING "<<&",l_gae04_1
   CALL cl_set_comp_att_text(l_field1,l_str1)
   LET l_str2 = ">=",l_aly04 USING "<<&",l_gae04_2
   CALL cl_set_comp_att_text(l_field2,l_str2)
  #FUN-C80102----mark---str---
  #IF tm.detail = 'Y' THEN
  ...
  DEFINE l_sql    STRING,
   ...
   CASE
      WHEN g_aza.aza19 = '1'            #取每月匯率
         ...
         IF tm.a='1' THEN
            LET l_sql=l_sql," AND n.azj02= YEAR(o.apa02)||MONTH(o.apa02) )"
         ELSE
            LET l_sql=l_sql," AND n.azj02= YEAR(o.alz12)||MONTH(o.alz12) )"
         END IF
         ...
         LET l_sql="UPDATE q191_tmp o",
                   ...
                   "                              WHERE azj01 =o.apa13 "
         IF tm.a='1' THEN
            LET l_sql=l_sql," AND azj02<= YEAR(o.apa02)||MONTH(o.apa02) )"
         ELSE
            LET l_sql=l_sql," AND azj02<= YEAR(o.alz12)||MONTH(o.alz12) )"
         END IF
         LET l_sql=l_sql,"  )",
         ...
         LET l_sql="UPDATE q191_tmp o",
                   "   SET o.net2=(",
                   "              SELECT azj041 FROM azj_file n,apa_file a",
                   "               WHERE n.azj01 = o.apa13 AND o.apa01=a.apa01 ",
                   "                 AND n.azj02= YEAR(a.apa64)||MONTH(a.apa64) )",
                   " WHERE o.apa13 <> '",g_aza.aza17,"'"
         ...
         LET l_sql="UPDATE q191_tmp o",
                   ...
                   "              SELECT azj041 FROM azj_file",
                   "              WHERE azj01 = o.apa13",
                   "                AND azj02 = (SELECT MAX(azj02) FROM azj_file,apa_file a ",
                   "                              WHERE azj01 =o.apa13 AND o.apa01=a.apa01 ",
                   "                                AND azj02<= YEAR(a.apa64)||MONTH(a.apa64) )",
                   "              )",
                   " WHERE o.apa13<>'",g_aza.aza17,"'"
   ...
   ELSE
      WHILE NOT cl_null(g_aly[i].aly04)
         LET l_sql="UPDATE q191_tmp SET num0",i USING "<<","=alz09,",
                   ...
                   "                    flag='1' ",
                   " WHERE CAST('",tm.edate,"' AS DATE)-CAST(alz07 AS DATE)<=",g_aly[i].aly04,
                   "   AND flag='0' "
      ...
      LET l_sql="UPDATE q191_tmp SET num0",i USING "<<","=alz09 ,",
                ...
                "                    flag='1' ",
                " WHERE CAST('",tm.edate,"' AS DATE)-CAST(alz07 AS DATE)>",g_aly[i-1].aly04,
                "   AND flag='0' "
