...
# Modify.........: No:FUN-H60008 17/06/22 By Mars      資料來源為MES拋轉判斷，增加條件aza139
# Modify.........: No:DEV-H70001 17/07/20 By Mars      增加SMES整合:委外回貨還原
# Modify.........: No:FUN-H60002 17/07/26 By pomelo    增加委外採購流程異動條碼庫存
# Modify.........: No.FUN-H70003 17/08/02 By Jessica   新增欄位sfp17預設值'N'
# Modify.........: No.FUN-H80003 17/08/21 By Mars      SMES整合:委外回貨還原 條件調整
# Modify.........: No.FUN-H70002 17/09/19 By Jessica   因應新目錄調整，針對二環境DB transaction無法一致，無法達到NWS環境下組出變更單xml的問題，調整相關程式
# Modify.........: No.FUN-I30003 18/03/13 By TSD.Harry 1.單據須為已確認才可做[EBO狀態查詢]
#                                                      2.單據須為已確認才顯示EBO拋轉碼
#                                                      3.增加非委外單據取消確認EBO拋轉邏輯
# Moidfy.........: No.FUN-I30005 18/03/26 By Jessica   若為MES拋轉，則單身資料不可新增及刪除
# Moidfy.........: No.DEV-I30008 18/03/29 By Jessica   增加委外採購收貨入庫/退貨單在已被EC端確認(sendtype='Y')後，仍可以執行取消確認的動作
       ...
       #@WHEN "確認還原"
         WHEN "undo_confirm"
            ...
            #FUN-BC0062 ---------End----------
               ...
               #IF NOT cl_null(l_rva35) AND g_prog = "apmt730" THEN  #rva35(MES單號)不為空  #FUN-H60008 mark
                IF NOT cl_null(l_rva35) AND g_prog = "apmt730" AND g_aza.aza139 = 'Y' THEN  #FUN-H60008 add
       ...
       #@WHEN "拋轉還原"
        ...
        #-----抓取eB-Online底稿區狀態資料
         IF g_aza.aza75 MATCHES '[Yy]' AND
              (g_prog='apmt720' OR g_prog='aict042' OR g_prog='apmt721' OR g_prog='aict043'
               OR g_prog='apmt722' OR g_prog='aict044' OR g_prog='apmt730' OR g_prog='apmt731') THEN                        
                #FUN-I30003 180313 By TSD.Harry------(S)
                #IF g_prog MATCHES 'apmt72[012]' AND g_rvu.rvuconf <> 'Y' THEN          #DEV-I30008 mark
                IF g_rvu.rvuconf <> 'Y' THEN                                            #DEV-I30008 add
                   #單據須已確認才可查EBO狀態
                   CONTINUE WHILE
                END IF 
                #FUN-I30003 180313 By TSD.Harry------(E)
           ...
           IF NOT cl_null(g_rvu.rvu03) THEN
               ...
               IF g_rvu.rvu00 ='3' AND NOT cl_null(g_rvu.rvu02) AND g_rvu.rvu08 = 'SUB' THEN
                  ...
                  IF NOT cl_null(l_rvu03) THEN 
                     IF YEAR(l_rvu03) <> YEAR(g_rvu.rvu03) OR MONTH(l_rvu03) <> MONTH(g_rvu.rvu03) THEN
    ...
    IF g_aza.aza75 MATCHES '[Yy]' AND
       ...
       #FUN-I30003 180313 By TSD.Harry------(E)
          ...
          ELSE
             ...
             END IF
          END IF
       END IF    #FUN-I30003 180313 By TSD.Harry
    END IF
   #FUN-D70075 add end-----
...
DEFINE l_rva35      LIKE rva_file.rva35    #FUN-I30005 add
...
#              END IF                    #No.FUN-A50054 Add   #FUN-A60035 ---MARK
        ...
        BEFORE INSERT
           ...
           #FUN-I30005 add str---
            #-->資料來源為MES拋轉不可新增
            SELECT rva35 INTO l_rva35 FROM rva_file WHERE rva01 = g_rvu.rvu02
            IF NOT cl_null(l_rva35) AND g_prog = "apmt730" AND g_aza.aza139 = 'Y' THEN 
...
FUNCTION t720_z()
  ...
  LET l_ebocode = NULL     #FUN-I30003 180313 By TSD.Harry
  #FUN-D70075 add str-------
  #-----判斷eB-Online底稿區資料是否已確認
   IF g_aza.aza75 MATCHES '[Yy]' AND
      ...
      IF g_rvu.rvu08='REG' OR g_rvu.rvu08='EXP' OR g_rvu.rvu08='CAP' OR g_rvu.rvu08='SUB'  THEN
         ...
         ##IF l_ebocode = "Y" THEN
         ##委外
         #IF l_ebocode = "Y" AND g_rvu.rvu08='SUB' THEN           
         ##FUN-I30003 180313 By TSD.Harry------(E)
...
#退貨方式為價格折讓，則不做異動
       ...
       ##刪除eB-Online底稿區資料
        ...
        #FUN-D70075 add end-----
         ...
         #FUN-G90010 add str---
         #取消扣帳barcode
         IF g_success='Y' AND (g_prog = 'apmt720' OR g_prog = 'apmt722' OR g_prog = 'apmt730') THEN   #FUN-H60002 add apmt730
         ...
         #取消barcode
         IF g_success='Y' AND (g_prog = 'apmt720' OR g_prog = 'apmt730') THEN   #FUN-H60002 add apmt730
         ...
         #DEV-H70001 add -str-
         IF g_success = 'Y' AND g_aza.aza139 MATCHES "[Yy]" AND g_prog = 'apmt730' THEN
            SELECT COUNT(1) INTO l_cnt
              FROM rva_file,rvb_file,pmm_file
             WHERE rva01 = rvb01 AND pmm01 = rvb04 AND rvb01 = g_rvu.rvu02
               AND rva35 IS NULL AND pmm54 IS NOT NULL
            #IF l_cnt = 0  THEN   #FUN-H80003 mark
            IF l_cnt > 0  THEN    #FUN-H80003 add
                 #CALL i720sub_smes('delete',g_rvu.rvu01)                                                         #FUN-H70002 mark
                 CALL s_wsel_cli('SMES',g_prog,'delete',g_rvu.rvu01,'','','','','','','') RETURNING g_success     #FUN-H70002 add
            END IF
         END IF
         #DEV-H70001 add -end-
...
FUNCTION t720_t_rvu14()
  ...
  IF g_rvu.rvu27 NOT MATCHES "[234]" THEN
    ...
    #IF cl_null(g_rvu.rvu27) OR g_rvu.rvu27 = '1' THEN   #TQC-E90005 mark
     IF (cl_null(g_rvu.rvu27) OR g_rvu.rvu27 = '1') AND NOT cl_null(g_rvu.rvu02) THEN  #TQC-E90005 add
        SELECT MAX(rvb22) INTO g_rvu.rvu15 FROM rvb_file
        WHERE rvb01=g_rvu.rvu02
     END IF 
    #MOD-E30130 mark end--
