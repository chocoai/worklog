...
# Modify.........:               14/09/18 By stevie 參照舊系統更改新系統程式
# Modify.........: 2016-09-01  by jianwei   （換貨訂單）不回寫客户产品档及客户产品价格
# Modify.........: 2016-11-07  by jianwei     不回寫客户产品档及客户产品价格
...
#  DEFINE l_oeb47   LIKE oeb_file.oeb47   #No.FUN-870007  #FUN-AB0061
   ...
   DEFINE l_tc_soa39 LIKE tc_soa_file.tc_soa39#by stevie 14/09/17
...
{   DECLARE t400sub_ware_cs1 CURSOR FOR
   ...
   FOREACH t400sub_ware_cs1 INTO l_oeb09
       IF NOT s_chk_ware(l_oeb09) THEN
          ...
          RETURN
       END IF
   END FOREACH}                       #mark by xiaoyan 15/03/06
   #No.FUN-AA0048  --End
...
#by stevie ---start--- 14/09/17
#---ADD:---單身單價為0或者是為空的時候，不允許審核！-----
 LET l_tc_soa39 =NULL
 SELECT tc_soa39 INTO l_tc_soa39 FROM tc_soa_file WHERE tc_soa00 = '0'
 IF cl_null(l_tc_soa39) THEN LET l_tc_soa39 = 'N' END IF
 IF l_tc_soa39 = 'N' THEN
  LET l_cnt=0
   SELECT COUNT(*) INTO l_cnt FROM oeb_file
       WHERE oeb01=l_oea.oea01  AND (oeb13=0  OR oeb13 IS NULL)
   IF l_cnt>0  THEN
       ERROR"單價為0或為空，不允許審核！"
       LET g_success = 'N'
       RETURN
   END IF
  END IF
#---ADD BY TENA 20110903  ---------
#by stevie ---end--- 14/09/17
...
FUNCTION t400sub_y_upd(p_oea01,p_action_choice,p_inTransaction)   #CHI-B80050
   ...
   DEFINE l_buf    LIKE oay_file.oayslip #by stevie 14/09/17
   DEFINE l_sql STRING#by stevie 14/09/17
...
#FUN-BC0071 -----------------END
        ...
        CALL t400sub_ins_oeh(l_oea.*) # add by lixwz180516
     END IF                           #FUN-BC0071 add
   END IF                             #FUN-BC0071 add

########add by beamon 150922 工单异动抛转中间表 start ######################
  IF g_success = 'Y' THEN
     CALL cs_ins_tcsob('axm30',l_oea.oea01,l_oea.oea01,'Y','SFIS','')
  END IF
########add by beamon 150922 工单异动抛转中间表 end ########################

  #by stevie ---start---  14/09/17
  IF g_success = 'Y' THEN
  	 LET l_buf = s_get_doc_no(l_oea.oea01)
  	 LET l_cnt = 0
  	 LET l_sql = " SELECT COUNT(*)  FROM gfa_file ",
                 " WHERE gfa01 = '1'   ",
                 "  AND gfa02 = '",l_buf CLIPPED,"'",
                 " AND gfa03 = 'apmt540' ",
                 " AND gfaacti = 'Y' ",
       "   AND  (ta_gfa01 LIKE  '%,",l_oea.oea03,"' OR ta_gfa01 like '",l_oea.oea03,",%' OR ta_gfa01 ='",l_oea.oea03,"' OR ta_gfa01 like '%,",l_oea.oea03,",%' ) "
       PREPARE t400_cnt_p1 FROM l_sql
       DECLARE t400_cnt_c1 CURSOR FOR t400_cnt_p1
       OPEN  t400_cnt_c1
       FETCH t400_cnt_c1 INTO l_cnt
       CLOSE t400_cnt_c1
     IF l_cnt >0 THEN
     	 CALL t400sub_exp_po(l_oea.oea01,'A','')
     END IF
  END IF
  #by stevie ---end--- 14/09/17
   IF g_success = 'Y' THEN
...
FUNCTION t400sub_y1(l_oea)
 ...
 DEFINE l_oeb   RECORD LIKE oeb_file.*
   ...
   FOREACH t400sub_y1_c INTO l_oeb.*
      ...
      If  l_oea.oea00 <> '2'  Then           #2016-09-01 by jianwei
#         CALL t400sub_bu3(l_oea.*,l_oeb.*) IF g_success = 'N' THEN RETURN END IF #更新產品客戶    #2016-11-07 不回写客户产品档
 #        CALL t400sub_bu4() IF g_success = 'N' THEN RETURN END IF #更新產品價格  #2016-11-07 不回写客户产品档
      End If
   END FOREACH
END FUNCTION
         ...
         IF g_oaz.oaz122 = '1'
            THEN # CALL cl_err('ccc','axm-104',1)           #MOD-780014 mark
                 ...
                 UPDATE oea_file SET oeahold=l_oea.oeahold
                        WHERE oea01=l_oea.oea01}#by stevie 14/09/17 注釋
   ...
   CALL cl_msg("bu3!")                #No.FUN-640248
      ...
      IF g_oaz.oaz44 = 'Y' THEN
         ...
         WHERE obk01 = l_oeb.oeb04
           ...
           AND obk03 = l_oea.oea11    #2017-05-24 by dengyu
         IF SQLCA.SQLERRD[3] = 0 THEN
...
FUNCTION t400sub_exp_po(p_oea01,p_tag,p_buf)   #TQC-730022
    ...
    IF p_tag = 'A' THEN  #由自動拋轉CALL 的
      ...
      SELECT * INTO l_gfa.*  FROM gfa_file
        WHERE gfa01 = '1'   #1:axmt410
          ...
          AND gfa03 = 'apmt540'
          AND gfaacti = 'Y'
      IF cl_null(l_gfa.gfa05) THEN
      	LET g_success = 'N'#by stevie 14/09/17
      	 RETURN
      END IF  #如果無設定單據自動化的資料就不再往下執行
    END IF
  ...
  #檢查是否都有設定主供應商
   ...
   #by stevie 14/09/17不能有多個供應商
   SELECT COUNT(DISTINCT ima54) INTO li_cnt
     FROM oeb_file,ima_file
    WHERE ima01 = oeb04
      AND oeb01 = l_oea.oea01
   IF li_cnt > 1 THEN
      CALL cl_err('不能有多個供應商!','!',0)
      LET g_success = 'N'
      RETURN
   END IF
   ...
   ELSE
     ...
     CALL cl_ui_locale("axmt400f")
      LET g_success = 'Y'#by stevie 14/09/17
   ...
   IF cl_null(tm.wc) THEN LET tm.wc = ' 1=1' END IF#by stevie 14/09/17
...
FUNCTION t400sub_ins_pmm(p_slip,p_ima54,l_oea)
  ...
  #LET l_pmm.pmm42 = l_oea.oea24 #匯率  #CHI-A50045 mark
   ...
   #by stevie 14/09/17
   LET l_pmm.ta_pmm13 = 'N'
   LET l_pmm.ta_pmm14 = l_oea.oea01
   LET l_pmm.ta_pmm15 = l_oea.oea10
   ...
   UPDATE oea_file SET ta_oea08 = 'Y',ta_oea09 = l_pmm.pmm01 WHERE oea01 = l_oea.oea01#by stevie 14/09/17
...
FUNCTION t400sub_ins_pmn_exp(l_pmm01,l_ima54,l_oea,l_wc)   #MOD-870269
 ...
 DECLARE t400sub_oeb_curs2 CURSOR FOR t400sub_oeb_pre2
   FOREACH t400sub_oeb_curs2 INTO l_oeb.*
         ...
         #by stevie 14/09/17
         UPDATE oeb_file SET ta_oeb04 = l_pmm01,ta_oeb05 = l_oeb03 WHERE oeb01 = l_oeb.oeb01
...
#         l_ima262    LIKE ima_file.ima262, #FUN-A20044
  ...
  DEFINE l_msg  STRING#by stevie 14/09/17
...
#  LET l_pmn.pmn98  = l_oeb.oeb1001  #MOD-C30368 mark
   ...
   #by stevie	 14/09/17
   IF l_pmn.pmn31 = 0 THEN
   	  LET l_msg =  "採購單未取到單價，請確認該料件是否核價!"
   	  CALL cl_err(l_msg,'!',1)
   	  LET g_success = 'N'
   END IF
...
#  LET l_pmn.pmn88t = cl_digcut(l_pmn.pmn31t*l_pmn.pmn20,t_azi04)   #CHI-B70039 mark
  ...
  # LET l_pmn.pmn73 = ' '           #No.FUN-870007
   ...
   LET l_pmn.ta_pmn03 = p_oeb01 #by stevie 14/09/17
   LET l_pmn.ta_pmn04 = p_oeb03 #by stevie 14/09/17
        ...
        END IF
         IF l_cnt = 1 THEN
            ...
            #CALL t400sub_ins_pmk(l_pmk01,l_oea.oea84)   #MOD-C90187 add #FUN-CC0082 Mark
             #CALL t400sub_ins_pmk(l_pmk01,l_oea.oea84,l_oea.oea95)       #FUN-CC0082 Add#by stevie 14/09/17 注釋
             CALL t400sub_ins_pmk(l_pmk01,l_oea.oea84,l_oea.oea95,p_oea01)#by stevie 14/09/17
#FUN-A80012 --modify
...
#FUNCTION t400sub_ins_pmk(p_pmk01,p_oea84,p_oea95)                             #FUN-CC0082 Add#by stevie 14/09/17注釋
FUNCTION t400sub_ins_pmk(p_pmk01,p_oea84,p_oea95,p_oea01)#by stevie 14/09/17
...
#DEFINE l_slip   LIKE type_file.chr5                               #MOD-C90187 mark
 ...
 DEFINE p_oea01  LIKE oea_file.oea01#by stevie 14/09/17
   ...
   #by stevie 14/09/17
   LET l_pmk.ta_pmk04 = p_oea01
   SELECT oea03,oea10 INTO l_pmk.ta_pmk05,l_pmk.ta_pmk06
     FROM oea_file
    WHERE oea01 = p_oea01
...
#         l_ima262    LIKE ima_file.ima262, #FUN-A20044
   ...
   #by stevie 14/09/17 ---start---
   DEFINE l_oeb13    LIKE oeb_file.oeb13
   DEFINE l_oea23    LIKE oea_file.oea23
   DEFINE l_azi03    LIKE azi_file.azi03
   DEFINE l_tc_tzp02 LIKE tc_tzp_file.tc_tzp02
   DEFINE l_tc_tzp03 LIKE tc_tzp_file.tc_tzp03
   #by stevie 14/09/17 ---end---
...
#FUN-B90101--add--end--
  ...
  #LET l_pml.pml91 = ' '  #FUN-980010 add 給初始值
   ...
   #by stevie 14/09/17  ---start---
   SELECT oea03,oea23,ta_oeb01,ta_oeb02,oeb13
     INTO l_pml.ta_pml08,l_oea23,l_pml.ta_pml09,l_pml.ta_pml10,l_oeb13
   ...
   LET l_tc_tzp02 = NULL
   LET l_tc_tzp03 = 0
   SELECT tc_tzp02,tc_tzp03 INTO l_tc_tzp02,l_tc_tzp03
     FROM tc_tzp_file
    WHERE tc_tzp01 = '1'
      AND tc_tzp02 = l_pml.ta_pml08
   IF NOT cl_null(l_tc_tzp02) THEN
      LET l_pml.pml31 = l_oeb13 /(1+l_tc_tzp03/100)  #減系數採購
      SELECT azi03 INTO l_azi03 FROM azi_file WHERE azi01 = l_oea23
      CALL cl_digcut(l_pml.pml31,l_azi03) RETURNING l_pml.pml31
   END IF


   SELECT ima39 INTO l_pml.pml40 FROM ima_file WHERE ima01 = l_pml.pml04
   #by stevie 14/09/17 ---end---
...
#要回寫每張訂單的己拋量和請購單號
   UPDATE oeb_file SET oeb27 = p_pmk01,
                       ta_oeb03 = l_pml.pml02,#by stevie 14/09/17
                       oeb28 = l_pml.pml20
                 WHERE oeb01 = p_oeb01
