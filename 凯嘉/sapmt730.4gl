...
# Modify.........: 20150305 wenny   在单身仓库录入时取消对MRP不可用仓不允许使用的管控
# Modify.........: 20151114 by wenny审核时判断单身项次中数量*单价=金额，方可审核

# Modify.........: 20160625 by jianwei   SCWIP修改为YFSCWIP
# Modify.........: 2017-03-20 by dengyu  程式审核时的金额差异控制在0.01元，金额四捨五入小數點后与原值的差异可以有0.01的差异     KF-2017030013
...
#                 rvuud11,rvuud12,rvuud13,rvuud14,rvuud15
        CONSTRUCT BY NAME g_wc ON              #螢幕上取單頭條件
                  ...
                  rvu08,rvu06,rvu07,rvu20,rvu09,rvu99,rvu117,rvu25,
                  ta_rvu02,ta_rvu03,ta_rvu04,ta_rvu05,rvuconf,
    ...
    ELSE
        CONSTRUCT BY NAME g_wc ON              #螢幕上取單頭條件
                 ...
                 #rvu08,rvu06,rvu07,rvu20,rvu09,rvu99,rvu117,rvu25,rvuconf,       #TQC-B60065
                  rvu08,rvu06,rvu07,rvu20,rvu09,rvu99,rvu117,rvu27,rvu25,
                  ta_rvu02,ta_rvu03,ta_rvu04,ta_rvu05,rvuconf, #TQC-B60065
                  rvucond,rvucont,rvuconu,rvu10,rvu11,rvu15,rvu13,rvu14,
   ...
   #FUN-B40031 End-----
       ...
       LET g_wc = g_wc CLIPPED,cl_get_extra_cond('rvuuser', 'rvugrup')
        ...
        CONSTRUCT g_wc2 ON rvv02,rvv05,rvv45,rvv46,qcl02,rvv47,rvv31,rvv031, #FUN-BC0104 add rvv45,rvv46,qcl02,rvv47
                          ...
                          rvv930,ta_rvv01,ta_rvv02,ta_rvv03,   #FUN-670051
                     ...
                     FROM s_rvv[1].rvv02, s_rvv[1].rvv05,
                          ...
                          s_rvv[1].rvv930,s_rvv[1].ta_rvv01,s_rvv[1].ta_rvv02,
                          s_rvv[1].ta_rvv03,   #FUN-670051
...
FUNCTION t720_menu()
   ...
   DEFINE l_num_cnt like type_file.num5
   ...
   WHILE TRUE
      ...
      CASE g_action_choice
         ...
         WHEN "controlg"
            CALL cl_cmdask()
         WHEN "callina4"
              IF cl_chk_act_auth() AND NOT cl_null(g_rvu.rvu01)  THEN
                 CALL callina4()
              END IF
       #@WHEN "轉報工單"
         ...
         WHEN "grupo_takes "
            IF cl_chk_act_auth() THEN
               IF NOT cl_null(g_rvu.rvu01) and g_rvu.rvuconf='Y'and g_rvu.ta_rvu02='Y' THEN
                  CALL r720(g_rvu.rvu01)
               ELSE
                  ERROR"只有集團內已經確認的單據才可以打印！"
               END IF
            END IF
       #@WHEN "折讓"
         WHEN "allowance"
         ...
         WHEN "confirm"
            IF cl_chk_act_auth() THEN
              ...
              #FUN-A60009 add str --------------------
               ...
               END IF
               #FUN-CB0087---add---end---
               #---start by wenny20151114-----数量*单价=金额才可以审核--------
               IF g_success = 'Y' THEN
                  CALL t720_price()
               END IF
               #----end by wenny20151114------数量*单价=金额才可以审核---------
               IF g_success = "Y" THEN
                  #CALL t720sub_y_upd(g_rvu.rvu01,g_argv1,g_argv2,g_argv3,g_chr,FALSE,'Y')    #MOD-C80049 mark
...
DEFINE l_gem02     LIKE  gem_file.gem02   #NO.FUN-960130
  ...
  IF NOT g_argv1 MATCHES '[56Z]' THEN                                     #No.FUN-940083
     DISPLAY BY NAME g_rvu.rvuoriu,g_rvu.rvuorig,
      ...
      g_rvu.rvucrat,g_rvu.ta_rvu02,g_rvu.ta_rvu03,g_rvu.ta_rvu04,g_rvu.ta_rvu05,      #TQC-9A0093
      g_rvu.rvuud01,g_rvu.rvuud02,g_rvu.rvuud03,g_rvu.rvuud04,
      g_rvu.rvuud05,g_rvu.rvuud06,g_rvu.rvuud07,g_rvu.rvuud08,
      g_rvu.rvuud09,g_rvu.rvuud10,g_rvu.rvuud11,g_rvu.rvuud12,
      g_rvu.rvuud13,g_rvu.rvuud14,g_rvu.rvuud15
...
#              END IF                    #No.FUN-A50054 Add   #FUN-A60035 ---MARK
        ...
        BEFORE INSERT
            ...
            LET g_rvv[l_ac].rvv89='N'
            LET g_rvv[l_ac].ta_rvv02 = 0      #FUN-940083
            CALL t720_set_entry_b(p_cmd)
            CALL t720_set_no_required(p_cmd)
            CALL cl_show_fld_cont()     #FUN-550037(smin)
...
#FUN-A60035 ---MARK END
        ...
        AFTER FIELD rvv05             #驗收項次
           IF NOT cl_null(g_rvu.rvu02) AND NOT cl_null(g_rvv[l_ac].rvv05) THEN
              ...
              LET g_rvv[l_ac].ta_rvv02 = g_rvv[l_ac].rvv17 #140716bm
           ...
           IF NOT cl_null(g_rvv[l_ac].rvv32) THEN
             ...
             END IF
              ...
              #FUN-B40098 End-----
               ...
               IF g_rvv[l_ac].rvv31[1,4] != 'MISC' AND g_rvu.rvu00!='2' THEN  #料號
                 ...
                 #IF sn1=1 AND g_rvv[l_ac].rvv32 != g_rvv_t.rvv32 THEN  #MOD-D30080 mark
                  IF sn1=1 THEN           #MOD-D30080 add
                   # IF NOT cl_null(g_rvv[l_ac].rvv32) OR g_rvv[l_ac].rvv32 != g_rvv_t.rvv32 THEN  #MOD-D30080 add         #zhushi by wenny20150305
                     IF NOT cl_null(g_rvv[l_ac].rvv32) AND g_rvv[l_ac].rvv32 != g_rvv_t.rvv32 THEN                          #add by wenny20150305
               ...
               END IF
               SELECT ta_imd20 INTO  g_rvv[l_ac].ta_rvv03 FROM imd_file WHERE imd01 =   g_rvv[l_ac].rvv32
               DISPLAY BY NAME  g_rvv[l_ac].ta_rvv03
           END IF
...
#MOD-B70231 -- end --
        ...
        AFTER FIELD rvv930
            ...
            ELSE
                ...
                DISPLAY BY NAME g_rvv[l_ac].gem02a
            END IF
        AFTER FIELD ta_rvv02   #140716bm
          IF g_rvv[l_ac].ta_rvv02 > g_rvv[l_ac].rvv17 THEN
             ERROR "不可大于入库数量！"
             NEXT FIELD ta_rvv02
          END IF
...
FUNCTION t720_b_move_to()
   ...
   LET g_rvv[l_ac].ta_rvv01    = b_rvv.ta_rvv01 #FUN-670051
   LET g_rvv[l_ac].ta_rvv02    = b_rvv.ta_rvv02
   LET g_rvv[l_ac].ta_rvv03    = b_rvv.ta_rvv03
...
DEFINE l_cnt LIKE type_file.num5
   ...
   LET b_rvv.ta_rvv01    = g_rvv[l_ac].ta_rvv01
   LET b_rvv.ta_rvv02    = g_rvv[l_ac].ta_rvv02
   LET b_rvv.ta_rvv03    = g_rvv[l_ac].ta_rvv03
  ...
  #程式�不再控管
    ...
    CALL t720sub_rvv39(b_rvv.rvv36,b_rvv.rvv39,b_rvv.rvv39t,g_rvu.rvu04,g_rvu.rvu01)  #MOD-BC0262 add
     ...
     #140719
    LET l_cnt = 0
    SELECT COUNT(*) INTO l_cnt FROM imz_file,ima_file
     WHERE imz01 = ima06
       AND ima01 = b_rvv.rvv31
       AND ta_imz01 = 'Y'
    IF l_cnt =0 THEN
       LET b_rvv.ta_rvv02= 0
       LET b_rvv.ta_rvv03=NULL
    ELSE
      LET l_tc_soa43 = NULL
      SELECT tc_soa43 INTO l_tc_soa43 FROM tc_soa_file WHERE tc_soa00 = '0'
      IF NOT cl_null(l_tc_soa43) THEN
       SELECT ta_imd20 INTO  b_rvv.ta_rvv03 FROM imd_file WHERE imd01 =   b_rvv.rvv32
      END IF
    END IF
...
DEFINE l_wc2    LIKE type_file.chr1000 #No.FUN-680136 VARCHAR(300)
    ...
    CONSTRUCT l_wc2 ON rvv02, rvv05, rvv45,rvv46,qcl02,rvv47,  #FUN-BC0104 add
                       rvv31, rvv031, rvv32, rvv33,ta_rvv01,ta_rvv02,ta_rvv03,
                  ...
                  FROM s_rvv[1].rvv02, s_rvv[1].rvv05,
                       ...
                       s_rvv[1].ta_rvv01,s_rvv[1].ta_rvv02,
                       s_rvv[1].ta_rvv03,
...
#FUN-A60035 ---MARK END
    ...
    IF g_prog = 'apmt720' OR g_prog = 'aict042' THEN  #MOD-D30083 add aict042
          ...
          "       rvv930,'',ta_rvv02,ta_rvv03,ta_rvv01, ",  #FUN-670051
          "       rvvud01,rvvud02,rvvud03,rvvud04,rvvud05,",
          "       rvvud06,rvvud07,rvvud08,rvvud09,rvvud10,",
          "       rvvud11,rvvud12,rvvud13,rvvud14,rvvud15 ",
#TQC-A60046 --Begin
...
#@    ON ACTION 查詢退料          # steven_yue add  20101208
    ...
    #@ON ACTION recall
      ON ACTION recall
         LET g_action_choice="recall"
         EXIT DIALOG
      ON ACTION grupo_takes
         LET g_action_choice="grupo_takes"
         EXIT DIALOG
    #@ON ACTION 作廢
      ON ACTION void
...
FUNCTION t720_y()
   DEFINE  l_a     LIKE type_file.chr1    #No.FUN-680136 VARCHAR(1)
   DEFINE  l_cmd   LIKE type_file.chr1000 #No.FUN-680136 VARCHAR(300)
   DEFINE  l_cnt,l_n1,l_cnt1   LIKE type_file.num5    #No.FUN-680136 SMALLINT
  ...
  DEFINE l_rva07   LIKE rva_file.rva07
  ...
  DEFINE l_sfa08   LIKE sfa_file.sfa08
   ...
   IF g_grup !='G1100' AND g_grup !='G3202'
   THEN
       LET l_cnt = 0
       SELECT COUNT(*) INTO l_cnt FROM rvv_file,imd_file
       WHERE rvv32 = imd01
             AND rvv32 !='WFWIP'
             AND rvv01 = g_rvu.rvu01
             AND imd16 != g_grup
       IF l_cnt >0 THEN
          LET g_msg = "不能過賬非本部門倉庫!"
          CALL cl_err(g_msg,'!',1)
          LET g_success = 'N'
          RETURN
       END IF
   ...
   #No.B561 010518 by linda 考慮免檢收貨call 入庫確認
   IF g_argv1='5' THEN
      SELECT ROWID,rvu_file.* INTO g_rvu_rowid,g_rvu.*
        FROM rvu_file
       WHERE rvu00 = '1'
         AND rvu02 = g_argv2
      IF SQLCA.SQLCODE THEN
#        CALL cl_err('sel rvu',SQLCA.SQLCODE,1) #No.FUN-660129
         CALL cl_err3("sel","rvu_file","","",SQLCA.sqlcode,"","sel rvu",1)  #No.FUN-660129
         RETURN
      END IF
   END IF
   #No.B561 end----
   ...
   LET l_cnt1=0
   SELECT COUNT(*) INTO l_cnt1 FROM rvv_file WHERE rvv01=g_rvu.rvu01 and rvv35=rvv86 and  rvv17<>rvv87
   IF l_cnt1>0 THEN
   ...
   END IF
    # by cjy 090724 控制防止計價數量與退貨數量不一致的情況
   IF  g_prog = 'apmt720' or g_prog = 'apmt730'THEN

    IF g_rvu.rvu01[5,13]<> g_rvu.rvu02[5,13] THEN
       CALL cl_err('','cpm-003',0)
      LET g_success = 'N'   #FUN-580113

        RETURN
   END IF
   END IF
    # by cjy 091202 控制防止收貨單與入庫單流水號一致的情況
...
#TQC-840039---begin---
 ...
 # by cjy 09/05/05 BUG處理不允許單頭供應商與單身不一致 start
  SELECT COUNT(*) INTO l_n1  FROM rvv_file,pmm_file
   WHERE rvv01=g_rvu.rvu01  AND rvv36=pmm01 AND rvv06 != pmm09
  IF cl_null(l_n1) THEN LET l_n1=0 END IF
  IF l_n1>0 THEN
     CALL cl_err(g_rvu.rvu01,'apm1017',1)
     RETURN
  END IF
  # by cjy 09/05/05 BUG處理不允許單頭供應商與單身不一致   end
...
　################# 委外入庫，RT0597,無訂單出貨單價必須爲0
  LET l_plant = g_rvu.rvu04
  LET l_dbs = NULL
  SELECT tc_szp03 INTO l_dbs FROM tc_szp_file WHERE tc_szp01 = l_plant
  IF  g_argv1='1' AND g_argv3='SUB' AND NOT cl_null(l_dbs) THEN
      LET l_rva07 = ''
      SELECT rva07 INTO l_rva07 FROM rva_file WHERE rva01 = g_rvu.rvu02
      IF NOT cl_null(l_rva07) THEN
            #IF g_rvu.rvu04 = 'RT0597' THEN LET l_dbs = 'shengrui' END IF
            LET l_sql = "SELECT oga09 FROM ",l_dbs CLIPPED,".oga_file WHERE oga01 ='",l_rva07 CLIPPED,"'"
            PREPARE oga09_p FROM l_sql
            DECLARE oga09_c CURSOR FOR oga09_p
            OPEN  oga09_c
            FETCH oga09_c INTO l_oga09
            CLOSE oga09_c

            IF l_oga09 = '3' THEN
               LET l_cnt = 0
               SELECT COUNT(*) INTO l_cnt FROM rvv_file
                WHERE (rvv39>0 OR rvv39t>0)
                  AND rvv01 = g_rvu.rvu01
               IF l_cnt >0 THEN
                  LET g_msg ="入庫單與送貨單價格信息不一致，請檢查!"
                  CALL cl_err(g_msg,'!',1)
                  LET g_success = 'N'
                 RETURN
               END IF
...
#bugno:5070........................................................
#  SELECT COUNT(*) INTO g_cnt FROM pmm_file,rvu_file,rvv_file
#     WHERE rvu01=rvv01 AND rvv36=pmm01 AND pmm04>g_rvu.rvu03
#  IF g_cnt>0 THEN
#     CALL cl_err('','apm-412',0) RETURN
#  END IF
#bug end...........................................................

   IF g_argv1 !='5' THEN    # g_argv1='5' 收貨免檢:驗收確認不再詢問
  ...
  #MOD-740519-end-mark
   ...
   OPEN t720_cl USING g_rvu_rowid
   IF STATUS THEN
      CALL cl_err("OPEN t720_cl:", STATUS, 1)
      CLOSE t720_cl
      ROLLBACK WORK
   ...
   ELSE
      ...
      IF g_rvu.rvu08 = 'TAP' OR g_rvu.rvu08 = 'TRI' THEN
        ...
        IF l_cnt > 0 THEN
            IF g_success='Y' THEN
               LET g_rvu.rvu20 = 'Y'
               LET g_rvu.rvu99 = l_rva99
               UPDATE rvu_file SET rvu99 = g_rvu.rvu99,
                                   rvu20 = g_rvu.rvu20
                WHERE rvu01 = g_rvu.rvu01
               IF STATUS OR sqlca.sqlerrd[3]=0 THEN
                  CALL cl_err3("upd","rvu_file",g_rvu.rvu01,"",SQLCA.sqlcode,"","update rvu99",1)
                  LET g_success='N'
               ELSE
                  DISPLAY BY NAME g_rvu.rvu99
                  DISPLAY BY NAME g_rvu.rvu20
      ...
      END IF
      #TQC-7C0146-end-add
      IF g_rvu.rvu00='1' AND g_rvu.rvu02 IS NOT NULL AND g_success='Y'
         AND g_qcz.qcz05='Y' THEN
         SELECT COUNT(*) INTO g_cnt FROM qcs_file
          WHERE qcs01=g_rvu.rvu02    #驗收單號
            AND qcs14='Y'            #確認碼
            AND qcs09<>'2'           #1.合格 2.退貨 3.特採
         IF g_cnt>0 THEN
            CALL t720_IQC_return2()	#將剩餘未入庫量轉為'驗退單'
         END IF
      END IF
      IF g_success='Y' THEN
         CALL t720_s1()
      END IF
      IF g_success = 'Y' THEN
         CALL t720_dj_s2()   #140716bm
      END IF

   END IF

   #121219 begin ################# by beamon 有送貨地址時自動産生對應雜收單
   IF g_success = 'Y' AND g_rvu.ta_rvu02 = 'Y' AND NOT cl_null(g_rvu.ta_rvu03) THEN
      CALL t730_ina(g_rvu.rvu01)
   END IF
   #121219 end ######################
   MESSAGE ""

 ####################################by beamon 130506 add 增加自動產生已過帳邊角料雜收單start#####################
  IF g_success = 'Y' THEN #1
   LET l_cnt=0
    SELECT COUNT(*) INTO l_cnt FROM rvv_file,sfb_file,ima_file
     WHERE rvv01=g_rvu.rvu01 and rvv18=sfb01 and sfb02='7' and rvv31=ima01  and ta_ima003='Y'

    IF cl_null(l_cnt) THEN LET l_cnt=0 END IF
    IF l_cnt>0 THEN #2

      LET l_cnt=0
        SELECT COUNT(*) INTO l_cnt FROM ina_file WHERE ina10 = g_rvu.rvu01 and inaconf <> 'X'
           AND ta_ina003='E'

      IF l_cnt>0 THEN #3
         CALL cl_err(g_rvu.rvu01,'csf024',0 )
         LET g_success='N'
       ELSE

       CALL gen_inainfo('SCI', g_rvu.rvu01,'E','4','', g_rvu.rvuuser,g_rvu.rvugrup,g_rvu.rvu03)
            RETURNING l_ina01  # by cjy 110120
       IF g_success = 'Y' THEN  #4
#         CALL gen_inbinfo(l_ina01, g_rvu.rvu01,'X03','SCWIP',g_rvu.rvu04)    #by jianwei 20160625
         CALL gen_inbinfo(l_ina01, g_rvu.rvu01,'X03','YFSCWIP',g_rvu.rvu04)  #by jianwei 20160625
         LET l_cnt=0
         SELECT COUNT(*) INTO l_cnt FROM inb_file WHERE inb01=l_ina01 #add by cathree 20140707  对未生成单身的单据做处理
         IF l_cnt=0 THEN
            LET g_msg="未生成对应边角料杂收单单身资料，请检查aimi100和aooi310中维护的资料"
            CALL cl_err(g_msg,'!',1)
            LET g_success = 'N'
         END IF
         IF g_success='Y' THEN
            CALL t370('4' ,l_ina01,'stock_post')
            LET g_rvu.ta_rvu05 = l_ina01
            UPDATE rvu_file SET ta_rvu05=g_rvu.ta_rvu05 WHERE rvu01=g_rvu.rvu01
...
###########################################################################end #####################################
   ...
   IF g_success = 'Y' AND g_rvu.rvu08 = 'SUB' THEN #更新在制
      LET l_cnt = 0
      SELECT COUNT(*) INTO l_cnt FROM tc_soa_file WHERE tc_soa04 = 'Y' AND tc_soa00 = '0'
      IF l_cnt >0 THEN
        DECLARE t730_make_c1 CURSOR FOR
        SELECT DISTINCT sfa27,sfa01,sfa08 FROM sfa_file,rvv_file
         WHERE sfa01 = rvv18
           AND rvv01 = g_rvu.rvu01
        FOREACH t730_make_c1 INTO l_sfa03,l_sfa01,l_sfa08
         IF NOT cs_upmake(l_sfa03,l_sfa01,1,0,g_today,l_sfa08) THEN
            LET g_success = 'N'
            EXIT FOREACH
         END IF
        END FOREACH
   ...
   #IF g_argv1 !='5' THEN   #No.B561 add  #MOD-740519 mark
     IF g_success = 'Y' THEN
        COMMIT WORK
        CALL cl_flow_notify(g_rvu.rvu01,'Y')
        LET g_rvu.rvuconf='Y'
        SELECT * INTO g_rvu.* FROM rvu_file WHERE rvu01 = g_rvu.rvu01
        CALL t720_show()
     ELSE
     ...
     END IF
   #END IF   #No.B561 add     #MOD-740519 mark

   IF g_rvu.rvu00='1' AND g_rvu.rvu02 IS NOT NULL AND g_success='Y' THEN
      CALL t720_IQC_return()	#將剩餘未入庫量轉為'驗退單'
   END IF

   IF g_argv1 !='5' THEN  #MOD-740519 add
   IF g_rvu.rvuconf = 'X' THEN
      LET g_chr = 'Y'
   ELSE
      LET g_chr = 'N'
...
DEFINE  l_ta_ima006 LIKE ima_file.ta_ima006,
        l_ta_ima007 LIKE ima_file.ta_ima007,
        l_chr  LIKE type_file.num5,
        l_ta_azf02  LIKE azf_file.ta_azf02
DEFINE  l_rvv31  LIKE rvv_file.rvv31
            ...
            LET l_sql =
             "SELECT  rvv02,ta_azf02,rvv17*(nvl(ta_ima004,0)-nvl(ima18,0) -nvl(ta_ima009,0)-nvl(ta_ima011,0) )/1000,rvv18,ima18,ta_ima004,rvv31 ",
             "  from rvv_file,ima_file,azf_file,sfb_file   ",
             " where rvv18=sfb01 and sfb02='7' and rvv01='",l_rva01,"' AND rvv31=ima01 ",       # steven_yue add ta_ima011 同時也減去中心料單重 20110320
           ...
           FOREACH inbinfo_curs2 INTO l_inb.inb03,l_inb.inb04,l_inb.inb09,l_inb.inb12,l_ima18,l_ta_ima004,l_rvv31
             ...
             LET l_inb.inb09= cl_digcut(l_inb.inb09,l_gfe03)
               ...
               ###處理同一料件產生兩種廢料之情況START by cjy 130529add  #########
               LET l_ta_ima006=''
               LET l_ta_ima007=0
               SELECT ta_ima006,ta_ima007  into  l_ta_ima006,l_ta_ima007  FROM ima_file where ima01=l_rvv31
               IF l_ta_ima007<0 OR l_ta_ima007>100 THEN
                 LET g_msg = "aimi100中第二廢料比率應該大于0，小于100，資料不符合，請聯繫工程更正!"
                 CALL cl_err(g_msg,'!',1)
                 LET g_success='N'
                 RETURN
               END IF
               LET l_chr=0
               SELECT ta_azf02 INTO l_ta_azf02 FROM azf_file
                where azf01=l_ta_ima006 and azf02='E'

               SELECT COUNT(*) INTO l_chr FROM ima_file WHERE ima01=l_ta_azf02

               IF l_chr = 0 THEN
                  LET l_inb.inb16 = l_inb.inb09
                  INSERT INTO inb_file VALUES(l_inb.*)
               ELSE
               	 LET l_inb09_2=l_inb.inb09
                 LET l_inb.inb09=l_inb.inb09 * (1-l_ta_ima007/100)
                 ...
                 IF SQLCA.sqlcode THEN
                    ...
                    RETURN
                 END IF
                 LET l_inb.inb03=l_inb.inb03 + 100
                 LET l_inb.inb04=l_ta_azf02

                 LET l_inb.inb09=l_inb09_2 * l_ta_ima007/100
...
FUNCTION t720_log_r()
  ...
  SELECT img09,img10,img26 INTO l_img09,l_img10,l_img26
    FROM img_file WHERE img01 = b_rvv.rvv31 AND img02 = b_rvv.rvv32
                    AND img03 = b_rvv.rvv33 AND img04 = b_rvv.rvv34
  IF SQLCA.sqlcode THEN
#    CALL cl_err('ckp#log',SQLCA.sqlcode,1) #No.FUN-660129
     CALL cl_err3("sel","img_file",b_rvv.rvv31,"",SQLCA.sqlcode,"","ckp#log",1) #No.FUN-660129
     LET g_success = 'N' RETURN
  END IF

     LET g_tlf.tlf01  = b_rvv.rvv31       #異動料件編號
     LET g_tlf.tlf020 = g_plant           #工廠別
     LET g_tlf.tlf02  = 50                #來源狀況
     LET g_tlf.tlf021=b_rvv.rvv32         #倉庫別
     LET g_tlf.tlf022=b_rvv.rvv33         #儲位別
     LET g_tlf.tlf023=b_rvv.rvv34         #批號
    ...
    #LET g_tlf.tlf03  = 60                #資料目的為倉庫
     ...
     LET g_tlf.tlf036=b_rvv.rvv01         #參考號碼(入庫單號)
     LET g_tlf.tlf037=b_rvv.rvv02
#--->異動數量
    LET g_tlf.tlf04=' '                  #工作站
    LET g_tlf.tlf05=' '                  #作業序號
    LET g_tlf.tlf06=b_rvv.rvv09          #日期
    LET g_tlf.tlf07=g_today              #異動資料產生日期
    LET g_tlf.tlf08=TIME                 #異動資料產生時:分:秒
    LET g_tlf.tlf09=g_user               #產生人
    LET g_tlf.tlf10=b_rvv.rvv17          #收料數量
    LET g_tlf.tlf11=b_rvv.rvv35          #收料單位
    LET g_tlf.tlf12=b_rvv.rvv35_fac      #收料/庫存轉換率
    LET g_tlf.tlf13='asfi511'            #異動命令代號
    LET g_tlf.tlf14= b_rvv.rvv26
    ...
    CALL s_imaQOH(b_rvv.rvv31)
         RETURNING g_tlf.tlf18           #異動後總庫存量
    LET g_tlf.tlf19= b_rvv.rvv06         #異動廠商/客戶編號
    LET g_tlf.tlf20= b_rvv.rvv34         #專案號碼
# add by raymon 1998/06/01 ------
    SELECT ima25       #庫存單位
      INTO g_ima25
      FROM ima_file WHERE ima01=b_rvv.rvv31 AND imaacti='Y'
    CALL s_umfchk(b_rvv.rvv31,b_rvv.rvv35,g_ima25)
...
#--------------------------------
    ...
    LET g_tlf.tlf62= b_rvv.rvv18
    LET g_tlf.tlf63= ' '
    LET g_tlf.tlf64 = b_rvv.rvv41        #手冊編號 no.A050
    LET g_tlf.tlf930 = b_rvv.rvv930  #FUN-670051
    CALL s_tlf(1,0)
END FUNCTION
...
#---------更新驗收單的相關欄位----------------------------------------
      ...
      SELECT rvb07 INTO g_qty FROM rvb_file
       WHERE rvb01=b_rvv.rvv04 AND rvb02=b_rvv.rvv05
      IF g_qty<(l_rvb30+l_rvb29) AND g_argv1 <>'2' THEN #BugNo:5547
      ...
      IF l_rvb39 = 'Y' THEN
          UPDATE rvb_file SET rvb29 = l_rvb29,         #退貨量
                             ...
                             #------------No:MOD-810078 modify
                              rvb31 = rvb33-l_rvb30 # 可入庫量   #No:MOD-5C0089 #CHI-6A0046 remark
                             #rvb31 = rvb33-l_rvb29-l_rvb30 # 可入庫量   #No:CHI-6A0046 add
                             #------------No:MOD-810078 end
           WHERE rvb01 = g_rvu.rvu02 AND rvb02 = b_rvv.rvv05
          IF SQLCA.sqlcode THEN
#            CALL cl_err('退 upd rvb29,rvb09,rvb31:',SQLCA.sqlcode,1) #No.FUN-660129
...
#            CALL cl_err('退 upd rvb29,rvb09,rvb31:',SQLCA.sqlcode,1) #No.FUN-660129
      ...
      END IF
       #MOD-4C0053
#-----更新採購單的相關欄位---------------------------------------
   IF b_rvv.rvv25='N' THEN
   #-- update 該工單全檢數量 -1998/04/02 ----
      IF NOT cl_null(b_rvv.rvv18) AND g_rvu.rvu08 = 'SUB' THEN
     ...
     SELECT SUM(rvv17) INTO l_rvv17     #計算此採購單對應之倉退量
      ...
      WHERE rvv37=b_rvv.rvv37 AND rvv36=b_rvv.rvv36
     ...
     IF SQLCA.SQLERRD[3]=0 OR SQLCA.SQLCODE THEN
        LET g_success = 'N'
#       CALL cl_err('驗退 pmn51,55:',SQLCA.sqlcode,1) #No.FUN-660129
        CALL cl_err3("upd","pmn_file",b_rvv.rvv36,b_rvv.rvv37,SQLCA.sqlcode,"","驗退 pmn51,55:",1)  #No.FUN-660129
        RETURN
     END IF
     CALL s_udpmn57(b_rvv.rvv36,b_rvv.rvv37) #更新超短交量 = 收貨量 - 訂購量
   END IF
END FUNCTION
...
FUNCTION t720_z()
   ...
   LET l_cnt=0
     SELECT COUNT(*)
       INTO l_cnt
       FROM ina_file
      WHERE ina10=g_rvu.rvu01
        AND inaconf<>'X'
   IF l_cnt>0 THEN
      ERROR "存在料件暫放雜發單，無法取消過賬！！！"
      RETURN
   END IF
  ...
  END IF
   ...
   IF g_rvu.ta_rvu02 = 'Y' AND NOT cl_null(g_rvu.ta_rvu03) AND NOT cl_null(g_rvu.ta_rvu04) THEN
     LET l_dbs = NULL
	 SELECT tc_szp03 INTO l_dbs FROM tc_szp_file WHERE tc_szp01 = g_rvu.ta_rvu03
	 IF cl_null(l_dbs) THEN
	    ERROR "不存在的營運中心!"
		RETURN
     END IF
	 LET l_cnt = 0
	 LET l_sql = "SELECT COUNT(*) FROM ",l_dbs CLIPPED,".ina_file ",
	             " WHERE ina01 = '",g_rvu.ta_rvu04 CLIPPED,"'",
				 "   AND inapost = 'Y' "
        PREPARE ina_z_p1 FROM l_sql
        DECLARE ina_z_c1 CURSOR FOR ina_z_p1
        OPEN  ina_z_c1
        FETCH ina_z_c1 INTO l_cnt
        CLOSE ina_z_c1
        IF cl_null(l_cnt) THEN LET l_cnt = 0 END IF
        IF l_cnt >0 THEN  #4
           LET g_msg = "對應雜收單已過賬，請檢查!"
           CALL cl_err(g_msg,'!',1)
           LET g_success = 'N'
           RETURN
        ELSE
            LET l_sql = " DELETE FROM ",l_dbs CLIPPED,".inb_file ",
                        "  WHERE inb01 ='",g_rvu.ta_rvu04,"' "
            PREPARE del_inb_pre FROM l_sql
            EXECUTE del_inb_pre
            IF SQLCA.SQLCODE THEN
              IF g_bgerr THEN
                 CALL s_errmsg("","","del inb",SQLCA.sqlcode,1)
              ELSE
                 CALL cl_err3("","","","",SQLCA.sqlcode,"","del inb",1)
              END IF
              LET g_success = 'N'
              RETURN
             END IF
             LET l_sql = " UPDATE ",l_dbs CLIPPED,".ina_file ",
                "    SET inaconf= 'X' ",
                "  WHERE ina01 ='",g_rvu.ta_rvu04,"' "
             PREPARE upd_ina_p1 FROM l_sql
             EXECUTE upd_ina_p1
             IF SQLCA.SQLCODE  THEN
               IF g_bgerr THEN
                 CALL s_errmsg("","","upd ina",SQLCA.sqlcode,1)
               ELSE
                 CALL cl_err3("","","","",SQLCA.sqlcode,"","upd ina",1)
               END IF
               LET g_success = 'N'
               RETURN
             END IF
...
###################130506邊角料雜收單### ##############
  IF NOT cl_null(g_rvu.ta_rvu05) THEN
	 LET l_cnt = 0
	 SELECT COUNT(*) INTO l_cnt FROM ina_file
	  WHERE ina01 =g_rvu.ta_rvu05
			AND inapost = 'Y'
    IF l_cnt >0 THEN  #4
       LET g_msg = "對應雜收單已過賬，請檢查!"
       CALL cl_err(g_msg,'!',1)
       LET g_success = 'N'
     ELSE
       DELETE FROM inb_file WHERE inb01 =g_rvu.ta_rvu05
       IF SQLCA.SQLCODE THEN
          IF g_bgerr THEN
             CALL s_errmsg("","","del inb",SQLCA.sqlcode,1)
          ELSE
          ...
          LET g_success = 'N'
        END IF
        UPDATE ina_file  SET inaconf= 'X'  WHERE ina01 =g_rvu.ta_rvu05
        IF SQLCA.SQLCODE  THEN
           IF g_bgerr THEN
             CALL s_errmsg("","","upd ina",SQLCA.sqlcode,1)
           ELSE
...
#######################################130506 end ##
   ...
   IF g_success = 'Y' AND g_rvu.rvu08 = 'SUB' THEN #更新在制
      ...
      SELECT COUNT(*) INTO l_cnt FROM tc_soa_file WHERE tc_soa04 = 'Y' AND tc_soa00 = '0'
      IF l_cnt >0 THEN
        DECLARE t730_make_c2 CURSOR FOR
        SELECT DISTINCT sfa27,sfa01,sfa08 FROM sfa_file,rvv_file
         WHERE sfa01 = rvv18
           AND rvv01 = g_rvu.rvu01
        FOREACH t730_make_c2 INTO l_sfa03,l_sfa01,l_sfa08
         IF NOT cs_upmake(l_sfa03,l_sfa01,1,0,g_today,l_sfa08) THEN
            LET g_success = 'N'
            EXIT FOREACH
         END IF
...
FUNCTION t720_s1()    #確認
  ...
  DEFINE l_msg    LIKE type_file.chr1000, #No.FUN-680136 VARCHAR(50)
         ...
         l_pmn32  LIKE pmn_file.pmn32,
         l_pmm04  LIKE pmm_file.pmm04
  DEFINE l_ta_pmn02 LIKE pmn_file.ta_pmn02  #雜項分類
   ...
   FOREACH t720_s1_c INTO b_rvv.*
      IF STATUS THEN
         EXIT FOREACH
      END IF

    ########121008 by beamon 客供設備單價清零########
     LET l_ta_pmn02 = ''
     SELECT ta_pmn02 INTO l_ta_pmn02 FROM pmn_file WHERE pmn01 = b_rvv.rvv36 and pmn02 =b_rvv.rvv37
     IF l_ta_pmn02 = 'ZK01' THEN
        UPDATE rvv_file SET rvv38=0,rvv38t=0,rvv39=0,rvv39t=0 WHERE rvv01 = b_rvv.rvv01 AND rvv02 = b_rvv.rvv02
     END IF
    #################################

      #-----No:FUN-5B0027-----
      SELECT COUNT(*) INTO g_cnt FROM img_file
       WHERE img01 = b_rvv.rvv31   #料號
         AND img02 = b_rvv.rvv32   #倉庫
         AND img03 = b_rvv.rvv33   #儲位
         AND img04 = b_rvv.rvv34   #批號
         AND img18 < g_rvu.rvu03
      IF g_cnt > 0 THEN    #大於有效日期
         call cl_err(b_rvv.rvv32,'aim-400',0)   #須修改
         LET g_success = 'N'
         RETURN
      END IF
      #-----No:FUN-5B0027 END-----
      ...
      #bugno:5070 By Carol ..............................
      LET l_pmm04 =''
      IF NOT cl_null(b_rvv.rvv36) THEN #BugNo:5806
         SELECT pmm04 INTO l_pmm04 FROM pmm_file
          WHERE pmm01 = b_rvv.rvv36
         IF STATUS OR l_pmm04 > g_rvu.rvu03 THEN
            #入庫日期不可小於採購單據日期
#           CALL cl_err(b_rvv.rvv36,'apm-412',1) #No.FUN-660129
            CALL cl_err3("sel","pmm_file",b_rvv.rvv36,"","apm-412","","",1)  #No.FUN-660129
            LET g_success = 'N'
            EXIT FOREACH
         END IF
      END IF
      ...
      IF g_rvu.rvu08='SUB' THEN
         SELECT pmn41,pmn43,pmn18,pmn32
          ...
          WHERE pmn01 = b_rvv.rvv36
            AND pmn02 = b_rvv.rvv37
         IF STATUS THEN
#           CALL cl_err('sel pmn_file',STATUS,1) #No.FUN-660129
            CALL cl_err3("sel","pmn_file",b_rvv.rvv36,b_rvv.rvv37,STATUS,"","sel pmn_file",1)  #No.FUN-660129
            LET g_success='N'
            RETURN
         END IF
         IF (NOT cl_null(l_pmn41) AND l_pmn43>0) OR   #W/O<>'' &製程序>0
            (NOT cl_null(l_pmn18) AND l_pmn32>0) THEN #RUN<>'' &製程序>0
            CALL t720_u_rvb('y')  #->更新驗收單單身檔
            CALL t720_u_pmn()     #->更新採購單單身檔
            CONTINUE FOREACH
         END IF
      END IF
      ## --------------------------------------------------

      IF b_rvv.rvv31 IS NULL THEN LET  b_rvv.rvv31 = ' ' END IF   #料
#     IF b_rvv.rvv32 IS NULL THEN LET  b_rvv.rvv32 = ' ' END IF   #倉
      IF cl_null(b_rvv.rvv32) AND b_rvv.rvv31[1,4] <> 'MISC' AND g_rvu.rvu00<>'2' THEN
         CALL cl_err(b_rvv.rvv31,'apm-259',1)
         LET g_success='N'
         RETURN
      END IF
      IF b_rvv.rvv33 IS NULL THEN LET  b_rvv.rvv33 = ' ' END IF   #儲
      IF b_rvv.rvv34 IS NULL THEN LET  b_rvv.rvv34 = ' ' END IF   #批
      MESSAGE '_s1() read no:',b_rvv.rvv02 USING '#####&','--> parts: ', b_rvv.rvv31
      CALL ui.Interface.refresh()
      IF cl_null(b_rvv.rvv04) AND g_rvu.rvu00='1' THEN
         CALL cl_err('receive number is empty!','',1)
         LET g_success='N'
         CONTINUE FOREACH
      END IF
#     IF b_rvv.rvv31[1,4] = 'MISC' THEN CONTINUE FOREACH END IF
      IF g_rvu.rvu00 !='1' AND b_rvv.rvv31[1,4]!='MISC' THEN
         SELECT ima25 INTO l_ima25 FROM ima_file
          WHERE ima01=b_rvv.rvv31
      END IF

      CASE
        WHEN g_rvu.rvu00='1'   # 異動類別:1.入庫
             IF b_rvv.rvv31[1,4]<>'MISC' THEN
                SELECT COUNT(*) INTO l_cnt FROM img_file
                 WHERE img01=b_rvv.rvv31 AND img02=b_rvv.rvv32
                   AND img03=b_rvv.rvv33 AND img04=b_rvv.rvv34
                IF l_cnt=0 THEN
                     CALL s_add_img(b_rvv.rvv31,b_rvv.rvv32,
                                    b_rvv.rvv33,b_rvv.rvv34,
                                    g_rvu.rvu01,b_rvv.rvv02,
                                    g_rvu.rvu03)
                END IF
                SELECT COUNT(*) INTO l_cnt FROM img_file
                 WHERE img01=b_rvv.rvv31 AND img02=b_rvv.rvv32
                   AND img03=b_rvv.rvv33 AND img04=b_rvv.rvv34
                IF l_cnt=0 THEN
                   LET l_msg=b_rvv.rvv31 CLIPPED,'-',b_rvv.rvv32 CLIPPED,
                             '-',b_rvv.rvv33 CLIPPED
             ...
             END IF
             CALL t720_t1()
        WHEN g_rvu.rvu00='2'   #2.驗退
             CALL t720_uh()
             CALL t720_log()
             #No.FUN-540027  --begin
             CALL t720_update_tlff()
             #No.FUN-540027  --end
        WHEN g_rvu.rvu00='3'   #3.倉退
             CALL t720_bu()
      END CASE
      IF g_success='N' THEN
          #TQC-620156...............begin
...
FUNCTION t720_t2()
  ...
  DECLARE t720_t2_c CURSOR FOR
   SELECT ROWID ,tlf_file.* FROM tlf_file
    WHERE tlf01=b_rvv.rvv31 AND tlf036=b_rvv.rvv01
      ...
      #AND (tlf13 = 'apmt150' OR tlf13 = 'asft6201' OR tlf13 = 'apmt230')   #MOD-960033#by stevie 150113 zhushi
  FOREACH t720_t2_c INTO g_rowid ,g_tlf.*
      ...
      CALL ui.Interface.refresh()
      #IF g_rva00 = '1' THEN#by stevie150113 zhushi
         SELECT pmn65 INTO l_pmn65 FROM pmn_file   #代買性質 1.一般採購 2.代買採購
            WHERE pmn01=b_rvv.rvv36 AND pmn02=b_rvv.rvv37
...
FUNCTION t720_t1()
   DEFINE l_k             LIKE type_file.num10,   #No.FUN-680136 INTEGER
          ...
          l_ima261        LIKE ima_file.ima261,   #不可用庫存數量
          l_ima262        LIKE ima_file.ima262,   #庫存可用數量
          ...
          l_rowid         LIKE type_file.chr18,   #No.FUN-680136 INT # saki 20070821 rowid chr18 -> num10
          l_rowid_qcz     LIKE type_file.num10,   #No.FUN-680136 INT # saki 20070821 rowid chr18 -> num10
          ...
          l_pmnrowid,l_imarowid LIKE type_file.chr18    #No.FUN-680136 INT
   ...
   IF b_rvv.rvv31[1,4] = 'MISC' THEN
      ...
      CALL t720_u_pmn()     #->更新採購單單身檔
      RETURN
   ELSE        ###b_rvv.rvv31[1,4] != 'MISC'
      SELECT pmm02 INTO l_pmm02 FROM pmm_file WHERE pmm01 = b_rvv.rvv36
      ...
      IF g_sma.sma12 = 'Y' THEN     #多倉管理
         ...
         #-----------------------------------------------------------------
         SELECT ima91,ima261,ima262 INTO l_ima91,l_ima261,l_ima262
           FROM ima_file
          WHERE ima01 = b_rvv.rvv31
         IF SQLCA.sqlcode THEN
#           CAll cl_err('Select ima91:',SQLCA.sqlcode,1) #No.FUN-660129
            CALL cl_err3("sel","pmm_file",b_rvv.rvv36,"",SQLCA.sqlcode,"","sel pmm:",1)  #No.FUN-660129
            LET g_success = 'N'
         END IF
         ...
         SELECT pmn07,pmn09,pmn44,pmn65 INTO l_pmn07,l_pmn09,l_pmn44,l_pmn65
          ...
          WHERE pmn01 = b_rvv.rvv36
            AND pmn02 = b_rvv.rvv37
         IF SQLCA.sqlcode THEN
#           CAll cl_err('Select pmn44:',SQLCA.sqlcode,1) #No.FUN-660129
            CALL cl_err3("sel","pmn_file",b_rvv.rvv36,b_rvv.rvv37,SQLCA.sqlcode,"","Select pmn44:",1) #No.FUN-660129
            LET g_success = 'N'
         END IF

         SELECT rvb10 INTO g_rvb10 FROM rvb_file
          WHERE rvb01 = b_rvv.rvv04
            AND rvb02 = b_rvv.rvv05
         IF SQLCA.sqlcode THEN
#           CALL cl_err('Select rvb10',SQLCA.sqlcode,1) #No.FUN-660129
            CALL cl_err3("sel","rvb_file",b_rvv.rvv04,b_rvv.rvv05,SQLCA.sqlcode,"","Select rvb10",1)  #No.FUN-660129
            LET g_success = 'N'
         END IF
         ...
         IF l_rvb39 = 'Y' THEN   #--檢驗否(Y/N) #no.7143
            IF l_rvb33 < b_rvv.rvv17 THEN
               IF g_qcz.qcz05 = 'N' THEN                           #No.8939
                  #入庫數量不可大於允收數量!
                  CALL cl_err(b_rvv.rvv31,'apm-282',1)
                  LET g_success='N'
               END IF                                              #No.8939
            END IF
         END IF

         IF b_rvv.rvv17=0 THEN     #入庫數量
            LET l_cost=0
         ELSE
            LET l_cost = ((l_ima91*(l_ima261+l_ima262))+(b_rvv.rvv17
                          * l_pmn44))/((l_ima261+l_ima262)+
                          (b_rvv.rvv17*b_rvv.rvv35_fac*l_pmn09))
         END IF
         ...
         UPDATE ima_file SET ima91 = l_cost
          WHERE ima01 = b_rvv.rvv31
         IF SQLCA.SQLCODE OR SQLCA.sqlerrd[3] = 0  THEN
#           CALL cl_err('Update ima91 fail:',SQLCA.sqlcode,1) #No.FUN-660129
            CALL cl_err3("upd","ima_file",b_rvv.rvv31,"",SQLCA.sqlcode,"","Update ima91 fail:",1)  #No.FUN-660129
            LET g_success = 'N'
         END IF
         ...
         #                              採  購           料件庫存
         #--------------------------------------------------------------
         SELECT ima25,ima39,ima91,ima44_fac,ima86,ima86_fac
           INTO l_ima25,l_ima39,l_ima91,l_ima44_fac,l_ima86,l_ima86_fac
           FROM ima_file
          WHERE ima01 = b_rvv.rvv31
         ...
         IF g_sma.sma25 = '1' THEN
            ...
            UPDATE imb_file SET imb118 = l_cost
             WHERE imb01 = b_rvv.rvv31
            IF SQLCA.sqlcode OR SQLCA.sqlerrd[3] = 0  THEN
               #CALL cl_err('Update imb118 fail-1:',SQLCA.sqlcode,1) #MOD-530510
#               CALL cl_err(b_rvv.rvv31,'apm-720',1)                 #MOD-530510 #No.FUN-660129
                CALL cl_err3("upd","imb_file",b_rvv.rvv31,"","apm-720","","",1) #No.FUN-660129
               LET g_success = 'N'
            END IF
         END IF

         IF g_sma.sma25 = '2' THEN
            LET l_cost = g_rvb10 / l_ima44_fac * l_ima86_fac
            IF cl_null(l_cost) THEN
               LET l_cost = 0
            END IF
            ...
            UPDATE imb_file SET imb118 = l_cost
             WHERE imb01 = b_rvv.rvv31
            IF SQLCA.sqlcode OR SQLCA.sqlerrd[3] = 0 THEN
               #CALL cl_err('Update imb118 fail-2:',SQLCA.sqlcode,1) #MOD-530510
#               CALL cl_err(b_rvv.rvv31,'apm-720',1)                 #MOD-530510 #No.FUN-660129
                CALL cl_err3("upd","imb_file",b_rvv.rvv31,"","apm-720","","",1) #No.FUN-660129
               LET g_success = 'N'
            END IF
         ...
         ## No:2370 modify 1998/07/09 ------------------
         SELECT pmn65 INTO l_pmn65
           FROM pmn_file
          WHERE pmn01 = b_rvv.rvv36 AND pmn02 = b_rvv.rvv37
         IF SQLCA.sqlcode THEN
#           CAll cl_err('Select pmn65:',SQLCA.sqlcode,1) #No.FUN-660129
            CALL cl_err3("sel","pmn_file",b_rvv.rvv36,b_rvv.rvv37,SQLCA.sqlcode,"","Select pmn65:",1) #No.FUN-660129
            LET g_success = 'N'
         END IF
         #### -----------------------------------------

         #---->更新庫存明細檔(img_file)--------------------------------------
         IF b_rvv.rvv32 IS NOT NULL AND l_pmn65='1' THEN
            #NO.3443 01/09/05 By Carol
            MESSAGE "update img_file ..."
            CALL ui.Interface.refresh()

            LET g_forupd_sql = " SELECT rowid FROM img_file ",
                               "  WHERE img01= ? ",
                               "    AND img02= ? ",
                               "    AND img03= ? ",
                               "    AND img04= ? ",
                               " FOR UPDATE NOWAIT "
            DECLARE img_lock_t1 CURSOR FROM g_forupd_sql

            OPEN img_lock_t1 USING b_rvv.rvv31,b_rvv.rvv32,b_rvv.rvv33,
                                   b_rvv.rvv34
            IF STATUS THEN
            ...
            FETCH img_lock_t1 INTO l_rowid
            IF STATUS THEN
            ...
            IF cl_null(b_rvv.rvv17) THEN
               LET b_rvv.rvv17 = 0
            END IF

            IF cl_null(b_rvv.rvv35_fac) THEN
               LET b_rvv.rvv35_fac = 1
            END IF

            CALL s_upimg(l_rowid,+1,b_rvv.rvv17*b_rvv.rvv35_fac,
                 g_rvu.rvu03,b_rvv.rvv31,b_rvv.rvv32,b_rvv.rvv33,
                 b_rvv.rvv34,b_rvv.rvv01,b_rvv.rvv02,l_pmn07,b_rvv.rvv17,
                 l_ima25,b_rvv.rvv35_fac,1,'','','','','','','','','','')
            IF STATUS OR g_success = 'N' THEN
               #CALL cl_err('t720_t_s_upimg():',SQLCA.sqlcode,1)
            ...
            IF STATUS THEN
#             CALL cl_err('SEL img:',STATUS,1) #No.FUN-660129
              CALL cl_err3("sel","img_file",b_rvv.rvv31,"",STATUS,"","SEL img:",1)  #No.FUN-660129
            END IF
            #4349

            IF l_img10+b_rvv.rvv17*b_rvv.rvv35_fac = 0 OR
               l_img10+b_rvv.rvv17*b_rvv.rvv35_fac IS NULL THEN
               LET l_img30_update = 0
               LET l_img31_update = 0
            ELSE
               LET l_img30_update = ((l_img10*l_img30)+(b_rvv.rvv17*l_pmn44))
                                    /(l_img10+b_rvv.rvv17*b_rvv.rvv35_fac)
               LET l_img31_update = (((l_img10*l_img30)+(b_rvv.rvv17*l_pmn44))
                                    /(l_img10+b_rvv.rvv17*b_rvv.rvv35_fac))*l_ima871
            END IF
            ...
            UPDATE img_file SET img30 = l_img30_update,
             ...
             WHERE img01 = b_rvv.rvv31
               ...
               AND img04 = b_rvv.rvv34
            IF SQLCA.SQLCODE OR SQLCA.sqlerrd[3]=0 THEN
#              CALL cl_err('Update img30 fail:',SQLCA.sqlcode,1) #No.FUN-660129
               CALL cl_err3("upd","img_file",b_rvv.rvv31,"",SQLCA.sqlcode,"","Update img30 fail:",1)  #No.FUN-660129
               LET g_success = 'N'
            END IF
         END IF
         #------>更新料件主檔 ----------------------------------------
         ...
         SELECT img23,img24 INTO g_img23,g_img24 FROM img_file
          WHERE img01 = b_rvv.rvv31
            AND img02 = b_rvv.rvv32
            AND img03 = b_rvv.rvv33
            AND img04 = b_rvv.rvv34
         IF SQLCA.sqlcode THEN
            SELECT ime05,ime06 INTO g_img23,g_img24 FROM ime_file
             WHERE ime01 = b_rvv.rvv32
               AND ime02 = b_rvv.rvv33
            IF SQLCA.sqlcode THEN
               SELECT imd11,imd12 INTO g_img23,g_img24 FROM imd_file
                WHERE imd01 = b_rvv.rvv32
               IF SQLCA.sqlcode THEN
#                 CALL cl_err('sel img23:',SQLCA.sqlcode,0) #No.FUN-660129
                  CALL cl_err3("sel","imd_file",b_rvv.rvv32,"",SQLCA.sqlcode,"","sel img23:",1)  #No.FUN-660129
               END IF
            END IF
         END IF

         CALL s_udima(b_rvv.rvv31,g_img23,g_img24,b_rvv.rvv17*b_rvv.rvv35_fac,
                      g_rvu.rvu03,1) RETURNING l_ss

         IF l_ss THEN
            #CALL cl_err('t720_t:s_udima:',l_ss,1) #No.+045 010409 by plum
      ...
      ELSE
         #------>單倉處理---------------------------------------------------------
         CALL s_udima(b_rvv.rvv31,'Y','Y',b_rvv.rvv17,g_rvu.rvu03,1) RETURNING l_ss
         IF l_ss THEN
   ...
   IF b_rvv.rvv31[1,4] != 'MISC' THEN
      LET l_imaqty = b_rvv.rvv17 *  l_pmn09
   END IF

   #--->委外更新工單及insert完工入庫檔----------------------
   IF g_rvu.rvu08='SUB' THEN
      ## No:2356 Modify 1998/07/09 (rvu00='1' 入庫)-------------------
      INITIALIZE g_sfb.* TO NULL
      SELECT * INTO g_sfb.* FROM sfb_file
       WHERE sfb01=b_rvv.rvv18   #單號
         AND sfb02=7                           #委外
      #================================================================
      #No.+151 010530 BY ANN CHEN
      #
      #IF g_sfb.sfb05=b_rvv.rvv31 #料件
      #   THEN
      #   UPDATE sfb_file SET sfb11=sfb11-b_rvv.rvv17   #數量(
      #    WHERE sfb01 = b_rvv.rvv18
      #   IF SQLCA.sqlcode OR SQLCA.sqlerrd[3]=0 THEN
      #      CALL cl_err('Update sfb',SQLCA.sqlcode,1)
      #      LET g_success = 'N'
      ...
      SELECT ima25,ima55 INTO l_ima25,l_ima55 FROM ima_file
       WHERE ima01 = b_rvv.rvv31
     # WHERE ima01 = l_sfb05
     #CALL s_umfchk(l_sfb05,b_rvv.rvv35,l_ima55)   #生產/入庫
...
#        CAll cl_err('Select pmn65:',SQLCA.sqlcode,1) #No.FUN-660129
      ...
      IF l_pmn65='1' THEN
        #BugNo:3337 01/09/06 mandy
        #BugNo:5475 01/09/06 Carol add l_factor的處理
        #UPDATE sfb_file SET sfb04='7', sfb09 = sfb09 + b_rvv.rvv17
         CALL t720_chk_sfb()      #add by chenyu --090112
         UPDATE sfb_file SET sfb04='7', sfb09 = sfb09 + (b_rvv.rvv17 * l_factor)
          WHERE sfb01 = b_rvv.rvv18
         IF SQLCA.sqlcode OR SQLCA.sqlerrd[3]=0 THEN
#           CALL cl_err('Update sfb',SQLCA.sqlcode,1)  #No.FUN-660129
            CALL cl_err3("upd","sfb_file",b_rvv.rvv18,"",SQLCA.sqlcode,"","Update sfb",1)  #No.FUN-660129
            LET g_success = 'N'
            RETURN
         ELSE
           #1999/03/05 modify by connie
           #CALL s_umfchk(b_rvv.rvv17,b_rvv.rvv35,l_ima25)
           #       RETURNING g_cnt,l_factor1
           #IF g_cnt THEN
           #   CALL cl_err('rvv35/ima25: ','abm-731',1)
           #   LET g_success ='N'
           # # LET l_factor1 = 1
         ...
         LET l_qty = b_rvv.rvv17 * l_factor
         ...
         INSERT INTO sfh_file(sfh01,sfh02,sfh03,sfh04,sfh05,
                       ...
                       VALUES(b_rvv.rvv18,g_rvu.rvu03,'2',l_sfb05,
                              b_rvv.rvv31,l_qty,l_ima55,b_rvv.rvv32,
                              b_rvv.rvv33,b_rvv.rvv34,' ',' ',g_rvu.rvu01,
                              b_rvv.rvv02,0,0,0,' ',' ',' ',g_plant,g_legal)
         IF SQLCA.sqlcode THEN
#           CALL cl_err('INSERT sfh_file :',SQLCA.sqlcode,1) #No.FUN-660129
            CALL cl_err3("ins","sfh_file",b_rvv.rvv18,"",SQLCA.sqlcode,"","INSERT sfh_file",1)  #No.FUN-660129
            LET g_success ='N'
         END IF
      ELSE
         #---代買入庫(update 已發料量)
         UPDATE sfa_file SET sfa06 = sfa06 + b_rvv.rvv17 * l_factor
          WHERE sfa01 = b_rvv.rvv18
            AND sfa03=b_rvv.rvv31
...
#           CALL cl_err('Update sfb',SQLCA.sqlcode,1)  #No.FUN-660129
         ...
         ELSE
            ...
            IF g_sma.sma115 = 'Y' AND l_ima906 MATCHES '[23]' THEN
               IF NOT cl_null(b_rvv.rvv83) THEN
                  IF NOT cl_null(b_rvv.rvv85) AND b_rvv.rvv85 <> 0 THEN
                     CALL t720_log_r_du('2',b_rvv.rvv83,b_rvv.rvv84,b_rvv.rvv85)
                   #---------------No:MOD-660046 modify
                    #LET g_success = 'N'
                      IF g_success='N' Then
                         RETURN
                      END IF
               ...
               #No.CHI-770019  --Begin
               IF l_ima906 <> '3' THEN
                  IF NOT cl_null(b_rvv.rvv80) THEN
                     IF NOT cl_null(b_rvv.rvv82) AND b_rvv.rvv82 <> 0 THEN
                      #---------------No:MOD-660046 modify
                       #CALL t720_log_r_du('2',b_rvv.rvv80,b_rvv.rvv81,b_rvv.rvv82)
                        CALL t720_log_r_du('1',b_rvv.rvv80,b_rvv.rvv81,b_rvv.rvv82)
                       #LET g_success = 'N'
                         IF g_success='N' Then
                            RETURN
                         END IF
...
#       SELECT pmn46 INTO l_pmn46 FROM pmn_file,rvb_file  #讀取製程序號
#        WHERE rvb01=b_rvv.rvv04 AND rvb02=b_rvv.rvv05
#          AND rvb04=pmn_file.pmn01 AND rvb03=pmn_file.pmn02
#       IF cl_null(l_pmn46) THEN LET g_success = 'N'  RETURN END IF
#       LET l_cmd = "asft700 '",b_rvv.rvv01,"' '",b_rvv.rvv02,"' 'a'" CLIPPED
#       CALL cl_cmdrun(l_cmd)
#
#       SELECT count(*) INTO g_cnt FROM shb_file   #確認報工單是否產生
#        WHERE shb14 = b_rvv.rvv01 AND shb15 = b_rvv.rvv02
...
#NO.3443 01/09/05 By Carol
    ...
    LET g_forupd_sql =
          " SELECT rowid FROM img_file ",
          "  WHERE img01= ? ",
          "    AND img02= ? ",
          "    AND img03= ? ",
          "    AND img04= ? ",
          " FOR UPDATE NOWAIT "
    DECLARE img_lock_bu CURSOR FROM g_forupd_sql

    OPEN img_lock_bu USING b_rvv.rvv31,b_rvv.rvv32,b_rvv.rvv33,b_rvv.rvv34
    IF STATUS THEN
       ...
       RETURN
    END IF
    FETCH img_lock_bu INTO g_rowid
    IF STATUS THEN
       CALL cl_err('img_lock_bu fail',STATUS,1) LET g_success='N' RETURN
    END IF
#NO.3443 END .......................................

    SELECT ROWID,img21,img23,img24
      INTO g_rowid,g_img21,g_img23,g_img24 FROM img_file
     WHERE img01=b_rvv.rvv31 AND img02=b_rvv.rvv32
       AND img03=b_rvv.rvv33 AND img04=b_rvv.rvv34
## No:2392 modify 199/08/04 -----------
        IF STATUS THEN
#          CALL cl_err('sel img rowid',STATUS,1)  #No.FUN-660129
           CALL cl_err3("sel","img_file",b_rvv.rvv31,"",SQLCA.sqlcode,"","sel img rowid",1)  #No.FUN-660129
           LET g_success='N'
           RETURN
        END IF
####
...
#-----更新倉庫庫存明細資料
   CALL s_upimg(g_rowid,0,l_qty1_img,b_rvv.rvv09,
                '','','','','','','','','','','','','','',0,0,'','','','','')
  #IF g_success = 'N' THEN RETURN  END IF
   IF g_success = 'N' THEN
...
#------若庫存異動後其庫存量小於等於零時將該筆資料刪除
   CALL s_delimg(b_rvv.rvv31,b_rvv.rvv32,b_rvv.rvv33,b_rvv.rvv34)
#------更新庫存主檔之庫存數量 (單位為主檔之庫存單位)
  #IF s_udima(b_rvv.rvv31,g_img23,g_img24,l_qty1_ima,b_rvv.rvv05,0) THEN  #MOD-830212
   IF s_udima(b_rvv.rvv31,g_img23,g_img24,l_qty1_ima,g_rvu.rvu03,0) THEN  #MOD-830212
      RETURN
   END IF
   IF g_success = 'N' THEN
      RETURN
 ...
 #No.FUN-540027  --begin
 IF g_sma.sma115 = 'Y' AND b_rvv.rvv31[1,4]!='MISC' THEN
    CALL t720_update_imgg(0)
 END IF
 #No.FUN-540027  --end
#--------產生異動記錄
   IF b_rvv.rvv31[1,4]!='MISC' THEN  #No.9733
      CALL t720_log()
      #No.FUN-540027  --begin
      CALL t720_update_tlff()
      #No.FUN-540027  --end
   ...
   IF SQLCA.sqlcode THEN
      ...
      RETURN
   END IF
   IF b_rvv.rvv36 IS NOT NULL AND b_rvv.rvv36 !=' ' THEN
      SELECT pmn07 INTO l_pmn07 FROM pmn_file
      WHERE pmn01 = b_rvv.rvv36 AND pmn02 = b_rvv.rvv37
      IF SQLCA.sqlcode THEN
#        CALL cl_err('pmn07(purchase unit not found):',SQLCA.sqlcode,1)   #No.FUN-660129
   ...
   IF NOT cl_null(g_rvu.rvu02) THEN
      ...
      IF l_rvb39 = 'Y' THEN
          UPDATE rvb_file SET rvb29 = l_rvb29,         #驗退量
                              rvb30 = l_rvb30,         #入庫量
                              rvb09 = l_rvb30-l_rvb291,#允請量
                             #rvb31 = rvb33-l_rvb29-l_rvb30 # 可入庫量 #MOD-530018 MARK
                            #-----------No:MOD-810078 modify
                              rvb31 = rvb33-l_rvb30         # 可入庫量 #MOD-530018 add  #CHI-6A0046 remark
                             #rvb31 = rvb33-l_rvb29-l_rvb30 # 可入庫量 #CHI-6A0046 add
                            #-----------No:MOD-810078 end
           WHERE rvb01 = g_rvu.rvu02 AND rvb02 = b_rvv.rvv05
          IF SQLCA.sqlcode THEN
#            CALL cl_err('退 upd rvb29,rvb09,rvb31:',SQLCA.sqlcode,1) #No.FUN-660129
...
#---------更新採購單退貨量
      IF NOT cl_null(b_rvv.rvv36) AND b_rvv.rvv25='N' THEN
         ...
         WHERE pmn01 = b_rvv.rvv36 AND pmn02 = b_rvv.rvv37
         IF SQLCA.sqlcode THEN
#           CALL cl_err('upd pmn55:',SQLCA.sqlcode,1)  #No.FUN-660129
...
FUNCTION t720_u_rvb(p_cmd) #------------------------------------ Update rvb_file
 ...
 MESSAGE "upadte rvb_file.." #MOD-640296
      SELECT SUM(rvv17) INTO l_rvb30 FROM rvv_file,rvu_file     #計算已入庫量
       WHERE rvv04=b_rvv.rvv04 AND rvv05=b_rvv.rvv05
             AND rvuconf='Y' AND rvu00='1' AND rvv01=rvu01
      IF cl_null(l_rvb30) THEN LET l_rvb30=0 END IF
   #----
      SELECT SUM(rvv17) INTO l_rvb29 FROM rvv_file,rvu_file     #計算退(只算
       WHERE rvv04=b_rvv.rvv04 AND rvv05=b_rvv.rvv05            #驗退)
             AND rvuconf='Y' AND rvu00='2' AND rvv01=rvu01
      IF cl_null(l_rvb29) THEN LET l_rvb29=0 END IF
   #----
      SELECT rvb07 INTO g_qty FROM rvb_file
...
#            CALL cl_err('upd rvb29,rvb09,rvb31:',SQLCA.sqlcode,1) #No.FUN-660129
#            CALL cl_err3("upd","rvb_file",g_rvu.rvu02,b_rvv.rvv05,SQLCA.sqlcode,"","upd rvb29,rvb09,rvb31:",1)  #No.FUN-660129  #MOD-670098 mark
             CALL cl_err3("upd","rvb_file",g_rvu.rvu02,b_rvv.rvv05,9050,"","upd rvb29,rvb09,rvb31:",1)  #MOD-670098  add
             LET g_success = 'N' RETURN
          END IF
      ELSE
         UPDATE rvb_file SET rvb29 = l_rvb29,         #退貨量
          ...
          IF SQLCA.sqlcode OR SQLCA.sqlerrd[3]=0 THEN  #MOD-670098 add sqlerrd[3]
#            CALL cl_err('upd rvb29,rvb09,rvb31:',SQLCA.sqlcode,1) #No.FUN-660129
#            CALL cl_err3("upd","rvb_file",g_rvu.rvu02,b_rvv.rvv05,SQLCA.sqlcode,"","upd rvb29,rvb09,rvb31:",1)  #No.FUN-660129 #MOD-670098
             CALL cl_err3("upd","rvb_file",g_rvu.rvu02,b_rvv.rvv05,9050,"","upd rvb29,rvb09,rvb31:",1)  #MOD-670098
             LET g_success = 'N' RETURN
          END IF
      END IF
   MESSAGE "" #MOD-640296
...
#-->更新採購單單身檔 ----------------------------------------------
   #No.9772
   IF cl_null(b_rvv.rvv36) OR cl_null(b_rvv.rvv37) THEN
      RETURN
   END IF
   #No.9772(end)
   IF b_rvv.rvv25='N' THEN
     ...
     IF SQLCA.sqlcode THEN
         LET g_success='N'
#        CALL cl_err('upd pmn53,51 s2:',SQLCA.sqlcode,1) #No.FUN-660129
         CALL cl_err3("upd","pmn_file",b_rvv.rvv36,"",SQLCA.sqlcode,"","upd pmn53,51 s2:",1)  #No.FUN-660129
         RETURN
     END IF
   END IF
END FUNCTION
...
FUNCTION t720_log()
  ...
  IF b_rvv.rvv31[1,4]!='MISC' AND g_rvu.rvu00!='2' THEN
     SELECT img09,img10,img26 INTO l_img09,l_img10,l_img26
       FROM img_file WHERE img01 = b_rvv.rvv31 AND img02 = b_rvv.rvv32
                       AND img03 = b_rvv.rvv33 AND img04 = b_rvv.rvv34
     IF SQLCA.sqlcode THEN
#       CALL cl_err('ckp#log',SQLCA.sqlcode,1) #No.FUN-660129
  ...
  IF g_rvu.rvu00 = '1' OR g_rvu.rvu00='2' THEN      #入庫
     IF g_rvu.rvu08='SUB' THEN
  ...
  ELSE
     ...
     LET g_tlf.tlf022 = b_rvv.rvv33          #儲位別
     LET g_tlf.tlf023 = b_rvv.rvv34          #批號
     LET g_tlf.tlf024 = g_img10              #異動後庫存數量
     LET g_tlf.tlf025 = l_ima25
  END IF
  IF g_rvu.rvu00='1' THEN
     LET g_tlf.tlf026 = b_rvv.rvv04          #單據號碼(驗收單號)
     LET g_tlf.tlf027 = b_rvv.rvv05          #單據項次(驗收序號)
  ELSE
     LET g_tlf.tlf026 = b_rvv.rvv01
     LET g_tlf.tlf027 = b_rvv.rvv02
  END IF
  CASE
     WHEN g_rvu.rvu00='1'
          LET g_tlf.tlf03=50                   #資料目的為倉庫
     WHEN g_rvu.rvu00='2'  OR g_rvu.rvu00='3'
          LET g_tlf.tlf03=31
  END CASE
  IF g_rvu.rvu00='1' OR g_rvu.rvu00='2' THEN #入庫
     LET g_tlf.tlf031=b_rvv.rvv32         #倉庫別
     LET g_tlf.tlf032=b_rvv.rvv33         #儲位別
     LET g_tlf.tlf033=b_rvv.rvv34         #批號
  ELSE
       LET g_tlf.tlf031=' '                 #倉庫別
       LET g_tlf.tlf032=' '                 #儲位別
       LET g_tlf.tlf033=' '                 #批號
  END IF
    ...
    ELSE
       ...
       LET g_tlf.tlf036=b_rvv.rvv04         #參考號碼(驗收單號)
       LET g_tlf.tlf037=b_rvv.rvv05
    END IF
#--->異動數量
    LET g_tlf.tlf04=' '                  #工作站
    LET g_tlf.tlf05=' '                  #作業序號
    ...
    LET g_tlf.tlf12=b_rvv.rvv35_fac      #收料/庫存轉換率
    CASE
       WHEN g_rvu.rvu00='1' AND g_rvu.rvu08!='SUB'
...
#--------------------------------
    ...
    IF g_rvu.rvu08='SUB' THEN
       ...
       END IF
    ELSE
       LET g_tlf.tlf62= b_rvv.rvv36
       LET g_tlf.tlf63= b_rvv.rvv37
    END IF
    LET g_tlf.tlf64 = b_rvv.rvv41 #no.A050
    LET g_tlf.tlf930 = b_rvv.rvv930  #FUN-670051
    CALL s_tlf(1,0)
END FUNCTION
...
FUNCTION t720_IQC_return()
   DEFINE l_n,l_cnt    LIKE type_file.num5    #No.FUN-680136 SMALLINT
   DEFINE l_rvu        RECORD LIKE rvu_file.*
   DEFINE l_rvv        RECORD LIKE rvv_file.*
   DEFINE l_rvv17      LIKE rvv_file.rvv17
   DEFINE l_smy51,l_slip    LIKE oay_file.oayslip  #No.FUN-680136 VARCHAR(05)        #No.FUN-550060
   DEFINE l_smy57     LIKE type_file.chr1          #No.FUN-680136  VARCHAR(01)
   DEFINE l_ins_rvv_ok LIKE type_file.chr1         #No.FUN-680136 VARCHAR(01) #MOD-530023
   DEFINE l_ima906     LIKE ima_file.ima906
   DEFINE li_result    LIKE type_file.num5                  #No.FUN-560060  #No.FUN-680136 SMALLINT
   DEFINE l_rvb82         LIKE rvb_file.rvb82 #MOD-760028 add
   DEFINE l_rvb85         LIKE rvb_file.rvb85 #MOD-760028 add
   DEFINE l_rvb39         LIKE rvb_file.rvb39 #MOD-760028 add
   DEFINE l_okqty_rvv82   LIKE rvv_file.rvv82 #MOD-760028 add
   DEFINE l_okqty_rvv85   LIKE rvv_file.rvv85 #MOD-760028 add
   DEFINE l_qcs22         LIKE qcs_file.qcs22 #MOD-760028 add
   DEFINE l_qcs091        LIKE qcs_file.qcs091#MOD-760028 add
   DEFINE l_qcs32         LIKE qcs_file.qcs32 #MOD-760028 add
   DEFINE l_qcs35         LIKE qcs_file.qcs35 #MOD-760028 add
   DEFINE l_qcs38         LIKE qcs_file.qcs38 #MOD-760028 add
   DEFINE l_qcs41         LIKE qcs_file.qcs41 #MOD-760028 add
   ...
   SELECT rvb07,rvb30 INTO l_rvb07,l_rvb30 FROM rvb_file
    WHERE rvb01=g_rvu.rvu02 AND rvb02=1

   #MOD-530023 MARK
  #SELECT COUNT(*) INTO l_n FROM rvu_file
  # WHERE rvu02=g_rvu.rvu02 AND rvu00='2'
  #判斷此驗收單是否已產生驗退單
  #IF l_n>0 THEN RETURN END IF          # 如果已有驗退單


{
   SELECT COUNT(*) INTO l_n FROM rvb_file
       WHERE rvb01=g_rvu.rvu02 AND rvb07>rvb30 #實收-入庫
}
## No:2449 modify 1998/09/28 --------------------------------------
   SELECT COUNT(*) INTO l_n FROM rvb_file,rvv_file
    WHERE rvb01=g_rvu.rvu02
     #AND rvu01=g_rvu.rvu01  #異動單號 #No:7922
      AND rvv01=g_rvu.rvu01  #異動單號 #No:7922
   ...
   IF NOT cl_confirm('apm-270') THEN RETURN END IF
       BEGIN WORK
       LET l_rvu.*    =g_rvu.*
       LET l_rvu.rvu00='2'           #異動類別
       LET g_msg=g_rvu.rvu01[1,g_doc_len]
       LET l_slip=g_rvu.rvu02[1,g_doc_len]
       LET l_slip = s_get_doc_no(g_rvu.rvu02)       #No.FUN-550060
       SELECT smy51,smy57[3,3] INTO l_smy51,l_smy57
          FROM smy_file WHERE smyslip=l_slip
       IF STATUS OR cl_null(l_smy51) THEN
#         CALL cl_err(l_slip,'apm-274',1) #No.FUN-660129
          CALL cl_err3("sel","smy_file",l_slip,"","apm-274","","",1)  #No.FUN-660129
          ROLLBACK WORK
          RETURN
       END IF
       IF l_smy57='Y' THEN
#          LET l_rvu.rvu01=l_smy51,g_rvu.rvu01[4,10]   #入庫單號
           LET l_rvu.rvu01=l_smy51,g_rvu.rvu01[g_no_sp-1,g_no_ep]   #No.FUN-550060
          SELECT COUNT(*) INTO g_cnt FROM rvu_file WHERE rvu01=l_rvu.rvu01
          IF g_cnt > 0 OR
             MONTH(g_rvu.rvu03) <> MONTH(g_today)   #收貨月份不等於入庫月份時
             THEN
            #LET g_msg=g_rvu.rvu01[1,3] #No:5365
             LET g_msg=l_smy51
#No.FUN-560060--begin
             CALL s_auto_assign_no("apm",g_msg,g_today,g_chr,"rvu_file","rvu01","","","")
                  RETURNING li_result,l_rvu.rvu01
             IF (NOT li_result) THEN
                LET g_success ='N'
                RETURN
             END IF
#            CALL s_smyauno(g_msg,g_today) RETURNING g_i,l_rvu.rvu01
#            IF g_i THEN LET g_success='N' RETURN END IF
#No.FUN-560060--end
          END IF
       ELSE
          #No.B552 010517 BY ANN CHEN
          #LET g_msg=g_rvu.rvu01[1,3]
          LET g_msg=l_smy51
          #No.B552 END
#No.FUN-560060--begin
             CALL s_auto_assign_no("apm",g_msg,g_today,g_chr,"rvu_file","rvu01","","","")
...
#No.FUN-560060--end
       ...
       IF STATUS THEN
#         CALL cl_err('ins rvu:',STATUS,1) #No.FUN-660129
          CALL cl_err3("ins","rvu_file",l_rvu.rvu01,"",SQLCA.sqlcode,"","ins rvu:",1)  #No.FUN-660129
          ROLLBACK WORK RETURN
       END IF
       #----單身
...
## No:2449 modify 1998/09/28 --------------------------------------
       DECLARE t110_y1 CURSOR WITH HOLD FOR
          SELECT * FROM rvv_file WHERE rvv01=g_rvu.rvu01 AND rvv17>=0
       FOREACH t110_y1 INTO b_rvv.*
          LET l_rvv.*    =b_rvv.*
         ...
         #IF l_rvv17>0 THEN CONTINUE FOREACH END IF
          ...
          SELECT SUM(rvv17) INTO l_rvv17 FROM rvv_file,rvu_file   #防止重覆過帳
           WHERE rvv04=l_rvv.rvv04 #收貨單號
             AND rvv05=l_rvv.rvv05 #收貨項次
             AND rvu00='2'         #驗退
             AND rvuconf <> 'X'
             AND rvu01 = rvv01
          #MOD-7B0144-begin-modify
          #IF l_rvv17>0 THEN CONTINUE FOREACH END IF
           IF l_rvv17>0 THEN
              CALL cl_err(l_rvv.rvv05,'apm-111',1)    #TQC-7B0085調整add ,
              CONTINUE FOREACH
           END IF
          #MOD-7B0144-end-modify
          #MOD-530023(end)


          SELECT rvb07-rvb30 INTO l_rvv.rvv17 FROM rvb_file
           WHERE rvb01=b_rvv.rvv04 AND rvb02=b_rvv.rvv05
         ...
         #LET l_rvv.rvv85=0
          ...
          SELECT SUM(qcs22),SUM(qcs091),SUM(qcs32),
            ...
            INTO l_qcs22,l_qcs091,l_qcs32,l_qcs35,l_qcs38,l_qcs41
             FROM qcs_file WHERE  qcs01 = b_rvv.rvv04
             AND qcs02 = b_rvv.rvv05
             AND qcs14 = 'Y'                  #確認否
            #AND (qcs09 = '1' OR qcs09 = '3') #合格否     #MOD-830202
          ...
          IF l_rvb39='Y' THEN
             ...
             #若勾選允收數與IQC勾稽 , 驗退量以qcs_file為主
             #若不勾選允收數與IQC勾稽 , 驗退量以rvb_file為主
                LET l_rvv.rvv17 = l_qcs22 - l_qcs091
                IF g_sma.sma115 = 'Y' AND l_ima906 MATCHES '[23]' THEN
                   LET l_rvv.rvv82=l_qcs32-l_qcs38
                   LET l_rvv.rvv85=l_qcs35-l_qcs41
                END IF
             #MOD-830209-begin-add
             ELSE
                IF g_sma.sma115 = 'Y' AND l_ima906 MATCHES '[23]' THEN
                   LET l_rvv.rvv82=l_rvb82-l_okqty_rvv82
                   LET l_rvv.rvv85=l_rvb85-l_okqty_rvv85
                END IF
             END IF
             #MOD-830209-end-add
          ELSE
         ...
         #----------------No:MOD-760028 end
          ...
          LET l_rvv.rvv03='2'
          LET l_rvv.rvv32=NULL
          LET l_rvv.rvv33=NULL

         #check 批號是否為專案代號
          SELECT COUNT(*) INTO l_cnt FROM pja_file WHERE pja01=b_rvv.rvv34
          IF l_cnt=0 THEN LET l_rvv.rvv34=NULL  END IF

         #--------------No:TQC-6C0131 add
          #重新計算計價數量，因為此地方用的變數是l_rvv
          #所以無法直接使用t720_set_rvv87()
           LET l_rvv.rvv87 = t720_set_iqc_rvv87(l_rvv.*)
         #--------------No:TQC-6C0131 end
          ...
          #No.FUN-540027  --end
              ...
              SELECT azi04 INTO t_azi04  #No.CHI-6A0004
                FROM pmm_file,azi_file
               WHERE pmm22=azi01
                 AND pmm01 = b_rvv.rvv36 AND pmm18 <> 'X'
               #MOD-7A0180-begin-add
                IF l_rvv.rvv87 = 0 AND cl_null(t_azi04) THEN
                   SELECT azi04 INTO t_azi04
                     FROM pmc_file,azi_file
                    WHERE pmc22=azi01
                      AND pmc01 = g_rvu.rvu04
               ...
               #MOD-7A0180-end-add
              IF cl_null(t_azi04) THEN LET t_azi04=0 END IF  #No.CHI-6A0004
              CALL cl_digcut(l_rvv.rvv39,t_azi04)  #No.CHI-6A0004
                                RETURNING l_rvv.rvv39
              CALL cl_digcut(l_rvv.rvv39t,t_azi04)  #No.CHI-6A0004
                                RETURNING l_rvv.rvv39t   #No.FUN-610018
              #No.+090 end----
              #CHI-830033-begin-add
              CALL t720_rvv39(b_rvv.rvv36,l_rvv.rvv39,l_rvv.rvv39t)
                   RETURNING l_rvv.rvv39,l_rvv.rvv39t
              #CHI-830033-end-add
## No:2807 modify 1998/11/19 ---------------------------------
...
#            CALL cl_err('ins rvv:',STATUS,1) #No.FUN-660129
  ...
  # END CASE
    ...
    CASE g_prog
       WHEN "apmt720"
         LET g_cmd = "apmt721 ","'",g_rvu.rvu02 CLIPPED,"' '' '2'"
       WHEN "apmt730"
          LET g_cmd = "apmt731 ","'",g_rvu.rvu02 CLIPPED,"' '' '2' 'SUB'"
       WHEN "apmt740"
         LET g_cmd = "apmt741 ","'",g_rvu.rvu02 CLIPPED,"' '' '2'"
    END CASE
...
FUNCTION t720_IQC_return2()
   DEFINE l_n,l_cnt  LIKE type_file.num5    #No.FUN-680136 SMALLINT
   DEFINE l_rvu      RECORD LIKE rvu_file.*
   DEFINE l_rvv      RECORD LIKE rvv_file.*
   DEFINE l_rvv17    LIKE rvv_file.rvv17
   DEFINE l_qty      LIKE rvv_file.rvv17
#   DEFINE l_smy51    VARCHAR(03)
   DEFINE l_smy51    LIKE smy_file.smy51    #No.FUN-680136 VARCHAR(05)    #No.FUN-550060
   ...
   IF l_n=0 THEN RETURN END IF		       #如果全部入庫
       ...
       DECLARE t110_y2 CURSOR WITH HOLD FOR
          SELECT * FROM rvv_file WHERE rvv01=g_rvu.rvu01 AND rvv17>=0
       FOREACH t110_y2 INTO b_rvv.*
          LET l_rvv.*    =b_rvv.*
          LET l_rvv.rvv01=l_rvu.rvu01     #單號

          SELECT SUM(rvv17) INTO l_rvv17 FROM rvv_file
           WHERE rvv01=g_rvu.rvu01 AND rvv17>=0
             AND rvv04=l_rvv.rvv04
             AND rvv05=l_rvv.rvv05
          IF cl_null(l_rvv17) THEN LET l_rvv17=0 END IF
          ...
          CALL cl_getmsg('apm-284',g_lang) RETURNING g_msg
          LET g_msg=b_rvv.rvv31 CLIPPED,':',g_msg CLIPPED
            LET INT_FLAG = 0  ######add for prompt bug
          IF NOT cl_prompt(18,10,g_msg) THEN CONTINUE FOREACH END IF

#         IF NOT cl_confirm('apm-284') THEN RETURN END IF
         ...
         #check 批號是否為專案代號
          ...
          #No.FUN-540027   --begin
          #LET l_rvv.rvv39=l_rvv.rvv17*l_rvv.rvv38
          #LET l_rvv.rvv39t=l_rvv.rvv17*l_rvv.rvv38
          LET l_rvv.rvv39=l_rvv.rvv87*l_rvv.rvv38
...
DEFINE   l_ima903  LIKE ima_file.ima903   #MOD-D40048 add
   ...
   IF NOT g_before_input_done THEN
     #  CALL cl_set_comp_entry("rvv35,rvv25",TRUE)
       CALL cl_set_comp_entry("rvv35",TRUE)
...
FUNCTION t720_update_tlff()
  DEFINE l_ima25   LIKE ima_file.ima25,
   ...
   SELECT ima906,ima907,ima25 INTO g_ima906,g_ima907,l_ima25 FROM ima_file
    WHERE ima01 = b_rvv.rvv31
   IF SQLCA.sqlcode THEN
      IF b_rvv.rvv31 MATCHES 'MISC*' THEN
         RETURN
      ELSE
         LET g_success='N' RETURN
      END IF
   ...
   IF g_ima906 = '2' THEN  #子母單位
      IF NOT cl_null(b_rvv.rvv83) THEN
         IF NOT cl_null(b_rvv.rvv85) AND b_rvv.rvv85 <> 0 THEN
            CALL t720_tlff('2',b_rvv.rvv83,b_rvv.rvv84,b_rvv.rvv85)
            IF g_success='N' THEN RETURN END IF
         END IF
      END IF
      IF NOT cl_null(b_rvv.rvv80) THEN
         IF NOT cl_null(b_rvv.rvv82) AND b_rvv.rvv82 <> 0 THEN
            CALL t720_tlff('1',b_rvv.rvv80,b_rvv.rvv81,b_rvv.rvv82)
            IF g_success='N' THEN RETURN END IF
         END IF
      END IF
   END IF
...
#No.FUN-540027  --begin
FUNCTION t720_update_imgg(p_type)
  DEFINE l_ima25   LIKE ima_file.ima25,
   ...
   IF g_ima906 = '2' THEN  #子母單位
      IF NOT cl_null(b_rvv.rvv83) THEN
         CALL t720_upd_imgg('1',b_rvv.rvv31,b_rvv.rvv32,b_rvv.rvv33,b_rvv.rvv34,b_rvv.rvv83,b_rvv.rvv84,b_rvv.rvv85,p_type,'2')
         IF g_success='N' THEN RETURN END IF
      END IF
      IF NOT cl_null(b_rvv.rvv80) THEN
         CALL t720_upd_imgg('1',b_rvv.rvv31,b_rvv.rvv32,b_rvv.rvv33,b_rvv.rvv34,b_rvv.rvv80,b_rvv.rvv81,b_rvv.rvv82,p_type,'1')
         IF g_success='N' THEN RETURN END IF
      END IF
   END IF
   IF g_ima906 = '3' THEN  #參考單位
      IF NOT cl_null(b_rvv.rvv83) THEN
         CALL t720_upd_imgg('2',b_rvv.rvv31,b_rvv.rvv32,b_rvv.rvv33,b_rvv.rvv34,b_rvv.rvv83,b_rvv.rvv84,b_rvv.rvv85,p_type,'2')
         IF g_success='N' THEN RETURN END IF
      END IF
   END IF
...
#--------->扣發料資料(代買用)--------------------------------------------
FUNCTION t720_log_r_du(p_flag,p_unit,p_fac,p_qty)
  ...
  SELECT imgg10,imgg26 INTO l_imgg10,l_imgg26
    FROM imgg_file WHERE imgg01 = b_rvv.rvv31 AND imgg02 = b_rvv.rvv32
                     AND imgg03 = b_rvv.rvv33 AND imgg04 = b_rvv.rvv34
                     AND imgg09 = p_unit
  IF SQLCA.sqlcode THEN
#    CALL cl_err('ckp#log',SQLCA.sqlcode,1) #No.FUN-660129
     CALL cl_err3("sel","imgg_file",b_rvv.rvv31,"",SQLCA.sqlcode,"","ckp#log",1) #No.FUN-660129
     LET g_success = 'N' RETURN
  END IF

     LET g_tlff.tlff01  = b_rvv.rvv31       #異動料件編號
     LET g_tlff.tlff020 = g_plant           #工廠別
     LET g_tlff.tlff02  = 50                #來源狀況
     LET g_tlff.tlff021 =b_rvv.rvv32         #倉庫別
     LET g_tlff.tlff022 =b_rvv.rvv33         #儲位別
     LET g_tlff.tlff023 =b_rvv.rvv34         #批號
    ...
    #LET g_tlff.tlff03  = 60                #資料目的為倉庫
     ...
     LET g_tlff.tlff035 =l_ima25             #庫存單位(ima_file or imgg_file)
     LET g_tlff.tlff036 =b_rvv.rvv01         #參考號碼(入庫單號)
     LET g_tlff.tlff037 =b_rvv.rvv02
#--->異動數量
    LET g_tlff.tlff04=' '                  #工作站
    LET g_tlff.tlff05=' '                  #作業序號
    LET g_tlff.tlff06=b_rvv.rvv09          #日期
    ...
    LET g_tlff.tlff19= b_rvv.rvv06         #異動廠商/客戶編號
    LET g_tlff.tlff20= b_rvv.rvv34         #專案號碼
# add by raymon 1998/06/01 ------
    SELECT ima25,ima906,ima907       #庫存單位
      INTO g_ima25,g_ima906,g_ima907
      FROM ima_file WHERE ima01=b_rvv.rvv31 AND imaacti='Y'
    CALL s_umfchk(b_rvv.rvv31,p_unit,g_ima25)
         RETURNING l_flag,l_ima25_fac
    IF l_flag AND NOT (g_ima906='3' AND p_flag ='2') THEN
      ###Modify:98/11/15 ----單位換算率抓不到 -----#####
      CALL cl_err('rvv35/ima25: ','abm-731',1)
      LET g_success ='N'
      LET l_ima25_fac = 1.0
...
#--------------------------------
    ...
    LET g_tlff.tlff61= g_ima86             #成本單位
    LET g_tlff.tlff62= b_rvv.rvv18
    LET g_tlff.tlff63= ' '
    LET g_tlff.tlff64 = b_rvv.rvv41        #手冊編號 no.A050
    LET g_tlff.tlff930= b_rvv.rvv930   #FUN-670051
    IF cl_null(b_rvv.rvv85) OR b_rvv.rvv85 = 0 THEN
       CALL s_tlff(p_flag,NULL)
    ELSE
       CALL s_tlff(p_flag,b_rvv.rvv83)
    END IF
END FUNCTION
...
DEFINE l_tc_soa43 LIKE tc_soa_file.tc_soa43
   ...
   LET l_tc_soa43 = NULL
   SELECT tc_soa43 INTO l_tc_soa43 FROM tc_soa_file WHERE tc_soa00='0'
   IF cl_null(l_tc_soa43) THEN LET l_tc_soa43 = 'N' END IF
   #IF l_tc_soa43 ='N' THEN
   #  CALL cl_set_comp_visible("ta_rvv02,ta_rvv03",FALSE)
  # END IF
...
#用于att01~att10這個輸入型屬性欄位的AFTER FIELD事件的判斷函數
...
FUNCTION t720_upd_imgg(p_imgg00,p_imgg01,p_imgg02,p_imgg03,p_imgg04,
  ...
  DEFINE p_imgg00   LIKE imgg_file.imgg00,
         ...
         p_no       LIKE type_file.chr1,    #No.FUN-680136 VARCHAR(01)
         l_rowid    LIKE type_file.chr18,   #No.FUN-680136 INT # saki 20070821 rowid chr18 -> num10
         p_type     LIKE type_file.num10    #No.FUN-680136 INTEGER
    ...
    SELECT ima25,ima906,ima907 INTO l_ima25,g_ima906,g_ima907
    ...
    IF g_cnt = 1 AND NOT (g_ima906 = '3' AND p_no='2') THEN
       ...
       LET g_success = 'N' RETURN
    END IF
    CALL s_upimgg(l_rowid,p_type,p_imgg10,g_rvu.rvu02,
          '','','','','','','','','','',l_imgg21,'','','','','','','',p_imgg211,'','','','')
    IF g_success='N' THEN RETURN END IF
...
#--------->新增異動記錄--------------------------------------------------
FUNCTION t720_tlff(p_flag,p_unit,p_fac,p_qty)
  DEFINE p_unit       LIKE imgg_file.imgg09,
         p_flag       LIKE type_file.chr1,    #No.FUN-680136 VARCHAR(1)
         p_fac        LIKE imgg_file.imgg21,
         p_qty        LIKE imgg_file.imgg10,
  ...
  IF b_rvv.rvv31[1,4]!='MISC' AND g_rvu.rvu00!='2' THEN
     SELECT imgg10,imgg26 INTO l_imgg10,l_imgg26
       FROM imgg_file WHERE imgg01 = b_rvv.rvv31 AND imgg02 = b_rvv.rvv32
                        AND imgg03 = b_rvv.rvv33 AND imgg04 = b_rvv.rvv34
                        AND imgg09 = p_unit
     IF SQLCA.sqlcode AND NOT (g_ima906='3' AND p_flag = '1') THEN
  ...
  END IF
  IF g_rvu.rvu00='1' THEN
     LET g_tlff.tlff026 = b_rvv.rvv04          #單據號碼(驗收單號)
     LET g_tlff.tlff027 = b_rvv.rvv05          #單據項次(驗收序號)
  ELSE
     LET g_tlff.tlff026 = b_rvv.rvv01
     LET g_tlff.tlff027 = b_rvv.rvv02
  END IF
  CASE
     WHEN g_rvu.rvu00='1'
  ...
  END CASE
  IF g_rvu.rvu00='1' OR g_rvu.rvu00='2' THEN #入庫
     LET g_tlff.tlff031=b_rvv.rvv32         #倉庫別
     LET g_tlff.tlff032=b_rvv.rvv33         #儲位別
     LET g_tlff.tlff033=b_rvv.rvv34         #批號
  ELSE
       LET g_tlff.tlff031=' '                 #倉庫別
       LET g_tlff.tlff032=' '                 #儲位別
       LET g_tlff.tlff033=' '                 #批號
  ...
  ELSE
     ...
     LET g_tlff.tlff036=b_rvv.rvv04         #參考號碼(驗收單號)
     LET g_tlff.tlff037=b_rvv.rvv05
  END IF
#--->異動數量
    LET g_tlff.tlff04=' '                  #工作站
    LET g_tlff.tlff05=' '                  #作業序號
...
#--------------------------------
    ...
    IF g_rvu.rvu08='SUB' THEN
       ...
       END IF
    ELSE
       LET g_tlff.tlff62= b_rvv.rvv36
       LET g_tlff.tlff63= b_rvv.rvv37
    END IF
    LET g_tlff.tlff64 = b_rvv.rvv41 #no.A050
    LET g_tlff.tlff930=b_rvv.rvv930   #FUN-670051
    IF cl_null(b_rvv.rvv85) OR b_rvv.rvv85 = 0 THEN
       CALL s_tlff(p_flag,NULL)
    ELSE
...
#----------------------No:TQC-6C0131 add
FUNCTION t720_set_iqc_rvv87(p_rvv)
  DEFINE    l_item   LIKE img_file.img01,     #料號
...
#CHI-830033-begin-add
FUNCTION t720_rvv39(p_rvv36,p_rvv39,p_rvv39t)
...
#add by chenyu --090112--begin
FUNCTION t720_chk_sfb()
  ...
  IF l_sfb39 != '2' THEN
     IF g_rvu.rvu08 = 'SUB' AND l_sfb05 = b_rvv.rvv31 THEN
        IF l_sfb93 != 'Y' THEN
           CALL t720_minp() RETURNING l_min_set
            LET l_min_set=l_min_set+0.49
            LET  l_min_set=cl_digcut(l_min_set,0)     # by cjy 09/02/13

           SELECT ima55 INTO l_ima55 FROM ima_file
            WHERE ima01 = b_rvv.rvv31
           SELECT pmn07 INTO l_pmn07 FROM pmn_file
            WHERE pmn01 = b_rvv.rvv36 AND pmn02 = b_rvv.rvv37
           CALL s_umfchk(b_rvv.rvv31,l_pmn07,l_ima55)
                RETURNING l_i,l_fac
           IF l_i = 1 THEN
              CALL cl_err(b_rvv.rvv31,'apm-120',1)
              LET l_fac = 1
           END IF
           LET l_min_set = l_min_set / l_fac

           IF b_rvv.rvv17 > l_min_set THEN
              CALL cl_err(b_rvv.rvv17,'apm-722',1)
              LET g_success = 'N'
              RETURN
           END IF
        END IF
...
FUNCTION t720_minp()
   ...
   LET l_min_set_t=0
    ...
    SELECT sfb08 INTO l_sfb08 FROM sfb_file WHERE sfb01 = b_rvv.rvv18
    IF cl_null(l_sfb08) THEN LET l_sfb08 = 0 END IF

    LET l_sql = " SELECT * FROM sfa_file ",
                " WHERE sfa01 ='",b_rvv.rvv18 CLIPPED,"' ",
                "   AND sfa065 = 0 ",
                "   AND sfa11 <> 'E' ",
                "   AND sfa161 > 0 "
   ...
   RETURN l_min_set_t
END FUNCTION
#add by chenyu --090112--end

#CHI-7B0023/CHI-7B0039
FUNCTION t730_ina(p_rvu01)
DEFINE p_rvu01   LIKE rvu_file.rvu01
DEFINE l_cnt,li_result     LIKE type_file.num5
DEFINE l_rvu     RECORD LIKE rvu_file.*
DEFINE l_rvv     RECORD LIKE rvv_file.*
...
DEFINE l_dbs     LIKE azp_file.azp03
DEFINE l_sql     LIKE type_file.chr1000
DEFINE l_t,l_s1  LIKE smy_file.smyslip
DEFINE l_plant1  LIKE tc_szp_file.tc_szp02 #add by cathree 20140914

  SELECT * INTO l_rvu.* FROM rvu_file WHERE rvu01 = p_rvu01
  IF cl_null(g_rvu.ta_rvu03) THEN
     LET g_msg = "沒有送貨地點!"
     CALL cl_err(g_msg,'!',1)
     LET g_success = 'N'
     RETURN
  ...
  LET l_plant = g_rvu.ta_rvu03

  LET l_dbs = NULL
  SELECT tc_szp02,tc_szp03 INTO l_plant1,l_dbs FROM tc_szp_file WHERE tc_szp01 = l_plant
  IF cl_null(l_dbs) THEN
     LET g_msg = "沒有對應營運中心資料!"
     CALL cl_err(g_msg,'!',1)
     LET g_success = 'N'
     RETURN
  END IF
   ...
   SELECT tc_smz02 INTO l_s1 FROM tc_smz_file
    WHERE tc_smz01 = l_t
      AND tc_smz03 = 'apmt7301'
      AND tc_smz05 =l_plant1

   IF cl_null(l_s1) THEN
      SELECT tc_smz02 INTO l_s1 FROM tc_smz_file
       WHERE tc_smz01 = l_t
         AND tc_smz03 = 'apmt7301'
   END IF
   ...
   IF cl_null(l_s1) THEN
      LET g_msg = "沒有維護對應雜收單別，請維護csmi900，程序代號：apmt7301"
       CALL cl_err(g_msg,'!',1)
       LET g_success = 'N'
      RETURN
   END IF
  ...
  INITIALIZE l_ina.* TO NULL
  LET l_ina.ina00 = '3'
  LET l_ina.ina02 = l_rvu.rvu03
  LET l_ina.ina03 = l_rvu.rvu03
  LET l_ina.ina04 = l_rvu.rvugrup
  LET l_ina.ina08 = '1'

  LET l_ina.ina11   = l_rvu.rvuuser
  LET l_ina.ina12   = 'N'
  LET l_ina.inaprsw = 0
  LET l_ina.inaconf = 'Y'
  LET l_ina.inapost = 'N'
  LET l_ina.inapos = '1'
  LET l_ina.inaplant = l_plant1
  LET l_ina.inalegal = l_plant1
  LET l_ina.inauser = g_user
  LET l_ina.inagrup = g_grup
  LET l_ina.inadate = g_today
  LET l_ina.inamksg = 'N'

  LET l_ina.ta_ina005 = g_plant
  LET l_ina.ta_ina006 = l_rvu.rvu01
  LET l_ina.ta_ina007 = '2'
  LET l_ina.ta_ina008 = 'Y'


  CALL s_auto_assign_no("AIM",l_s1,l_ina.ina03,'1',"ina_file","ina01",l_dbs,"","")
    RETURNING li_result,l_ina.ina01

  LET l_sql = "INSERT INTO ",l_dbs CLIPPED,".ina_file",
                           "(ina00 ,ina01 ,ina02  ,ina03  ,ina04,",
                           " ina08 ,ina11,ina12 ,inaprsw ,inaconf ,inapost,inapos,inaplant,inalegal ,", #add by endy 150114 增加几个非空字段
                           " inauser ,inagrup ,inadate ,inamksg  ,ta_ina005 ,",
                           " ta_ina006 ,ta_ina007 ,ta_ina008) ",
                           " VALUES( ?,?,?,?,?, ?,?,?,?,?, ",
                           "         ?,?,?,?,?, ?,?,?,?,?, ?,?)"
                    PREPARE ins_ina FROM l_sql
                    EXECUTE ins_ina USING
                                 l_ina.ina00 ,l_ina.ina01 ,l_ina.ina02 ,l_ina.ina03 ,l_ina.ina04,
                                 l_ina.ina08 ,l_ina.ina11,l_ina.ina12 ,l_ina.inaprsw ,l_ina.inaconf ,
                                 l_ina.inapost,l_ina.inapos,l_ina.inaplant,l_ina.inalegal, #add by endy 150114 增加几个非空字段
                                 l_ina.inauser ,l_ina.inagrup ,l_ina.inadate ,l_ina.inamksg ,l_ina.ta_ina005,
                                 l_ina.ta_ina006 ,l_ina.ta_ina007 ,l_ina.ta_ina008
                    IF SQLCA.sqlcode<>0 THEN
                       LET g_success = 'N'
                       IF g_bgerr THEN
                          CALL s_errmsg("","","ins ina.",SQLCA.sqlcode,1)
                       ELSE
                          CALL cl_err3("","","","",SQLCA.sqlcode,"","ins ina.",1)
                       END IF
                       RETURN
                    END IF
   ...
   DECLARE t730_rvv_cs1 CURSOR FOR
   SELECT * FROM rvv_file WHERE rvv01 = p_rvu01 ORDER BY rvv02

   FOREACH t730_rvv_cs1 INTO l_rvv.*
      INITIALIZE l_inb.* TO NULL
      LET l_inb.inb01 = l_ina.ina01
      LET l_inb.inb03 = l_rvv.rvv02
      LET l_inb.inb04 = l_rvv.rvv31
      #LET l_inb.inb05= 'RSSCA'
      #LET l_inb.inb06= g_plant
	  LET l_sql = "SELECT ima35,ima36 FROM ",l_dbs CLIPPED,".ima_file ",
              " WHERE ima01 ='",l_inb.inb04 CLIPPED,"'"
      PREPARE ima_p1 FROM l_sql
      DECLARE ima_c1 CURSOR FOR ima_p1
      OPEN  ima_c1
      FETCH ima_c1 INTO l_inb.inb05,l_inb.inb06
      CLOSE ima_c1
      IF cl_null(l_inb.inb05) THEN
         LET g_msg = "請維護料件主要倉庫，料號：",l_inb.inb04
         CALL cl_err(g_msg,'!',1)
         LET g_success = 'N'
         RETURN
      END IF
      ...
      LET l_inb.inb07  = ' '
      LET l_inb.inb08 = l_rvv.rvv35
      LET l_inb.inb08_fac = l_rvv.rvv35_fac
      LET l_inb.inb09 = l_rvv.rvv17
      LET l_inb.inb13  = 0
      LET l_inb.inb14  = 0
      LET l_inb.inb15= 'D11'
      LET l_inb.inb902 = l_inb.inb08
      ...
      LET l_inb.inb904 = l_inb.inb09
      LET l_inb.inb930 = ' '
      LET l_inb.inbplant = l_plant1
      LET l_inb.inblegal = l_plant1

              LET l_sql = "INSERT INTO ",l_dbs CLIPPED,".inb_file",
                                 "(inb01 ,inb03 ,inb04,inb05,inb06,",
                                 " inb07 ,inb08 ,inb08_fac ,inb09 ,inb10 ,",
                                 " inb11 , inb12,inb13,inb14 ,inb15 ,",
                                 " inb901 ,inb902 ,inb903 ,inb904 ,inb905 ,",
                                 " inb906 ,inb907 ,inb930,inbplant,inblegal )", #add by endy 150114 增加几个非空字段
                                 " VALUES( ?,?,?,?,?, ?,?,?,?,?, ?,?, ",
                                 "         ?,?,?,?,?, ?,?,?,?,?, ?,?,?)"
                    PREPARE ins_inb FROM l_sql
                    EXECUTE ins_inb USING
                                 l_inb.inb01 ,l_inb.inb03 ,l_inb.inb04 ,l_inb.inb05,l_inb.inb06,
                                 l_inb.inb07 ,l_inb.inb08 ,l_inb.inb08_fac ,l_inb.inb09,l_inb.inb10,
                                 l_inb.inb11 ,l_inb.inb12 ,l_inb.inb13 , l_inb.inb14,l_inb.inb15,
                                 l_inb.inb901 ,l_inb.inb902 ,l_inb.inb903 ,l_inb.inb904 ,l_inb.inb905,
                                 l_inb.inb906 ,l_inb.inb907 ,l_inb.inb930,l_inb.inbplant,l_inb.inblegal #add by endy 150114 增加几个非空字段
                    IF SQLCA.sqlcode<>0 THEN
                       LET g_success = 'N'
                       IF g_bgerr THEN
                          CALL s_errmsg("","","ins inb.",SQLCA.sqlcode,1)
                       ELSE
                          CALL cl_err3("","","","",SQLCA.sqlcode,"","ins inb.",1)
                       END IF
                       RETURN
                    END IF
   ...
   IF g_success = 'Y' THEN
      UPDATE rvu_file SET ta_rvu04 = l_ina.ina01 WHERE rvu01 = p_rvu01
   END IF
END FUNCTION
...
FUNCTION callina4()
  ...
  SELECT ina01 INTO l_ina01 FROM ina_file
   WHERE ina10 = g_rvu.rvu01  and ina00='4' and inaconf<>'X'
     AND ina01 LIKE 'SCI%'          # add by steven_yue 20110320
  IF SQLCA.sqlcode THEN
     IF SQLCA.sqlerrd[3]=0 THEN

        CALL cl_err3("sel","ina_file",g_rvu.rvu01,"","asf-020","","",1)  #No.FUN-660128
        RETURN
     ELSE
     RETURN
     END IF
...
FUNCTION t720_dj_s2()
 ...
 DEFINE l_img30_update  LIKE img_file.img30,    #更新img30的值
        l_img31_update  LIKE img_file.img31     #更新img31的值
 DEFINE l_img10  LIKE img_file.img10,
        l_img26  LIKE img_file.img26,
        l_img30  LIKE img_file.img30,
        l_img31  LIKE img_file.img31
 DEFINE l_pmn65         LIKE pmn_file.pmn65
 DEFINE l_pmn07         LIKE pmn_file.pmn07
 ...
 DEFINE l_pmn09         LIKE pmn_file.pmn09
   ...
   SELECT tc_soa43 INTO l_tc_soa43 FROM tc_soa_file WHERE tc_soa00 = '0'
   IF cl_null(l_tc_soa43) THEN  LET l_tc_soa43 = 'N' END IF
   IF l_tc_soa43 = 'N' THEN RETURN END IF

   LET l_cnt = 0
   SELECT COUNT(*) INTO l_cnt FROM imd_file,rvv_file,ima_file,imz_file
    WHERE imd01 = rvv32
      AND rvv31 = ima01
      AND ima06 = imz01
      AND ta_imz01 = 'Y'
      AND ta_imd20 IS NOT NULL
      AND ta_rvv02 >0
      AND rvv01 = g_rvu.rvu01
   IF l_cnt =0 THEN RETURN END IF

   DECLARE t720_djs2_c CURSOR FOR
   SELECT ta_imd20,rvv_file.* FROM imd_file,rvv_file
    WHERE rvv32=imd01 AND rvv01 = g_rvu.rvu01
      AND ta_imd20 IS NOT NULL
      AND ta_rvv02 >0

   FOREACH t720_djs2_c INTO l_ta_imd20,b_rvv.*
      IF STATUS THEN
         EXIT FOREACH
      END IF

      LET b_rvv.rvv32 = l_ta_imd20
      LET b_rvv.rvv17 = b_rvv.ta_rvv02
...
#     IF b_rvv.rvv32 IS NULL THEN LET  b_rvv.rvv32 = ' ' END IF   #倉
      ...
      IF b_rvv.rvv33 IS NULL THEN LET  b_rvv.rvv33 = ' ' END IF   #儲
      IF b_rvv.rvv34 IS NULL THEN LET  b_rvv.rvv34 = ' ' END IF   #批
      MESSAGE 'dj_s2() read no:',b_rvv.rvv02 USING '#####&','--> parts: ', b_rvv.rvv31
      CALL ui.Interface.refresh()
      IF cl_null(b_rvv.rvv04) AND g_rvu.rvu00='1' THEN
         CALL cl_err('receive number is empty!','',1)
         LET g_success='N'
...
#     IF b_rvv.rvv31[1,4] = 'MISC' THEN CONTINUE FOREACH END IF
      ...
      IF b_rvv.rvv32 IS NOT NULL AND l_pmn65='1' THEN
            ...
            LET g_forupd_sql = " SELECT rowid FROM img_file ",
                               ...
                               "    AND img04= ? ",
                               " FOR UPDATE NOWAIT "
            DECLARE img_lock_dj_t2 CURSOR FROM g_forupd_sql

            OPEN img_lock_dj_t2 USING b_rvv.rvv31,b_rvv.rvv32,b_rvv.rvv33,b_rvv.rvv34
            IF STATUS THEN
               CALL cl_err("OPEN img_lock_dj_t2:", STATUS, 1)
               CLOSE img_lock_dj_t2
               ROLLBACK WORK
               LET g_success = 'N'
               RETURN
            END IF

            FETCH img_lock_dj_t2 INTO l_rowid
            IF STATUS THEN
               CALL cl_err('img_lock_dj_t2 fail',STATUS,1)
               LET g_success = 'N'
               RETURN
            END IF
            #NO.3443 END .......................................
...
DEFINE l_sr  RECORD
           ...
           l_rvv35_fac  LIKE rvv_file.rvv35_fac,
           l_rvv17  LIKE rvv_file.rvv17
           END RECORD


 LET l_ina.ina01=''
 SELECT ina01 INTO l_ina.ina01 FROM ina_file WHERE ina10=g_rvu.rvu01 AND ina00='3' AND inaconf<>'X'

 IF cl_null(l_ina.ina01) THEN
    LET l_ina.ina00='3'
    LET l_ina.ina02=g_rvu.rvu03
    LET l_ina.ina03=g_rvu.rvu03
    LET l_ina.ina11=g_rvu.rvu07
    LET l_ina.ina12='N'
    LET l_ina.ina04=g_rvu.rvu06
    LET l_ina.ina07='驗退料件暫放'
    LET l_ina.ina10=g_rvu.rvu01
    LET l_ina.inaconf='Y'
    LET l_ina.ina08='1'
    LET l_ina.inapost='N'
    LET l_ina.inauser=g_user
    ...
    LET l_ina.ina01='SCI-'
    CALL s_auto_assign_no("aim",l_ina.ina01,l_ina.ina03,"3","ina_file","ina01","","","") RETURNING li_result,l_ina.ina01
    IF (NOT li_result) THEN
       ERROR "生成單據失敗，自動編號錯誤！"
       LET g_success='N'
       RETURN
    ...
    INSERT INTO ina_file VALUES(l_ina.*)
    IF STATUS THEN
       CALL s_errmsg('ina01',l_ina.ina01,'ins ina:',STATUS,1)
       LET g_success='N'
       RETURN
    END IF
 END IF
...
# INITIALIZE l_ina.* TO NULL
 ...
 LET l_sql=" SELECT rvv02,rvv31,rvv35,rvv35_fac,rvv17",
           "   FROM rvv_file",
           "  WHERE rvv01='",g_rvu.rvu01,"'"

 LET l_cnt=1
 PREPARE t721_rvv_p1 FROM l_sql
 DECLARE t721_rvv_c1 CURSOR FOR t721_rvv_p1
 FOREACH t721_rvv_c1 INTO l_sr.*
    LET l_inb.inb01=l_ina.ina01
    LET l_inb.inb03=l_sr.l_rvv02
    LET l_inb.inb04=l_sr.l_rvv31
    SELECT COUNT(*)
      INTO l_cnt
      FROM ima_file,imz_file
     WHERE ima06=imz01
       AND imz15='Y'
       AND ima01=l_sr.l_rvv31
    IF l_cnt>0 THEN
       LET l_inb.inb05='HZ'
    ELSE
       LET l_inb.inb05='LZ'
    ...
    LET l_inb.inb06=' '
    LET l_inb.inb07=' '
    LET l_inb.inb08=l_sr.l_rvv35
    LET l_inb.inb08_FAC=l_sr.l_rvv35_fac
    LET l_inb.inb09=l_sr.l_rvv17
    LET l_inb.inb15='ZFL'
#    CALL t370_update(l_inb.inb01,l_inb.inb04,l_inb.inb05,l_inb.inb06,l_inb.inb07,l_inb.inb09,l_inb.inb08,l_inb.inb08_FAC)
               LET l_inb.inblegal = g_legal
               LET l_inb.inbplant = g_plant
               LET l_inb.inb16 = l_inb.inb09
    INSERT INTO inb_file VALUES(l_inb.*)
     IF SQLCA.SQLCODE THEN
        CALL cl_err('i inb:',STATUS,1)  #No:FUN-660128
        CALL cl_err3("ins","inb_file",l_inb.inb01,"",STATUS,"","i inb:",1)    #No:FUN-660128
        DELETE FROM ina_file where sfp01=l_ina.ina01
        DELETE FROM inb_file where sfq01=l_ina.ina01
        LET g_success='N'
        RETURN
     END IF
 END FOREACH
...
FUNCTION create_aimt301()
DEFINE p_ina   RECORD LIKE ina_file.*
DEFINE p_inb   RECORD LIKE inb_file.*
DEFINE l_sql   STRING
DEFINE l_cnt,li_result   LIKE type_file.num5
DEFINE p_sr  RECORD
           l_rvv02  LIKE rvv_file.rvv02,
 ...
 LET p_ina.ina01=''
 SELECT ina01 INTO p_ina.ina01 FROM ina_file WHERE ina10=g_rvu.rvu01 AND ina00='1' AND inaconf<>'X'

 IF cl_null(p_ina.ina01) THEN
    LET p_ina.ina00='1'
    LET p_ina.ina02=g_rvu.rvu03
    LET p_ina.ina03=g_rvu.rvu03
    LET p_ina.ina11=g_rvu.rvu07
    LET p_ina.ina04=g_rvu.rvu06
    LET p_ina.ina07='驗退料件退回'
    LET p_ina.ina10=g_rvu.rvu01
    LET p_ina.inaconf='Y'
    LET p_ina.ina08='1'
    LET p_ina.inapost='N'
    LET p_ina.inauser=g_user
    LET p_ina.inagrup=g_grup
    LET p_ina.inadate=g_today
    LET p_ina.inapos = '1'
    LET p_ina.inaplant = g_plant
    LET p_ina.inalegal = g_legal
    LET p_ina.ina12='N'

    LET p_ina.ina01='LS4-'
    CALL s_check_no("aim",p_ina.ina01,"","1","ina_file","ina01","")
                RETURNING li_result,p_ina.ina01
            # DISPLAY BY NAME g_rvu.rvu01
    CALL s_auto_assign_no("aim",p_ina.ina01,g_today,"1","ina_file","ina01","","","") RETURNING li_result,p_ina.ina01
    IF (NOT li_result) THEN
       ERROR "生成單據失敗，自動編號錯誤！"
       LET g_success='N'
       RETURN
    ...
    INSERT INTO ina_file VALUES(p_ina.*)
    IF STATUS THEN
       CALL s_errmsg('ina01',p_ina.ina01,'ins ina:',STATUS,1)
       LET g_success='N'
       RETURN
    END IF
 END IF
...
# INITIALIZE l_ina.* TO NULL
 INITIALIZE p_inb.* TO NULL

 LET l_sql=" SELECT rvv02,rvv31,rvv35,rvv35_fac,rvv17",
           "   FROM rvv_file",
           "  WHERE rvv01='",g_rvu.rvu01,"'"
 LET l_cnt=1
 PREPARE t721_rvv_p2 FROM l_sql
 DECLARE t721_rvv_c2 CURSOR FOR t721_rvv_p2
 FOREACH t721_rvv_c2 INTO p_sr.*
    LET p_inb.inb01=p_ina.ina01
    LET p_inb.inb03=l_cnt
    LET p_inb.inb04=p_sr.l_rvv31
    LET l_cnt=0
    SELECT COUNT(*)
      INTO l_cnt
      FROM ima_file,imz_file
    ...
    LET p_inb.inb06=' '
    LET p_inb.inb07=' '
    LET p_inb.inb08=p_sr.l_rvv35
    LET p_inb.inb08_FAC=p_sr.l_rvv35_fac
    LET p_inb.inb09=p_sr.l_rvv17
    LET p_inb.inb15='GHL'

  #  CALL t370_update(p_inb.inb01,p_inb.inb04,p_inb.inb05,p_inb.inb06,p_inb.inb07,p_inb.inb09,p_inb.inb08,p_inb.inb08_FAC)
               LET p_inb.inblegal = g_legal
               LET p_inb.inbplant = g_plant
               LET p_inb.inb16 = p_inb.inb09
    INSERT INTO inb_file VALUES(p_inb.*)
     IF SQLCA.SQLCODE THEN
        CALL cl_err('i inb:',STATUS,1)  #No:FUN-660128
        CALL cl_err3("ins","inb_file",p_inb.inb01,"",STATUS,"","i inb:",1)    #No:FUN-660128
        DELETE FROM ina_file where sfp01=p_ina.ina01
        DELETE FROM inb_file where sfq01=p_ina.ina01
        LET g_success='N'
        RETURN
     END IF
 END FOREACH

 LET l_sql="aimt301 '",p_ina.ina01 CLIPPED ,"'"
...
{FUNCTION t370_update(p_inb01,p_inb04,p_ware,p_loca,p_lot,p_qty,p_uom,p_factor)
  ...
  DEFINE l_ina00   LIKE ina_file.ina00
    ...
    LET g_forupd_sql =
        "SELECT rowid,img10,img16,img23,img24,img09,img21 FROM img_file ",
    ...
    IF STATUS THEN
       IF g_bgerr THEN
          CALL s_errmsg('','','OPEN img_lock:',STATUS,1)
       ELSE
    ...
    SELECT ina00 INTO l_ina00 FROM ina_file WHERE ina01=p_inb01

    CASE WHEN l_ina00 MATCHES "[12]" LET u_type=-1
            LET l_qty= l_img.img10 - p_qty
	 WHEN l_ina00 MATCHES "[34]" LET u_type=+1
            LET l_qty= l_img.img10 + p_qty
	 WHEN l_ina00 MATCHES "[56]" LET u_type=0
            LET l_qty= l_img.img10 - p_qty
    END CASE
    #CALL s_upimg(l_img.rowid,u_type,p_qty*p_factor,g_ina.ina02,
    #      '','','','',g_ina.ina01,g_inb[1].inb03,'','','','','','','','','','','','')
    CALL s_upimg(l_img.rowid,u_type,p_qty*p_factor,g_ina.ina02,
    ...
    LET g_forupd_sql =
        "SELECT ima25 FROM ima_file WHERE ima01= ? FOR UPDATE NOWAIT "
...
FUNCTION t370_tlf(p_ware,p_loca,p_lot,p_unit,p_qty,p_img10,p_uom,p_factor,
                     u_type)
   DEFINE
      #p_ware  VARCHAR(10),             ##倉庫              #FUN-660078 remark
      #p_loca  VARCHAR(10),	     ##儲位              #FUN-660078 remark
      #p_lot   VARCHAR(24),	     #MOD-520023 #批號   #FUN-660078 remark
      p_ware   LIKE img_file.img02,  #倉庫               #FUN-660078
      p_loca   LIKE img_file.img03,  #儲位               #FUN-660078
      p_lot    LIKE img_file.img04,  #批號               #FUN-660078
      #p_qty   DECIMAL (11,3),            ##數量
      p_qty    LIKE tlf_file.tlf10,       #MOD-570234
      #p_uom   VARCHAR(04),                  ##img 單位   #FUN-660078 remark
...
#  CALL s_defcour('asfi500',l_sfb03,'')RETURNING l_sta,g_tlf.tlf15,g_tlf.tlf16
   LET g_tlf.tlf17=g_ina.ina07              #Remark
#  CALL s_imaQOH(b_inb.inb04) RETURNING g_tlf.tlf18
...
DEFINE l_rvv36      LIKE rvv_file.rvv36
DEFINE l_rvv39_t    LIKE rvv_file.rvv39
DEFINE l_msg        STRING

   LET l_sql ="SELECT rvv02,rvv38,rvv17,rvv39,rvv36 ",
              "  FROM rvv_file ",
              " WHERE rvv01 = '",g_rvu.rvu01,"'"
   PREPARE p_per FROM l_sql
   DECLARE c_cus CURSOR FOR p_per
   FOREACH c_cus INTO l_rvv02,l_rvv38,l_rvv17,l_rvv39,l_rvv36
      IF STATUS THEN
         CALL cl_err('foreach:',SQLCA.sqlcode,0)
         RETURN
      END IF
      LET l_rvv39_t = l_rvv38 * l_rvv17
      CALL t720_get_azi3(l_rvv36)
      LET l_rvv39_t = cl_digcut(l_rvv39_t,t_azi04)
      IF l_rvv39_t <>l_rvv39 THEN
      	# LET l_msg = "项次",l_rvv02 CLIPPED,"：数量",l_rvv17 CLIPPED,"*单价",l_rvv38 CLIPPED,"!=金额",l_rvv39 CLIPPED ,"请检查"
      	 LET l_msg = "项次",l_rvv02 CLIPPED,"：数量*单价 != 金额 ，请检查"
      	 CALL cl_err(l_msg,'!',1)
      	 LET g_success = 'N'
      	 RETURN
      END IF
   END FOREACH
END FUNCTION
FUNCTION t720_get_azi3(l_rvv36)
DEFINE l_rva113          LIKE rva_file.rva113
DEFINE l_rvv36           LIKE rvv_file.rvv36
   ...
   IF (g_rva00 = '1' AND g_rvu.rvu00 = '3') OR cl_null(g_rva00) THEN
      IF NOT cl_null(l_rvv36) THEN
         SELECT pmm22 INTO l_rva113 FROM pmm_file WHERE pmm01 =l_rvv36
      ELSE
         LET l_rva113 = g_rvu.rvu113
      END IF
      SELECT azi03,azi04 INTO t_azi03,t_azi04
...
DEFINE l_rvv87      LIKE rvv_file.rvv17
DEFINE l_rvv39      LIKE rvv_file.rvv39
DEFINE l_rvv36      LIKE rvv_file.rvv36
DEFINE l_rvv39_t    LIKE rvv_file.rvv39
DEFINE l_rvv39t     LIKE rvv_file.rvv39t
DEFINE l_rvv39t_t   LIKE rvv_file.rvv39t
DEFINE l_rvv39_t_1  LIKE rvv_file.rvv39
DEFINE l_rvv39t_t_1 LIKE rvv_file.rvv39t
DEFINE l_msg        STRING

   LET l_sql ="SELECT rvv02,rvv38,rvv87,rvv39,rvv36,rvv39t,rvv38t ",
              "  FROM rvv_file ",
              " WHERE rvv01 = '",g_rvu.rvu01,"'"
   PREPARE p_per FROM l_sql
   DECLARE c_cus CURSOR FOR p_per
   FOREACH c_cus INTO l_rvv02,l_rvv38,l_rvv87,l_rvv39,l_rvv36,l_rvv39t,l_rvv38t
      IF STATUS THEN
         CALL cl_err('foreach:',SQLCA.sqlcode,0)
         RETURN
      END IF
      LET l_rvv39_t = l_rvv38 * l_rvv87
      LET l_rvv39t_t = l_rvv38t*l_rvv87
      CALL t720_get_azi3(l_rvv36)
      LET l_rvv39_t = cl_digcut(l_rvv39_t,t_azi04)
      LET l_rvv39t_t = cl_digcut(l_rvv39t_t,t_azi04)
      LET l_rvv39t = cl_digcut(l_rvv39t,t_azi04)        #2017-03-20 by dengyu

       CALL t720_rvv39_1(l_rvv36,l_rvv39_t,l_rvv39t_t,g_rvu.rvu04,g_rvu.rvu01)
                 RETURNING l_rvv39_t_1,l_rvv39t_t_1
      IF (l_rvv39_t =l_rvv39 AND l_rvv39t_t = l_rvv39t)
       OR(l_rvv39_t_1 =l_rvv39 AND l_rvv39t_t_1 = l_rvv39t) THEN
      ELSE
         IF (l_rvv39t-l_rvv39t_t)>0.01 OR (l_rvv39t-l_rvv39t_t)<-0.01 THEN     #2017-03-20 by dengyu
      	     LET l_msg = "项次",l_rvv02 CLIPPED,"：数量*单价 != 金额 ，请检查"
      	     CALL cl_err(l_msg,'!',1)
      	     LET g_success = 'N'
      	     RETURN
...
FUNCTION t720_rvv39_1(p_rvv36,p_rvv39,p_rvv39t,p_rvu04,p_rvu01)
  DEFINE    l_gec07   LIKE gec_file.gec07,     #含稅否
  ...
  ELSE
     ...
     IF NOT cl_null(l_rvu02) THEN
        SELECT rva00,rva115,rva116
          INTO l_rva00,l_rva115,l_pmm43
          FROM rva_file
         WHERE rva01 = l_rvu02
        IF l_rva00 = '2' THEN
           SELECT gec07,gec05
             INTO l_gec07,l_gec05
             FROM gec_file
            WHERE gec01 = l_rva115
        ELSE
           IF cl_null(p_rvv36) THEN
              LET l_pmm43 = l_rvu12
           ELSE
              SELECT gec07,gec05,pmm43
                INTO l_gec07,l_gec05,l_pmm43
                FROM gec_file,pmm_file
               WHERE gec01 = pmm21
                 AND pmm01 = p_rvv36
                 AND gec011 = '1'
           END IF
