...
# Modify.........: No.MOD-E90119 14/09/26 By doris 產生累折科目除了faj09 != '5'還要加faj28!='0'才可以
# Modify.........: No.CHI-EC0047 15/01/08 By doris 1.列管資產貸方科目改抓取fbe20/fbe201設定
#                                                  2.列管資產建議以資產性質不為土地且不提折舊的方式認定,列管資產分錄應為 D:其他應收 C:其他收入,不能有成本與處份損益
# Modify.........: No.MOD-F90153 15/10/05 By doris 出售額調整,取消l_fbe.fbe12 = 'N'的條件 
...
#  DEFINE l_t1          LIKE type_file.chr3         #No.FUN-680070 VARCHAR(03)
   ...
   IF p_npptype = "0" THEN   #FUN-B60140   Add   
     #IF p_date < g_faa.faa09 THEN   #立帳日期小於關帳日期  #CHI-E20004 mark
      IF p_date <=g_faa.faa09 THEN                          #CHI-E20004
         CALL cl_err(g_no,'aap-176',1) RETURN 
      END IF
#FUN-B60140   ---start   Add
    ELSE
      #IF p_date < g_faa.faa092 THEN   #立帳日期小於關帳日期   #CHI-E20004 mark
       IF p_date <=g_faa.faa092 THEN                           #CHI-E20004
...
DEFINE l_fbz10    LIKE fbz_file.fbz10,  
       ...
       l_fbf09    LIKE fbf_file.fbf09,      #CHI-EC0047 add
       l_faj17    LIKE faj_file.faj17,
       l_faj23    LIKE faj_file.faj23,
       l_faj24    LIKE faj_file.faj24,
       l_faj28    LIKE faj_file.faj28,      #MOD-E90119 add
      ...
      #l_sql      LIKE type_file.chr1000    #No.FUN-680070 VARCHAR(600)   #MOD-E90119 mark
       l_sql      STRING                                                  #MOD-E90119 add
#No:A099
...
#     CALL cl_err('ins npp',STATUS,1)    #No.FUN-660136
  ...
  #            "       faj232,faj242,faj322,faj142,faj1412,faj582,faj592,faj602,",  #FUN-AB0088 
  #            "       faj582,faj592,faj602 ",
  #MOD-E90119---mark---end---                         
  #MOD-E90119---add---str---加faj28
   LET l_sql = "SELECT fbf_file.*,faj17,faj23,faj24,faj28, ",
               "       faj53,faj531,faj54,faj541,faj32, ",
               "       faj14,faj141,faj59,faj58,faj60, ",                     
               "       fab24,fab241,faj09,faj232,faj242, ",                           
               "       faj322,faj142,faj1412,faj582,faj592,",  
               "       faj602,faj582,faj592,faj602 ",
  #MOD-E90119---add---end--- 
  ...
  #                    l_faj232,l_faj242,l_faj322,l_faj142,l_faj1412,l_faj582,l_faj592,l_faj602,  #FUN-AB0088 
  #                    l_faj582,l_faj592,l_faj602  
  #MOD-E90119---mark---end---                                  
  #MOD-E90119---add---str---加l_faj28
   FOREACH t110_c INTO l_fbf.*,l_faj17,l_faj23,l_faj24,l_faj28,
                       l_faj53,l_faj531,l_faj54,l_faj541,l_faj32,
                       l_faj14,l_faj141,l_faj59,l_faj58,l_faj60,                      
                       l_fab24,l_fab241,l_faj09,l_faj232,l_faj242, 
                       l_faj322,l_faj142,l_faj1412,l_faj582,l_faj592, 
                       l_faj602,l_faj582,l_faj592,l_faj602
  #MOD-E90119---add---end--- 
#No.FUN-710028 --begin                                                                                                              
      IF g_success='N' THEN                                                                                                         
         LET g_totsuccess='N'                                                                                                       
...
#              LET g_success='N' EXIT FOREACH        #No.FUN-710028                                  
      ...
      #end No:A099
      #-------Dr.: 累折科目 (faj54) -----------
     #IF l_faj09 != '5' THEN   #MOD-990191 add    #MOD-E90119 mark
     #IF l_faj09 != '5' AND l_faj28 != '0' THEN   #MOD-E90119 add   #CHI-EC0047 mark
...
#        LET g_npq.npq03 = l_fbz10       #出售應收科目
        ...
        #IF l_cnt = l_cnt2 AND l_fbe.fbe12 = 'N' THEN           #MOD-D10111        #MOD-F90153 mark
         IF l_cnt = l_cnt2 THEN                                                    #MOD-F90153 add
...
#-----------MOD-C30722--mark--end
   ...
   #MOD-C30722--add--end
  #CHI-EC0047---add---str--
  #列管資產貸方科目改抓取fbe20/fbe201設定
   LET l_cnt = 0
   SELECT COUNT(*) INTO l_cnt
     FROM fbf_file,faj_file
    WHERE fbf01 = l_fbe.fbe01
      AND fbf03 = faj02
      AND fbf031 = faj022 
      AND faj09 <> '3' 
      AND faj28 = '0'
   ...
   IF l_cnt > 0 THEN
      ...
      LET g_npq25 = g_npq.npq25
      IF g_npq.npqtype = '0' THEN
         LET g_npq.npq03 = l_fbe.fbe20  
      ELSE
         LET g_npq.npq03 = l_fbe.fbe201 
      END IF
      LET g_npq.npq04 = NULL
      LET l_aag05 = NULL
      IF g_npq.npqtype = '0' THEN
      ...
      SELECT SUM(fbf09) INTO l_fbf09 
        FROM fbf_file A
       WHERE EXISTS (SELECT fbf03||fbf031
                       FROM fbf_file,faj_file
                      WHERE fbf01 = l_fbe.fbe01
                        AND fbf03 = faj02
                        AND fbf031 = faj022
                        ...
                        AND fbf01 = l_fbe.fbe01
      IF l_fbf09 < 0 THEN
         LET l_fbf09 = l_fbf09 * -1
      END IF
      LET g_npq.npq07f = l_fbf09 / l_fbe.fbe06
      LET g_npq.npq07 = l_fbf09
      IF cl_null(g_npq.npq07f) THEN LET g_npq.npq07f = 0 END IF
      IF cl_null(g_npq.npq07) THEN LET g_npq.npq07 = 0 END IF
      LET g_npq.npq23 = ' '
      MESSAGE '>',g_npq.npq02,' ',g_npq.npq03
