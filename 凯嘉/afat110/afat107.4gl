# Prog. Version..: '5.30.26-17.12.13(00010)'     #
#
...
# Modify.........: No.CHI-CB0022 13/05/10 By Polly 折舊月份和累計折舊調整為折畢時，需一併調整資產狀態為折畢
...
# Modify.........: No:MOD-D80055 13/08/12 By Lori fbd09 預設值應包含 faj141 數字
# Modify.........: No:MOD-DA0127 13/10/17 By yuhuabao AFTER FIELD fbd12依據幣別取位後應重新DISPLAY
...
# Modify.........: No:MOD-E20131 14/02/27 By yihsuan 1.t107_6 稅簽過帳時,應新增/更新 fap71 而非 l_fap711
#                                                    2.t107_7 稅簽過帳還原時,應用 fap34 還原更新 faj63
# Modify.........: No:CHI-E60034 14/06/20 By yinhy 盤點中的資產不可確認、取消確認、過賬、過賬還原等動作
# Modify.........: No:CHI-E60048 14/06/30 By yinhy fan14/fan15賦值
# Modify.........: No:FUN-E80012 14/08/06 By qiull 修改付款日期之後,tic現金流量明細重複
# Modify.........: No:TQC-E80008 14/08/12 By doris FUNCTION t107_fin_audit2 比照 GP 5.1 版本發現 BEFORE FIELD fbd042 應為 BEFORE FIELD fbd092
# Modify.........: No:MOD-E80046 14/08/13 By yihsuan FOREACH t107_cur2/t107_cur22 中
#                                                    財簽    l_fap661  = fbd09 - fbd04   改用 l_fap54  ; l_fap661  直接給予 fbd04
#                                                    財簽二  l_fap6612 = fbd092 - fbd042 改用 l_fap542 ; l_fap6612 直接給予 fbd042
...
# Modify.........: No.MOD-F50137 15/06/02 By doris 過帳段(財簽/財簽二/稅簽)若為折畢再提時,應備份折畢再提預留殘值
# Modify.........: No:FUN-F50016 15/06/03 By jwlin 解決新增、複製後記錄key問題
# Modify.........: No:CHI-FA0016 15/10/23 By doris 增加異動前/異動後耐用年限欄位
# Modify.........: No:MOD-G10102 16/01/21 By doris 1.單身維護後自動產生分錄
#                                                  2.確認時，檢查帳款金額與分錄底搞金額是否一致
# Modify.........: No:MOD-G10142 16/01/26 By doris AFTER FIELD fbd03取l_n之SQL增加附號條件
# Modify.........: No:TQC-G20003 16/02/19 By doris 延續MOD-G10102,檢查帳款金額如 <0,應 *-1
# Modify.........: No:MOD-G20066 16/02/22 By doris 1.AFTER FIELD fbd03取消附號給予一個空白
#                                                  2.AFTER IINPUT 增加afa-105控卡
# Modify.........: No:MOD-GA0025 16/10/11 By Sabrina 1.在控卡axr-070之前要先判斷是否做"系統自動拋轉總帳"
#                                                    2.當g_success = 'N' 時，要rollback work
...
DEFINE
    ...
    g_fbd           DYNAMIC ARRAY OF RECORD    #程式變數(Program Variables)
                    ...
                    fbd25     LIKE fbd_file.fbd25,    #CHI-FA0016 add
                    fbd27     LIKE fbd_file.fbd27,    #CHI-FA0016 add
    ...
    g_e             DYNAMIC ARRAY OF RECORD        #稅簽資料維護
                    ...
                    ,fbd26    LIKE fbd_file.fbd26   #CHI-FA0016 add
                    ,fbd28    LIKE fbd_file.fbd28   #CHI-FA0016 add
                    END RECORD,
...
DEFINE g_fbd2       DYNAMIC ARRAY OF RECORD    #祘Α跑计(Program Variables)
                    ...
                    fbd252    LIKE fbd_file.fbd252,   #CHI-FA0016 add
                    fbd272    LIKE fbd_file.fbd272,   #CHI-FA0016 add
...
MAIN
DEFINE
   #l_sql         LIKE type_file.chr1000      #No.FUN-680070 VARCHAR(200)   #TQC-E80008 mark
    l_sql         STRING                                                    #TQC-E80008 add
   ...
   IF NOT cl_null(g_argv1) THEN
      CASE g_argv2
         ...
         WHEN "insert"
            ...
            IF cl_chk_act_auth() THEN
               CALL t107_a()
               LET g_data_keyvalue = g_fbc.fbc01      #No:FUN-F50016
            END IF
         WHEN "efconfirm"
            CALL t107_q()
...
DEFINE  lc_qbe_sn       LIKE    gbm_file.gbm01    #No.FUN-580031  HCN
   ...
   INITIALIZE g_fbc.* TO NULL    #No.FUN-750051
      ...
      CONSTRUCT g_wc2 ON fbd02,fbd03,fbd031,faj06,fbd04,faj141,fbd09,fbd05,fbd10,fbd25,fbd27,fbd06,   #No.MOD-490181   #MOD-780022   #CHI-FA0016 add fbd25,fbd27
                         fbd11,fbd07,fbd12
            ...
            FROM s_fbd[1].fbd02, s_fbd[1].fbd03, s_fbd[1].fbd031,  #No.MOD-490181
                 ...
                 s_fbd[1].fbd05, s_fbd[1].fbd10, s_fbd[1].fbd25, s_fbd[1].fbd27,    #CHI-FA0016 add fbd25,fbd27
            ...
            IF cl_chk_act_auth() AND g_fbc.fbcconf <> 'X' AND g_fah.fahfa1 = 'Y' THEN #MOD-BC0125 add fahfa1
              ...
              #FUN-C30140---add---end---
                  IF g_fbc.fbcconf = 'N' THEN  #MOD-690081
                     ...
                     LET g_t1 = s_get_doc_no(g_fbc.fbc01)                       #MOD-G10102 add
            ...
            IF cl_chk_act_auth() AND g_fbc.fbcconf <> 'X' AND g_fah.fahfa2 = 'Y' THEN #MOD-BC0125 add fahfa2
              ...
              #FUN-C30140---add---end---
                  IF g_fbc.fbcconf = 'N' THEN
                     ...
                     LET g_t1 = s_get_doc_no(g_fbc.fbc01)                       #MOD-G10102 add
                     SELECT * INTO g_fah.* FROM fah_file WHERE fahslip = g_t1   #MOD-G10102 add
                     IF g_fah.fahdmy32 = 'Y' THEN                               #MOD-G10102 add
                        CALL t107_gl(g_fbc.fbc01,g_fbc.fbc02,'1')
                     ELSE                            #MOD-G10102 add
                        CALL cl_err('','mfg9310',1)  #MOD-G10102 add
                     END IF                          #MOD-G10102 add
...
DEFINE
    ...
    LET g_data_keyvalue = g_fbc.fbc01  #FUN-DA0124 add
    CALL s_get_bookno(g_yy)                          #No.TQC-7B0060
         RETURNING g_flag,g_bookno1,g_bookno2
    #FUN-AB0088---add---str---
    #固資已不和二套掛鉤，所以原本利用s_get_bookno取到賬別一及賬別二是用是否使用二套賬來判斷
...
#FUN-BB0122--end add 
   ...
   DEFINE l_i          LIKE type_file.num5     #CHI-FA0016 add
    ...
    LET g_forupd_sql = " SELECT fbd02,fbd03,fbd031,'',fbd04,'',fbd09,fbd05,fbd10,fbd25,fbd27,fbd06, ",   #MOD-780022   #CHI-FA0016 add fbd25,fbd27
                       " fbd11,fbd07,fbd12 " ,
                       " FROM fbd_file ",
                       " WHERE fbd01 = ? ",
                       " AND fbd02   = ? ",
      ...
      INPUT ARRAY g_fbd WITHOUT DEFAULTS FROM s_fbd.*
	    ...
	    BEGIN WORK
            ...
            IF g_rec_b >= l_ac THEN
                ...
                ELSE
                  ...
                  #CHI-FA0016---add---str--
                   #舊資料會沒有值
                   IF STATUS != 100 AND cl_null(g_fbd[l_ac].fbd25) THEN
                      IF g_fbc.fbcpost = 'Y' THEN
                         SELECT fap83,fap93 INTO g_fbd[l_ac].fbd25,g_fbd[l_ac].fbd27
                           FROM fap_file
                          WHERE fap02 = g_fbd[l_ac].fbd03
                            AND fap021 = g_fbd[l_ac].fbd031
                            AND fap50 = g_fbc.fbc01
                      ELSE
                         SELECT faj29 INTO g_fbd[l_ac].fbd25
                           FROM faj_file
                          WHERE faj02 = g_fbd[l_ac].fbd03
                            AND faj022 = g_fbd[l_ac].fbd031
                         LET g_fbd[l_ac].fbd27 = g_fbd[l_ac].fbd25
                      END IF
                   END IF
                  #CHI-FA0016---add---end--
                END IF
        ...
        AFTER INSERT
          ...
          #MOD-G10142---add---str--
           CALL t107_fbd031('d')
           IF NOT cl_null(g_errno) THEN
              CALL cl_err(g_fbd[l_ac].fbd03,g_errno,1)
              NEXT FIELD fbd03
           END IF
          #MOD-G10142---add---end--
           #FUN-BB0122---being add
         ...
         #CHI-C60010---str---
          ...
          #CHI-C60010---end---
             INSERT INTO fbd_file(fbd01,fbd02,fbd03,fbd031,fbd04, #No.MOD-470041
                                 ...
                                 ,fbd25,fbd26,fbd27,fbd28         #CHI-FA0016 add
                                 ,fbdud01,fbdud02,fbdud03,fbdud04,fbdud05,
                                 fbdud06,fbdud07,fbdud08,fbdud09,fbdud10,
                                 fbdud11,fbdud12,fbdud13,fbdud14,fbdud15,
             ...
             VALUES(g_fbc.fbc01,g_fbd[l_ac].fbd02,g_fbd[l_ac].fbd03,
                   ...
                   g_fbc.fbc03,g_fbd[l_ac].fbd25,g_e[l_ac].fbd26,g_fbd[l_ac].fbd27,g_e[l_ac].fbd28   #CHI-FA0016 add fbd25~fbd28
       ...
       #FUN-BC0004--mark--end
           ...
           IF NOT cl_null(g_fbd[l_ac].fbd03) AND g_fbd[l_ac].fbd031 IS NOT NULL THEN 
              IF p_cmd = 'a' OR
                (p_cmd = 'u' AND (g_fbd[l_ac].fbd03 != g_fbd_t.fbd03 OR
                 ...
                 SELECT COUNT(*) INTO l_n FROM faj_file
                  WHERE faj02 = g_fbd[l_ac].fbd03
                    AND faj022 = g_fbd[l_ac].fbd031   #MOD-G10142 add
                ...
                #   CALL cl_err(g_fbd[l_ac].fbd03,'afa-093',0)    #MOD-G10142 mark
                #   LET g_fbd[l_ac].fbd03 = g_fbd_t.fbd03         #MOD-G10142 mark
                #   NEXT FIELD fbd03                              #MOD-G10142 mark
                END IF
              END IF
            END IF 
        ...
        AFTER FIELD fbd09
          ...
          #IF cl_null(g_e[l_ac].fbd19) OR g_e[l_ac].fbd19 = 0   #MOD-E20131 mark 
          #THEN LET g_e[l_ac].fbd19 = g_fbd[l_ac].fbd09         #MOD-E20131 mark 
          #END IF                                               #MOD-E20131 mark
        ...
        AFTER FIELD fbd12
           ...
           DISPLAY BY NAME g_fbd[l_ac].fbd12  #MOD-DA0127 add
...
#                                g_fbd[l_ac].fbd10)*l_fab23/100      #MOD-A40033 mark
          ...
          #CHI-FA0016---mark---end--
          #CHI-FA0016---add---str--
           IF g_fbd[l_ac].fbd11 > g_fbd[l_ac].fbd27 THEN
              CALL cl_err('','afa-214',1)
              NEXT FIELD fbd11
           END IF
          #CHI-FA0016---add---end--
            ...
            ELSE
              ...
              #MOD-G10142---add---end--
               UPDATE fbd_file SET
                     ...
                     ,fbd25 = g_fbd[l_ac].fbd25   #CHI-FA0016 add
                     ,fbd27 = g_fbd[l_ac].fbd27   #CHI-FA0016 add
        ...
        AFTER ROW
           ...
           #LET l_ac_t = l_ac  #FUN-D30032 mark
            ...
            CLOSE t107_bcl
            COMMIT WORK
            CALL g_fbd.deleteElement(g_rec_b+1)   #MOD-G10142 add
 
       #CHI-FA0016---add---str--
        AFTER INPUT
           CALL s_showmsg_init()
           LET g_totsuccess = 'Y'
           FOR l_i = 1 TO g_fbd.getLength()
              LET g_success = 'Y'
              IF g_fbd[l_i].fbd11 > g_fbd[l_i].fbd27 THEN
                 CALL s_errmsg('fbd11',g_fbd[l_i].fbd11,'','afa-214',1)
                 LET g_success = 'N'
                 LET g_totsuccess = 'N'
                #CONTINUE FOR   #MOD-G20066 mark
              END IF
             #MOD-G20066---add---str--
              IF g_fbd[l_i].fbd031 IS NULL THEN
                 LET g_fbd[l_i].fbd031 = ' '
              END IF
              SELECT COUNT(*) INTO l_n FROM fbd_file
               WHERE fbd01  = g_fbc.fbc01
                 AND fbd03  = g_fbd[l_i].fbd03
                 AND fbd031 = g_fbd[l_i].fbd031
                 AND fbd02 <> g_fbd[l_i].fbd02
              IF l_n > 0 THEN
                 CALL s_errmsg('fbd03',g_fbd[l_i].fbd03,'','afa-105',1)
                 LET g_success = 'N'
                 LET g_totsuccess = 'N'
                 CONTINUE FOR
              END IF
           ...
           CALL s_showmsg()       
           IF g_success = 'N' THEN
             #NEXT FIELD fbd11   #MOD-G20066 mark
              NEXT FIELD fbd03   #MOD-G20066 add 
           END IF 
       #CHI-FA0016---add---end--
...
FUNCTION t107_fbd031(p_cmd)
   DEFINE   p_cmd       LIKE type_file.chr1,         #No.FUN-680070 VARCHAR(1)
            ...
            l_faj201    LIKE faj_file.faj201,   #MOD-F50137 add
            l_faj32     LIKE faj_file.faj32,
            l_faj29     LIKE faj_file.faj29,    #CHI-FA0016 add
            ...
            l_faj63     LIKE faj_file.faj63,   #MOD-E20131 add
   ...
   SELECT count(*) INTO l_n FROM fbd_file,fbc_file
     WHERE fbd01 = fbc01
      AND fbd03  = g_fbd[l_ac].fbd03
      AND fbd031 = g_fbd[l_ac].fbd031
     #AND fbc02 <= g_fbc.fbc02    #MOD-D60067 mark
      AND fbcpost = 'N'
      AND fbc01 != g_fbc.fbc01
      AND fbcconf <> 'X'
   IF l_n  > 0 THEN
   ...
   #CHI-B80007 -- end --
  #MOD-F50137---mark---str--
  #SELECT faj06,faj14,faj141,faj32,faj30,faj31,faj35,faj67,faj62,faj63,faj65,faj66,faj72,   #MOD-780022   #MOD-E20131 add faj63
  #       faj43,fajconf,faj100,faj28,faj141
  #  INTO l_faj06,l_faj14,l_faj141,l_faj32,l_faj30,l_faj31,l_faj35,l_faj67,   #MOD-780022
  #       l_faj62,l_faj63,l_faj65,                                            #MOD-E20131 add l_faj63
  #       l_faj66,l_faj72,l_faj43,l_fajconf,l_faj100,g_faj28,g_faj141
  #MOD-F50137---mark---end--
  #MOD-F50137---add---str--
   SELECT faj06,faj14,faj141,faj201,faj32,
          faj29,faj30,faj31,faj35,faj67,faj62,   #CHI-FA0016 add faj29
          faj63,faj65,faj66,faj72,faj43,
          fajconf,faj100,faj28,faj141
     INTO l_faj06,l_faj14,l_faj141,l_faj201,l_faj32,
          l_faj29,l_faj30,l_faj31,l_faj35,l_faj67,l_faj62,   #CHI-FA0016 add l_faj29
          l_faj63,l_faj65,l_faj66,l_faj72,l_faj43,
          l_fajconf,l_faj100,g_faj28,g_faj141
  #MOD-F50137---add---end--
     FROM faj_file
    WHERE faj02  = g_fbd[l_ac].fbd03
      AND faj022 = g_fbd[l_ac].fbd031
   ...
   IF l_faj43 = '7' THEN    #折畢再提
      LET l_faj31 = l_faj35
   END IF                   #MOD-F50137 add
   IF l_faj201 = '7' THEN   #MOD-F50137 add
      LET l_faj66 = l_faj72
   END IF
   IF ( p_cmd = 'a' OR p_cmd = 'u') AND cl_null(g_errno) THEN      #MOD-950166     
      ...
      LET g_fbd[l_ac].fbd25 = l_faj29     #耐用年限  #CHI-FA0016 add
      LET g_fbd[l_ac].fbd06 = l_faj30     #未用年限
      LET g_fbd[l_ac].fbd07 = l_faj31     #預估殘值
      LET g_fbd[l_ac].fbd09 = l_faj14 + l_faj141     #MOD-D80055 mod +faj141
      LET g_fbd[l_ac].fbd10 = l_faj32
      LET g_fbd[l_ac].fbd27 = l_faj29                #CHI-FA0016 add
      ...
      LET g_e[l_ac].fbd19   = l_faj62 + l_faj63   #MOD-E20131 add
      DISPLAY BY NAME g_fbd[l_ac].fbd04     #MOD-D80055 
      DISPLAY BY NAME g_fbd[l_ac].fbd05     #MOD-D80055 
      DISPLAY BY NAME g_fbd[l_ac].fbd06     #MOD-D80055 
      DISPLAY BY NAME g_fbd[l_ac].fbd07     #MOD-D80055 
      DISPLAY BY NAME g_fbd[l_ac].fbd09     #MOD-D80055 
      DISPLAY BY NAME g_fbd[l_ac].fbd10     #MOD-D80055 
      DISPLAY BY NAME g_fbd[l_ac].fbd11     #MOD-D80055 
      DISPLAY BY NAME g_fbd[l_ac].fbd12     #MOD-D80055
      DISPLAY BY NAME g_fbd[l_ac].fbd25     #CHI-FA0016 add
      DISPLAY BY NAME g_fbd[l_ac].fbd27     #CHI-FA0016 add 
   END IF
END FUNCTION
 
FUNCTION t107_b_askkey()
#DEFINE l_wc2           LIKE type_file.chr1000      #No.FUN-680070 VARCHAR(200)   #TQC-E80008 mark
 DEFINE l_wc2           STRING                                                    #TQC-E80008 add
...
#DEFINE p_wc2           LIKE type_file.chr1000      #No.FUN-680070 VARCHAR(200)   #TQC-E80008 mark
 DEFINE p_wc2           STRING                                                    #TQC-E80008 add
 
    LET g_sql =
        "SELECT fbd02,fbd03,fbd031,faj06,fbd04,faj141,fbd09,",   #MOD-780022
        "       fbd05,fbd10,fbd25,fbd27,fbd06,fbd11,fbd07,fbd12 ",   #CHI-FA0016 add fbd25,fbd27
        "       ,fbdud01,fbdud02,fbdud03,fbdud04,fbdud05,",
        "       fbdud06,fbdud07,fbdud08,fbdud09,fbdud10,",
        "       fbdud11,fbdud12,fbdud13,fbdud14,fbdud15", 
    ...
    FOREACH fbd_curs INTO g_fbd[g_cnt].*   #單身 ARRAY 填充
       ...
       #CHI-FA0016---add---str--
        #舊資料會沒有值
        IF cl_null(g_fbd[g_cnt].fbd25) THEN
           IF g_fbc.fbcpost = 'Y' THEN
              SELECT fap83,fap93 INTO g_fbd[g_cnt].fbd25,g_fbd[g_cnt].fbd27 
                FROM fap_file
               WHERE fap02 = g_fbd[g_cnt].fbd03
                 AND fap021 = g_fbd[g_cnt].fbd031 
                 AND fap50 = g_fbc.fbc01
           ELSE
              SELECT faj29 INTO g_fbd[g_cnt].fbd25
                FROM faj_file
               WHERE faj02 = g_fbd[g_cnt].fbd03 
                 AND faj022 = g_fbd[g_cnt].fbd031
              LET g_fbd[g_cnt].fbd27 = g_fbd[g_cnt].fbd25 
           END IF 
        END IF
       #CHI-FA0016---add---end--
        LET g_cnt = g_cnt + 1
...
#DEFINE  l_wc        LIKE type_file.chr1000,      #No.FUN-680070 VARCHAR(300)   #TQC-E80008 mark
#        l_sql       LIKE type_file.chr1000,      #No.FUN-680070 VARCHAR(600)   #TQC-E80008 mark
 DEFINE  l_wc        STRING,                                                    #TQC-E80008 add
         l_sql       STRING,                                                    #TQC-E80008 add
         l_faj       RECORD
               ...
               faj141    LIKE faj_file.faj141,   #MOD-780022
               faj201    LIKE faj_file.faj201,   #MOD-F50137 add
               faj32     LIKE faj_file.faj32,
               faj30     LIKE faj_file.faj30,
               faj31     LIKE faj_file.faj31,
               faj35     LIKE faj_file.faj35,   #MOD-F50137 add
               faj43     LIKE faj_file.faj43,   #MOD-F50137 add
               faj67     LIKE faj_file.faj67,
               faj62     LIKE faj_file.faj62,
               faj63     LIKE faj_file.faj63,   #MOD-E20131 add
               faj65     LIKE faj_file.faj65,
               faj66     LIKE faj_file.faj66,   #MOD-F50137 add ,
               faj72     LIKE faj_file.faj72    #MOD-F50137 add
               ,faj29     LIKE faj_file.faj29   #CHI-FA0016 add
               END RECORD,
...
#FUN-BB0122--end add
  ...
  #---------詢問是否自動新增單身--------------
     ...
     #LET l_sql ="SELECT faj02,faj022,faj06,faj14,faj141,faj32,faj30,faj31,faj67,",   #MOD-780022   #MOD-F50137 mark
     #           "faj62,faj63,faj65,faj66",   #MOD-E20131 add faj63                                 #MOD-F50137 mark
     #MOD-F50137---add---str--
      LET l_sql = "SELECT faj02,faj022,faj06,faj14,faj141, ",
                  "       faj201,faj32,faj30,faj31,faj35, ",
                  "       faj43,faj67,faj62,faj63,faj65, ",
                  "       faj66,faj72,faj29 ",   #CHI-FA0016 add faj29
     #MOD-F50137---add---end--
                 "  FROM faj_file ",
     ...
     display l_i,l_faj.*
      ...
      #                    l_faj.faj14+l_faj.faj141,l_faj.faj32,l_faj.faj30,l_faj.faj31,   #MOD-D80055 add faj141
      ...
      #--MOD-E20131 mark end
      #MOD-F50137---add---str--
       IF l_faj.faj43 = '7' THEN    #折畢再提(財簽)
          LET l_faj.faj31 = l_faj.faj35
       END IF
       IF l_faj.faj201 = '7' THEN   #折畢再提(稅簽)
          LET l_faj.faj66 = l_faj.faj72
       END IF
      #MOD-F50137---add---end--
      #--MOD-E20131 add start--
       INSERT INTO fbd_file(fbd01,fbd02,fbd03,fbd031,fbd04,
                            fbd05,fbd06,fbd07,fbd09,fbd10,
                            fbd11,fbd12,fbd14,fbd15,fbd16,
                            fbd17,fbd19,fbd20,fbd21,fbd22,
                            fbd25,fbd27,   #CHI-FA0016 add fbd25,fbd27
                            fbd042,fbd052,fbd062,fbd072,fbd092,
                            fbd102,fbd112,fbd122,fbdlegal)
                     VALUES(g_fbc.fbc01,l_i,l_faj.faj02,l_faj.faj022,l_faj.faj14,
                            l_faj.faj32,l_faj.faj30,l_faj.faj31,l_faj.faj14+l_faj.faj141,l_faj.faj32,
                            l_faj.faj30,l_faj.faj31,l_faj.faj62,l_faj.faj67,l_faj.faj65,
                            l_faj.faj66,l_faj.faj62+l_faj.faj63,l_faj.faj67,l_faj.faj65,l_faj.faj66,l_faj.faj29,l_faj.faj29,    #CHI-FA0016 add faj29
                            l_faj142,l_faj322,l_faj302,l_faj312,l_faj142,
                            l_faj322,l_faj302,l_faj312,g_legal)
      #--MOD-E20131 add end--
       IF STATUS  THEN
...
FUNCTION t107_k()
   ...
   DECLARE t107_kkk_c CURSOR FOR
          SELECT fbd02, fbd03, fbd031, faj06, fbd04, fbd09, fbd05,   #No:BUG-480098,No.MOD-490181   #MOD-780022   #CHI-860025 取消mark
                fbd10, fbd25,fbd27,fbd06, fbd11, fbd07, fbd12, fbd14,   #CHI-FA0016 add fbd25,fbd27
                fbd15, fbd26,fbd16, fbd17, fbd19, fbd20, fbd28,fbd21,   #CHI-FA0016 add fbd26,fbd28
                fbd22
           FROM fbd_file LEFT OUTER JOIN faj_file ON fbd03 = faj02 AND fbd031 = faj022
            WHERE fbd01 = g_fbc.fbc01
          ORDER BY 1
   ...
   FOREACH t107_kkk_c INTO g_fbd[k].*
     ...
     #CHI-FA0016---add---str--
      #舊資料會沒有值
      IF STATUS != 100 AND cl_null(g_fbd[k].fbd26) THEN
         IF g_fbc.fbcpost2 = 'Y' THEN
            SELECT fap84,fap94 INTO g_fbd[k].fbd26,g_fbd[k].fbd28
              FROM fap_file
             WHERE fap02 = g_fbd[k].fbd03
               AND fap021 = g_fbd[k].fbd031
               AND fap50 = g_fbc.fbc01
         ELSE
            SELECT faj64 INTO g_fbd[k].fbd26
              FROM faj_file
             WHERE faj02 = g_fbd[k].fbd03
               AND faj022 = g_fbd[k].fbd031
            LET g_fbd[k].fbd28 = g_fbd[k].fbd26
         END IF
      END IF
     #CHI-FA0016---add---end--
      #No.MOD-AC0055  --Begin
   ...
   INPUT ARRAY g_fbd WITHOUT DEFAULTS FROM s_fbd.*
     ...
     #CHI-FA0016---add---str--
      AFTER FIELD fbd21
         IF g_fbd[l_ac].fbd21 > g_fbd[l_ac].fbd28 THEN
            CALL cl_err('','afa-214',1)
            NEXT FIELD fbd21
         END IF
     #CHI-FA0016---add---end--
...
#               LET l_fbd22 = (g_fbd[i].fbd19 - g_fbd[i].fbd20) * l_fab23 /100  #MOD-A40033 mark
      ...
      ON ROW CHANGE
          ...
          ELSE
             UPDATE fbd_file SET fbd19 = g_fbd[i].fbd19,
                                 ...
                                 fbd21 = g_fbd[i].fbd21,
                                 fbd22 = g_fbd[i].fbd22
                                 ,fbd28 = g_fbd[i].fbd28   #CHI-FA0016 add
                      WHERE fbd01 = g_fbc.fbc01 AND fbd02 = g_fbd_t.fbd02
             IF STATUS THEN
                  CALL cl_err3("upd","fbd_file",g_fbc.fbc01,g_fbd_t.fbd02,SQLCA.sqlcode,"","upd fbd:",1)  #No.FUN-660136
                  LET g_success = 'N'
     ...
     #CHI-FA0016---add---str--
      AFTER INPUT
         ...
         FOR l_i = 1 TO g_fbd.getLength()
            LET g_success = 'Y'
            IF g_fbd[l_i].fbd21 > g_fbd[l_i].fbd28 THEN
               CALL s_errmsg('fbd21',g_fbd[l_i].fbd21,'','afa-214',1)
               LET g_success = 'N'
               LET g_totsuccess = 'N'
               CONTINUE FOR
            END IF
...
DEFINE l_cnt LIKE type_file.num5         #No.FUN-680070 SMALLINT
   ...
   #FUN-B50090 add -end--------------------------
   #-->立帳日期小於關帳日期
  #IF g_fbc.fbc02 < g_faa.faa09 THEN   #CHI-E20004 mark
   IF g_fbc.fbc02 <=g_faa.faa09 THEN   #CHI-E20004
      CALL cl_err(g_fbc.fbc01,'aap-176',1) RETURN
   END IF
    #-----No:FUN-B60140-----
   IF g_faa.faa31 = "Y" THEN
      #-->立帳日期小於關帳日期
     #IF g_fbc.fbc02 < g_faa.faa092 THEN   #CHI-E20004 mark
      IF g_fbc.fbc02 <=g_faa.faa092 THEN   #CHI-E20004
         CALL cl_err(g_fbc.fbc01,'aap-176',1) RETURN
      END IF
   END IF
   #-----No:FUN-B60140 END-----
...
#FUN-AB0088---add---end---		  
  ...
  DEFINE l_cnt2   LIKE type_file.num5         #MOD-E10114
  DEFINE l_i      LIKE type_file.num5         #CHI-FA0016 add
  DEFINE l_npq07_d    LIKE npq_file.npq07     #MOD-G10102 add
  DEFINE l_npq07_c    LIKE npq_file.npq07     #MOD-G10102 add
  DEFINE l_faj141     LIKE faj_file.faj141    #MOD-G10102 add
...
#CHI-C30107 --------------- add ---------------- begin
  ...
  IF g_fbc.fbcconf = 'X' THEN
     ...
     RETURN
  END IF
  CALL t107_chk_frozen()                #CHI-E60034
...
#CHI-C30107 --------------- add ---------------- end
  ...
  #MOD-G10102---add---str--
   ...
   IF g_fah.fahdmy3 = 'Y' THEN
      ...
      SELECT SUM(fbd04),SUM(fbd05),SUM(fbd09),SUM(fbd10),SUM(faj141) INTO l_fbd04,l_fbd05,l_fbd09,l_fbd10,l_faj141
        FROM fbd_file,fbc_file,faj_file
       WHERE fbd01 = fbc01
         AND fbd03 = faj02 AND fbd031=faj022
         AND fbc01 = g_fbc.fbc01 
      IF cl_null(l_fbd04) THEN LET l_fbd04 = 0 END IF
      IF cl_null(l_fbd05) THEN LET l_fbd05 = 0 END IF
      IF cl_null(l_fbd09) THEN LET l_fbd09 = 0 END IF
      IF cl_null(l_fbd10) THEN LET l_fbd10 = 0 END IF
      IF cl_null(l_faj141) THEN LET l_faj141 = 0 END IF
      LET l_fbd09 = l_fbd09 - l_fbd04 - l_faj141
      LET l_fbd10 = l_fbd10 - l_fbd05
     #TQC-G20003---add---str--
      IF l_fbd09 < 0 THEN 
         LET l_fbd09 = l_fbd09 * -1
      END IF
      IF l_fbd10 < 0 THEN
         LET l_fbd10 = l_fbd10 * -1
      END IF
     #TQC-G20003---add---end-- 
      IF l_fbd09+l_fbd10 <> l_npq07_d OR l_fbd09+l_fbd10 <> l_npq07_c THEN
          LET g_success = 'N'
   ...
   IF g_faa.faa31='Y' AND g_fah.fahdmy32 = 'Y' THEN
      LET l_npq07_d = 0
      LET l_npq07_c = 0
      LET l_fbd042 = 0
      LET l_fbd052 = 0
      ...
      IF cl_null(l_npq07_d) THEN LET l_npq07_d = 0 END IF
      IF cl_null(l_npq07_c) THEN LET l_npq07_c = 0 END IF
      SELECT SUM(fbd042),SUM(fbd052),SUM(fbd092),SUM(fbd102) INTO l_fbd042,l_fbd052,l_fbd092,l_fbd102
        FROM fbd_file,fbc_file
       WHERE fbd01 = fbc01
         AND fbc01 = g_fbc.fbc01
      IF cl_null(l_fbd042) THEN LET l_fbd042 = 0 END IF
      IF cl_null(l_fbd052) THEN LET l_fbd052 = 0 END IF
      IF cl_null(l_fbd092) THEN LET l_fbd092 = 0 END IF
      IF cl_null(l_fbd102) THEN LET l_fbd102 = 0 END IF
      LET l_fbd092 = l_fbd092 - l_fbd042
      LET l_fbd102 = l_fbd102 - l_fbd052
     #TQC-G20003---add---str--
      IF l_fbd092 < 0 THEN
         LET l_fbd092 = l_fbd092 * -1
      END IF
      IF l_fbd102 < 0 THEN
         LET l_fbd102 = l_fbd102 * -1
      END IF
     #TQC-G20003---add---end--
      IF l_fbd092+l_fbd102 <> l_npq07_d OR l_fbd092+l_fbd102 <> l_npq07_c THEN
          LET g_success = 'N'
          CALL cl_err('','afa-154',1)
          RETURN
      END IF
 ...
 #IF g_fbc.fbc02 < g_faa.faa09 THEN   #CHI-E20004 mark
  ...
  FOR l_cnt2 = 1 TO g_rec_b
     ...
     SELECT faj100 INTO l_faj100 FROM faj_file
         WHERE faj02  = g_fbd[l_cnt2].fbd03
           AND faj022 = g_fbd[l_cnt2].fbd031
     IF l_faj100 > g_fbc.fbc02 THEN
        LET g_success = 'N'
        CALL cl_err(g_fbc.fbc02,'afa-113',0)
        RETURN
     END IF
  END FOR
  #-MOD-E10114-end--
   ...
   FOREACH t107_fbd_cur INTO l_fbd.*
      SELECT count(*) INTO l_cnt FROM fbd_file,fbc_file
       WHERE fbd01 = fbc01
         ...
         AND fbd031= l_fbd.fbd031     #AND fbc02 <= g_fbc.fbc02   #MOD-D60067 mark
         AND fbcpost = 'N'
         AND fbc01 != g_fbc.fbc01
         AND fbcconf <> 'X'
      IF l_cnt  > 0 THEN
...
#FUN-C30313---add---START-----
     ...
     #IF l_n = 0 THEN                                                                     #MOD-C50255 mark
      ...
      IF g_success = 'Y' THEN
         ...
         END IF
      END IF
     #IF g_success = 'N' THEN RETURN END IF    #MOD-GA0025 mark
     #MOD-GA0025---add---start---
      IF g_success = 'N' THEN 
         ROLLBACK WORK
         RETURN 
      END IF
...
######################################
  ...
  THEN
     LET g_success='Y'
    #LET g_fbc.fbc08='S' #FUN-F10019 mark
    #FUN-F10019 add(S)
    #重新更新狀態碼顯示
     SELECT fbc08 INTO g_fbc.fbc08 FROM fbc_file WHERE fbc01 = g_fbc.fbc01
    #FUN-F10019 add(E)
     DISPLAY BY NAME g_fbc.fbc08
  ELSE
     LET g_success='N'
...
#FUN-AB0088---add---end---   
   ...
   #CHI-CA0063---End---
  #DEFINE l_faj29      LIKE faj_file.faj29     #FUN-C90088   #CHI-FA0016 mark
  #DEFINE l_faj292     LIKE faj_file.faj292    #FUN-C90088   #CHI-FA0016 mark
  #DEFINE l_faj64      LIKE faj_file.faj64     #FUN-C90088   #CHI-FA0016 mark
  #---------------------CHI-CB0022--------------(S)
   DEFINE l_faj28      LIKE faj_file.faj28
   DEFINE l_faj30      LIKE faj_file.faj30
   DEFINE l_faj31      LIKE faj_file.faj31
   DEFINE l_faj33      LIKE faj_file.faj33
   DEFINE l_faj43      LIKE faj_file.faj43   #MOD-F50137 add
  #---------------------CHI-CB0022--------------(E)
   DEFINE l_fap08      LIKE fap_file.fap08   #MOD-F50137 add
   DEFINE l_fap54      LIKE fap_file.fap54   #MOD-E80046 add
   DEFINE l_fap542     LIKE fap_file.fap542  #MOD-E80046 add
...
#  IF g_aza.aza63 = 'Y' THEN
   ...
   END IF
   #No.CHI-E60034  --End   
#  LET l_yy = YEAR(g_fbc.fbc02)    #FUN-D70122  mark
#  LET l_mm = MONTH(g_fbc.fbc02)   #FUN-D70122  mark
   LET l_yy = s_get_aznn(g_plant,g_bookno1,g_fbc.fbc02,1)  #FUN-D70122  add
   LET l_yy = s_get_aznn(g_plant,g_bookno1,g_fbc.fbc02,3)  #FUN-D70122  add
   ...
   FOREACH t107_cur2 INTO l_fbd.*
      ...
      IF STATUS THEN
         ...
         LET g_success = 'N'
      END IF
     #LET l_fap661 = l_fbd.fbd09 - l_fbd.fbd04  #本幣成本       #MOD-E80046 mark
      LET l_fap55  = l_fbd.fbd10 - l_fbd.fbd05  #累折
      LET l_fap552 = l_fbd.fbd102 - l_fbd.fbd052  #FUN-AB0088 
     #LET l_fap6612 = l_fbd.fbd092 - l_fbd.fbd042 #FUN-AB0088   #MOD-E80046 mark
      #--MOD-E80046 add start--
      LET l_fap54 = l_fbd.fbd09 - l_fbd.fbd04
      LET l_fap542 = l_fbd.fbd092 - l_fbd.fbd042
      LET l_fap661 = l_fbd.fbd04
      LET l_fap6612 = l_fbd.fbd042
      #--MOD-E80046 add end--
...
#FUN-AB0088---add---end---
     ...
     #CHI-C60010---str---
      ...
      CALL cl_digcut(l_fap542,g_azi04_1) RETURNING l_fap542   #MOD-E80046 add
     ...
     #CHI-FA0016---mark---end--
     #MOD-F50137---add---str--
      IF l_faj.faj43 = '7' THEN      #財簽折畢再提
         LET l_fap08 = l_faj.faj35
      ELSE 
         LET l_fap08 = l_faj.faj31    
      END IF
     #MOD-F50137---add---end--
      IF l_cnt = 0 THEN   #無fap_file資料
         INSERT INTO fap_file (fap01,fap02,fap021,fap03,fap04,
                               ...
                               fap55,fap52,fap53,fap54,fap711,             #No.MOD-A20122 add fap54
                               fap72,fap69,fap70,fap121,fap131,
                              #--MOD-E80046 mark start--
                              #fap141,fap77,fap56,  #CHI-9B0032 add fap77  #TQC-B30156 add fap56
                              ##FUN-AB0088----add----str-----------      
                              ...
                              #--MOD-E80046 mark end--
                              #--MOD-E80046 add start--
                               fap141,fap77,fap56,fap052,fap062,
                               fap072,fap082,fap092,fap103,fap1012,
                               fap112,fap152,fap162,fap212,fap222,
                               fap232,fap242,fap252,fap262,fap522,
                               fap532,fap542,fap552,fap6612,fap562,
                               fap772,fap83,fap832,fap84,fap93,
                               fap932,fap94,faplegal)
                              #--MOD-E80046 add end--
        #--MOD-E80046 mark start--
        ...
        #--MOD-E80046 add start--
            VALUES (l_faj.faj01,l_fbd.fbd03,l_fbd.fbd031,'9',g_fbc.fbc02,
                   #l_faj.faj43,l_faj.faj28,l_faj.faj30,l_faj.faj31,l_faj.faj14,   #MOD-F50137 mark
                    l_faj.faj43,l_faj.faj28,l_faj.faj30,l_fap08,l_faj.faj14,       #MOD-F50137 add
                    ...
                    l_fap55,l_fbd.fbd11,l_fbd.fbd12,l_fap54,0,
                    0,l_fbd.fbd21,l_fbd.fbd22,l_faj.faj531,l_faj.faj541,
                    l_faj.faj551,l_faj.faj43,0,l_faj.faj432,l_faj.faj282,
                    ...
                    l_fbd.fbd122,l_fap542,l_fap552,l_fap6612,0,
                   #l_faj.faj432,l_faj.faj29,l_faj.faj292,l_faj.faj64,l_faj29,       #CHI-FA0016 mark
                   #l_faj292,l_faj64,g_legal)                                        #CHI-FA0016 mark
                    l_faj.faj432,l_faj.faj29,l_faj.faj292,l_faj.faj64,l_fbd.fbd27,   #CHI-FA0016 add 
                    l_fbd.fbd272,l_fbd.fbd28,g_legal)                                #CHI-FA0016 add
        #--MOD-E80046 add end-- 
        ELSE
         UPDATE fap_file SET fap05 = l_faj.faj43,
                             fap06 = l_faj.faj28,
                             fap07 = l_faj.faj30,
                            #fap08 = l_faj.faj31,   #MOD-F50137 mark
                             fap08 = l_fap08,       #MOD-F50137 add
                             fap09 = l_faj.faj14,
                             fap10 = l_faj.faj141,
                             ...
                             fap52 = l_fbd.fbd11,
                             fap53 = l_fbd.fbd12,
                            #fap54 = l_fbd.fbd09-l_fbd.fbd04-l_faj.faj141,  #TQC-BB0012 mod fbd02->fbd04   #MOD-E80046 mark
                             fap54 = l_fap54,   #MOD-E80046 add
                             fap55 = l_fap55,
                             fap661= l_fap661,
                             fap56 = 0
                            #CHI-FA0016---add---str--
      ...
      IF l_faj.faj43 = '7' THEN     #折畢再提
        UPDATE faj_file SET
              ...
              #faj29  = l_faj29,                                 #FUN-C90088 耐用年限   #CHI-FA0016 mark
               faj29  = l_fbd.fbd27,                                                    #CHI-FA0016 add 
      ...
      END IF
     #-----------------------------CHI-CB0022------------------------(S)
     #資產折舊月份和累計折舊調整為折畢，需調整資產狀態
      SELECT faj28,faj30,faj31,faj33,faj43             #MOD-F50137 add faj43
        INTO l_faj28,l_faj30,l_faj31,l_faj33,l_faj43   #MOD-F50137 add l_faj43
        FROM faj_file
       WHERE faj02 = l_fbd.fbd03
         AND faj022= l_fbd.fbd031
      IF l_faj43 <> '7' THEN                           #MOD-F50137 add
         IF (l_faj31 = l_faj33) OR (l_faj30 = 0 AND l_faj28 <> '0') THEN
      ...
      END IF
       ...
       INSERT INTO fan_file(fan01,fan02,fan03,fan04,fan041,fan05,fan06, #No:BUG-470041 #No.MOD-470565
           ...
           VALUES (l_fbd.fbd03,l_fbd.fbd031,g_yy,g_mm,'2',l_faj.faj23,  #No.TQC-7B0060 l_yy->g_yy l_mm->g_mm
                   ...
                   l_faj.faj55,l_faj.faj24,l_fbd.fbd09,l_fbd.fbd10,1,l_faj.faj101,0,         #CHI-E60048 
...
FUNCTION t107_6()
   DEFINE l_fbd       RECORD LIKE fbd_file.*,
       ...
       #  l_fap6612   LIKE fap_file.fap6612,   #FUN-B60140   Add #No:FUN-BA0112 mark
          ...
          l_fap71     LIKE fap_file.fap71,     #MOD-E20131 add
  ...
  #DEFINE l_faj292    LIKE faj_file.faj292        #FUN-C90088   #CHI-FA0016 mark
  #DEFINE l_faj64     LIKE faj_file.faj64         #FUN-C90088   #CHI-FA0016 mark
   DEFINE l_fap32     LIKE fap_file.fap32         #MOD-F50137 add
  ...
  # IF g_fbc.fbcconf != 'Y' OR g_fbc.fbcpost != 'Y' OR g_fbc.fbcpost2!= 'N' THEN   #FUN-580109
   ...
   #FUN-B50090 add -end--------------------------
   #--->折舊年月判斷
  #IF g_fbc.fbc02 < g_faa.faa13 THEN   #CHI-E20004 mark
   IF g_fbc.fbc02 <=g_faa.faa13 THEN   #CHI-E20004
      CALL cl_err(g_fbc.fbc02,'afa-308',1)   #MOD-910240 mod
      RETURN
   END IF
   #No.CHI-E60034  --Begin
   ...
   FOREACH t107_cur21 INTO l_fbd.*
      ...
      IF SQLCA.sqlcode THEN
         ...
         EXIT FOREACH
      END IF
     #LET l_fap711 = l_fbd.fbd19 - l_fbd.fbd14  #MOD-E20131 mark   #稅簽本幣成本
      LET l_fap71  = l_fbd.fbd19 - l_fbd.fbd14  #MOD-E20131 add
  ...
  #   LET l_fap6612 = l_fbd.fbd092 - l_fbd.fbd042    #FUN-B60140    Add	#No:FUN-BA0112 mark
     ...
     #CHI-FA0016---mark---end--
      IF l_cnt = 0 THEN   #無fap_file資料
        ...
        #--MOD-C70198--mark
        #MOD-F50137---add---str--
         IF l_faj.faj201 = '7' THEN      #稅簽折畢再提
            LET l_fap32 = l_faj.faj72
         ELSE
            LET l_fap32 = l_faj.faj66
         END IF
        #MOD-F50137---add---end--
         INSERT INTO fap_file (fap01,fap02,fap021,fap03,fap04,
                               fap05,fap06,fap07,fap08,fap09,
                               ...
                               fap562,fap772,faplegal,fap42,              #CHI-CB0022 add fap42
                               fap83,fap832,fap84,   #FUN-C90088
                               fap93,fap932,fap94    #FUN-C90088
                               )                  
         VALUES (l_faj.faj01,l_fbd.fbd03,l_fbd.fbd031,'9',g_fbc.fbc02,
                 ...
                 l_faj.faj21,l_faj.faj17,l_faj.faj171,l_faj.faj58,l_faj.faj59,
                 l_faj.faj60,l_faj.faj34,l_faj.faj35,l_faj.faj36,l_faj.faj61,
                #l_faj.faj65,l_faj.faj66,l_faj.faj62,l_faj.faj63,l_faj.faj68,   #MOD-F50137 mark
                 l_faj.faj65,l_fap32,l_faj.faj62,l_faj.faj63,l_faj.faj68,       #MOD-F50137 add
                 l_faj.faj67,l_faj.faj69,l_faj.faj70,l_faj.faj71,l_faj.faj72,
                 l_faj.faj73,l_faj.faj100,l_fbd.fbd01,l_fbd.fbd02,l_fap661,
                 l_fap55,l_fbd.fbd11,l_fbd.fbd12,l_fbd.fbd09-l_fbd.fbd04-l_faj.faj141,l_fap711,   #MOD-C70198 0 mod l_fap711
                #0,l_fbd.fbd21,l_fbd.fbd22,l_fbd.fbd19,l_faj.faj531,   #MOD-E20131 mark
                 0,l_fbd.fbd21,l_fbd.fbd22,l_fap71,l_faj.faj531,       #MOD-E20131 add
                 ...
                 0,l_faj.faj432,g_legal,l_faj.faj201,                                   #CHI-CB0022 add faj201
                 l_faj.faj29,l_faj.faj292,l_faj.faj64,  #FUN-C90088
                #l_faj29,l_faj292,l_faj64               #FUN-C90088   #CHI-FA0016 mark
                 l_fbd.fbd27,l_fbd.fbd272,l_fbd.fbd28                 #CHI-FA0016 add
                 )                             
        #-----------------------------------MOD-C70198-----------------------------(E)
                 
    ELSE
     #UPDATE fap_file SET fap711 = l_fap711,   #MOD-E20131 mark
      UPDATE fap_file SET fap71  = l_fap71,    #MOD-E20131 add
                          fap72  = l_fap72,
                          ...
                          #  fap32  = l_faj66                   #預留殘值   #No:FUN-B60140 #No:FUN-BA0112 mark
                            ...
                            #fap32  = l_faj.faj66,              #預留殘值    #No:FUN-BA0112 add   #MOD-F50137 mark
                             fap32  = l_fap32,                                                    #MOD-F50137 add
                             fap42  = l_faj.faj201              #資產狀態 #CHI-CB0022 add
                             ,fap84  = l_faj.faj64              #CHI-FA0016 add
                             ,fap94  = l_fbd.fbd28              #CHI-FA0016 add  
      ...
      END IF
      #--->(1.2)更新資產主檔
     #IF l_faj43 = '7' THEN    #折畢再提         #MOD-F50137 mark
      IF l_faj.faj201 = '7' THEN                 #MOD-F50137 add
         UPDATE faj_file SET
               ...
               #faj64  = l_faj64,                           #耐用年限 #FUN-C90088   #CHI-FA0016 mark
                faj64  = l_fbd.fbd28,                                               #CHI-FA0016 add
                faj72  = l_fbd.fbd22,                       #預留殘值
               #faj63  = l_fap711                           #MOD-E20131 mark #MOD-C70198 add
                faj63  = l_fap71                            #MOD-E20131 add
          WHERE faj02  = l_fbd.fbd03 AND faj022=l_fbd.fbd031
      ELSE
         UPDATE faj_file SET
               #faj62  = l_fbd.fbd19,                       #稅簽成本 #MOD-C70198 mark
     ...
     #資產折舊月份和累計折舊調整為折畢，需調整資產狀態
      IF l_faj.faj201 <> '7' THEN   #MOD-F50137 add
         IF (l_faj.faj66 = l_faj.faj68) OR (l_faj.faj65 = 0 AND l_faj.faj61 <> '0') THEN
...
FUNCTION t107_w()
  ...
  #DEFINE l_sql    LIKE type_file.chr1000             #No.FUN-680028       #No.FUN-680070 VARCHAR(1000)   #TQC-E80008 mark
   DEFINE l_sql    STRING                                                                                 #TQC-E80008 add
...
#FUN-AB0088---add---end---
   ...
   DEFINE l_fap05  LIKE fap_file.fap05     #CHI-CB0022 add
   ...
   FOREACH t107_cur3 INTO l_fbd.*
      ...
      #----- 找出 fap_file 之 fap05 以便 update faj_file.faj43
       SELECT fap10,fap11,fap101,fap07,fap08,fap41                #No.MOD-490181 改fap10
             ,fap83,fap05                                             #FUN-C90088 #CHI-CB0022 add fap05
        INTO l_fap10,l_fap11,l_fap101,l_fap07,l_fap08,l_fap41
             ,l_fap83,l_fap05                                         #FUN-C90088 #CHI-CB0022 add fap05
        FROM fap_file
...
FUNCTION t107_7()
   DEFINE l_fbd     RECORD LIKE fbd_file.*,
          ...
          l_fap34   LIKE fap_file.fap34,   #MOD-E20131 add
   ...
   DEFINE l_fap42   LIKE fap_file.fap42                  #CHI-CB0022 add
  ...
  #IF g_fbc.fbc02 < g_faa.faa13 THEN   #CHI-E20004 mark
   ...
   FOREACH t107_cur31 INTO l_fbd.*
     ...
     #SELECT fap33,fap35,fap341,fap31,fap32,fap711,              #MOD-E20131 mark   #MOD-C70198 add
      SELECT fap33,fap35,fap341,fap31,fap32,fap34,               #MOD-E20131 add
             fap84,fap42                                         #FUN-C90088 #CHI-CB0022 add fap42
     #  INTO l_fap33,l_fap35,l_fap341,l_fap31,l_fap32,l_fap711,  #MOD-E20131 mark   #MOD-C70198 add
        INTO l_fap33,l_fap35,l_fap341,l_fap31,l_fap32,l_fap34,   #MOD-E20131 add
             l_fap84,l_fap42                                     #FUN-C90088 #CHI-CB0022 add fap42
        FROM fap_file
       WHERE fap50=l_fbd.fbd01
         AND fap501=l_fbd.fbd02
         AND fap03='9'    ## 異動代號
      ...
      END IF
      #--->(1.1)更新資產主檔
     #IF l_faj.faj43 = '7' THEN   #折畢再提   #MOD-F50137 mark
      IF l_faj.faj201 = '7' THEN              #MOD-F50137 add
         UPDATE faj_file SET
                ...
                faj72  = l_fap32,                                 #預估殘值
               #faj63  = l_fap711,                                #MOD-E20131 mark   #MOD-C70198 add
                faj63  = l_fap34,                                 #MOD-E20131 add
                faj201 = l_fap42                                  #資產狀態#CHI-CB0022
          WHERE faj02  = l_fbd.fbd03 AND faj022=l_fbd.fbd031
      ELSE
         UPDATE faj_file SET
 ...
 DEFINE ls_tmp STRING
   ...
   OPEN WINDOW t107_e1 AT p_row,p_col WITH FORM "afa/42f/afat1071"
    ...
    CALL cl_ui_locale("afat1071")
      ...
      SELECT fbd14,fbd15,fbd26,fbd16,fbd17,fbd19,fbd20,fbd28,fbd21,fbd22        #CHI-FA0016 add fbd26,fbd28
        INTO g_e[l_ac].fbd14,g_e[l_ac].fbd15,g_e[l_ac].fbd26,g_e[l_ac].fbd16,   #CHI-FA0016 add fbd26
             g_e[l_ac].fbd17,g_e[l_ac].fbd19,g_e[l_ac].fbd20,
             g_e[l_ac].fbd28,g_e[l_ac].fbd21,g_e[l_ac].fbd22                    #CHI-FA0016 add fbd26
        FROM fbd_file
       WHERE fbd01 = g_fbc.fbc01
         AND fbd02 = g_fbd[l_ac].fbd02
      IF SQLCA.sqlcode THEN
         ...
         LET g_e[l_ac].fbd26 = 0   LET g_e[l_ac].fbd28 = 0   #CHI-FA0016 add
      END IF
  ...
  #CHI-FA0016---add---str--
   #舊資料會沒有值
   IF STATUS != 100 AND cl_null(g_e[l_ac].fbd26) THEN
      IF g_fbc.fbcpost2 = 'Y' THEN
         SELECT fap84,fap94 INTO g_e[l_ac].fbd26,g_e[l_ac].fbd28
           FROM fap_file
          WHERE fap02 = g_fbd[l_ac].fbd03
            AND fap021 = g_fbd[l_ac].fbd031
            AND fap50 = g_fbc.fbc01
      ELSE
         SELECT faj64 INTO g_e[l_ac].fbd26
           FROM faj_file
          WHERE faj02 = g_fbd[l_ac].fbd03
            AND faj022 = g_fbd[l_ac].fbd031
         LET g_e[l_ac].fbd28 = g_e[l_ac].fbd26
      END IF
   END IF
   DISPLAY g_e[l_ac].fbd26 TO fbd26
   DISPLAY g_e[l_ac].fbd28 TO fbd28
  #CHI-FA0016---add---end--
 
   INPUT g_e[l_ac].fbd19,g_e[l_ac].fbd20,g_e[l_ac].fbd28,g_e[l_ac].fbd21,g_e[l_ac].fbd22   #CHI-FA0016 add fbd28
          WITHOUT DEFAULTS
    FROM fbd19,fbd20,fbd28,fbd21,fbd22   #CHI-FA0016 add fbd28
   IF INT_FLAG THEN LET INT_FLAG = 0 CLOSE WINDOW t107_e1 RETURN END IF

     #CHI-FA0016---add---str--
      IF g_e[l_ac].fbd21 > g_e[l_ac].fbd28 THEN
         CALL cl_err('','afa-214',1)
         RETURN 
      END IF
     #CHI-FA0016---add---end--
      ...
      UPDATE fbd_file SET
                  ...
                  ,fbd26=g_e[l_ac].fbd26,fbd28=g_e[l_ac].fbd28   #CHI-FA0016 add
...
FUNCTION t107_x(p_type)                  #FUN-D20035
   ...
   IF g_fbc.fbcconf='X' AND NOT cl_null(g_faa.faa09) AND g_fbc.fbc02<= g_faa.faa09 THEN
   ...
   END IF
  #-->作廢轉換01/08/01
 # Prog. Version..: '5.30.26-17.12.13(0,0,g_fbc.fbcconf)   THEN        #FUN-D20035
...
#No:FUN-C60006---add--star---
      LET g_fbc.fbcmodu = g_user
      LET g_fbc.fbcdate = g_today
      DISPLAY BY NAME g_fbc.fbcmodu
      DISPLAY BY NAME g_fbc.fbcdate
  ...
  DEFINE p_fah     RECORD LIKE fah_file.*
   ...
   #MOD-GA0025---mark---end---
    IF g_fah.fahdmy3 = 'Y' AND g_fah.fahglcr = 'Y' THEN #FUN-C30313 add
      #MOD-GA0025---add---start--- 
       IF cl_null(p_fah.fahgslp) THEN
          CALL s_errmsg('','',p_fbc.fbc01,'axr-070',1)
          LET g_success = 'N'
          RETURN
       END IF       
      #MOD-GA0025---add---end---
...
FUNCTION t107_undo_carry_voucher() 
 ...
 #DEFINE l_sql      LIKE type_file.chr1000      #No.FUN-680070 VARCHAR(1000)   #TQC-E80008 mark
  DEFINE l_sql      STRING                                                     #TQC-E80008 add
  DEFINE l_dbs      STRING
...
FUNCTION t107_fin_audit2()
   ...
   DEFINE l_faj292   LIKE faj_file.faj292   #CHI-FA0016 add
   DEFINE l_faj302   LIKE faj_file.faj302
   DEFINE l_faj312   LIKE faj_file.faj312
   DEFINE l_faj352   LIKE faj_file.faj352   #MOD-F50137 add
   DEFINE l_faj432   LIKE faj_file.faj432   #MOD-F50137 add
   ...
   LET g_sql =
        ...
        "       fbd252,fbd272,fbd062,fbd112,fbd072,fbd122 ",   #CHI-FA0016 add fbd252,fbd272
        "  FROM fbd_file ",
        " WHERE fbd01  ='",g_fbc.fbc01,"'",
        " ORDER BY 1"   
   #FUN-BC0004--add--end
         ...
         BEFORE ROW            
            ...
            IF l_rec_b2 >= l_ac THEN 
               ...
               ELSE
                  ...
                  ELSE
                     SELECT faj06 #,faj142,faj322,faj292,faj302,faj312 #FUN-BB0122 mark   #CHI-FA0016 add faj292 
                       INTO g_fbd2[l_ac].faj06 #,l_faj142,l_faj322,l_faj292,l_faj302,l_faj312 #FUN-BB0122 mark   #CHI-FA0016 add l_faj292
                       FROM faj_file
            ...
            END IF
        #FUN-BC0004--add-str
       #BEFORE FIELD fbd042   #TQC-E80008 mark
        BEFORE FIELD fbd092   #TQC-E80008 add
           SELECT faj06,faj142,faj322,faj302,faj312,                        #MOD-F50137 add ,
                  faj352,faj432                                             #MOD-F50137 add
             INTO g_fbd2[l_ac].faj06,l_faj142,l_faj322,l_faj302,l_faj312,   #MOD-F50137 add ,
                  l_faj352,l_faj432                                         #MOD-F50137 add
             FROM faj_file
            WHERE faj02  = g_fbd2[l_ac].fbd03
              AND faj022 = g_fbd2[l_ac].fbd031
          #MOD-F50137---add---str--
          ...
          #MOD-F50137---add---end--
           ...
           END IF
          #CHI-FA0016---add---str--
           IF cl_null(g_fbd2[l_ac].fbd252) OR g_fbd2[l_ac].fbd252 = 0 THEN
              LET g_fbd2[l_ac].fbd252 = l_faj292
           END IF
          #CHI-FA0016---add---end--
           IF cl_null(g_fbd2[l_ac].fbd062) OR g_fbd2[l_ac].fbd062 = 0 THEN
              LET g_fbd2[l_ac].fbd062 = l_faj302
           ...
           END IF
          #CHI-FA0016---add---str--
           IF cl_null(g_fbd2[l_ac].fbd272) OR g_fbd2[l_ac].fbd272 = 0 THEN
              LET g_fbd2[l_ac].fbd272 = l_faj292
           END IF
          #CHI-FA0016---add---end-- 
           IF cl_null(g_fbd2[l_ac].fbd112) OR g_fbd2[l_ac].fbd112 = 0 THEN
              LET g_fbd2[l_ac].fbd112 = l_faj302
          ...
          #CHI-C60010---end---
           ...
           DISPLAY BY NAME g_fbd2[l_ac].fbd112
           DISPLAY BY NAME g_fbd2[l_ac].fbd122
           DISPLAY BY NAME g_fbd2[l_ac].fbd252   #CHI-FA0016 add 
           DISPLAY BY NAME g_fbd2[l_ac].fbd272   #CHI-FA0016 add
        #FUN-BC0004--add-end
        AFTER FIELD fbd092
        ...
        AFTER FIELD fbd112
           ...
           END IF
          #CHI-FA0016---add---str--
           IF g_fbd2[l_ac].fbd112 > g_fbd2[l_ac].fbd272 THEN
              CALL cl_err('','afa-214',1)
              NEXT FIELD fbd112
           END IF
          #CHI-FA0016---add---end--
         ...
         ON ROW CHANGE
          ...
          #CHI-C60010---end--- 
           UPDATE fbd_file SET fbd042 = g_fbd2[l_ac].fbd042,
                               ...
                               ,fbd252 = g_fbd2[l_ac].fbd252   #CHI-FA0016 add
                               ,fbd272 = g_fbd2[l_ac].fbd272   #CHI-FA0016 add
             WHERE fbd01 = g_fbc.fbc01
        ...
        #CHI-FA0016---add---str--
         AFTER INPUT
            CALL s_showmsg_init()
            LET g_totsuccess = 'Y'
            FOR l_i = 1 TO g_fbd2.getLength()
               LET g_success = 'Y'
               IF g_fbd2[l_i].fbd112 > g_fbd2[l_i].fbd272 THEN
                  CALL s_errmsg('fbd112',g_fbd2[l_i].fbd112,'','afa-214',1)
                  LET g_success = 'N'
                  LET g_totsuccess = 'N'
                  CONTINUE FOR
               END IF
...
FUNCTION t107_s2()
  ...
  #DEFINE l_faj292     LIKE faj_file.faj292    #FUN-C90088   #CHI-FA0016 mark
  #DEFINE l_faj64      LIKE faj_file.faj64     #FUN-C90088   #CHI-FA0016 mark
   DEFINE l_faj282    LIKE faj_file.faj282    #CHI-CB0022 add
   DEFINE l_faj302    LIKE faj_file.faj302    #CHI-CB0022 add
   DEFINE l_faj312    LIKE faj_file.faj312    #CHI-CB0022 add
   DEFINE l_faj332    LIKE faj_file.faj332    #CHI-CB0022 add
   DEFINE l_faj432    LIKE faj_file.faj432    #MOD-F50137 add
   DEFINE l_fap082    LIKE fap_file.fap082    #MOD-F50137 add
   DEFINE l_fap54     LIKE fap_file.fap54     #MOD-E80046 add
   DEFINE l_fap542    LIKE fap_file.fap542    #MOD-E80046 add
...
#  LET l_mm = MONTH(g_fbc.fbc02)      #FUN-D70122 mark
   ...
   FOREACH t107_cur22 INTO l_fbd.*
     ...
     #LET l_fap6612 = l_fbd.fbd092 - l_fbd.fbd042           #MOD-E80046 mark
      #--MOD-E80046 add start--
      LET l_fap54 = l_fbd.fbd09 - l_fbd.fbd04
      LET l_fap542 = l_fbd.fbd092 - l_fbd.fbd042
      LET l_fap661 = l_fbd.fbd04
     ...
     #CHI-FA0016---mark---end--
     #MOD-F50137---add---str--
      IF l_faj.faj432 = '7' THEN     #財簽二折畢再提 
         LET l_fap082 = l_faj.faj352
      ELSE
         LET l_fap082 = l_faj.faj312
      END IF
     #MOD-F50137---add---end--
      IF l_cnt = 0 THEN   #無fap_file資料
         INSERT INTO fap_file (fap01,fap02,fap021,fap03,fap04,
                             ...
                             #--MOD-E80046 mark end--
                             #--MOD-E80046 add start--
                               fap562,fap772,faplegal,fap83,fap832,
                               fap84,fap93,fap932,fap94
                             #--MOD-E80046 add end--
                              )
         VALUES (l_faj.faj01,l_fbd.fbd03,l_fbd.fbd031,'9',g_fbc.fbc02,
                 l_faj.faj43,l_faj.faj28,l_faj.faj30,l_faj.faj31,l_faj.faj14,   
                 ...
                 l_faj.faj541,l_faj.faj551,l_faj.faj43,l_faj.faj432,l_faj.faj282,
                #l_faj.faj302,l_faj.faj312,l_faj.faj142,l_faj.faj1412,l_faj.faj332+l_faj.faj3312,   #MOD-F50137 mark
                 l_faj.faj302,l_fap082,l_faj.faj142,l_faj.faj1412,l_faj.faj332+l_faj.faj3312,       #MOD-F50137 add
                 l_faj.faj322,l_faj.faj232,l_faj.faj242,l_faj.faj582,l_faj.faj592,
                 l_faj.faj602,l_faj.faj342,l_faj.faj352,l_faj.faj362,l_fbd.fbd112,
                 l_fbd.fbd122,l_fap542,l_fap552,l_fap6612,0,
               #--MOD-E80046 mark start--
               ...
               #--MOD-E80046 mark end--
               #--MOD-E80046 add start--
                 0,l_faj.faj432,g_legal,l_faj.faj29,l_faj.faj292,
                #l_faj.faj64,l_faj29,l_faj292,l_faj64   #CHI-FA0016 mark
                 l_faj.faj64,l_fbd.fbd27,l_fbd.fbd272,l_fbd.fbd28   #CHI-FA0016 add
               #--MOD-E80046 add end--
                 )
      ELSE
         UPDATE fap_file SET fap052 = l_faj.faj432,
                             fap062 = l_faj.faj282,
                             fap072 = l_faj.faj302,
                            #fap082 = l_faj.faj312,   #MOD-F50137 mark
                             fap082 = l_fap082,       #MOD-F50137 add
                             fap092 = l_faj.faj142,
                             ...
                             fap522 = l_fbd.fbd112,
                             fap532 = l_fbd.fbd122,
                            #fap542 = l_fbd.fbd092-l_fbd.fbd042-l_faj.faj1412,  #TQC-BB0012 mod fbd02->fbd042   #MOD-E80046 mark
                             fap542 = l_fap542,   #MOD-E80046 add
                             fap552 = l_fap552,
                             fap6612= l_fap6612,
                             fap562 = 0
                             ,fap832 = l_faj.faj292   #CHI-FA0016 add
                             ,fap932 = l_fbd.fbd272   #CHI-FA0016 add
                       WHERE fap50  = l_fbd.fbd01
                         AND fap501 = l_fbd.fbd02
                         AND fap03  = '9'              ## 異動代號
      END IF
      ...
      IF l_faj.faj432 = '7' THEN     #折畢再提
        UPDATE faj_file SET
              ...
              #faj292  = l_faj292,                                #FUN-C90088 耐用年限   #CHI-FA0016 mark
               faj292  = l_fbd.fbd272,                                                   #CHI-FA0016 add
     ...
     #--------------------CHI-CB0022----------------------(S)
     #資產折舊月份和累計折舊調整為折畢，需調整資產狀態
      SELECT faj282,faj302,faj312,faj332,faj432             #MOD-F50137 add faj432
        INTO l_faj282,l_faj302,l_faj312,l_faj332,l_faj432   #MOD-F50137 add l_faj432
        FROM faj_file
       WHERE faj02 = l_fbd.fbd03
         AND faj022= l_fbd.fbd031
      IF l_faj432 <> '7' THEN                               #MOD-F50137 add
         IF (l_faj312 = l_faj332) OR (l_faj302 = 0 AND l_faj282 <> '0') THEN
...
FUNCTION t107_w2()
   ...
   DEFINE l_aba19  LIKE aba_file.aba19
  #DEFINE l_sql    LIKE type_file.chr1000   #TQC-E80008 mark
   DEFINE l_sql    STRING                   #TQC-E80008 add
   DEFINE l_dbs    STRING
   ...
   DEFINE l_fap052 LIKE fap_file.fap052    #CHI-CB0022 add
  ...
  #---------------------------CHI-C90051--------------------(E)
   ...
   FOREACH t107_cur32 INTO l_fbd.*
      ...
      #----- 找出 fap_file 之 fap05 以便 update faj_file.faj43
       SELECT fap103,fap112,fap1012,fap072,fap082,  #No:FUN-B30036
              fap832,fap052                                        #FUN-C90088 #CHI-CB0022 add fap052
         INTO l_fap103,l_fap112,l_fap1012,l_fap072,l_fap082,  #No:FUN-B30036
              l_fap832,l_fap052                                    #FUN-C90088 #CHI-CB0022 add fap052
         FROM fap_file
        WHERE fap50=l_fbd.fbd01  AND fap501=l_fbd.fbd02 AND fap03='9'
      IF STATUS THEN
         LET g_showmsg = l_fbd.fbd01,"/",l_fbd.fbd02,"/",'9'
     ...
     #CHI-C60010---end--
      IF l_faj.faj432 = '7' THEN    #折畢再提
         UPDATE faj_file SET
                ...
                faj432  = l_fap052                            #資產狀態 #CHI-CB0022 add
          WHERE faj02  = l_fbd.fbd03 AND faj022=l_fbd.fbd031
      ELSE
         UPDATE faj_file SET
                faj1412 = l_fap103,    #調整成本
...
#No.CHI-E60034  --Begin
FUNCTION t107_chk_frozen()
   DEFINE l_fbd03      LIKE fbd_file.fbd03
   DEFINE l_fbd031     LIKE fbd_file.fbd031   
   DEFINE l_msg        STRING
   DEFINE l_msg1       STRING
   DEFINE l_msg2       STRING
   
   DECLARE t107_fbd_frozen CURSOR FOR
       SELECT fbd03,fbd031 FROM fbd_file
        WHERE fbd01 = g_fbc.fbc01
   FOREACH t107_fbd_frozen INTO l_fbd03,l_fbd031 
     IF SQLCA.sqlcode THEN EXIT FOREACH END IF 
     IF NOT cl_null(l_fbd03) THEN
     	  LET g_cnt = 0
        SELECT COUNT(*) INTO g_cnt FROM fca_file
         WHERE fca03  = l_fbd03
           AND fca031 = l_fbd031
           AND fca15  = 'N'
        IF g_cnt > 0 THEN          	
        	 IF NOT cl_null(l_fbd031) THEN 
        	    LET l_msg1=l_fbd03,'-',l_fbd031
        	 ELSE
        	    LET l_msg1=l_fbd03 
        	 END IF
   ...
   IF NOT cl_null(l_msg) THEN  
   	  CALL s_showmsg_init() 
      CALL s_errmsg('fbd01',l_msg,'','afa-097',1)   
      LET g_success = 'N' 
      CALL s_showmsg() 
      RETURN     
   END IF    
