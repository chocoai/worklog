...
DEFINE  g_hrat     DYNAMIC ARRAY OF RECORD
           ...
           hrat04   LIKE  type_file.chr50, #modify by wangwy 2015-09-02
           hrat05   LIKE  type_file.chr50, #modify by wangwy 2015-09-02
           hrat25   LIKE  hrat_file.hrat25,
           hrat19   LIKE  type_file.chr50, #modify by wangwy 2015-09-02
           beiz     LIKE  type_file.chr100
             END  RECORD
       ...
       AFTER FIELD hrat01
            IF NOT cl_null(g_hrat[l_ac].hrat01) THEN
               IF g_hrat[l_ac].hrat01 != g_hrat_t.hrat01 OR
                  ...
                  END IF 
                  #年假显示时间
                  SELECT hratid INTO l_id FROM hrat_file WHERE hrat01=g_hrat[l_ac].hrat01
                  SELECT NVL(d.hrdq06,NVL(e.hrdq06,NVL(f.hrdq06,g.hrdq06)))
                      INTO l_hrcp04
                      FROM hrat_file a
                      LEFT JOIN hrcp_file b ON b.hrcp02 = a.hratid AND b.hrcp03 = g_hrcd.hrcd02
                      LEFT JOIN hrcb_file c ON c.hrcb05 = a.hratid AND g_hrcd.hrcd02 BETWEEN c.hrcb06 AND c.hrcb07
                      LEFT JOIN hrdq_file d ON d.hrdq03 = a.hratid AND d.hrdq05 = g_hrcd.hrcd02 AND d.hrdq02=1
                      LEFT JOIN hrdq_file e ON e.hrdq03 = c.hrcb01 AND e.hrdq05 = g_hrcd.hrcd02 AND e.hrdq02=4
                      LEFT JOIN hrdq_file f ON f.hrdq03 = a.hrat03 AND f.hrdq05 = g_hrcd.hrcd02 AND f.hrdq02=3
                      LEFT JOIN hrdq_file g ON g.hrdq03 = a.hrat04 AND g.hrdq05 = g_hrcd.hrcd02 AND g.hrdq02=2
                      WHERE a.hratid = l_id
                  SELECT hrbo03 INTO l_hrbo03 FROM hrbo_file WHERE hrbo02=l_hrcp04 AND hrbo01='0000'
                  LET l_str = l_hrbo03
                  LET l_sql_1="SELECT distinct hrby06 FROM hrby_file WHERE hrby05=to_date('",g_hrcd.hrcd02,"','yy/mm/dd') AND hrby09='",l_id,"' AND hrby12='1' AND hrby10='1' order by hrby06"
                  PREPARE i044_qk FROM l_sql_1
                  DECLARE i044_js1 CURSOR FOR i044_qk
                  FOREACH i044_js1 INTO l_hrby06
                  LET l_str = l_str,",",l_hrby06
                  END FOREACH 
                  LET g_hrat[l_ac].beiz = l_str
                  DISPLAY g_hrat[l_ac].beiz TO beiz
                  #end
               END IF
            END IF    
              ...
              CASE 
                WHEN INFIELD(hrat01)
                   IF p_cmd = 'a' THEN 
                    ...
                    # CALL cq_hrat1(TRUE,TRUE,g_hrat[l_ac].hrat01,g_hrat[l_ac].hrat02,g_hrat[l_ac].hrat04,g_hrat[l_ac].hrat05,g_hrat[l_ac].hrat25,g_hrat[l_ac].hrat19,'N')
                     #      RETURNING g_hrat[l_ac].hrat01,g_hrat[l_ac].hrat02,g_hrat[l_ac].hrat04,g_hrat[l_ac].hrat05,
                     #               g_hrat[l_ac].hrat25,g_hrat[l_ac].hrat19      #addbywangww180119
                   ELSE 
                      CALL cl_init_qry_var()
                      LET g_qryparam.form = "q_hrat"
...
FUNCTION sghri044_p3_set_b_no_entry(p_cmd)
  ...
  CALL cl_set_comp_entry("hrat02,hrat04,hrat05,hrat25,hrat19,beiz",FALSE) 
END FUNCTION 
...
FUNCTION sghri044_p3_hrat01(p_cmd)
 DEFINE p_cmd         LIKE type_file.chr1
   ...
   DEFINE l_hrat04    LIKE type_file.chr50 #modify by wangwy 2015-09-02
   DEFINE l_hrat05    LIKE type_file.chr50 #modify by wangwy 2015-09-02
   DEFINE l_hrat25    LIKE hrat_file.hrat25
   DEFINE l_hrat19    LIKE type_file.chr50 #modify by wangwy 2015-09-02
   ...
   SELECT hrat02,hrat04,hrat05,hrat25,hrat19,hratconf 
    ...
    WHERE hrat01=g_hrat[l_ac].hrat01
          AND hrat19<>'3001'    #addbywangww180103
    CASE
...
#130912 add by wangxh--end--
  ...
  DEFINE l_cp04    LIKE hrcp_file.hrcp04
  DEFINE l_hrbo06  LIKE hrbo_file.hrbo06
   ...
   FOR l_n = 0 TO l_end STEP 1
      ...
      CALL sghri044_p3_bc(p_hrcd.hrcd09,l_day) RETURNING l_cp04   #add by wangwy 20170106 --> 调用取班次函数取得班次
      SELECT hrbo06 INTO l_hrbo06 FROM hrbo_file WHERE hrbo02 = l_cp04
      IF l_hrbo06 = 'Y' THEN 
        CONTINUE FOR 
      END IF 
      ...
      ELSE   
          ...
          FOR i = 1 TO l_hrch.getLength()
             IF l_hrch[i].hrch19 IS NOT NULL THEN 
                ...
                IF l_hrch[i].l_day_unit='003' AND l_hrch[i].l_day_umf=11 THEN 
                   LET l_hrch[i].l_day_umf=8
                END IF 
                IF l_hrch[i].l_day_unit='003' AND l_hrch[i].l_day_umf=5.5 THEN 
                   LET l_hrch[i].l_day_umf=4
                END IF 
...
END FUNCTION
FUNCTION sghri044_p3_bc(p_hratid,p_date)    #add by wangwy 20170106  增加班次判断 用于年假跳过休息班次扣减
DEFINE p_hratid       LIKE hrat_file.hratid
DEFINE p_date         LIKE hrcda_file.hrcda04
...
DEFINE l_hrcp04       LIKE hrcp_file.hrcp04
   ...
   IF l_hrcp07 = 'Y' THEN 
      SELECT hrcp04 INTO l_hrcp04 FROM hrcp_file
       WHERE hrcp02=p_hratid AND hrcp03=p_date
   ELSE 
      SELECT NVL(d.hrdq06,NVL(e.hrdq06,NVL(f.hrdq06,g.hrdq06))) INTO l_hrcp04
          FROM hrat_file a
          LEFT JOIN hrcp_file b ON b.hrcp02 = a.hratid AND b.hrcp03 = p_date
          LEFT JOIN hrcb_file c ON c.hrcb05 = a.hratid AND p_date BETWEEN c.hrcb06 AND c.hrcb07
          LEFT JOIN hrdq_file d ON d.hrdq03 = a.hratid AND d.hrdq05 = p_date AND d.hrdq02=1
          LEFT JOIN hrdq_file e ON e.hrdq03 = c.hrcb01 AND e.hrdq05 = p_date AND e.hrdq02=4
          LEFT JOIN hrdq_file f ON f.hrdq03 = a.hrat03 AND f.hrdq05 = p_date AND f.hrdq02=3
          LEFT JOIN hrdq_file g ON g.hrdq03 = a.hrat04 AND g.hrdq05 = p_date AND g.hrdq02=2
          WHERE a.hratid = p_hratid
   END IF 	  
   RETURN l_hrcp04 
END FUNCTION 
