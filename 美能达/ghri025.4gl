...
DEFINE g_hrat01               LIKE type_file.chr1000           #须要离职结清的员工工号
...
FUNCTION i025_curs()
  ...
  IF g_argv1 IS NULL THEN
    CONSTRUCT BY NAME g_wc ON
        hrbh01,hrbh02,hrbh03,hrbh04,hrbh05,hrbh06,
        ...
        ON ACTION controlp
           CASE
              WHEN INFIELD(hrbh01)
                 ...
                 LET g_qryparam.form = "q_hrat02_003"
    ...
    WHILE TRUE
        ...
        SELECT hratid INTO l_hrbh.hrbh01 FROM hrat_file
         WHERE (hrat01=g_hrbh.hrbh01 or hratid=g_hrbh.hrbh01)
           AND hratacti='Y'
           AND hratconf='Y'
           AND (hrat77=to_date('9999/12/31','yy/mm/dd') or hrat77 is null ) #addbywangww171106
...
FUNCTION i025_i(p_cmd)
   ...
   DEFINE l_sql         STRING
   DEFINE l_hrat42_t         LIKE hrat_file.hrat42   # add by lixwz 180504
   DEFINE l_hrat42_desc_t         LIKE hrai_file.hrai04   # add by lixwz 180504
      ...
      AFTER FIELD hrbh01
         IF NOT cl_null(g_hrbh.hrbh01) THEN
          ...
          #add by yinbq 201411 考虑到同一员工可以二次入职直至多次离职，移除员工唯一性校验
            ...
            # add by lixwz180504
            # 优先取ghri016
            SELECT hraz12,hrai04 INTO l_hrat42_t,l_hrat42_desc_t FROM
                  ( SELECT hraz12 FROM hraz_file WHERE hraz32 = g_hrbh.hrbh01 AND hraz23 < g_today   ORDER BY hraz05 DESC)
                  LEFT JOIN hrai_file h ON hrai03=hraz12
                  WHERE rownum = 1
            IF NOT cl_null(l_hrat42_t) THEN LET l_hrat42 = l_hrat42_t END IF
            IF NOT cl_null(l_hrat42_desc_t) THEN LET l_hrat42_desc = l_hrat42_desc_t END IF
            # add by lixwz180504
 ...
 AFTER FIELD hrbh04
         IF NOT cl_null(g_hrbh.hrbh04) AND NOT cl_null(g_hrbh.hrbh03) THEN
          IF g_hrbh.hrbh04<g_hrbh.hrbh03 THEN
            ...
            NEXT FIELD hrbh04
        ELSE
          DISPLAY g_hrbh.hrbh04-1 TO hrbh11   #xie150414
          END IF
      ...
      AFTER FIELD hrbh05
         IF g_hrbh.hrbh05 IS NOT NULL THEN
            ...
            DISPLAY g_hrbh.hrbh04-1 TO hrbh11
         END IF
...
DEFINE l_hrat42_t       LIKE hrat_file.hrat42   # add by lixwz 180504
DEFINE l_hrat42_desc_t  LIKE hrai_file.hrai04
...
#    #姓名,部门编号,职位编号,直接主管,性别,学历,员工状态,成本中心,入职日期
#    SELECT hrat02,hrat04,hrat05,hrat06,hrat17,hrat22,hrat19,hrat42,hrat66
#      INTO l_hrat02,l_hrat04,l_hrat05,l_hrat06,l_hrat17,l_hrat22,l_hrat19,
#           l_hrat42,l_hrat66
#      FROM hrat_file
...
#
#    #部门名称
#    SELECT hrao02 INTO l_hrat04_desc FROM hrao_file WHERE hrao01=l_hrat04
#    #职位编号
#    SELECT hrap06 INTO l_hrat05_desc FROM hrap_file WHERE hrap01=l_hrat04 AND hrap05=l_hrat05
#    #直接主管名称
#    SELECT hrat02 INTO l_hrat06_desc FROM hrat_file WHERE hrat01=l_hrat06
#    #性别
#    SELECT hrag07 INTO l_hrat17_desc FROM hrag_file WHERE hrag01='333' AND hrag06=l_hrat17
#    #学历
#    SELECT hrag07 INTO l_hrat22_desc FROM hrag_file WHERE hrag01='317' AND hrag06=l_hrat22
#    #员工状态
##    SELECT hrad03 INTO l_hrat19_desc FROM hrad_file WHERE hrad02=l_hrat19
#    #成本中心
#    SELECT hrai04 INTO l_hrat42_desc FROM hrai_file WHERE hrai03=l_hrat42
#
#    DISPLAY l_hrat02 TO hrat02
#    DISPLAY l_hrat04 TO hrat04
#    DISPLAY l_hrat04_desc TO hrat04_desc
...
DEFINE l_hratid    LIKE   hrat_file.hratid
    ...
    IF cl_delete() THEN
       ...
       LET g_doc.value1 = g_hrbh.hrbh01
       CALL cl_del_doc()
       DELETE FROM hrbh_file WHERE hrbh01 = l_hratid AND hrbh03 = g_hrbh.hrbh03
...
#不只失效合同，协议同时失效
#mark by wangwy 2015-10-21客户不需要更改合同信息
...
#不只失效合同，协议同时失效
      ...
      UPDATE hrbh_file
       ...
       IF SQLCA.SQLCODE OR SQLCA.SQLERRD[3] = 0 THEN
          ...
          RETURN
       ELSE
       	  UPDATE hrbw_file SET hrbw06=g_hrbh.hrbh11,hrbwud02='2' WHERE hrbw01=l_hratid   #add by wangwy 20150526
          DISPLAY BY NAME g_hrbh.hrbhconf,g_hrbh.hrbhmodu,g_hrbh.hrbhdate
       END IF
   END IF
   CLOSE i025_cl
...
FUNCTION i025_undo_confirm()
   ...
   ELSE
      #回写员工信息
      UPDATE hrat_file SET hrat19=g_hrbh.hrbh12,hrat77 = '' WHERE hratid=l_hratid
      IF SQLCA.SQLCODE THEN
          CALL cl_err('upd hrat:',SQLCA.SQLCODE,0)
          RETURN
      END IF
      ...
      UPDATE hrbh_file
       ...
       IF SQLCA.SQLCODE OR SQLCA.SQLERRD[3] = 0 THEN
          ...
          RETURN
       ELSE
       	  UPDATE hrbw_file SET hrbw06 = to_date('2099/12/31','YYYY/MM/DD'),hrbwud02='2' WHERE hrbw01=l_hratid     #add by wangwy 20150526
          DISPLAY BY NAME g_hrbh.hrbhconf,g_hrbh.hrbhmodu,g_hrbh.hrbhdate
       END IF
   END IF
   CLOSE i025_cl
...
DEFINE l_cnt LIKE type_file.num5
   ...
   LET g_sql = "SELECT COUNT(*) INTO l_cnt FROM hrbh_file LEFT JOIN hrat_file on hratid=hrbh01 WHERE hrat01 IN ('",g_hrat01,"') AND hrbhconf = '1'"
   ...
   ELSE
      ...
      LET g_sql = "UPDATE hrbh_file SET hrbh09 = 'Y' WHERE EXISTS(SELECT 1 FROM HRAT_FILE WHERE HRAT01 IN (",g_hrat01,") AND HRATID=HRBH01)"
