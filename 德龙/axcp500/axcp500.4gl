...
#FUN-A50075--MOD--END
    ...
    #增加條件判斷計算非[銷退則讓][倉退則讓]才時排除數量為0的tlf
     IF q_tlf.tlf13 != 'aomt800' AND (q_tlf.tlf13 NOT MATCHES 'apmt107*' ) AND q_tlf.tlf13 != 'apmt150'   #160711 add apmt150 by tangyh
...
#MOD-B30205 ------------------------------End----------------------------------
          ...
          #WHEN q_tlf.tlf13 = 'aimt324'     #倉庫調撥  #TQC-B30129
           ...
           WHEN q_tlf.tlf13 = 'aomt800'     #銷退入庫
                ...
                # add by lixwz180117 s
                # 都抓不到成本则抓取入库（销退除外）的平均成本
                IF l_ccc23a = 0 AND l_ccc23b = 0 AND l_ccc23c = 0 AND
                   l_ccc23d = 0 AND l_ccc23e = 0 AND l_ccc23f = 0 AND
                   l_ccc23g = 0 AND l_ccc23h = 0 THEN
                   CALL p500_get_avg_in() RETURNING l_ccc23a,l_ccc23b,l_ccc23c,l_ccc23d,
                                                 l_ccc23e,l_ccc23f,l_ccc23g,l_ccc23h
                END IF
                # add by lixwz180117 e
...
FUNCTION p500_ccg5_cost()      # 加上WIP入庫金額
   DEFINE l_ccu       RECORD LIKE ccu_file.*
   DEFINE l_sql       STRING   #add by pane 160603

   INITIALIZE l_ccu.* TO NULL
 #mark by pane 160603 begin#
 # SELECT SUM(ccu31 *-1),
 ...
 #mark by pane 160603 end#
 #add by pane 160603 begin#
   LET l_sql = " SELECT ccu01,SUM(ccu31*-1),SUM(ccu32 *-1),SUM(ccu32a*-1),SUM(ccu32b*-1),",
               "  SUM(ccu32c*-1),SUM(ccu32d*-1),SUM(ccu32e*-1),SUM(ccu32f*-1),SUM(ccu32g*-1),SUM(ccu32h*-1) ",
               " FROM ccu_file,sfb_file ",
               " WHERE ccu04 = '",g_ima01,"' AND ccu02 =",yy," AND ccu03 = ",mm," ",
               " AND ccu06 = '",type,"' AND ccu07 = '",g_tlfcost,"'",
               " AND ccu01 =sfb01 AND sfb02='11' AND sfb99 = 'Y' ",
               " GROUP BY ccu01"
   PREPARE p500_ccu32 FROM l_sql
   DECLARE p500_ccu32_sum CURSOR FOR p500_ccu32
   FOREACH p500_ccu32_sum INTO l_ccu.ccu01,l_ccu.ccu31,l_ccu.ccu32, l_ccu.ccu32a,l_ccu.ccu32b,
                               l_ccu.ccu32c,l_ccu.ccu32d,l_ccu.ccu32e,l_ccu.ccu32f,l_ccu.ccu32g,l_ccu.ccu32h
 #add by pane 160603 end#
      ...
      CALL cl_digcut(l_ccu.ccu32,g_ccz.ccz26) RETURNING l_ccu.ccu32 #add by wy20160407
      LET g_ccc.ccc32 =g_ccc.ccc32 +l_ccu.ccu32 #本月轉出金額(a+b+c+d+e+f+g+h) #mark by wy20160407 拆件工单ccu32保留3位小数,ccc32保留两位小数,ccc32就有一个尾差
   END FOREACH    #add by pane 160603
END FUNCTION
...
DEFINE l_cnn       LIKE type_file.num5#guorui161102

#guorui161102 str--
   SELECT count(1) INTO l_cnn FROM cdl_file
                       WHERE cdl02=yy AND cld03=mm AND cdl04= type
                         AND (cdl06='1423' OR cld06='1424')
   IF l_cnn>0 THEN

      SELECT count(1) INTO l_cnn FROM ccb_file
       WHERE ccb23=5 AND ccb02=yy AND ccb03=mm AND ccb06=type
         AND ccb01 in(SELECT DISTINCT cld07 FROM cdl_file
                       WHERE cdl02=yy AND cld03=mm AND cdl04= type
                         AND (cdl06='1423' OR cld06='1424'))
       IF l_cnn <= 0 THEN

         CALL cl_err('','axc-500',1)
         LET g_success ='N'
         RETURN
...
DEFINE l_ccc23a    LIKE ccc_file.ccc23a,
       ...
       l_yy        LIKE ccz_file.ccz01,   #No.FUN-680122SMALLINT, #TQC-790122
       l_mm        LIKE ccz_file.ccz02,
       l_sql       STRING,
       l_cnt       LIKE type_file.num5,
  ...
  LET l_yy = yy
  LET l_mm = mm
  SELECT sum(ccg31) INTO l_ccg31
   FROM ccg_file
  WHERE ccg04 = g_ima01
    AND ccg02 = l_yy
    AND ccg03 = l_mm
  IF cl_null(l_ccg31) AND l_ccg31 <> 0 THEN

    SELECT sum(ccg32a),sum(ccg32a),sum(ccg32a),sum(ccg32a),sum(ccg32a)
      INTO l_ccc23a,l_ccc23b,l_ccc23c,l_ccc23d,l_ccc23e
      FROM ccg_file
     WHERE ccg04 = g_ima01
       AND ccg02 = l_yy
    ...
    LET l_ccc23a = l_ccc23a/l_ccg31
    LET l_ccc23b = l_ccc23b/l_ccg31
    LET l_ccc23c = l_ccc23c/l_ccg31
    LET l_ccc23d = l_ccc23d/l_ccg31
    LET l_ccc23e = l_ccc23e/l_ccg31
    LET l_ccc23f = 0
    LET l_ccc23g = 0
    LET l_ccc23h = 0
  ...
  #LET l_sql = "SELECT (ccc221+ccc222+ccc223+ccc224+ccc225) / amt,",
  #            "(ccc22a1+ccc22a2+ccc22a3+ccc22a4+ccc22a5)/ amt,",
  #            "(ccc22b1+ccc22b2+ccc22b3+ccc22b4+ccc22b5)/ amt,",
  #            "(ccc22c1+ccc22c2+ccc22c3+ccc22c4+ccc22a5)/ amt,",
  #            "(ccc22c1+ccc22c2+ccc22c3+ccc22c4+ccc22c5)/ amt,",
  #            "(ccc22d1+ccc22d2+ccc22d3+ccc22d4+ccc22d5)/ amt,",
  #            "(ccc22e1+ccc22e2+ccc22e3+ccc22e4+ccc22e5)/ amt,",
  #            "(ccc22f1+ccc22f2+ccc22f3+ccc22f4+ccc22f5)/ amt,",
  #            "(ccc22g1+ccc22g2+ccc22g3+ccc22g4+ccc22g5)/ amt,",
  #            "(ccc22h1+ccc22h2+ccc22h3+ccc22h4+ccc22h5)/ amt",
  #            " FROM ( SELECT ccc_file.*,CASE WHEN (CCC211+CCC212+CCC213+CCC214+CCC215)=0 THEN 1 ",
  #            " ELSE (CCC211+CCC212+CCC213+CCC214+CCC215) END amt FROM ccc_file WHERE ccc01 = '",
  #            g_ima01 CLIPPED,"' AND ccc02 = ? AND ccc03 = ? )"
  #PREPARE p500_get_avg_in FROM l_sql
  #DECLARE p500_get_avg CURSOR FOR p500_get_avg_in
#
  #FOR l_cnt = 1 TO 12
  #    EXECUTE p500_get_avg USING l_yy,l_mm INTO l_ccc23a,l_ccc23b,l_ccc23c,l_ccc23d,l_ccc23e,
  #                                              l_ccc23f,l_ccc23g,l_ccc23h
  #    IF cl_null(l_ccc23a) THEN LET l_ccc23a = 0 END IF
  #    IF cl_null(l_ccc23b) THEN LET l_ccc23b = 0 END IF
  #    IF cl_null(l_ccc23c) THEN LET l_ccc23c = 0 END IF
  ...
  #    IF (l_ccc23a = 0 AND l_ccc23b = 0 AND l_ccc23c = 0 AND
  #                 l_ccc23d = 0 AND l_ccc23e = 0 AND l_ccc23f = 0 AND
  #                 l_ccc23g = 0 AND l_ccc23h = 0) OR SQLCA.sqlcode THEN
  #        IF l_mm = 1 THEN
  #            LET l_mm = 12
  #            LET l_yy = l_yy -1
  #        ELSE
  ...
  RETURN l_ccc23a,l_ccc23b,l_ccc23c,l_ccc23d,l_ccc23e,
         l_ccc23f,l_ccc23g,l_ccc23h
END FUNCTION
# add by lixwz180117 e