     ...
     #LET l_sql = " UPDATE ima_file SET ima57=?,ima905=? ",               #FUN-C30315 mark
      ...
      IF g_bgjob = "N" THEN
         ...
         IF cl_sure(18,20) THEN 
           ...
           #CHI-A90001---add---start---
            IF  g_success = 'Y' THEN 
                #add by liudong 150105
                LET l_sql = "MERGE INTO ima_file A ",
                            " USING (SELECT ima01,ima57,ima905 FROM ima270_tmp) B ",
                            "   ON (A.ima01=B.ima01) ",
                            " WHEN MATCHED THEN UPDATE SET A.ima57=B.ima57,A.ima905=B.ima905,A.imadate='",g_today,"'",
                             "                    WHERE A.ima01=B.ima01 "
                PREPARE mer_ge_ima57 FROM l_sql
                EXECUTE mer_ge_ima57 
                IF  SQLCA.sqlcode OR SQLCA.sqlerrd[3] = 0 THEN
                    LET g_msg = 'merge ima_file err '
                    CALL cl_err(g_msg,sqlca.sqlcode,1)
                    LET g_success = 'N'
                END IF
                #ead by liudong 150105
                #mak by liudong 150105
                {
        ...
        #CHI-A90001---add---start---
         IF  g_success = 'Y' THEN 
                ...
                LET l_sql = "MERGE INTO ima_file A ",
                            ...
                            " WHEN MATCHED THEN UPDATE SET A.ima57=B.ima57,A.ima905=B.ima905,A.imadate='",g_today,"'",
                             "                    WHERE A.ima01=B.ima01 "
                PREPARE mer_ge1_ima57 FROM l_sql
                EXECUTE mer_ge1_ima57 
                IF  SQLCA.sqlcode OR SQLCA.sqlerrd[3] = 0 THEN
                    LET g_msg = 'merge ima_file err '
                    CALL cl_err(g_msg,sqlca.sqlcode,1)
                    LET g_success = 'N'
...
FUNCTION p270_p()
   ...
   END IF
     ...
     #CHI-A90001---add---start---
      ...
      INSERT INTO ima270_tmp
      SELECT ima01,ima08,ima16,ima57,ima80,ima905 FROM ima_file
     #CHI-A90001---add---end---
